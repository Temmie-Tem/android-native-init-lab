...
- **Galaxy S22+ (`SM-S906N`, FYG8)**: 인증된 native-PID1 USB 경로는 P348까지
  ...
  작업은 인증된 루트 명령 콘솔과 PID 1이 직접 그리는 네이티브 상태 HUD를 더했고,
  HUD는 현재 uptime, 콘솔 상태, 메모리, 종합 CPU 사용률, 배터리 용량·충전 상태·온도를
  싣습니다. 이어서 P384는 설치·인증된 건강·기한 내 정확한 Download·새 커널 부팅
  신원 위의 same-N 복원·정확한 Android 정리로 이어지는 완전한 입회 네이티브 왕복을
  닫았고, P385는 복원 가능한 첫 네이티브 기준 이미지를 승인했습니다. 그래도 부팅당
  8회 인증·900초 한계는 그대로입니다. 운영자는 그 출력을 패널에서 관측했으며 기계적 픽셀 증명은 주장하지
  않습니다. 상주 설치나 반복 부팅·장시간 운용의 주장도 아닙니다. 정확한 현재 기능
  버전과 활성 bounded unit은 [`GOAL.md`](GOAL.md)를 정본으로 관리합니다. P349의
...
이 요약의 S22+ 항목은 2026-09-12, 다른 대상은 2026-09-05 확인한 기록 기준입니다. 기기별 페이지에서 인정된 결과의
