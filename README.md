...
- active research: native Wi-Fi bring-up — cycle V1253 (PMIC power-surface write-gate for SDX50M power-up); see `CLAUDE.md` and `docs/plans/NATIVE_INIT_NEXT_WORK_2026-04-25.md`
...
- SDX50M eSoC power-up gate — `pm-service`가 `/dev/subsys_esoc0`를 열어
  `mdm_subsys_powerup`까지 진입하지만 MDM3가 `OFFLINING`에 머무는 원인 규명.
  V1251이 재현 후보 입증: PM8150L soft-reset GPIO9 `MUX UNCLAIMED`, PCIe GDSC
  `0mV`. V1253은 bounded PMIC power-surface write-gate helper(v261)로 진행 중
...
전체 문서 목록과 읽는 순서, 사이클별 리포트(v40–V1253+)는 `docs/README.md`를
