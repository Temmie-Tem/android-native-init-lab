...
- Wi-Fi lifecycle research: v203-v220에서 Android/TWRP/native Wi-Fi evidence, vendor firmware path, ICNSS reprobe safety stop, ICNSS/CNSS lifecycle map, Android service replay model, ICNSS debug/recovery inventory, CNSS daemon dry-run model, native Android-env shim matrix, lifecycle-aware preflight gate v2를 수집했고 v220 gate decision은 `no-go`이므로 active scan/connect는 계속 차단
