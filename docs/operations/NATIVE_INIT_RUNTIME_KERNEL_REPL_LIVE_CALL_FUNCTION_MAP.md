...
- Bitmap helper sweep: `__bitmap_weight`, `__bitmap_subset`, `find_next_bit`, `find_next_zero_bit`, and
  ...
  boundary, and full-size popcount cases. `__bitmap_subset` covers zero-size true, empty-source true,
  low-tail/first-word/second-word positive subset cases, a missing bit-90 negative case, full-size
  partial-mask negative, and full-size full-mask positive. `find_next_bit` covers low-word set hit,
