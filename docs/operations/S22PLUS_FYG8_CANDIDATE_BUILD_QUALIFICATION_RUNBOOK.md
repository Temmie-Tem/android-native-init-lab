...
The host guard must also prove its actual lifetime independently of that budget.
...
in the real isolated helper, not only with mocked child return codes.
...
| A scoped P2.92 static replay supplied explicit `--nm`/`--objdump` arguments but failed in 3.57 seconds because `aarch64-linux-gnu-nm` was absent from PATH | The top-level linked audit consumes the explicit paths, while the fresh userspace sub-audit independently resolves the GNU tools by basename | Preserve the fail-closed output and obey the attempt's stop condition; do not invoke promotion or silently retry with a corrected shell | Before consuming a bounded static attempt, validate the complete process environment: prepend the pinned private GNU-binutils directory to PATH, check every basename used by nested audits, and separately verify the explicit tool paths |
...
| P2.80 A/B were byte-identical, but the linked audit compared a logical five-byte descriptor encoding with the AArch64 six-byte struct-array storage | The kernel table is correct; `u16,u8,u8,u8` has one zero tail-padding byte per entry. This is a post-build verifier ABI-layout defect, not a compile/link defect | Preserve both immutable bundles. Correct only the versioned linked adapter, verify every physical byte and zero pad, normalize to the unchanged logical contract, and rerun the GNU audit | For every linked C struct table, distinguish logical field encoding from target-ABI storage. Mutation-test packed storage, nonzero padding, and field changes; never rebuild unless a frozen kernel input changed |
| The independent candidate checker reran the common linked audit directly and bypassed the registered P2.80 adapter; it then restored the dynamic ELF entrypoint override before nested effective-rootfs validation | Selection existed in the linked audit but not at the checker call boundary, and the versioned entrypoint context covered construction but not its nested validator | Select the registered adapter by exact source-contract ID, compare its fresh result byte-for-byte with the frozen receipt, and keep the exact userspace-derived entrypoint context across the entire nested audit | Test adapter selection, wrong-contract rejection, historical fallback, context lifetime, and restoration. A downstream verifier repair must not alter source-bound candidate inputs |
| Process v2 promotion supplied the full 60-module P2.80 closure directly to the isolated P2.42 generic-rootfs audit | The effective-rootfs path already applies the version chain `P2.57 full -> P2.53 legacy -> P2.42 legacy`; the independent AP path omitted both views | Preserve and validate the full 60-module closure, derive the exact two-step legacy view only for generic-rootfs replay, and compare the replay with the already-verified effective-rootfs result | Every versioned closure must test both full-rootfs and generic-rootfs consumers. Reuse the established view chain; do not hand-edit counts or rebuild an unchanged candidate |
