# S22+ FYG8 R4W1-B Connected Read-Only Requalification Clause

Status: inert host-only draft. After independent binding review, replace the
entire existing R4W1-B connected clause in `AGENTS.md` (from its current
`Pending exception` header through the paragraph immediately before the R4W1-A
stream-only exception) with the exact fenced block below. Never append this
block to the old clause: the replacement must remove the former standalone
connected ACTIVE sentinel. This file authorizes no device contact.
