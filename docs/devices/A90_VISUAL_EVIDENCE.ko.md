...
그리고 그 화면들이 방금 올린 바로 그 커널 위에서 네이티브 init이 인수합니다. 세 항목이
화면에서 해결됩니다 — `SD`가 `MOUNT/RW OK`에서 `RW TEST OK`로, `STORAGE`가
`CACHE FALLBACK`에서 `SD MAIN READY`로, `SERIAL`이 `USB ACM STARTING`에서
`TTYGS0 READY`로. 클립은 HUD가 올라와 `BOOT OK WIFI-V641- 4.8S`, 5,375 MB 중 276 MB,
실시간 로그 꼬리를 보고하는 지점에서 끝납니다. 빌드 `0.12.008 / H41-BADAPPLE-VIDEO-DEMO-V2`.
...
**증거 경계** — 한 번의 부팅에서 발췌해 초당 8프레임으로 재샘플링하고 용량을 위해
디노이즈했습니다. 이는 **그 녹화의 두 구간을 이어붙인 것**입니다. 콘솔이
`RUNTIME HUD MENU LOADING`에서 변화 없이 머무른 약 2.4초를 제거했으므로, 화면상의 경과
시간은 실제 시간이 아니며 이 클립은 타이밍이 아니라 브링업 순서를 보여줍니다. 두 구간
...
TWRP `System` 재부팅과 클립에서 제거된 정지 구간을 포함한 전체 테이크가 담겨 있어, 여기
설명한 이어붙임을 대조해 확인할 수 있습니다. 부팅 직전 기기는 동작 중인 Android 세션이
아니라 TWRP에 있었습니다.
