...
제어로 방향을 틀었으나 그쪽은 미증명으로 남아 있습니다. P375 이후의 작업은 인증된
루트 명령 콘솔과 PID 1이 직접 그리는 네이티브 상태 HUD를 더했습니다.
P349 RAM 작업공간 단위는
...
네이티브 PID 1이 패널에 자기 상태 텍스트를 그리는 과정을 한 번에 이어서 녹화한
영상들은 자체 페이지에 모여 있습니다. P376 부팅 HUD, 그다음 P384 게이지 HUD와
Download 전환입니다:
...
P376 이후의 작업은 HUD를 고정 시스템 상태 수집까지 확장했습니다. uptime, 콘솔 상태,
...
버전 이름을 붙이며, 그 규칙이 각 버전을 변경되지 않은 산출물과 역사적 P 번호에
대응시킵니다. **정확한 현재 기능 버전과 활성 bounded unit은 이 페이지가 아니라
[`GOAL.md`](../../GOAL.md)를 정본으로 관리합니다.** 버전 이름은 검증된 한정 범위를
기록할 뿐, 새 빌드나 상주 네이티브 설치가 아니며 안정성·반복 부팅·장시간 운용을
주장하지도 않습니다.
