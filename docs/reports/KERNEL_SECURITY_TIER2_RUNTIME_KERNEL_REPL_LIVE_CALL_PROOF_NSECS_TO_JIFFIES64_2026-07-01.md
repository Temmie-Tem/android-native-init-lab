# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: nsecs_to_jiffies64

- Date: 2026-07-01
- Decision: `a90-repl-live-call-proof-nsecs_to_jiffies64-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-nsecs-to-jiffies64-20260701/proof/a90_repl_evidence.json`

## Candidate Selection

This unit selected `nsecs_to_jiffies64` from the remaining time/jiffies scalar helpers. The adjacent
`nsec_to_clock_t` stayed parked because C1 identity verification failed for that symbol in the
current map/image pair. `nsecs_to_jiffies64` was chosen because it is scalar-only, C1-verified by
`export-recovery`, has no pre-call argument memory dereference, and its current-image body is a
compact auditable fixed-point conversion leaf.

The trusted contract is intentionally narrow: current-image scalar `u64` nanosecond inputs return
`(n * 0xd6bf94d5e57a42bd) >> 87`, equivalent to divide-by-10000000 for the fixed proof cases.

## Static Gate

Target:

- `nsecs_to_jiffies64`: `0xffffff80081585cc`
- Resolution method: `export-recovery`
- Direct BL xrefs: `1`
- Next symbol boundary: `nsecs_to_jiffies` at `+0x20`
- Source declaration: `include/linux/jiffies.h:453`,
...
- Static word checks:
  ...
  - `0xd65f03c0`: return.
  - `0x00be7bad`: next-entry guard before `nsecs_to_jiffies`.
...
PYTHONPATH=tests PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 -m unittest \
  ...
  tests.test_a90_repl.SelftestIntegrationTests.test_call_proof_nsecs_to_jiffies64_passes_with_fixed_point_contract
...
- CLI sweep: advisory `SAFE-SCALAR`, scalar-only source declaration selected from
  `include/linux/jiffies.h:453`, gate seeded and auto-call allowed only for this vetted target.
- Full `tests.test_a90_repl`: `Ran 153 tests`, `OK`.
- `git diff --check`: pass.
...
- A prompt echo was parsed as an unknown command during the helper transcript, but the helper's
  bounded `version/status` verification passed and the subsequent REPL selftest passed.
...
```sh
PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 \
  workspace/public/src/scripts/revalidation/a90_repl.py call-proof nsecs_to_jiffies64 \
  --map workspace/private/runs/kernel/v2c-c2b-kallsyms-padding-fix/System.map \
  --image workspace/private/inputs/boot_images/boot_linux_tier2_repl_v1_repl.img \
  --source-root workspace/private/inputs/kernel_source/SM-A908N_KOR_12_Opensource/Kernel \
  --evidence-dir workspace/private/runs/kernel/live-call-proof-nsecs-to-jiffies64-20260701/proof
```
...
| --- | --- | --- | --- |
| nsecs-to-jiffies64-div10000000-zero | `0x0` | `0x0` | `0x0` |
| nsecs-to-jiffies64-div10000000-sub-tick | `0x98967f` | `0x0` | `0x0` |
| nsecs-to-jiffies64-div10000000-one-tick | `0x989680` | `0x1` | `0x1` |
| nsecs-to-jiffies64-div10000000-mixed-123456789 | `0x75bcd15` | `0xc` | `0xc` |
| nsecs-to-jiffies64-div10000000-one-second | `0x3b9aca00` | `0x64` | `0x64` |
| nsecs-to-jiffies64-div10000000-max-u64 | `0xffffffffffffffff` | `0x1ad7f29abca` | `0x1ad7f29abca` |

Checks:

- `static-c1-identity`: OK, `nsecs_to_jiffies64` resolved by `export-recovery`.
- `static-next-symbol-boundary`: OK, next symbol `nsecs_to_jiffies` at `+0x20`.
- `static-source-contract`: OK, scalar-only `extern u64 nsecs_to_jiffies64(u64 n)` source
...
`nsecs_to_jiffies64` is trusted only under the current-image fixed-point divide-by-10000000 contract.
This does not authorize `nsec_to_clock_t`, `nsecs_to_jiffies`, alternate timer configurations where
the conversion body differs, or mass calling.
