...
| ② `get_stackid` 콜스택 | 관측 | **런타임 확정(V2193)** — stackid 반환 확인, stackmap value dump·심볼화는 미구현 | 다음 P1 |
...
따라서 현 시점의 합성 갭은 `current_task`에서 `task_struct` offset-chain을 읽는
작은 extractor다. raw stack address 회수와 KASLR slide 기반 심볼화는 stackmap
value dump를 추가한 뒤 판정한다.
...
- ① `get_current_task` 앵커는 V2193에서 capability 확정됨 → emitter 확장 →
  `task_struct` 최소 그래프(`pid/tgid/comm/cred uid/euid`) 관측.
- stackmap value dump 추가 → raw stack address 회수 여부 확인 → 커널 심볼맵 생성
  (소스 보유) → 콜스택 심볼화 파이프라인.
