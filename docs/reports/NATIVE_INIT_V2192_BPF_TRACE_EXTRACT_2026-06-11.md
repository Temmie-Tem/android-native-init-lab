...
| `bpf_get_current_task()` (helper 35) | **현재 실행 task_struct** | 후보(헬퍼 존재) |
...
| ① `get_current_task` 앵커 | 관측 | 낮음 — emitter에 deref base=`R0=task` 분기 + 플래그 | "확장→재빌드→관측" 그대로 |
| ② `get_stackid` 콜스택 | 관측 | 중 — STACK_TRACE map + 유저스페이스 stack덤프·심볼화 | 〃 |
| ③ `probe_write_user` | **제어** | 중 — 헬퍼 호출은 쉬우나 유저메모리 쓰기 | 별도 게이트 트랙 |
| ④ cgroup-BPF 정책 | **제어** | 높 — 다른 prog타입+다른 attach(cgroup fd) | 별도 도구 |

규율: 각 헬퍼는 **최소 capability probe(더미 prog 로드, attach 안 함)로 verifier
수용 먼저 확인** 후 emitter에 통합. 추천 순서 ① → ②, 그다음 ③④(별도 안전 결정).
...
- ① `get_current_task` 앵커 capability probe → emitter 확장 → task_struct 그래프 관측.
- 커널 심볼맵 생성(소스 보유) → 누출 포인터/콜스택 심볼화 파이프라인.
