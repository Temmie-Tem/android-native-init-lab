# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: find_next_zero_bit

- Date: 2026-06-30
- Decision: `a90-repl-live-call-proof-find_next_zero_bit-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-find-next-zero-bit-20260630/proof/a90_repl_evidence.json`

## Candidate Selection

This unit added the next proof target after the existing proof set had no unproven implemented
targets left. Advisory sweeps produced allocator-like `memblock_*` names, but those were rejected as
behavior-changing state helpers. `find_next_bit` remains parked because the current analyzer still
reports size/offset address-flow conservatively.

The selected target was `find_next_zero_bit`: it has a clean exported identity, leaf/no-BL shape,
and a source signature that declares only x0 as the pointer argument. The proof deliberately builds
an owned bitmap inside the kernel and bounds all size/offset arguments inside that allocation.

## Static Gate

Target:

- `find_next_zero_bit`: `0xffffff8008564e94`
...
- Source signature: `include/asm-generic/bitops/find.h:29`,
...
PYTHONPATH=tests PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 -m unittest \
  ...
  tests.test_a90_repl.SelftestIntegrationTests.test_call_proof_find_next_zero_bit_passes_with_owned_bitmap_contract
...
- `py_compile`: pass.
- CLI classify: `SAFE-WITH-VALID-PTR`, verified by `export-recovery`, direct-BL xrefs `120`,
...
- Full `tests.test_a90_repl`: `Ran 129 tests`, `OK`.
...
- First REPL selftest attempt hit a transient serial END-marker miss while setting
  `panic_on_oops`; the equivalent shell run then completed, and the retried REPL selftest returned
...
PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 \
  ...
  --evidence-dir workspace/private/runs/kernel/live-call-proof-find-next-zero-bit-20260630/proof \
  find_next_zero_bit
```
...
| --- | ---: | ---: | --- | --- |
| first-zero-low-word | 128 | 0 | `0x9` | `0x9` |
| skip-low-zero | 128 | 10 | `0x49` | `0x49` |
| no-zero-after-second | 128 | 74 | `0x80` | `0x80` |
| bounded-second-word-hit | 80 | 64 | `0x49` | `0x49` |
| bounded-tail-miss | 80 | 74 | `0x50` | `0x50` |

Checks:

- `static-c1-identity`: OK, `find_next_zero_bit` resolved by `export-recovery`.
...
- `kmalloc-owned-find-next-zero-bit-bitmap`: OK, owned kernel bitmap allocation returned sane lowmem.
- `owned-find-next-zero-bit-bitmap-poke-peek`: OK, 128-bit bitmap had zero bits at `9` and `73`.
- `find-next-zero-bit-case-table`: OK, all 5 calls returned expected bit indices.
- `find-next-zero-bit-bitmap-immutability`: OK, bitmap and canary stayed unchanged.
- `kfree-owned-find-next-zero-bit-bitmap`: OK.
...
`find_next_zero_bit` is now live-proven under an owned unsigned-long bitmap contract with scalar
size/offset bounded inside that bitmap. The proof confirms the intended exported helper was reached,
returned the expected first-zero or size result for low-word, high-word, full-size miss, and bounded
tail cases, did not modify the bitmap/canary, and cleaned up the owned allocation. This does not
authorize arbitrary bitmap pointers, unbounded sizes, negative or out-of-allocation offsets, broader
bitops state, or mass calling. The device was rolled back to clean v2321 with final `selftest fail=0`.
