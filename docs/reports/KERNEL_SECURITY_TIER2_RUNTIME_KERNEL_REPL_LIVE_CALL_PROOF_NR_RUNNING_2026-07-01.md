# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: nr_running

- Date: 2026-07-01
- Decision: `a90-repl-live-call-proof-nr_running-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-nr-running-20260701T045709Z/proof/a90_repl_evidence.json`
- Private timeline: `workspace/private/runs/kernel/live-call-proof-nr-running-20260701T045709Z/timeline.json`

## Target

`nr_running(void)` was selected from the previously aborted scheduler-counter
batch as a fresh one-target proof. The earlier batch attempts recorded
`nr_running` as `not called`, and later one-target recovery runs promoted only
`nr_context_switches` and `nr_processes`. This run calls only the runnable-count
getter through the checked `call-proof` CLI.

Trusted contract:

- No arguments.
- The function reads scheduler runnable counters and returns an `unsigned long`.
...
- Address: `nr_running=0xffffff80080edebc`.
- Resolution: `disasm-signature+xref+map`, C1 verified.
- Source declaration: `extern unsigned long nr_running(void)` at
  `include/linux/sched/stat.h:19`.
- ABI: no pointer arguments.
- C1 safety tier: `SAFE-SCALAR`, no required pointer args.
- Direct BL xrefs: `4`.
- Next-symbol boundary: `single_task_running` at `+0xa0`.
- Static word checks pinned all 40 words, including the `cpumask_next` loop
  over possible CPUs and the per-CPU runnable-count load/add sequence.
- Same classifier run left `nr_iowait` as a safe but uncalled/unpromoted
  same-shape neighbor; `nr_iowait_cpu`, `single_task_running`, and
...
Both reads were positive, inside the sane count range, and stable across the
...
- Final explicit health first hit serial framing noise on `version`, then
  `hide` plus a `12s`-settled retry passed.
...
| Phase | Elapsed |
| --- | ---: |
| candidate flash helper total | `64.761s` |
| candidate flash start to boot ready | `65s` |
| candidate explicit health initial | `11s` |
| candidate explicit health retry | `1s` |
| live call-proof | `6s` |
| post-proof candidate health | `1s` |
| rollback flash helper total | `64.784s` |
| rollback flash start to boot ready | `64s` |
| final health initial | `11s` |
| final health retry | `2s` |
| candidate start to final health done | `252s` |
...
- Focused result: `Ran 2 tests in 17.925s`, `OK`.
- Static classifier:
  - `nr_running`: `SAFE-SCALAR`, `disasm-signature+xref+map`,
    `0xffffff80080edebc`.
  - `nr_iowait`: `SAFE-SCALAR`, same-shape neighbor but not called or promoted.
  - `nr_iowait_cpu`, `single_task_running`, `si_swapinfo`: `DENY`.

## Function Map Entry

`nr_running` is live-proven under exactly this contract:

- No arguments.
- The return value is a sane positive scheduler runnable count in this proof
  environment.
