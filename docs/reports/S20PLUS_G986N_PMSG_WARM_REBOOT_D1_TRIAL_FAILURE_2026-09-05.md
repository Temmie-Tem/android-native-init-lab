...
HOST SCRIPT DEFECT PROVEN; V2 IMPLEMENTED DORMANT, REVIEW PENDING**.
...
The full V2 shape, including the descriptor-4 rechecks, was verified end to end
against the target's own mksh in the reproduction above. The reader script needs
the same treatment for its `/proc/self/fd/3` metadata comparison.
...
| Identity | Value |
|---|---|
| dormant source SHA-256 | `3dae228ddd89147b403ad561ff4145efd5628d9ff2f0f12862075a839f68267f` |
| normalized SHA-256 | `afdaf08adc12f453239f20c8527ae8dba0e3432421c770e702bc0f8ae91452ce` |
...
the frame error: two shape guards assert that no generated script addresses a
descriptor through `/proc/self` and that the pin still precedes the write, and
four fixture tests execute the real generated bytes through the target's own
shell under qemu. One of them asserts that reverting to `/proc/self` fails there
with exit `65`, and one records that the host shell accepts both shapes and
therefore constrains nothing. The suite passes 6/6, the existing V1 suite still
passes 21/21 unchanged, and the readiness suite passes 18/18.

Activation still requires a fresh independent review and a fresh current
operator request. V2 inherits no authority from V1's consumed trial.
