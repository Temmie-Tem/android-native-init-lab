# Native Init V1456 Wi-Fi Test Boot Handoff

## Summary

- Cycle: `V1456`
- Type: bounded live test-boot handoff with rollback
- Decision: `v1456-test-boot-provider-trigger-no-downstream-rollback-pass`
- Result: PASS
...
- `pid1_rc1_watcher_result_file`: `state=triggered source=/proc/kmsg write_rc=0 errno=0 detect_elapsed_ms=7364 write_elapsed_ms=8517 delay_ms=0 retry_count=0 retry_delay_ms=0 line=<3>[    9.112765]  [3:   Binder:589_3:  610] subsys-restart: __subsystem_get(): __subsystem_get: esoc0 count:0`
- `pid1_rc1_window_sampler_requested`: `None`
- `pid1_rc1_window_result_summary`: `None`
- `pid1_rc1_window_result_file`: `state=armed sampler=read-only-v1454-exact-provider-long-endpoint detect_elapsed_ms=7364 delay_ms=0 exact_provider_line=1 long_provider_window=1 line=<3>[ 9.112765] [3: Binder:589_3: 610] subsys-restart: __subsystem_get(): __subsystem_get: esoc0 count:0`
- `pid1_rc1_window_sample_count`: `1`
- `pid1_rc1_window_has_post_500ms`: `False`

## Key Evidence

- Test boot version: `A90 Linux init 0.9.84 (v1454-wifitest)`.
- Rollback version after handoff: `A90 Linux init 0.9.68 (v724)`.
- Watcher result captured the exact provider line:
  `__subsystem_get: esoc0 count:0`.
- Window header recorded `exact_provider_line=1` and
  `long_provider_window=1`.
- The long-window file contains thirteen `rc1_micro_sample` records through
  `provider_micro_after_trigger_1000ms` plus `post_provider_micro_1200ms`.
- `wlan0=absent` and no RC1/MHI/WLFW/BDF/FW-ready downstream progress marker
  was observed.
...
- Test image: `tmp/wifi/v1454-wifi-test-boot-exact-provider-long-endpoint-sampler/boot_linux_v1454_wifi_test.img`
