# Native Init V3298 GPU H5 Visual Triangle Hold Live Validation

## Summary

- Cycle: `V3298`
- Decision: `v3298-gpu-h5-visual-triangle-hold-live-pass`
- Artifact flashed: `workspace/private/inputs/boot_images/boot_linux_v3297_gpu_h5_visual_triangle_hold_probe.img`
- Artifact SHA256: `a0728c476f7fa6793d28fc930d7dcdf8c3eac99dc3db44e7044274c5431f4e80`
- Resident after flash: `A90 Linux init 0.11.74 (v3297-gpu-h5-visual-triangle-hold-probe)`
- Result: PASS. The H5 command presented a recognizable centered triangle on the existing KMS panel path, held it for
  `30000ms`, and the operator visually confirmed the triangle was visible.
...
- Pre-flash resident: `A90 Linux init 0.11.73 (v3295-gpu-h5-strict-triangle-kms-probe)`
- Pre-flash selftest: `pass=12 warn=1 fail=0`
- Post-flash resident: `A90 Linux init 0.11.74 (v3297-gpu-h5-visual-triangle-hold-probe)`
...
- Final status: resident still `0.11.74`, display `1080x2400`, autohud `stopped`, transport `ready`.

## H5 Probe

The first H5 command attempt was blocked by the auto menu before the command handler ran:

```text
[busy] auto menu active; send hide/q before command
```

`hide` cleared the menu, then the command was rerun:

```bash
python3 workspace/public/src/scripts/revalidation/a90ctl.py --input-mode slow --timeout 120 \
  gpu h5-triangle-kms-probe --timeout-ms 5000 --hold-ms 30000 --materialize-devnode
```

Key telemetry:

- `gpu.h5.kms.scope=first-triangle-h5-visual-close-held-kms-probe`
- `gpu.h5.vis.mode=linear-nonzero-mask-solid-fill-centered`
- `gpu.h5.vis.hold_ms=30000`
- `gpu.h5.kms.blit_mode=h3-private-linear-snapshot-solid-triangle-mask-to-kms-dumb-framebuffer`
- `gpu.h5.kms.raw_tile_order_visualization=0`
- `gpu.h5.kms.linearized_tile6_3_a2d_blit=1`
- `gpu.h5.kms.power_write_attempted=0`
- `gpu.h5.kms.proprietary_blob_attempted=0`
- `gpu.h5.vis.autohud_stop_attempted=1`
...
- `gpu.h5.vis.source_bbox=64,64,126,126`
- `gpu.h5.vis.visual_shape=solid-mask-centered`
- `gpu.h5.kms.blit_rc=0`
- `gpu.h5.kms.blit_rect=67,727,945,945`
- `gpu.h5.kms.blit_scale=15`
- `gpu.h5.kms.present_rc=0`
- `gpu.h5.kms.result=h3-visual-triangle-kms-presented`
- `gpu.h5.vis.hold_begin=1`
- `gpu.h5.vis.hold_elapsed_ms=30000`
- `gpu.h5.vis.result=triangle-presented-held`
- `gpu.h5.kms.total_elapsed_ms=30159`
...
Operator visual confirmation during the hold window:
...
expected first-use `a640_zap` load/reset lines and an unrelated Wi-Fi firmware wait timeout.

## Conclusion

V3298 closes the human visual H5 confirmation gate: the same strict first-triangle proof from V3296 still passes, the
A2D-linearized nonzero triangle mask is converted into a centered high-contrast KMS framebuffer shape, KMS presents it
successfully, and the image remains on screen long enough for operator confirmation. The path stayed within the
recoverable KGSL/KMS envelope: existing `/dev/dri/card0` KMS only, no panel re-init, no power/backlight/PWM/PMIC/
regulator/GDSC/GPIO write, no proprietary blob path, and no forbidden partition path.
