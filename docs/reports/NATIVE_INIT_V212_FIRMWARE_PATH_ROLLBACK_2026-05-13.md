...
- status: `DRY-RUN PASS / APPLY PENDING`
...
- reason: dry-run mount and request resolution passed; rerun with `--apply` to
  test reversible sysfs write rollback

v212 implementation exists, but the risky sysfs write phase has not been run in
this report. The dry-run did not write `firmware_class.path`.
...
- `scripts/revalidation/native_firmware_path_apply_probe.py`
  ...
  - rejects plain `echo`, bind mounts, Wi-Fi bring-up, daemon starts, module
    mutation, firmware copy, and persistent path mutations
...
The next live command, when explicitly approved, is:
...
Post-apply acceptance requires:
...
The v212 implementation is ready for the explicit `--apply` rollback test. The
dry-run proves the mount and request-resolution side is stable and leaves the
