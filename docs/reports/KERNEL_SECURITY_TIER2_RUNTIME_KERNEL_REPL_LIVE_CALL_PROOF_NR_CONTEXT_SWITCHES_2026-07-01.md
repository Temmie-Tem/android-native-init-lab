# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: nr_context_switches

- Date: 2026-07-01
- Decision: `a90-repl-live-call-proof-nr_context_switches-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-nr-context-switches-20260701T043820Z/proof/a90_repl_evidence.json`
- Private timeline: `workspace/private/runs/kernel/live-call-proof-nr-context-switches-20260701T043820Z/timeline.json`

## Target

`nr_context_switches(void)` was selected from the previously aborted scheduler
counter batch as a fresh one-target proof. The earlier scheduler-counter
attempts stopped before any target call due host-side bridge/import-wrapper
failures. This run calls only the context-switch counter getter directly through
the checked `call-proof` CLI.

Trusted contract:

- No arguments.
- The function reads per-CPU context-switch counters and returns an
  `unsigned long long`.
- A valid result is a sane nonnegative counter in the proof range and
  nondecreasing across a short repeated call.
- No returned pointer is dereferenced or freed.

## Static Gate

- Address: `nr_context_switches=0xffffff80080edf84`.
- Resolution: `disasm-signature+xref+map`, C1 verified.
- Source declaration: `extern unsigned long long nr_context_switches(void)` at
  `include/linux/kernel_stat.h:52`.
- ABI: no pointer arguments.
- C1 safety tier: `SAFE-SCALAR`, no required pointer args.
- Direct BL xrefs: `4`.
- Next-symbol boundary: `nr_iowait` at `+0xa0`.
- Static word checks pinned all 40 words, including the `cpumask_next` loop
  over possible CPUs and the per-CPU counter load/add sequence.
...
- The first explicit post-flash `hide` hit serial framing noise. After a
  `12s` settle, bridge status was connected and explicit candidate `selftest`
  passed with `pass=11 warn=1 fail=0`.

Observed public values:

| Case | Return | Delta from first | Result |
| --- | ---: | ---: | --- |
| read 1 | `0x1ff6a` | n/a | PASS |
| read 2 | `0x201b9` | `0x24f` | PASS |

Both reads were inside the sane counter range and nondecreasing. The target
returned through the REPL normally; no oops/panic signature was observed in the
proof path. Post-proof candidate health stayed clean:
...
- Final explicit `version` first lost framing, then `hide` plus retry passed.
...
| Phase | Elapsed |
| --- | ---: |
| candidate flash helper total | `65s` |
...
| final health total | `19s` |
| final health retry | `1` retry |
| candidate start to final health done | `446s` |

Notes:

- The total includes serial resync/settle time after the first explicit
  post-flash `hide` framing failure and the final explicit `version` retry.
...
- Focused result: `Ran 2 tests in 18.425s`, `OK`.
- Static classifier:
  - `nr_context_switches`: `SAFE-SCALAR`, `disasm-signature+xref+map`,
    `0xffffff80080edf84`.
  - `nr_iowait_cpu`, `single_task_running`, `si_swapinfo`: `DENY`.

## Function Map Entry

`nr_context_switches` is live-proven under exactly this contract:

- No arguments.
- The return value is a sane `unsigned long long` context-switch counter.
- Short repeated calls must be nondecreasing.
