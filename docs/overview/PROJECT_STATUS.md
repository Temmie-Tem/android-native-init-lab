...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1328 bounded mdm2ap_timing live sampler PASS → V1329 Android-only SDX50M response-prerequisite classifier` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1329** — Android-only SDX50M response-prerequisite classifier
   (V1328 full-window no-transition과 Android-positive evidence를 비교해 Android가 native에 없는 어떤 response prerequisite을 제공하는지 host/read-only 우선 분류)
