...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1271` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1272** — host-only AP2MDM value observer classifier
   (V1271 결과 기준으로 PMIC GPIO9/TLMM GPIO135/142의 debugfs GPIO value line이
   직접 노출되지 않는 문제를 분류하고, 다음 read-only block sampler 범위를 확정)
