...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1277 SOURCE/BUILD PASS → V1278 deploy-only` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1278** — deploy-only helper v267 TLMM range-slice sampler
   (V1277에서 source/build pass; helper sha256
   `eccd9ca475927c2a37551304fedcc6740d19aeb048ebd137f966a18c269f0337`)
2. **V1279** — bounded live TLMM range sampler
   (PMIC GPIO9은 Android와 같은 `out/high`로 분류됐으므로, GPIO135/GPIO142 range
   visibility와 PCIe response를 read-only로 더 좁게 관찰)
