...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1326 source/build timing sampler support PASS → V1327 deploy-only for helper v276` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1327** — helper v276 deploy-only
   (V1326에서 빌드된 compact `mdm2ap_timing` sampler를 `/cache/bin/a90_android_execns_probe`에 배포하고 marker/sha/selftest만 확인)
2. **V1328** — bounded MDM2AP timing sampler live run
   (GPIO142/MDM errfatal/PCIe/MHI/ks/WLFW/`wlan0` timing을 compact summary로 확인; Wi-Fi HAL/scan/connect 없음)
3. **SDX50M eSoC power-up response gate** — `pm-service`가 `/dev/subsys_esoc0`를 열어
