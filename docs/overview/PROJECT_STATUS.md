...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1320 PASS → V1321 mdm_helper/ks/MHI image-link gate` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1321** — fail-closed `mdm_helper`/`ks`/MHI image-link gate
   (V1320에서 native post-GPIO135 response gap이 Android `mdm_helper`/`ks`/MHI contract와 연결됨)
2. **SDX50M eSoC power-up response gate** — `pm-service`가 `/dev/subsys_esoc0`를 열어
   `mdm_subsys_powerup`까지 진입하지만 Android식 `ks`/MHI image-link 없이 GPIO142/PCIe RC1/MHI/WLFW/`wlan0`가 absent인 원인 규명
