...
5. 이후 Wi-Fi Baseline Refresh / Network Exposure Hardening
   ...
   - v382 final readiness: `scripts/revalidation/wifi_v382_final_readiness.py`가 deploy plan/preflight, live plan/no-approval, router regression/no-approval route를 한 번에 검증한다. dirty-tree run은 의도적으로 blocked이며, 커밋 후 clean HEAD에서 approval-only 상태를 확인한다
