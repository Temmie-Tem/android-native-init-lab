...
- next execution item: V429 minimal lshal status split. Avoid `-p -e -c` first, split VINTF-only and binderized-only status columns, and mirror Android boot-complete explicit-column evidence before deciding whether to pivot to Android-managed Wi-Fi runtime control.
