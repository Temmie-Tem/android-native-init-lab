...
- V423 Android hwservice inventory result: read-only ADB collector is implemented and statically validated; current live preflight reports `v423-android-hwservice-waiting-for-android`, so the collector is ready but full Android is not currently online. Plan: `docs/plans/NATIVE_INIT_V423_ANDROID_HWSERVICE_INVENTORY_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V423_ANDROID_HWSERVICE_INVENTORY_2026-05-20.md`.
- next execution item: prepare a bounded Android handoff/rollback packet that boots Android, runs the V423 read-only hwservice inventory, then restores native init; keep scan/connect/link-up and Wi-Fi bring-up out of scope until registration is proven.
