...
- next execution item: V427 minimal native read-only service-query improvement plan. Decide whether to add the smallest missing service-manager/hwservice/framework/supplicant surface to native private runtime, or pivot to an Android-managed runtime control path before any Wi-Fi bring-up attempt.
