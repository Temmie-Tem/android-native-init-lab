...
reinterpreted. The promotion runner reuses the existing staging, flash,
journal, and rollback owner and adds only the second-boot health tail. It has
no final connected manifest or authority, so exact V2321 remains resident and
the D1 live runner stays inactive.
...
The bounded H0 runner now reuses the existing candidate, journal, and rollback
owner and adds only the resident reboot/second-health tail. It validates the
prior canonical Process-v2 closure for the same A90, the exact corrected
Debian A/B input, strict raw native health on both historical endpoints, and a
journal-first repairable promoted terminal. Focused regression passed
`215/215`; independent review returned PASS with no unresolved finding. No
device was contacted. The runner remains inactive without a final connected
