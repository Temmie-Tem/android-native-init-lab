...
The current unit is H0 documentation and architecture cleanup only. It records
the active H24 call path, explains why the handoff accumulated experimental
weight, separates irreducible safety mechanisms from optional product features,
and defines a smaller headless successor direction.
...
boot partition, private evidence, or another target.

The next implementation candidate must use a fresh identity after H25 and is
expected to be headless:
...
The exact successor ordinal, version, build, enable/latch paths, artifact pins,
qualification, D0, F1 approval, and D1 approval are future work. Nothing in
this document reserves or authorizes them.
...
- Wi-Fi handoff input needed for cable-free server operation;
- the current SD evidence sidecar until an independently reviewed same-run
  observer replacement exists;
...
- the hard SD evidence bind, after its proof role moves to a reviewed host/UFS
  same-run observer;
...
2. Design one fresh headless candidate; do not patch or reuse H25.
3. Build and host-validate it, including a before/after size and handoff-time
   baseline. Qualification remains host-only.
4. Require fresh connected D0 and exact attended F1 approval before one
   boot-only resident install.
5. Require exact resident health, then a separate attended D1 approval before
   one arm/reboot/handoff.
6. While Debian remains the persistent live runtime, publish only its exact
   ...
   not a failure.
7. Audit and replace two remaining SD couplings before removing the card: the
   hard evidence bind and the compiled Wi-Fi property-root path. Absence of SD
   is not yet a proved successor invariant.
8. After repeatable headless success, split experimental source modules and
   rebuild the Debian firstboot content as a separate rootfs/config change.
9. Add display later as a separate optional capability, preferably owned by
   Debian. Any persistent native HUD needs a fresh hazard design and review.
10. Consider section garbage collection and then Full-LTO only after functional
