...
P3.37 is closed, healthy, consumed and never replayable with exact candidate/rollback transfers 1/1 and formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p337_authenticated_resident_open_read_diagnostic_unproved_rollback_verified`. Candidate raw `97B/066a4780` contains the exact native-PID1 banner, stage-0/code-0 console-entry frame and stage-3/code-`-71` first-OPEN-read diagnostic; the canonical rejected receipt `7406B/8a938155` retains host OPEN TX `32B/4df3616f` and proves that device `read_frame()` returned `EPROTO`, but does not distinguish a propagated header/body read errno from header validation or body CRC rejection and does not prove which bytes were consumed. No OPEN_PARSED, HMAC, command, clean close or residency is proved. Recovery resumed only the journal after a host endpoint-inventory stop, exact rollback completed once and rooted FYG8 health passed. Independently reviewed host-only commits `1f201ba1c4` and `d10be8e1fd` repaired only the P337 receipt/stock projection and closed the retained journal without device contact; state is `18129B/72091518`, result `21075B/391ea331`, journal CLOSED at 19 records and `recovery_required=false`. The next proportional unit is an H0 comparison of the P3.35-success versus P3.36/P3.37 first-OPEN paths; only if still needed should one bounded branch ordinal separate header read, header validation, body read and CRC rejection. See `docs/reports/S22PLUS_FYG8_P337_F1_OPEN_READ_DIAGNOSTIC_NO_PROOF_2026-09-04.md`.
