...
P3.40 is consumed and recovery-pending after the operator returned exact approval digest `dfde76fb` for `p340-ready1-prepared-20260905-2`. Candidate AP `28631081B/114523aa` transferred once; rollback is not attempted (1/0). The journal has 10 records through `candidate_boot_ready`/`OBSERVED`; execution stopped with `measured USB endpoint inventory failed` while awaiting physical Download for exact Magisk rollback `23367721B/d2373bf8`. No candidate replay or new experiment is allowed; resume only the same journal's recovery after exact endpoint binding. The new collector preserved all 193 bytes (`f461d251`): banner, stage 0/code 0, stage 3/code 1, and stages 4–7. The complete rejected header is `533232504c55532d465947382d45333a`, exactly the 16-byte banner prefix `S22PLUS-FYG8-E3:`, not the host's valid `S328` OPEN. Diagnostic capture succeeded; authenticated sessions are 0, and candidate success/causality remain false. Echo is a hypothesis, not yet an attributed mechanism. Final health and F1 closure remain pending; append the matching F1 closure ledger row only with durable close. P3.39 remains CLOSED/healthy/consumed after exact candidate/rollback 1/1 and formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK`. P339 and P340 are never replayable. See `docs/reports/S22PLUS_FYG8_P339_INITIAL_CAPTURE_INCIDENT_2026-09-05.md` for qualification, preparation and retained run evidence.
