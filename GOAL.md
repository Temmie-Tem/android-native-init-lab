...
> **✅ POST-PROMOTION OBSERVABILITY RECORD (V2822–V2825, 2026-06-19): Tier-C audio status/selftest/screen UI observability is device-validated on `0.10.4`.** Artifact `workspace/private/inputs/boot_images/boot_linux_v2824_audio_screenapp_map.img` (SHA256 `2f6b1c902ee3ad1e06850feb04df847cc4c85881154af0ae28f4ce6c56d8035c`, tag `v2824-audio-screenapp-map`) keeps the promoted audio core, keeps the V2820 read-only `selftest verbose` audio row (`PASS audio ... boost=blocked sp=unverified`), keeps the V2822 `screenapp audio-status` summary, and adds display-only `screenapp audio-map` / `screenapp speaker-map` for the speaker/route map. V2825 validated `audio status` (16/16), `selftest verbose` audio markers (8/8), and `screenapp audio-map` markers (6/6) on-device, then rolled back to `v2321` with `selftest fail=0` and no recovery fallback. This is the latest observability candidate, not a rollback-net replacement.
