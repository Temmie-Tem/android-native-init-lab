...
P3.32 is independently qualified `PASS_GO_P332_H0` and now has one fresh exact-target D0 preparation. It tests one narrow correction to consumed P3.31: the exact proved P3.30 authenticated three-command runtime performs two complete logical sessions on one already-open tty FD, with session two gated by session-one success and physical reopen/reconnect counts fixed at zero. Final boot-only A/B AP is `28631081B/e1309080`, exact rollback remains `23367721B/d2373bf8`, candidate-static is `31856B/85313bb9`, and ready manifest is `6827B/a95641eb`. D0 `-1` is a non-reusable 0/0 host abort caused solely by the first ADB daemon startup notice on stderr before any target-specific command. Fresh D0 `p332-ready1-prepared-20260904-2` then selected the healthy rooted FYG8 S22+ while the attached S20+ received no target-specific command, proved Download absent and accepted only the exact `2097136B/f33384fb` consumed-P331 baseline under the reviewed P332 rule. Its preflight is `3261B/f7eadc6b`, prepared record is `20693B/5b93e5bb`, and the one-use approval binding is `fbae30c8`; no write, reboot, Odin, transfer or F1 action occurred. P3.30 remains the latest complete authenticated command proof and P3.31 remains closed, consumed and never replayable. The next step is the exact fresh attended F1 approval for this prepared binding. See `docs/reports/S22PLUS_FYG8_P332_LOGICAL_RESIDENT_SAME_FD_H0_2026-09-03.md`.
