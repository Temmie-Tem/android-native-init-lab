...
P3.27 is independently reviewed `PASS_GO_P327_FRAMED_EXEC_CANDIDATE_H0` at H0 only. Fresh same-length run ID `c327...3f3b`, `S327` OPEN/READY, exactly three BusyBox proof commands, streamed DATA/EXIT, finite bounds and CLOSE/DONE join Image, `/init`, and byte-identical boot-only A/B AP `28631081B/024322fd` under auditable private `-04`; the protocol predecessor was separately `PASS_GO_P327_FRAMED_EXEC_H0`. It still has no common Process-v2 registration, target-contract activation, ready manifest, approval, device contact, or live authority; see `docs/reports/S22PLUS_FYG8_P327_FRAMED_EXEC_H0_2026-09-02.md`.
