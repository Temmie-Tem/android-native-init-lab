...
P3.34 is closed, healthy, consumed and never replayable with formal `PASS_F1_V2_P334_AUTHENTICATED_LOGICAL_RESIDENT_FIRST_CONSOLE_RETURN_AND_ROLLED_BACK`. Exact candidate `28631081B/d79ecf07` and Magisk rollback `23367721B/d2373bf8` transferred once each with no attempt 2. The accepted `1162B/06a1cdcf` raw and `15385B/811d8c03` receipt prove native-PID1 authenticated bidirectional USB, two complete logical sessions on one tty FD, distinct HMAC nonces, stage 0/OPEN_PARSED/RNG in order, fixed BusyBox commands 6/6 with zero exits, clean close and zero reconnect/reopen; the second session proves the first console call returned. The deferred generation-107/stage-147 numeric return-detail receipt was not published before rollback, so exact errno is not claimed and the supplemental Carrier remains `P334_STOCK_PARSER_EXCEPTION`. An exception-type mismatch stopped host finalization after exact rollback and health; two independently reviewed host-only cuts added only the missing closure/result, with no device recontact or transfer replay. Final state is `13105B/ebfc0e3a`, result `15807B/91ba8a7e`, journal is CLOSED at 19 records and `recovery_required=false`. This proves bounded fixed-command logical residency, not interactive PTY, caller-selected/general shell, file transfer, persistent service or standing authority. See `docs/reports/S22PLUS_FYG8_P334_F1_LOGICAL_RESIDENT_PASS_2026-09-04.md`.
