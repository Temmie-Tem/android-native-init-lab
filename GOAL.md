...
P3.44 completed PASS_F1_V2_P344_NAMED_EXPLORATION_AND_ROLLED_BACK, CLOSED/19 with candidate/rollback1/1, result38199B/aba403cc, state34217B/d9029e02 and recovery_required=false. Initial four-session/twelve-command USB authentication and120s idle reuse passed (observer35538B/b14b751a). Five later named read actions kernel/processes/mounts/memory/usb-state all passed actual execution, result publication, lease accounting and final summary (5completed/provedtrue, no failed/pending action). This proves the bounded named read-only exploration workflow, not arbitrary shell, indefinite residency, persistent installation, reboot/Download command or autonomous recovery. Exact Magisk rollback and rooted FYG8 final health passed; current device is returned Android, not an active candidate session. P344 candidate and all actions are consumed and never replayable. P343 remains CLOSED healthy NO_PROOF and is not relabeled. Next research can build on the verified five-query exploration foundation using a fresh authorized candidate/session; no standing live authority. Current result/timeline: `docs/reports/S22PLUS_FYG8_P344_NAMED_EXPLORATION_PREPARATION_2026-09-05.md`; predecessor: `docs/reports/S22PLUS_FYG8_P343_NAMED_EXPLORATION_PREPARATION_2026-09-05.md`. P345 is the current H0 successor: OS-level read-only child view, authenticated cancellation and five fixed same-FD qualification sessions before mandatory rollback, not a later-action lease or standing arbitrary shell. Actual C/Python framing, HMAC, child execution, cancellation, 15-second timeout and a following command passed locally; filtered native BusyBox smoke and AArch64 compilation passed. Same-size fresh Image and A/B boot-only AP built under stock-candidate-build-v1-20260906-02 (AP28631081B/7ee59a13, result66136B/8bbbaea0); full offline bundle rehearsal passed. Live registration, raw-receipt reopening/terminal tests and independent changed-closure review now pass. Ready2 is published (6883B/0a416988). First D0 stopped on retained P344 baseline (result3251B/ff8cc018); its directory remains non-reusable. The operator-authorized conditional normal reboot completed exactly once with changed boot and rooted healthy return (linked result2963B/de04816f). Fresh p345-ready2-prepared-20260906-2 passed D0 with clean baseline2097136B/c0ffdd08 (zero family/exact markers), D0result3261B/8b4f9921 and prepared27550B/00651bc1. Fresh approval digest9056571c is issued but not returned/executed; no P345 F1 transfer, Download or Odin occurred. The one reboot is consumed; no standing reboot or shell authority. Host tests do not prove the real device mount/chroot boundary. Design: `docs/plans/S22PLUS_FYG8_RESEARCH_SHELL_DESIGN_2026-09-06.md`.
