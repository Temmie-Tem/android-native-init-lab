...
The forward frontier has moved off the connector-side Max77705 USB2 MUX discriminator to the role chain: role request, UDC bind, DWC3 pull-up/connect, physical host attach. The reviewed stock-witness runtime/build closure includes the `-32`/`-33` Image-only predecessor and earlier `-24`/`-25` runtime closure; all remain preserved. The reviewed `-46`/`-47`/`-07` closure is superseded by typed intent module-plan binding; current `-48`/`-49`/`-08` is independently reviewed H0-only `PASS_GO`, with no ready/run manifest or live authority.
...
Historical Stage-A evidence retains the permanent D0/F1 raw-first handle boundary and independent H0 PASS_GO. Stage B has since run and read the mxim debug register dump 0x00-0x10; that dump does not contain CONTROL1; it was not side-effect free. Its retained pair must not be cited as two candidate boots because the candidate observer was rejected. The prior `-44`/`-45`/`-06` and reviewed `-46`/`-47`/`-07` qualification closures are superseded; current P3.19 `-48`/`-49`/`-08` is independently reviewed H0-only `PASS_GO` for typed module-plan binding: no ready/run manifest, Process integration, fresh baseline, live authority, causal result, candidate success, MUX, or host-silent claim exists; rollback is only an H0 input.
