...
The forward frontier has moved off the connector-side Max77705 USB2 MUX discriminator to the role chain: role request, UDC bind, DWC3 pull-up/connect, physical host attach. No successor candidate build exists yet.
Stage A's first directory-only D0 requested zero attribute/I2C reads but the host parser discarded raw stdout before reporting a shape mismatch; the permanent D0/F1 raw-first handle boundary and granular Stage A parser then took independent H0 PASS_GO, and Stage B has since run and read the mxim debug register dump 0x00-0x10, so regmap presence and the Stage B target are proved rather than pending; that dump does not contain CONTROL1, which lives behind the opcode interface, and the read consumed a latched REG_VDM_INT so it was not side-effect free. An independent review demoted the MUX and 2026-08-19 host-only work supplied the structural ground: the P3.17 plan substitutes `s22plus_max77705_mux_diag.ko` for the stock driver on the same `maxim,max77705` compatible, so `max77705_muic_attach_usb_path` never runs on that candidate; the retained `0x3f`/`0x09`/`0x09` tuple shows the command protocol reachable and says nothing about the stock attach path, and it must not be cited as two candidate boots because the candidate observer was rejected `endpoint-timeout` and the retained reads are the rollback-observer pair. `B_SESS_VLD`, not `vbus_active`, remains the DWC3 peripheral gate, but the later binary audit proved this build registers no DT extcon notifier and therefore supplied no mechanism that arms sticky `EUD_SPOOF_DISCONNECT`. Direct ELF and closed-corpus audit now prove the stock DT route is pm8350c GPIO5 and that dynamically allocated parent/base/child IRQ numbers preserve PDIC offsets 22/23/26 in both captures carrying nested numbers; the current ten notifier captures contain 17 AP paths, 17 `06 09` attempts and 17 attaches, eight in IRQ context and nine outside it, with the 16/1 ordering split reported rather than accepted as a constant. A stock DT or shipped demultiplexer defect therefore does not explain candidate silence. The eight AP-path captures without a chgtyp IRQ are exactly eight boot-time `modprobe` initial-detect sequences: three printed status bytes, form-1 classification, and the AP mux write occur before `cc_booting_complete` and parent unmask. The reviewed V2 boundary proves the pre-MUIC APC/SYSMSG/VDM/VIR return is discarded, while nested UIADC/CHGT/DCD/VBADC/VBUSDET registration reuses one return slot: a nonfinal failure may be masked by later success, but final VBUSDET failure skips initial detect even though the outer probe can still report complete. Re-extraction of S7A2 proves its controller/MFD/PDIC plan, but its non-stopping `/dev/kmsg`-only loader and zero retained marker do not prove live loading. The successor transport audit further proves that stock retained-log survival does not transfer to the candidate: P3.18 omits `sec_log_buf`, MFD and stock PDIC while its direct Carrier freezes the reserved header index, so adding the Samsung logger would invalidate Carrier updates. The reviewed successor-plan V2 derivation proves the fourteen-member PDIC closure overlaps eleven current rows, so only `spu_verify`, `mfd_max77705`, and `pdic_max77705` are appended. Its exact 73-row source bundle is now materialized and independently reviewed: the historical predecessor wording was "current receipt is `8b8c1f5a`, review pending", and that review is now closed; the plan itself renders EUD index 38, all three added module bytes and the existing `dwc3-msm` provider are frozen from byte-identical vendor_boot/vendor_dlkm copies, and exact ELF recheck binds the DWC3 export to the PDIC call at `0x12318`. The parser predecessor receipt `14ca869c` has scoped independent H0 review; that predecessor correctly recorded that no canonical Carrier encoding exists. The reviewed parser drains after every successful load, binds rows 69/71/72 and preserves strict byte/sequence accounting. Its successor now defines distinct Envelope-v5 bytes without reinterpreting byte-identical V4: a canonical 76-byte structured witness fills the fixed 128-byte Carrier pair, exact generated C and Python agree, later non-row72 logs cannot overwrite the completed chain, and the decoder structurally denies V4 timing/poll/MUX causal authority. Materialized driver source prints the two already-read status bytes 3/4 with zero new I2C access and adds one checked register-`0x23` readback after the existing write. Review rejected `3cfa3959`, then found `c3b25c4f` selected the old default path; no-clobber `05ee3385` is now the independently reviewed plain-CLI current receipt with source-impossible MUX/result/execution states and native/Python classification-name parity closed. No successor candidate build exists yet; the next unit is the host-only build closure and creates no standing device authority.
The two historical corpus consumers now have a V3 semantic-manifest successor: the 47,799-byte `c1c75743` projection binds all 121 distinct capture identities and their complete census while excluding only duplicate-path population serialization; receipts `48c389e4` and `7744d9e7` are implemented review-pending, and the reviewed V2 receipts remain preserved. The stopped D0 is `docs/reports/S22PLUS_FYG8_P319_MAX77705_ATTRIBUTE_STAGE_A_D0_OBSERVER_STOP_2026-08-17.md`; the raw-first H0 closure is `docs/reports/S22PLUS_FYG8_P319_RAW_FIRST_OBSERVER_BOUNDARY_H0_2026-08-17.md`; the host-only closure of the review's five ranked items is `docs/reports/S22PLUS_FYG8_P319_STOCK_USERSPACE_CHOREOGRAPHY_H0_2026-08-19.md`; the reviewed module materialization is `docs/reports/S22PLUS_FYG8_P319_SUCCESSOR_MODULE_MATERIALIZATION_H0_2026-08-20.md`; the independently reviewed parser predecessor is `docs/reports/S22PLUS_FYG8_P319_CANDIDATE_WITNESS_PARSER_PREDECESSOR_H0_2026-08-20.md`; the Envelope-v5 implementation is `docs/reports/S22PLUS_FYG8_P319_CANDIDATE_WITNESS_CARRIER_V5_H0_2026-08-20.md`; the source authority remains
