...
P3.33 is closed, healthy, consumed and never replayable after exact candidate/rollback transfers `1/1`, a 19-record CLOSED journal and final rooted FYG8 health. Its narrow diagnostic succeeded: exact raw RX `73B/18bc55ba` is the 49-byte P3.33 native-PID1 banner followed by one CRC-valid `stage 0/code 0` frame emitted immediately before the out-of-line console call. The host wrote exact OPEN `32B/549e387c`, but no `OPEN_PARSED` followed; session 1 stopped at `open-diagnostic-read` and attempted/successful/reconnected counts are `1/0/0`. This rules out failure to enter `p328_framed_console()` while leaving the first device read/OPEN parse cause unproved. Formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p333_authenticated_logical_resident_open_entry_diagnostic_unproved_rollback_verified`; it proves no HMAC, command, second session, resident loop or causal USB claim. The ordinary publisher stopped only after closure because required arrival projection made state `32855B`, 87 bytes over the 32 KiB intermediate bound. Independently reviewed exact-run finalizer commit `74cc071092` kept that bound unchanged and published mode-0400/link1 state `32855B/232aeecc` plus result `36652B/207c36a1` with zero device/ADB/USB/Odin/transfer action. The proportional successor should retain only the first device-side `p328_read_frame()` return code outside the tty response path, without protocol rewrite or retry. See `docs/reports/S22PLUS_FYG8_P333_F1_OPEN_ENTRY_DIAGNOSTIC_NO_PROOF_2026-09-04.md`.
