...
P3.41 candidate observation is accepted, but recovery/final closure are pending. Exact approved run `p341-ready1-prepared-20260905-1` consumed candidate AP `28631081B/714c6c25` once; rollback is not attempted (1/0). Published receipt `28495B/01c6663e` and raw RX `1983B/da3438a4` prove three authenticated sessions, nine fixed commands with exit 0, two same-FD sessions and one planned host close/reopen, per-boot identity and complete diagnostic order. Host-first OPEN avoided the previously observed banner-as-header failure in this run; it does not prove historical echo attribution or activate a later-action lease. The journal has 10 records through OBSERVED/candidate_boot_ready. The runner stopped with `measured USB endpoint evidence failed` while waiting for physical Download; resume only the same journal's exact Magisk rollback `23367721B/d2373bf8` and final health. No candidate or command replay, no next experiment, and no final F1 PASS before healthy close. The fresh approval is consumed. P3.40 remains CLOSED/healthy/consumed at transfers1/1 and formal NO_PROOF; P340 is never replayable. P341 H0/preparation and current live evidence are recorded in `docs/reports/S22PLUS_FYG8_P341_HOST_FIRST_OPEN_PREPARATION_2026-09-05.md`.
