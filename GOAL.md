...
P3.24 is closed and consumed after exact candidate/rollback transfers 1/1 and healthy rooted FYG8 return. P300/P324 host evidence proves exact `04e8:6861` `cdc_acm`/`ttyACM0` candidate enumeration at `usb:3-1.3`, candidate count one, source/foreign count zero and continuous Type-C partner; it does not prove banner bytes, native PID 1 or USB data success.
The observer stopped pre-open at zero bytes because its ModemManager verifier queried the USB interface node instead of the tty node that actually carried both ignore flags. Formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p324_acm_primary_native_pid1_arrival_unproved_rollback_verified`; exact rollback/final health are proved and P3.24 is never replayable. An independently reviewed run-specific host-only finalizer published 34672B/9f23ecbf state and 38558B/b2eca7a4 result after the ordinary 32 KiB bound stopped post-close, with no device action.
