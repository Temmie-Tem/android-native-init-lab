...
P3.29 is independently reviewed `PASS_GO_P329_H0` and ready for one fresh connected Process-v2 preparation. It keeps the exact P3.28 authenticated protocol/key/BusyBox runtime and all P3.24-P3.28 lane, guard, boot-only, rollback and health behavior; its sole live acquisition change is a maximum 500-ms, 25-ms-poll settle for both ModemManager ignore flags on the already selected exact tty while guard, USB identity, major/minor and `usb:3-1.3` topology remain stable. It adds no fallback, selector widening, open retry, protocol retry, PTY, resident service or standing authority.
Fresh P3.29 run `c329f1e0a90b5e6d7c8a9b0c1d2e3f1b` uses byte-identical boot-only A/B AP `28631081B/7a83b7c3`, exact rollback `23367721B/d2373bf8`, candidate-static `25843B/2a64e338` and ready manifest `5205B/22431a21`. Independent review found and commit `029db236cb` fixed a P329 receipt-reopen branch-order defect before publication; common/P3.28/P3.29 tests pass 154/154 and the real raw-first audit passes. This H0 state grants no D0/F1/device/live authority; next is exactly one fresh D0 prepare, then a new attended approval if clean. See `docs/reports/S22PLUS_FYG8_P329_BOUNDED_UDEV_SETTLE_H0_2026-09-03.md`.
