...
P3.33 is H0-ready under `PASS_GO_P333_H0` and freshly D0-prepared in campaign-visible run `p333-ready1-prepared-20260904-1`. It keeps P3.32's two authenticated P3.30 exchanges on one tty descriptor, fixed commands, zero reconnects and zero protocol retries, changing only the remaining first-session entry boundary: existing bounded diagnostic stage `0` is emitted immediately before each out-of-line console call and required before `OPEN_PARSED` and RNG. The byte-identical A/B boot-only candidate is `28631081B/1a6036b6`, exact rollback is `23367721B/d2373bf8`, candidate-static is `31201B/f728aeea`, and ready_1 is `7090B/6fb30bec`. Fresh D0 selected the healthy rooted FYG8 S22+ while the separately attached S20+ received no target-specific command; Download was absent and the exact retained P3.32 baseline `2097136B/455eec00` was accepted only by the reviewed current-run-absent rule. Preflight is `3261B/6befd4fd` and prepared binding is mode-0400/link1 `20988B/3261045d`; it issued new one-use approval digest `34787824`. D0 performed no device write, reboot, Download request, Odin, transfer or F1 action. No new role, framing, command, retry, reconnect, persistence, PTY, file transfer, gate or causal claim was added. P3.32 remains closed, healthy, consumed and never replayable. Only the exact fresh attended P3.33 approval may start F1. See `docs/reports/S22PLUS_FYG8_P333_OPEN_ENTRY_DIAGNOSTIC_H0_2026-09-04.md`.
