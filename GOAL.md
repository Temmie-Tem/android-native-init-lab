...
P3.36 is closed, healthy, consumed and never replayable with formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p336_authenticated_long_idle_resident_unproved_rollback_verified`. Exact candidate `28631081B/d19f9781` and Magisk rollback `23367721B/d2373bf8` transferred once each with no attempt 2. Raw `73B/19955936` proves the exact P336 native-PID1 banner plus stage-0/code-0 console-entry diagnostic, but no receipt, OPEN_PARSED, HMAC, command, clean close, logical residency, retained listener or lease was proved. The observer incorrectly treated an empty partial proof as a namespace error, and post-rollback finalization omitted the P336 stock projection; independently reviewed host-only finalization closed the 19-record journal without device contact, with state `12791B/db4299b3`, result `15428B/9941d5e2`, healthy rooted FYG8 return and `recovery_required=false`. Commit `b2cc9b8992` repairs only those host no-proof/reporting paths without weakening success, timeout, authority or replay rules. The proportional next step is a narrow P3.35/P3.36 initial-session delta and explicit host-OPEN/device-first-read retention, not another broad protocol or gate. See `docs/reports/S22PLUS_FYG8_P336_F1_LONG_IDLE_NO_PROOF_2026-09-04.md`.
