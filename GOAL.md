...
P3.25 remains the current proportional successor: it keeps the exact P324 lane/selector/guard/receipt semantics, redirects only both read-time guard probes to the selected tty class node, prevents the P324 32 KiB post-close duplication, and has independent artifact verdict `PASS_GO_P325_PROCESS_V2_H0`. Fresh boot-only AP 27279401B/486fd1f2 is unchanged. One prepare stopped pre-binding on the one-time ADB daemon banner and the next connected read-only D0 durably stopped without effects because the common decoder lacked the P324-to-P325 successor-baseline case. Its 2097136B/e64815cf raw independently redecodes as one integrity-clean, foreign-free P324 `NO_PROOF_OBSERVER` record at offset 1657196. The exact bytes/placement/semantics repair, private promotion `-04`, and public `ready_3` 3327B/a1977097 have independent verdict `PASS_GO_P325_D0_PREDECESSOR_BASELINE_H0`; no P3.25 approval, candidate intent, F1, Odin, or transfer exists, so one fresh connected prepare is next.
