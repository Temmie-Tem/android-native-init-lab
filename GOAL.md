...
P3.31 is host-qualified and ready for a fresh attended Process-v2 preparation as a bounded resident-reconnect precursor, not a resident installation. Exact boot-only candidate AP `28631081B/729b33c3` with run `c331f1e0a90b5e6d7c8a9b0c1d2e3f9b` permits only two HMAC-authenticated native-PID1 sessions, one clean reconnect, and one fixed heartbeat/status command per session; each session requires a fresh distinct nonce, clean close and zero trailing bytes. PTY, caller-selected commands, arbitrary file transfer, persistence and unbounded looping remain absent. Durable reopening now binds each ordered RX slice and decoded challenge nonce to raw bytes and each bounded TX segment to the overall TX identity. Focused `28/28`, common/predecessor `151/151`, prepare `4/4`, full current-tree raw-first scan and independent hostile re-review pass; ready manifest is `6159B/63b39229`, while `device_contact=false`, `live_authorized=false` and `odin_invoked=false`. Fresh exact D0, newly returned F1 approval, attendance, mandatory rollback and final health are still required. P3.30 remains closed, consumed, never replayable and the first authenticated bounded-command success; see `docs/reports/S22PLUS_FYG8_P330_F1_AUTHENTICATED_COMMAND_PASS_2026-09-03.md` and `docs/reports/S22PLUS_FYG8_P331_BOUNDED_RESIDENT_RECONNECT_H0_2026-09-03.md`.
