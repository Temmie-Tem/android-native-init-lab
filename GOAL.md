...
The first approved P3.19 F1 invocation stopped before any transaction or device effect because its successful D0 `prepared.json` entered the prerequisite's candidate-occurrence scan and made candidate-static Integration regeneration reject its own immutable snapshot. The retained run has no transaction, Download intent, registry claim, transfer attempt, live state or result; registry history remains 0/0, so candidate and rollback were not sent and the candidate is not consumed. The host-only Rule-7 repair is independently reviewed `PASS_GO` under topic 53: only a typed pre-effect prepared record with the exact four-child namespace, reopened D0/private-target/USB-trace receipts, current ready/closure identity, and stable parse identity is non-consuming; the retained stopped run is separately pinned by its complete `prepared.json` identity. Validate/prepare still regenerate Integration, while execute/recover reopen the exact pinned Integration through a P3.19 runtime-bound validator and retain the live registry/journal as no-replay authority. The final chain is raw `-24` `15075B/54db40b2`, registry qualification `-02` `2964B/7be7e29c`, prerequisite `-14` `13186B/4e29a563`, Integration `-19` `126133B/dc9380a7`, candidate-static `-14` `35307B/37654942`, promotion `-14`, and ready manifest `2432B/e84e8183`; intermediate Integration `-17/-18`, candidate-static `-12/-13`, and promotion `-12/-13` remain preserved. Candidate AP `db5666ac` and rollback `d2373bf8` are unchanged. The old approval binding is stale and grants no retry, D0, F1, recovery or live authority; a fresh prepare/approval remain mandatory.
