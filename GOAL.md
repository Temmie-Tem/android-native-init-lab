...
P3.30 is closed, consumed, and the first S22+ authenticated native-PID1 bounded-command success. After one clean-baseline normal reboot, exact candidate AP `28631081B/f458498c` transferred once. The accepted `557B/cda7ec0f` device stream and `8502B/089150d8` receipt prove the fresh banner, zero-code `OPEN_PARSED` and `RNG` diagnostics, a fresh 32-byte challenge, HMAC-SHA256 authentication, READY, all three fixed BusyBox commands with zero exits, clean DONE, zero trailing bytes, exact candidate lane and continuous Type-C partner. Host TX was `369B/75ae9c61`; nonce acquisition used zero EAGAIN retries. The first process stopped after durable observation while measuring the rollback endpoint; recovery resumed only the journal, never replayed the candidate, transferred exact Magisk rollback `23367721B/d2373bf8` once, and verified rooted FYG8 final health. The journal is `CLOSED` at 19 records with no attempt 2 and `recovery_required=false`; final state is `32367B/63bddfd1` and result `36175B/05092220`. Formal verdict is `PASS_F1_V2_P330_AUTHENTICATED_PID1_BOUNDED_COMMANDS_DIAGNOSTIC_AND_ROLLED_BACK` / `p330_authenticated_pid1_bounded_commands_diagnostic_rollback_verified`. Supplemental Carrier remains `NO_PROOF_OBSERVER`, so no Max77705 causal claim, interactive PTY, persistent service, or unrestricted shell is promoted. P3.30 is never replayable; see `docs/reports/S22PLUS_FYG8_P330_F1_AUTHENTICATED_COMMAND_PASS_2026-09-03.md`.
