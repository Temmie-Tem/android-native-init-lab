...
P3.37 is the active independently reviewed H0 successor to closed, healthy and consumed P3.36. It keeps the proved P3.35 wire, three fixed commands, 300-second observation, boot-only transfer and exact Magisk rollback, and adds only one best-effort type-0x86 stage-3 diagnostic when the first framed OPEN read fails; it adds no retry, wait, command, lease or general shell. Candidate AP is `28631081B/0c1f6f23`, builder result `58673B/8a034575` and static receipt `36651B/341e25d2`. The first ready-6 invocation closed `ABORTED` at four journal records before Download intent, registry claim or transfer because the host arm helper omitted P3.37 from its retained-reopen label set; candidate/rollback counts are 0/0 and recovery is not required. Independently reviewed `PASS_GO_P337_PRE_CANDIDATE_ARM_REPAIR_H0` adds only that label and the P337 lease-schema selection, with an actual ready-spec arm regression; P337 tests 16/16 and common Process-v2 tests 131/131 pass. Fresh ready declaration is `9464B/444322ec` `ready-7`; neither it nor the H0/D1/D0 `PASS_GO` verdicts grants F1 authority. P3.36 remains formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` with exact candidate/rollback transfers 1/1, healthy rooted return and no replay; raw `73B/19955936` proves only its native-PID1 banner plus stage-0/code-0 console-entry diagnostic. See `docs/reports/S22PLUS_FYG8_P336_F1_LONG_IDLE_NO_PROOF_2026-09-04.md`.
