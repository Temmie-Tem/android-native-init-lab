...
P3.31 is host-qualified as a bounded resident-reconnect precursor, not a resident installation. Exact boot-only candidate AP `28631081B/729b33c3` with run `c331f1e0a90b5e6d7c8a9b0c1d2e3f9b` permits only two HMAC-authenticated native-PID1 sessions, one clean reconnect, and one fixed heartbeat/status command per session; each session requires a fresh distinct nonce, clean close and zero trailing bytes. PTY, caller-selected commands, arbitrary file transfer, persistence and unbounded looping remain absent. Durable reopening binds each ordered RX slice and decoded challenge nonce to raw bytes and each bounded TX segment to the overall TX identity. Focused `28/28`, common/predecessor `151/151`, prepare `4/4`, full current-tree raw-first scan and independent hostile re-review pass. Ready-1 D0 then contacted the exact S22+ but stopped with zero reboot/Download/Odin/transfer because its exact `2097136B/3136c504` baseline retained one integrity-clean, foreign-free P330 `NO_PROOF_OBSERVER` record. A reviewed P331-only exception accepts only that raw identity, exact P330 run/offset/valid slots and no-proof semantics as current P331 absence; drift or candidate intent expires it. Current ready-2 is `6159B/d3857ebb`, while `live_authorized=false` and `odin_invoked=false`. Fresh D0, newly returned F1 approval, attendance, mandatory rollback and final health are still required. P3.30 remains closed, consumed, never replayable and the first authenticated bounded-command success; see `docs/reports/S22PLUS_FYG8_P330_F1_AUTHENTICATED_COMMAND_PASS_2026-09-03.md` and `docs/reports/S22PLUS_FYG8_P331_BOUNDED_RESIDENT_RECONNECT_H0_2026-09-03.md`.
