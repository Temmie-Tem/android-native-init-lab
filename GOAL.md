...
P3.34 is packaged and Process-v2 H0-ready after independent `PASS_GO_P334_PROCESS_V2_H0`. It preserves P3.33's same-FD two-session authenticated console and changes only the first outer console return report to bounded `0xB000|errno`/`0xBFFF`; no response, retry, reconnect, command, PTY or persistence is added. The linked console remains 534 instructions with 525 identical words and nine rodata-address ADD fixups of exactly `+0x2c`, not a control-flow change. Fresh run `c334...f6b` has byte-identical boot-only A/B AP `28631081B/d79ecf07`, exact rollback `23367721B/d2373bf8`, candidate-static `32039B/e77e4b9c` and ready manifest `7580B/a662083e`. Package 20/20, common/static/prepare 11/11, P3.32/P3.33 regressions 9/9, P3.34 raw-first 2/2 and the CLI audit pass with zero device contact. The proportional next step is one fresh D0; only if the exact P3.33 baseline cannot be accepted is the operator-approved single normal-reboot D1 followed by a new D0 used. F1 remains separately token-bound. See `docs/reports/S22PLUS_FYG8_P334_FIRST_CONSOLE_RETURN_PROCESS_V2_H0_2026-09-04.md`.
