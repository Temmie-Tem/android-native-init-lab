...
P3.39 is `PASS_GO_P339_H0` plus `PASS_GO_P339_LIVE_MAPPING_REPAIR_H0` and `ready_2` only is current for a fresh connected preparation; unused `ready_1` is blocked. It preserves the consumed P3.38 authenticated resident path and adds no retry, wait, handshake, command, D0/D1 wrapper or gate: stage-3 code `1` is base-header grammar, code `4` is OPEN semantic rejection, and only those failures best-effort return the rejected 16-byte header in four existing diagnostic frames. Exact A/B boot-only AP is `28631081B/80830eed`, rollback remains `23367721B/d2373bf8`, and ready manifest is `11056B/80d1a1a6`; H0 grants no F1 authority. P3.38 remains closed, healthy, consumed and never replayable after exact 1/1 transfers, raw `97B/78137243` (fresh banner, stage 0/code 0, stage 3/code 1), exact rollback and rooted final health; its formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p338_authenticated_resident_open_read_branch_unproved_rollback_verified`. See `docs/reports/S22PLUS_FYG8_P339_OPEN_HEADER_CAPTURE_PROCESS_V2_H0_2026-09-05.md` and `docs/reports/S22PLUS_FYG8_P338_F1_OPEN_READ_BRANCH_NO_PROOF_2026-09-05.md`.
