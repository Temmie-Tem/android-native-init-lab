...
> **✅ POST-PROMOTION OBSERVABILITY RECORD (V2822/V2823, 2026-06-19): Tier-C audio status/selftest/screen UI observability is device-validated on `0.10.3`.** Artifact `workspace/private/inputs/boot_images/boot_linux_v2822_audio_screenapp_status.img` (SHA256 `30eb1ce0cb49143e3dca212cc44dce7ecf833163e130c2ff978d2125b785f3a8`, tag `v2822-audio-screenapp-status`) keeps the promoted audio core, keeps the V2820 read-only `selftest verbose` audio row (`PASS audio ... boost=blocked sp=unverified`), and adds display-only `screenapp audio-status` / `screenapp audio` for the compiled audio core/profile/route/safety summary. V2823 validated `audio status` (16/16), `selftest verbose` audio markers (8/8), and `screenapp audio-status` markers (6/6) on-device, then rolled back to `v2321` with `selftest fail=0` and no recovery fallback. This is the latest observability candidate, not a rollback-net replacement.
