...
P3.43 named read-only exploration is implemented with independent PASS_GO_P343_H0 and published ready_1 (14294B/54b9d578). Candidate A/B AP28631081B/14691cee has fresh run c343f1e0a90b5e6d7c8a9b0c1d2e3f9b, preserves host-first authenticated initial four-session/120s idle proof, and allows only five named middle queries (kernel/processes/mounts/memory/usb-state). Initial proof may open the existing bounded attended lease in a separate P343 namespace (one hour/16 actions); each action keeps id/query/nonce, pre-EXEC boot check, raw capture and one-shot journal. Exploration PASS additionally requires at least one successful named action with none pending/failed, exact rollback and healthy return. No arbitrary shell, writes, later reboot/control or autonomous recovery is activated. Actual C/PTY, lease/cut/summary tests, independent combined suite and canonical rehearsal pass; raw-first audit and real live closure validation pass. Connected D0 p343-ready1-prepared-20260905-1 stopped on retained baseline (result3251B/82d91d1c, raw2097136B/4ea961f0 matching P342); initial rooted Android was healthy, but no prepared binding or approval was issued. Authorized normal-reboot D1 completed once with healthy changed-boot return (2963B/340d49b4). D0 directory2 passed clean baseline but prepared publication exceeded32KiB; independently reviewed P343-only existing64KiB writer repair passed. Fresh directory3 is prepared34917B/e77c1f69, D0result3261B/50e876b9, clean baseline2097136B/da1405cb, approval digest b35d3045. Returned approval b35d3045 was used in directory3; observer arming aborted before Download/candidate intent (ABORTED/4, result1351B/710c0400, transfers0/0, recovery_required=false). H0 actual entry reproduces MAX_SESSIONS AttributeError: retained_reopen omits P3.43. The one-line P343 registration repair passed independent PASS_GO_P343_ARM_REOPEN_DISPATCH_H0 and actual backend arm regression; live739087B/abb7e93e. Fresh directory4 prepared34917B/80ca0926 and D0result3261B/63dcb6eb passed clean baseline and rooted health; returned approval091f1c67 sent candidate once: initial four-session/idle USB proof passed and bounded lease opened. First later kernel action retained RX661B/TX369B/stdout120B but result projection read nonexistent signal_number (actual term_signal), recording uncertain; no other action attempted. Exact rollback and rooted FYG8 final health completed: CLOSED/19, candidate1/rollback1, state33262B/ab4e2343, result37182B/edb77ee6, recovery_required=false. Formal NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK / p343_named_exploration_unproved_rollback_verified; initial USB proof passed but named action remains uncertain. P343 candidate/action are consumed, never replayable. Host result-projection repair now independently PASS_GO_P343_ACTION_RESULT_PROJECTION_REPAIR_H0: read term_signal, retain JSON signal_number; all5 actual PTY results pass publication/lease/summary and result-cut preserves uncertain/no-replay. Combined15/15 and raw audit pass; actionSHA3ef85fe5. Retained661B has13CRC-valid frames and3zero exits without changing the consumed NO_PROOF. Next is fresh successor identity/build/registration and preparation, not P343 replay; no new candidate or approval exists yet. No additional reboot/build; directory3 and its approval are not reusable. Directory3 had no candidate experiment; directory4 is consumed and CLOSED healthy. Report: `docs/reports/S22PLUS_FYG8_P343_NAMED_EXPLORATION_PREPARATION_2026-09-05.md`. P342 remains CLOSED/19, healthy, consumed and never replayable: PASS_F1_V2_P342_AUTHENTICATED_IDLE_REUSE_AND_ROLLED_BACK, four sessions/twelve fixed commands/120s idle, transfers1/1, result35666B/0136a663, recovery_required=false; canonical timeline in `docs/reports/S22PLUS_FYG8_P342_IDLE_REUSE_PREPARATION_2026-09-05.md`.
P3.35 is closed, healthy, consumed and never replayable with formal `PASS_F1_V2_P335_AUTHENTICATED_ATTENDED_RESIDENT_AND_ROLLED_BACK`. Exact candidate `28631081B/99a6299e` and Magisk rollback `23367721B/d2373bf8` transferred once each with no attempt 2. Accepted receipt `18882B/1b4aa474` proves native-PID1 authenticated bidirectional USB across three complete sessions: two on one tty FD and one after the planned host close/reopen, with distinct HMAC nonces, one authenticated per-boot identity, fixed BusyBox commands 9/9 with zero exits and clean close. One later named action remained only conservative `uncertain`: its digest resolves to `open-diagnostic-read`, before AUTH or EXEC, so it executed no fixed command, terminated the lease and was never retried. Possible queued-preamble resynchronization after the long idle is a strong hypothesis but not proved because partial TX/RX was not retained on that cut. Recovery resumed only the journal; exact rollback and rooted FYG8 final health passed, the journal is CLOSED at 19 records, state is `26952B/17877f36`, result is `30474B/c8005dee`, and `recovery_required=false`. Supplemental Carrier remains `AMBIGUOUS_INTEGRITY_FAILURE`; no causal USB, interactive PTY, caller-selected shell, file transfer, persistence, reboot, Download control or standing authority is promoted. The proportional successor should retain partial failure bytes and establish one fresh preamble boundary before repeating idle/reopen sessions. See `docs/reports/S22PLUS_FYG8_P335_F1_ATTENDED_RESIDENT_PASS_2026-09-04.md`.
P3.29 is closed and consumed after exact candidate/rollback transfers 1/1 and final healthy rooted FYG8 return. The exact `04e8:6861` CDC-ACM tty appeared on `usb:3-1.3`, both required ModemManager ignore flags were present, the Type-C partner stayed continuous, and raw-first capture received the exact 49-byte P329 native PID1 banner. This proves that the bounded udev settle repaired the P3.28 pre-open race and reached device-to-host candidate bytes.
The authenticated exchange then produced no CHALLENGE. The observer finalized `open-failed` almost exactly 120 seconds after the banner, while its partial exchange/exception state was not serialized; strict reopen therefore projected `interrupted-before-receipt`. Host OPEN-write completion versus device frame/nonce/challenge failure remains unresolved, so no host-to-device bytes, HMAC, BusyBox command, framed close or resident capability is proved. Formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p329_authenticated_framed_exec_unproved_rollback_verified`; state is `31298B/79a29d73`, result `34978B/73a11e9b`, recovery is not required, and P3.29 is never replayable. The proportional successor should retain partial host TX/RX plus one device pre-auth stage code before changing nonce or protocol behavior. See `docs/reports/S22PLUS_FYG8_P329_AUTH_HANDSHAKE_NO_PROOF_2026-09-03.md`.
P3.25 is closed, consumed, and the first S22+ ACM-primary success. After exact predecessor-baseline repair and a pre-session global-registry `st_dev` rebind, fresh approval `1155089e` sent candidate AP 27279401B/486fd1f2 once. The operator observed normal boot without a loop; the tty-node guard accepted the exact 49-byte native banner, durable arrival proof is true, and this proves native PID1 USB arrival. A first rollback-endpoint measurement stop resumed only the journal; exact Magisk rollback d2373bf8 then transferred once, rooted FYG8 final health passed, and the journal closed at 19 records with no attempt 2. The secondary retained Carrier remains `NO_PROOF_OBSERVER`, so no Max77705 scientific/causal result is promoted. The 30011B/c6094a55 final state fit the shared bound, while canonical result 33677B/c96b7900 exceeded it by 909 bytes; independently reviewed host-only finalizer published the exact mode-0400 result with formal verdict `PASS_F1_V2_P325_ACM_PRIMARY_NATIVE_PID1_USB_ARRIVAL_AND_ROLLED_BACK`, outcome `p325_acm_primary_native_pid1_usb_arrival_rollback_verified`, and `recovery_required=false`. P3.25 is never replayable. P3.26 is now closed, consumed, and the first S22+ native-PID1 bidirectional USB plus BusyBox-shell proof. Exact candidate AP 28631081B/954b5603 transferred once; the accepted 145B/8e5136ac transcript proves the fixed 77-byte two-write PING/PONG and SHELL/SHELL-OK exchange, PID1 reply, and static BusyBox `ash -c` execution with zero trailing bytes. Recovery resumed only the durable journal after a post-observation endpoint-measurement stop; exact Magisk rollback d2373bf8 transferred once, rooted FYG8 final health passed, and the journal closed at 19 records with no attempt 2. Supplemental Carrier remains `NO_PROOF_OBSERVER`, and this fixed exchange is not a general interactive shell. Canonical result 33879B/a78a9c50 exceeded the shared bound only after closure; independently reviewed host-only finalizer published it mode 0400 with formal verdict `PASS_F1_V2_P326_NATIVE_PID1_BIDIRECTIONAL_USB_BUSYBOX_SHELL_AND_ROLLED_BACK`, outcome `p326_native_pid1_bidirectional_usb_busybox_shell_rollback_verified`, and `recovery_required=false`. P3.26 is never replayable.
P3.27 is closed, consumed, and the first S22+ bounded framed fixed-command success. Exact candidate AP `28631081B/024322fd` transferred once; the accepted `445B/100bf5c9` raw stream contains the fresh 49-byte banner and eight valid `S327` response frames for the three pinned BusyBox commands, with all exits zero, PID1 framed execution, BusyBox `ash` execution, clean `DONE`, and zero trailing bytes. The exact P324 lane inventory and same-run Type-C partner continuity passed. Recovery resumed only the durable journal after a post-observation endpoint-evidence stop; exact Magisk rollback `23367721B/d2373bf8` transferred once, final rooted FYG8 health passed, and the journal closed at 19 records with no attempt 2. A host reopen bug looked for two nonexistent top-level topology-digest fields instead of the stored literal lane names and nested inventory digests, and the ordinary final result then exceeded 32 KiB. Independently reviewed commit `365eb86f54` repaired only that parser projection and published exact `30381B/138cf273` state plus mode-0400 `34047B/ee36e0db` result without device contact. Formal verdict is `PASS_F1_V2_P327_NATIVE_PID1_FRAMED_EXEC_FIXED_COMMANDS_AND_ROLLED_BACK`; supplemental Carrier remains `NO_PROOF_OBSERVER`, and no interactive PTY, caller-selected command, general shell, or Max77705 causal claim is promoted. P3.27 is never replayable; see `docs/reports/S22PLUS_FYG8_P327_F1_FRAMED_EXEC_FIXED_COMMANDS_PASS_2026-09-02.md`. P3.28 is now closed and consumed with exact candidate/rollback transfers 1/1 and final healthy rooted FYG8 return. The exact candidate enumerated once as `04e8:6861` CDC-ACM on `usb:3-1.3` with continuous Type-C partner, but the primary observer stopped before opening the tty: its first and only 131-byte udev-property probe raced ahead of completed tty udev processing and lacked both required ModemManager ignore flags, while the same-run udev trace about 15.8 ms later contains both. Raw RX/TX are zero, so no banner, nonce, HMAC, BusyBox command or framed close was tested; formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p328_authenticated_framed_exec_unproved_rollback_verified`. Exact Magisk rollback completed once, the journal closed at 19 records, and the new 64 KiB final-result-only writer published `35135B/aed4dc0c` normally while ordinary records retained the 32 KiB bound. P3.28 is never replayable. The proportional successor keeps both flags mandatory and adds only a short exact-endpoint udev settle before the open; see `docs/reports/S22PLUS_FYG8_P328_F1_AUTH_GUARD_RACE_NO_PROOF_2026-09-03.md`.
...
The forward frontier has moved off the connector-side Max77705 USB2 MUX discriminator to the role chain: role request, UDC bind, DWC3 pull-up/connect, physical host attach. The reviewed stock-witness runtime/build closure includes the `-32`/`-33` Image-only predecessor and earlier `-24`/`-25` runtime closure; all remain preserved. The reviewed `-46`/`-47`/`-07` closure is superseded by typed intent module-plan binding; `-48`/`-49`/`-08` remains preserved as independently reviewed H0-only `PASS_GO` evidence with no ready/run manifest or live authority. Current `-52`/`-53`/`-10` requalification binds all 437 current source keys, reproduces identical candidate bytes, and is independently reviewed H0-only `PASS_GO` under topic 34; topic-34 review is closed and grants no authority. The prerequisite gates retain scoped independent H0 `PASS_GO`; the runner-consumed global candidate registry now has independent H0 capability `PASS_GO`, and its common Process-v2 section remains the permanent restrictive no-replay boundary independently reviewed by eaff1d48d3. The auditor repin/cardinality closure is independently reviewed H0-only `PASS_GO` under topic 31, and topic 32 now independently closes the durable Download-request-cut recovery owner. The FYD9-to-FYG8 22-file delta is host-audited under review-pending topic 33: its two USB changes are notify-layer-only, relevant to HOST restriction and alternate-mode readiness, and do not prove gadget silence. The exact 73-row SSUSB/UDC static membership/order/ABI closure is independently reviewed H0-only `PASS_GO` under topic 37; dynamic bind, probe and candidate UDC creation remain unproved. The older-kernel TWRP architectural comparison is independently reviewed H0-only `PASS_GO` under topic 38 and does not replace the current direct-role path or prove candidate runtime. Topic 39 independently reviews only the predecessor H0 fresh-baseline reducer and integration fail-open repair. Topic 40 independently reviewed the unchanged D1 producer source and permanent byte-frozen global acquisition-detector membership. Topic 41 independently reviews the D0 producer under `14240B/34203813` binding, repins the unchanged D1 source to `4128B/0a91beec`, and retains D0 as an active raw-first source. Both producer execution flags are true only for an exact deterministically reopened normalized result. The exact D1 run `d1-fresh-baseline-1` consumed its approval and stopped after arm before start: no reboot dispatch, start or result exists, device contact is unknown and formal health is pending, so replay is forbidden and D0 cannot proceed. An independently reviewed V2 H0 capability mechanically reproduces the inherited raw-capture-unbound topology path and direct-inventory raw bypass without retroactively relabeling V1; it uses a new fixed ordinal, binds raw capture before inventory, rejects pre-existing or concurrent arm ownership without appending a stop, preserves transient post-reboot property failures within the bounded P2.96 poll, and creates no approval, arm or run. Topic 43 independently reviews the final V2 D0 producer/reducer under canonical `14292B/440d9672` binding; it accepts only the exact D1 V2 result and preserves V1 bytes; no D1 V2 result exists, so machine integration remains `BLOCKED_P319_PROCESS_V2_INTEGRATION_H0` only on `FRESH_BASELINE_MISSING`. Capability review is not current operator approval; no ready/run manifest or live authority exists, and `1ea4f930`/`cb36cce8`/`d92e7e46` remain unconsumed predecessors.
D1 V2 ordinal `d1-fresh-baseline-2` is consumed without replay after its production arm writer emitted indented JSON that the compact-canonical post-arm validator rejected before any device command; only `773B/86dfc3ef` arm and `1989B/bc438727` stop exist, with no ADB snapshot, raw directory, start, result or reboot. The D1 V3 canonical-arm successor binds both consumed V1/V2 histories and is independently reviewed H0-only `PASS_GO`. The 46,505-byte `e93ec6df` normal-reboot live wrapper is independently reviewed H0-only `PASS_GO` under binding `2550B/465ed3c5`; one exact attended pre-F1 session then used it. The first prepare stopped before device contact because the fixed private parent was absent, exact 0700 parent provisioning enabled the single Rule-7 corrected prepare, and ordinal `d1-fresh-baseline-3` performed one normal reboot. Its `39594B/8dc828d3` result proves changed boot identity, healthy rooted FYG8 return, stable selected serial/topology, complete raw evidence, zero other-target commands and no Download/Odin/payload/F1/replay; the outer journal is `CLOSED` at `1287B/ea7de807` and grants no remaining session authority. The independently reviewed D0 V3 consumer then consumed one exact approval under binding `14545B/fd595a25`: its `874B/84383f92` arm, `57888B/360a3849` result and `2097136B/ec4fe9fb` raw observer prove one clean FYG8 baseline with nine successful raw-ADB handles, no candidate marker, stable returned boot, and no write/reboot/Download/Odin/partition/F1/replay; the host-only reducer published canonical `56204B/fba4dc9f` normalized V3. The approval is consumed with no remaining D0 authority. The reviewed V2 `-01` integration result remains preserved. The current-authority repin is independently reviewed H0-only `PASS_GO`: the unchanged candidate requalified under `-11/-54/-55` with identical boot/AP bytes, executability and prerequisite consume the current target/raw-first identities, and private `-02` is `118384B/d21bf634` with zero blockers but `runtime_classification_gate_pending=true`. Review-rejected private `-03` proved the phase receipts but only compared their nested artifact identities; repaired independently reviewed H0-only `-04` reopens all baseline/current A/B AP, boot, LZ4, init and child files, proves exact bytes with only the target-contract source key changed, and remains runtime-pending. Independently reviewed H0-only stock-adapter registration preserves COMPLETE as `NONCAUSAL_SUCCESS_PATH`, separates both NO_PROOF classes, keeps ACM supplemental and explicitly blocks offline promotion until a P319 static artifact exists; review found ordinary Python equality accepted nested boolean/integer/float type substitutions, and final repair now rejects them with one recursive exact-type comparator at both raw-final and durable-reopen seams. Its shared-source changes make `-04` a predecessor. The post-registration repin is now independently reviewed H0-only `PASS_GO`: it binds raw-first `-18`, the current live-registry qualification and prerequisite, and publishes Integration V2 `-05` `125814B/16c9901f` with zero blockers and byte-identical independent regeneration, while remaining `runtime_classification_gate_pending=true`. Independently reviewed H0-only candidate-static `34782B/86cc71e1` rederives `-05`, binds all three real terminal encodings and the four future post-run witness requirements, and keeps runtime values unobserved; shared offline registration and a ready manifest remain separate gates. All ready/run/approval/live/action/candidate-success/causal flags remain false.
The independently reviewed `-23/-11/-15/-10` offline-ready chain is preserved as a predecessor. Main-tree validation then proved that five unrelated A90 revalidation files changed only the excluded raw census from 1747/412 to 1752/415 while the semantic projection stayed `5b1f42dd`; copying those diagnostic integers back into the exact prerequisite receipt had made the ready bundle cross-target-sensitive. The independently reviewed census-decoupled successor keeps raw-first `-23` `15075B/608799f1`, advances prerequisite to `-12` `13190B/4a18cf11`, Integration V2 to `-16` `126135B/1506e898`, candidate-static to `-11` `35309B/d00f422e`, promotion to `-11`, and publishes tracked `2432B/e6c75846` `ready-for-f1-approval`. The prerequisite now stores only the already-audited semantic projection while still exercising count-change controls; the tracked public declaration accepts only 0644 or Git-umask-equivalent 0664, while private promotion files remain exact 0400. The non-consuming declaration still exact-checks every acceptance and artifact-identity scalar and reopens all three private promotion identities; the prerequisite proves the current global registry has zero records and zero active candidate claims, while the shared verifier re-executes the exact self-bound candidate-static builder, freshly regenerates Integration, recursively enforces JSON types, and source-loads the complete P319 adapter dependency closure without `.pyc`. Exact predecessor commit `0659738a49` and census-decoupling commit `6d7bd57378` received independent `PASS_GO`; the current capability creates no approval, prepared live run, consumed claim, device action, D0/D1/F1/recovery/replay/live authority, candidate-success, causal, MUX, or host-silent claim. A fresh attended F1 approval remains mandatory. The e721bf2a24 provenance finding withdrew the old run-bound vmlinux/symvers/.config lane. The Image-only IKCONFIG, vermagic, PREL32 provider and CRC repair is now independently reviewed `PASS_GO`, with current authority exclusively `-32`/`-33` receipts `a6e1734b`/`e491c797` and the current auditor tuple in `docs/reports/S22PLUS_FYG8_P319_STOCK_IMAGE_PROVENANCE_REPAIR_H0_2026-08-21.md`; it made no PDIC rebuild or candidate-byte change. The earlier `-24`/`-25` (`621623bb`/`44f4b412`) closure is a superseded prior reviewed closure, and the older `bd94a638`/`231316f0` sentence in the following historical notes refers only to superseded intermediate receipts. There is no standing live authority.
Historical Stage-A evidence retains the permanent D0/F1 raw-first handle boundary and independent H0 PASS_GO. Stage B has since run and read the mxim debug register dump 0x00-0x10; that dump does not contain CONTROL1; it was not side-effect free. Its retained pair must not be cited as two candidate boots because the candidate observer was rejected. The prior `-44`/`-45`/`-06` and reviewed `-46`/`-47`/`-07` qualification closures are superseded; P3.19 `-48`/`-49`/`-08` remains preserved as independently reviewed H0-only `PASS_GO` evidence for typed module-plan binding with no ready/run manifest or live authority. Reviewed `-52`/`-53`/`-10` remains preserved under topic 34; independently reviewed H0-only `-54`/`-55`/`-11` reproduces the same boot/AP bytes under the current target contract. The prerequisite integration admits three terminal proof classes and passes static executability after current-identity repin. Preserved V1 remains blocked on its unavailable V2 baseline and reviewed V2 `-01` preserves the four stale-pin blockers; independently reviewed H0-only V2 `-02` has no blockers but remains not ready on the declared candidate-runtime witness set. None creates a ready/run manifest, live authority, causal result, candidate success, MUX, or host-silent claim. Rollback remains only historical H0 usability evidence.
The two historical corpus consumers now have a V3 semantic-manifest successor: the 47,799-byte `c1c75743` projection binds all 121 distinct capture identities and their complete census while excluding only duplicate-path population serialization; receipts `48c389e4` and `7744d9e7` are implemented review-pending, and the reviewed V2 receipts remain preserved. The stopped D0 is `docs/reports/S22PLUS_FYG8_P319_MAX77705_ATTRIBUTE_STAGE_A_D0_OBSERVER_STOP_2026-08-17.md`; the raw-first H0 closure is `docs/reports/S22PLUS_FYG8_P319_RAW_FIRST_OBSERVER_BOUNDARY_H0_2026-08-17.md`; the host-only closure of the review's five ranked items is `docs/reports/S22PLUS_FYG8_P319_STOCK_USERSPACE_CHOREOGRAPHY_H0_2026-08-19.md`; the reviewed module materialization is `docs/reports/S22PLUS_FYG8_P319_SUCCESSOR_MODULE_MATERIALIZATION_H0_2026-08-20.md`; the independently reviewed parser predecessor is `docs/reports/S22PLUS_FYG8_P319_CANDIDATE_WITNESS_PARSER_PREDECESSOR_H0_2026-08-20.md`; the Envelope-v5 implementation is `docs/reports/S22PLUS_FYG8_P319_CANDIDATE_WITNESS_CARRIER_V5_H0_2026-08-20.md`; the cross-target raw-first successor remains independently reviewed `PASS_GO_P319_RAW_FIRST_CROSS_TARGET_MEMBERSHIP_H0_CAPABILITY_V1`. The current candidate-requalification raw-first successor is receipt `20260823-04` `11012B/ac3876c0`; pre-final `20260823-03` `ab698ffb`, independently reviewed request-cut `20260823-02` `d8448713`, prior `-04` `d3e501`, `-03` `2016f6cb`, and earlier receipts remain preserved. S22 pre-boundary inventory is `127/ce35f2b8`, host-only non-acquiring is `1/341e71f4`, and target-external membership is 52 with the S20+ coordinator and dormant health source membership-only. It creates no device/live authority; the source authority remains
`docs/reports/S22PLUS_FYG8_MAX77705_CONTROL_PLANE_SUCCESSOR_FEASIBILITY_H0_2026-08-11.md`. The preceding `20260823-04`/`127` raw-first sentence and topic-40 `20260824-01` (`11285B/b56ef046`) and `-02` (`11285B/ff1cab64`) receipts are preserved predecessor statements. Topic 41's reviewed predecessor is `raw-first-observer-audit-20260824-03-p319-d0-fresh-baseline.json` (`11792B/9c5d892c`). The corrected independently reviewed V2 D1 capability retains raw receipt `raw-first-observer-audit-20260824-05-p319-d1-v2-no-replay.json` (`12394B/2d5fc042`) unchanged and promotes only its binding from the `5300B/917daa02` predecessor to `5351B/65e2953e`; topic 43's reviewed D0 V2 remains `72288B/e1190b66`; independent review blocked reducer `54126B/ea72adab` and binding `14251B/bda6b82d`, first repaired under the same obligation as reducer `54939B/d6d4c766` and binding `14251B/4be15cba`, then cross-binding-repaired as reducer `55631B/13496eba` and binding `14251B/151c2f1a`, then fixed-result-path-repaired as reducer `56272B/0658ca30` and binding `14251B/b0cc446f`, then direct-output-namespace-repaired as reducer `62093B/12aa83bf` and binding `14251B/bc3b44bc`, then atomic-publication-repaired as reducer `66217B/cd4aa54e` and binding `14251B/e41d40fa`, then inode-continuity-repaired as reducer `68081B/60ab41da` and binding `14251B/42e7d146`, finally nameless-publication-repaired as reducer `68400B/241c216e`; independent review promotes only the binding tuple from pending `14251B/f4ccb03a` to pass-go `14292B/440d9672`, raw `-06` `12916B/66658f67`, active 18 and unchanged `128/fcb3bb80`/52; `-04` remains a byte-preserved predecessor. Those are topic-43 predecessor values; current final-repaired review-pending P319 registration keeps counts 128/52 and the exact S22 pre-boundary hash `f18e2b0d`, advances the raw-first receipt to `-18` `15075B/0ffd6306`, and preserves incomplete first-repair `-17` `15075B/9db3ed71` plus review-rejected `-16` `15075B/8fb85249`. V1 remains explicit byte-frozen pre-boundary D1 detector membership, D1 V2 is the 17th active raw-first source, and D0 V2 is the 18th.
The first approved P3.19 F1 invocation stopped before any transaction or device effect because its successful D0 `prepared.json` entered the prerequisite's candidate-occurrence scan and made candidate-static Integration regeneration reject its own immutable snapshot. The retained run has no transaction, Download intent, registry claim, transfer attempt, live state or result; registry history remains 0/0, so candidate and rollback were not sent and that stopped invocation did not consume the candidate. The host-only Rule-7 repair is independently reviewed `PASS_GO` under topic 53: only a typed pre-effect prepared record with the exact four-child namespace, reopened D0/private-target/USB-trace receipts, current ready/closure identity, and stable parse identity is non-consuming; the retained stopped run is separately pinned by its complete `prepared.json` identity. Validate/prepare still regenerate Integration, while execute/recover reopen the exact pinned Integration through a P3.19 runtime-bound validator and retain the live registry/journal as no-replay authority. The final pre-live chain was raw `-24` `15075B/54db40b2`, registry qualification `-02` `2964B/7be7e29c`, prerequisite `-14` `13186B/4e29a563`, Integration `-19` `126133B/dc9380a7`, candidate-static `-14` `35307B/37654942`, promotion `-14`, and ready manifest `2432B/e84e8183`; intermediate Integration `-17/-18`, candidate-static `-12/-13`, and promotion `-12/-13` remain preserved. Candidate AP `db5666ac` was subsequently transferred and globally consumed by P3.19 ordinal 1; exact rollback `d2373bf8` completed once and final health passed. That live binding grants no retry, D0, F1, recovery, or standing authority. Any successor requires new candidate bytes, fresh closure, baseline, prepare, and approval.
