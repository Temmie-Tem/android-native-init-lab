...
P3.31 is closed and consumed after exact candidate/rollback transfers `1/1`, a 19-record CLOSED journal and final healthy rooted FYG8 return. The candidate booted without a reported loop; raw-first capture received the exact `49B/99eab40e` native-PID1 banner and retained one `32B/ab8a5c73` host OPEN frame, but the first session timed out at `open-diagnostic-read` before `OPEN_PARSED`/`RNG`, challenge, HMAC, heartbeat, clean close or reconnect. Sessions attempted/successful/reconnected are `1/0/0`, so formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p331_authenticated_resident_heartbeat_unproved_rollback_verified`, not resident success. A reviewed host-only finalizer repaired only the post-CLOSED P331-to-P328 result-validation fallthrough and published exact mode-0400 result `15182B/172952c9`; its post-publication audit caused zero device or transfer action. P3.30 remains the latest complete authenticated command proof, while P3.31 is never replayable. The proportional successor stays at the first-session OPEN lifecycle boundary: compare the P331 outer resident loop with the proved P330 single-session entry and change only what blocks the first diagnostic, without adding persistence, PTY, arbitrary commands, broad retries or another gate. See `docs/reports/S22PLUS_FYG8_P331_F1_RESIDENT_RECONNECT_NO_PROOF_2026-09-03.md`.
