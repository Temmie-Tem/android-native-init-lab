...
P3.27 is independently reviewed `PASS_GO_P327_FRAMED_EXEC_H0` only as an H0 framed fixed-command prototype: fresh `S327` OPEN/READY binding, exactly three BusyBox proof commands, streamed DATA/EXIT, finite timeout/output bounds, and CLOSE/DONE all pass focused fixtures and a static AArch64 full-closure link. It has no artifact builder, AP, Process-v2 registration, target-contract activation, ready manifest, approval, or device authority; see `docs/reports/S22PLUS_FYG8_P327_FRAMED_EXEC_H0_2026-09-02.md`.
