...
P3.38 is independently reviewed H0 under `PASS_GO_P338_H0`, `PASS_GO_P338_D0_BASELINE_REPAIR_H0` and `PASS_GO_P338_LIVE_MAPPING_REPAIR_H0`; current `ready_3` is preparation data, not F1 authority. It preserves P3.37's authenticated three-session/fixed-command success path and adds only one best-effort stage-3/type-`0x86` first-OPEN branch ordinal: header-read errno, header validation, body-read errno or CRC rejection. The original failure is returned unchanged; no retry, wait, command, reconnect, lease action, shell or gate is added. Fresh run `c338f1e0a90b5e6d7c8a9b0c1d2e3f2b` has byte-identical boot-only A/B AP `28631081B/2f6dc740`, exact rollback `23367721B/d2373bf8`, builder `61618B/c23dd514`, static `38958B/f1c63d23` and current ready manifest `10246B/44f192f2`. First D0 stopped 0/0 because the exact P337 baseline dispatch was omitted; `21f338f418` repaired it and the second D0 then passed exact healthy/root/raw baseline reads but stopped before prepared publication because three live P338 membership sets ended at P337. `09ba2854fd` adds only the exact P338 mapping and its direct ready-bundle test; `713ce80058` repins raw-first identities. The third D0 published one-use digest `31fab860`; its F1 invocation then closed 0/0 before candidate because the Codex executor's `NoNewPrivs:1` suppressed the otherwise-correct root-owned mode-4755 `pkexec`, so the observer guard could not arm. Result is `ABORTED/candidate_observer_arm_failed_before_candidate`, candidate `not-attempted`, recovery false, and that run/approval is never reused. A fourth fresh D0 now passes the same exact healthy/root/raw baseline and publishes mode-0400/link-1 `prepared.json` `25187B/36218e4d` in `p338-ready3-prepared-20260905-4`, binding one-use approval digest `18c992d8`; D1 remains unnecessary. P3.37 remains closed, healthy, consumed and never replayable with exact transfers 1/1 and formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p337_authenticated_resident_open_read_diagnostic_unproved_rollback_verified`. The next unit is attended P3.38 F1 from the operator's normal terminal with only that exact fresh approval, followed by mandatory rollback and final health. See `docs/reports/S22PLUS_FYG8_P338_OPEN_READ_BRANCH_PROCESS_V2_H0_2026-09-05.md` and `docs/reports/S22PLUS_FYG8_P337_F1_OPEN_READ_DIAGNOSTIC_NO_PROOF_2026-09-04.md`.
