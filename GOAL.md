...
P3.34 H0 design now follows a complete P3.26-P3.33 comparison. P3.33 remains closed, healthy, consumed and never replayable after exact 1/1 transfers; its `73B/18bc55ba` RX proves native-PID1 banner plus CRC-valid pre-console `stage 0/code 0`, while exact host OPEN `32B/549e387c` received no `OPEN_PARSED`. Binary inspection corrects one earlier hypothesis: P3.31's resident loop and console were inlined into `p318_run`, while only P3.32/P3.33 retain independent console symbols, so out-of-line compilation alone cannot explain all failures. The missing discriminator is the first console call return. The new review-pending P3.34 runtime exact-loads P3.33, leaves the console body/protocol/commands/timeouts/two-session same-FD shape untouched, captures only the first outer call result, and encodes `0xB000|errno` or sentinel `0xBFFF` in the existing post-rollback checkpoint terminal detail. It adds no checkpoint generation, tty response, retry, reconnect, command or persistent write. Focused transform/encoding/mutation tests pass 4/4; actual P3.33 runtime `474779B/b90287b2` transforms to `475102B/bac655d6` under fresh run `c334...f6b`. Exact-toolchain cross-compile yields a static AArch64 ELF with no undefined symbols and preserves the P3.33 console symbol address, `0x858` size and complete disassembly byte-for-byte. No AP, ready manifest, D0, approval, F1 or device contact exists. Next is deterministic packaging that retains this equality and independent review, not another protocol change. See `docs/reports/S22PLUS_FYG8_P334_FIRST_READ_RETURN_CODE_DESIGN_H0_2026-09-04.md`.
