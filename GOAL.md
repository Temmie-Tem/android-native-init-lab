...
Next bounded unit: dormant H0 named read-only exploration core implemented in `s22plus_fyg8_readonly_exploration.py`. It preserves three-command authenticated sessions (identity / selected query / run nonce), permits only kernel, processes, mounts, memory and fixed UDC state, and accepts no caller command/path. Actual generated C allowlist matches the host across valid/invalid inputs; actual host-first authenticated PTY exchange transmits the selected memory query; focused tests6/6 pass. Device validator transformation is H0 only, not a rebuilt or flashed candidate. Fresh successor artifact, selected-action receipt parser, existing P335 lease/action integration adapted to host-first ordering, independent review and ordinary preparation/F1 approval remain pending. No current lease/device authority is activated and P342 cannot be reused. P3.42 is CLOSED/19, healthy, consumed and never replayable: `PASS_F1_V2_P342_AUTHENTICATED_IDLE_REUSE_AND_ROLLED_BACK`. Run `p342-ready1-prepared-20260905-2` completed candidate AP28631081B/76e23db4 and exact Magisk rollback23367721B/d2373bf8 once each (1/1). Accepted receipt35488B/ee91e9fa and RX2644B/ce3ce118 prove four authenticated sessions/twelve fixed commands, 120.000105308s silent same-FD idle, third-session reuse and one planned reopen/fourth session. Final rooted FYG8 health and identical logs were captured before a host classifier indentation error; independently reviewed retained-only finalization closed normally without new device effects. Result35666B/0136a663, state31802B/005635a1, recovery_required=false. The one-line permanent fix has an actual common-classifier regression (failed before, focused8/8 after). Supplemental Carrier remains NO_PROOF_OBSERVER; no scientific/causal claim, arbitrary shell, later-action lease, indefinite residency or autonomous mode control is promoted. P341 remains CLOSED/healthy/consumed with three sessions/nine commands and transfers1/1. Current report and canonical timeline: `docs/reports/S22PLUS_FYG8_P342_IDLE_REUSE_PREPARATION_2026-09-05.md`.
