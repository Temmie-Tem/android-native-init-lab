...
> **✅ POST-PROMOTION OBSERVABILITY RECORD (V2822–V2832, 2026-06-19): Tier-C audio status/selftest/screen UI/API observability is device-validated on `0.10.7`.** Artifact `workspace/private/inputs/boot_images/boot_linux_v2831_audio_profile_screen.img` (SHA256 `4edcc6bb871e71d6d25713811d956940e950c6ff0ba2d05a0500b13e1d95915c`, tag `v2831-audio-profile-screen`) keeps the promoted audio core, keeps the V2820 read-only `selftest verbose` audio row (`PASS audio ... boost=blocked sp=unverified`), keeps the V2822 `screenapp audio-status` summary, keeps the APPS/AUDIO menu page, keeps the V2828 route-map safety labels, and adds display-only `screenapp audio-profile` / APPS/AUDIO `PROFILE` for profile id, endpoint, PCM tuple, App Type Config tuples, ACDB SET order, and stage counts. V2829 validated `audio status` (16/16), `selftest verbose` audio markers (8/8), and `screenapp audio-map` markers (6/6) on-device; V2830 validated the callable read-only `audio profiles`, `audio profile`, `audio stages`, and `audio speaker-map` API surfaces (4/4, 21/21, 17/17, 19/19 markers) on the previous `0.10.6` candidate; V2832 validated `audio status` (16/16), `selftest verbose` audio markers (8/8), and `screenapp audio-profile` markers (6/6) on `0.10.7`. These runs rolled back to `v2321` with `selftest fail=0` and no recovery fallback. This is the latest observability candidate, not a rollback-net replacement.
