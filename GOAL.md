...
P3.32 is closed, consumed, and never replayable after exact candidate/rollback transfers `1/1`, a 19-record CLOSED journal and final healthy rooted FYG8 return. The operator reported normal candidate boot without a loop. Raw-first evidence retained the exact `49B/ac1cd943` P3.32 native-PID1 banner and one `32B/cfe6ca09` host OPEN frame, but session 1 timed out at `open-diagnostic-read` before diagnostics, challenge, HMAC, command, clean close or session 2; sessions attempted/successful/reconnected are `1/0/0`. Because P3.32 removed P3.31's physical reopen/reconnect and reproduced the same first-session boundary, reconnect is not supported as the sole necessary cause, but the remaining cause is unproved. Formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p332_authenticated_logical_resident_same_fd_unproved_rollback_verified`, state `32779B/7bf83fb1`, result `36558B/3a76c74e`, and `recovery_required=false`. The ordinary publisher exceeded only the 32 KiB intermediate-state bound by 11 bytes after CLOSED; an independently reviewed exact-run H0 finalizer kept the common bound unchanged and published the exact mode-0400 state/result with zero device/ADB/USB/Odin/transfer action. P3.30 remains the latest complete authenticated command proof. The proportional successor should isolate only the first-session entry regression with one small device-side pre-OPEN/OPEN diagnostic. See `docs/reports/S22PLUS_FYG8_P332_F1_SAME_FD_NO_PROOF_2026-09-04.md`.
