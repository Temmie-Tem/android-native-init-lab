...
P3.27 is independently reviewed `PASS_GO_P327_PROCESS_V2_H0` at H0 only. Fresh run ID `c327...3f3b`, fixed three-command `S327` frames, raw-first/trailing rejection, distinct common dispatch and byte-identical boot-only A/B AP `28631081B/024322fd` are bound by ready manifest `4105B/c5a4a4ab`; a real durable receipt round trip closes the initial reopen-hash blocker. No connected preparation, approval, device contact or live authority exists; next is one campaign-visible `p327-ready1-prepared-...` D0 prepare, and any later transferred run must add one post-`CAMPAIGN_CLOSED` P327 F1 ledger row without backfilling P320-P322; see `docs/reports/S22PLUS_FYG8_P327_FRAMED_EXEC_H0_2026-09-02.md`.
