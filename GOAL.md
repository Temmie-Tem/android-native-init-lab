...
P3.41 is ready for fresh attended F1 approval, not executed. Independent `PASS_GO_P341_H0` and canonical rehearsal passed; ready_1 is `12341B/bc498d9b`, builder -02 result `79393B/5733271e`, A/B boot-only AP `28631081B/714c6c25`, and exact Magisk rollback `23367721B/d2373bf8`. The host now sends the existing OPEN after raw setup, before the device banner; empty idle is silent and partial input is consumed without retry. Three authenticated sessions, fixed commands, deadlines and mandatory rollback remain; no later resident-action lease is activated. One preapproved D1 reboot ran but its final property capture was empty, so the primitive remains interrupted/consumed with no fabricated result or repeated reboot. Fresh D0 health `1516B/e5e0eb81` verified the same target and changed healthy rooted boot. Ordinary preparation in `p341-ready1-prepared-20260905-1` then passed with clean baseline `2097136B/2fce77d1`, preflight `3261B/ba258295`, prepared `28109B/ac1f7ab5`, approval digest `381707d5`; current prepared reopening and post-reboot boot continuity passed. Candidate/rollback are 0/0 and no F1 transaction exists. Stop before F1 until the operator returns that exact fresh code. P3.40 remains CLOSED/healthy/consumed at exact transfers 1/1, journal19, state `19204B/826c8958`, result `22241B/ed2d5d1d`, formal `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK`; its 193-byte diagnostic proves rejected header `S22PLUS-FYG8-E3:`, not definitive echo attribution. P340 and its approval are never replayable. See `docs/reports/S22PLUS_FYG8_P341_HOST_FIRST_OPEN_PREPARATION_2026-09-05.md`.
