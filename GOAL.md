...
The forward frontier has moved off the connector-side Max77705 USB2 MUX discriminator to the role chain: role request, UDC bind, DWC3 pull-up/connect, physical host attach. The stock-witness runtime/build closure is independently reviewed `PASS_GO`, H0-only; its `-24`/`-25` bookkeeping is superseded by the current Image-only provenance closure `-32`/`-33` and exact 73-row plan; no live authority exists.
