...
P3.32 is independently qualified `PASS_GO_P332_H0` and ready only for a fresh connected preparation. It tests one narrow correction to consumed P3.31: the candidate uses the exact proved P3.30 authenticated three-command runtime and performs two complete logical sessions on one already-open tty FD, with session two gated by session-one success and physical reopen/reconnect counts fixed at zero. Final boot-only A/B AP is `28631081B/e1309080`, exact rollback remains `23367721B/d2373bf8`, candidate-static is `31856B/85313bb9`, and ready manifest is `6827B/a95641eb`; builder/static/prepare audits, common host preflight, raw-first audit, focused suites and P3.30/P3.31 regressions pass. Independent review found and closed the sole success-type blocker (`False == 0` for physical reopen count) by requiring exact integer zero. No device, D0, approval, F1, transfer, resident install, PTY, arbitrary command or persistent authority was created. P3.30 remains the latest complete authenticated command proof; P3.31 remains closed, consumed and never replayable. Next is one fresh exact-target D0 prepare, then a separately attended F1 approval if the baseline passes. See `docs/reports/S22PLUS_FYG8_P332_LOGICAL_RESIDENT_SAME_FD_H0_2026-09-03.md`.
