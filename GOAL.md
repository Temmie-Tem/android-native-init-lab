...
The forward frontier has moved off the connector-side Max77705 USB2 MUX discriminator to the role chain: role request, UDC bind, DWC3 pull-up/connect, physical host attach. The reviewed stock-witness runtime/build closure includes the `-32`/`-33` Image-only predecessor and earlier `-24`/`-25` runtime closure; all remain preserved. The prior `-44`/`-45`/`-06` qualification is superseded by strict on-disk intent TOCTOU verification; current `-46`/`-47`/`-07` is independently reviewed H0-only `PASS_GO`, with no ready/run manifest or live authority.
