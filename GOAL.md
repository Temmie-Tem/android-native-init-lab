...
P3.38 is closed, healthy, consumed and never replayable with exact candidate/rollback transfers 1/1. Candidate raw `97B/78137243` proves the fresh native-PID1 banner, stage-0/code-0 console entry and the new stage-3 branch ordinal `1`, narrowing P3.37's undifferentiated `EPROTO` to `header-validation`: the fixed header read completed, then either base frame magic/version/maximum-length validation or parsed OPEN type/sequence/exact-length/run-ID validation rejected the input. No OPEN_PARSED, challenge, HMAC, command, clean close or resident session is proved. Recovery resumed only the durable journal after endpoint measurement stopped the first process, transferred exact Magisk rollback once, and verified rooted FYG8 Android health; journal is CLOSED at 19 records. A final host-only publication failure was caused solely by Python integer branch-map keys differing from their JSON string-key reopen. Independent `PASS_GO_P338_TERMINAL_REEMIT_H0` normalized that one projection without semantic change and emitted mode-0400/link-1 result `20258B/6525e96c`; permanent fix `6dd202876b` adds the JSON-roundtrip regression. Formal terminal is `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK` / `p338_authenticated_resident_open_read_branch_unproved_rollback_verified`, `recovery_required=false`. The proportional successor is one no-retry subreason split inside branch 1, not a wider protocol or another P3.38 attempt. See `docs/reports/S22PLUS_FYG8_P338_F1_OPEN_READ_BRANCH_NO_PROOF_2026-09-05.md`.
