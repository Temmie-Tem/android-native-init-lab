...
- **Active research cycle**: V1259 LIVE PASS → V1260 PMIC GPIO9 line-info gate — V1256 proved PM8150L gpiochip devnode feasibility read-only: `/proc/devices` has `254 gpiochip`; `/sys/bus/gpio/devices/gpiochip2/dev=254:2`; `/sys/class/gpio/gpiochip1263` labels PM8150L with `ngpio=11`; debugfs confirms `gpiochip2` range `1263-1273` and PMIC GPIO9 offset `7`. V1257 built `a90_android_execns_probe v262`; V1258 deployed it to `/cache/bin/a90_android_execns_probe`; V1259 executed only temporary `/dev/gpiochip2` devnode-open proof. Result: sysfs contract matched, temp `mknod` succeeded, read-only open succeeded, `GPIO_GET_CHIPINFO_IOCTL` succeeded (`chip_name=gpiochip2`, `chip_lines=11`), cleanup unlink succeeded, postflight selftest stayed `fail=0`, and all forbidden markers stayed zero. Next V1260 should prefer read-only `GPIO_GET_LINEINFO_IOCTL` for PMIC GPIO9 offset `7` before any GPIO line request or PMIC GPIO9 hold. Still no PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
