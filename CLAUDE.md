...
- **Active research cycle**: V1245 HOST-ONLY PASS → V1246 same-run stack+power sampler or PMIC pinctrl setup planned — V1245 added `scripts/revalidation/native_wifi_pmic_gdsc_reachability_classifier_v1245.py` and tied V918/V1243/V1244 evidence together. Native can reach the proprietary SDX50M soft-reset stack (`sdx50m_toggle_soft_reset -> mdm4x_do_first_power_on -> mdm_cmd_exe -> mdm_subsys_powerup`), and current native V1243 reaches `pm-service` `/dev/subsys_esoc0`; however PM8150L soft-reset `gpio9` remains `MUX UNCLAIMED`, PCIe GDSC lines remain `0mV`, GPIO142 IRQ remains `0`, PCI/MHI remain absent, and `wlan0` remains absent. Android-positive evidence claims PM8150L `gpio9` as output and reaches PCIe RC1/WLAN-PD/ICNSS-QMI/FW-ready/`wlan0`. The remaining gap is inside or immediately after the proprietary soft-reset/power path, before Android-equivalent PMIC/GDSC/GPIO142/PCIe response becomes visible. V1246 should capture soft-reset stack plus PMIC/GDSC response in one bounded run or reproduce Android PMIC pinctrl setup before another bounded `esoc0` trigger; still no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
