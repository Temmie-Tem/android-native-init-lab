...
- **Rollback checkpoint (resident, promoted V2316): `A90 Linux init 0.9.280 (v2316-usb-linux-identity)`** — image `workspace/private/inputs/boot_images/boot_linux_v2316_usb_linux_identity.img`, SHA256 `cf54ff0ae3cca4af31263140e588920296abecdb0ffb690a807b3d8b393f452a`. Carries the full V2313–V2315 USB control surface + the WLAN event epic; redacts the real device serial to placeholder `A90NATIVE001` and sets honest device-side gadget strings (`A90 NativeInit` / `A90 Linux (ARM)`). Live this cycle: `version/status/selftest fail=0`, `usb status` `control.ok=1`, USB persona `expose`/`remove` regression PASS with the identity persisting across the reconfigure, serial redaction confirmed host-side. **Caveat — Wi-Fi:** the Wi-Fi stack is **byte-identical** to v2237 (V2313–V2316 changed only USB gadget + identity strings; zero Wi-Fi code change) and live readiness was confirmed (`wlan0` present, supplicant root-exec ok, NCM/tcpctl transport ready), but **both-band connect→DHCP→ping was NOT re-run this cycle** (host↔device NCM-link carrier did not come up for creds staging — a host-plumbing blocker, not a device/Wi-Fi regression); the Wi-Fi rollback-grade guarantee therefore rests on the unchanged-stack argument + v2237 lineage. **Caveat — identity:** host-visible USB `iManufacturer`/`iProduct` remain Samsung-forced by the kernel gadget driver (configfs strings are stored but not presented on the wire); only `serialnumber` is host-controllable from userspace. **`A90 Linux init 0.9.268 (v2237-supplicant-terminate-poll)`** (image `workspace/private/inputs/boot_images/boot_linux_v2237_supplicant_terminate_poll.img`, SHA256 `b2ea2d26d160b7702ce7d4438b84367788eea26c6a5bbe4ed93f3d270292ac7f`) is **retained as the deeper, fully-Wi-Fi-proven known-good fallback** (`version/status/selftest` clean, both-band connect→DHCP→ping validated in lineage).
...
- **Phase status (2026-06-14):** the WLAN kernel-interface event epic is closed at V2312. USB gadget runtime control layer ① is closed at V2315: U1 (`usb status`) at V2313, U2 (`usb mass-storage add/remove`) at V2314, and U3 read-only mass-storage persona end-to-end at V2315. V2316 promoted the resident rollback checkpoint (`0.9.280 (v2316-usb-linux-identity)`): redacted the real device serial → placeholder and set honest device-side gadget strings. **Finding (V2316):** the boot-time gadget identity is written by the prebuilt `a90_usbnet` helper (bundled in the ramdisk from `workspace/private/inputs/external_tools/userland/bin/a90_usbnet-aarch64-static`), so changing `a90_usb_gadget.c`/`a90_usbnet.c` source requires **recompiling that prebuilt** for the change to take effect. The host-visible `iManufacturer`/`iProduct` are Samsung-forced by the kernel gadget driver and are not changeable from userspace (only `serialnumber` is). Do not start adb-over-ffs layer ② or HID/BadUSB layer ③ without a new explicit goal. Kernel-observation reached exact KASLR slide (V2216 codeword-exact); ROPP full-stack symbolization needs the RKP-protected per-boot key = out of read-only scope. Kernel-security recon is also closed below.
