...
- **Active research cycle**: V1418 rollbackable live handoff PASS for procedure/downstream RC1, still BLOCKED for Wi-Fi connect readiness (`v1418-test-boot-downstream-progress-rollback-pass`). The same V1414 delayed-RC1 test boot was rerun with expanded dmesg capture. It now proves the corrected RC1 path executes assert reset, `PCIE20_PARF_INT_ALL_MASK=0x7f80c202`, PHY ready, release reset, and LTSSM, with `esoc0_to_assert` about `0.277s` versus Android about `0.255s`. RC1 still fails in `LTSSM_POLL_COMPLIANCE` before L0; no MHI/WLFW/BDF/FW-ready/`wlan0`, `connect_ready=false`. Rollback verified v724/selftest fail=0. Next V1419 should be host/source-only endpoint-readiness design after PERST release, likely a read-only GPIO142/interrupt sampler around the RC1 window and Android V852 timing comparison. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
