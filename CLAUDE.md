...
- **Current validated test artifact (resident after V2313 live validation): `A90 Linux init 0.9.277 (v2313-usb-status-inventory)`** — opens the USB gadget runtime-control epic with a read-only `usb status` command. Live flash/status/selftest passed; `usb status` over the serial bridge reported configfs gadget `g1` bound to UDC `a600000.dwc3`, VID/PID `04e8:6861`, config `b.1`, linked control functions `ncm.usb0` (`f2`) and `acm.usb0` (`f1`), `control.ok=1`, serial redacted, and `mutation_attempted=0`. This is the current validated test baseline, but **not** the safety rollback baseline.
