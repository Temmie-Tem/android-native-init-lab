...
- **Active research cycle**: V1243 LIVE PASS → V1244 Android-positive power-surface comparison planned — V1243 deployed helper `a90_android_execns_probe v259` and repeated the bounded late `per_proxy` trigger with source-aware pinctrl/regulator sampling. `pm-service` reached `/dev/subsys_esoc0`, but GPIO142 IRQ stayed `0`, `mdm3` stayed `OFFLINING`, PCI/MHI stayed absent, and `wlan0` never appeared. The corrected PM8150L soft-reset observer stayed `pin 7 (gpio9): (MUX UNCLAIMED)` and PCIe GDSC lines stayed at `0mV` across all samples. The remaining gap is below Binder/peripheral-manager delivery and inside/after `mdm_subsys_powerup`, before Android's PMIC/GDSC/GPIO142/PCIe RC1/SSCTL/MHI/WLFW response. V1244 should compare the same PMIC soft-reset and PCIe GDSC surfaces against Android-positive evidence or prove whether the proprietary eSoC powerup path reaches those operations; still no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
