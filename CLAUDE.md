...
- **Active research cycle**: V1303 PASS → V1304 AP2MDM/MDM2AP response classifier — V1303 proved the deployed helper v273 marker works live. `powerup_marker` covered all 42 compact samples, with `max_powerup_thread_count=1`, `first_syscall_name=openat`, `first_path=/dev/subsys_esoc0`, and `first_wchan=mdm_subsys_powerup`. GPIO142/MHI/WLFW/`wlan0` stayed absent (`max_mdm_status_count_total=0`, `max_mhi_bus_count=0`). GPIO observations include `gpio135 : out 0 16mA no pull` and `gpio142 : in  0 8mA no pull`. Post-run status/selftest are healthy. Next gate is V1304 host-only/read-only AP2MDM/MDM2AP response classification. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
