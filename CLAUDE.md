...
- **Active research cycle**: V1442 micro endpoint artifact sanity PASS (`v1442-wifi-test-boot-micro-endpoint-artifact-sanity-pass`). V1441 built a rollbackable Wi-Fi test boot with a concurrent corrected-RC1 writer plus minimal fast reader for selected interrupts, exact endpoint GPIOs, and pcie1 link-state files. V1442 verifies the exact V1441 manifest, static binaries, marker contract, v724 header/kernel parity, private modes, and forbidden credential-like byte absence. Next gate may be V1443 rollbackable live handoff of only the V1441 test image, with mandatory rollback to v724 and selftest fail=0 verification. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
