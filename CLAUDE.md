...
- **Active research cycle**: V1276 HOST-ONLY PASS → V1277 helper v267 source/build-only — V1275 bounded block sampler shows PMIC GPIO1270 block present in 14/14 samples and PMIC GPIO9 line text as `out ... high ...`, while GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`. Cleanup reboot restored healthy v724 with selftest `fail=0`. V1276 compared this with Android/reference evidence and classified PMIC GPIO9 as matching Android `out/high`; PMIC GPIO9 write/hold is rejected. Remaining gate is downstream TLMM GPIO135/GPIO142 visibility and PCIe/SDX50M response after PM-service enters `mdm_subsys_powerup`. Next selected gate is V1277 source/build-only helper v267 for read-only TLMM GPIO range-slice capture around GPIO135/GPIO142 plus AP2MDM/MDM2AP pinmux/pinconf and PCIe RC1/GDSC snapshots. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
