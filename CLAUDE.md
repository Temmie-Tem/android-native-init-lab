...
- **Active research cycle**: V1298 PASS → V1299 compact dense live sampler — V1298 deployed static helper `a90_android_execns_probe v272` (`1344b4ac101aa0cde56a46f1274b2d01f25d11b424158d822bff71234a1e7885`) to `/cache/bin/a90_android_execns_probe` using serial fallback. Remote sha/marker/compact flag and postflight selftest `fail=0` are confirmed. Next gate is V1299 bounded compact dense live sampling. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
