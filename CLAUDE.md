...
- **Active research cycle**: V1282 DEPLOY PASS → V1283 bounded PCIe/GDSC/kmsg sampler live — V1279 confirmed TLMM `gpiochip0` range `0-174` is visible and GPIO135/GPIO142 pinmux ownership belongs to `soc:qcom,mdm3`, but exact GPIO135/GPIO142 debugfs value lines remain absent. GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`; post-run health check returned v724 selftest `fail=0`. V1280 selected PCIe/GDSC/RC1 response sampling as the next gate. V1281 built helper v268 with read-only `/dev/kmsg` filtered counts; V1282 deployed it to `/cache/bin/a90_android_execns_probe` by serial fallback and verified remote SHA plus selftest `fail=0`. Next selected gate is V1283 bounded live PCIe/GDSC/kmsg sampler. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
