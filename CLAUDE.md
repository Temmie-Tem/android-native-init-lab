...
- Audio tinyalsa inventory latest state: V2355 used the exact AUD-3C approval, flashed V2334, accepted one ADSP boot, reproduced `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`), and proved the V2354 NCM re-enumeration repair works by restoring host ping and tcpctl after candidate USB re-enumeration. Tinyalsa inventory still did not execute: `install-tinymix` failed before tool transfer because `tcpctl_host.py install` used its stale default `/cache/bin/toybox`, while the native-init runtime has `/cache/bin/busybox`, `/bin/busybox`, and `/bin/toybox` but no `/cache/bin/toybox`; rollback to V2321 succeeded with `selftest fail=0`. V2356 is the host-only repair: the V2349 runner now passes `tcpctl_host.py --toybox /cache/bin/busybox` for tool staging and tcpctl-run checks, and dry-run/tests assert `/cache/bin/toybox` is absent. The next safe unit is a fresh exact-gated live AUD-3C inventory attempt with this toolbox-path runner; do not proceed to `tinyplay` or mixer writes.
