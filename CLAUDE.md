...
- **Active research cycle**: V1415 local artifact sanity PASS (`v1415-wifi-test-boot-delayed-rc1-artifact-sanity-pass`) for the exact V1414 delayed-RC1 test boot. The staged image is `A90 Linux init 0.9.75 (v1414-wifitest)` at `tmp/wifi/v1414-wifi-test-boot-delayed-rc1/boot_linux_v1414_wifi_test.img`; checks passed for manifest decision, static PID1/helper, ramdisk entries, boot markers including `delay_ms=%d`, the `250ms` RC1 watcher delay contract, v724 header/kernel parity, forbidden credential-like byte absence, and private modes. Next is V1416 rollbackable live handoff of only this V1414 test image. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
