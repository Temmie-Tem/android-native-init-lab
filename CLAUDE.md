...
- **Active research cycle**: V1420 source/build-only PASS (`v1420-wifi-test-boot-rc1-window-sampler-source-build-pass`). V1420 produced a rollbackable test boot artifact with native init `0.9.76 (v1420-wifitest)`, preserving the V1414 `250ms` delayed corrected-RC1 path while adding a read-only PID1 RC1-window sampler for `/proc/interrupts`, GPIO142/`mdm status`, and GPIO 102/104/135/142 debugfs/pinctrl excerpts at pre-delay, pre-RC1, +50ms, +150ms, and +500ms. Next gate is V1421 artifact sanity, then V1422 rollbackable live handoff only if sanity passes. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, pci-msm `case` writes beyond the generated corrected-RC1 test path, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
