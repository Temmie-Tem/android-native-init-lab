...
- **Active research cycle**: V1449 provider-vs-RC1 timing classifier PASS (`v1449-provider-precedes-rc1-case-no-l0`). V1447/V1448 proved case-aligned RC1 samples still had GPIO135/GPIO142 low and no L0/MHI/WLFW/`wlan0`. V1449 shows `__subsystem_get: esoc0` occurred before the explicit RC1 debugfs `case=11` trigger: esoc0 at `9.243453s`, RC1 `TEST: 11` at `9.520422s`, link fail at `9.635246s`. Therefore V1447 sampled post-RC1-case state, not the provider transition itself. Next gate should be V1450 source/build-only provider-trigger micro sampler keyed on `__subsystem_get: esoc0`/`mdm_subsys_powerup`. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
