...
- **Active research cycle**: V1297 BUILD PASS → V1298 deploy helper v272 — V1297 added `--pm-observer-late-per-proxy-compact-response-sampler` and built static helper `a90_android_execns_probe v272` (`1344b4ac101aa0cde56a46f1274b2d01f25d11b424158d822bff71234a1e7885`). This closes the V1296 stdout-cap blocker by adding a compact no-write dense sampler below the helper output budget. Next gate is V1298 deploy-only, then V1299 bounded compact dense live sampling. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
