...
- **Active research cycle**: V1257 SOURCE/BUILD PASS → V1258 helper v262 deploy-only — V1256 proved PM8150L gpiochip devnode feasibility read-only: `/proc/devices` has `254 gpiochip`; `/sys/bus/gpio/devices/gpiochip2/dev=254:2`; `/sys/class/gpio/gpiochip1263` labels PM8150L with `ngpio=11`; debugfs confirms `gpiochip2` range `1263-1273` and PMIC GPIO9 offset `7`. V1257 built `a90_android_execns_probe v262` with `wifi-companion-pmic-gpiochip-devnode-open-preflight` / `--allow-pmic-gpiochip-devnode-open-preflight` to validate that contract, create a temporary future live `/dev/gpiochip2` char node, open it read-only, run `GPIO_GET_CHIPINFO_IOCTL`, close it, and unlink it. V1257 did not deploy or contact the device. Next V1258 is deploy-only; V1259 is the bounded live devnode-open proof. Still no GPIO line request, PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
