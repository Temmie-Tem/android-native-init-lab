...
- **Active research cycle**: V1432 host-only classifier PASS (`v1432-ap-rc1-prereqs-toggle-but-endpoint-no-l0`). V1431 test boot ran and rolled back to v724/selftest fail=0. V1432 classifies the endpoint window: AP-side pcie1 GDSC/clocks do toggle in the corrected-RC1 window (`pcie_1_gdsc` enable 0 -> 1 -> 0, pcie1 clocks enabled at `pre_rc1`, GPIO103/CLKREQ high/pull-up), but the endpoint still fails before L0 with no MHI/WLFW/BDF/FW-ready/`wlan0`. Next gate should stay host/source-only first: either refine native sampler output to avoid clock-summary truncation and emit exact pcie1 clock/GDSC/PERST/CLKREQ fields, or capture an Android-side pcie1 clock/GDSC/CLKREQ reference for the known-good L0 path. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
