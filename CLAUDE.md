...
- **Active research cycle**: V1440 immediate endpoint classifier PASS (`v1440-immediate-sampler-too-slow-no-l0`). V1439 rollbackable live handoff of the V1437 image passed and returned to healthy v724/selftest, but Wi-Fi remains below userspace: corrected RC1/LTSSM still fails before L0 and MHI/WLFW/BDF/FW-ready/`wlan0` remain absent. V1440 shows why the V1437 immediate exact sampler is not enough: debugfs regulator/clock scans are slower than the RC1 active window (`after_case_1ms` appeared at `2402ms`, `after_case_20ms` at `8634ms`). Next gate should be V1441 source/build-only: a micro-sampler with concurrent writer plus minimal fast reader, avoiding full debugfs summary scans in the active RC1 window. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
