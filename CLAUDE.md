...
- **Active research cycle**: V1309 PASS → V1310 classify exact safe lower prerequisite / reduce sampler stdout — V1309 ran helper `a90_android_execns_probe v274` focused mode `late-per-proxy-focused-pmic-gdsc-transition`. The helper stdout hit the existing `1MiB` cap, so the result is a partial-window PASS: `76` focused samples covered `pm-service` reaching `/dev/subsys_esoc0` and blocking in `mdm_subsys_powerup`, while GPIO135/GPIO142 stayed low, PM8150L soft-reset stayed `MUX UNCLAIMED`, PCIe0/PCIe1 GDSCs stayed at `0mV`, and MHI/`ks`/`wlan0` stayed absent. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
