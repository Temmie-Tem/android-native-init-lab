...
- **Active research cycle**: V1260 SOURCE/BUILD PASS → V1261 helper v263 deploy-only — V1259 executed only temporary `/dev/gpiochip2` devnode-open proof. Result: sysfs contract matched, temp `mknod` succeeded, read-only open succeeded, `GPIO_GET_CHIPINFO_IOCTL` succeeded (`chip_name=gpiochip2`, `chip_lines=11`), cleanup unlink succeeded, postflight selftest stayed `fail=0`, and all forbidden markers stayed zero. V1260 built `a90_android_execns_probe v263` with `wifi-companion-pmic-gpiochip-line-info-preflight` / `--allow-pmic-gpiochip-line-info-preflight`. The new mode reuses the temporary devnode path and adds only read-only `GPIO_GET_LINEINFO_IOCTL` for PMIC GPIO9 offset `7`, printing line flags/name/consumer while keeping GPIO line request and PMIC write blocked. Next V1261 is deploy-only; V1262 is bounded live line-info proof. Still no PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
