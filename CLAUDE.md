...
- **Active research cycle**: V1296 PASS → V1297 compact dense sampler build — V1296 classified V1295's `14/40` dense sample result as helper stdout cap truncation, not a proven runtime stop: `A90_EXECNS_STDOUT_END truncated=1 bytes=1048576` appears during `late_per_proxy_poll_13`, with `response_sampler.end` absent and helper/cmdv1 exit still `rc=0`. Next gate is source/build-only V1297 compact/file-backed dense sampling so the same bounded PM-service path can capture the full 40-sample window. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
