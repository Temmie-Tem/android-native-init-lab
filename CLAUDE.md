...
- **Active research cycle**: V1416 rollbackable live handoff PASS for procedure/downstream RC1, still BLOCKED for Wi-Fi connect readiness (`v1416-test-boot-downstream-progress-rollback-pass`). V1414 booted, the `/proc/kmsg` PID1 watcher delayed corrected RC1 by `250ms`, and rollback verified v724/selftest fail=0. Timing is now near Android reference: `esoc0_to_test11` about `0.275s` vs Android `esoc0_to_assert` about `0.255s`, but RC1 still fails in `LTSSM_POLL_COMPLIANCE` before L0 with no MHI/WLFW/BDF/FW-ready/`wlan0`; `connect_ready=false`. Next is V1417 host-only comparison of V1416/V1413/Android RC1 semantics, including reset/PERST/refclk markers. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
