...
- **Active research cycle**: V1426 local artifact sanity PASS (`v1426-wifi-test-boot-rc1-retry-artifact-sanity-pass`). The exact V1425 rollbackable test boot artifact passed manifest/hash/static/header/marker/credential-scan/private-mode checks. Next gate is V1427 rollbackable live handoff for `tmp/wifi/v1425-wifi-test-boot-rc1-retry/boot_linux_v1425_wifi_test.img`, collecting the V1425 log, summary, RC1 watcher result, RC1-window result, expanded dmesg markers, and `wlan0` state before mandatory rollback to v724 and selftest verification. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside this explicit test-boot/rollback gate, boot image write outside this explicit test-boot/rollback gate, or partition write.
