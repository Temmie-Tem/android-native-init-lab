...
- **Active epic — internal audio (ADSP/Q6) feasibility (V2324–V2330, 2026-06-14):** research whether the internal speaker/headphone path can be driven under native init (speaker *playback only*; modem/call audio and `q6voice` are out of scope). See `GOAL.md` active-epic block. AUD-0 (V2324, host-only) inventory: audio drivers are **built into the stock kernel image** (no vendor `.ko` to insert), and the ADSP firmware (`ADSP.MDT` + sparse `.b0x` segments), 33 `.acdb` calibration files, `mixer_paths_{pahu,tavil}.xml`, and the heavy Binder/HIDL `audio.primary.msmnile.so` HAL (itself libtinyalsa-based) all exist in the stock vendor image → **not non-viable**; mainline q6 path rejected (needs a kernel we do not run). New on-device read-only `audio adsp-status` (V2326) plus a heavily-gated one-shot `audio adsp-boot-once AUD2_ONE_SHOT_ADSP_BOOT` (token + firmware preflight; writes `1` to `/sys/kernel/boot_adsp/boot` once) (V2327) were exercised live under explicit per-run operator approval (V2328, V2330). **No ADSP boot has been triggered yet** — every live preflight self-blocked before the write (`activation_write_attempted=0`), then rolled back to V2321 with `selftest fail=0`. Current blocker (V2330): the effective kernel `firmware_class.path` is `/mnt/vendor/firmware` (set by the Wi-Fi feeder) and does **not** expose the ADSP files present under `/vendor/firmware_mnt/image`; the gate refuses until `audio.firmware_class.adsp_complete=yes`. Next safe unit = host/source-side serve-path fix to expose the complete sparse ADSP set on the effective firmware_class path before any AUD-2 retry (no partition writes). Latest audio test build: `0.9.290 (v2329-audio-adsp-fw-preflight)`, image `boot_linux_v2329_audio_adsp_fw_preflight.img`, SHA256 `8ab5ae3bbd750ee871c52b040905652683894f9816e7c4e29bfae2b2695638d8`. **Device is currently resident on the V2321 rollback checkpoint.**
