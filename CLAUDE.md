...
- **Active research cycle**: V1423 host-only classifier PASS (`v1423-gpio135-low-is-not-actionable-by-itself`). V1422 proved corrected RC1 timing/reset/release execution but still failed before L0; GPIO135/AP2MDM stayed low, GPIO142/MDM2AP stayed low, `mdm status` IRQ stayed `0`, and MHI/WLFW/BDF/FW-ready/`wlan0` stayed absent. V1423 compares Android evidence and classifies GPIO135 low as non-actionable by itself because Android-positive steady-state captures also show GPIO135 low; the actionable delta is native's missing GPIO142 IRQ/L0/MHI/WLFW response. Next gate should stay below connect: either host-only Android-vs-V1422 timing comparison or source/build-only higher-frequency read-only RC1/interrupt sampler. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, pci-msm `case` writes beyond approved test artifacts, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
