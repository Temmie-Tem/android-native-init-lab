...
- **Active research cycle**: V1271 LIVE PASS → V1272 host-only observer classifier — V1271 used helper v265 in the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. It preserved the V1267 line-info result: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, zero forbidden action markers, GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. New V1271 value/power evidence: debugfs GPIO was mounted/readable, PMIC GPIO9/TLMM GPIO135/GPIO142 pinctrl surfaces were present, PCIe GDSC lines remained `0mV`, but exact debugfs-gpio value lines for global GPIO1270 / TLMM GPIO135 / TLMM GPIO142 were not found. Cleanup reboot restored healthy v724 with selftest `fail=0`. Next gate is a host-only classifier for a broader read-only debugfs GPIO/pinconf block sampler. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
