...
- **Active research cycle**: V1422 rollbackable live handoff PASS (`v1422-test-boot-downstream-progress-rollback-pass`). V1420 test boot reached corrected RC1 with Android-like timing, collected all five RC1-window samples, then rolled back to v724 with selftest fail=0. Evidence classifies the current blocker as `rc1-ltssm-link-failed-no-l0`: GPIO135/AP2MDM stayed `out 0`, GPIO142/MDM2AP stayed `in 0`, `mdm status` IRQ stayed `0`, and MHI/WLFW/BDF/FW-ready/`wlan0` stayed absent. Next gate should be V1423 host-only/read-only classification against Android positive evidence to determine whether GPIO135 is active-low/pulsed too briefly or whether native never asserts the comparable AP2MDM condition. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, pci-msm `case` writes beyond approved test artifacts, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
