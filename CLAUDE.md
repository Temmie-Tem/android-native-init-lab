...
- Audio tinyalsa inventory latest state: V2350 used the exact AUD-3C approval and ran the V2349 live inventory route twice. Both attempts flashed V2334, reproduced ADSP + `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`), then rolled back to V2321 with `selftest fail=0`; no `tinyplay`, mixer write, PCM playback/write, audio HAL, or adsprpc path ran. Tinyalsa inventory itself remained blocked before execution: first by a disallowed install root, then by a `tcpctl_host.py install` timeout connecting to `192.168.7.2:2325`. V2351 is a host-only runner repair: tools now stage as `/cache/bin/tinymix` and `/cache/bin/tinypcminfo`, the live path probes host NCM and `tcpctl` before install, and `--inventory-transport auto` selects `tcpctl` when ready or serial/bridge fallback when only NCM is ready. The next safe unit is a fresh exact-gated live AUD-3C inventory attempt with this readiness/fallback runner.
