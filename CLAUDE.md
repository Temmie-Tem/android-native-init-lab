...
- **Active research cycle**: V1300 PASS → V1301 compact powerup marker build — V1300 corrected the V1299 manifest interpretation. V1299 completed the full compact dense window with no stdout truncation, and the transcript still contains `/dev/subsys_esoc0` / `mdm_subsys_powerup` evidence (`path_esoc0=2`, `wchan_powerup=13`). The false negative came from fd-only compact classification: a blocked `openat("/dev/subsys_esoc0")` does not create a visible completed fd, and the compact loop omitted repeated syscall/kmsg probes. Next gate is source/build-only V1301 adding a compact per-sample powerup/path marker without restoring verbose blocks. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
