...
- **Active research cycle**: V1307 PASS → V1308 deploy helper v274 only — V1307 updated `a90_android_execns_probe.c` to helper marker `v274` and added `--pm-observer-late-per-proxy-pmic-gdsc-transition-sampler`. The new focused mode is `late-per-proxy-focused-pmic-gdsc-transition`, intended for `80` samples at `50ms`, emitting reduced no-write lower-surface fields: AP2MDM/MDM2AP target lines, PM8150L soft-reset source/line, PCIe0/PCIe1 GDSC source/line, PCI/MHI counts, MHI pipe, `ks`, `wlan0`, and safety zeros. Static aarch64 build PASS, sha256 `eb96072631ca38c3296f5da1756a93765e198e8fdd4dc010d087bc4b3b5fc180`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
