...
- Audio route latest state: V2360 reconciled V2359 read-only tinyalsa inventory with private `mixer_paths_tavil.xml`, `audio_platform_info.xml`, `audio_policy_configuration.xml`, and staged tinyalsa source. `deep-buffer-playback headphones` is a clean control match (`SLIMBUS_0_RX Audio Mixer MultiMedia1 = 1`) and card0/device0 supports 48 kHz stereo PCM, but A90 5G has no 3.5 mm jack, so this is not a reliable audible-output proof under the current USB-gadget control setup. The internal-speaker path is not ready: `SND_DEVICE_OUT_SPEAKER` maps to `SEC_TDM_RX_0`, while the XML speaker route references `SLIMBUS_0_RX` and `SpkrLeft*` / `SLIM_0_RX*` controls missing by exact name from V2359 tinymix; live `SEC_TDM_RX_*`, `WSA_*`, `COMP*`, and `RX INT7*` controls need host-only reconciliation. Playback remains unproven; do not run `tinymix set`, PCM playback opens, or `tinyplay` without a fresh exact playback gate.
