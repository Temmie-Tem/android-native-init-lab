...
- **Active research cycle**: V1275 LIVE PASS → V1276 host-only PMIC polarity/value classifier — V1271 used helper v265 in the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. It preserved the V1267 line-info result: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, zero forbidden action markers, GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. V1273 built helper v266, SHA256 `3bf4105d685f023ccdeb75ae28d7d104ca005fc9f70870dc6f402a9ea4038ed4`, and V1274 deployed it. V1275 bounded block sampler shows PMIC GPIO1270 block present in 14/14 samples and PMIC GPIO9 line text as `out ... high ...`, while GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`. Cleanup reboot restored healthy v724 with selftest `fail=0`. Next classify PMIC GPIO9 polarity/value versus Android/DTS before any write-side experiment. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
