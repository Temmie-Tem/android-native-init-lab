...
- **Active research cycle**: V1421 local artifact sanity PASS (`v1421-wifi-test-boot-rc1-window-sampler-artifact-sanity-pass`). The exact V1420 rollbackable test boot artifact passed manifest/hash/static/header/marker/credential-scan/private-mode checks. Next gate is V1422 rollbackable live handoff for `tmp/wifi/v1420-wifi-test-boot-rc1-window-sampler/boot_linux_v1420_wifi_test.img`, collecting the V1420 log, summary, RC1 watcher result, RC1-window result, expanded dmesg markers, and `wlan0` state before mandatory rollback to v724 and selftest verification. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, pci-msm `case` writes beyond the generated corrected-RC1 test path, global PCI rescan, platform bind/unbind, flash outside this explicit test-boot/rollback gate, boot image write outside this explicit test-boot/rollback gate, or partition write.
