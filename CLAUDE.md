...
- **Active research cycle**: V1411 source/build PASS (`v1411-wifi-test-boot-kmsg-fallback-source-build-pass`). V1411 stages `tmp/wifi/v1411-wifi-test-boot-kmsg-fallback/boot_linux_v1411_wifi_test.img` with `A90 Linux init 0.9.74 (v1411-wifitest)`. The PID1 RC1 watcher now falls back from absent `/dev/kmsg` to `/proc/kmsg` and drains existing records before watching future `esoc0`/powerup markers. No device command or flash occurred. Next is V1412 local artifact sanity over the exact V1411 manifest/image before any rollbackable live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
