...
- **Active research cycle**: V1409 local artifact sanity PASS (`v1409-wifi-test-boot-pid1-rc1-watcher-artifact-sanity-pass`). The exact V1408 boot image `tmp/wifi/v1408-wifi-test-boot-pid1-rc1-watcher/boot_linux_v1408_wifi_test.img` passed static/header/kernel/ramdisk/marker/forbidden-byte/private-mode checks. Next live gate may flash only that V1408 test image, expect `A90 Linux init 0.9.73 (v1408-wifitest)`, collect V1408 log/summary/RC1 watcher result/dmesg/`wlan0`, then roll back to `stage3/boot_linux_v724.img`. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
