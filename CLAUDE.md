...
- **Active research cycle**: V1436 focused endpoint classifier PASS (`v1436-focused-window-race-endpoint-no-l0`). V1435 proved the V1433 test boot reaches corrected RC1/LTSSM but still fails before L0; MHI/WLFW/BDF/FW-ready/`wlan0` remain absent and rollback returned to healthy v724/selftest. V1436 classified the focused evidence: GPIO103/CLKREQ stays high, GPIO142/MDM2AP stays low, and same-window `pre_rc1` pcie1 GDSC/clock reads are timing-sensitive because broad reads saw enable activity while later focused exact reads had them off. Next gate should be V1437 source/build-only: a tighter in-PID1 around-write sampler, or Android positive reference capture, before any new live mutation. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
