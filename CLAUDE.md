...
- **Active research cycle**: V1266 DEPLOY PASS → V1267 bounded read-only ext-mdm/AP2MDM observer — V1262 proved PMIC GPIO9 offset `7` line-info: flags `0x1`, `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. V1263 rejected direct userspace PMIC GPIO9 line request/hold as the next safe path, and V1264 fixed the next observer contract. V1265 bumped `a90_android_execns_probe` to v264 and extended the existing late `per_proxy` / PM-service response sampler with read-only PMIC GPIO9 `GPIO_GET_LINEINFO_IOCTL` snapshots in the same `/dev/subsys_esoc0` window. V1266 deployed helper v264 to `/cache/bin/a90_android_execns_probe` with SHA256 `a06ff29245023c265c69e58e2ae3f32a4facbc291bcb63a4450f39efd9515dc5`; post-deploy selftest remains `fail=0`. The blocker remains below the reproduced PM-service entry: native reaches `mdm_subsys_powerup`, but does not get Android's downstream GPIO142/PCIe/MHI/WLFW response. V1267 should run the bounded read-only observer and parse `gpiochip_lineinfo_*` fields from the same response samples. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
