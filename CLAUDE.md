...
- **Active research cycle**: V1444 micro endpoint classifier PASS (`v1444-micro-sampler-case-write-late-no-l0`). V1443 rollbackable live handoff of the V1441 image passed and returned to healthy v724/selftest. It still fails before `LTSSM L0` with no MHI/WLFW/BDF/FW-ready/`wlan0`. V1444 shows the V1441 micro reader started before the actual corrected RC1 `case=11` write completed: case elapsed `7790ms`, micro start `7675ms`, only `micro_after_case_150ms` landed after case, and GPIO135/GPIO142 stayed low in samples. Next gate should be V1445 source/build-only: make the writer signal after `case=11` returns, then start the parent micro samples at `0ms` through `150ms` after confirmed case completion. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
