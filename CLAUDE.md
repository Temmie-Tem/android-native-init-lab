...
- **Active research cycle**: V1407 host-only classifier PASS (`v1407-test-boot-rc1-trigger-still-late-no-l0`). V1406 proved the V1404 debugfs-prepared test boot can mount debugfs from PID1 and reach corrected RC1 enumerate (`TEST: 11`), but V1407 shows the trigger still lands in the same late class as V1391: about `3.598s` after `esoc0`, versus Android's `0.255s`. RC1 then fails before L0, with no MHI/WLFW/BDF/FW-ready/`wlan0`; `connect_ready=false`. Next is V1408 source/build-only: split corrected RC1 into a tiny PID1-started parallel watcher with no service snapshots, then verify the artifact before any rollbackable live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
