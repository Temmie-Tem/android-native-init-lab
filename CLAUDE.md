...
- **Active research cycle**: V1431 rollbackable live handoff PASS (`v1431-test-boot-downstream-progress-rollback-pass`). V1429 test boot ran and rolled back to v724/selftest fail=0. It still failed before PCIe L0 (`rc1-ltssm-link-failed-no-l0`) with no MHI/WLFW/BDF/FW-ready/`wlan0`, but the endpoint sampler worked: it captured `read-only-v1429-endpoint-prereq`, five window samples, GPIO103/CLKREQ high/pull-up, `pcie_1_gdsc`/pcie1 clock enable during `pre_rc1`, and clocks/GDSC off again after link failure. Next gate should classify V1431 endpoint evidence before another live mutation; likely refine the sampler or compare Android-side pcie1 clock/GDSC/CLKREQ parity. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
