...
- **Active research cycle**: V1274 DEPLOY PASS → V1275 bounded AP2MDM block sampler — V1271 used helper v265 in the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. It preserved the V1267 line-info result: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, zero forbidden action markers, GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. New V1271 value/power evidence: debugfs GPIO was mounted/readable, PMIC GPIO9/TLMM GPIO135/GPIO142 pinctrl surfaces were present, PCIe GDSC lines remained `0mV`, but exact debugfs-gpio value lines for global GPIO1270 / TLMM GPIO135 / TLMM GPIO142 were not found. V1272 selected a broader read-only debugfs GPIO/pinconf block sampler. V1273 built helper v266, SHA256 `3bf4105d685f023ccdeb75ae28d7d104ca005fc9f70870dc6f402a9ea4038ed4`, adding compact read-only block fields. V1274 deployed helper v266 to `/cache/bin/a90_android_execns_probe` by serial fallback; remote SHA verified and selftest `fail=0`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
