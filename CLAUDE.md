...
- **Active research cycle**: V1299 PASS → V1300 host-only late per_proxy delivery classifier — V1299 proved the compact dense sampler captures the full 42-sample window (`pre`, `poll_00..39`, `post`) with `response_sampler.end=1` and no stdout truncation (`778235` bytes). It did not reproduce `pm-service` opening `/dev/subsys_esoc0`; late `per_proxy` started and `per_mgr` held `/dev/subsys_modem`, but `per_mgr_subsys_esoc0_count` stayed `0`. Next gate is host-only V1300 comparing V1295/V1299 late `per_proxy`, `pm_proxy_helper`, `per_mgr`, Binder delivery, and compact-vs-verbose side effects. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
...
V1299 added `scripts/revalidation/native_wifi_compact_dense_response_sampler_live_v1299.py` and ran the bounded compact dense no-write late-`per_proxy` response sampler with helper `v272`. Result: `v1299-compact-dense-full-window-response-sampled-no-esoc0-trigger` PASS. The compact sampler completed the full window: mode `late-per-proxy-dense-compact-pinctrl-irq-pcie`, sample count `42`, phases `pre_late_per_proxy`, `late_per_proxy_poll_00..39`, `post_late_per_proxy`, `response_sampler.end=1`, and helper stdout `truncated=0 bytes=778235`. GPIO142 IRQ, PCIe/MHI/WLFW/SDX50M kmsg, MHI pipe, `ks`, and `wlan0` stayed absent; `mdm3` stayed `OFFLINING`. Unlike V1295, `pm-service` did not open `/dev/subsys_esoc0`; late `per_proxy` started, `per_mgr` held `/dev/subsys_modem`, and `per_mgr_subsys_esoc0_count` remained `0` across all 40 polls. Cleanup reboot restored healthy v724 with selftest `pass=11 warn=1 fail=0` and netservice disabled/stopped. No Wi-Fi HAL, scan/connect, credential use, DHCP/routes, external ping, PMIC write, GPIO line request/hold, direct eSoC ioctl, flash, boot image write, or partition write occurred. Next is V1300 host-only comparison of V1295 vs V1299 to explain the missing `/dev/subsys_esoc0` attempt before any new live trigger.
