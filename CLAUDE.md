...
- **Active research cycle**: V1427 rollbackable live handoff PASS (`v1427-test-boot-downstream-progress-rollback-pass`). V1425 executed three corrected-RC1 attempts total: initial plus two retries spaced by `500ms`. All three reached the same reset/release/LTSSM path and failed before L0 (`TEST: 11` count `3`, link-failure count `3`, L0 count `0`), with no MHI/WLFW/BDF/FW-ready/`wlan0`, then rolled back to v724 with selftest fail=0. This rules out a single early corrected-RC1 attempt as the primary blocker. Next gate should stop widening retry count and return to lower endpoint prerequisites: RC1 power/refclk/PERST/PMIC state or another narrowly justified pre-RC1 prerequisite test, still below connect. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
