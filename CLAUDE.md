...
- **Active research cycle**: V1424 host-only classifier PASS (`v1424-rc1-timing-precondition-parity-but-endpoint-no-l0`). Android V852 reaches RC1 assert about `254.929ms` after `esoc0`; native V1422 reaches corrected RC1 assert about `287.384ms` after `esoc0` (gap `32.455ms`) with matching reset/release and `PCIE20_PARF_INT_ALL_MASK=0x7f80c202`. The blocker is now post-PERST-release endpoint response: Android reaches L0 in about `16.666ms`, while native fails before L0 after about `109.086ms`, with no MHI/WLFW/BDF/FW-ready/`wlan0`. Next gate should target post-release endpoint response below connect: source/build-only higher-resolution read-only sampler, or a narrowly justified rollbackable test that changes only RC1 timing/retry policy. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
