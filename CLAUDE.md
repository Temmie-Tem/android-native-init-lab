...
- **Active research cycle**: V1417 host-only classifier PASS (`v1417-delayed-rc1-timing-aligned-filtered-dmesg-recapture-needed`). V1416 aligned corrected RC1 timing with Android within `~20ms` (`0.275121s` vs `0.254929s`) and still failed before L0 in `LTSSM_POLL_COMPLIANCE`; no MHI/WLFW/BDF/FW-ready/`wlan0`, `connect_ready=false`. However the V1416 dmesg grep pattern omitted endpoint reset/release strings, so reset/release marker absence is not proven. Next V1418 should rerun the same V1414 test image with an expanded dmesg pattern including endpoint reset/release and `PCIE20_PARF_INT` markers before changing timing or trigger design. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
