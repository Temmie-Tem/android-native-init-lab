...
- **Active research cycle**: V1446 case-aligned micro endpoint artifact sanity PASS (`v1446-wifi-test-boot-case-aligned-micro-endpoint-artifact-sanity-pass`). V1445 builds a rollbackable Wi-Fi test boot that waits for the writer to return from corrected RC1 `case=11`, records `rc1_micro_writer_summary`, then starts parent samples at `0ms` through `150ms` after confirmed case completion. V1446 verifies the exact V1445 manifest, static binaries, marker contract, v724 header/kernel parity, private modes, and forbidden credential-like byte absence. Next gate may be V1447 rollbackable live handoff of only the V1445 test image, with mandatory rollback to v724 and selftest fail=0 verification. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
