...
- **Current validated test artifact (resident after V2310 live validation): `A90 Linux init 0.9.274 (v2310-nl80211-events)`** — adds read-only `wifi events [timeout_ms]` nl80211 multicast monitoring (`mlme`/`scan`/`config`) on top of V2309 rtnetlink monitoring. Live boot/status/selftest passed; `wifi events 1000` resolved nl80211 multicast groups and joined all three groups without scan/connect. The connect-event assertion remains parked because Wi-Fi creds are absent. It is **not** the safety rollback baseline.
