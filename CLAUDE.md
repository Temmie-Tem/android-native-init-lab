...
- Audio tinyalsa inventory latest state: V2357 used the exact AUD-3C approval, flashed V2334, accepted one ADSP boot, reproduced `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`), and again proved the V2354 NCM re-enumeration repair works by restoring host ping and tcpctl after candidate USB re-enumeration. Tinyalsa inventory still did not execute: `install-tinymix` failed before tool transfer because V2356's `/cache/bin/busybox` path fix exposed a netcat semantic mismatch; BusyBox `nc` exited immediately for `netcat -l -p PORT COMMAND...`, while `/bin/toybox netcat` supports that listener child-command form. Rollback to V2321 succeeded with `selftest fail=0`. V2358 is the host-only repair: the V2349 runner now passes `tcpctl_host.py --toybox /bin/toybox`, and dry-run/tests assert stale `/cache/bin/toybox` and incompatible `/cache/bin/busybox` install-toolbox choices are absent. The next safe unit is a fresh exact-gated live AUD-3C inventory attempt with this toybox-semantic runner; do not proceed to `tinyplay` or mixer writes.
