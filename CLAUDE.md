...
- Audio tinyalsa inventory latest live result: V2350 used the exact AUD-3C approval and ran the V2349 live inventory route twice. Both attempts flashed V2334, reproduced ADSP + `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`), then rolled back to V2321 with `selftest fail=0`; no `tinyplay`, mixer write, PCM playback/write, audio HAL, or adsprpc path ran. Tinyalsa inventory itself remained blocked before execution: first by a disallowed install root (`/cache/a90-audio/...`), then after a runner fix to `/cache/a90-runtime/bin/v2349-tinyalsa-inventory/` by a `tcpctl_host.py install` timeout connecting to `192.168.7.2:2325`. The next safe unit is host-only transfer/readiness repair before another exact-gated live tinyalsa inventory attempt.
