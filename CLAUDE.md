...
- **Active research cycle**: V1458 exact provider thread-state test boot source/build PASS (`v1458-wifi-test-boot-exact-provider-thread-state-source-build-pass`). The artifact `tmp/wifi/v1458-wifi-test-boot-exact-provider-thread-state-sampler/boot_linux_v1458_wifi_test.img` uses native init `0.9.85 (v1458-wifitest)`, keeps the exact provider trigger and long read-only endpoint window, and additionally samples the triggering provider thread PID with `/proc/<pid>/comm`, `/proc/<pid>/wchan`, `/proc/<pid>/stat`, and selected `/proc/<pid>/status` fields. Next gate should be V1459 local-only artifact sanity before any live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
