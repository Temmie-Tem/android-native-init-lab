...
- **Active research cycle**: V1324 plan ready → V1324 Android-vs-native provider response delta classifier implementation — V1323 classified the current wait cause as the proprietary ext-mdm provider power-up response path after SDX50M soft-reset/AP2MDM activity and before GPIO142/PCIe/MHI/WLFW response, not public `wait_for_err_ready()` or the earlier image-link gate. V1324 is host/source-only and should classify GPIO142/errfatal/soft-reset/PCIe timing deltas before any live sampler. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, direct GDSC write, blind eSoC notify/BOOT_DONE, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
