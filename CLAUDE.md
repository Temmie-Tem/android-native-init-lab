...
- **Active research cycle**: V1408 source/build PASS (`v1408-wifi-test-boot-pid1-rc1-watcher-source-build-pass`). V1408 stages `tmp/wifi/v1408-wifi-test-boot-pid1-rc1-watcher/boot_linux_v1408_wifi_test.img` with `A90 Linux init 0.9.73 (v1408-wifitest)`. It adds a PID1-started parallel RC1 watcher that reads future `/dev/kmsg` lines, triggers corrected RC1 on the first `esoc0`/powerup marker, and omits the helper's duplicate corrected-RC1 flag. No device command or flash occurred. Next is V1409 local artifact sanity over the exact V1408 manifest/image before any rollbackable live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
