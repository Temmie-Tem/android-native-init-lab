...
- Audio tinyalsa inventory latest state: V2359 used the exact AUD-3C approval, flashed V2334 `0.9.292`, accepted one ADSP boot, reproduced `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`), repaired post-reenumeration NCM/tcpctl reachability, staged `tinymix` and `tinypcminfo` over tcpctl using `/bin/toybox netcat`, and completed read-only tinyalsa inventory. `tinymix -D 0` identified mixer card `sm8150-tavil-snd-card` with `Number of controls: 3628`; `tinymix -D 0 --all-values` captured values/ranges; `tinypcminfo -D 0 -d 0` returned PCM out/in caps for device 0. Candidate post-inventory selftest and final rollback V2321 selftest both returned `fail=0`. Playback is still unproven; next safe unit is host-only route analysis against the private vendor `mixer_paths_tavil.xml` / audio platform files. Do not run `tinymix set`, PCM playback opens, or `tinyplay` without a fresh exact playback gate.
