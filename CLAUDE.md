...
- **Active research cycle**: V1286 LIVE PASS → V1287 SDX50M power/GPIO prerequisite classifier — V1285 deployed helper v269 (`dbb1f67652913ffe94b1f083a082d8f221820040b9f28e08b226eb1e0a50fc83`) by serial fallback. V1286 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v269; the repaired klog collector worked through `syslog-read-all` (`kmsg_open_seen=true`, `max_kmsg_filtered_count=32`). PM-service reached eSoC, but GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, PCIe kmsg/MHI kmsg/WLFW kmsg counts remain `0`, and PCIe GDSC lines remain `0mV`. Post-reboot health passed on v724 (`selftest pass=11 warn=1 fail=0`). Next selected gate is V1287 host-only classification of SDX50M power/GPIO prerequisites using V1286 and Android positive evidence. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
