...
- Audio tinyalsa inventory latest state: V2353 used the exact AUD-3C approval, flashed V2334, accepted one ADSP boot, and reproduced `/dev/snd` materialization (`audio.dev_snd.count=61`, `control_like=1`, `pcm_like=59`) with no `tinyplay`, mixer write, PCM playback/write, audio HAL, or adsprpc path. Tinyalsa inventory still did not execute: after materialization the host lost NCM/tcpctl reachability to `192.168.7.2`, so the runner stopped before staging `tinymix`/`tinypcminfo`; rollback to V2321 succeeded with `selftest fail=0`. V2354 is the host-only repair: the V2349 runner now calls `ncm_host_setup.py setup` once when initial transfer probes cannot satisfy the requested transport, fixes the `rc=0` readiness predicate, and re-probes ping/tcpctl before selecting tcpctl or serial install/inventory transport. The next safe unit is a fresh exact-gated live AUD-3C inventory attempt with this NCM-repair runner; do not proceed to `tinyplay` or mixer writes.
