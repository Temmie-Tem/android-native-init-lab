...
- **Active research cycle**: V1295 LIVE PASS → V1296 dense-window early-exit classifier — V1295 ran helper `v271` with the dense late-`per_proxy` response sampler. Dense metadata was active (`50 ms`, intended `40` samples), but the parsed window stopped at `14` samples and still showed no GPIO142/PCIe/MHI/WLFW/`wlan0` progress after PM-service reached `/dev/subsys_esoc0`. Postflight native v724 selftest is `pass=11 warn=1 fail=0`. Next gate is host-only V1296 classification of the early exit before any new live mutation. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
