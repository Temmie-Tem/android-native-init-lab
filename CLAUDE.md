...
- **Active research cycle**: V1428 host-only classifier PASS (`v1428-rc1-retry-closed-pre-rc1-endpoint-prereq-next`). V1427 proved the V1425 rollbackable test boot executed three corrected-RC1 attempts total (initial plus two `500ms` retries); all three reached reset/release/LTSSM and failed before L0, with no MHI/WLFW/BDF/FW-ready/`wlan0`, then rolled back to v724 with selftest fail=0. V1428 closes retry widening, GPIO135, PON, provider-held ordering, and timing as the shortest next branches. Next gate should be V1429 source/build-only: a read-only Wi-Fi test-boot endpoint-prerequisite sampler around PERST release for GPIO102/PERST, GPIO103/CLKREQ, GPIO104/WAKE, `pcie_1_gdsc`, refclk/pipe clocks, GPIO142/MDM2AP IRQ, and LTSSM terminal state. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
