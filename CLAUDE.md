...
- **Active research cycle**: V1277 SOURCE/BUILD PASS → V1278 helper v267 deploy-only — V1275 bounded block sampler shows PMIC GPIO1270 block present in 14/14 samples and PMIC GPIO9 line text as `out ... high ...`, while GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`. V1276 classified native PMIC GPIO9 as matching Android/reference `out/high`, so PMIC GPIO9 write/hold is rejected. V1277 built `a90_android_execns_probe v267` (`stage3/linux_init/helpers/a90_android_execns_probe_v267`, sha256 `eccd9ca475927c2a37551304fedcc6740d19aeb048ebd137f966a18c269f0337`) with read-only TLMM GPIO135/GPIO142 debugfs range-slice capture to distinguish missing line names from missing controller/range visibility. Next selected gate is V1278 deploy-only for helper v267; V1279 should run the bounded TLMM range sampler live. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
