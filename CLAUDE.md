...
- **Active research cycle**: V1438 artifact sanity PASS (`v1438-wifi-test-boot-immediate-endpoint-artifact-sanity-pass`). It verified the exact V1437 image `tmp/wifi/v1437-wifi-test-boot-immediate-endpoint-sampler/boot_linux_v1437_wifi_test.img` with native init `0.9.80 (v1437-wifitest)`, static binaries, ramdisk entries, immediate endpoint sampler markers, no retry markers, v724 header/kernel parity, private modes, and forbidden credential-like byte absence. V1439 may be a rollbackable live handoff for only this image, with mandatory collection of V1437 log/summary/RC1 watcher/immediate window/dmesg/`wlan0`, then rollback to v724/selftest. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
