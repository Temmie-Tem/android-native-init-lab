...
- **Active research cycle**: V1253 SOURCE/BUILD PASS → V1254 helper v261 deploy-only — V1251 proved the native PMIC/GDSC reproduction candidate: PM8150L soft-reset GPIO remained `MUX UNCLAIMED`, PCIe GDSC lines remained `0mV`, `mdm3=OFFLINING`, GPIO142 IRQ count stayed `0`, and all zero-action markers passed. V1252 selected the bounded PMIC/power-surface write-gate path. V1253 built helper `a90_android_execns_probe v261` (`37947e378f4743a6661a03ee36dfc95ddf5ce9cd79acec0862a28a4564573a7c`) with mode `wifi-companion-pmic-power-surface-write-gate-preflight`, which verifies the V1251 read contract, scans PM8150L `gpiochip` identity/offset without writing, and prints `gpio_line_request_executed=0`, `esoc_ioctl_executed=0`, and `pm_actor_executed=0`. Next V1254 is deploy-only; no `/dev/subsys_esoc0`, PM/CNSS/HAL start, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
