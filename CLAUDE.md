...
- **Active research cycle**: V1457 exact provider long-window classifier PASS (`v1457-exact-provider-long-window-low-no-downstream`). V1456 booted the V1454 exact-line test image, triggered on the exact `__subsystem_get: esoc0` line, collected `0ms` through `1000ms` samples plus `1200ms` context, then rolled back to `A90 Linux init 0.9.68 (v724)` with selftest fail=0. GPIO135/GPIO142 stayed low, endpoint IRQs stayed zero, pcie1 GDSC/clocks stayed off, and no RC1/MHI/WLFW/BDF/FW-ready/`wlan0` marker appeared. Next gate should be V1458 source/build-only provider-trigger thread-state sampler to locate where the triggering Binder thread blocks before GPIO/pcie1 side effects. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
