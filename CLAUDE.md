...
- **Active research cycle**: V1445 case-aligned micro endpoint source/build PASS (`v1445-wifi-test-boot-case-aligned-micro-endpoint-source-build-pass`). V1443/V1444 proved V1441's micro reader started before the actual corrected RC1 `case=11` write completed. V1445 builds a rollbackable Wi-Fi test boot that waits for the writer to return from `case=11`, records `rc1_micro_writer_summary`, then starts parent samples at `0ms` through `150ms` after confirmed case completion. Next gate should be V1446 local-only artifact sanity before any rollbackable V1447 live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
