...
- **Active research cycle**: V1304 PASS → V1305 AP2MDM/MDM2AP transition timing or ext-mdm PMIC/pinctrl classifier — V1304 classified existing V1303 evidence host-only as `v1304-ap2mdm-assertion-visibility-gap-classified`. V1303 proves `pm-service` reaches `/dev/subsys_esoc0` via `openat` and blocks in `mdm_subsys_powerup`, but every sampled powerup phase still shows `gpio135 : out 0 16mA no pull` and `gpio142 : in  0 8mA no pull`; MDM status count, PCIe/MHI, WLFW, and `wlan0` remain absent. Reference docs say ext-sdx50m powerup should assert AP2MDM GPIO135 before MDM2AP/PCIe progress. Treat this as an assertion/visibility boundary, not a standalone GPIO root-cause proof, because Android post-boot snapshots can also show low GPIO lines. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
