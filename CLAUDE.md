...
- **Active research cycle**: V1310 PASS → V1311 stdout-reduced full-window lower-sequence sampler — V1310 host-only classified the active blocker as dynamic PCIe/GDSC/eSoC lower power sequencing after `mdm_subsys_powerup`. Static PMIC GPIO9 shape is closed by V1276 and static TLMM GPIO135/GPIO142 shape is closed by V1291; V1309 reconfirmed `/dev/subsys_esoc0` → `mdm_subsys_powerup` with PCIe0/PCIe1 GDSCs at `0mV` and MHI/`ks`/`wlan0` absent. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
