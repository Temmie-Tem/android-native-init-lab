...
- **Active research cycle**: V1269 SOURCE/BUILD PASS → V1270 helper v265 deploy-only — V1267 carried PMIC GPIO9 line-info into the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. V1267 evidence: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, zero forbidden action markers, GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. V1268 selected value/power snapshots as the next observer target. V1269 bumped `a90_android_execns_probe` to v265 and extended the same response sampler with read-only `/sys/kernel/debug/gpio` lines for global GPIO1270 / GPIO135 / GPIO142 if present, PMIC GPIO9 pinconf, TLMM GPIO135/142 pinconf, PCIe GDSC regulator lines, and PCIe RC1 state. Helper v265 static build SHA256 is `97ffa91a1aa7b8f4ab2c3a74716ae5664c703e98fe19a322351b1277fbd282b2`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
