...
- **Active research cycle**: V1251 READ-ONLY LIVE PASS → V1252 bounded PMIC/power-surface write-gate design — V1249 deployed helper `a90_android_execns_probe v260` to `/cache/bin/a90_android_execns_probe` over serial fallback (`0313d613d95c56af5681871062b7fceb47ede3c3ef8fcff534d0eea3338eaa2f`). V1251 temporarily mounted debugfs, ran `wifi-companion-pmic-soft-reset-preflight`, then unmounted/cleaned up and verified selftest. Result: `v1251-pmic-debugfs-native-reproduction-candidate`; `debugfs_pinctrl_present=1`, `debugfs_regulator_present=1`, PM8150L soft-reset GPIO remained `MUX UNCLAIMED`, PCIe GDSC lines remained `0mV`, `mdm3=OFFLINING`, GPIO142 IRQ count stayed `0`, and all zero-action markers passed. Next V1252 must be source/plan-first for a fail-closed bounded PMIC/power-surface write gate; no blind `/dev/subsys_esoc0` retry and no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
