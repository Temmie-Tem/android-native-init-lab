...
- **Current validated test artifact (resident after V2309 live validation): `A90 Linux init 0.9.273 (v2309-rtnetlink-events)`** — adds read-only `wifi netevents [timeout_ms]` rtnetlink monitoring for `wlan0`/`ncm0`; live boot/status/selftest passed and bounded `ncm0` down/up generated rtnetlink events. It is **not** the safety rollback baseline.
