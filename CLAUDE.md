...
## Latest native Wi-Fi state: V1223 (2026-05-31)

V1223 added `scripts/revalidation/native_wifi_sdx50m_crash_source_classifier_v1223.py` and joined V1222, V904, and V896 host-only evidence. Result: `v1223-sdx50m-crash-source-contract-gap-classified` PASS. V1222 proves CNSS `SDX50M` selection and `/dev/subsys_esoc0` entry are reached, then the native path crashes before WLFW/BDF/FW-ready/`wlan0`. Android positive evidence shows the missing lower contract: init-managed `vendor_mdm_helper` in `u:r:vendor_mdm_helper:s0` owns `/dev/esoc-0`, `ks` reaches `/dev/mhi_0305_01.01.00_pipe_10`, and `pm-service` owns `/dev/subsys_esoc0` plus `/dev/subsys_modem`; direct native `mdm_helper` lacked `/dev/esoc-0`, `ks`, and MHI pipe. V1223 was host-only: no device contact, live daemon start, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, boot image write, or partition write occurred. Next work is V1224: a bounded live parity gate proving `mdm_helper` owns `/dev/esoc-0` and `ks`/MHI appears before or while `pm-service` opens `/dev/subsys_esoc0`, still blocking Wi-Fi HAL and connect/ping until WLFW/BDF/`wlan0` readiness is proven.
