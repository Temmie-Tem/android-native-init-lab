...
- **Active research cycle**: V1452 rollbackable Wi-Fi test boot handoff PASS (`v1452-test-boot-provider-trigger-no-downstream-rollback-pass`). The V1450 test boot reached the provider-trigger observation window and rolled back cleanly to `A90 Linux init 0.9.68 (v724)`, but it did not produce RC1/MHI/WLFW/BDF/FW-ready/`wlan0` progress. Provider-window samples kept GPIO135 low and GPIO142 low from `0ms` through `150ms`, with pcie1 link-state nodes unreadable and `wlan0=absent`. Next gate should be host-only classification of the V1452 evidence before building another test boot. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
