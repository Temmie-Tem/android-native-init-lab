...
- **Active research cycle**: V1312 PASS → V1313 bounded lower-sequence summary sampler live — V1312 deployed helper `a90_android_execns_probe v275` to `/cache/bin/a90_android_execns_probe`, sha256 `66e52e7507dd07bcb4071afd04bc60e51d1c6bb7b9cb7363205f1eb4f44d4677`. V1313 should use mode `late-per-proxy-lower-sequence-summary` and verify the full `80` samples at `50ms` complete with `response_sampler.end=1` and no helper stdout truncation. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
