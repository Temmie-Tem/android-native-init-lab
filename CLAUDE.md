...
- **Active research cycle**: V1256 READ-ONLY LIVE PASS → V1257 helper v262 source/build-only — V1255 showed PMIC debugfs identifies `gpiochip2` range `1263-1273`, PMIC GPIO9 global line `1270`, offset `7`, but `/dev/gpiochip*` was absent. V1256 classified devnode feasibility read-only: `/proc/devices` has `254 gpiochip`; `/sys/bus/gpio/devices/gpiochip2/dev=254:2`; `/sys/class/gpio/gpiochip1263` labels PM8150L with `ngpio=11`; debugfs confirms `gpiochip2` PM8150L range and PMIC GPIO9 config. Next V1257 should source/build helper v262 to materialize a temporary `/dev/gpiochip2` node from sysfs metadata and open it read-only with `GPIO_GET_CHIPINFO_IOCTL`; still no GPIO line request, PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
