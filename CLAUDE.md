...
- **Active research cycle**: V1410 rollbackable live handoff completed with rollback OK but Wi-Fi progress BLOCKED (`v1410-test-boot-provider-trigger-no-downstream-wifi-progress-blocked`). V1408 booted and PID1 mounted debugfs, but the PID1 RC1 watcher failed immediately because `/dev/kmsg` is absent (`state=open-kmsg-failed rc=-2 errno=2`); host read-only follow-up showed `/proc/kmsg` exists. No RC1/MHI/WLFW/BDF/FW-ready/`wlan0`; rollback verified v724 and selftest fail=0. Next is V1411 source/build-only: change the watcher to fall back to `/proc/kmsg`, drain existing lines before watching, then rebuild/sanity-check before any live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
