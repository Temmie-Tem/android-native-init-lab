...
- **Active research cycle**: V1292 HOST PASS → V1293 dense dynamic response sampler build — V1292 selected a source/build-only helper update because V1291 closed static GPIO/PMIC shape, but V1290's response sampler uses `1000 ms` cadence while Android-positive PCIe RC1 appears `519 ms` after `subsys_esoc0_get`. V1293 should bump `a90_android_execns_probe` to `v271` and add an opt-in dense late-`per_proxy` sampler reusing `append_pm_esoc_response_sample()` at `50 ms` cadence for `40` samples. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
