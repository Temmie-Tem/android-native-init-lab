...
## Latest native Wi-Fi state: V1222 (2026-05-31)

V1222 added `scripts/revalidation/native_wifi_post_esoc_power_boundary_v1222.py` and reran the private CNSS `SDX50M` path with a 45s post-hold. Result: `v1222-esoc-powerup-crash-before-wlfw` PASS. `pm-service` reached `/dev/subsys_esoc0` and stayed in `mdm_subsys_powerup`; 46 post-hold samples kept `mdm3=OFFLINING`, modem-down/crash dmesg count rose to `4`, and WLFW/BDF/`wlan0` markers stayed `0`. No Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, boot image write, or vendor partition write occurred; postflight selftest stayed fail0. Next work should classify the lower SDX50M power-up/MHI/firmware handoff difference between Android's successful `mdm_helper`/`ks` path and native's crash-before-WLFW path.
