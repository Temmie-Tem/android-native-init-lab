...
- **Active research cycle**: V1294 DEPLOY PASS → V1295 dense no-write live sampler — V1294 deployed helper `v271` to `/cache/bin/a90_android_execns_probe` by serial fallback (`1010` chunks). Remote sha256 is `335b875516e76419933f2e0ab6e21cd7ee4d1d217b32f378f1925adc30010a24`, marker is `a90_android_execns_probe v271`, and usage includes `--pm-observer-late-per-proxy-dense-response-sampler`. No daemon start or Wi-Fi bring-up occurred. Next gate is V1295 bounded dense live sampler with helper v271. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
