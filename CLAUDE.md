...
- **Active research cycle**: V1412 local artifact sanity PASS (`v1412-wifi-test-boot-kmsg-fallback-artifact-sanity-pass`). The exact V1411 boot image `tmp/wifi/v1411-wifi-test-boot-kmsg-fallback/boot_linux_v1411_wifi_test.img` passed static/header/kernel/ramdisk/marker/forbidden-byte/private-mode checks. Next live gate may flash only that V1411 test image, expect `A90 Linux init 0.9.74 (v1411-wifitest)`, collect V1411 log/summary/RC1 watcher result/dmesg/`wlan0`, then roll back to `stage3/boot_linux_v724.img`. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
