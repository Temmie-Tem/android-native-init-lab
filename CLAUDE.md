...
- **Active research cycle**: V1305 PASS → V1306 ext-mdm PMIC/pinctrl/GDSC branch classifier — V1305 extracted the V1303 response-sample timeline and proved the AP2MDM-low observation was not just a short sampler blind spot. `pm-service` stayed blocked in `mdm_subsys_powerup` for `5013ms` across `42` samples; GPIO135 stayed `gpio135 : out 0 16mA no pull`, GPIO142 stayed `gpio142 : in  0 8mA no pull`, and MDM status count, PCIe/MHI, WLFW, `ks`, and `wlan0` remained absent. Next classify why visible AP2MDM assertion is absent: PM8150L soft-reset pinctrl mismatch, PCIe GDSC/runtime power prerequisite mismatch, or branch-before-`mdm_do_first_power_on` inside proprietary `mdm_subsys_powerup`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
