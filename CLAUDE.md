...
- **Active research cycle**: V1262 LIVE PASS → V1263 kernel-owned AP2MDM_SOFT_RESET classifier — V1261 deployed `a90_android_execns_probe v263` to `/cache/bin/a90_android_execns_probe` and remote SHA matched `32ac877a165a266d96589387d9974dfea38c81d0adb368bf17ff15de77a9f9fb`. V1262 ran the bounded temporary `/dev/gpiochip2` plus read-only PMIC GPIO9 line-info proof. Result: sysfs contract matched, temp `mknod` succeeded, read-only open succeeded, chipinfo succeeded, `GPIO_GET_LINEINFO_IOCTL` for offset `7` succeeded, cleanup unlink succeeded, postflight selftest stayed `fail=0`, and all forbidden markers stayed zero. Crucial finding: line flags `0x1` / `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. Direct userspace PMIC GPIO9 line request/hold is therefore not the next safe path. Next V1263 should classify the kernel-owned AP2MDM_SOFT_RESET/ext-mdm power-up contract without PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
