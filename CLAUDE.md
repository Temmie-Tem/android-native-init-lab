...
- **Active research cycle**: V1265 SOURCE/BUILD PASS → V1266 helper v264 deploy-only — V1262 proved PMIC GPIO9 offset `7` line-info: flags `0x1`, `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. V1263 rejected direct userspace PMIC GPIO9 line request/hold as the next safe path, and V1264 fixed the next observer contract. V1265 bumped `a90_android_execns_probe` to v264 and extended the existing late `per_proxy` / PM-service response sampler with read-only PMIC GPIO9 `GPIO_GET_LINEINFO_IOCTL` snapshots in the same `/dev/subsys_esoc0` window. The blocker remains below the reproduced PM-service entry: native reaches `mdm_subsys_powerup`, but does not get Android's downstream GPIO142/PCIe/MHI/WLFW response. V1266 should deploy helper v264 only; V1267 should run the bounded read-only ext-mdm/AP2MDM observer. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
