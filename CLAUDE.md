...
- **Active research cycle**: V1462 exact provider tracepoint source/build PASS (`v1462-wifi-test-boot-exact-provider-tracepoint-source-build-pass`). Built rollbackable `0.9.86 (v1462-wifitest)` test boot image at `tmp/wifi/v1462-wifi-test-boot-exact-provider-tracepoint-sampler/boot_linux_v1462_wifi_test.img` (boot sha256 `a584d18cc6255e146e8bf46e052c5afd0afca3899856ff76751a1f6c717246c2`). It keeps exact provider/thread-state sampling, arms `gpio_value`/`gpio_direction` tracepoints before helper start, samples GPIO1270/GPIO135/GPIO142/GPIO141 trace output around provider phases, and does not issue RC1 debugfs `rc_sel`/`case`, PMIC/GPIO/GDSC control writes, eSoC notify/`BOOT_DONE`, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, or external ping. Next gate: V1463 local-only artifact sanity before any V1464 rollbackable live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
