...
- **Active research cycle**: V1461 provider thread-state classifier PASS (`v1461-provider-thread-state-powerup-block-no-downstream`). V1460 rollbackable test boot ran `0.9.85 (v1458-wifitest)`, collected exact provider thread-state evidence, and rolled back to `stage3/boot_linux_v724.img`; current device verification is v724 with selftest fail=0. V1461 proves the exact provider Binder thread enters `sdx50m_toggle_soft_reset`, then `msleep`, then remains blocked in `mdm_subsys_powerup` while GPIO135/GPIO142, MDM status IRQ, pcie1 clocks/GDSC, RC1/MHI/WLFW/BDF/FW-ready, and `wlan0` stay inactive. Next gate: V1462 source/build-only exact-provider tracepoint test boot for GPIO1270/GPIO135/GPIO142 plus pcie1 clock/GDSC timing around provider phases. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
