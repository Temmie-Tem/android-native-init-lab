...
- **Active research cycle**: V1313 PASS → V1314 classify exact safe dynamic GDSC/eSoC prerequisite — V1313 ran helper `a90_android_execns_probe v275` mode `late-per-proxy-lower-sequence-summary` live. Full summary window completed without stdout truncation: `81` samples, `mdm_subsys_powerup` seen, max MDM status/PCI/MHI/MHI pipe/`ks` all `0`, `wlan0` absent, PCIe0/PCIe1 GDSCs at `0mV`, PMIC soft-reset pinmux still `MUX UNCLAIMED`, TLMM GPIO135/GPIO142 static lines unchanged. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
