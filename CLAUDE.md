...
- Audio tinyalsa inventory live runner latest host-only checkpoint: V2349 adds `native_audio_tinyalsa_inventory_live_handoff_v2349.py`, an exact-gated runner that will reproduce the proven V2334 ADSP + `/dev/snd` materialization window, install pinned V2345 `tinymix`/`tinypcminfo` under `/cache/a90-audio/v2349-tinyalsa-inventory/`, run only read-only `tinymix -D 0`, `tinymix -D 0 --all-values`, and `tinypcminfo -D 0 -d 0`, then roll back to V2321. It excludes `tinyplay`, mixer set operands, PCM playback/write, audio HAL, and adsprpc. This was host-only; dry-run OK, focused tests 4, full suite 1020. Future live run requires the exact `AUD-3C-tinyalsa-inventory go: ...` phrase.
