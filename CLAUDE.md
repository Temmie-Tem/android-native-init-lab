...
- **Active research cycle**: V1293 BUILD PASS → V1294 helper v271 deploy-only — V1293 bumped `a90_android_execns_probe` to `v271` and added `--pm-observer-late-per-proxy-dense-response-sampler`, fail-closed behind `--pm-observer-late-per-proxy-response-sampler`. Dense mode reuses `append_pm_esoc_response_sample()` at `50 ms` cadence for `40` samples (`2000 ms` window) and built static aarch64 helper sha256 `335b875516e76419933f2e0ab6e21cd7ee4d1d217b32f378f1925adc30010a24`. Next gate is V1294 deploy-only, then V1295 bounded dense no-write live sampler. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
