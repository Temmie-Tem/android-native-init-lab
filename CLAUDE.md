...
- **Active research cycle**: V1414 source/build-only PASS (`v1414-wifi-test-boot-delayed-rc1-source-build-pass`). The staged test boot is `A90 Linux init 0.9.75 (v1414-wifitest)` at `tmp/wifi/v1414-wifi-test-boot-delayed-rc1/boot_linux_v1414_wifi_test.img`. It keeps the `/proc/kmsg` PID1 RC1 watcher and adds a `250ms` delay after the first `esoc0`/powerup marker before corrected RC1 enumerate, with `detect_elapsed_ms`, `write_elapsed_ms`, and `delay_ms` in the watcher result. Next is V1415 local artifact sanity over the exact V1414 manifest/image before any live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
