...
- **Active research cycle**: V1279 LIVE PASS → V1280 host-only response-gap classifier — V1279 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v267 and confirmed TLMM `gpiochip0` range `0-174` is visible, GPIO135/GPIO142 pinmux ownership belongs to `soc:qcom,mdm3`, but exact GPIO135/GPIO142 debugfs value lines remain absent. GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`; post-run health check returned v724 selftest `fail=0`. Next selected gate is V1280 host-only comparison of V1279 native evidence against Android positive GPIO/PCIe timing evidence to choose between PCIe/GDSC enablement, AP2MDM/MDM2AP transition timing, or Android-side early sampler gaps. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
