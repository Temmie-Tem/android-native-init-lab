...
- **Active research cycle**: V1419 host/source-only design PASS (`v1419-endpoint-readiness-sampler-design-ready`). V1418 proved corrected RC1 executes the normal reset/release/LTSSM path with Android-like timing but fails before L0, so the next narrow target is endpoint response after PERST release. V1420 should build a new rollbackable test boot preserving the V1414 `250ms` delayed-RC1 path while adding a read-only PID1 RC1-window sampler for `/proc/interrupts`, GPIO142/`mdm status`, GPIO 102/104/135/142 debugfs/pinctrl excerpts, and optional ordinary-read PCIe status at pre-delay, pre-RC1, +50ms, +150ms, and +500ms. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, pci-msm `case` writes, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
