...
- **Active research cycle**: V1306 PASS → V1307 focused no-write PMIC/GDSC transition sampler support — V1306 classified the lower branch as `v1306-pmic-gdsc-prereq-gap-classified`. During the extended native `mdm_subsys_powerup` window, PM8150L soft-reset stayed `MUX UNCLAIMED`, PCIe0/PCIe1 GDSCs stayed `0mV`, AP2MDM/MDM2AP stayed low, and MDM status/MHI/WLFW/`ks`/`wlan0` stayed absent. Android-positive evidence has PMIC GPIO9 configured and PCIe RC1 progress. The blocker is aligned with an ext-mdm PMIC/GDSC prerequisite branch below upper PM/CNSS delivery. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
