		...
		const struct s22plus_max77705_result *result)
{
	...
	cursor += scnprintf(cursor, PAGE_SIZE - (cursor - cached_result),
			"v=1 stage=%u rc=%d pmic_v=%02x pmic_id=%02x "
			"pmic_rev=%02x uic0_v=%u uic0=%02x issued=%02x "
			"seen=%02x wr_attempt=%u wr_amb=%u "
...
MODULE_PARM_DESC(result, "cached bounded Max77705 MUX diagnostic result");
...
MODULE_DESCRIPTION("Bounded S22+ Max77705 CONTROL1 MUX diagnostic");
