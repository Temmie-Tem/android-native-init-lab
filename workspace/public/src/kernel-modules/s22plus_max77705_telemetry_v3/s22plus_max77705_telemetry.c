...
static int telemetry_probe(struct platform_device *pdev)
{
    ...
    /* Exact root model observed on rooted FYG8 Android after P379 closed.
     * Native boot rechecks this value; phone/firmware binding also remains
     * the F1 owner's responsibility. No wildcard or SoC-only fallback. */
    if (!error && strcmp(model, "Samsung G0Q PROJECT (board-id,12)"))
        error = -ENODEV;
    of_node_put(root);
    if (error)
