/*
 * V2568 common-topology hook for ACDB pre-init-tail per-device capture.
 *
 * libacdbloader enters common topology during acdb_loader_init_v3() and later
 * crashes in the init_v4 tail before init_v3 can return.  This interposer runs
 * the real common-topology function, patches the known initialized flag, drives
 * send_audio_cal_v5() while acdbtap is armed, and exits before returning to the
 * crashing tail.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2568-preinit-perdevice-events.jsonl"
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2568_preinit_perdevice\",\"stage\":\"");
