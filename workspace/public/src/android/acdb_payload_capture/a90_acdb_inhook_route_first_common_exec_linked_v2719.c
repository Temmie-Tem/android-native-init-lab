/*
 * V2719 ARM32 own-process ACDB in-hook route-first common-topology caller.
 *
 * V2718 showed that the post-init continuation can SIGSEGV before the helper
 * regains control. This helper therefore only starts acdb_loader_init_v3(); the
 * paired preload performs arm -> send_audio_cal_v5 -> real common topology
 * inside the init-time common-topology hook and exits the process after the
 * capture attempt. The helper does not open /dev/msm_audio_cal, does not issue
 * ioctls, and does not write speaker audio.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2719-inhook-route-first-helper-events.jsonl"
...
static void a90_write_event(const char *stage, int32_t code)
{
    ...
    if (fd < 0)
        return;
    a90_write_str(fd, "{\"event\":\"v2719_inhook_route_first_helper\",\"stage\":\"");
...
void _start(void)
{
    ...
    a90_write_event("init_returned_unexpectedly_after_inhook_route_first", 0);
    a90_exit(0);
}
