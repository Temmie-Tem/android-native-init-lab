/*
 * V2508 ARM32 own-process ACDB pure-read GET probe, exec-linked variant.
 *
 ...
 * blocker by letting the dynamic linker load the ACDB dependency set at process
 * startup.  The helper calls acdb_loader_init_v3() and acdb_ioctl() directly,
 * then issues the same bounded read-only GET matrix.
 *
...
extern int32_t acdb_ioctl(uint32_t cmd, const uint8_t *in_buf,
    ...
    for (i = 0; i < sizeof(a90_commands) / sizeof(a90_commands[0]); i++) {
        for (j = 0; j < sizeof(a90_in_lens) / sizeof(a90_in_lens[0]); j++) {
            for (k = 0; k < sizeof(a90_out_lens) / sizeof(a90_out_lens[0]); k++) {
                a90_capture_one(acdb_ioctl, a90_commands[i],
