/*
 * V2611 ARM32 own-process ACDB meta-list post-init send_audio_cal_v5 driver.
 *
 * The paired preload skips common-topology work during acdb_loader_init_v3()
 * and patches the initialized flag without arming the acdb_ioctl tap.  This
 * helper passes a process-local empty meta-list head as init_v3 arg3 so the
 * init_v4 tail can finish, then arms capture only after init_v3 returns and
 * calls acdb_loader_send_audio_cal_v5() outside the init-time hook/mutex window.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2611-meta-list-postinit-send-v5-events.jsonl"
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2608_postinit_send_v5\",\"stage\":\"");
