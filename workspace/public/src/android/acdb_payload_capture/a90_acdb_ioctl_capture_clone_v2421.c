// V2421 Android-side msm_audio_cal clone-following ioctl observer.
// Measurement-only helper: attaches to an Android audio process, follows new
// threads with PTRACE_O_TRACECLONE, records fd-filtered ioctl metadata, and
...
static void write_header(FILE *fp, const struct options *opts) {
    fprintf(fp,
            ...
            "\"max_bytes\":%zu,\"trace_mode\":\"clone-following\",\"device_substr\":\"",
...
int main(int argc, char **argv) {
    ...
    if (ptrace(PTRACE_ATTACH, opts.pid, NULL, NULL) != 0) {
        write_error(out, "PTRACE_ATTACH", opts.pid, errno);
        fclose(out);
        return 3;
    }
    ...
    int status = 0;
    int wait_rc = wait_for_specific_stop(opts.pid, &status, deadline, out, "waitpid-attach");
    if (wait_rc != 0) {
    ...
    int task_count = 0;
    struct traced_task *initial_task = add_task(tasks, &task_count, opts.pid, out);
    if (!initial_task || set_trace_options(initial_task, out) != 0 || resume_syscall(initial_task, 0, out) != 0) {
