/*
 * V2666 init-time real-common custom-topology hook.
 *
 * First init-time acdb_loader_send_common_custom_topology() call:
 *   - patch libacdbloader's initialized flag,
 *   - call the real common-topology implementation once while the loader is
 *     still inside init_v3,
 ...
 *
 * V2665 proved that waiting for acdb_loader_init_v3 to return is the wrong
 * control point: init_v3 fake-allocates the target 10/14/24 cal blocks and then
 * the own-process helper SIGSEGVs before its post-init common call.  This hook
 * banks the common-topology SET rows at the init-time call site instead.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2666-init-real-common-events.jsonl"
...
static void a90_write_event(const char *stage, int32_t code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2666_init_real_common\",\"stage\":\"");
