#!/usr/bin/env python3
"""Build a P3.12 fixed-P3.10-Image boot-only candidate."""
...
import s22plus_fyg8_p312_candidate_contract as candidate_contract
import s22plus_fyg8_p312_e2_stock_closure as closure
import s22plus_fyg8_p312_overlay_contract as contract
import s22plus_fyg8_p312_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p312_candidate_artifact_result_v1"
VERDICT = "PASS_P312_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p312/candidate-a")
...
def verify_repro_result(
    ...
    **_ignored,
    ...
    if result_path != root / contract.PARENT_REPRO_RESULT:
        raise BuildError("P3.12 P3.10 closure path differs")
    value, result_receipt = _read_json(result_path, "P3.12 P3.10 independent closure")
    if (
        ...
        raise BuildError("P3.12 fixed P3.10 closure differs")
    return {
...
def artifact_safety(exact_contract: dict) -> dict:
    ...
    result.update({
        ...
        "p312_kernel_rebuild": False,
        "p312_full_lto_ab": False,
        "p312_observer": exact_contract["observer"],
        "p312_cross_gate_audit": exact_contract["cross_gate_audit"],
        "p312_carrier_decoder_authority": exact_contract["carrier_decoder_authority"],
        "p312_tracefs_abi": exact_contract["tracefs_abi"],
