#!/usr/bin/env python3
"""Build V2783 native-init audio route API boot image.

V2783 splits pure route and speaker-map query helpers into `a90_audio_route.{h,c}` while preserving the validated audio command behavior.
...
CYCLE = "V2783"
INIT_VERSION = "0.9.299"
INIT_BUILD = "v2783-audio-route-api"
BUILD_TAG = "v2783-audio-route-api"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2783_AUDIO_ROUTE_API_SOURCE_BUILD_2026-06-19.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2783_audio_route_api.img", legacy_fallback=False
)
BASE_BOOT = workspace_private_input_path(
    "boot_images", "boot_linux_v2334_audio_snd_nodes_preflight.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2783_audio_route_api"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2783_audio_route_api.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v444_audio_route_api"
...
def configure() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--wifi-test-klog-prefix": "A90v2783",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2783.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2783.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2783.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2783-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2783.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2783-supervisor.pid",
    }
...
def main() -> int:
    ...
    manifest.update({
        ...
        "parent_baseline": "v2782-audio-stage-module-device-validation",
        ...
        "audio_route_api_source_validation": {
            "native_route_api_module_compiled": True,
    ...
    (OUT_DIR / "promotion-candidate.json").write_text(json.dumps({
        ...
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "note": "V2783 is a test image for the audio route API module split, not a promoted rollback baseline.",
