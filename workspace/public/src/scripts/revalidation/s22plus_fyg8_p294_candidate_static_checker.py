#!/usr/bin/env python3
"""Independently audit one deterministic P2.94 boot-only candidate."""
...
import build_s22plus_fyg8_p294_candidate as candidate
import s22plus_fyg8_p286_candidate_static_checker as base
import s22plus_fyg8_p294_build_repro_check as repro
import s22plus_fyg8_p294_candidate_contract as contract
import s22plus_fyg8_p294_e2_stock_closure as p294_closure
import s22plus_fyg8_p294_postbuild_linked_audit as postbuild_audit
import s22plus_fyg8_p294_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p294_candidate_static_checker_v1"
VERDICT = "PASS_P294_INDEPENDENT_ARTIFACT_CLOSURE_HOST_ONLY"
TARGET = contract.TARGET
DEFAULT_CANDIDATE = candidate.DEFAULT_OUT
DEFAULT_CANDIDATE_B = Path("workspace/private/outputs/s22plus_fyg8_p294/candidate-b")
...
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p294/static-check-result.json"
)
...
def verify_repro(root: Path, args, exact_contract):  # noqa: ANN001, ANN201
    result, payload = base.read_json(
        ...
        "P2.94 build reproducibility result",
        16 * 1024 * 1024,
    )
    ...
    if (
        ...
        raise CheckError(
            "P2.94 reproducibility result differs from fresh post-build verification"
        )
