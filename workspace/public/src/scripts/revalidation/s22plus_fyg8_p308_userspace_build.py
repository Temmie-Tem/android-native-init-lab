#!/usr/bin/env python3
"""Build P3.08 userspace twice against the fixed P3.00 Image."""
...
import s22plus_fyg8_p304_userspace_build as parent
import s22plus_fyg8_p308_candidate_contract as candidate_contract
import s22plus_fyg8_p308_overlay_contract as contract


SCHEMA = "s22plus_fyg8_p308_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p308/userspace")
COMPILE_FLAGS = parent.COMPILE_FLAGS
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("unsupported P3.08 userspace profile or parent contract")
    return VERDICT
...
def _source_contract(exact: dict) -> dict:
    return {
        ...
        "qscratch_audit": exact["qscratch_audit"],
        "telemetry": exact["telemetry"],
        "cross_gate_audit": exact["cross_gate_audit"],
        "module_delta": exact["module_delta"],
        "folded_tail": exact["folded_tail"],
        "observer": exact["observer"],
        "carrier_v2_design_input": exact["carrier_v2_design_input"],
        "verified": True,
    }
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.08 userspace output exists: {output}")
    ...
    tools = parent.base.require_tools()
    builds = []
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p308-userspace-{label}-") as name:
            directory = Path(name)
            result = parent._compile(root, directory, exact, materialized, tools)  # noqa: SLF001
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.08 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
