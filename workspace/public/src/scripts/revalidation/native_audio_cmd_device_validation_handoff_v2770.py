#!/usr/bin/env python3
"""V2770 live validation for the native-init audio command surface.

This is the GOAL.md anti-churn checkpoint: flash the current `a90_audio.c`
command surface, prove the command path on hardware, and roll back to V2321.
It intentionally does not try to work around the current source boundary where
`audio play --execute` reports `execute-not-implemented-native-pcm`.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2770-audio-cmd-device-validation/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2770_audio_cmd.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2770_AUDIO_CMD_DEVICE_VALIDATION_2026-06-19.md"

CANDIDATE_VERSION = "0.9.293"
CANDIDATE_TAG = "v2770-audio-cmd"
...
def preflight_state() -> dict[str, Any]:
    ...
    return {
        ...
        "live_scope": [
            ...
            "run one audio play execute boundary probe; no retry",
            "rollback to v2321",
        ],
    }
...
def summarize_step(step: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_play_execute_refusal": "execute-not-implemented-native-pcm" in text,
...
def render_report(result: dict[str, Any]) -> str:
    ...
    summary_lines = [
        f"- `{item['name']}`: ok={item['ok']} rc={item['rc']} dry_run_ok={item['has_play_dry_run_ok']} execute_refusal={item['has_play_execute_refusal']}"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2770-audio-cmd-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2770-audio-cmd-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        audio_steps = [
            ...
            ("audio-play-execute-boundary", ["audio", "play", PROFILE, "--mode", "probe", "--execute"], True),
        ]
        ...
        result["execute_boundary_observed"] = any(item.get("has_play_execute_refusal") for item in command_summaries)
        result["dry_run_ok_observed"] = any(item.get("has_play_dry_run_ok") for item in command_summaries)
        result["decision"] = (
            "v2770-audio-cmd-device-validated-play-execute-boundary"
            if result["execute_boundary_observed"]
            else "v2770-audio-cmd-device-validation-unexpected-play-execute-result"
        )
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2770-audio-cmd-device-validation-dry-run",
