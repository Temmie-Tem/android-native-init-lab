#!/usr/bin/env python3
"""Host-only closure for the P2.96 built-in DWC3 telemetry successor."""
...
SCHEMA = "s22plus_fyg8_p296_telemetry_closure_v1"
VERDICT = "PASS_P296_BUILTIN_DWC3_TELEMETRY_CLOSURE_HOST_ONLY"
...
def _classifier_tu(runtime: bytes) -> bytes:
    ...
    if struct_start < 0 or struct_end < 0:
        raise ClosureError("P2.96 capture state source is absent")
...
def audit_runtime_classifier(runtime: bytes) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="s22-p296-classifier-") as tmp:
        directory = Path(tmp)
        ...
        if compiled.returncode != 0:
            raise ClosureError(
                "P2.96 classifier host compile failed: "
    ...
    if executed.returncode != 0 or actual != expected_line:
        raise ClosureError(
            f"P2.96 classifier SoT differs: expected={expected_line!r}, "
...
def audit_delivery(artifacts: dict[str, bytes]) -> dict[str, Any]:
    ...
    if any(token in value for value in artifacts.values() for token in forbidden):
        raise ClosureError("P2.96 external-module dependency remains")
    if (
        patch.count(b"s22_p294_dwc3_state_snapshot") != 2
        or descriptor.count(b"s22_p294_dwc3_state_snapshot") != 1
        or runtime.count(b"g_p294_capture.dwc3_seen") < 2
        or b"#define P282_CYCLE_EVENT_COUNT 16U" not in descriptor
        or b"#define P282_BIND_EVENT_COUNT 7U" not in descriptor
    ):
        raise ClosureError("P2.96 built-in delivery closure differs")
    return {
        "required_builtin_symbols": ["s22_p294_dwc3_state_snapshot"],
        "required_external_module_symbols": [],
...
def run_closure(root: Path) -> dict[str, Any]:
    ...
    adjacency = audit_pair_adjacency(artifacts["p290_e3_runtime_include"])
