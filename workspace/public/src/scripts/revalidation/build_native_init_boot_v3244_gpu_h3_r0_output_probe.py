#!/usr/bin/env python3
"""Build V3244 GPU H3 r0 output probe."""
...
import build_native_init_boot_v3242_gpu_h3_direct_render_marker_probe as previous

base = previous.base

CYCLE = "V3244"
INIT_VERSION = "0.11.49"
INIT_BUILD = "v3244-gpu-h3-r0-output-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3244_GPU_H3_R0_OUTPUT_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3244_gpu_h3_r0_output_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3244_gpu_h3_r0_output_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v578_gpu_h3_r0_output_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3244"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3244.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3244.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3244"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3244-gpu-h3-r0-output-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3244-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3244-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3244-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3244-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3244-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3244-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3244-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-r0-output-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-r0-output-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3244")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3244")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3244")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3244")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3244")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3244")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3244")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3244")
SFX_STREAM_MARKER = "a90.doomgeneric.v3244.audio=real-sfx-pcm-stream-gpu-h3-r0-output-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-r0-output-probe-v3244"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3244.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3244_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3242": b"a90-doomgeneric-v3244",
        b"a90.doomgeneric.v3242": b"a90.doomgeneric.v3244",
        b"v3242": b"v3244",
        b"V3242": b"V3244",
        b"first-triangle-h3-direct-render-marker-r1-footprint2-mov-f32-shader":
...
REQUIRED_STRINGS = tuple(_rewrite_v3244_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        ...
        "scope": "first-triangle-h3-r0-output-full-state-mov-f32-shader",
        ...
        "sp_cntl0_source": "Mesa A6xx SP_VS_CNTL_0/SP_PS_CNTL_0 fullregfootprint=2 retained while probing r0 output regid, mergedregs, and FS inoutregoverlap fields",
        ...
        "next_live_validation": [
            "flash-v3244-through-native-init-flash",
            "post-flash-health-check",
            "verify-fresh-boot-fwclass-path-before-h3",
            "gpu-h3-r0-output-full-state-mov-f32-shader-timeout-guard",
...
def v3244_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-r0-output-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-r0-output-probe")
        .replace("v3208", "v3244")
        .replace("V3208", "V3244")
    )


def _overlay_preserved_v3244_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3242 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3244 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3244 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3244 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3244-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            if item == "--ramdisk":
                ...
                raise RuntimeError("V3242 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3244 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3244 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3242-ramdisk-overlay-v3244-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-r0-output-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-r0-output-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-r0-output-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3242-ramdisk-overlay-v3244-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-r0-output-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-r0-output-live-validation"
...
def _apply_v3244_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-r0-output-probe")
        ...
        "v3210_adapter_source": v3244_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3244_ramdisk,
