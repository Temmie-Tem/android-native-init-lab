#!/usr/bin/env python3
"""V2792 live handoff for native-init integrated audio playback.

This is the audio core closure gate: flash the V2792 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2792-audio-integrated-play-settle/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2792_audio_integrated_play_settle.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2792_AUDIO_INTEGRATED_PLAY_SETTLE_LIVE_2026-06-19.md"

CYCLE = "V2792"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.305"
CANDIDATE_TAG = "v2792-audio-integrated-play-settle"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2792-audio-integrated-play-settle-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2792-audio-integrated-play-settle-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if not result["play_output_pass"] or play.get("rc") != 0:
            result["decision"] = "v2792-audio-integrated-play-settle-live-command-failed-before-rollback"
        ...
        result["decision"] = "v2792-audio-integrated-play-settle-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2792-audio-integrated-play-settle-live-blocked")
        if result["decision"] == "v2792-audio-integrated-play-settle-live-started":
            result["decision"] = "v2792-audio-integrated-play-settle-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2792-audio-integrated-play-settle-live-dry-run",
