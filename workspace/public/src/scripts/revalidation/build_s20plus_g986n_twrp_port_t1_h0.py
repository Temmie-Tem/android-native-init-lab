#!/usr/bin/env python3
"""Build a host-only IYC2-stock-substrate, sanitized y2q TWRP T1 image.
...
SCHEMA = "s20plus_g986n_twrp_port_t1_build_v1"
VERDICT = "PASS_S20PLUS_G986N_TWRP_PORT_T1_HOST_BUILT_REVIEW_PENDING"
...
MARKER_ENTRY = "init.s20plus_g986n_twrp_port_t1"
...
POST_HOOK_INERT = (
    b"#!/system/bin/sh\n"
    b"# S20+ G986N T1: stock-recovery restoration files are left untouched.\n"
    b"exit 0\n"
)

SAFE_TWRP_FLAGS = b"""# S20+ G986N T1 lab-safe TWRP UI surface.
...
SAFE_USB_RC = b"""# S20+ G986N T1 first-boot USB: ADB only.
...
MARKER = (
    b"S20PLUS_G986N_TWRP_PORT_T1_V1\n"
...
def sanitize_qcom_rc(data: bytes) -> bytes:
    ...
    replacements = (
        ...
        (
            ...
            b"# T1 leaves /data/vendor/keymaster untouched during first boot.\n\n",
        ...
        (
            ...
            b"    start spdaemon\n"
            b"    start sec_nvm\n\n",
            b"# T1 does not auto-start keymaster, spdaemon, or sec_nvm.\n\n",
...
def build(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="s20plus-twrp-port-t1-", dir=output.parent
    ) as temporary:
        ...
        manifest = {
            ...
            "avb": {
                ...
                "candidate_runtime_acceptance": "UNKNOWN_REQUIRES_T0_FIRST",
            },
            "candidate": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="absent private output directory; defaults to the fixed S20+ T1 path",
    )
