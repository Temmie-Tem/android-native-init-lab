...
import s22plus_fyg8_p286_boot_only_packager as packager  # noqa: E402
...
import s22plus_fyg8_p286_userspace_build as userspace  # noqa: E402
...
import s22plus_fyg8_p284_contract_spec as p284_spec  # noqa: E402
import s22plus_fyg8_p286_contract_spec as p286_spec  # noqa: E402
import s22plus_fyg8_p286_e2_stock_closure as p286_closure  # noqa: E402


SCHEMA = "s22plus_fyg8_p286_candidate_artifact_result_v1"
VERDICT = "PASS_P286_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
P286_SOURCE_CONTRACT_ID = "s22plus-fyg8-p286-parent-tail-bounded-restart-v1"
DEFAULT_IMAGE = repro.DEFAULT_BUILD_A / "Image"
DEFAULT_REPRO_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p286/build-repro-result.json"
)
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p286/candidate-a")
...
def artifact_safety(exact_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    elif exact_contract.get("source_contract_id") in {
        P282_SOURCE_CONTRACT_ID,
        ...
        safety.update(
            {
                "userspace_sysfs_configfs_write_scope": (
                    selected_spec.SAFETY_USERSPACE_WRITE_SCOPE
                ),
                "usb_scope": selected_spec.SAFETY_USB_SCOPE,
                "module_init_probe_authority": "active-live-unproved",
                **selected_spec.RUNTIME_AUTHORITY,
            }
        )
...
def verify_repro_result(
    ...
    *,
    ...
    proof_bound_adapters = {
        ...
        P284_SOURCE_CONTRACT_ID: "s22plus-fyg8-p284-linked-audit-v1",
        P286_SOURCE_CONTRACT_ID: "s22plus-fyg8-p286-linked-audit-v1",
    }
    ...
    if required_adapter is not None and (
        ...
        if (
            ...
            raise BuildError("P2.84 linked audit adapter mismatch")
        if (
            exact_contract.get("source_contract_id")
            == P286_SOURCE_CONTRACT_ID
        ):
            raise BuildError("P2.86 linked audit adapter mismatch")
    ...
    if source_contract_id in {
        ...
        repro.P286_SOURCE_CONTRACT_ID,
    }:
        if intent_path is None or patch_path is None:
            label = (
                {
                    repro.P280_SOURCE_CONTRACT_ID: "P2.80",
                    repro.P282_SOURCE_CONTRACT_ID: "P2.82",
                    repro.P284_SOURCE_CONTRACT_ID: "P2.84",
                    repro.P286_SOURCE_CONTRACT_ID: "P2.86",
                }[source_contract_id]
            )
            raise BuildError(
        ...
        try:
            ...
            elif source_contract_id == repro.P284_SOURCE_CONTRACT_ID:
                pre_lto_qualification = (
                    repro.verify_p284_qualification_file(
                        value.get("pre_lto_qualification"),
                        exact_contract,
                        intent_path=intent_path,
                        patch_path=patch_path,
            ...
            elif source_contract_id == repro.P286_SOURCE_CONTRACT_ID:
                pre_lto_qualification = (
...
def build_candidate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if exact_contract["profile"] == "E1B":
        ...
        closure_api = (
            ...
            else e2_closure_selector.select(source_contract_id)
        )
        plan_header = None
    ...
    with tempfile.TemporaryDirectory(
        ...
        except (packager.PackageError, carrier.BuildError) as exc:
            raise BuildError(str(exc)) from exc
        frame_path = staging / "boot.img.lz4"
        ap_path = staging / "odin4/AP.tar.md5"
        shutil.rmtree(work)
