#!/usr/bin/env python3
"""Build V3232 GPU H3 Static Context probe."""
...
import build_native_init_boot_v3230_gpu_h3_vpc_lm_siv_probe as previous

base = previous.base

CYCLE = "V3232"
INIT_VERSION = "0.11.43"
INIT_BUILD = "v3232-gpu-h3-static-context-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3232_GPU_H3_STATIC_CONTEXT_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3232_gpu_h3_static_context_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3232_gpu_h3_static_context_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v574_gpu_h3_static_context_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3232"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3232.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3232.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3232"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3232-gpu-h3-static-context-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3232-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3232-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3232-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3232-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3232-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3232-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3232-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-static-context-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-static-context-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3232")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3232")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3232")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3232")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3232")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3232")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3232")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3232")
SFX_STREAM_MARKER = "a90.doomgeneric.v3232.audio=real-sfx-pcm-stream-gpu-h3-static-context-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-static-context-probe-v3232"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3232.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3232_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3230": b"a90-doomgeneric-v3232",
        b"a90.doomgeneric.v3230": b"a90.doomgeneric.v3232",
        b"v3230": b"v3232",
        b"V3230": b"V3232",
...
GPU_H3_STATIC_CONTEXT_MARKERS = (
    ...
    b"gpu.h3.draw.gras_su_conservative_ras_cntl=0x%x",
    b"gpu.h3.draw.vpc_unknown_9210=0x%x",
    b"gpu.h3.draw.vpc_so_override=0x%x",
    b"gpu.h3.draw.vpc_rast_stream_cntl=0x%x",
    b"gpu.h3.draw.pc_stereo_rendering_cntl=0x%x",
    b"gpu.h3.draw.tpl1_ps_swizzle_cntl=0x%x",
    b"gpu.h3.draw.sp_reg_prog_id_3=0x%x",
)

REQUIRED_STRINGS = tuple(_rewrite_v3232_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3230-gpu-h3-vpc-lm-siv-probe",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-static-context-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3232-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-static-context-mov-f32-shader-timeout-guard",
...
def v3232_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-static-context-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-static-context-probe")
        .replace("v3208", "v3232")
        .replace("V3208", "V3232")
    )


def _overlay_preserved_v3232_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3230 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3232 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3232 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3232 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3232-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            if item == "--ramdisk":
                ...
                raise RuntimeError("V3230 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3232 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3232 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3230-ramdisk-overlay-v3232-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-static-context-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-static-context-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-static-context-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3230-ramdisk-overlay-v3232-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-static-context-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-static-context-live-validation"
...
def _apply_v3232_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-static-context-probe")
        ...
        "v3210_adapter_source": v3232_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3232_ramdisk,
