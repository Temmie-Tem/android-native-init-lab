#!/usr/bin/env python3
"""Guarded S22+ M34 S5 soft-connect runtime-gadget live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M34 S5 keeps S4's stock-kernel-proven ssusb speed/mode levers, binds
`UDC=a600000.dwc3`, then writes the UDC soft_connect fallback before parking
for host observation.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M34-S5-SOFT-CONNECT-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M34-S5-SOFT-CONNECT-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_STAGE = "S5"
EXPECTED_M34_MARKER = "S22_NATIVE_INIT_M34_RUNTIME_GADGET_SPLIT_S5"
EXPECTED_M34_AP_SHA256 = "3a63dc339577d4aaf550159743b81edd9c1318ef5c6c4b745ed363f171d30d5e"
EXPECTED_M34_BOOT_SHA256 = "09751f5fce9f25be3ce7b814f00c04cafd22ae9a96d8c69ab9d52b6274951a95"
EXPECTED_M34_INIT_SHA256 = "efecaf1842aff95907b2f2780dc12531b0980acff6cbe64f789e9ad4b6c3c55c"
EXPECTED_M34_MODULE_LIST_SHA256 = "2291dc1c72add131c42d0b4ed6649880c20316d0598e0a2af942cc774949062c"
EXPECTED_M34_TEMPLATE_SOURCE_SHA256 = "bf90fbadbaf72bb9287150d769104b97ec8faaae0ce1c0591aaafdeb88004fb8"
...
EXPECTED_MODULE_ENTRY = "s22plus_m34_s5_runtime_gadget_split.modules"
...
DEFAULT_M34_AP = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_4/S5/odin4/AP.tar.md5")
DEFAULT_M34_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_4/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m34_s5_soft_connect_live_gate_{utc_stamp()}")
...
def policy_required_markers() -> list[str]:
    return [
        "S22+ M34 S5 soft-connect runtime-gadget native-init boot-only",
...
def agents_exception_draft() -> str:
    ...
    return f"""   **DRAFT ONLY - Narrow operator-authorized exception (2026-07-09, S22+ M34 S5 soft-connect runtime-gadget boot-only live gate):**
   ...
   one bounded attended boot-partition-only M34 S5 live gate on the Samsung S22+
   ...
   It must make no descriptor or companion-function change.
   It must have no
   ...
   authorize S1/S2/S3/S4 repeat, S5 repeat, post-pullup command channels, DTBO surgery,
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    if draft_only:
        raise SystemExit("AGENTS.md contains draft-only M34 S5 authorization text; refuse to treat draft as active live auth")
    missing = missing_policy_markers(agents)
    append_log(log_path, f"agents_exception_missing={missing}")
    if missing:
        raise SystemExit(f"AGENTS.md missing M34 S5 runtime-gadget authorization markers: {missing}")
...
def verify_m34_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m34_s5_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m34_s5_manifest_runtime_steps={json.dumps(runtime_steps, sort_keys=True)}")
    append_log(log_path, f"m34_s5_manifest_closure={json.dumps(closure, sort_keys=True)}")
    append_log(log_path, f"m34_s5_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")
    append_log(log_path, f"m34_s5_manifest_init_file={init_info.get('file', '')}")
    ...
    if matrix.get("live_order") != ["S1", "S2", "S3", "S4", "S5"]:
    ...
    if stage.get("stage_number") != 5:
        raise SystemExit(f"M34 S5 stage number mismatch: {stage.get('stage_number')!r}")
    expected_steps = {
    ...
    if runtime_steps != expected_steps:
        raise SystemExit(f"M34 S5 runtime steps mismatch: {runtime_steps!r}")
    required_hashes = {
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M34 S5 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M34 S5 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    if closure.get("modules") != EXPECTED_MODULES:
        raise SystemExit(f"M34 S5 module closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M34 S5 module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M34_MODULE_LIST_SHA256:
        raise SystemExit(f"M34 S5 module list SHA mismatch: {closure.get('module_sha256')!r}")
    if any(module in closure.get("modules", []) for module in ("phy-msm-ssusb-qmp.ko", "eud.ko")):
        raise SystemExit("M34 S5 excluded QMP/EUD module leaked into closure")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M34 S5 manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M34 S5 module list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M34 S5 must not inject module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 1:
        raise SystemExit("M34 S5 must inject exactly one module-list text file into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M34 S5 required string missing from manifest: {required}")
    if "/sys/class/usb_role" in required_strings or "phase=usb_role_done" in required_strings:
        raise SystemExit("M34 S5 manifest still requires the dead /sys/class/usb_role path")

    objdump_path = path.parent / EXPECTED_STAGE / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M34 S5 objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S5 /init does not load arm64 __NR_finit_module (273)")
    if any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S5 /init must not load arm64 __NR_reboot (142)")
...
def verify_m34_artifacts(
    *,
    ...
    verify_ap(m34_ap, EXPECTED_M34_AP_SHA256, "m34_s5_candidate", log_path)
...
def is_m34_s5_acm(device: dict[str, Any]) -> bool:
    return (
...
def observe_park_survival(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m34_s5_observe_start_utc={utc_now()}")
    append_log(log_path, f"m34_s5_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m34_s5_park_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m34_s5_result=unexpected_odin_before_survival_window elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "unexpected-odin-before-survival-window", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M34 S5 observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m34_s5_result=unexpected_adb_before_survival_window elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-survival-window", None
            for device in acm_devices(log_path, label):
                if is_m34_s5_acm(device):
                    append_log(log_path, f"m34_s5_acm_seen=1 elapsed_sec={elapsed:.3f} device={json.dumps(device, sort_keys=True)}")
                    return "acm-seen-manual-download-required", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m34_s5_survival_window_pass=1")
    append_log(log_path, "m34_s5_result=survived-observation-window-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m34_s5_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m34_s5_{label}_boot_restore_hash_check=skipped rollback_target=stock")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m34_s5_{label}_rollback", android, marker=EXPECTED_M34_MARKER)
    append_log(log_path, f"m34_s5_{label}_retained_marker_found={int(marker_found)}")
    return (0, android)
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m34-s5-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M34 S5 rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    if args.observe_sec < 60:
        raise SystemExit("--observe-sec must be at least 60 for the M34 S5 soft-connect discriminator")
    if args.snapshot_interval_sec < 1.0:
        raise SystemExit("--snapshot-interval-sec must be at least 1.0")

    root = repo_root()
    run_dir = resolve_run_dir(root, args.run_dir)
    log_path = run_dir / "s22plus_m34_s5_soft_connect_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M34 S5 soft-connect live gate ===")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M34 S5 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M34 S5 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M34 S5 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M34 S5 live gate starting. This candidate should park after S5 ssusb soft-connect setup "
    ...
    if odin_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print("download mode did not appear for M34 S5 candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        record_timeline_event(run_dir, "live_session_end")
        print(f"M34 S5 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m34_s5_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M34 S5 candidate flashed. Observing for {args.observe_sec}s survival window.", flush=True)
    ...
    if not successful_result:
        if rollback_device is None:
            record_timeline_event(run_dir, "live_session_end")
            print(
                f"M34 S5 stopped before survival proof ({result}). If the device is not in Android, "
    ...
    print(
        f"M34 S5 result={result}. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print(
            f"M34 S5 survival window passed, but manual Download mode did not appear. "
    ...
    record_timeline_event(run_dir, "live_session_end")
    print(f"M34 S5 live gate completed rc={rc}; result={result}; log={log_path}")
    return rc
