"""Fixed P357 namespace over sealed P353 H0 machinery; no device authority."""
...
def load(namespace):
    name = Path(namespace['__file__']).name.replace('p357', 'p353')
    relative, expected = PREDECESSORS[name]
    path = ROOT / relative
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != expected:
        raise ValueError('P357 sealed predecessor changed: ' + name)
    raw = raw.replace(b'P353', b'P357').replace(b'p353', b'p357')
    raw = raw.replace(b'c353f1e0a90b5e6d7c8a9b0c1d2e3f0b', b'c357f1e0a90b5e6d7c8a9b0c1d2e3f0b')
    raw = raw.replace(b'68f6aa66cb29cf94cd7c857545cffd54fc0a250f9bb0a98208a7a153a32e84d9',
                      b'5c9af16e809e9a17de851cc2707d93d20dac3ac14df3da161fae4740e9c99541')
    tree = ast.parse(raw)
    tree.body = [n for n in tree.body if not (isinstance(n, ast.If) and '__name__' in ast.unparse(n.test))]
    entry = namespace['__name__']
    try:
        namespace['__name__'] = entry if entry != '__main__' else 'p357_h0_template'
        exec(compile(ast.fix_missing_locations(tree), str(path) + '#p357', 'exec'), namespace)
    finally:
