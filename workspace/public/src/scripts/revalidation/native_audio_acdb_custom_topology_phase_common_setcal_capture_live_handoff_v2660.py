#!/usr/bin/env python3
"""V2660 Android handoff wrapper for V2659 custom-topology ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2659 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2659
...
The own-process send path is the V2553 full-manifest helper selected by V2659:
...
The V2659 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
ROOT = v2659.ROOT
RUN_ID = "V2660"
BUILD_TAG = "v2660-audio-acdb-custom-topology-phase-common-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2660_AUDIO_ACDB_CUSTOM_TOPOLOGY_PHASE_COMMON_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2660-acdb-custom-topology-phase-common-setcal-capture-{stamp}"
...
def build_v2659_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2659_build_root,
        manifest=args.v2659_manifest_path,
        report=v2659.DEFAULT_REPORT,
    ...
    payload = v2659.make_payload(build_args)
    args.v2659_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2659_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2659_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2659_artifacts:
        build_v2659_artifacts(args)
    manifest = read_v2659_manifest(args.v2659_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2660-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2660-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    if real_set_events:
        classification = "v2660-boundary-violation-real-audio-set-passthrough"
    elif reentry_neutralized and not rows:
        classification = "v2660-phase-common-reentry-neutralized-no-setcal"
    elif postinit_real_common_called and not rows:
        classification = "v2660-postinit-real-common-returned-before-setcal-no-setcal"
    elif init_short_success and not rows:
        classification = "v2660-init-short-success-sigsegv-before-postinit-common-no-setcal"
    elif not rows:
        classification = f"v2660-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2660-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2660-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2660-setcal-records-no-custom-topology"
    else:
        classification = "v2660-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2660-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2660-custom-topology-setcal-partial",
            "v2660-setcal-records-no-custom-topology",
            "v2660-phase-common-reentry-neutralized-no-setcal",
            "v2660-postinit-real-common-returned-before-setcal-no-setcal",
            "v2660-init-short-success-sigsegv-before-postinit-common-no-setcal",
        },
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2660-acdb-custom-topology-phase-common-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2659_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2659 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2660 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2659_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2660-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2659_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2659 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2660-init-short-success-sigsegv-before-postinit-common-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
    ...
    if summary.get("classification") == "v2660-phase-common-reentry-neutralized-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2659 phase hook neutralized nested common-topology",
    ...
    if summary.get("classification") == "v2660-postinit-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the corrected V2659 preload reached the real common-topology",
            ...
            "- this is useful frontier evidence: V2659 passed the init-short phase and reached",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2659-artifacts", action="store_true")
    parser.add_argument("--v2659-build-root", type=Path, default=v2659.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2659-manifest-path", type=Path, default=v2659.DEFAULT_MANIFEST)
