#!/usr/bin/env python3
"""V2422 exact-gated clone-following Android msm_audio_cal payload capture.

This runner executes the V2421 clone-following observer plan under the checked
Android handoff model. It boots the pinned stock Android image, verifies Magisk
root, stages the private clone-following observer, captures fd-filtered
/dev/msm_audio_cal ioctl metadata around bounded Android AudioTrack speaker
playback, pulls private artifacts, cleans up, reboots to recovery, and rolls
back to V2321.
...
import native_audio_acdb_clone_follow_planner_v2421 as v2421
import native_audio_android_route_delta_handoff_v2365 as route


RUN_ID = "V2422"
BUILD_TAG = "v2422-audio-acdb-clone-follow-live"
ROOT = v2421.ROOT
APPROVAL_PHRASE = v2421.APPROVAL_PHRASE
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_CAPTURE_WARMUP_SEC = 0.75


def rel(path: Path | str) -> str:
    return v2421.rel(path)


def default_live_out_dir() -> Path:
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    return DEFAULT_OUT_BASE / f"v2422-acdb-clone-follow-capture-{stamp}"
...
def args_for_v2421(args: argparse.Namespace) -> argparse.Namespace:
...
def ensure_live_approval(args: argparse.Namespace) -> None:
    if args.approval != APPROVAL_PHRASE:
        raise RuntimeError("exact AUD-5E clone-follow ACDB payload capture approval phrase is required for --run-live")
...
def summarize_capture_artifacts(out_dir: Path) -> dict[str, Any]:
    ...
    for jsonl_path in sorted(artifact_root.rglob("msm-audio-cal-clone-*.jsonl")):
    ...
    if summary["ioctl_entries"]:
        ...
        summary["classification"] = "clone-follow-helper-started-no-msm-audio-cal-ioctl"
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    route_args = v2415_android_args(args)
    plan = v2421.dry_run_payload(args_for_v2421(args))
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2422 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2422 command safety failed: {plan.get('command_safety')}")
    ...
    result: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2422-acdb-clone-follow-capture-live-started",
        "out_dir": rel(out_dir),
        "approval_ok": True,
        "plan": plan,
        "steps": steps,
    ...
    try:
        ...
        for index, command in enumerate(plan["commands"]["stage_clone_follow_helper_and_stimulus"]):
        ...
        logcat_capture["record"]["name"] = "clone-follow-payload-capture-logcat"
        logcat_capture["record"]["filter_regex_offline"] = v2396.LOG_FILTER_REGEX
        steps.append(logcat_capture["record"])

        steps.append(route.run_step("payload-capture-start-background", v2421.capture_start_command(args), out_dir, timeout_sec=args.adb_command_timeout))
        ...
        steps.append(route.run_step("prepare-private-artifacts-for-pull", v2421.prepare_collect_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False))
        steps.append(route.run_step("collect-private-artifacts", v2421.collect_command(args)[:-1] + [str(out_dir / "device-artifacts")], out_dir, timeout_sec=args.adb_command_timeout, check=False))
        result["payload_capture_summary"] = summarize_capture_artifacts(out_dir)

        for index, command in enumerate(v2421.cleanup_commands(args)):
        ...
        if classification == "captured-msm-audio-cal-payload-events":
            result["decision"] = "v2422-acdb-clone-follow-capture-events-before-rollback"
        else:
            result["decision"] = f"v2422-acdb-clone-follow-capture-{classification}-before-rollback"
        result["ok"] = True
    except Exception as error:
        result["decision"] = "v2422-acdb-clone-follow-capture-failed-before-rollback"
...
def v2415_android_args(args: argparse.Namespace) -> argparse.Namespace:
    return v2421.v2415.android_args(args)


def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    namespace = args_for_v2421(args)
    namespace.materialize_capture_helper = args.materialize_capture_helper
    payload = v2421.dry_run_payload(namespace)
    payload.update({
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2422-acdb-clone-follow-capture-live-dry-run",
        "host_only": True,
        "device_action": "none",
        "live_runner": rel(ROOT / "workspace/public/src/scripts/revalidation/native_audio_acdb_clone_follow_live_handoff_v2422.py"),
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2422 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2422 clone-following capture")
    parser.add_argument("--materialize-capture-helper", action="store_true", help="compile private AArch64 clone-follow observer for dry-run readiness")
    parser.add_argument("--helper-out-dir", type=Path, default=v2421.DEFAULT_HELPER_OUT_DIR)
    parser.add_argument("--cc", default=v2421.DEFAULT_CC)
    ...
    parser.add_argument("--capture-duration-sec", type=int, default=v2421.DEFAULT_DURATION_SEC)
    parser.add_argument("--capture-warmup-sec", type=float, default=DEFAULT_CAPTURE_WARMUP_SEC)
    parser.add_argument("--max-bytes", type=int, default=v2421.DEFAULT_MAX_BYTES)
    parser.add_argument("--process-poll-sec", type=float, default=v2421.DEFAULT_PROCESS_POLL_SEC)
    ...
    if args.run_live:
        try:
            ...
            payload = {
                "run_id": RUN_ID,
                "build_tag": BUILD_TAG,
                "decision": "v2422-acdb-clone-follow-capture-live-refused",
                "ok": False,
                "rolled_back": False,
                "reason": str(error),
            }
