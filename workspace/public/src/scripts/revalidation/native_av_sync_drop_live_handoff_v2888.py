#!/usr/bin/env python3
"""V2888 live validation for V2887 audio-sync late-frame drop accounting.

This reuses the V2882 PCM-file audio + page-flip video co-run envelope, but
flashes the V2887 candidate and passes ``video stream --sync-audio-status`` so
video frame deadlines are anchored to the audio PCM worker's listen_begin_ns.
It validates late-frame accounting and scheduling, not human-perceptual A/V sync
quality.
"""
...
RUN_ID = "V2888"
CANDIDATE_IMAGE = v2882.ROOT / "workspace/private/inputs/boot_images/boot_linux_v2887_av_sync_drop.img"
CANDIDATE_VERSION = "0.10.29"
CANDIDATE_TAG = "v2887-av-sync-drop"
CANDIDATE_SHA256 = "c9daccf175fdf794c6ef4d6c1cb02180380f041094de32a8afdcd161e11cb5aa"
REPORT_PATH = v2882.ROOT / "docs/reports/NATIVE_INIT_V2888_AV_SYNC_DROP_LIVE_2026-06-19.md"
REMOTE_AV_ROOT = "/mnt/sdext/a90/runtime/av/v2888"
...
def sync_summary(video_text: str, audio_timeline: dict[str, Any]) -> dict[str, Any]:
    ...
    dropped_frames = marker_int(video_text, "video.stream.dropped_frames")
    first_presented_frame = marker_int(video_text, "video.stream.audio_sync.first_presented_frame")
    initial_drop_late_ns = marker_int(video_text, "video.stream.audio_sync.initial_drop_late_ns")
    ...
    return {
        ...
        "drop_policy_marker": "video.stream.audio_sync.drop_policy=late-frame-skip" in video_text,
        "dropped_frames": dropped_frames,
        "dropped_frames_present": dropped_frames is not None and dropped_frames >= 0,
...
def sync_stream_pass(sync: dict[str, Any]) -> bool:
    return bool(
        ...
        and sync.get("drop_policy_marker")
        and sync.get("dropped_frames_present")
        and sync.get("first_presented_frame_present")
        and sync.get("initial_drop_late_present")
    )


def classify_drop_video(video_text: str, expected_frames: int) -> dict[str, Any]:
    base = v2882.video_live.classify_stream_output(video_text, expected_frames)
    presented = marker_int(video_text, "video.stream.presented") or 0
    dropped = marker_int(video_text, "video.stream.dropped_frames")
    flip_events = marker_int(video_text, "video.stream.flip_events") or 0
    if dropped is None:
        dropped = -1
    ...
    base.update({
        ...
        "frame_accounting_ok": frame_accounting_ok,
        "flip_accounting_ok": flip_accounting_ok,
        "pass": frame_accounting_ok
        and flip_accounting_ok
        and base.get("sha256_match")
        and base.get("sha256_checked")
        and base.get("pixel_format")
        and base.get("requested_pageflip")
        and base.get("present_pageflip")
        and base.get("path_ok"),
    })
    return base
...
def classify_av_result(video_text: str, audio_text: str, expected_frames: int) -> dict[str, Any]:
    ...
    video_summary = classify_drop_video(video_text, expected_frames)
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        *installed_lines,
        ...
        f"- Video presented/dropped/expected: `{video_summary.get('presented')}` / `{video_summary.get('dropped_frames')}` / `{video_summary.get('expected_frames')}`",
        f"- Video accounted frames: `{video_summary.get('accounted_frames')}`",
        f"- Video flip events/presented: `{video_summary.get('flip_events')}` / `{video_summary.get('presented')}`",
        ...
        f"- Sync-drop pass: `{int(bool(av.get('sync_stream_pass')))}`",
        "",
        "## Sync Stream Markers",
        "",
        *sync_lines,
...
def normalize_decision(result: dict[str, Any]) -> None:
    ...
    if isinstance(decision, str) and decision.startswith("v2882-"):
        result["decision"] = decision.replace("v2882-", "v2888-", 1)
...
def main() -> int:
    ...
    return 0 if (
        result.get("decision") == "v2888-av-corun-live-pass-before-rollback"
