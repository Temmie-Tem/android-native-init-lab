#!/usr/bin/env python3
"""Guarded S22+ M31B watchdog-managed park native-init live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M31B is not a Download beacon. It loads only the stock watchdog dependency
closure and parks. The live proof is survival past the PMIC/PON reset window;
manual Download mode is recovery-only for rollback, not a self-download pass.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M31B-WDT-MANAGED-PARK-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M31B-WDT-MANAGED-PARK-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M31B_MARKER = "S22_NATIVE_INIT_M31B_WDT_MANAGED_PARK"
EXPECTED_M31B_AP_SHA256 = "06d1c149c7c09a284062826f21ac848220e99d552d6b91762abbfb80f3679527"
EXPECTED_M31B_BOOT_SHA256 = "206fbb40df69a496f7fbe67e32cf862049d9258ef518db6949e1b5db2f4afdc4"
EXPECTED_M31B_INIT_SHA256 = "b01e52d3762e3cbdcba3501b00bb1dc9f9084899550ea23b92df43884bed23d0"
EXPECTED_M31B_MODULE_LIST_SHA256 = "80da959311e4a0f6bedb40da3c6f74c7fd5918017e40e0787b3e17c153cfe937"
EXPECTED_M31B_SOURCE_SHA256 = "32d85b4aeb64e5e1615b175b93fde166795598bfa0614934a9dcfb1bb165230d"
EXPECTED_M31B_KERNEL_SHA256 = "bceca73edbfca3499148e16741c939779157925949ef6bc8a8e31d6b68fc2cff"
EXPECTED_M31B_BASE_BOOT_SHA256 = EXPECTED_BASE_BOOT_SHA256
...
DEFAULT_M31B_AP = Path("workspace/private/outputs/s22plus_native_init/m31b_wdt_managed_park_v0_1/odin4/AP.tar.md5")
DEFAULT_M31B_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m31b_wdt_managed_park_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m31b_wdt_managed_park_live_gate_{utc_stamp()}")
...
def policy_required_markers() -> list[str]:
    return [
        "S22+ M31B watchdog-managed park native-init boot-only",
        "workspace/public/src/scripts/revalidation/s22plus_m31b_wdt_managed_park_live_gate.py",
        LIVE_ACK_TOKEN,
        ROLLBACK_ACK_TOKEN,
        EXPECTED_TARGET,
        EXPECTED_M31B_MARKER,
        EXPECTED_M31B_AP_SHA256,
        EXPECTED_M31B_BOOT_SHA256,
        EXPECTED_M31B_INIT_SHA256,
        EXPECTED_M31B_MODULE_LIST_SHA256,
        EXPECTED_M31B_SOURCE_SHA256,
        EXPECTED_M31B_KERNEL_SHA256,
        EXPECTED_M31B_BASE_BOOT_SHA256,
        "watchdog-managed park",
        "no reboot syscall",
        "no Download beacon",
        "no USB/configfs/ACM",
        "manual Download rollback is recovery-only",
        "survives past 60-120 seconds",
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M31B watchdog-managed park authorization markers: {missing}")


def verify_m31b_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M31B manifest missing: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    hashes = data.get("hashes", {})
    safety = data.get("safety", {})
    closure = data.get("closure", {})
    ramdisk = data.get("ramdisk", {})
    init_info = data.get("m31b_init", {})
    tar_members_seen = data.get("tar_members")
    append_log(log_path, f"m31b_manifest_path={path}")
    append_log(log_path, f"m31b_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    append_log(log_path, f"m31b_manifest_safety={json.dumps(safety, sort_keys=True)}")
    append_log(log_path, f"m31b_manifest_closure={json.dumps(closure, sort_keys=True)}")
    append_log(log_path, f"m31b_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")
    append_log(log_path, f"m31b_manifest_init_file={init_info.get('file', '')}")

    if data.get("target") != EXPECTED_TARGET:
        raise SystemExit(f"M31B target mismatch: {data.get('target')!r}")
    required_hashes = {
        "ap_tar_md5": EXPECTED_M31B_AP_SHA256,
        "boot_img": EXPECTED_M31B_BOOT_SHA256,
        "base_boot": EXPECTED_M31B_BASE_BOOT_SHA256,
        "kernel": EXPECTED_M31B_KERNEL_SHA256,
        "m31b_init": EXPECTED_M31B_INIT_SHA256,
        "m31b_wdt_modules": EXPECTED_M31B_MODULE_LIST_SHA256,
        "source": EXPECTED_M31B_SOURCE_SHA256,
    }
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M31B manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M31B manifest tar members mismatch: {tar_members_seen!r}")
    ...
    for key, expected in required_safety.items():
        if safety.get(key) != expected:
            raise SystemExit(f"M31B manifest safety {key} mismatch: {safety.get(key)!r} != {expected!r}")
    if closure.get("modules") != EXPECTED_MODULES:
        raise SystemExit(f"M31B watchdog closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M31B watchdog closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M31B_MODULE_LIST_SHA256:
        raise SystemExit(f"M31B module list SHA mismatch: {closure.get('module_sha256')!r}")
    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M31B manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != "s22plus_m31b_wdt_managed.modules":
        raise SystemExit(f"M31B module list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M31B must not inject module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 1:
        raise SystemExit("M31B must inject exactly one module-list text file into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M31B required string missing from manifest: {required}")
...
def verify_m31b_artifacts(
    *,
    ...
    verify_ap(m31b_ap, EXPECTED_M31B_AP_SHA256, "m31b_candidate", log_path)
    verify_m31b_manifest(m31b_manifest, log_path)
...
def observe_park_survival(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m31b_observe_start_utc={utc_now()}")
    append_log(log_path, f"m31b_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m31b_park_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m31b_result=unexpected_odin_before_survival_window elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "unexpected-odin-before-survival-window", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M31B observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m31b_result=unexpected_adb_before_survival_window elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-survival-window", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m31b_survival_window_pass=1")
    append_log(log_path, "m31b_result=survived-observation-window-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m31b_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if rollback_target == ROLLBACK_MAGISK:
        verify_partition_hash(log_path, android, "boot", EXPECTED_M31B_BASE_BOOT_SHA256, f"{label}_boot_restore")
    else:
        append_log(log_path, f"m31b_{label}_boot_restore_hash_check=skipped rollback_target=stock")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m31b_{label}_rollback", android, marker=EXPECTED_M31B_MARKER)
    append_log(log_path, f"m31b_{label}_retained_marker_found={int(marker_found)}")
    return (0, android)
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m31b-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M31B rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--m31b-ap", type=Path, default=DEFAULT_M31B_AP)
    parser.add_argument("--m31b-manifest", type=Path, default=DEFAULT_M31B_MANIFEST)
    ...
    if args.observe_sec < 60:
        raise SystemExit("--observe-sec must be at least 60 for the M31B watchdog-ceiling discriminator")
    if args.snapshot_interval_sec < 1.0:
        raise SystemExit("--snapshot-interval-sec must be at least 1.0")

    root = repo_root()
    run_dir = resolve_run_dir(root, args.run_dir)
    log_path = run_dir / "s22plus_m31b_wdt_managed_park_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M31B watchdog-managed park live gate ===")
    append_log(log_path, f"target={EXPECTED_TARGET}")

    odin = resolve(root, args.odin)
    m31b_ap = resolve(root, args.m31b_ap)
    m31b_manifest = resolve(root, args.m31b_manifest)
    ...
    verify_m31b_artifacts(
        m31b_ap=m31b_ap,
        m31b_manifest=m31b_manifest,
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M31B candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M31B rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    verify_partition_hash(log_path, selected_serial, "boot", EXPECTED_M31B_BASE_BOOT_SHA256, "current")
    host_snapshot(run_dir, log_path, "dryrun_current", odin)

    if not args.live:
        print(
            "dry-run ok: M31B candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M31B live gate starting. This candidate should park; do not press keys during "
    ...
    if odin_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print("download mode did not appear for M31B candidate flash", file=sys.stderr)
        return 2

    record_timeline_event(run_dir, "candidate_flash_start")
    candidate_rc = flash_ap(odin, m31b_ap, odin_device, log_path, "candidate")
    record_timeline_event(run_dir, "candidate_flash_done")
    if candidate_rc != 0:
        record_timeline_event(run_dir, "live_session_end")
        print(f"M31B candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m31b_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M31B candidate flashed. Observing for {args.observe_sec}s survival window.", flush=True)
    ...
    if result != "survived-observation-window-manual-download-required":
        if rollback_device is None:
            record_timeline_event(run_dir, "live_session_end")
            print(
                f"M31B stopped before survival proof ({result}). If the device is not in Android, "
    ...
    print(
        "M31B survived the observation window. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print(
            f"M31B survival window passed, but manual Download mode did not appear. "
    ...
    record_timeline_event(run_dir, "live_session_end")
    print(f"M31B live gate completed rc={rc}; result={result}; log={log_path}")
    return rc
