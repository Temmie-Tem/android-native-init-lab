#!/usr/bin/env python3
"""V2669 Android handoff wrapper for V2668 direct-real-common ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2668 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2668
...
The own-process send path is the V2668 direct-real-common preload:
...
The V2668 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
import build_android_acdb_common_direct_real_setcal_capture_v2668 as v2668
...
ROOT = v2668.ROOT
RUN_ID = "V2669"
BUILD_TAG = "v2669-audio-acdb-direct-real-common-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2669_AUDIO_ACDB_DIRECT_REAL_COMMON_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2669-acdb-direct-real-common-setcal-capture-{stamp}"
...
def build_v2668_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2668_build_root,
        manifest=args.v2668_manifest_path,
        report=v2668.DEFAULT_REPORT,
        clang=v2668.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2668.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    payload = v2668.make_payload(build_args)
    args.v2668_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2668_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2668_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    setcal_contract_ok = bool(
        ...
        and required.get("phase_hook_init_real_event_path_v2668")
        ...
        and required.get("phase_hook_direct_common_offset")
        and required.get("phase_hook_direct_common_address_event")
        and required.get("phase_hook_avoids_rtld_next_common_lookup")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2668_artifacts:
        build_v2668_artifacts(args)
    manifest = read_v2668_manifest(args.v2668_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2669-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2669-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    phase_path = artifact_dir / "acdb-v2668-direct-real-common-events.jsonl"
    setcal_events = [
    ...
    phase_stages = [
        ...
        if event.get("event") == "v2668_direct_real_common"
    ]
    ...
    if real_set_events:
        classification = "v2669-boundary-violation-real-audio-set-passthrough"
    elif reentry_neutralized and not rows:
        classification = "v2669-phase-common-reentry-neutralized-no-setcal"
    elif direct_real_common_called and not rows:
        classification = "v2669-direct-real-common-returned-before-setcal-no-setcal"
    elif init_short_success and not rows:
        classification = "v2669-init-hook-entered-no-setcal"
    elif not rows:
        classification = f"v2669-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2669-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2669-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2669-setcal-records-no-custom-topology"
    else:
        classification = "v2669-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2669-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2669-custom-topology-setcal-partial",
            "v2669-setcal-records-no-custom-topology",
            "v2669-phase-common-reentry-neutralized-no-setcal",
            "v2669-direct-real-common-returned-before-setcal-no-setcal",
            "v2669-init-hook-entered-no-setcal",
        },
        "operator_valuable": bool(rows or init_short_success or direct_real_common_called or reentry_neutralized),
        "counts_toward_fails_twice": (
            False
            if init_short_success or direct_real_common_called or reentry_neutralized
        ...
        "direct_real_common_called": direct_real_common_called,
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2669-acdb-direct-real-common-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2668_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2668 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2669 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2668_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2669-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2668_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
        ...
        f"- direct_real_common_called: `{summary.get('direct_real_common_called')}`",
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2668 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2669-init-hook-entered-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2668 init hook was entered:",
    ...
    if summary.get("classification") == "v2669-phase-common-reentry-neutralized-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2668 phase hook neutralized nested common-topology",
    ...
    if summary.get("classification") == "v2669-direct-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2668 preload reached the real common-topology",
            ...
            "- this is useful frontier evidence: V2668 passed the init-short phase and reached",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2668-artifacts", action="store_true")
    parser.add_argument("--v2668-build-root", type=Path, default=v2668.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2668-manifest-path", type=Path, default=v2668.DEFAULT_MANIFEST)
