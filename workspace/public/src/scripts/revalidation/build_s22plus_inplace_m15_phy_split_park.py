#!/usr/bin/env python3
"""Build the S22+ M15 PHY-split add-back native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M15 starts from the stable M13 no-module floor and reintroduces only the two
PHY-side modules from the failed M14 core group. This separates "can the PHY
half be loaded under native-init" from "does loading dwc3-msm/usb_f_ss_acm
cause the loop". The boot ramdisk gets only a small text list; module binaries
remain in stock vendor_boot /lib/modules.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m15_phy_split_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m15_phy_split_park.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M15"
...
RUNTIME_MODULES_PHY_SPLIT_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_PHY_SPLIT_RAMDISK = "s22plus_m15_phy_split.modules"
...
M14_WITHHELD_AFTER_M15 = [
    "dwc3-msm.ko",
    "usb_f_ss_acm.ko",
]

M12_WITHHELD_AFTER_M15 = [
    "usb_f_diag.ko",
    "usb_f_qdss.ko",
    "usb_f_gsi.ko",
...
def is_blocked_module(name: str) -> bool:
    if name in EXPLICIT_M15_BLOCKLIST:
        return True
    lower = name.lower()
...
def derive_phy_split(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    recovery_set = set(recovery_lines)
    dep_map = parse_modules_dep(modules_dep_lines)

    missing_recovery = sorted(module for module in PHY_SPLIT_MODULES if module not in recovery_set)
    missing_dep = sorted(module for module in PHY_SPLIT_MODULES if module not in dep_map)
    if missing_recovery:
        raise SystemExit(f"PHY-split module missing from modules.load.recovery: {missing_recovery}")
    if missing_dep:
        raise SystemExit(f"PHY-split module missing from modules.dep: {missing_dep}")

    ordered = list(PHY_SPLIT_MODULES)
    if len(ordered) != 2:
        raise SystemExit(f"PHY-split module count changed: {len(ordered)} != 2")
    if len(set(ordered)) != len(ordered):
        raise SystemExit("PHY-split common module list contains duplicates")
    if any(is_blocked_module(name) for name in ordered):
        raise SystemExit("PHY-split common module list includes an explicit M15 blocklist item")
    if not ordered:
        raise SystemExit("derived M15 PHY split is empty")

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_PHY_SPLIT_BUF:
        raise SystemExit(
            "M15 PHY split does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_PHY_SPLIT_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M15 PHY split basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "phy_split_modules": PHY_SPLIT_MODULES,
        "phy_split_count": len(PHY_SPLIT_MODULES),
        "m14_withheld_after_m15": M14_WITHHELD_AFTER_M15,
        "m14_withheld_after_m15_count": len(M14_WITHHELD_AFTER_M15),
        "m12_withheld_after_m15": M12_WITHHELD_AFTER_M15,
        "m12_withheld_after_m15_count": len(M12_WITHHELD_AFTER_M15),
        "order_source": "M14 core group split: PHY-side modules only",
        "explicit_m15_blocklist": sorted(EXPLICIT_M15_BLOCKLIST),
        ...
        "runtime_modules_phy_split_buffer": RUNTIME_MODULES_PHY_SPLIT_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    phy_split = derive_phy_split(recovery_lines, modules_dep_lines)

    missing_ko = sorted(module for module in PHY_SPLIT_MODULES if module not in ko_names)
    missing_recovery = sorted(module for module in PHY_SPLIT_MODULES if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"PHY-split vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"PHY-split module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M15 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in PHY_SPLIT_MODULES}
    ...
    (build_dir / MODULES_PHY_SPLIT_RAMDISK).write_text(str(phy_split["subset_text"]), encoding="ascii")
    (build_dir / "m15_phy_split.txt").write_text(str(phy_split["subset_text"]), encoding="ascii")
    return {
        ...
        "m15_phy_split": {key: value for key, value in phy_split.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M15 PHY-split init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M15 PHY-split init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M15 PHY-split init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M15 PHY-split init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M15 PHY-split init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M15 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M15 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M15 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M15 /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M15 init unexpectedly contains arm64 __NR_reboot (142)")
    ...
    (build_dir / "m15_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m15_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m15_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m15_init = build_dir / "s22plus_init_usb_acm_m15"
    m15_init_info = compile_init(source, m15_init, build_dir)
    ...
    subset_file = build_dir / MODULES_PHY_SPLIT_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M15 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m15_init}"],
        work_dir,
        "replace /init with M15 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_PHY_SPLIT_RAMDISK} {subset_file}"],
        work_dir,
        "add M15 PHY split list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M15 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m15_init):
        raise SystemExit("replaced /init does not match compiled M15 init")
    extracted_subset = build_dir / MODULES_PHY_SPLIT_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_PHY_SPLIT_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M15 PHY split list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M15 PHY split list does not match builder output")
    ...
    sizes = {
        ...
        "m15_phy_split": subset_file.stat().st_size,
        "m15_init": m15_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M15 in-place park-based USB-ACM native-init using the two PHY-side modules from the failed M14 core group",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_PHY_SPLIT_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_PHY_SPLIT_RAMDISK,
