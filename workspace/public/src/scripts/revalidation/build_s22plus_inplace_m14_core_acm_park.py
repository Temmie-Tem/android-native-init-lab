#!/usr/bin/env python3
"""Build the S22+ M14 core-ACM add-back native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M14 starts from the stable M13 no-module floor and reintroduces only four M12
core USB/ACM modules. This is a smaller add-back than M12's 24-module floor:
if it loops, the fault is in the core USB/ACM group; if it parks, the next unit
can add role/PD modules without changing configfs at the same time. The boot
ramdisk gets only a small text list; module binaries remain in stock
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m14_core_acm_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m14_core_acm_park.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M14"
...
RUNTIME_MODULES_CORE_ACM_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_CORE_ACM_RAMDISK = "s22plus_m14_core_acm.modules"
...
def is_blocked_module(name: str) -> bool:
    if name in EXPLICIT_M14_BLOCKLIST:
        return True
    lower = name.lower()
...
def derive_core_acm(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    recovery_set = set(recovery_lines)
    dep_map = parse_modules_dep(modules_dep_lines)

    missing_recovery = sorted(module for module in CORE_ACM_MODULES if module not in recovery_set)
    missing_dep = sorted(module for module in CORE_ACM_MODULES if module not in dep_map)
    if missing_recovery:
        raise SystemExit(f"core-ACM module missing from modules.load.recovery: {missing_recovery}")
    if missing_dep:
        raise SystemExit(f"core-ACM module missing from modules.dep: {missing_dep}")

    ordered = list(CORE_ACM_MODULES)
    if len(ordered) != 4:
        raise SystemExit(f"core-ACM module count changed: {len(ordered)} != 4")
    if len(set(ordered)) != len(ordered):
        raise SystemExit("core-ACM common module list contains duplicates")
    if any(is_blocked_module(name) for name in ordered):
        raise SystemExit("core-ACM common module list includes an explicit M14 blocklist item")
    if not ordered:
        raise SystemExit("derived M14 core ACM is empty")

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_CORE_ACM_BUF:
        raise SystemExit(
            "M14 core ACM does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_CORE_ACM_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M14 core ACM basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "core_acm_modules": CORE_ACM_MODULES,
        "core_acm_count": len(CORE_ACM_MODULES),
        "m12_withheld_after_m14": M12_WITHHELD_AFTER_M14,
        "m12_withheld_after_m14_count": len(M12_WITHHELD_AFTER_M14),
        "order_source": "M12 floor order filtered to core USB/ACM bring-up modules",
        "explicit_m14_blocklist": sorted(EXPLICIT_M14_BLOCKLIST),
        ...
        "runtime_modules_core_acm_buffer": RUNTIME_MODULES_CORE_ACM_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    core_acm = derive_core_acm(recovery_lines, modules_dep_lines)

    missing_ko = sorted(module for module in CORE_ACM_MODULES if module not in ko_names)
    missing_recovery = sorted(module for module in CORE_ACM_MODULES if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"core-ACM vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"core-ACM module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M14 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in CORE_ACM_MODULES}
    ...
    (build_dir / MODULES_CORE_ACM_RAMDISK).write_text(str(core_acm["subset_text"]), encoding="ascii")
    (build_dir / "m14_core_acm.txt").write_text(str(core_acm["subset_text"]), encoding="ascii")
    return {
        ...
        "m14_core_acm": {key: value for key, value in core_acm.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M14 core-ACM init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M14 core-ACM init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M14 core-ACM init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M14 core-ACM init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M14 core-ACM init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M14 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M14 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M14 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M14 /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M14 init unexpectedly contains arm64 __NR_reboot (142)")
    ...
    (build_dir / "m14_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m14_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m14_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m14_init = build_dir / "s22plus_init_usb_acm_m14"
    m14_init_info = compile_init(source, m14_init, build_dir)
    ...
    subset_file = build_dir / MODULES_CORE_ACM_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M14 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m14_init}"],
        work_dir,
        "replace /init with M14 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_CORE_ACM_RAMDISK} {subset_file}"],
        work_dir,
        "add M14 core ACM list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M14 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m14_init):
        raise SystemExit("replaced /init does not match compiled M14 init")
    extracted_subset = build_dir / MODULES_CORE_ACM_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_CORE_ACM_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M14 core ACM list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M14 core ACM list does not match builder output")
    ...
    sizes = {
        ...
        "m14_core_acm": subset_file.stat().st_size,
        "m14_init": m14_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M14 in-place park-based USB-ACM native-init using four core USB/ACM add-back modules from vendor_boot",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_CORE_ACM_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_CORE_ACM_RAMDISK,
