#!/usr/bin/env python3
"""Bind the unchanged stock module/rootfs closure to the P3.13 plan."""
...
from s22plus_fyg8_p312_e2_stock_closure import *  # noqa: F403
import s22plus_fyg8_p304_e2_stock_closure as module_parent
import s22plus_fyg8_p312_e2_stock_closure as parent
import s22plus_fyg8_p313_overlay_contract as overlay


SCHEMA = "s22plus_fyg8_p313_stock_closure_h0_v1"
VERDICT = "PASS_P313_STOCK_61_MODULE_CLOSURE_HOST_ONLY"
ClosureError = parent.ClosureError
INCIDENTAL_PATH = parent.parent.INCIDENTAL_PATH
INCIDENTAL_PATH_OFFSET = 0x45B9
INCIDENTAL_INSTRUCTION_WINDOW_OFFSET = 0x45B8
INCIDENTAL_INSTRUCTION_WINDOW = parent.parent.INCIDENTAL_INSTRUCTION_WINDOW


def select(source_contract_id: str | None):
    if source_contract_id != overlay.PARENT_SOURCE_CONTRACT_ID:
        raise ClosureError("P3.13 source contract differs")
...
def derive_module_closure(
    ...
    if plan_header is None:
        raise ClosureError("P3.13 exact materialized plan is missing")
    ...
    supplied = module_parent.p241.stable_read(
        plan_header, "P3.13 materialized plan", 1024 * 1024
    )
    expected = exact.get("generated_artifacts", {}).get("plan_header")
    if module_parent.receipt(supplied) != expected:
        raise ClosureError("P3.13 supplied plan identity differs")
    ...
    if result.get("plan_header") != expected:
        raise ClosureError("P3.13 derived plan receipt differs")
    return result
...
def _scrub_exact_incidental_opcode_path(data: bytes) -> bytes:
    if (
        ...
        raise ClosureError("P3.13 incidental opcode-path receipt mismatch")
    return (
...
def _validate_p313_authority_strings(data: bytes) -> None:
    p310 = parent.p310_parent
    p300 = p310.parent
    ...
    if (
        ...
        raise ClosureError("P3.13 candidate absolute-path authority mismatch")
...
def exact_init_authority(expected: bytes) -> Iterator[None]:
    def validate(data: bytes) -> None:
        if data != expected:
            raise ClosureError("P3.13 effective init differs from source-bound userspace")
        _validate_p313_authority_strings(data)

    p300 = parent.p310_parent.parent
