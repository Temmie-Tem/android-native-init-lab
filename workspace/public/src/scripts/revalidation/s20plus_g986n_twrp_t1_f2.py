#!/usr/bin/env python3
"""Connected owner for the exact S20+ TWRP T1 campaign.

This is deliberately a separate owner from the boot-only B0 process.  It can
address only the SHA-pinned T1 candidate and exact-stock rollback AP archives,
...
import s20plus_g986n_twrp_t1_profile_h0 as h0  # noqa: E402
import s20plus_g986n_recovery_digest_profile_h0 as recovery_digest  # noqa: E402
import s20plus_g986n_recovery_canary_t0_f2 as t0_predecessor  # noqa: E402
...
RUN_ROOT = ROOT / "workspace/private/runs/s20plus-g986n-twrp-t1-f2"
...
T0_PREDECESSOR_RUN_ID = "run-1788181993142157566"
T0_PREDECESSOR_TERMINAL = (
    ROOT
    / "workspace/private/runs/s20plus-g986n-recovery-canary-t0-f2"
    / T0_PREDECESSOR_RUN_ID
    / "terminal.json"
)
T0_PREDECESSOR_TERMINAL_SIZE = 2_188
T0_PREDECESSOR_TERMINAL_SHA256 = (
    "94a7edbf5607f60cb19eeb3c7f09795682ccecb26a4dc807bed2485263b9e67a"
)

RUN_ID_RE = re.compile(r"run-[0-9]{19}")
HEX64_RE = re.compile(r"[0-9a-f]{64}")
APPROVAL_PREFIX = "S20PLUS-G986N-TWRP-T1-F2-APPROVE:"
PHYSICAL_PREFIX = "S20PLUS-G986N-T1-PHYSICAL-ROLLBACK-CONFIRM:"
...
DEPENDENCIES = {
    ...
    "t0_predecessor_owner": (
        ROOT / "workspace/public/src/scripts/revalidation/s20plus_g986n_recovery_canary_t0_f2.py",
        153_798,
        "a91d45e14f4cb82f10a83a8c2bdc38126deb20b9b841918888ec45842ad51e75",
    ),
    "h0_closure": (
        ROOT / "workspace/public/src/scripts/revalidation/s20plus_g986n_twrp_t1_profile_h0.py",
        12_843,
...
_BOUND_APIS = {
    ...
    "t0_resolve_run": t0_predecessor.resolve_run,
    "t0_read_prepared": t0_predecessor.read_prepared,
    "t0_validate_journal": t0_predecessor.validate_journal,
    "t0_validate_terminal": t0_predecessor._validate_terminal,
}


class T1F2Error(RuntimeError):
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise T1F2Error("value is not canonical JSON") from exc
...
def _exact(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != keys:
        raise T1F2Error(f"{label} schema differs")
    return value
...
def _text(value: Any, label: str, maximum: int = 4096) -> str:
    if (
        ...
        raise T1F2Error(f"{label} is not bounded text")
    return value
...
def _hex64(value: Any, label: str) -> str:
    ...
    if HEX64_RE.fullmatch(text) is None:
        raise T1F2Error(f"{label} is not SHA-256")
    return text


def _integer(value: Any, label: str, minimum: int = 0) -> int:
    if type(value) is not int or value < minimum:
        raise T1F2Error(f"{label} is not an integer")
    return value
...
def _validate_endpoint(value: Any, label: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise T1F2Error(f"{label} identity differs")
    return item
...
def _validate_capture_receipt(
    ...
    if path.parent != run_dir or not path.name.endswith(".capture.json"):
        raise T1F2Error(f"{label} path escapes the run")
...
def _require_capture_name(receipt: dict[str, Any], pattern: str, label: str) -> None:
    if re.fullmatch(pattern, Path(receipt["path"]).name) is None:
        raise T1F2Error(f"{label} capture name differs")
...
def _validate_health(value: Any, run_dir: Path, label: str) -> dict[str, Any]:
    ...
    if item["target"] != h0.EXPECTED_TARGET:
        raise T1F2Error(f"{label} target differs")
    ...
    if item["topology_sha256"] != b0.EXPECTED_ANDROID_TOPOLOGY_SHA256:
        raise T1F2Error(f"{label} topology differs")
    expected_public = {
    ...
    if item["public_health"] != expected_public or item["root_health"] != b0.EXPECTED_ROOT_OUTPUT:
        raise T1F2Error(f"{label} Android/root health differs")
    ...
    if (
        ...
        raise T1F2Error(f"{label} root capture differs")
    ...
    if (
        ...
        raise T1F2Error(f"{label} recovery is not exact stock")
    ...
    if (
        ...
        raise T1F2Error(f"{label} recovery capture differs")
    match = re.fullmatch(
        r"(prepare|execute-pre-download|pre-candidate-abort|final)-"
    ...
    if match is None:
        raise T1F2Error(f"{label} recovery capture phase differs")
    phase = match.group(1)
    intent = _exact(
        b0.read_json(
    ...
    if (
        intent["schema"]
        != "s20plus_g986n_twrp_t1_recovery_read_intent_v1"
        ...
        raise T1F2Error(f"{label} recovery read intent differs")
...
def _validate_baseline(value: Any, label: str) -> dict[str, Any]:
    try:
        ...
        raise T1F2Error(f"{label} differs") from exc
...
def _validate_arrival(value: Any, prepared: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    # producer's digest domain instead of the T1 binding digest domain.
    if item["baseline_sha256"] != b0.digest(baseline):
        raise T1F2Error(f"{label} is not descended from the prepared baseline")
    return item
...
def _file_receipt(path: Path, size: int, sha256: str, label: str) -> dict[str, Any]:
    try:
        ...
        raise T1F2Error(f"{label} is unavailable") from exc
    if (
        path.is_symlink()
...
def normalized_self_sha256() -> str:
    ...
    if active_count != 1 or hash_count != 1:
        raise T1F2Error("runner normalization grammar changed")
...
def self_receipt(*, enforce_reviewed: bool = True) -> dict[str, Any]:
    ...
    if (
        ...
        raise T1F2Error("runner source is indirect")
    if enforce_reviewed and normalized != EXPECTED_REVIEWED_NORMALIZED_SHA256:
        raise T1F2Error("runner source is not independently reviewed")
    return {
        "path": str(SCRIPT),
...
def source_closure(*, enforce_self: bool = True) -> dict[str, Any]:
    ...
    if any(current[name] is not expected for name, expected in _BOUND_APIS.items()):
        raise T1F2Error("an imported execution API was rebound")
    receipts = {
...
def validate_t0_predecessor(
    *, expected_serial_sha256: str | None = None
) -> dict[str, Any]:
    """Re-derive only the fixed predecessor facts needed by T1."""

    receipt = _file_receipt(
        T0_PREDECESSOR_TERMINAL,
        T0_PREDECESSOR_TERMINAL_SIZE,
        T0_PREDECESSOR_TERMINAL_SHA256,
        "T0 predecessor terminal",
    )
    run_dir = t0_predecessor.resolve_run(T0_PREDECESSOR_RUN_ID)
    prepared = t0_predecessor.read_prepared(
        run_dir,
        require_unexpired=False,
        require_current_guard=False,
    )
    actual = t0_predecessor.validate_journal(run_dir, prepared)
    terminal = t0_predecessor._validate_terminal(run_dir, prepared, actual)
    final_health = terminal.get("final_health")
    if (
        terminal.get("verdict")
        != "NO_PROOF_T0_RETURNED_STOCK_RECOVERY_HEALTHY"
        ...
        raise T1F2Error("T0 predecessor no longer proves transfer and stock recovery")
    predecessor_serial = final_health.get("serial_sha256")
    _hex64(predecessor_serial, "T0 predecessor serial")
    if expected_serial_sha256 is not None:
        _hex64(expected_serial_sha256, "current T1 serial")
        if predecessor_serial != expected_serial_sha256:
            raise T1F2Error("current T1 serial is not the T0-qualified device")
    return {
        "terminal": receipt,
...
def validate_host_closure(
    *, enforce_self: bool = True, expected_serial_sha256: str | None = None
    ...
    if (
        ...
        raise T1F2Error("T1 H0 artifact closure differs")
    predecessor = validate_t0_predecessor(
        expected_serial_sha256=expected_serial_sha256
    )
    return {"sources": sources, "host": host, "t0_predecessor": predecessor}


def require_active() -> None:
    if not T1_F2_ACTIVE:
        raise T1F2Error("S20+ TWRP T1 F2 is not active")
    source_closure()
...
def _run_id(path: Path) -> str:
    if path.parent != RUN_ROOT or RUN_ID_RE.fullmatch(path.name) is None:
        raise T1F2Error("run path is outside the closed namespace")
    return path.name
...
def allocate_run_dir() -> Path:
    ...
    for _ in range(32):
        ...
        b0.fsync_dir(RUN_ROOT)
        return path
    raise T1F2Error("unable to allocate a unique T1 run")


def resolve_run(run_id: str) -> Path:
    _ensure_private_roots()
    if RUN_ID_RE.fullmatch(run_id) is None:
        raise T1F2Error("run ID grammar differs")
    ...
    if (
        ...
        raise T1F2Error("run directory is indirect")
...
def validate_namespace(run_dir: Path) -> set[str]:
    ...
    with os.scandir(run_dir) as entries:
        for entry in entries:
            ...
            if (
                ...
                raise T1F2Error("T1 journal contains an indirect node")
    ...
    if unknown:
        raise T1F2Error("T1 journal contains an unknown node")
    one_shot_prefixes = (
    ...
    for prefix in one_shot_prefixes:
        ...
        if len(ordinals) > 1:
            raise T1F2Error(f"T1 journal reuses one-shot raw prefix {prefix}")
    ...
    if (
        ...
        raise T1F2Error("T1 run directory changed during validation")
    return actual


def _guard_value(run_dir: Path) -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_twrp_t1_guard_v1",
        "version": VERSION,
...
def acquire_guard(run_dir: Path) -> None:
    if os.path.lexists(SHARED_GUARD):
        raise T1F2Error("shared S20+ action guard is already held")
...
def require_guard(run_dir: Path) -> None:
    if b0.read_json(SHARED_GUARD, "shared S20+ guard") != _guard_value(run_dir):
        raise T1F2Error("shared S20+ action guard differs")
...
def validate_candidate_claim(
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_global_claim_v1"
        ...
        raise T1F2Error("global candidate claim differs")
...
def require_fresh_candidate_unclaimed() -> None:
    ...
    if os.path.lexists(path):
        try:
            ...
            raise T1F2Error("global candidate claim is malformed") from exc
        raise T1F2Error("this exact T1 candidate is permanently consumed")
...
def _next_capture_name(run_dir: Path, prefix: str) -> str:
    for ordinal in range(1, 10_000):
        ...
        if not any(
            ...
            return name
    raise T1F2Error("raw-capture ordinal space is exhausted")
...
def android_stock_recovery_health(
    ...
    if prefix not in {
        ...
        raise T1F2Error("unknown fixed recovery-health phase")
    ...
    if os.path.lexists(read_intent_path):
        ...
        if (
            ...
            raise T1F2Error(f"{prefix} recovery read was consumed without a receipt")
        ...
        if captures or any(
            ...
            raise T1F2Error(f"{prefix} recovery evidence exists without intent")
    ...
    if (
        ...
        raise T1F2Error("Android identity changed during recovery digest read")
    return {
        "target": dict(h0.EXPECTED_TARGET),
        "serial_sha256": before["serial_sha256"],
        "topology_sha256": before["topology_sha256"],
...
def _adb_reboot_download(
    ...
    *,
    ...
    return {
        "schema": "s20plus_g986n_twrp_t1_reboot_result_v1",
        "version": VERSION,
        "phase": phase,
...
def prepare() -> dict[str, Any]:
    ...
    b0.durable_json(
        ...
        {
            "schema": "s20plus_g986n_twrp_t1_allocation_v1",
            "version": VERSION,
            "run_id": _run_id(run_dir),
    ...
    try:
        ...
        if validate_t0_predecessor(
            expected_serial_sha256=health["serial_sha256"]
        ) != closure["t0_predecessor"]:
            raise T1F2Error("T0 qualification changed during T1 preparation")
        ...
        return {
            "verdict": "PREPARED_S20PLUS_G986N_TWRP_T1_AWAITING_APPROVAL",
            "run_id": _run_id(run_dir),
        ...
        b0.durable_json(
            ...
            {
                "schema": "s20plus_g986n_twrp_t1_failure_v1",
                "version": VERSION,
...
def read_prepared(
    ...
    *,
    ...
    prepared = b0.read_json(run_dir / "prepared.json", "T1 prepared binding")
    allocation = b0.read_json(run_dir / "allocation.json", "T1 allocation")
    if (
        set(prepared) != {
            "schema", "version", "binding", "binding_sha256", "approval_token", "at"
        }
        or prepared.get("schema") != "s20plus_g986n_twrp_t1_prepared_v1"
        ...
        raise T1F2Error("prepared T1 binding is malformed")
    binding = _exact(
        prepared["binding"],
        {
    ...
    if (
        binding["schema"] != "s20plus_g986n_twrp_t1_binding_v1"
        ...
        raise T1F2Error("prepared T1 binding fields differ")
    ...
    if stored_baseline != baseline:
        raise T1F2Error("stored Download baseline differs from preparation")
    expected_allocation = {
        "schema": "s20plus_g986n_twrp_t1_allocation_v1",
        "version": VERSION,
        "run_id": _run_id(run_dir),
    ...
    if allocation != expected_allocation:
        raise T1F2Error("T1 allocation differs")
    ...
    if type(expires) is not int or (require_unexpired and int(time.time()) > expires):
        raise T1F2Error("prepared T1 approval expired")
...
def _validate_cage_binding(
    ...
    if (
        ...
        raise T1F2Error(f"{kind} process cage differs")
...
def _validate_cage_quiescence(
    ...
    if (
        ...
        raise T1F2Error(f"{kind} cage quiescence differs")
...
def _validate_approval(value: Any, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_approval_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T1F2Error("T1 approval differs")
...
def _validate_reboot_result(
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_reboot_result_v1"
        or item["version"] != VERSION
        or item["phase"] != phase
        or item["binding_sha256"] != binding_sha256
        ...
        raise T1F2Error(f"{phase} reboot result differs")
    ...
    if capture is None:
        if (
            ...
            raise T1F2Error(f"{phase} cut result differs")
        return item
    ...
    if (
        ...
        raise T1F2Error(f"{phase} reboot result is not raw-derived")
    return item
...
def _validate_transfer_intent_record(
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_transfer_intent_v1"
        or item["version"] != VERSION
        or item["kind"] != kind
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["ap"] != {
        ...
        raise T1F2Error(f"{kind} transfer intent differs")
    ...
    if os.path.lexists(run_dir / arrival_name):
        ...
        if not b0.same_download_session(arrival.get("endpoint", {}), item["endpoint"]):
            raise T1F2Error(f"{kind} intent endpoint is not arrival-bound")
...
def _validate_transfer_result_record(
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_transfer_result_v1"
        or item["version"] != VERSION
        or item["kind"] != kind
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T1F2Error(f"{kind} transfer result differs")
    ...
    if item["endpoint_post_state"] in {"same", "changed"}:
        ...
        if (
            ...
            raise T1F2Error(f"{kind} endpoint post identity differs")
    elif item["endpoint_post_identity"] is not None:
        raise T1F2Error(f"{kind} endpoint post state differs")
    capture = item["raw_capture"]
    if capture is None:
        if (
            ...
            raise T1F2Error(f"{kind} missing capture gained a result")
        return item
    ...
    if (
        ...
        raise T1F2Error(f"{kind} transfer result is not raw-derived")
    return item
...
def _validate_observation(
    ...
    if (
        ...
        raise T1F2Error("candidate recovery observation schema differs")
    item = value
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_observation_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["claim_verdict"] not in {"PROVED", "NO_PROOF"}
    ):
        raise T1F2Error("candidate recovery observation differs")
    _text(item["at"], "candidate observation time", 128)
    if item["claim_verdict"] == "NO_PROOF":
        if set(item) != no_proof:
            raise T1F2Error("NO_PROOF observation gained evidence")
        _text(item["reason"], "NO_PROOF reason")
        return item
    if set(item) != proved:
        raise T1F2Error("PROVED observation lacks evidence")
    if (
        ...
        raise T1F2Error("PROVED recovery identity differs")
    ...
    if (
        ...
        raise T1F2Error("PROVED recovery observation is not raw-derived")
    return item
...
def _validate_physical_arm(value: Any, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_physical_arm_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["mode"] not in {
        ...
        raise T1F2Error("physical rollback arm differs")
    ...
    if item["mode"] == "empty-baseline-physical-entry":
        ...
        if (
            ...
            raise T1F2Error("physical empty baseline contains an endpoint")
    else:
        if item["baseline"] is not None:
            raise T1F2Error("already-Download arm contains a baseline")
...
def _validate_physical_observation_record(
    ...
    if kind not in {"initial", "resume"}:
        raise T1F2Error("physical observation kind differs")
    ...
    if not os.path.lexists(intent_path):
        if os.path.lexists(miss_path):
            raise T1F2Error("physical observation miss lacks intent")
        return None
    intent = _exact(
    ...
    if (
        intent["schema"]
        != "s20plus_g986n_twrp_t1_physical_observation_intent_v1"
        or intent["version"] != VERSION
        ...
        raise T1F2Error(f"physical {kind} observation intent differs")
    ...
    if (
        miss["schema"]
        != "s20plus_g986n_twrp_t1_physical_observation_miss_v1"
        ...
        raise T1F2Error(f"physical {kind} observation miss differs")
...
def _validate_rollback_arrival(
    ...
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_rollback_arrival_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T1F2Error("rollback Download arrival differs")
    ...
    if item["branch"] == "candidate-outcome-unproved-already-download":
        ...
        if (
            ...
            raise T1F2Error("candidate-failure rollback arrival provenance differs")
        ...
        if (
            ...
            raise T1F2Error("automatic rollback arrival lacks its reboot result")
        baseline = _validate_baseline(
            b0.read_json(
        ...
        if (
            ...
            raise T1F2Error("automatic rollback arrival provenance differs")
    else:
        ...
        if (
            ...
            raise T1F2Error("physical rollback arrival provenance differs")
...
def validate_journal(run_dir: Path, prepared: dict[str, Any]) -> set[str]:
    ...
    for node, required in dependencies.items():
        if node in actual and not required <= actual:
            raise T1F2Error(f"T1 journal dependency is missing for {node}")
    for phase in (
        "prepare", "execute-pre-download", "pre-candidate-abort", "final"
    ):
        ...
        if (
            ...
            raise T1F2Error(f"{phase} recovery read intent differs")
    ...
    if "candidate-download-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_twrp_t1_download_intent_v1"
            or item["version"] != VERSION
            ...
            raise T1F2Error("candidate Download intent differs")
    ...
    if "recovery-boot-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_twrp_t1_recovery_boot_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            ...
            raise T1F2Error("recovery boot intent differs")
    ...
    if "candidate-observation-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_twrp_t1_observation_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            or type(item["attempt"]) is not int
            or item["attempt"] != 1
            ...
            raise T1F2Error("observation intent differs")
    ...
    if "rollback-download-intent.json" in actual:
        ...
        if (
            item["schema"]
            != "s20plus_g986n_twrp_t1_rollback_download_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            ...
            raise T1F2Error("rollback Download intent differs")
    ...
    if "physical-rollback-arm.json" in actual:
        ...
        if "physical-rollback-confirmation.json" in actual:
            ...
            if (
                confirmation["schema"]
                != "s20plus_g986n_twrp_t1_physical_confirmation_v1"
                ...
                raise T1F2Error("physical confirmation differs")
            _text(confirmation["at"], "physical confirmation time", 128)
        ...
        if (
            ...
            raise T1F2Error("physical observation has both miss and arrival")
    ...
    if os.path.lexists(claim_path):
        ...
        if "candidate-download-arrival.json" not in actual:
            raise T1F2Error("global candidate claim lacks candidate Download arrival")
    elif "candidate-intent.json" in actual:
        raise T1F2Error("candidate intent lacks the global no-replay claim")
...
def _validate_terminal(
    ...
    terminal = b0.read_json(run_dir / "terminal.json", "T1 terminal")
    if terminal.get("schema") == "s20plus_g986n_twrp_t1_retained_terminal_v1":
        item = _exact(
            terminal,
            {
        ...
        if not required <= actual or forbidden & actual:
            raise T1F2Error("retained T1 terminal graph differs")
        candidate = _validate_transfer_result_record(
            run_dir,
            b0.read_json(run_dir / "candidate-result.json", "candidate result"),
            "candidate",
        ...
        if (
            ...
            or item["binding_sha256"] != prepared["binding_sha256"]
            or item["verdict"] != "PROVED_T1_RECOVERY_RETAINED"
            ...
            raise T1F2Error("retained T1 terminal is not graph-derived")
        _text(item["at"], "retained T1 terminal time", 128)
        return item
    if terminal.get("schema") == "s20plus_g986n_twrp_t1_pre_candidate_abort_v1":
        item = _exact(
            terminal,
            {
                "schema", "version", "binding_sha256", "verdict",
        ...
        if (
            ...
            raise T1F2Error("pre-candidate terminal differs")
        ...
        if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
            raise T1F2Error("pre-candidate abort did not return through a new boot")
    ...
    if not {
        ...
        raise T1F2Error("T1 terminal lacks its complete result graph")
    candidate = _validate_transfer_result_record(
        run_dir,
        b0.read_json(run_dir / "candidate-result.json", "candidate result"),
        "candidate",
    ...
    expected_verdict = (
        "PROVED_T1_RETURNED_STOCK_RECOVERY_HEALTHY"
        if proved
        else (
            "NO_PROOF_T1_RETURNED_STOCK_RECOVERY_HEALTHY"
            if rollback_complete
            else "NO_PROOF_T1_STOCK_RECOVERY_HEALTHY_TRANSFER_UNPROVED"
        )
    )
    if (
        item["schema"] != "s20plus_g986n_twrp_t1_terminal_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T1F2Error("T1 terminal is not graph-derived")
...
def _same_endpoint(expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if not b0.same_download_session(current, expected):
        raise T1F2Error("Download endpoint session changed")
    return current
...
def _transfer(
    ...
    if kind == "candidate":
        ...
        raise T1F2Error("unknown recovery transfer kind")
    ...
    if os.path.lexists(intent_path) or os.path.lexists(result_path):
        raise T1F2Error(f"{kind} attempt is already consumed")
    ...
    try:
        with h0.pin_regular_file(
            ...
            if (
                ...
                raise T1F2Error("Download endpoint changed at Odin dispatch")
            handle, stdout, stderr = _capture(
                run_dir,
                f"{kind}-transfer",
                [
                    str(b0.DASH), "-c", b0.ODIN_CAGE_ENTRY_SCRIPT,
                    "s20plus-t1-odin-cage", cage_binding["cage"], *command,
                ],
                timeout=300,
            ...
            if (
                ...
                raise T1F2Error("an opened transfer input changed across Odin")
            try:
...
def recover_transfer_result(
    ...
    if len(captures) > 1:
        raise T1F2Error(f"{kind} transfer has multiple raw receipts")
    handle = None
    stdout = b""
    stderr = b""
    raw_receipt = None
...
def recover_reboot_result(
    ...
    if len(captures) > 1:
        raise T1F2Error(f"{phase} reboot has multiple raw receipts")
    handle = None
    stdout = b""
    stderr = b""
    raw_receipt = None
...
def _candidate_claim(run_dir: Path, prepared: dict[str, Any]) -> None:
    ...
    b0.durable_json(
        path,
        {
            "schema": "s20plus_g986n_twrp_t1_global_claim_v1",
            "version": VERSION,
            "run_id": _run_id(run_dir),
            "binding_sha256": prepared["binding_sha256"],
            "candidate_sha256": h0.CANDIDATE_AP_SHA256,
...
def _rollback_from_download(
    ...
    if not os.path.lexists(arrival_path):
        raise T1F2Error("stock rollback requires a durable bound Download arrival")
    ...
    if (
        ...
        raise T1F2Error("rollback Download arrival differs from dispatch input")
...
def execute(run_dir: Path, approval: str) -> dict[str, Any]:
    ...
    if approval != prepared["approval_token"]:
        raise T1F2Error("approval token does not match the prepared T1 binding")
    if os.path.lexists(run_dir / "approval.json"):
        raise T1F2Error("prepared T1 approval is already consumed")
    if digest(validate_host_closure()) != prepared["binding"]["closure_sha256"]:
        raise T1F2Error("T1 execution closure changed after preparation")
    b0.durable_json(
        run_dir / "approval.json",
        {
            "schema": "s20plus_g986n_twrp_t1_approval_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
    ...
    if (
        ...
        raise T1F2Error("prepared Android boot changed before candidate Download")
    require_fresh_candidate_unclaimed()
    if digest(
        ...
        raise T1F2Error("T1 execution closure changed before candidate Download")
    baseline = prepared["binding"]["initial_download_baseline"]
    if digest(baseline) != digest(
        ...
        raise T1F2Error("candidate Download baseline changed")
    intent = {
        "schema": "s20plus_g986n_twrp_t1_download_intent_v1",
        "version": VERSION,
        "phase": "candidate",
        "binding_sha256": prepared["binding_sha256"],
        "source": preflight,
    ...
    try:
        ...
        if not b0.same_download_session(current, endpoint):
            raise T1F2Error("post-candidate Download endpoint differs from the bound session")
        devices, listing_sha256 = b0.enumerate_download()
        if devices != [current["device"]]:
            raise T1F2Error("post-candidate Download listing differs")
        b0.durable_json(
            run_dir / "rollback-download-arrival.json",
            {
                "schema": "s20plus_g986n_twrp_t1_rollback_arrival_v1",
                "version": VERSION,
                "binding_sha256": prepared["binding_sha256"],
...
def abort_pre_candidate(run_dir: Path) -> dict[str, Any]:
    ...
    if any(os.path.lexists(run_dir / name) for name in forbidden):
        raise T1F2Error("pre-candidate abort is unavailable after a transfer intent")
    ...
    if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
        raise T1F2Error("pre-candidate abort requires a later Android boot")
    terminal = {
        "schema": "s20plus_g986n_twrp_t1_pre_candidate_abort_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "verdict": (
            "ABORTED_AFTER_CANDIDATE_CLAIM_NO_TRANSFER_STOCK_RECOVERY_HEALTHY"
...
def _recovery_observation(run_dir: Path, prepared: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    if os.path.lexists(run_dir / "candidate-observation-intent.json"):
        raise T1F2Error("candidate recovery observation is already consumed")
    b0.durable_json(
        run_dir / "candidate-observation-intent.json",
        {
            "schema": "s20plus_g986n_twrp_t1_observation_intent_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
            "attempt": 1,
            "no_replay": True,
    ...
    def no_proof(reason: str) -> tuple[dict[str, Any], None]:
        return {
            "schema": "s20plus_g986n_twrp_t1_observation_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if hashlib.sha256(devpath.encode()).hexdigest() != prepared["binding"]["preflight"]["topology_sha256"]:
                raise T1F2Error("recovery ADB topology differs")
            handle, stdout, stderr = _capture(
                run_dir,
                "candidate-recovery-observer",
            ...
            if len(final_rows) != 1 or final_rows[0]["state"] != "recovery":
                raise T1F2Error("TWRP ADB identity or state changed")
...
def _automatic_rollback_download(
    ...
    intent = {
        "schema": "s20plus_g986n_twrp_t1_rollback_download_intent_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
...
def observe_recovery(run_dir: Path) -> dict[str, Any]:
    ...
    if result["classification"] != "odin_transfer_completed":
        raise T1F2Error("only a completed candidate may enter recovery observation")
    b0.read_json(run_dir / "recovery-boot-intent.json", "recovery boot intent")
    ...
    if serial is None:
        return {
            "verdict": "NO_PROOF_T1_PHYSICAL_ROLLBACK_REQUIRED",
            "run_id": _run_id(run_dir),
            "candidate_replay": False,
        }
    if validate_namespace(run_dir) & PHYSICAL_ROLLBACK_NODES:
        return _pending(
            run_dir,
            prepared,
            "PROVED_T1_PHYSICAL_ROLLBACK_ALREADY_ARMED",
        )
...
def retain_proved_twrp(
    ...
    if observation.get("claim_verdict") != "PROVED":
        raise T1F2Error("only a proved TWRP observation may be retained")
    actual = validate_namespace(run_dir)
    if actual & PHYSICAL_ROLLBACK_NODES:
        raise T1F2Error("an armed physical rollback forbids TWRP retention")
    candidate = _validate_transfer_result_record(
        run_dir,
        b0.read_json(run_dir / "candidate-result.json", "candidate result"),
        "candidate",
    ...
    if candidate["classification"] != "odin_transfer_completed":
        raise T1F2Error("unproved candidate transfer cannot be retained")
    terminal = {
        "schema": "s20plus_g986n_twrp_t1_retained_terminal_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "verdict": "PROVED_T1_RECOVERY_RETAINED",
...
def arm_physical_rollback(run_dir: Path) -> dict[str, Any]:
    ...
    if os.path.lexists(run_dir / "candidate-observation.json"):
        ...
        if observation["claim_verdict"] == "PROVED":
            raise T1F2Error("proved TWRP cannot arm a competing physical rollback")
    if os.path.lexists(run_dir / "rollback-intent.json"):
        raise T1F2Error("stock recovery rollback is already consumed")
    if not os.path.lexists(run_dir / "candidate-intent.json"):
        raise T1F2Error("physical stock rollback requires a consumed candidate intent")
    path = run_dir / "physical-rollback-arm.json"
    if os.path.lexists(path):
        raise T1F2Error("physical rollback arm is already consumed")
    if os.path.lexists(run_dir / "rollback-download-arrival.json"):
        raise T1F2Error("rollback Download arrival is already bound")
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T1F2Error("physical rollback baseline is ambiguous")
    ...
    if not devices:
        ...
        if (
            ...
            raise T1F2Error("existing Download endpoint is not the candidate session")
        baseline = None
...
def _publish_physical_observation_intent(
    ...
    if os.path.lexists(path):
        ...
        if record is None or record.get("schema") != (
            ...
            raise T1F2Error(f"physical {kind} observation is already closed")
        return record
    value = {
        "schema": "s20plus_g986n_twrp_t1_physical_observation_intent_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "kind": kind,
...
def _publish_physical_observation_miss(
    ...
    b0.durable_json(
        ...
        {
            "schema": "s20plus_g986n_twrp_t1_physical_observation_miss_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
            "kind": kind,
...
def confirm_physical_rollback(
    ...
    if (
        ...
        raise T1F2Error("physical rollback confirmation differs or expired")
    if os.path.lexists(run_dir / "physical-rollback-confirmation.json"):
        raise T1F2Error("physical rollback confirmation is consumed; use --resume")
    b0.durable_json(
        run_dir / "physical-rollback-confirmation.json",
        {
            "schema": "s20plus_g986n_twrp_t1_physical_confirmation_v1",
...
def _wait_android_health(
    ...
    while time.monotonic() < deadline:
        try:
            ...
            raise T1F2Error("final ADB inventory is malformed") from exc
        ...
        if len(matches) > 1:
            raise T1F2Error("final exact-target ADB inventory is ambiguous")
        if not matches:
            if expected_rows:
                raise T1F2Error(
        ...
        if (
            ...
            raise T1F2Error("final exact target state or identity differs")
        break
    else:
        raise T1F2Error("final exact Android arrival timed out")
    health, _serial = android_stock_recovery_health(
        run_dir,
        "final",
        expected_serial,
...
def _publish_recovery_boot_intent(
    ...
    if os.path.lexists(run_dir / "recovery-boot-intent.json"):
        return
    if candidate["classification"] != "odin_transfer_completed":
        raise T1F2Error("unproved candidate cannot gain a recovery boot intent")
    b0.durable_json(
        run_dir / "recovery-boot-intent.json",
        {
            "schema": "s20plus_g986n_twrp_t1_recovery_boot_intent_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
...
def _bind_current_candidate_download(
    ...
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T1F2Error("candidate Download resume state is ambiguous")
    if not devices:
        return None
    endpoint = b0.identify_download()
    if endpoint["device"] != devices[0]:
        raise T1F2Error("candidate Download listing changed")
    arrival = {
        "endpoint": endpoint,
...
def _bind_current_rollback_download(
    ...
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T1F2Error("rollback Download resume state is ambiguous")
    if not devices:
        return None
    endpoint = b0.identify_download()
    if endpoint["device"] != devices[0]:
        raise T1F2Error("rollback Download listing changed")
    if branch == "automatic-recovery-adb":
        ...
        raise T1F2Error("rollback resume branch differs")
    arrival = {
        "schema": "s20plus_g986n_twrp_t1_rollback_arrival_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
...
def _pending(run_dir: Path, prepared: dict[str, Any], verdict: str, **extra: Any) -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_twrp_t1_pending_v1",
        "version": VERSION,
        "run_id": _run_id(run_dir),
        "binding_sha256": prepared["binding_sha256"],
        "verdict": verdict,
...
def resume_run(run_dir: Path) -> dict[str, Any]:
    ...
    if candidate["classification"] != "odin_transfer_completed":
        ...
        if observation["claim_verdict"] == "PROVED":
            ...
            return _pending(
                run_dir, prepared, "NO_PROOF_T1_PHYSICAL_ROLLBACK_REQUIRED"
            )
...
def finalize(run_dir: Path) -> dict[str, Any]:
    ...
    if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
        raise T1F2Error("final Android reused the prepared source boot ID")
    observation = (
        _validate_observation(
            run_dir,
    ...
    if proved:
        verdict = "PROVED_T1_RETURNED_STOCK_RECOVERY_HEALTHY"
    elif rollback_complete:
        verdict = "NO_PROOF_T1_RETURNED_STOCK_RECOVERY_HEALTHY"
    else:
        verdict = "NO_PROOF_T1_STOCK_RECOVERY_HEALTHY_TRANSFER_UNPROVED"
    terminal = {
        "schema": "s20plus_g986n_twrp_t1_terminal_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "verdict": verdict,
        "candidate_claim": (
...
def render_plan() -> dict[str, Any]:
    ...
    return {
        ...
        "version": VERSION,
        "status": (
            "BINDING_ATTENDED_TWRP_T1_F2_ACTIVE"
            if T1_F2_ACTIVE
            else "DORMANT_REVIEW_PENDING_NOT_ACTIVE"
        ),
        "active": T1_F2_ACTIVE,
        "live_authority": T1_F2_ACTIVE,
        "target": dict(h0.EXPECTED_TARGET),
        "candidate": {
            "size": h0.CANDIDATE_AP_SIZE,
            "sha256": h0.CANDIDATE_AP_SHA256,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Operate the exact S20+ TWRP T1 F2 owner"
    )
