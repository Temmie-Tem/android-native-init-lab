#!/usr/bin/env python3
"""Guarded S22+ M3.2 marker-only native-init live gate.

Dry-run is the default.  Live mode requires:

- the exact SHA-pinned M3.2 AGENTS.md exception;
- exact M3.2 boot-only AP hash and single `boot.img.lz4` tar member;
...
M3.2 is marker-only and uses stock-format legacy-LZ4 ramdisk packaging.  It does
not bring up USB/NCM itself; the host observes download-mode return and collects
pstore plus last-kmsg evidence after rollback.
"""
...
ACK_TOKEN = "S22PLUS-M32-MARKER-LIVE-GATE"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M32_AP_SHA256 = "6073e4988a98f741fa207df4efb8a05e144ad16b3a90f43db2ec408657936fc2"
EXPECTED_M32_BOOT_SHA256 = "0bb1ef280e42aa2c6069538e77fc21b5330cf9419a19785f79d05da8429bf1fc"
EXPECTED_M32_MARKER = "S22_NATIVE_INIT_MARKER_ONLY_M32"

DEFAULT_M32_AP = Path("workspace/private/outputs/s22plus_native_init/marker_m32_lz4ramdisk_v0_1/odin4/AP.tar.md5")
DEFAULT_M32_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/marker_m32_lz4ramdisk_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        run_dir = DEFAULT_RUN_ROOT / f"s22plus_m32_marker_live_gate_{stamp}"
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    required = [
        "S22+ M3.2 marker-only native-init boot-only",
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M3.2 live authorization markers: {missing}")


def verify_m32_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M3.2 manifest missing: {path}")
    ...
    append_log(log_path, f"m32_manifest_ramdisk_packaging={json.dumps(packaging, sort_keys=True)}")
    if hashes.get("ap_tar_md5") != EXPECTED_M32_AP_SHA256:
        raise SystemExit("M3.2 manifest AP hash does not match expected M3.2 AP")
    if hashes.get("boot_img") != EXPECTED_M32_BOOT_SHA256:
        raise SystemExit("M3.2 manifest boot image hash does not match expected M3.2 boot image")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M3.2 manifest tar members mismatch: {tar_members_seen!r}")
    if safety.get("auto_reboot") != "download-after-10s-observation":
        raise SystemExit(f"M3.2 manifest auto_reboot mismatch: {safety.get('auto_reboot')!r}")
    if safety.get("module_insertions") is not False:
        raise SystemExit(f"M3.2 manifest module_insertions mismatch: {safety.get('module_insertions')!r}")
    if safety.get("configfs_runtime_gadget") is not False:
        raise SystemExit(f"M3.2 manifest configfs_runtime_gadget mismatch: {safety.get('configfs_runtime_gadget')!r}")
    if packaging.get("format") != "legacy-lz4":
        raise SystemExit(f"M3.2 ramdisk format mismatch: {packaging.get('format')!r}")
    if packaging.get("magic_hex") != "02214c18":
        raise SystemExit(f"M3.2 ramdisk magic mismatch: {packaging.get('magic_hex')!r}")
    if packaging.get("roundtrip_sha256") != hashes.get("ramdisk_cpio"):
        raise SystemExit("M3.2 ramdisk roundtrip hash does not match ramdisk_cpio hash")
...
def main(argv: list[str]) -> int:
    ...
    log_path = run_dir / "s22plus_m32_marker_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus m32 marker live gate ===")
    ...
    if not args.live:
        print(f"dry-run ok: M3.2 candidate, rollback APs, AGENTS exception, and Android preflight verified; log={log_path}")
        return 0
    ...
    if odin_device is None:
        print("download mode did not appear for M3.2 candidate flash", file=sys.stderr)
        return 2

    candidate_rc = flash_ap(odin, m32_ap, odin_device, log_path, "candidate")
    if candidate_rc != 0:
        print(f"M3.2 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
        return candidate_rc or 3

    observe_candidate(run_dir, log_path, args.candidate_observe_sec, odin)
    print("M3.2 observation window ended. Waiting for download mode for rollback.")
    ...
    if not android_ok:
        ...
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", post_rollback_serial, marker=EXPECTED_M32_MARKER)
    print(f"M3.2 live gate completed with rollback ok; log={log_path}")
    return 0
