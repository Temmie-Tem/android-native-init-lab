#!/usr/bin/env python3
"""Build the S22+ M9A in-place C first-action reboot-download candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M9A is the post-M8A lower-layer discriminator. It preserves the freestanding C
native-init build style used by M5B/M8A, but removes every VFS/kmsg/mount/sleep
side effect. The replacement PID1's first externally observable action is one
direct reboot(2) syscall requesting Samsung download mode; if it returns, PID1
parks forever.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m9a_c_first_reboot_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m9a_c_first_reboot.c")
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | int | list[str]]:
    ...
    require_ok(result, "compile M9A C first-action reboot init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M9A C first-action reboot init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M9A C first-action reboot init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", "-S", out_path])
    require_ok(readelf, "readelf M9A C first-action reboot init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M9A C first-action reboot init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M9A init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M9A init is not AArch64")
    ...
    if svc_count != 1:
        raise SystemExit(f"M9A init expected exactly one svc instruction, saw {svc_count}: {svc_lines}")
    if not any("mov" in line and "x8" in line and "#0x8e" in line for line in objdump_lines):
        raise SystemExit("M9A init does not load arm64 __NR_reboot (142) into x8")

    binary = out_path.read_bytes()
    if DOWNLOAD_ARG.encode("ascii") not in binary:
        raise SystemExit("M9A /init missing download reboot argument")
    for forbidden in FORBIDDEN_STRINGS:
        if forbidden in binary:
            raise SystemExit(f"M9A /init contains forbidden string: {forbidden!r}")

    (build_dir / "m9a_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m9a_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m9a_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
        "file": (file_info.stdout + file_info.stderr).decode("utf-8", errors="replace").strip(),
        "readelf": readelf_text,
        "objdump": objdump_text,
        "svc_count": svc_count,
        "required_strings": [DOWNLOAD_ARG],
...
def main(argv: list[str]) -> int:
    ...
    m9a_init = build_dir / "s22plus_init_m9a_c_first_reboot"
    m9a_init_info = compile_init(source, m9a_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m9a_init}"], work_dir, "replace /init with M9A init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M9A patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m9a_init):
        raise SystemExit("replaced /init does not match compiled M9A init")
    ...
    sizes = {
        ...
        "m9a_init": m9a_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M9A in-place freestanding C first-action reboot-download discriminator",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns": "freestanding C entry and immediate reboot syscall path are viable; failure moves to VFS/kmsg/mount/sleep side effects",
            "no_download_mode": "freestanding C entry, compiler metadata, or stack/helper path remains suspect before adding any VFS work",
        },
    }
    ...
    (out_dir / "required_strings.txt").write_text(DOWNLOAD_ARG + "\n", encoding="ascii")
