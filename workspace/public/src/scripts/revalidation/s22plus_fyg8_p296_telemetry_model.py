#!/usr/bin/env python3
"""P2.96 retained model with built-in-only telemetry semantics."""
...
SCHEMA = "s22plus_fyg8_p296_builtin_dwc3_telemetry_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P2.96 profile: {profile}")
    if repair.is_publication_error_detail(outcome, detail):
        ...
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P2.96 publication-error record is at the wrong position"
            )
        return
    try:
        spec.validate_slot(
