#!/usr/bin/env python3
"""P2.98 retained model with gadget-start/event semantics."""
...
SCHEMA = "s22plus_fyg8_p298_gadget_start_event_telemetry_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P2.98 profile: {profile}")
    if repair.is_publication_error_detail(outcome, detail):
        ...
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P2.98 publication-error record is at the wrong position"
            )
        return
    try:
        spec.validate_slot(
