#!/usr/bin/env python3
"""Build the P3.04 userspace twice against the fixed P3.00 Image."""
...
SCHEMA = "s22plus_fyg8_p304_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p304/userspace")
COMPILE_FLAGS = base.COMPILE_FLAGS
BuildError = base.BuildError


@contextmanager
def module_count_context() -> Iterator[None]:
    selected = base._selected(contract.PARENT_SOURCE_CONTRACT_ID, contract.PROFILE)  # noqa: SLF001
    previous = selected.module.MODULE_PLAN_COUNT
    selected.module.MODULE_PLAN_COUNT = plan.MODULE_PLAN_COUNT
    try:
        yield
    finally:
        selected.module.MODULE_PLAN_COUNT = previous
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("unsupported P3.04 userspace profile or parent contract")
    return VERDICT


def _compile(root: Path, directory: Path, exact: dict, materialized: Path, tools: dict[str, str]) -> dict:
    with module_count_context():
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.04 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p304-userspace-{label}-") as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.04 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
        ...
        "module_plan_count": plan.MODULE_PLAN_COUNT,
        "safety": {
            "host_only": True,
            "kernel_built": False,
