#!/usr/bin/env python3
"""Build one P3.16 boot-only candidate after its closure validates."""

from __future__ import annotations

import argparse
import copy
import hashlib
import json
import os
from pathlib import Path
import shutil
import subprocess
import tempfile
from typing import Any

import build_s22plus_fyg8_p310_candidate as p310_builder
import build_s22plus_fyg8_p315_candidate as parent
import s22plus_fyg8_max77705_custom_surface_contract as surface
import s22plus_fyg8_p316_candidate_contract as candidate_contract
import s22plus_fyg8_p316_e2_stock_closure as closure
import s22plus_fyg8_p316_overlay_contract as contract
import s22plus_fyg8_p316_qualification_closure as qualification
import s22plus_fyg8_p316_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p316_candidate_artifact_result_v1"
VERDICT = "PASS_P316_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = contract.TARGET
P286_SOURCE_CONTRACT_ID = contract.PARENT_SOURCE_CONTRACT_ID
DEFAULT_IMAGE = contract.PARENT_IMAGE
DEFAULT_REPRO_RESULT = contract.PARENT_REPRO_RESULT
DEFAULT_USERSPACE = userspace.DEFAULT_OUT
DEFAULT_BASE_BOOT = parent.DEFAULT_BASE_BOOT
DEFAULT_VENDOR_RAMDISK = parent.DEFAULT_VENDOR_RAMDISK
DEFAULT_LZ4 = parent.DEFAULT_LZ4
DEFAULT_MAGISKBOOT = parent.DEFAULT_MAGISKBOOT
DEFAULT_DIAGNOSTIC = Path(
    "workspace/private/outputs/s22plus_fyg8_max77705_gate0/"
    "custom-module-build-20260812-07/module-stage/"
    "s22plus_max77705_mux_diag.ko"
)
DEFAULT_PREPACKAGING = qualification.DEFAULT_PREPACKAGING
DEFAULT_OUT = qualification.DEFAULT_CANDIDATE_A
DIAGNOSTIC_NAME = "s22plus_max77705_mux_diag.ko"
DIAGNOSTIC_RAMDISK_PATH = f"lib/modules/{DIAGNOSTIC_NAME}"
BOOT_SIZE = parent.BOOT_SIZE
KERNEL_START = parent.KERNEL_START
KERNEL_END = parent.KERNEL_END
BuildError = parent.BuildError
receipt = parent.receipt

_PREPACKAGING_RECEIPT: dict[str, Any] | None = None
_PREPACKAGING_VALIDATION: dict[str, Any] | None = None


def _generic_base():
    return parent._generic_base()  # noqa: SLF001


def _read_json(path: Path, label: str) -> tuple[bytes, dict[str, Any]]:
    payload = candidate_contract.stable_read(path, label, 64 * 1024 * 1024)
    try:
        value = json.loads(payload.decode("ascii"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise BuildError(f"{label} is not ASCII JSON") from exc
    if not isinstance(value, dict):
        raise BuildError(f"{label} root differs")
    return payload, value


def verify_repro_result(
    result_path: Path,
    image_receipt: dict[str, Any],
    exact_contract: dict[str, Any],
    **_ignored: Any,
) -> dict[str, Any]:
    return parent.verify_repro_result(result_path, image_receipt, exact_contract)


def artifact_safety(exact_contract: dict[str, Any]) -> dict[str, Any]:
    if _PREPACKAGING_RECEIPT is None or _PREPACKAGING_VALIDATION is None:
        raise BuildError("P3.16 prepackaging gate was not established")
    result = p310_builder._BASE_ARTIFACT_SAFETY(exact_contract)  # noqa: SLF001
    result.update(
        {
            "fixed_p310_image": True,
            "p316_kernel_rebuild": False,
            "p316_full_lto_ab": False,
            "early_stock_module_count": 64,
            "custom_module_binaries_injected": 1,
            "diagnostic_late_load_only": True,
            "diagnostic_absent_from_early_plan": True,
            "p316_prepackaging_closure": _PREPACKAGING_RECEIPT,
            "p316_prepackaging_validation": _PREPACKAGING_VALIDATION,
            "p316_runtime_fixture": exact_contract["runtime_fixture"],
            "p316_late_loader_lifecycle": exact_contract[
                "late_loader_lifecycle"
            ],
            "p316_sidecar_positive_control": exact_contract[
                "sidecar_positive_control"
            ],
            "fixed_image_sha256": contract.EXPECTED_IMAGE["sha256"],
        }
    )
    return result


def _configure() -> None:
    parent._configure()  # noqa: SLF001
    candidate_contract._configure()  # noqa: SLF001
    base = _generic_base()
    base.candidate_contract = candidate_contract
    base.userspace = userspace
    base.p286_closure = closure
    base.SCHEMA = SCHEMA
    base.VERDICT = VERDICT
    base.TARGET = TARGET
    base.P286_SOURCE_CONTRACT_ID = P286_SOURCE_CONTRACT_ID
    base.DEFAULT_IMAGE = DEFAULT_IMAGE
    base.DEFAULT_REPRO_RESULT = DEFAULT_REPRO_RESULT
    base.DEFAULT_USERSPACE = DEFAULT_USERSPACE
    base.DEFAULT_OUT = DEFAULT_OUT
    base.verify_repro_result = verify_repro_result
    base.artifact_safety = artifact_safety


def _inject_diagnostic(
    *,
    root: Path,
    base_candidate: Path,
    staging: Path,
    diagnostic: bytes,
    magiskboot: bytes,
    lz4: bytes,
    image: bytes,
    userspace_dir: Path,
) -> tuple[dict[str, Any], dict[str, Any]]:
    base = _generic_base()
    audit = staging / "audit"
    work = staging / "work"
    final_unpack = staging / "final-unpack"
    for directory in (audit, work, final_unpack):
        directory.mkdir()
    pinned_magiskboot = audit / "magiskboot"
    pinned_lz4 = audit / "lz4"
    diagnostic_path = audit / DIAGNOSTIC_NAME
    base.carrier.stage_file(pinned_magiskboot, magiskboot, executable=True)
    base.carrier.stage_file(pinned_lz4, lz4, executable=True)
    base.carrier.stage_file(diagnostic_path, diagnostic)
    source_boot = base_candidate / "boot.img"
    source_boot_bytes = candidate_contract.stable_read(
        source_boot, "P3.16 base candidate boot", BOOT_SIZE
    )
    if len(source_boot_bytes) != BOOT_SIZE:
        raise BuildError("P3.16 base candidate boot size differs")
    base.carrier.run_in_dir(
        [pinned_magiskboot, "unpack", "-h", source_boot],
        work,
        "P3.16 unpack base candidate",
    )
    ramdisk = work / "ramdisk.cpio"
    exists = base.carrier.run(
        [pinned_magiskboot, "cpio", ramdisk, f"exists {DIAGNOSTIC_RAMDISK_PATH}"],
        cwd=work,
    )
    if exists.returncode != 1:
        raise BuildError("P3.16 diagnostic is not absent from base ramdisk")
    base.carrier.run_in_dir(
        [
            pinned_magiskboot,
            "cpio",
            ramdisk,
            f"add 644 {DIAGNOSTIC_RAMDISK_PATH} {diagnostic_path}",
        ],
        work,
        "P3.16 add diagnostic module",
    )
    boot_path = staging / "boot.img"
    base.carrier.run_in_dir(
        [pinned_magiskboot, "repack", source_boot, boot_path],
        work,
        "P3.16 repack diagnostic carrier",
    )
    candidate = boot_path.read_bytes()
    if len(candidate) != BOOT_SIZE:
        raise BuildError("P3.16 diagnostic candidate boot size differs")
    base.carrier.run_in_dir(
        [pinned_magiskboot, "unpack", "-h", boot_path],
        final_unpack,
        "P3.16 unpack final candidate",
    )
    final_ramdisk = final_unpack / "ramdisk.cpio"
    final_init = audit / "init.final"
    final_child = audit / "child.final"
    final_diag = audit / "diag.final"
    for member, target in (
        ("init", final_init),
        ("s22-e1-child", final_child),
        (DIAGNOSTIC_RAMDISK_PATH, final_diag),
    ):
        base.carrier.run_in_dir(
            [pinned_magiskboot, "cpio", final_ramdisk, f"extract {member} {target}"],
            final_unpack,
            f"P3.16 extract final {member}",
        )
    expected_init = candidate_contract.stable_read(
        userspace_dir / "init", "P3.16 exact init", 1024 * 1024
    )
    expected_child = candidate_contract.stable_read(
        userspace_dir / "s22-e1-child", "P3.16 exact child", 1024 * 1024
    )
    if (
        final_init.read_bytes() != expected_init
        or final_child.read_bytes() != expected_child
        or final_diag.read_bytes() != diagnostic
        or (final_unpack / "kernel").read_bytes() != image
    ):
        raise BuildError("P3.16 final candidate content differs")
    try:
        package_result = base.packager.package(
            boot_path=boot_path,
            lz4_path=pinned_lz4,
            output_dir=staging,
            audit_dir=audit,
        )
    except (base.packager.PackageError, base.carrier.BuildError) as exc:
        raise BuildError(str(exc)) from exc
    construction = {
        "parent_boot": receipt(source_boot_bytes),
        "diagnostic_module": receipt(diagnostic),
        "diagnostic_staged_path": DIAGNOSTIC_RAMDISK_PATH,
        "diagnostic_absent_from_base": True,
        "diagnostic_absent_from_early_plan": True,
        "diagnostic_staged_exactly_once": True,
        "ramdisk_diagnostic_mode": "0644",
        "kernel_exact_fixed_image": True,
        "init_and_child_exact_userspace": True,
    }
    outputs = {
        "boot_img": receipt(candidate),
        "boot_img_lz4": package_result["boot_img_lz4"],
        "ap_tar_md5": package_result["ap_tar_md5"],
        "ap_structure": package_result["ap_structure"],
        "packager": package_result,
    }
    shutil.rmtree(work)
    shutil.rmtree(final_unpack)
    shutil.rmtree(audit)
    return construction, outputs


def build_candidate(args: argparse.Namespace) -> dict[str, Any]:
    global _PREPACKAGING_RECEIPT, _PREPACKAGING_VALIDATION

    root = candidate_contract.intent.repo_root()
    output = candidate_contract.intent.resolve(root, args.out)
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.16 candidate output exists: {output}")
    pre_path = candidate_contract.intent.resolve(root, args.prepackaging)
    pre_payload, pre = _read_json(pre_path, "P3.16 prepackaging closure")
    validation = qualification.validate_prepackaging_artifact(
        pre, root=root, intent_path=candidate_contract.intent.resolve(root, args.intent)
    )
    diagnostic = candidate_contract.stable_read(
        candidate_contract.intent.resolve(root, args.diagnostic),
        "P3.16 exact diagnostic module",
        4 * 1024 * 1024,
    )
    if receipt(diagnostic) != {
        "size": surface.DIAG_MODULE_IDENTITY[0],
        "sha256": surface.DIAG_MODULE_IDENTITY[1],
    }:
        raise BuildError("P3.16 diagnostic module identity differs")
    image = candidate_contract.stable_read(
        candidate_contract.intent.resolve(root, args.image),
        "P3.16 fixed Image",
        KERNEL_END - KERNEL_START,
    )
    if receipt(image) != contract.EXPECTED_IMAGE:
        raise BuildError("P3.16 fixed Image identity differs")
    lz4 = candidate_contract.stable_read(
        candidate_contract.intent.resolve(root, args.lz4), "P3.16 pinned lz4", 8 * 1024 * 1024
    )
    magiskboot = candidate_contract.stable_read(
        candidate_contract.intent.resolve(root, args.magiskboot),
        "P3.16 pinned magiskboot",
        32 * 1024 * 1024,
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    _PREPACKAGING_RECEIPT = receipt(pre_payload)
    _PREPACKAGING_VALIDATION = validation
    try:
        _configure()
        with tempfile.TemporaryDirectory(
            prefix=f".{output.name}.", dir=output.parent
        ) as temporary:
            temporary_path = Path(temporary)
            base_output = temporary_path / "base-candidate"
            final_output = temporary_path / "final-candidate"
            base_args = copy.copy(args)
            base_args.out = base_output
            base_result = _generic_base().build_candidate(base_args)
            final_output.mkdir()
            construction, outputs = _inject_diagnostic(
                root=root,
                base_candidate=base_output,
                staging=final_output,
                diagnostic=diagnostic,
                magiskboot=magiskboot,
                lz4=lz4,
                image=image,
                userspace_dir=candidate_contract.intent.resolve(root, args.userspace),
            )
            result = {
                **base_result,
                "schema": SCHEMA,
                "verdict": VERDICT,
                "construction": {
                    **base_result["construction"],
                    **construction,
                    "module_binaries_injected": 1,
                    "vendor_ramdisk_modules_reused": True,
                },
                "outputs": outputs,
                "prepackaging_closure": receipt(pre_payload),
                "prepackaging_validation": validation,
                "safety": artifact_safety(base_result["candidate_contract"]),
            }
            (final_output / "artifact-result.json").write_text(
                json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n",
                encoding="ascii",
            )
            os.replace(final_output, output)
    finally:
        _PREPACKAGING_RECEIPT = None
        _PREPACKAGING_VALIDATION = None
    return result


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    _configure()
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument("--diagnostic", type=Path, default=DEFAULT_DIAGNOSTIC)
    parser.add_argument("--prepackaging", type=Path, default=DEFAULT_PREPACKAGING)
    selected, remaining = parser.parse_known_args(argv)
    args = _generic_base().parse_args(remaining)
    args.diagnostic = selected.diagnostic
    args.prepackaging = selected.prepackaging
    return args


def main(argv: list[str] | None = None) -> int:
    try:
        result = build_candidate(parse_args(argv))
    except (
        BuildError,
        qualification.QualificationError,
        candidate_contract.ContractError,
        candidate_contract.intent.IntentError,
        subprocess.TimeoutExpired,
        OSError,
        ValueError,
    ) as exc:
        print(json.dumps({"schema": SCHEMA, "verdict": "FAIL_CLOSED", "error": str(exc)}))
        return 1
    print(
        json.dumps(
            {
                "schema": SCHEMA,
                "verdict": result["verdict"],
                "boot_sha256": result["outputs"]["boot_img"]["sha256"],
                "ap_sha256": result["outputs"]["ap_tar_md5"]["sha256"],
            },
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
