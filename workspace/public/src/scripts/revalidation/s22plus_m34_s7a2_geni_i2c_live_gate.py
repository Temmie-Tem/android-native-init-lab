#!/usr/bin/env python3
"""Guarded S22+ M34 S7A2 GENI I2C runtime-gadget live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M34 S7A2 starts from S7A, adds the missing GENI I2C transport for the
max77705 bus, keeps the minimal ACM-only configfs recipe, keeps
`ssusb/mode=peripheral`, uses a bounded TypeC role-write discriminator when no
partner is visible, binds `UDC=a600000.dwc3`, and parks for host observation.
It does not write EUD sysfs knobs, does not soft_connect, and does not add
FunctionFS or conn_gadget.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M34-S7A2-GENI-I2C-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M34-S7A2-GENI-I2C-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_STAGE = "S7A2"
EXPECTED_M34_MARKER = "S22_NATIVE_INIT_M34_RUNTIME_GADGET_SPLIT_S7A2"
EXPECTED_M34_AP_SHA256 = "cb89ccf9c8c5481938ddd415930c78a23e1a679d45fdc57f95e6d1b48776bd59"
EXPECTED_M34_BOOT_SHA256 = "b9a4d4c2170da2ed6125aa44734005303d81d874b72402513def97b2f8406a54"
EXPECTED_M34_INIT_SHA256 = "8f8eb4a6f4d94bc552ec61819b9c2b4ea4ec4de7fb7aa097fab7193c6f117e5a"
EXPECTED_M34_MODULE_LIST_SHA256 = "c0c35e02fe61a3f6c18c221a9ae2cc1a54aafd38374117fa954dbfa675700998"
EXPECTED_M34_TEMPLATE_SOURCE_SHA256 = "ce12ea11a6c0f73f5f042801435b419637b473eff6631155f45d4ad382d8a80a"
...
EXPECTED_MODULE_ENTRY = "s22plus_m34_s7a2_runtime_gadget_split.modules"
EXPECTED_STOCK_RECIPE_REPORT = "docs/reports/S22PLUS_STOCK_USB_GADGET_ACM_RECIPE_2026-07-09.md"
M34_S7A2_RISK_MODULES = M34_S7A_RISK_MODULES
M34_S7A2_I2C_ORDER_ACTUAL = ["msm-geni-se.ko", "gpi.ko", "i2c-msm-geni.ko"]
EXPECTED_MODULES = (
    "smem.ko",
    "minidump.ko",
    "sec_debug.ko",
...
DEFAULT_M34_AP = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_7/S7A2/odin4/AP.tar.md5")
DEFAULT_M34_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_7/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m34_s7a2_geni_i2c_live_gate_{utc_stamp()}")
...
def agents_exception_draft() -> str:
    ...
    return f"""   **DRAFT ONLY - Narrow operator-authorized exception (2026-07-09, S22+ M34 S7A2 GENI I2C runtime-gadget boot-only live gate):**
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    if draft_only:
        raise SystemExit("AGENTS.md contains draft-only M34 S7A2 authorization text; refuse to treat draft as active live auth")
    missing = missing_policy_markers(agents)
    append_log(log_path, f"agents_exception_missing={missing}")
    if missing:
        raise SystemExit(f"AGENTS.md missing M34 S7A2 runtime-gadget authorization markers: {missing}")
...
def verify_m34_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m34_s7a2_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m34_s7a2_manifest_runtime_steps={json.dumps(runtime_steps, sort_keys=True)}")
    append_log(log_path, f"m34_s7a2_manifest_closure={json.dumps(closure, sort_keys=True)}")
    append_log(log_path, f"m34_s7a2_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")
    append_log(log_path, f"m34_s7a2_manifest_init_file={init_info.get('file', '')}")
    ...
    if matrix.get("host_build_order") != ["S1", "S2", "S3", "S4", "S5", "S6", "S7A", "S7A2"]:
    ...
    if matrix.get("next_host_only_candidate") != "S7A2":
    ...
    if matrix.get("s7a2_closes_missing_geni_i2c_transport") is not True:
        raise SystemExit("M34 S7A2 matrix does not prove GENI I2C transport closure")
    if matrix.get("s7a2_typec_role_write_discriminator") is not True:
        raise SystemExit("M34 S7A2 matrix does not prove the role-write discriminator")
    if matrix.get("s7a2_geni_i2c_transport_targets") != M34_S7A2_I2C_GENI_TRANSPORT_TARGETS:
        raise SystemExit(
            f"M34 S7A2 GENI I2C targets mismatch: {matrix.get('s7a2_geni_i2c_transport_targets')!r}"
        )
    if matrix.get("s7a2_geni_i2c_transport_order_actual") != M34_S7A2_I2C_ORDER_ACTUAL:
        raise SystemExit(
            f"M34 S7A2 GENI I2C order mismatch: {matrix.get('s7a2_geni_i2c_transport_order_actual')!r}"
        )
    if matrix.get("s7a2_session_producer_targets") != M34_S7A_SESSION_PRODUCER_TARGETS:
        raise SystemExit(f"M34 S7A2 session-producer targets mismatch: {matrix.get('s7a2_session_producer_targets')!r}")
    if matrix.get("s7a2_session_producer_new_modules") != M34_S7A2_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S7A2 session-producer new module list mismatch: {matrix.get('s7a2_session_producer_new_modules')!r}")
    if matrix.get("s7a2_risk_modules") != M34_S7A2_RISK_MODULES:
        raise SystemExit(f"M34 S7A2 risk module list mismatch: {matrix.get('s7a2_risk_modules')!r}")

    if stage.get("stage_number") != 8:
        raise SystemExit(f"M34 S7A2 stage number mismatch: {stage.get('stage_number')!r}")
    expected_steps = {
    ...
    if runtime_steps != expected_steps:
        raise SystemExit(f"M34 S7A2 runtime steps mismatch: {runtime_steps!r}")
    required_hashes = {
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M34 S7A2 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M34 S7A2 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    if closure.get("modules") != list(EXPECTED_MODULES):
        raise SystemExit(f"M34 S7A2 module closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M34 S7A2 module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M34_MODULE_LIST_SHA256:
        raise SystemExit(f"M34 S7A2 module list SHA mismatch: {closure.get('module_sha256')!r}")
    for module in ("phy-msm-ssusb-qmp.ko", "eud.ko", "ucsi_glink.ko"):
        if module not in closure.get("modules", []):
            raise SystemExit(f"M34 S7A2 stock softdep module missing from closure: {module}")
    if closure.get("stock_softdep_targets") != M34_S6_STOCK_SOFTDEP_TARGETS:
        raise SystemExit(f"M34 S7A2 closure S6 stock targets mismatch: {closure.get('stock_softdep_targets')!r}")
    if closure.get("stock_softdep_new_modules") != M34_S6_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S7A2 closure S6 stock new modules mismatch: {closure.get('stock_softdep_new_modules')!r}")
    if closure.get("additional_targets") != M34_S7A2_SESSION_PRODUCER_TARGETS:
        raise SystemExit(f"M34 S7A2 closure additional targets mismatch: {closure.get('additional_targets')!r}")
    if closure.get("additional_new_modules") != M34_S7A2_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S7A2 closure additional new modules mismatch: {closure.get('additional_new_modules')!r}")
    if closure.get("geni_i2c_transport_targets") != M34_S7A2_I2C_GENI_TRANSPORT_TARGETS:
        raise SystemExit(f"M34 S7A2 closure GENI I2C targets mismatch: {closure.get('geni_i2c_transport_targets')!r}")
    if closure.get("geni_i2c_transport_new_modules") != M34_S7A2_I2C_GENI_TRANSPORT_TARGETS:
        raise SystemExit(
            f"M34 S7A2 closure GENI I2C new modules mismatch: {closure.get('geni_i2c_transport_new_modules')!r}"
        )
    if closure.get("geni_i2c_transport_order_actual") != M34_S7A2_I2C_ORDER_ACTUAL:
        raise SystemExit(
            f"M34 S7A2 closure GENI I2C order mismatch: {closure.get('geni_i2c_transport_order_actual')!r}"
        )
    modules = closure.get("modules", [])
    if modules.index("i2c-msm-geni.ko") >= modules.index("pdic_max77705.ko"):
        raise SystemExit("M34 S7A2 module order must load i2c-msm-geni.ko before pdic_max77705.ko")
    if closure.get("session_producer_targets") != M34_S7A_SESSION_PRODUCER_TARGETS:
        raise SystemExit(f"M34 S7A2 closure stock targets mismatch: {closure.get('session_producer_targets')!r}")
    if closure.get("session_producer_new_modules") != M34_S7A_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S7A2 closure new modules mismatch: {closure.get('session_producer_new_modules')!r}")
    if closure.get("risk_modules") != M34_S7A2_RISK_MODULES:
        raise SystemExit(f"M34 S7A2 closure risk modules mismatch: {closure.get('risk_modules')!r}")
    if closure.get("contains_sec_debug_region") is not True:
        raise SystemExit("M34 S7A2 closure does not explicitly carry sec_debug_region")
    if closure.get("requires_live_risk_review") is not True:
        raise SystemExit("M34 S7A2 closure does not require live risk review")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M34 S7A2 manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M34 S7A2 module list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M34 S7A2 must not inject module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 1:
        raise SystemExit("M34 S7A2 must inject exactly one module-list text file into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M34 S7A2 required string missing from manifest: {required}")
    ...
    for forbidden in forbidden_required_strings:
        if forbidden in required_strings:
            raise SystemExit(f"M34 S7A2 manifest unexpectedly requires forbidden string: {forbidden}")

    objdump_path = path.parent / EXPECTED_STAGE / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M34 S7A2 objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S7A2 /init does not load arm64 __NR_finit_module (273)")
    if any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S7A2 /init must not load arm64 __NR_reboot (142)")
...
def verify_m34_artifacts(
    *,
    ...
    verify_ap(m34_ap, EXPECTED_M34_AP_SHA256, "m34_s7a2_candidate", log_path)
...
def is_m34_s7a2_acm(device: dict[str, Any]) -> bool:
    return (
...
def observe_park_survival(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m34_s7a2_observe_start_utc={utc_now()}")
    append_log(log_path, f"m34_s7a2_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m34_s7a2_park_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m34_s7a2_result=unexpected_odin_before_survival_window elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "unexpected-odin-before-survival-window", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M34 S7A2 observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m34_s7a2_result=unexpected_adb_before_survival_window elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-survival-window", None
            for device in acm_devices(log_path, label):
                if is_m34_s7a2_acm(device):
                    append_log(log_path, f"m34_s7a2_acm_seen=1 elapsed_sec={elapsed:.3f} device={json.dumps(device, sort_keys=True)}")
                    return "acm-seen-manual-download-required", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m34_s7a2_survival_window_pass=1")
    append_log(log_path, "m34_s7a2_result=survived-observation-window-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m34_s7a2_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m34_s7a2_{label}_boot_restore_hash_check=skipped rollback_target=stock")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m34_s7a2_{label}_rollback", android, marker=EXPECTED_M34_MARKER)
    append_log(log_path, f"m34_s7a2_{label}_retained_marker_found={int(marker_found)}")
    return (0, android)
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m34-s7a2-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M34 S7A2 rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    if args.observe_sec < 60:
        raise SystemExit("--observe-sec must be at least 60 for the M34 S7A2 session-producer discriminator")
    if args.snapshot_interval_sec < 1.0:
        raise SystemExit("--snapshot-interval-sec must be at least 1.0")

    root = repo_root()
    run_dir = resolve_run_dir(root, args.run_dir)
    log_path = run_dir / "s22plus_m34_s7a2_geni_i2c_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M34 S7A2 session-producer live gate ===")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M34 S7A2 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M34 S7A2 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M34 S7A2 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M34 S7A2 live gate starting. This candidate should park after stock-speed QMP/EUD/ucsi setup "
    ...
    if odin_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print("download mode did not appear for M34 S7A2 candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        record_timeline_event(run_dir, "live_session_end")
        print(f"M34 S7A2 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m34_s7a2_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M34 S7A2 candidate flashed. Observing for {args.observe_sec}s survival window.", flush=True)
    ...
    if not successful_result:
        if rollback_device is None:
            record_timeline_event(run_dir, "live_session_end")
            print(
                f"M34 S7A2 stopped before survival proof ({result}). If the device is not in Android, "
    ...
    print(
        f"M34 S7A2 result={result}. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print(
            f"M34 S7A2 survival window passed, but manual Download mode did not appear. "
    ...
    record_timeline_event(run_dir, "live_session_end")
    print(f"M34 S7A2 live gate completed rc={rc}; result={result}; log={log_path}")
    return rc
