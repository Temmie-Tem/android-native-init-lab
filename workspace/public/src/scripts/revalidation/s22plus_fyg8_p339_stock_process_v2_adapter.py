#!/usr/bin/env python3
"""Host-only P3.39 Process-v2 adapter over the exact P3.38 ABI.

The adapter preserves P338's target, boot-only rollback, 300-second
observation, three fixed commands, three sessions, one reconnect, and lease
limits.  It changes only the fresh run namespace, separates OPEN semantics,
and projects the fixed four-word rejected-header capture.  No device, ADB,
Odin, or live authority operation is exposed here.
"""
...
import s22plus_fyg8_p338_stock_process_v2_adapter as predecessor
import s22plus_fyg8_p339_open_read_branch_acm_observer as observer
...
P339_OVERLAY_CONTRACT_ID = "s22plus-fyg8-p339-open-header-capture-v1"
P339_DECODER_ID = "s22plus_fyg8_p339_open_header_capture_v1"
P339_OBSERVER_CONTRACT_ID = observer.CONTRACT_ID
P339_POLICY_PREIMAGE = (
    "S22PLUS_FYG8_P339_OPEN_HEADER_CAPTURE_V1|"
    "protocol=p339-open-header-capture-v1|"
    "initial-proof=sessions-3,same-fd,host-close-reopen-1|"
    "first-open=header-read-errno,header-grammar,body-read-errno,crc,open-semantic|"
    "rejected-header=stage4-7,four-le32-words,best-effort|"
    "original-return=unchanged|commands=fixed-p338-three-commands|"
    "transport=one-open-fd-per-session|per-boot-identity=unchanged-nonzero|"
    "retry=none|usb=bidirectional-primary|run="
    + P339_RUN_ID_HEX
    + "|causal=false"
)
P339_POLICY_ID = hashlib.sha256(P339_POLICY_PREIMAGE.encode("ascii")).hexdigest()[:32]
...
LEASE_SCHEMA = "s22plus_fyg8_p339_open_header_capture_lease_v1"

SCHEMA = "s22plus_fyg8_p339_stock_process_v2_adapter_v1"
OVERLAY_CONTRACT_ID = P339_OVERLAY_CONTRACT_ID
DECODER_ID = P339_DECODER_ID
OBSERVER_CONTRACT_ID = P339_OBSERVER_CONTRACT_ID
POLICY_PREIMAGE = P339_POLICY_PREIMAGE
POLICY_ID = P339_POLICY_ID
RUN_ID = P339_RUN_ID
STOCK_RUN_ID = P339_RUN_ID
...
P339_ADAPTER_SOURCE = Path(__file__).resolve()
P338_ADAPTER_SOURCE = P339_ADAPTER_SOURCE
P337_ADAPTER_SOURCE = P339_ADAPTER_SOURCE
P336_ADAPTER_SOURCE = P339_ADAPTER_SOURCE
P335_ADAPTER_SOURCE = P339_ADAPTER_SOURCE
P334_ADAPTER_SOURCE = P339_ADAPTER_SOURCE
...
P339_DETAIL_BOOT_ID = getattr(runtime, "P338_DETAIL_BOOT_ID", 0xC350)
P339_DETAIL_INITIAL_SESSION = getattr(runtime, "P338_DETAIL_INITIAL_SESSION", 0xC351)
P339_DETAIL_LISTENER_BANNER = getattr(runtime, "P338_DETAIL_LISTENER_BANNER", 0xC352)
P339_DETAIL_LISTENER_PROTOCOL = getattr(runtime, "P338_DETAIL_LISTENER_PROTOCOL", 0xC353)


class AdapterIdentityError(ValueError):
    """The exact P3.38 adapter or fresh P3.39 binding differs."""


class ContractError(ValueError):
    """The public P3.39 contract projection is incomplete or changed."""
...
def _stable_source() -> bytes:
    ...
    try:
        ...
        raise AdapterIdentityError("P3.38 adapter source is unavailable") from exc
    if (
        ...
        raise AdapterIdentityError("P3.38 adapter binding differs")
    return payload
...
def _fresh(value: Any) -> Any:
    """Project an inherited result without retaining P338 authority labels."""
...
def audit() -> dict[str, Any]:
    ...
    value.update(
        {
            "schema": SCHEMA,
            "verdict": "PASS_P339_STOCK_PROCESS_V2_ADAPTER_H0_OPEN_HEADER_CAPTURE",
            ...
            "predecessor_run_id_rejected": P338_PREDECESSOR_RUN_ID_HEX,
            "encoder_failure_class": "P339_STOCK_ENCODER_FAILURE",
            "open_read_branch_ordinals": _serialized_branch_ordinals(),
            "open_read_branch_count": len(runtime.OPEN_READ_BRANCHES),
            "open_header_word_stages": list(runtime.OPEN_HEADER_WORD_STAGES),
            "open_header_size": runtime.OPEN_HEADER_SIZE,
...
def validate_contract(value: Any) -> dict[str, Any]:
    if type(value) is not dict:
        raise ContractError("P339 overlay contract is not an object")
    expected = {
        "userspace_overlay_contract_id": OVERLAY_CONTRACT_ID,
        "decoder": DECODER_ID,
        "policy_id": POLICY_ID,
...
def validate_acceptance_item(value: Any) -> dict[str, Any]:
    ...
    for name, item in contract.items():
        if (
            ...
            raise ContractError(f"P339 acceptance contract {name} differs")
    return value
...
def classify_observation(
    ...
    *,
    ...
    if expected_profile != PROFILE or expected_run_id != P339_RUN_ID:
        raise AdapterIdentityError("P339 observation binding differs")
    try:
        value = predecessor.classify_observation(
            payload,
            expected_profile=predecessor.PROFILE,
            expected_run_id=predecessor.P338_RUN_ID,
        )
    except Exception as exc:
        raise AdapterIdentityError(str(exc)) from exc
    return _fresh(value)
...
def classify_clean_baseline(
    ...
    *,
    ...
    if expected_profile != PROFILE or expected_run_id != P339_RUN_ID:
        raise AdapterIdentityError("P339 baseline binding differs")
    try:
