#!/usr/bin/env python3
"""Build the S22+ M17 power-QMP add-back native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M17 starts from the stable M13 no-module floor and reintroduces the
power/clock substrate dependency closure plus phy-msm-ssusb-qmp.ko. It does
not add the HS/eUSB PHYs, dwc3-msm, function drivers, or role/PD stack. The
boot ramdisk gets only a small text list; module binaries remain in stock
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m17_power_qmp_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m17_power_qmp_park.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M17"
...
RUNTIME_MODULES_POWER_QMP_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_POWER_QMP_RAMDISK = "s22plus_m17_power_qmp.modules"
...
PHYS_WITHHELD_AFTER_M17 = [
    "phy-generic.ko",
    "phy-msm-snps-hs.ko",
    "phy-msm-snps-eusb2.ko",
]

M14_WITHHELD_AFTER_M17 = [
    "dwc3-msm.ko",
    "usb_f_ss_acm.ko",
]

M12_WITHHELD_AFTER_M17 = [
    "usb_f_diag.ko",
    "usb_f_qdss.ko",
    "usb_f_gsi.ko",
...
ROLE_AND_GLINK_WITHHELD_AFTER_M17 = [
    "pmic_glink.ko",
    "altmode-glink.ko",
    "ucsi_glink.ko",
    "qcom_glink.ko",
...
def derive_power_qmp(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    ...
    missing_recovery = sorted(module for module in POWER_QMP_SEED_MODULES if module not in recovery_set)
    missing_dep = sorted(module for module in POWER_QMP_SEED_MODULES if module not in dep_map)
    if missing_recovery:
        raise SystemExit(f"power-QMP module missing from modules.load.recovery: {missing_recovery}")
    if missing_dep:
        raise SystemExit(f"power-QMP module missing from modules.dep: {missing_dep}")

    closure: set[str] = set()
    stack = list(POWER_QMP_SEED_MODULES)
    while stack:
        name = stack.pop()
        if name in closure:
            continue
    ...
    if closure_not_recovery:
        raise SystemExit(f"power-QMP dependency closure is not wholly in recovery order: {closure_not_recovery[:10]}")

    blocked_from_closure = sorted(name for name in closure if is_blocked_module(name))
    blocked_watchdogs_present = sorted(name for name in closure if name in WATCHDOG_BLOCKLIST)
    ordered = [name for name in recovery_unique if name in closure and not is_blocked_module(name)]
    if ordered != EXPECTED_POWER_QMP_SUBSET:
        raise SystemExit(f"power-QMP subset drifted: {ordered!r} != {EXPECTED_POWER_QMP_SUBSET!r}")
    if len(ordered) != 21:
        raise SystemExit(f"power-QMP module count changed: {len(ordered)} != 21")
    if len(set(ordered)) != len(ordered):
        raise SystemExit("power-QMP common module list contains duplicates")
    blocked_required = sorted(module for module in POWER_QMP_SEED_MODULES if module in blocked_from_closure)
    if blocked_required:
        raise SystemExit(f"power-QMP seed module unexpectedly blocklisted: {blocked_required}")
    if not ordered:
        raise SystemExit("derived M17 power QMP is empty")

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_POWER_QMP_BUF:
        raise SystemExit(
            "M17 power QMP does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_POWER_QMP_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M17 power QMP basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "power_qmp_seed_modules": POWER_QMP_SEED_MODULES,
        "power_qmp_seed_count": len(POWER_QMP_SEED_MODULES),
        "dependency_closure": sorted(closure, key=lambda name: recovery_unique.index(name)),
        "dependency_closure_count": len(closure),
        "blocked_from_closure": blocked_from_closure,
        "blocked_watchdogs_present_in_closure": blocked_watchdogs_present,
        "phys_withheld_after_m17": PHYS_WITHHELD_AFTER_M17,
        "phys_withheld_after_m17_count": len(PHYS_WITHHELD_AFTER_M17),
        "m14_withheld_after_m17": M14_WITHHELD_AFTER_M17,
        "m14_withheld_after_m17_count": len(M14_WITHHELD_AFTER_M17),
        "m12_withheld_after_m17": M12_WITHHELD_AFTER_M17,
        "m12_withheld_after_m17_count": len(M12_WITHHELD_AFTER_M17),
        "role_and_glink_withheld_after_m17": ROLE_AND_GLINK_WITHHELD_AFTER_M17,
        "role_and_glink_withheld_after_m17_count": len(ROLE_AND_GLINK_WITHHELD_AFTER_M17),
        "order_source": "M13 park floor plus recovery-order power/clock substrate dependency closure and QMP PHY",
        ...
        "runtime_modules_power_qmp_buffer": RUNTIME_MODULES_POWER_QMP_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    power_qmp = derive_power_qmp(recovery_lines, modules_dep_lines)

    missing_ko = sorted(module for module in EXPECTED_POWER_QMP_SUBSET if module not in ko_names)
    missing_recovery = sorted(module for module in EXPECTED_POWER_QMP_SUBSET if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"power-QMP vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"power-QMP module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M17 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in EXPECTED_POWER_QMP_SUBSET}
    ...
    (build_dir / MODULES_POWER_QMP_RAMDISK).write_text(str(power_qmp["subset_text"]), encoding="ascii")
    (build_dir / "m17_power_qmp.txt").write_text(str(power_qmp["subset_text"]), encoding="ascii")
    return {
        ...
        "m17_power_qmp": {key: value for key, value in power_qmp.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M17 power-QMP init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M17 power-QMP init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M17 power-QMP init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M17 power-QMP init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M17 power-QMP init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M17 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M17 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M17 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M17 /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M17 init unexpectedly contains arm64 __NR_reboot (142)")
    ...
    (build_dir / "m17_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m17_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m17_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m17_init = build_dir / "s22plus_init_usb_acm_m17"
    m17_init_info = compile_init(source, m17_init, build_dir)
    ...
    subset_file = build_dir / MODULES_POWER_QMP_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M17 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m17_init}"],
        work_dir,
        "replace /init with M17 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_POWER_QMP_RAMDISK} {subset_file}"],
        work_dir,
        "add M17 power QMP list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M17 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m17_init):
        raise SystemExit("replaced /init does not match compiled M17 init")
    extracted_subset = build_dir / MODULES_POWER_QMP_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_POWER_QMP_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M17 power QMP list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M17 power QMP list does not match builder output")
    ...
    sizes = {
        ...
        "m17_power_qmp": subset_file.stat().st_size,
        "m17_init": m17_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M17 in-place park-based USB-ACM native-init using only the power/clock substrate dependency closure plus phy-msm-ssusb-qmp.ko",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_POWER_QMP_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_POWER_QMP_RAMDISK,
