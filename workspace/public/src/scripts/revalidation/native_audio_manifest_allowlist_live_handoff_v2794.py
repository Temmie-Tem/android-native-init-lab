#!/usr/bin/env python3
"""V2794 live handoff for native-init audio manifest allowlist.

This is the audio core closure gate: flash the V2794 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2794-audio-manifest-allowlist/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2794_audio_manifest_allowlist.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2794_AUDIO_MANIFEST_ALLOWLIST_LIVE_2026-06-19.md"

CYCLE = "V2794"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.307"
CANDIDATE_TAG = "v2794-audio-manifest-allowlist"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2794-audio-manifest-allowlist-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2794-audio-manifest-allowlist-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2794-audio-manifest-allowlist-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio manifest allowlist did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-manifest-allowlist-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2794-audio-manifest-allowlist-live-worker-failed-before-rollback"
            raise RuntimeError("native audio manifest allowlist did not emit all required pass markers")
        ...
        result["decision"] = "v2794-audio-manifest-allowlist-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2794-audio-manifest-allowlist-live-blocked")
        if result["decision"] == "v2794-audio-manifest-allowlist-live-started":
            result["decision"] = "v2794-audio-manifest-allowlist-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2794-audio-manifest-allowlist-live-dry-run",
