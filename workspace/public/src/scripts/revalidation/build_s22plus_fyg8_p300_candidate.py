#!/usr/bin/env python3
"""Build one deterministic boot-only P3.00 FYG8 candidate host-only."""
...
import s22plus_fyg8_p300_boot_only_packager as packager
...
SCHEMA = "s22plus_fyg8_p300_candidate_artifact_result_v1"
VERDICT = "PASS_P300_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P300_SOURCE_CONTRACT_ID = repro.P300_SOURCE_CONTRACT_ID
P286_SOURCE_CONTRACT_ID = P300_SOURCE_CONTRACT_ID
DEFAULT_IMAGE = repro.DEFAULT_BUILD_A / "Image"
DEFAULT_REPRO_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p300/build-repro-result.json"
)
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p300/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    value, result_receipt = base._read_json(  # noqa: SLF001
        result_path,
        "P3.00 build reproducibility result",
    )
    if (
        ...
        raise BuildError("P3.00 build reproducibility result is not accepted")
    linked = value.get("linked_audit", {})
    if (
        linked.get("audit_adapter") != "s22plus-fyg8-p300-linked-audit-v1"
        or linked.get("postbuild_audit", {}).get("verified") is not True
    ):
        raise BuildError("P3.00 linked audit adapter mismatch")
    expected_image = value.get("build_a", {}).get("artifacts", {}).get("Image")
    if expected_image != image_receipt:
        raise BuildError("P3.00 supplied Image differs from reproducibility closure")
    if intent_path is None or patch_path is None:
        raise BuildError("P3.00 selected build-input paths are missing")
    try:
        qualification = repro.verify_p300_qualification_file(
