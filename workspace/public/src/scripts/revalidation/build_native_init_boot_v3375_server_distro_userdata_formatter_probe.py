#!/usr/bin/env python3
"""Build V3375 native-init boot image for the server-distro D4C formatter probe surface."""
...
CYCLE = "V3375"
INIT_VERSION = "0.11.135"
INIT_BUILD = "v3375-server-distro-userdata-formatter-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3375_SERVER_DISTRO_D4C_FORMATTER_PROBE_SOURCE_BUILD_2026-07-03.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3375_server_distro_userdata_formatter_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3375_server_distro_userdata_formatter_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3375_server_distro_userdata_formatter_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v629_server_distro_switchroot"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3375"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3375.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3375.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3375"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3375-server-distro-userdata-formatter-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3375-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3375-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3375-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3375-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3375-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3375-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3375-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3375.c"
...
def _rewrite_v3375_text(text: str) -> str:
    ...
    replacements = (
        ...
        ("hot-reload-autohud", "server-distro-userdata-formatter-probe"),
        ("v3368", "v3375"),
        ("V3368", "V3375"),
        ("a90-doomgeneric-v3368", "a90-doomgeneric-v3375"),
        ("a90.doomgeneric.v3368", "a90.doomgeneric.v3375"),
    )
...
def _rewrite_v3375_bytes(item: bytes) -> bytes:
    return _rewrite_v3375_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3375_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3375_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3375_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3375_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3375_bytes(marker) for marker in previous.REQUIRED_STRINGS)

REQUIRED_STRINGS = PREVIOUS_REQUIRED_STRINGS + (
    b"0.11.135",
    b"v3375-server-distro-userdata-formatter-probe",
...
def _boot_audit_manifest() -> dict[str, Any]:
    ...
    manifest["rung"] = "server-distro-d4c-formatter-probe"
    manifest["scope"] = "D4C-entry-non-destructive-formatter-probe-surface"
    manifest["server_distro_contract"] = {
        ...
        "formatter_policy": "non-destructive probe formats an approved SD runtime regular file with busybox mke2fs -t ext4, verifies ext4 magic, unlinks the probe, and never materializes userdata",
...
def v3375_adapter_source() -> str:
    return _rewrite_v3375_text(ORIG_PREVIOUS_ADAPTER_SOURCE())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "server-distro-userdata-formatter-probe.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "server-distro-d4c-formatter-probe",
...
def _normalize_manifest_for_v3375(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest.update({
        "decision": DECISION,
        "cycle": CYCLE,
        "candidate_tag": INIT_BUILD,
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest = _normalize_manifest_for_v3375(json.loads(manifest_path.read_text(encoding="utf-8")))
    if base_main_error:
...
def _postprocess_manifest() -> dict[str, Any]:
    manifest = _normalize_manifest_for_v3375(ORIG_PREVIOUS_POSTPROCESS())
    manifest_path = OUT_DIR / "manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_PATH.write_text(
        render_report(
...
def _overlay_preserved_v3375_ramdisk() -> dict[str, Any]:
    overlay = ORIG_PREVIOUS_OVERLAY()
    overlay["mode"] = "preserve-v3335-ramdisk-overlay-v3375-init-helper-engine"
    return overlay


def _patch_v3368_module_for_v3375() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "v3368_adapter_source": v3375_adapter_source,
        "_rewrite_v3368_text": _rewrite_v3375_text,
        "_rewrite_v3368_bytes": _rewrite_v3375_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_overlay_preserved_v3368_ramdisk": _overlay_preserved_v3375_ramdisk,
