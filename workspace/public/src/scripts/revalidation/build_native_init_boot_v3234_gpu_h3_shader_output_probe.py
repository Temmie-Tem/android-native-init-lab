#!/usr/bin/env python3
"""Build V3234 GPU H3 Shader Output probe."""
...
CYCLE = "V3234"
INIT_VERSION = "0.11.44"
INIT_BUILD = "v3234-gpu-h3-shader-output-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3234_GPU_H3_SHADER_OUTPUT_SOURCE_BUILD_2026-06-25.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3234_gpu_h3_shader_output_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3234_gpu_h3_shader_output_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3234_gpu_h3_shader_output_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v576_gpu_h3_shader_output_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3234"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3234.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3234.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3234"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3234-gpu-h3-shader-output-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3234-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3234-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3234-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3234-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3234-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3234-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3234-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-shader-output-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-shader-output-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3234")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3234")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3234")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3234")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3234")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3234")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3234")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3234")
SFX_STREAM_MARKER = "a90.doomgeneric.v3234.audio=real-sfx-pcm-stream-gpu-h3-shader-output-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-shader-output-probe-v3234"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3234.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3234_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3232": b"a90-doomgeneric-v3234",
        b"a90.doomgeneric.v3232": b"a90.doomgeneric.v3234",
        b"v3232": b"v3234",
        b"V3232": b"V3234",
        b"first-triangle-h3-static-context-mov-f32-shader":
            b"first-triangle-h3-shader-output-r1-mov-f32-shader",
...
REQUIRED_STRINGS = tuple(_rewrite_v3234_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3232-gpu-h3-static-context-probe",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-shader-output-r1-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3234-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-shader-output-r1-mov-f32-shader-timeout-guard",
...
def v3234_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-shader-output-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-shader-output-probe")
        .replace("v3208", "v3234")
        .replace("V3208", "V3234")
    )


def _overlay_preserved_v3234_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3232 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3234 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3234 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3234 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3234-overlay-") as temp_name:
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3234 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3234 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3232-ramdisk-overlay-v3234-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-shader-output-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-shader-output-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-shader-output-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3232-ramdisk-overlay-v3234-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-shader-output-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-shader-output-live-validation"
...
def _apply_v3234_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-shader-output-probe")
        ...
        "v3210_adapter_source": v3234_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3234_ramdisk,
