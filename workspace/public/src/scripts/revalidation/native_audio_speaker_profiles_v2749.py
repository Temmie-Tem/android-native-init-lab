...
AUDIO_FEATURE_STAGES = (
    ...
    AudioFeatureStage(
        stage_id="apply-playback-speaker-route",
        order=60,
        owner="native-init",
        phase="route",
        command_template=("audio", "route", "{profile}", "--apply", "--layer", "playback"),
        native_implemented=True,
        writes_runtime_state=True,
        rollback_boundary=False,
        speaker_scope="shared",
        note="applies core, feedback, and non-boost endpoint controls; smart-amp boost remains blocked",
    ),
    AudioFeatureStage(
    ...
    AudioFeatureStage(
        ...
        rollback_boundary=False,
        speaker_scope="internal-speaker",
        note="plans PCM stop, reverse ACDB deallocation, and playback route reset without touching ALSA or calibration ioctls",
    ),
    AudioFeatureStage(
        stage_id="reset-playback-speaker-route",
        order=80,
        owner="native-init",
        phase="cleanup",
        command_template=("audio", "route", "{profile}", "--reset", "--layer", "playback"),
        native_implemented=True,
        writes_runtime_state=True,
        rollback_boundary=False,
        speaker_scope="shared",
        note="resets the same playback controls in reverse order while leaving boost blocked",
    ),
    AudioFeatureStage(
