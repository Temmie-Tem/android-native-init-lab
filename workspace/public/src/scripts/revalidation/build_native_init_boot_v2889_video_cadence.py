#!/usr/bin/env python3
"""Build V2889 native-init candidate with page-flip cadence telemetry."""
...
CYCLE = "V2889"
INIT_VERSION = "0.10.30"
INIT_BUILD = "v2889-video-cadence"
BUILD_TAG = INIT_BUILD
DECISION = "v2889-video-cadence-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2889_VIDEO_CADENCE_SOURCE_BUILD_2026-06-19.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2889_video_cadence.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2889_video_cadence"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2889_video_cadence.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v460_video_cadence"
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2889 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    markers = manifest.get("v2889_marker_strings", [])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "video-cadence-candidate",
        "parent_test_artifact": "v2888-av-sync-drop",
        "av_sync_stream": {
            "version": 1,
            "source_unit": "V2889",
            ...
            "late_frame_policy": "drop if now > deadline + interval/2, never drop the final requested frame",
            "cadence_telemetry": "flip_delta_count/min/max/avg/target_us from DRM page-flip event timestamps",
            "live_validation": "pending",
        },
        "v2889_marker_strings": marker_strings,
    ...
    (OUT_DIR / "video-cadence-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "video-cadence-candidate",
