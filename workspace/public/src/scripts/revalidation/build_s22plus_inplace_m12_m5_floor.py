#!/usr/bin/env python3
"""Build the S22+ M12 M5-floor USB-ACM native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M12 keeps the M11 freestanding raw-syscall PID1/configfs/role-force/park shape,
but lowers the module set back to the M5/M11 common floor. This distinguishes
"M11-only substrate caused the loop" from "the M11 vendor_boot loader/runtime
path itself lost the M5 floor." The boot ramdisk gets only a small text list;
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m12_m5_floor_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m12_m5_floor.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M12"
...
RUNTIME_MODULES_M5_FLOOR_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_M5_FLOOR_RAMDISK = "s22plus_m12_m5_floor.modules"
...
M5_ONLY_WITHHELD_MODULES = [
    "usb_notifier_qcom.ko",
    "qc_usb_audio.ko",
]

M11_ONLY_REFERENCE_MODULES = [
    "clk-rpmh.ko",
    "gcc-waipio.ko",
    "icc-rpmh.ko",
    "qcom_ipc_logging.ko",
...
def is_blocked_module(name: str) -> bool:
    if name in EXPLICIT_M12_BLOCKLIST:
        return True
    lower = name.lower()
...
def derive_m5_floor(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    recovery_set = set(recovery_lines)
    dep_map = parse_modules_dep(modules_dep_lines)

    missing_recovery = sorted(module for module in M5_FLOOR_COMMON_MODULES if module not in recovery_set)
    missing_dep = sorted(module for module in M5_FLOOR_COMMON_MODULES if module not in dep_map)
    if missing_recovery:
        raise SystemExit(f"M5-floor module missing from modules.load.recovery: {missing_recovery}")
    if missing_dep:
        raise SystemExit(f"M5-floor module missing from modules.dep: {missing_dep}")

    ordered = list(M5_FLOOR_COMMON_MODULES)
    if len(ordered) != 24:
        raise SystemExit(f"M5-floor common count changed: {len(ordered)} != 24")
    if len(set(ordered)) != len(ordered):
        raise SystemExit("M5-floor common module list contains duplicates")
    if any(is_blocked_module(name) for name in ordered):
        raise SystemExit("M5-floor common module list includes an explicit M12 blocklist item")
    if not ordered:
        raise SystemExit("derived M12 M5 floor is empty")

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_M5_FLOOR_BUF:
        raise SystemExit(
            "M12 M5 floor does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_M5_FLOOR_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M12 M5 floor basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "m5_floor_common_modules": M5_FLOOR_COMMON_MODULES,
        "m5_floor_common_count": len(M5_FLOOR_COMMON_MODULES),
        "m5_only_withheld_modules": M5_ONLY_WITHHELD_MODULES,
        "m11_only_reference_modules": M11_ONLY_REFERENCE_MODULES,
        "m11_only_reference_count": len(M11_ONLY_REFERENCE_MODULES),
        "order_source": "M5 v0.4 order filtered to modules common with M11",
        "explicit_m12_blocklist": sorted(EXPLICIT_M12_BLOCKLIST),
        ...
        "runtime_modules_m5_floor_buffer": RUNTIME_MODULES_M5_FLOOR_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    m5_floor = derive_m5_floor(recovery_lines, modules_dep_lines)

    missing_ko = sorted(module for module in M5_FLOOR_COMMON_MODULES if module not in ko_names)
    missing_recovery = sorted(module for module in M5_FLOOR_COMMON_MODULES if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"M5-floor vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"M5-floor module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M12 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in M5_FLOOR_COMMON_MODULES}
    ...
    (build_dir / MODULES_M5_FLOOR_RAMDISK).write_text(str(m5_floor["subset_text"]), encoding="ascii")
    (build_dir / "m12_m5_floor.txt").write_text(str(m5_floor["subset_text"]), encoding="ascii")
    return {
        ...
        "m12_m5_floor": {key: value for key, value in m5_floor.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M12 M5-floor init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M12 M5-floor init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M12 M5-floor init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M12 M5-floor init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M12 M5-floor init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M12 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M12 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M12 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M12 /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M12 init unexpectedly contains arm64 __NR_reboot (142)")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"s22plus-m5", b"modules.load.recovery", b"download"):
        if forbidden in binary:
            raise SystemExit(f"M12 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m12_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m12_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m12_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m12_init = build_dir / "s22plus_init_usb_acm_m12"
    m12_init_info = compile_init(source, m12_init, build_dir)
    ...
    subset_file = build_dir / MODULES_M5_FLOOR_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M12 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m12_init}"],
        work_dir,
        "replace /init with M12 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_M5_FLOOR_RAMDISK} {subset_file}"],
        work_dir,
        "add M12 M5 floor list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M12 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m12_init):
        raise SystemExit("replaced /init does not match compiled M12 init")
    extracted_subset = build_dir / MODULES_M5_FLOOR_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_M5_FLOOR_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M12 M5 floor list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M12 M5 floor list does not match builder output")
    ...
    sizes = {
        ...
        "m12_m5_floor": subset_file.stat().st_size,
        "m12_init": m12_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M12 in-place park-based USB-ACM native-init using M5/M11 common floor modules from vendor_boot",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_M5_FLOOR_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_M5_FLOOR_RAMDISK,
