...
blocked while the V2 binding is review-pending and later requires the exact
...
RUN_PARENT = ROOT / "workspace/private/runs/device-action-d1-p319-fresh-baseline-v2"
RUN_DIR = RUN_PARENT / "p319-fresh-baseline-2"
RUN_ARM = RUN_PARENT / "p319-fresh-baseline-2.arm.json"
RUN_STOP = RUN_DIR / "stop.json"
RAW_ROOT = RUN_PARENT / "p319-fresh-baseline-2-raw"
RAW_ADB_DIR = RAW_ROOT / "raw-adb"
ADB_SNAPSHOT = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p319/d1-fresh-baseline-v2/adb-"
    "05a1a4435e436230931acd8737fd68f31542d652731d3ca8c464cab7a42be226"
)

TARGET = {"model": "SM-S906N", "codename": "g0q", "build": "S906NKSS7FYG8"}
BINDING_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_execution_binding_v2"
BINDING_ID = "s22plus-fyg8-p319-d1-fresh-baseline-v2"
AUTHORITY_PREFIX = "DEVICE-ACTION-D1-P319-FRESH-BASELINE-V2-APPROVE:"
REVIEW_VERDICT = "PASS_GO_P319_D1_FRESH_BASELINE_RAW_FIRST_V2_H0_CAPABILITY_V1"
ARM_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_arm_v2"
START_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_start_v2"
RESULT_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_v2_result"
RESULT_VERDICT = "PASS_P319_D1_FRESH_BASELINE_V2_EXACT_NORMAL_REBOOT_RETURN_HEALTH"
STOP_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_v2_stop"
STOP_VERDICT = "STOP_P319_D1_FRESH_BASELINE_V2_CONSUMED_NO_REPLAY"
RAW_INVENTORY_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_v2_raw_inventory"
...
class SuccessorError(RuntimeError):
    """The V2 successor failed closed."""
...
def _input_payloads() -> dict[str, bytes]:
    return {
        "successor_source": _stable(SCRIPT, "V2 successor source", maximum=1024 * 1024),
...
def _validated_static_inputs() -> dict[str, Any]:
    ...
    binding_payload = _stable(BINDING_MANIFEST, "V2 binding", maximum=256 * 1024)
    binding = _strict(binding_payload, "V2 binding")
    review = binding.get("independent_review")
    if review not in (
        {"status": "review-pending", "verdict": None},
        {"status": "pass-go", "verdict": REVIEW_VERDICT},
    ):
        raise SuccessorError("V2 review state differs")
    if not _typed_equal(binding, _expected_manifest(payloads, review)):
        raise SuccessorError("V2 binding does not match exact inputs")
...
def _validated_execution_inputs(static: Mapping[str, Any]) -> dict[str, Any]:
    v1 = _compile("p319_d1_v2_bound_v1", V1_SOURCE, static["payloads"]["v1_source"])
    raw = _compile("p319_d1_v2_bound_raw", RAW_CAPTURE, static["payloads"]["raw_capture"])
    ...
    for name, path in (
        ...
        if _stable(
            ...
            mode=0o400 if name in {"v1_arm", "v1_stop"} else None,
...
def _result_complete(value: Mapping[str, Any], inputs: Mapping[str, Any]) -> bool:
    return (
        ...
        and value.get("ordinal") == "d1-fresh-baseline-2"
        and value.get("run_directory") == _relative(RUN_DIR)
        and value.get("arm")
        == _direct_receipt(RUN_ARM, "V2 result arm", maximum=64 * 1024)
        and value.get("start")
        == _direct_receipt(RUN_DIR / "start.json", "V2 result start")
...
def _cut_state(inputs: Mapping[str, Any]) -> dict[str, Any]:
    arm = _journal_state(RUN_ARM, "V2 arm", inputs, _arm_complete)
    start = _journal_state(
        RUN_DIR / "start.json", "V2 start", inputs, _start_complete
    )
    result = _journal_state(
        RUN_DIR / "result.json", "V2 result", inputs, _result_complete
    )
    return {
...
def _validate_parent_namespace(value: Mapping[str, Any], *, include_stop: bool) -> None:
    ...
    if (
        ...
        raise SuccessorError("V2 parent namespace differs")
    ...
    if {path.name for path in RUN_PARENT.iterdir()} != expected_parent:
        raise SuccessorError("V2 parent child set differs")
    run = RUN_DIR.lstat()
    if (
        ...
        raise SuccessorError("V2 run namespace differs")
    expected_run = set()
    ...
    if {path.name for path in RUN_DIR.iterdir()} != expected_run:
        raise SuccessorError("V2 run child set differs")
...
def validate_stop(
    ...
    *,
    ...
    if not isinstance(value, dict) or set(value) != required:
        raise SuccessorError("V2 stop key set differs")
    if (
    ...
    for key in (
        ...
        if type(value[key]) is not bool:
            raise SuccessorError("V2 stop boolean is untyped")
    if (
        ...
        raise SuccessorError("V2 stop safety state differs")
    expected_stage = (
    ...
    if value["stage"] != expected_stage:
        raise SuccessorError("V2 stop stage differs")
    if value["adb_snapshot"] != _adb_snapshot_state():
        raise SuccessorError("V2 stop ADB snapshot differs")
    if value["raw_evidence"] != _raw_inventory(bound["raw"]):
        raise SuccessorError("V2 stop raw evidence differs")
    cut = _cut_state(bound)
    if {key: value[key] for key in cut} != cut:
        raise SuccessorError("V2 stop cut state differs")
    if not publishing:
        stop_path = RUN_STOP if path is None else path
        if stop_path != RUN_STOP:
            raise SuccessorError("V2 stop path differs")
        payload = _stable(stop_path, "V2 fixed stop", maximum=512 * 1024, mode=0o400)
        if _strict(payload, "V2 fixed stop") != value:
            raise SuccessorError("V2 fixed stop bytes differ")
...
def validate_stop_file(path: Path | None = None) -> dict[str, Any]:
    ...
    payload = _stable(selected, "V2 stop", maximum=512 * 1024, mode=0o400)
    value = _strict(payload, "V2 stop")
    result = validate_stop(value, path=selected)
    if _stable(selected, "final V2 stop", maximum=512 * 1024, mode=0o400) != payload:
        raise SuccessorError("V2 stop changed during validation")
    return result
...
def _publish_stop(inputs: Mapping[str, Any], error: BaseException) -> None:
    ...
    if RUN_DIR.exists() or RUN_DIR.is_symlink():
        ...
        if (
            ...
            raise SuccessorError("V2 stop run directory differs")
    else:
...
def _result_binding(inputs: Mapping[str, Any], raw_evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ordinal": "d1-fresh-baseline-2",
        "run_directory": _relative(RUN_DIR),
        "arm": _direct_receipt(RUN_ARM, "V2 result arm", maximum=64 * 1024),
        "start": _direct_receipt(RUN_DIR / "start.json", "V2 result start"),
...
def _execute_primitive(
    ...
    module.SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_v2"
...
def _post_validate(inputs: Mapping[str, Any]) -> dict[str, Any]:
    binding_payload = _stable(
        BINDING_MANIFEST, "post-run V2 binding", maximum=256 * 1024
    )
    if binding_payload != inputs["manifest_payload"]:
        raise SuccessorError("post-run V2 binding differs")
    for key, path in (
        ...
        if _stable(
            ...
            mode=0o400 if key in {"v1_arm", "v1_stop"} else None,
            owner=None if key == "host_adb" else os.getuid(),
        ) != payload:
            raise SuccessorError(f"post-run {key} differs")
    result_payload = _stable(
        RUN_DIR / "result.json", "V2 result", maximum=512 * 1024, mode=0o400
    )
    result = _strict(result_payload, "V2 result")
    if not _result_complete(result, inputs):
        raise SuccessorError("V2 result validation differs")
    if result["raw_evidence"] != _raw_inventory(inputs["raw"]):
        raise SuccessorError("V2 result raw evidence changed")
...
def _validate_post_arm_pre_acquisition(inputs: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ClassifiedFailure("namespace-publication", "NAMESPACE_PUBLICATION")
    arm = _journal_state(RUN_ARM, "post-arm V2 arm", inputs, _arm_complete)
    if arm != {
        "present": True,
        "node_valid": True,
        "bytes_complete": True,
...
def _preflight_new_run_namespace() -> None:
    for path in (RUN_ARM, RUN_DIR, RAW_ROOT, ADB_SNAPSHOT):
        ...
        raise SuccessorError(
            f"V2 fixed namespace already exists: {path.name}; replay is forbidden"
        )
    try:
        parent = RUN_PARENT.lstat()
    except FileNotFoundError:
    ...
    if (
        ...
        raise SuccessorError(
            "V2 run parent is not a fresh direct namespace; replay is forbidden"
        )
...
def run_live(approval: str) -> dict[str, Any]:
    ...
    if static["manifest"]["independent_review"] != {
        ...
        raise SuccessorError("independent V2 review is absent")
    if approval != static["authority"]:
        raise SuccessorError("exact V2 approval is absent")
    ...
    try:
        ...
        if not arm_completed and _duplicate_arm_error(exc, p318):
            raise SuccessorError(
                "V2 approval arm already exists; replay is forbidden"
            ) from exc
        try:
        ...
        if not arm_completed and not (arm_creation_attempted and arm_present):
            raise SuccessorError("V2 stopped before durable arm") from exc
        try:
            _publish_stop(inputs, exc)
        except BaseException as stop_exc:
            raise SuccessorError(
                f"V2 stopped after arm; stop publication failed: {type(stop_exc).__name__}"
            ) from exc
        raise SuccessorError("V2 stopped after arm; consumed without replay") from exc
...
def _source_contract() -> dict[str, Any]:
    ...
    if direct_bounded:
        raise SuccessorError("V2 source retains direct bounded_command bypass")
    required = (
    ...
    if any(token not in text for token in required):
        raise SuccessorError("V2 raw-first source seam differs")
    bind_before_inventory = text.index(
        "self.client.bind_raw_capture_dir(raw_root)"
    ) < text.index("        def _inventory(self")
    if not bind_before_inventory:
        raise SuccessorError("V2 raw capture is not bound before inventory")
    return {
...
def self_test() -> dict[str, Any]:
    ...
    return {
        "schema": "s22plus_fyg8_p319_d1_fresh_baseline_v2_fixture_h0",
        "verdict": "PASS_P319_D1_FRESH_BASELINE_V2_CAUSE_AND_SOURCE_FIXTURE_H0",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"P3.19 D1 V2 blocked: {exc}", file=sys.stderr)
        return 2
    except (OSError, KeyError, TypeError) as exc:
        print(
            "P3.19 D1 V2 blocked: host input validation failed ("
            + type(exc).__name__
            + ")",
            file=sys.stderr,
        )
