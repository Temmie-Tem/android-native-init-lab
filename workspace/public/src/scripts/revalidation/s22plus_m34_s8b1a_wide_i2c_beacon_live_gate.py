#!/usr/bin/env python3
"""Guarded S22+ M34 S8B1A download-beacon state-probe live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M34 S8B1A keeps the S7A2 module recipe fixed, skips downstream configfs/UDC
...
LIVE_ACK_TOKEN = "S22PLUS-M34-S8B1A-BEACON-PROBE-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M34-S8B1A-BEACON-PROBE-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_STAGE = "S8B1A"
EXPECTED_M34_MARKER = "S22_NATIVE_INIT_M34_RUNTIME_GADGET_SPLIT_S8B1A"
EXPECTED_M34_AP_SHA256 = "5c5df5f3fd83adf15c521f4509f90696ba3372e1aee5a79128a29f74a701ceb1"
EXPECTED_M34_BOOT_SHA256 = "df3ee853bb84541f9d494a97f9ba3db5d08bda67662782de0868e90c49d22145"
EXPECTED_M34_INIT_SHA256 = "6aec230f27edae8e0070b367bf78d2b074f67a289b378958a36e908bb60bf83e"
EXPECTED_M34_MODULE_LIST_SHA256 = "c0c35e02fe61a3f6c18c221a9ae2cc1a54aafd38374117fa954dbfa675700998"
EXPECTED_M34_TEMPLATE_SOURCE_SHA256 = "87f45e212b52e517a078c1af7666924c3a62901918a47f710e06cf28332f4353"
...
EXPECTED_MODULE_ENTRY = "s22plus_m34_s8b1a_runtime_gadget_split.modules"
...
M34_S8B1A_RISK_MODULES = M34_S7A_RISK_MODULES

DEFAULT_M34_AP = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_9/S8B1A/odin4/AP.tar.md5")
DEFAULT_M34_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_9/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m34_s8b1a_wide_i2c_beacon_live_gate_{utc_stamp()}")
...
def policy_required_markers() -> list[str]:
    return [
        "S22+ M34 S8B1A download-beacon state-probe native-init boot-only",
        ...
        "S8B1A keeps the S7A2 module recipe fixed",
...
def agents_exception_draft() -> str:
    ...
    return f"""   **DRAFT ONLY - Narrow operator-authorized exception (2026-07-09, S22+ M34 S8B1A download-beacon state-probe boot-only live gate):**
   ...
   one bounded attended boot-partition-only M34 S8B1A live gate on the Samsung S22+
   ...
   The candidate is limited to freestanding direct PID1 M34 S8B1A behavior.
   S8B1A keeps the S7A2 module recipe fixed: GENI I2C transport closure,
   ...
   S8B1A intentionally performs no downstream USB gadget work:
   ...
   repeat, B2/B3/B4, descriptor/composition pivots, FunctionFS/conn_gadget parity,
...
def verify_agents_text(agents: str, log_path: Path, *, source_label: str) -> None:
    ...
    if draft_only:
        raise SystemExit(
            f"{source_label} contains draft-only M34 S8B1A authorization text; "
    ...
    if missing:
        raise SystemExit(f"{source_label} missing M34 S8B1A beacon-probe authorization markers: {missing}")
    ...
    if not active_template_present:
        raise SystemExit(
            f"{source_label} marker coverage is present but exact M34 S8B1A active authorization template is absent"
        )
...
def agents_candidate_text(current_agents: str) -> str:
    ...
    if has_draft_only_m34_exception(current_agents):
        raise SystemExit("source AGENTS contains draft-only M34 S8B1A text; refuse to build active candidate over it")
    if has_exact_active_exception_template(current_agents):
        return current_agents
    if not missing_policy_markers(current_agents):
        raise SystemExit("source AGENTS is marker-complete but exact M34 S8B1A active template is absent")
...
def verify_m34_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m34_s8b1a_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m34_s8b1a_manifest_runtime_steps={json.dumps(runtime_steps, sort_keys=True)}")
    append_log(log_path, f"m34_s8b1a_manifest_closure={json.dumps(closure, sort_keys=True)}")
    ...
    if matrix.get("host_build_order") != ["S1", "S2", "S3", "S4", "S5", "S6", "S7A", "S7A2", "S8B1", "S8B1A"]:
        raise SystemExit(f"M34 host-build order mismatch: {matrix.get('host_build_order')!r}")
    if matrix.get("next_host_only_candidate") != "S8B1A":
        raise SystemExit(f"M34 next host-only candidate mismatch: {matrix.get('next_host_only_candidate')!r}")
    if matrix.get("s8b1a_download_beacon_probe") != EXPECTED_PROBE:
        raise SystemExit(f"M34 S8B1A probe mismatch: {matrix.get('s8b1a_download_beacon_probe')!r}")
    if matrix.get("s8b1a_true_action") != "reboot(download)" or matrix.get("s8b1a_false_action") != "park":
        raise SystemExit("M34 S8B1A true/false action mismatch")
    if matrix.get("s8b1a_probe_paths") != EXPECTED_PROBE_PATHS:
        raise SystemExit(f"M34 S8B1A probe paths mismatch: {matrix.get('s8b1a_probe_paths')!r}")
    if matrix.get("s8b1a_keeps_s7a2_module_recipe") is not True:
        raise SystemExit("M34 S8B1A matrix does not prove S7A2 module recipe reuse")
    if matrix.get("s8b1a_skips_downstream_configfs_and_udc_to_isolate_probe") is not True:
        raise SystemExit("M34 S8B1A matrix does not prove downstream USB isolation")

    if matrix.get("s8b1a_widens_i2c_adapter_number_assumption") is not True:
        raise SystemExit("M34 S8B1A matrix does not prove widened I2C adapter-number probe")

    if stage.get("stage_number") != 10:
        raise SystemExit(f"M34 S8B1A stage number mismatch: {stage.get('stage_number')!r}")
    expected_steps = {
    ...
    if runtime_steps != expected_steps:
        raise SystemExit(f"M34 S8B1A runtime steps mismatch: {runtime_steps!r}")
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M34 S8B1A manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if stage.get("tar_members") != [EXPECTED_MEMBER]:
        raise SystemExit(f"M34 S8B1A manifest tar members mismatch: {stage.get('tar_members')!r}")
    ...
    required_safety = {
        ...
        "stage_s8b1a_beacon_probe": EXPECTED_PROBE,
    ...
    if closure.get("modules") != list(EXPECTED_MODULES):
        raise SystemExit(f"M34 S8B1A module closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M34 S8B1A module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M34_MODULE_LIST_SHA256:
        raise SystemExit(f"M34 S8B1A module list SHA mismatch: {closure.get('module_sha256')!r}")
    if closure.get("geni_i2c_transport_targets") != M34_S7A2_I2C_GENI_TRANSPORT_TARGETS:
        raise SystemExit(f"M34 S8B1A closure GENI I2C targets mismatch: {closure.get('geni_i2c_transport_targets')!r}")
    if closure.get("geni_i2c_transport_order_actual") != M34_S7A2_I2C_ORDER_ACTUAL:
        raise SystemExit(f"M34 S8B1A closure GENI I2C order mismatch: {closure.get('geni_i2c_transport_order_actual')!r}")
    modules = closure.get("modules", [])
    if modules.index("i2c-msm-geni.ko") >= modules.index("pdic_max77705.ko"):
        raise SystemExit("M34 S8B1A module order must load i2c-msm-geni.ko before pdic_max77705.ko")
    if closure.get("session_producer_targets") != M34_S7A_SESSION_PRODUCER_TARGETS:
        raise SystemExit(f"M34 S8B1A session-producer targets mismatch: {closure.get('session_producer_targets')!r}")
    if closure.get("additional_new_modules") != M34_S7A2_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S8B1A additional module list mismatch: {closure.get('additional_new_modules')!r}")
    if closure.get("risk_modules") != M34_S8B1A_RISK_MODULES:
        raise SystemExit(f"M34 S8B1A risk module list mismatch: {closure.get('risk_modules')!r}")
    if closure.get("contains_sec_debug_region") is not True:
        raise SystemExit("M34 S8B1A closure does not explicitly carry sec_debug_region")
    if closure.get("requires_live_risk_review") is not True:
        raise SystemExit("M34 S8B1A closure does not require live risk review")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M34 S8B1A manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M34 S8B1A module-list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M34 S8B1A must not inject module binaries into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M34 S8B1A required string missing from manifest: {required}")
    for forbidden in [
        "phase=configfs_done",
        ...
        if forbidden in required_strings:
            raise SystemExit(f"M34 S8B1A manifest unexpectedly requires forbidden string: {forbidden}")

    objdump_path = path.parent / EXPECTED_STAGE / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M34 S8B1A objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S8B1A /init does not load arm64 __NR_finit_module (273)")
    if not any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S8B1A /init does not load arm64 __NR_reboot (142)")
...
def verify_m34_artifacts(
    *,
    ...
    verify_ap(m34_ap, EXPECTED_M34_AP_SHA256, "m34_s8b1a_candidate", log_path)
...
def android_predicate_baseline_path(run_dir: Path) -> Path:
    return run_dir / "s22plus_m34_s8b1a_android_predicate_baseline.json"


def android_reset_context_baseline_path(run_dir: Path) -> Path:
    return run_dir / "s22plus_m34_s8b1a_android_reset_context_baseline.json"
...
def read_android_root_text_value(serial: str, path: str, log_path: Path) -> str:
    ...
    append_log(log_path, f"s8b1a_android_root_value_rc_{key}={proc.returncode}")
    if proc.returncode != 0:
        append_log(log_path, f"s8b1a_android_root_value_stderr_{key}={proc.stderr.strip()!r}")
        return "<missing>"
    return proc.stdout.replace("\n", " ").strip()


def collect_android_s8b1a_predicate_baseline(
    *,
    run_dir: Path,
    log_path: Path,
    serial: str,
...
""".replace("__PROBE_PATHS__", probe_paths).replace("__CLASS_VALUE_PATHS__", class_values)
    ...
    append_log(log_path, f"s8b1a_android_predicate_baseline_rc={proc.returncode}")
    append_log(log_path, f"s8b1a_android_predicate_baseline_stdout={proc.stdout.strip()!r}")
    append_log(log_path, f"s8b1a_android_predicate_baseline_stderr={proc.stderr.strip()!r}")
    if proc.returncode != 0:
        raise SystemExit(f"failed to collect Android S8B1A predicate baseline: rc={proc.returncode}")
    ...
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s8b1a_android_predicate_baseline_v1",
    ...
    append_log(log_path, f"s8b1a_android_predicate_baseline_json={path}")
    append_log(log_path, f"s8b1a_android_predicate_baseline={json.dumps(payload, sort_keys=True)}")
    if not predicate_true:
        raise SystemExit(f"Android S8B1A predicate baseline is false for {EXPECTED_PROBE_PATHS}")
    return payload
...
def collect_android_reset_context_baseline(
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s8b1a_android_reset_context_baseline_v1",
        "timestamp_utc": utc_now(),
        "serial": serial,
        "summary": summary,
    }
    ...
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    append_log(log_path, f"s8b1a_android_reset_context_baseline_json={path}")
    append_log(
        log_path,
        "s8b1a_android_reset_context_baseline="
...
def run_android_readonly_preflight(
    *,
    ...
    collect_android_s8b1a_predicate_baseline(run_dir=run_dir, log_path=log_path, serial=selected_serial)
...
def write_result_summary(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s8b1a_result_v1",
        "timestamp_utc": utc_now(),
...
def write_result_analysis(run_dir: Path, log_path: Path, result_json: Path) -> None:
    try:
        from analyze_s22plus_m34_s8b1a_result import classify_result, load_json
        ...
        analysis_path = run_dir / "s22plus_m34_s8b1a_result_analysis.json"
...
def live_runbook(args: argparse.Namespace, packet_path: Path | None = None) -> str:
    helper = "workspace/public/src/scripts/revalidation/s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.py"
    analyzer = "workspace/public/src/scripts/revalidation/analyze_s22plus_m34_s8b1a_result.py"
    ...
    lines = [
        "# S22+ M34 S8B1A live runbook (no command below inserts AGENTS.md by itself)",
...
def write_prelive_packet(
    *,
    ...
    runbook_path = run_dir / "s22plus_m34_s8b1a_live_runbook.txt"
    active_template_path = run_dir / "s22plus_m34_s8b1a_active_exception_template.txt"
    packet_path = run_dir / "s22plus_m34_s8b1a_prelive_packet.json"
    runbook = live_runbook(packet_args, packet_path=packet_path)
    baseline_path = android_predicate_baseline_path(run_dir)
    if not baseline_path.is_file():
        raise SystemExit(f"prelive packet requires Android S8B1A predicate baseline: {baseline_path}")
    baseline_payload = json.loads(baseline_path.read_text(encoding="utf-8"))
    if not baseline_payload.get("predicate_true"):
        raise SystemExit("prelive packet refuses false Android S8B1A predicate baseline")
    ...
    if reset_baseline_payload.get("schema") != "s22plus_m34_s8b1a_android_reset_context_baseline_v1":
        raise SystemExit(
    ...
    material_paths = {
        ...
        "android_s8b1a_predicate_baseline_json": baseline_path,
        "android_reset_context_baseline_json": reset_baseline_path,
    }
    payload = {
        "schema": "s22plus_m34_s8b1a_prelive_packet_v1",
        ...
        "android_s8b1a_predicate_baseline": baseline_payload,
        "android_s8b1a_predicate_baseline_json": str(baseline_path),
...
def verify_prelive_packet(
    *,
    ...
    baseline = payload.get("android_s8b1a_predicate_baseline")
    if not isinstance(baseline, dict):
        raise SystemExit("prelive packet missing Android S8B1A predicate baseline")
    if baseline.get("schema") != "s22plus_m34_s8b1a_android_predicate_baseline_v1":
        raise SystemExit(f"prelive packet Android S8B1A predicate baseline schema mismatch: {baseline.get('schema')!r}")
    if baseline.get("serial") != selected_serial:
        raise SystemExit("prelive packet Android S8B1A predicate baseline serial mismatch")
    if baseline.get("probe") != EXPECTED_PROBE or baseline.get("probe_paths") != EXPECTED_PROBE_PATHS:
        raise SystemExit("prelive packet Android S8B1A predicate baseline probe mismatch")
    if not baseline.get("predicate_true"):
        raise SystemExit("prelive packet Android S8B1A predicate baseline is false")
    hints = baseline.get("future_b2_hints")
    if not isinstance(hints, dict):
        raise SystemExit("prelive packet Android S8B1A predicate baseline missing future_b2_hints")
    if hints.get("candidate_paths") != ANDROID_S8B2_HINT_PATHS:
        raise SystemExit("prelive packet Android S8B1A future B2 candidate path mismatch")
    if hints.get("value_paths") != ANDROID_S8B2_HINT_VALUE_PATHS:
        raise SystemExit("prelive packet Android S8B1A future B2 value path mismatch")
    if not isinstance(hints.get("paths"), dict) or not isinstance(hints.get("values"), dict):
        raise SystemExit("prelive packet Android S8B1A future B2 hints are malformed")
    baseline_json = Path(str(payload.get("android_s8b1a_predicate_baseline_json", "")))
    if baseline_json != android_predicate_baseline_path(packet_run_dir):
        raise SystemExit("prelive packet Android S8B1A predicate baseline path mismatch")
    if not baseline_json.is_file():
        raise SystemExit(f"prelive packet Android S8B1A predicate baseline JSON missing: {baseline_json}")
    if json.loads(baseline_json.read_text(encoding="utf-8")) != baseline:
        raise SystemExit("prelive packet Android S8B1A predicate baseline JSON is stale")
    ...
    if reset_baseline.get("schema") != "s22plus_m34_s8b1a_android_reset_context_baseline_v1":
        raise SystemExit(
            "prelive packet Android reset-context baseline schema mismatch: "
    ...
    expected_material_paths = {
        "runbook": runbook_path,
        "active_exception_template": active_template_path,
        "android_s8b1a_predicate_baseline_json": baseline_json,
...
def verify_prelive_packet_dryrun_evidence(dryrun_dir: Path, log_path: Path) -> None:
    dryrun_log = dryrun_dir / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt"
...
def observe_download_beacon(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m34_s8b1a_observe_start_utc={utc_now()}")
    append_log(log_path, f"m34_s8b1a_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m34_s8b1a_beacon_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m34_s8b1a_result=download-beacon-hit elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "download-beacon-hit", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M34 S8B1A observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m34_s8b1a_result=unexpected-adb-before-rollback elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-rollback", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m34_s8b1a_result=download-beacon-miss-parked-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m34_s8b1a_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if actual_rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m34_s8b1a_{label}_boot_restore_hash_check=skipped rollback_target={actual_rollback_target}")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m34_s8b1a_{label}_rollback", android, marker=EXPECTED_M34_MARKER)
    append_log(log_path, f"m34_s8b1a_{label}_retained_marker_found={int(marker_found)}")
    return RollbackResult(
        rc=0,
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m34-s8b1a-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M34 S8B1A rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    if args.observe_sec < 30:
        raise SystemExit("--observe-sec must be at least 30 for the M34 S8B1A download-beacon probe")
    ...
    if print_only_mode:
        with tempfile.TemporaryDirectory(prefix="s22plus_m34_s8b1a_print_") as tmp:
            log_path = Path(tmp) / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt"
            append_log(log_path, f"=== {utc_now()} s22plus M34 S8B1A download-beacon probe live gate ===")
            ...
            if args.write_agents_candidate is not None:
                ...
                print(
                    "write-agents-candidate ok: exact M34 S8B1A active exception inserted into candidate; "
            ...
            if args.verify_agents_candidate is not None:
                ...
                print(
                    "verify-agents-candidate ok: exact M34 S8B1A active exception is present; "
                    f"no AGENTS.md write, no device action; candidate={candidate_path}"
                )
                return 0
    ...
    log_path = run_dir / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M34 S8B1A download-beacon probe live gate ===")
    append_log(log_path, f"target={EXPECTED_TARGET}")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M34 S8B1A candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.readonly_preflight:
        ...
        print(
            "readonly-preflight ok: M34 S8B1A candidate, rollback APs, Android stability, "
    ...
    if args.prelive_packet:
        ...
        print(
            "prelive-packet ok: M34 S8B1A artifacts, rollback APs, Android stability, "
    ...
    if args.verify_prelive_packet is not None:
        ...
        print(
            "verify-prelive-packet ok: packet matches current S8B1A helper contract, "
    ...
    if args.verify_prelive_packet_after_dryrun is not None:
        ...
        print(
            "verify-prelive-packet-after-dryrun ok: packet matches current S8B1A helper contract, "
    ...
    if args.rollback_from_download:
        ...
        print(f"M34 S8B1A rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M34 S8B1A candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M34 S8B1A live gate starting. HIT should self-enter Download mode if "
    ...
    if odin_device is None:
        ...
        print("download mode did not appear for M34 S8B1A candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        ...
        print(f"M34 S8B1A candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m34_s8b1a_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M34 S8B1A candidate flashed. Observing download beacon for {args.observe_sec}s.", flush=True)
    ...
    if result == "download-beacon-hit":
        if rollback_device is None:
            ...
            print(f"M34 S8B1A HIT recorded but no rollback device is available. log={log_path}", file=sys.stderr)
            return 4
        rollback = rollback_boot_only_from_download(
            odin=odin,
            rollback_ap=rollback_ap,
        ...
        print(f"M34 S8B1A live gate completed rc={rollback.rc}; result={result}; log={log_path}")
        return rollback.rc
    ...
    if result != "download-beacon-miss-parked-manual-download-required":
        ...
        print(
            f"M34 S8B1A stopped before clean HIT/MISS proof ({result}). If the device is not in Android, "
    ...
    print(
        f"M34 S8B1A result={result}. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        ...
        print(
            f"M34 S8B1A MISS observed, but manual Download mode did not appear. "
