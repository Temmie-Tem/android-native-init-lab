#!/usr/bin/env python3
"""Build V3370 native-init boot image for the server-distro D3B switch_root handoff."""
...
CYCLE = "V3370"
INIT_VERSION = "0.11.131"
INIT_BUILD = "v3370-server-distro-switchroot-loopfix"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3370_SERVER_DISTRO_D3B_SWITCHROOT_LOOPFIX_SOURCE_BUILD_2026-07-03.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3370_server_distro_switchroot_loopfix"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3370_server_distro_switchroot_loopfix.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v629_server_distro_switchroot"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3370"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3370.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3370.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3370"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3370-server-distro-switchroot-loopfix"

FRAME_PATH = "/tmp/a90-doomgeneric-v3370-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3370-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3370-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3370-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3370-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3370-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3370-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3370.c"
...
def _rewrite_v3370_text(text: str) -> str:
    ...
    replacements = (
        ...
        ("hot-reload-autohud", "server-distro-switchroot-loopfix"),
        ("v3368", "v3370"),
        ("V3368", "V3370"),
        ("a90-doomgeneric-v3368", "a90-doomgeneric-v3370"),
        ("a90.doomgeneric.v3368", "a90.doomgeneric.v3370"),
    )
...
def _rewrite_v3370_bytes(item: bytes) -> bytes:
    return _rewrite_v3370_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3370_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3370_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3370_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3370_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3370_bytes(marker) for marker in previous.REQUIRED_STRINGS)

REQUIRED_STRINGS = PREVIOUS_REQUIRED_STRINGS + (
    b"0.11.131",
    b"v3370-server-distro-switchroot-loopfix",
...
def v3370_adapter_source() -> str:
    return _rewrite_v3370_text(ORIG_PREVIOUS_ADAPTER_SOURCE())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "server-distro-switchroot-loopfix.json").write_text(json.dumps({
...
def _normalize_manifest_for_v3370(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest.update({
        "decision": DECISION,
        "cycle": CYCLE,
        "candidate_tag": INIT_BUILD,
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest = _normalize_manifest_for_v3370(json.loads(manifest_path.read_text(encoding="utf-8")))
    if base_main_error:
...
def _postprocess_manifest() -> dict[str, Any]:
    manifest = _normalize_manifest_for_v3370(ORIG_PREVIOUS_POSTPROCESS())
    manifest_path = OUT_DIR / "manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_PATH.write_text(
        render_report(
...
def _overlay_preserved_v3370_ramdisk() -> dict[str, Any]:
    overlay = ORIG_PREVIOUS_OVERLAY()
    overlay["mode"] = "preserve-v3335-ramdisk-overlay-v3370-init-helper-engine"
    return overlay


def _patch_v3368_module_for_v3370() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "v3368_adapter_source": v3370_adapter_source,
        "_rewrite_v3368_text": _rewrite_v3370_text,
        "_rewrite_v3368_bytes": _rewrite_v3370_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_overlay_preserved_v3368_ramdisk": _overlay_preserved_v3370_ramdisk,
