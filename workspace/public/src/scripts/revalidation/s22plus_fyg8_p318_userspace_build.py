#!/usr/bin/env python3
"""Build P3.18 userspace twice against the fixed P3.10 Image."""
...
SCHEMA = "s22plus_fyg8_p318_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p318/userspace")
COMPILE_FLAGS = base.COMPILE_FLAGS
BuildError = base.BuildError


class _P318SourceModule:
    MODULE_PLAN_COUNT = 70
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("P3.18 userspace profile/source contract differs")
    return VERDICT
...
def _compile(
    ...
    def p318_selected(source_contract_id: str, profile: str):  # noqa: ANN202
        selected = original_selected(source_contract_id, profile)
        return replace(selected, module=_P318SourceModule(selected.module))

    base._selected = p318_selected  # noqa: SLF001
    try:
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.18 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p318-userspace-{label}-") as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.18 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
