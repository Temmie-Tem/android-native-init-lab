#!/usr/bin/env python3
"""Build a P3.03 candidate with fixed P3.00 Image and exact FYG8 HS-PHY module."""
...
import s22plus_fyg8_p303_telemetry_spec as p303_spec
import s22plus_fyg8_p303_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p303_candidate_artifact_result_v1"
VERDICT = "PASS_P303_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p303/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    if result_path != root / contract.PARENT_REPRO_RESULT:
        raise BuildError("P3.03 parent reproducibility path differs")
    value, result_receipt = _read_json(
        result_path, "P3.03 fixed P3.00 reproducibility result"
    )
    parent = exact_contract.get("parent_candidate_contract")
    ...
    if (
        ...
        or value.get("candidate_contract") != parent
        ...
        or linked.get("audit_adapter") != "s22plus-fyg8-p300-linked-audit-v1"
        ...
        raise BuildError("P3.03 fixed P3.00 reproducibility closure differs")
    return {
    ...
    result.update(
        {
            ...
            "built_in_p300_telemetry_reused": True,
            "p303_kernel_rebuild": False,
            "p303_full_lto_ab": False,
            "fixed_image_sha256": contract.EXPECTED_IMAGE["sha256"],
            "hsphy_module_runtime_source": "unchanged FYG8 vendor_dlkm",
            "hsphy_module_sha256": p303_spec.MODULE_SHA256,
            "hsphy_module_build_id": p303_spec.MODULE_BUILD_ID,
            "post_bl_callsite_probe_count": p303_spec.CALLSITE_COUNT,
