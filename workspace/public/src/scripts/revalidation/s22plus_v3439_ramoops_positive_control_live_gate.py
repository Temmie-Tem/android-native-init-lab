#!/usr/bin/env python3
"""Guarded, resumable S22+ V3439 ramoops Android positive-control helper.

Only --offline-check and --print-plan are usable while the two V3439 AGENTS
...
DTBO_POLICY_MARKER = "S22+ V3439 ramoops DTBO maintenance live gate"
PANIC_POLICY_MARKER = "S22+ V3439 ramoops intentional-panic live gate"
DTBO_ACK_TOKEN = "S22PLUS-V3439-RAMOOPS-DTBO-MAINTENANCE"
PANIC_ACK_TOKEN = "S22PLUS-V3439-RAMOOPS-INTENTIONAL-PANIC"
RESTORE_ACK_TOKEN = "S22PLUS-V3439-RAMOOPS-STOCK-DTBO-RESTORE"
DTBO_ACTIVE_SENTINEL = "S22PLUS_V3439_DTBO_POLICY_STATE=ACTIVE"
PANIC_ACTIVE_SENTINEL = "S22PLUS_V3439_PANIC_POLICY_STATE=ACTIVE"

DTBO_POLICY_DRAFT = Path(
    "docs/operations/"
    "S22PLUS_V3439_RAMOOPS_DTBO_MAINTENANCE_AGENTS_EXCEPTION_DRAFT_2026-07-11.md"
)
PANIC_POLICY_DRAFT = Path(
    "docs/operations/"
    "S22PLUS_V3439_RAMOOPS_INTENTIONAL_PANIC_AGENTS_EXCEPTION_DRAFT_2026-07-11.md"
)
V3436_CONTRACT = design.OUTPUT
V3436_CONTRACT_SHA256 = (
    "c96ac1ce196e3584fab2af13f728655486d27ea4f417c93fd9b6558707d86de7"
)
V3438_POSTMORTEM = Path(
    "docs/plans/s22plus-v3438-ramoops-backend-postmortem.json"
)
V3438_POSTMORTEM_SHA256 = (
    "f5c12e50e01d9b7938a2482f4990a0620f9d7e7cb0fb6837350357af009ec5a4"
)
DTBO_POLICY_DRAFT_SHA256 = (
    "dbb6b43e0040c58612f5c9322bdd99a93f958012f655ab4248aebbcf7e3f4f18"
)
PANIC_POLICY_DRAFT_SHA256 = (
    "de5cd61381b8fabe0e1385ff59d9bfef7a65c8ed33b7b1d0fa5160650c4084bd"
)

SESSION_SCHEMA = "s22plus_v3439_ramoops_positive_control_session_v1"
...
def allocate_run_dir(root: Path, requested: Path | None) -> Path:
    ...
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_v3439_ramoops_{stamp}")
...
def verify_artifacts(root: Path) -> dict[str, Any]:
    ...
    postmortem_path = resolve(root, V3438_POSTMORTEM)
    if sha256_file(postmortem_path) != V3438_POSTMORTEM_SHA256:
        raise GateError("V3438 postmortem hash mismatch")
    postmortem = json.loads(postmortem_path.read_text(encoding="utf-8"))
    if (
        postmortem.get("verdict")
        != "HOST_POSTMORTEM_PASS_V3437_BACKEND_GATE_FALSE_NEGATIVE"
    ):
        raise GateError("V3438 postmortem verdict mismatch")
    return contract
...
def require_active_policies(root: Path, *, panic: bool) -> None:
    ...
    if not status["dtbo_active"]:
        raise GateError("V3439 DTBO maintenance policy is inactive")
    if panic and not status["panic_active"]:
        raise GateError("V3439 intentional-panic policy is inactive")
...
def verify_patched_backend(serial: str, log_path: Path) -> dict[str, Any]:
    ...
    backend_text = root_bytes(
        ...
        "printf 'pstore_backend='; cat /sys/module/pstore/parameters/backend; "
        "count=0; "
        "for p in /sys/bus/platform/drivers/ramoops/*; do "
        "[ -L \"$p\" ] || continue; count=$((count + 1)); "
        "printf 'bound_device=%s\\n' \"${p##*/}\"; "
        "printf 'bound_compatible='; tr -d '\\000' < \"$p/of_node/compatible\"; echo; "
        "printf 'bound_status='; tr -d '\\000' < \"$p/of_node/status\"; echo; "
        "done; printf 'bound_count=%s\\n' \"$count\"",
    ).decode("utf-8", errors="replace")
    backend = dict(
        line.split("=", 1)
    ...
    early_log_text = root_bytes(
        serial,
        "dmesg | grep -E "
        "'Registered ramoops as persistent store backend|ramoops: using' "
        "| tail -10 || true",
    ).decode("utf-8", errors="replace")
    ...
    durable_append(log_path, f"candidate_backend={json.dumps(backend, sort_keys=True)}")
    durable_append(log_path, f"candidate_early_backend_log={early_log_text}")
    ...
    if any(backend.get(key) != value for key, value in expected_backend.items()):
        raise GateError(
            "ramoops backend/binding proof mismatch: "
            + json.dumps(backend, sort_keys=True)
        )
    if not backend.get("bound_device"):
        raise GateError("ramoops bound platform device name missing")
    return {
        "live_dt": live,
        "parameters": parameters,
        "backend": backend,
        "early_log": early_log_text,
    }
...
def rollback_from_android(
    ...
    reboot_android_to_download(serial, log_path, "v3439-stock-dtbo-rollback")
    timeline.append("rollback_flash_start")
    rc = flash_selected_ap(
        odin, resolve(root, design.ROLLBACK_AP), log_path, "v3439-stock-dtbo-rollback", wait_seconds
    )
    if rc != 0:
...
def run_live_session(
    ...
    log_path = run_dir / "v3439_live_gate.log"
    ...
    try:
        ...
        reboot_android_to_download(serial, log_path, "v3439-candidate-dtbo")
...
def load_resume(run_dir: Path) -> tuple[Session, Timeline, Path]:
    ...
    return session, timeline, run_dir / "v3439_live_gate.log"


def print_plan() -> None:
    print("V3439 ramoops positive-control plan (policy-gated)")
    ...
    print("4. live-session: candidate -> sysfs/binding proof -> markers -> one panic")
...
def main(argv: list[str]) -> int:
    ...
    if args.dry_run:
        run_dir = allocate_run_dir(root, args.run_dir)
        log_path = run_dir / "v3439_dry_run.log"
        serial = require_current_android(log_path, args.serial)
        verify_stock_baseline(serial, log_path, args.stability_samples)
