#!/usr/bin/env python3
"""Build V2563 ARM32 post-initialize target-only ACDB capture artifacts.
...
The key V2563 delta is that the acdb_ioctl wrapper is target-only after INITIALIZE_V2:
calls before ACDB_CMD_INITIALIZE_V2 returns pass through without dump/hash/file I/O.  After INITIALIZE_V2 succeeds, only the custom-topology size/data commands are captured; all other init-time calls still pass through silently.  The preload still fakes the audio calibration allocate/dealloc/
...
RUN_ID = "V2563"
BUILD_TAG = "v2563-acdb-postinitialize-topology-capture-host-only"
...
HELPER_ARTIFACT_NAME = "a90_acdb_postinitialize_topology_exec_linked_v2563"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_postinitialize_topology_capture_v2563.so"
...
def source_state() -> dict[str, Any]:
    ...
    required = {
        ...
        "helper_has_postinit_fallback_arm": "a90_arm_capture();" in helper_text and "armed_before_common_topology" in helper_text,
        "helper_calls_common_topology_fallback": "acdb_loader_send_common_custom_topology()" in helper_text,
...
def vendor_lib_state(readelf: str) -> dict[str, Any]:
    ...
    state["required_for_v2563_ok"] = all(state["required_for_v2563"].values())
    return state
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    helper_obj = obj_dir / "a90_acdb_postinitialize_topology_exec_linked_v2563.o"
    tap_obj = obj_dir / "libacdbtap_v2475_postinitialize_target.o"
    ...
    return {
        ...
        "compile": {"helper": helper_compile, "acdbtap_postinitialize_target": tap_compile, "ioctl_trace": ioctl_compile},
...
def manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            ...
            "unarmed_policy": "all acdb_ioctl calls before INITIALIZE_V2 returns pass through with no dump/hash/file I/O",
            "arm_point": "auto-arm immediately after ACDB_CMD_INITIALIZE_V2 returns ret==0; helper post-init arm remains fallback-only",
            "armed_policy": "capture only custom-topology size/data commands; exit immediately after first ret==0 non-all-zero 4916-byte buffer",
    ...
    payload["ok"] = bool(
        ...
        and payload["vendor_lib_state"].get("required_for_v2563_ok")
