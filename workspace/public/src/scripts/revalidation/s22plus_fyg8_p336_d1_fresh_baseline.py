#!/usr/bin/env python3
"""P3.36 one-shot attended D1 baseline rotation.

The reviewed P3.35 wrapper is loaded as an isolated source graph.  Only the
P3.36 binding, run identity, and no-replay namespace are new; the P2.96
...
P335_SOURCE = SCRIPT_DIR / "s22plus_fyg8_p335_d1_fresh_baseline.py"
...
RUN_PARENT = ROOT / "workspace/private/runs/device-action-d1-p336-fresh-baseline"
RUN_DIR = RUN_PARENT / "p336-d1-fresh-baseline-1"
RUN_ARM = RUN_PARENT / "p336-d1-fresh-baseline-1.arm.json"
RUN_STOP = RUN_DIR / "stop.json"
RAW_ROOT = RUN_PARENT / "p336-d1-fresh-baseline-1-raw"
RAW_ADB_DIR = RAW_ROOT / "raw-adb"
ADB_SNAPSHOT = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p336/d1-fresh-baseline-1/"
...
ORDINAL = "p336-d1-fresh-baseline-1"
BINDING_SCHEMA = "s22plus_fyg8_p336_d1_fresh_baseline_execution_binding_v1"
BINDING_ID = "s22plus-fyg8-p336-d1-fresh-baseline-v1"
AUTHORITY_PREFIX = "DEVICE-ACTION-D1-P336-FRESH-BASELINE-1-APPROVE:"
REVIEW_VERDICT = "PASS_GO_P336_D1_FRESH_BASELINE_H0_CAPABILITY_V1"
RESULT_SCHEMA = "s22plus_fyg8_p336_d1_fresh_baseline_result_v1"
RESULT_VERDICT = "PASS_P336_D1_FRESH_BASELINE_EXACT_NORMAL_REBOOT_RETURN_HEALTH"
STOP_SCHEMA = "s22plus_fyg8_p336_d1_fresh_baseline_stop_v1"
STOP_VERDICT = "STOP_P336_D1_FRESH_BASELINE_CONSUMED_NO_REPLAY"
HOST_ADB_SIZE = 716_968
HOST_ADB_SHA256 = "05a1a4435e436230931acd8737fd68f31542d652731d3ca8c464cab7a42be226"


class D1Error(RuntimeError):
    """The P3.36 D1 action failed closed."""
...
def _load_p335() -> types.ModuleType:
    payload = _stable(P335_SOURCE, "P3.35 D1 source", maximum=2 * 1024 * 1024)
    module = types.ModuleType("s22plus_fyg8_p335_d1_bound_for_p336")
    module.__file__ = str(P335_SOURCE)
    module.__package__ = ""
    try:
        exec(compile(payload, str(P335_SOURCE), "exec", dont_inherit=True), module.__dict__)
    except BaseException as exc:
        raise D1Error(f"P3.35 D1 source failed to load: {type(exc).__name__}") from exc
    replacements = {
        "ROOT": ROOT,
        "SCRIPT": SCRIPT,
    ...
    def source_receipts() -> dict[str, dict[str, Any]]:
        ...
        value["p335_source"] = _receipt(
            P335_SOURCE,
            _stable(P335_SOURCE, "P3.35 D1 source", maximum=2 * 1024 * 1024),
        )
        return value
...
def _validated_static_inputs() -> dict[str, Any]:
    return _load_p335()._validated_static_inputs()
...
def self_test() -> dict[str, Any]:
    ...
    if value.get("reboot_count") != 1 or value.get("other_target_commands") != 0 or value.get("device_contact") is not False:
        raise D1Error("P3.36 D1 fixture result differs")
    return {
        "schema": "s22plus_fyg8_p336_d1_fresh_baseline_fixture_h0",
        "verdict": "PASS_P336_D1_FRESH_BASELINE_FIXTURE_H0",
        "ordinal": ORDINAL,
        "run_id": P336_RUN_ID,
        "reboot_count": 1,
...
def run_live(approval: str) -> dict[str, Any]:
    ...
    result.update({
        ...
        "version": "device-action-d1-p336-v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"P3.36 D1 blocked: {exc}", file=sys.stderr)
        return 2
