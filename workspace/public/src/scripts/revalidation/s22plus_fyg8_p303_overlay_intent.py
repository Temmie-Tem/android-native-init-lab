#!/usr/bin/env python3
"""Materialize one P3.03 userspace intent over exact P3.01-r1."""
...
def create(root: Path, output: Path) -> dict:
    if output.exists() or output.is_symlink():
        raise IntentError(f"P3.03 overlay intent output exists: {output}")
    ...
    try:
        if os.write(descriptor, payload) != len(payload):
            raise IntentError("short P3.03 overlay intent write")
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
