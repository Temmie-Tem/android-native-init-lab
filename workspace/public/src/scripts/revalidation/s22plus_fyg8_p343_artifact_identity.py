#!/usr/bin/env python3
"""Host-only P3.43 Image/AP identity binding.

The consumed P3.42 Image is transformed once by replacing its 32-byte run
marker in the raw region and in the same-length IKCONFIG gzip stream.  The
candidate remains boot-only; no device, ADB, Odin, or partition operation is
performed here.
"""
...
import s22plus_fyg8_p342_artifact_identity as predecessor
...
# Filled from the deterministic H0 transform/build and retained as the
# expected target identity for A/B package joins.
P343_IMAGE_IDENTITY = {
    "size": 41_490_944,
    "sha256": "10386e0ce53f295bf081f32c9004a85ffc746ca16966ca35a0ad767b5dd6caa2",
}
P343_AP_IDENTITY: dict[str, Any] = {
    "size": 28_631_081,
    "sha256": "14691cee846f41e112fef3eb5252c5c8eef2f734e4bb94ba7bd457f765e843ef",
}
...
P343_UNSAT_TAG_HEX = predecessor.P342_UNSAT_TAG_HEX
...
if identity(SOURCE.read_bytes()) != SOURCE_IDENTITY:
    raise ArtifactIdentityError("P3.42 artifact helper source identity differs")
if predecessor.P342_RUN_ID_HEX != P342_PREDECESSOR_RUN_ID_HEX:
    raise ArtifactIdentityError("P3.42 artifact helper binding differs")
...
def _stale_counts(payload: bytes) -> dict[str, int]:
    ...
    value[P342_PREDECESSOR_RUN_ID_HEX] = payload.count(
        P342_PREDECESSOR_RUN_ID_HEX.encode("ascii")
    )
    value[P343_RUN_ID_HEX] = payload.count(P343_RUN_ID_HEX.encode("ascii"))
    return value


def validate_image(
    image: bytes, *, expected_run_id: bytes = P343_RUN_ID
) -> dict[str, Any]:
    if type(expected_run_id) is not bytes or expected_run_id != P343_RUN_ID:
        raise ArtifactIdentityError("P3.43 Image run ID differs")
    compressed_start, end, compressed, config = _gzip_config(image, "P343 Image")
    values = _config_values(config, "P343 Image")
    run_match = re.fullmatch(r'"([0-9a-f]{32})"', values[RUN_CONFIG_KEY])
    unsat_match = re.fullmatch(r'"([0-9a-f]{32})"', values[UNSAT_CONFIG_KEY])
    if run_match is None or unsat_match is None or run_match.group(1) != P343_RUN_ID_HEX:
        raise ArtifactIdentityError("P343 Image IKCONFIG run ID differs")
    if values[UNSAT_CONFIG_KEY] != f'"{P343_UNSAT_TAG_HEX}"':
        raise ArtifactIdentityError("P343 Image IKCONFIG UNSAT tag differs")
    outside = image[:compressed_start] + image[end:]
    counts = _stale_counts(outside)
    if counts[P343_RUN_ID_HEX] != 1 or any(
        count != 0 for key, count in counts.items() if key != P343_RUN_ID_HEX
    ):
        raise ArtifactIdentityError("P343 Image raw run-ID occurrences are not exact")
    raw_offset = image.find(P343_RUN_ID_HEX.encode("ascii"))
    if raw_offset < 0 or compressed_start <= raw_offset < end:
        raise ArtifactIdentityError("P343 Image raw run-ID is inside IKCONFIG")
    sections = _section_receipts(image, "P343 Image")
    return {
...
def transform_image(original: bytes) -> tuple[bytes, dict[str, Any]]:
    """Replace only P342's run ID in one same-length Image transform."""

    if identity(original) != P342_IMAGE_IDENTITY:
        raise ArtifactIdentityError("P3.42 source Image identity differs")
    compressed_start, end, compressed, config = _gzip_config(
        original, "P3.42 source Image"
    )
    values = _config_values(config, "P3.42 source Image")
    old_line = f'{RUN_CONFIG_KEY}="{P342_PREDECESSOR_RUN_ID_HEX}"'.encode("ascii")
    new_line = f'{RUN_CONFIG_KEY}="{P343_RUN_ID_HEX}"'.encode("ascii")
    if config.count(old_line) != 1 or config.count(new_line):
        raise ArtifactIdentityError("P3.42 source IKCONFIG run-ID line is not exact")
    if values[UNSAT_CONFIG_KEY] != f'"{P343_UNSAT_TAG_HEX}"':
        raise ArtifactIdentityError("P3.42 source UNSAT tag differs")
    old_raw = P342_PREDECESSOR_RUN_ID_HEX.encode("ascii")
    new_raw = P343_RUN_ID_HEX.encode("ascii")
    outside = original[:compressed_start] + original[end:]
    if outside.count(old_raw) != 1 or outside.count(new_raw):
        raise ArtifactIdentityError("P3.42 source raw run-ID occurrence is not unique")
    raw_offset = original.find(old_raw)
    if raw_offset < 0 or compressed_start <= raw_offset < end:
        raise ArtifactIdentityError("P3.42 source raw run-ID is inside IKCONFIG")
    ...
    if (
        ...
        raise ArtifactIdentityError("P3.43 IKCONFIG gzip transform is not same-length exact")
    ...
    if len(transformed) != len(original):
        raise ArtifactIdentityError("P3.43 Image size changed during transform")
    checked = validate_image(transformed)
    if checked["identity"] != P343_IMAGE_IDENTITY:
        raise ArtifactIdentityError("P3.43 Image identity differs")
    return transformed, {
        "method": "identity_only_post_link_v1",
        "source": {"run_id_hex": P342_PREDECESSOR_RUN_ID_HEX, **identity(original)},
        "target": {"run_id_hex": P343_RUN_ID_HEX, **identity(transformed)},
        "raw_rodata": {
            "offset": raw_offset,
            "size": len(old_raw),
            "old_ascii": P342_PREDECESSOR_RUN_ID_HEX,
            "new_ascii": P343_RUN_ID_HEX,
...
def _validate_init(init: bytes, expected_run_id: bytes = P343_RUN_ID) -> dict[str, Any]:
    if type(expected_run_id) is not bytes or expected_run_id != P343_RUN_ID:
        raise ArtifactIdentityError("P3.43 init run ID differs")
    counts = _stale_counts(init)
    if counts[P343_RUN_ID_HEX] != 1 or any(
        count != 0 for key, count in counts.items() if key != P343_RUN_ID_HEX
    ):
        raise ArtifactIdentityError("P343 /init run-ID occurrences are not exact")
    return {"identity": identity(init), "run_id_hex": P343_RUN_ID_HEX, "counts": counts}
...
def inspect_ap(
    ...
    *,
    ...
    label: str = "P343 candidate AP",
) -> dict[str, Any]:
    if expected_run_id != P343_RUN_ID:
        raise ArtifactIdentityError("P3.43 AP run ID differs")
    ...
    try:
        ...
        raise ArtifactIdentityError(str(exc)) from exc
    with tempfile.TemporaryDirectory(prefix="p343-ap-identity-") as temporary:
        work = Path(temporary)
        ...
        image_result = validate_image(image, expected_run_id=P343_RUN_ID)
    ...
    init_result = _validate_init(init, P343_RUN_ID)
...
def validate_rollback_ap(
    ...
    *,
    label: str = "P343 exact rollback AP",
) -> dict[str, Any]:
...
def validate_p343_identity() -> dict[str, Any]:
    return {
        "schema": "s22plus_fyg8_p343_artifact_identity_v1",
        "run_id_hex": P343_RUN_ID_HEX,
        "predecessor_run_id_rejected": P342_PREDECESSOR_RUN_ID_HEX,
        "predecessor_ap_identity_rejected": dict(P342_AP_IDENTITY),
        "image_identity": dict(P343_IMAGE_IDENTITY),
        "ap_identity": dict(P343_AP_IDENTITY),
        "boot_only": True,
        "ab_must_match": True,
