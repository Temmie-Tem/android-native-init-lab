#!/usr/bin/env python3
"""Build V2277 workqueue execute_start stack + codeword sampler test boot.

This source/build wrapper keeps the V2237 Wi-Fi route and packages both the
workqueue execute_start stack sampler and the perf-regs/codeword sampler in the
...
OUT_DIR = workspace_private_build_path("native-init", "v2277-workqueue-exec-stack")
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2277_WORKQUEUE_EXEC_STACK_SOURCE_BUILD_2026-06-12.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2277_workqueue_exec_stack.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2277_workqueue_exec_stack"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2277_workqueue_exec_stack.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v433_workqueue_exec_stack"
BPF_HELPER_BINARY = OUT_DIR / "a90_bpf_workqueue_exec_stack_sample_ring"
BPF_HELPER_SOURCE = REPO_ROOT / "workspace/public/src/native-init/helpers/a90_bpf_workqueue_exec_stack_sample_ring.c"
BPF_HELPER_RAMDISK_PATH = "bin/a90_bpf_workqueue_exec_stack_sample_ring"
BPF_HELPER_RUNTIME_PATH = "/bin/a90_bpf_workqueue_exec_stack_sample_ring"
...
EXPECTED_HELPER_MARKER = "a90_android_execns_probe v433"
EXTRA_INIT_FLAGS = v2237.EXTRA_INIT_FLAGS
HELPER_MODE = v2237.HELPER_MODE
HELPER_RUNTIME_MODE = v2237.HELPER_RUNTIME_MODE
WORKQUEUE_LOG_PATH = "/cache/native-init-v2277-workqueue-exec-stack.log"
WORKQUEUE_DURATION_MS = 45000
WORKQUEUE_PRINT_LIMIT = 512
WORKQUEUE_WAIT_MS = 60000
TAIL_LOG_PATH = "/cache/native-init-v2277-tail-perf-regs-codeword.log"
...
def configure_base() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--init-build": "v2277-workqueue-exec-stack",
        "--out-dir": str(OUT_DIR),
        "--init-binary": str(INIT_BINARY),
        "--helper-binary": str(HELPER_BINARY),
        "--ramdisk-cpio": str(RAMDISK_CPIO),
        "--boot-image": str(BOOT_IMAGE),
        "--wifi-test-klog-prefix": "A90v2277",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2277.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2277.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2277.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2277-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2277.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2277-supervisor.pid",
...
def main() -> int:
    ...
    manifest["candidate_tag"] = "v2277-workqueue-exec-stack"
    ...
    live_candidate_path.write_text(json.dumps({
        "candidate_tag": "v2277-workqueue-exec-stack",
        ...
        "next_live_cycle": "V2278",
        "note": "V2277 keeps the V2237 WLAN route and adds read-only workqueue execute_start stack and codeword samplers around the post-FWREADY boot_wlan/firmware_class window.",
