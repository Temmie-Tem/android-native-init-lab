#!/usr/bin/env python3
"""Independently audit one deterministic P3.00 boot-only candidate."""
...
import build_s22plus_fyg8_p300_candidate as candidate
import s22plus_fyg8_p286_candidate_static_checker as base
import s22plus_fyg8_p300_build_repro_check as repro
import s22plus_fyg8_p300_candidate_contract as contract
...
SCHEMA = "s22plus_fyg8_p300_candidate_static_checker_v1"
VERDICT = "PASS_P300_INDEPENDENT_ARTIFACT_CLOSURE_HOST_ONLY"
TARGET = contract.TARGET
DEFAULT_CANDIDATE = candidate.DEFAULT_OUT
DEFAULT_CANDIDATE_B = Path("workspace/private/outputs/s22plus_fyg8_p300/candidate-b")
...
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p300/static-check-result.json"
)
...
def verify_repro(root: Path, args, exact_contract):  # noqa: ANN001, ANN201
    ...
    if (
        ...
        raise CheckError(
            "P3.00 reproducibility result differs from fresh verification"
        )
