#!/usr/bin/env python3
"""P3.00 post-build proof with linked event-ingress IRQ closure."""
...
import s22plus_fyg8_p298_postbuild_linked_audit as inherited
...
IMPLEMENTATION_ID = "s22plus-fyg8-p300-linked-event-ingress-irq-v1"
...
def host_validator_tu(patch: bytes) -> bytes:
    ...
    if source.count(assignment) != 1 or source.count(b"request.detail = 0;") != 1:
        raise AuditError("P3.00 host validator detail injection differs")
...
def host_native_exhaustive(root: Path) -> dict[str, Any]:
    patch = p300.generate(root)["patch"]
    result = run_host_validator_tu(host_validator_tu(patch))
    result["identity_patch"] = p300.receipt(patch)
    result["production_validator_source"] = p300.receipt(
...
def linked_table_data(args, result: dict[str, Any]) -> dict[str, Any]:  # noqa: ANN001
    ...
    vmlinux = repro.candidate_contract.stable_read(
        directory / "vmlinux",
        "P3.00 direct-ELF linked vmlinux",
    ...
    if receipt != expected_receipt:
        raise AuditError("P3.00 linked vmlinux changed after reproducibility audit")
    proof = verify_linked_table_data(vmlinux, p300.linked_table_bytes())
...
def _disassemble_symbol(tool: Path, vmlinux: Path, symbol: str) -> str:
    ...
    if completed.returncode != 0 or completed.stderr:
        raise AuditError(
            f"P3.00 A/B disassembly failed for {symbol}: "
...
def check(args):  # noqa: ANN001, ANN201
    ...
    if proof.get("verdict") != closure.VERDICT or proof.get("verified") is not True:
        raise AuditError("P3.00 telemetry postbuild closure differs")
    linked_result = result["linked_audit"]
    linked_result["postbuild_audit"]["event_ingress_irq_telemetry"] = proof
    linked_result["postbuild_audit"]["full_lto_p300_probe_callsites"] = (
        callsite_pair
    )
