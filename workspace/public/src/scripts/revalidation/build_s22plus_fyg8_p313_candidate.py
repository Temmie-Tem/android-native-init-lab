#!/usr/bin/env python3
"""Build a P3.13 fixed-P3.10-Image boot-only candidate."""
...
import s22plus_fyg8_p313_candidate_contract as candidate_contract
import s22plus_fyg8_p313_e2_stock_closure as closure
import s22plus_fyg8_p313_overlay_contract as contract
import s22plus_fyg8_p313_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p313_candidate_artifact_result_v1"
VERDICT = "PASS_P313_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p313/candidate-a")
...
def verify_repro_result(
    ...
    **_ignored,
    ...
    if result_path != root / contract.PARENT_REPRO_RESULT:
        raise BuildError("P3.13 P3.10 closure path differs")
    value, result_receipt = _read_json(result_path, "P3.13 P3.10 independent closure")
    if (
        ...
        or value.get("candidate_contract")
        != exact_contract.get("parent_candidate_contract")
    ):
        raise BuildError("P3.13 fixed P3.10 closure differs")
    return {
...
def artifact_safety(exact_contract: dict) -> dict:
    ...
    result.update({
        ...
        "p313_kernel_rebuild": False,
        "p313_full_lto_ab": False,
        "p313_observer": exact_contract["observer"],
        "p313_cross_gate_audit": exact_contract["cross_gate_audit"],
        "p313_hazard_closure": exact_contract["hazard_closure"],
        "p313_tracefs_abi": exact_contract["tracefs_abi"],
