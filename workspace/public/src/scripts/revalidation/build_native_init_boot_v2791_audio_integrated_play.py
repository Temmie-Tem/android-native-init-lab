#!/usr/bin/env python3
"""Build V2791 native-init integrated audio play boot image.

V2791 compiles the integrated native `audio play --execute` path.
...
CYCLE = "V2791"
INIT_VERSION = "0.9.304"
INIT_BUILD = "v2791-audio-integrated-play"
BUILD_TAG = "v2791-audio-integrated-play"
DECISION = "v2791-audio-integrated-play-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2791_AUDIO_INTEGRATED_PLAY_SOURCE_BUILD_2026-06-19.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2791_audio_integrated_play.img", legacy_fallback=False
)
BASE_BOOT = workspace_private_input_path(
    "boot_images", "boot_linux_v2334_audio_snd_nodes_preflight.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2791_audio_integrated_play"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2791_audio_integrated_play.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v444_audio_integrated_play"
...
def configure() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--wifi-test-klog-prefix": "A90v2791",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2791.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2791.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2791.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2791-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2791.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2791-supervisor.pid",
    }
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    return "\n".join([
        "# Native Init V2791 Audio Integrated Play Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "## Purpose",
        "",
        "- Compiles the V2791 source change that turns `audio play --execute` into the integrated native path: ADSP boot, `/dev/snd` materialization, App Type Config, ACDB SET hold, route apply, bounded PCM, route reset, and SET deallocate.",
        ...
        "- The next live unit must flash this image, deploy the private SET-cal manifest bundle, run the integrated native `audio play` path, confirm audible sound, and rollback to `v2321`.",
        "",
        "## Metadata",
        "",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "parent_baseline": "v2790-audio-setcal-execute",
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "deeper_fallback_baseline": "v2237-supplicant-terminate-poll",
        "audio_integrated_play_source_validation": {
            "play_execute_supported": True,
            "integrated_play_executor_compiled": True,
            ...
            "integrated_sequence": ["adsp", "snd", "app_type", "setcal_hold", "route_core", "pcm", "route_core_reset", "setcal_deallocate"],
    ...
    (OUT_DIR / "promotion-candidate.json").write_text(json.dumps({
        ...
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "note": "V2791 is a test image for integrated native audio playback, not a promoted rollback baseline.",
