#!/usr/bin/env python3
"""V2423 host-only hybrid thread-set clone-following ACDB payload observer planner.

This unit is source/build/test only. It prepares the next Android/Magisk-root M0
observer after V2422 proved process-main-TID clone following is insufficient.
The helper attaches every existing TID in each target process before playback
and enables PTRACE_O_TRACECLONE on every traced TID. It does not boot Android,
...
RUN_ID = "V2423"
BUILD_TAG = "v2423-audio-acdb-threadset-clone-follow-observer"
ROOT = v2415.ROOT
HELPER_SOURCE = ROOT / "workspace/public/src/android/acdb_payload_capture/a90_acdb_ioctl_capture_threadset_v2423.c"
DEFAULT_HELPER_OUT_DIR = ROOT / "workspace/private/builds/audio/v2423-acdb-threadset-clone-follow-helper"
DEFAULT_CC = v2415.DEFAULT_CC
DEFAULT_REMOTE_DIR = "/data/local/tmp/a90-audio-acdb-v2423"
REMOTE_HELPER = f"{DEFAULT_REMOTE_DIR}/a90_acdb_ioctl_capture_threadset_v2423"
REMOTE_SCRIPT = f"{DEFAULT_REMOTE_DIR}/a90_acdb_threadset_clone_follow_capture.sh"
REMOTE_ARTIFACT_DIR = f"{DEFAULT_REMOTE_DIR}/artifacts"
DEFAULT_DURATION_SEC = 8
DEFAULT_MAX_BYTES = 512
DEFAULT_PROCESS_POLL_SEC = 0.2
APPROVAL_PHRASE = (
    "AUD-5F-acdb-threadset-clone-follow-capture go: rollbackable Android AudioTrack speaker "
    "msm_audio_cal ioctl payload capture with hybrid thread-set clone-following ptrace observer, "
...
def source_state() -> dict[str, Any]:
    ...
    if HELPER_SOURCE.exists():
        ...
        state.update({
            ...
            "contains_task_enumeration": "opendir(path)" in text and "/proc/%ld/task" in text,
            "contains_tgid_option": "--tgid" in text,
...
def build_capture_helper(out_dir: Path, *, cc: str = DEFAULT_CC) -> dict[str, Any]:
    ...
    output = out_dir / "a90_acdb_ioctl_capture_threadset_v2423"
...
set -eu
OUT="${{A90_V2423_OUT:-{REMOTE_ARTIFACT_DIR}}}"
HELPER="${{A90_V2423_HELPER:-{REMOTE_HELPER}}}"
DURATION="${{A90_V2423_DURATION:-{duration_sec}}}"
MAX_BYTES="${{A90_V2423_MAX_BYTES:-{max_bytes}}}"
PROCESS_POLL_SEC="${{A90_V2423_PROCESS_POLL_SEC:-{process_poll_sec}}}"
mkdir -p "$OUT"
chmod 700 "$OUT" 2>/dev/null || true
START_TS="$(date +%s)"
END_TS="$((START_TS + DURATION))"
echo "A90_V2423_CAPTURE_BEGIN duration=$DURATION max_bytes=$MAX_BYTES process_poll_sec=$PROCESS_POLL_SEC mode=threadset-clone-following" > "$OUT/capture-controller.log"
...
start_helper_for_pid() {{
  ...
  echo "A90_V2423_HELPER_START tgid=$pid remaining=$remaining mode=threadset-clone-following" >> "$OUT/capture-controller.log"
  "$HELPER" --tgid "$pid" --fd-pid "$pid" --out "$OUT/msm-audio-cal-threadset-p${{pid}}.jsonl" --duration-sec "$remaining" --max-bytes "$MAX_BYTES" >> "$OUT/capture-controller.log" 2>&1 &
  helper_pid="$!"
  echo "$helper_pid $pid threadset-clone-following" >> "$OUT/helper-pids.txt"
...
echo "A90_V2423_CAPTURE_END" >> "$OUT/capture-controller.log"
exit 0
'''
...
def materialize_capture_bundle(out_dir: Path, *, cc: str, duration_sec: int, max_bytes: int, process_poll_sec: float) -> dict[str, Any]:
    out_dir.mkdir(parents=True, exist_ok=True)
    os.chmod(out_dir, 0o700)
    script_path = out_dir / "a90_acdb_threadset_clone_follow_capture.sh"
...
def helper_bundle_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.materialize_capture_helper:
        ...
        binary = args.helper_out_dir / "a90_acdb_ioctl_capture_threadset_v2423"
        script = args.helper_out_dir / "a90_acdb_threadset_clone_follow_capture.sh"
...
def stage_commands(args: argparse.Namespace, bundle: dict[str, Any]) -> list[list[str]]:
    ...
    binary_path = (
        ...
        or rel(args.helper_out_dir / "a90_acdb_ioctl_capture_threadset_v2423")
    )
    script_path = (
        build_manifest.get("controller_script", {}).get("path")
        or rel(args.helper_out_dir / "a90_acdb_threadset_clone_follow_capture.sh")
    )
    return [
...
def capture_start_command(args: argparse.Namespace) -> list[str]:
    env = (
        f"A90_V2423_OUT={REMOTE_ARTIFACT_DIR} A90_V2423_DURATION={args.capture_duration_sec} "
        f"A90_V2423_MAX_BYTES={args.max_bytes} A90_V2423_PROCESS_POLL_SEC={args.process_poll_sec}"
    )
...
def command_safety(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_observer_tokens": ["/proc/<tgid>/task enumeration", "PTRACE_ATTACH per TID", "PTRACE_O_TRACECLONE", "PTRACE_GETEVENTMSG", "process_vm_readv", "/dev/msm_audio_cal fd match"],
...
def magisk_strategy() -> dict[str, Any]:
    ...
    return {
        ...
        "default_tier": "M0-threadset-clone-following-transient-helper",
        ...
        "single_observer_semantics": "M0 and any future M1 use the same hybrid thread-set clone-following ptrace observer; M1 changes delivery timing only, not capture semantics",
        "tiers": [
            {
                "tier": "M0-threadset-clone-following-transient-helper",
                "default": True,
                "mechanism": "stage one ptrace observer per audio process under /data/local/tmp; attach every existing TID before playback and follow new worker TIDs with PTRACE_O_TRACECLONE",
                "addresses_v2422": "covers existing HAL worker pools and stops newly-created worker threads at birth",
            ...
            },
            {
                ...
                "default": False,
                "mechanism": "temporary Magisk post-fs-data.sh/service.sh packaging of the same hybrid thread-set clone-following observer",
                "gate": "new exact approval and separate V-iteration only if hybrid thread-set M0 cannot attach early enough after Android handoff or must exist before the audio HAL process or worker pool starts",
                ...
                "package_shape": {
                    ...
                    "service_sh": "wait for target audio process, exec hybrid thread-set clone-following helper, write JSONL under /data/local/tmp or /data/adb/a90-acdb private scratch",
            ...
            },
            {
                ...
                "gate": "defer unless both hybrid thread-set M0 and M1 fail to expose one identified payload edge",
            },
        ],
    }
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    v2422_report = file_state(ROOT / "docs/reports/NATIVE_INIT_V2422_AUDIO_ACDB_CLONE_FOLLOW_LIVE_2026-06-15.md", require_private=False)
    ...
    commands = {
        ...
        "stage_threadset_clone_follow_helper_and_stimulus": stage_commands(args, bundle),
    ...
    plan: dict[str, Any] = {
        ...
        "host_only": True,
        "device_action": "none",
        "future_live_run_id": "V2424",
        "approval_phrase_required_for_future_live": APPROVAL_PHRASE,
        "v2422_boundary_report": v2422_report,
        ...
        "capture_contract": {
            ...
            "watcher": {
                ...
                "process_poll_env": "A90_V2423_PROCESS_POLL_SEC",
        ...
        "hard_boundary": [
            "V2423 is host-only",
            "future V2424 live capture is Android/Magisk measurement only",
    ...
    plan["ok"] = bool(v2422_report.get("ok") and source_state().get("ok") and safety.get("ok"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="emit the V2423 host-only plan; no device action")
