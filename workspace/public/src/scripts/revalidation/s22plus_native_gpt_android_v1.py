...
def metadata(stdout,sealed,kind):
    ...
    user=gpt.NEW_USER if kind=='proposed' else gpt.OLD_USER
...
def storage_stat(text,basis):
    lines=text.splitlines();require(len(lines)==2,'Android storage-stat row count differs')
    if basis['layout']=='proposed':
        require(lines[0]==f'NATIVE_PARTITION 41 {28750592*8} {gpt.NATIVE_SECTORS} native_data',
    ...
    else:require(blocks*block<=gpt.OLD_USER*512,'original Android filesystem exceeds original userdata')
    return dict(status='PASS_ANDROID_REPORTED_CAPACITY',block_size=block,total_blocks=blocks,
        ...
        native_partition_present=basis['layout']=='proposed')
...
def projection(folder,adapter,request):
    ...
    proof=metadata(raw.read_stdout(capture,maximum=65536),gpt.vectors(request['N']['gpt']),basis['layout'])
    ...
    storage=storage_stat(text,basis)
...
def observe(adapter,step,request,*,guard):
    ...
    before.wait_ready(deadline_ns=clock()+180_000_000_000);before.health()
