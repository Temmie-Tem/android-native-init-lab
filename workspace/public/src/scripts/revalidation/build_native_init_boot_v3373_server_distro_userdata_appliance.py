#!/usr/bin/env python3
"""Build V3373 native-init boot image for the server-distro D4B userdata appliance surface."""
...
CYCLE = "V3373"
INIT_VERSION = "0.11.134"
INIT_BUILD = "v3373-server-distro-userdata-appliance"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3373_SERVER_DISTRO_D4B_USERDATA_APPLIANCE_SOURCE_BUILD_2026-07-03.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3373_server_distro_userdata_appliance.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3373_server_distro_userdata_appliance"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3373_server_distro_userdata_appliance.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v629_server_distro_switchroot"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3373"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3373.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3373.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3373"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3373-server-distro-userdata-appliance"

FRAME_PATH = "/tmp/a90-doomgeneric-v3373-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3373-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3373-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3373-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3373-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3373-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3373-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3373.c"
...
def _rewrite_v3373_text(text: str) -> str:
    ...
    replacements = (
        ...
        ("hot-reload-autohud", "server-distro-userdata-appliance"),
        ("v3368", "v3373"),
        ("V3368", "V3373"),
        ("a90-doomgeneric-v3368", "a90-doomgeneric-v3373"),
        ("a90.doomgeneric.v3368", "a90.doomgeneric.v3373"),
    )
...
def _rewrite_v3373_bytes(item: bytes) -> bytes:
    return _rewrite_v3373_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3373_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3373_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3373_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3373_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3373_bytes(marker) for marker in previous.REQUIRED_STRINGS)

REQUIRED_STRINGS = PREVIOUS_REQUIRED_STRINGS + (
    b"0.11.134",
    b"v3373-server-distro-userdata-appliance",
...
def _boot_audit_manifest() -> dict[str, Any]:
    ...
    manifest["rung"] = "server-distro-d4b-userdata-appliance"
    manifest["scope"] = "D4B-fail-closed-userdata-appliance-native-init-surface"
    manifest["server_distro_contract"] = {
        ...
        "formatter_policy": "destructive format uses token-gated busybox mke2fs -t ext4 path; D4C may enter only after the formatter is device-proven",
...
def v3373_adapter_source() -> str:
    return _rewrite_v3373_text(ORIG_PREVIOUS_ADAPTER_SOURCE())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "server-distro-userdata-appliance.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "server-distro-d4b-userdata-appliance",
...
def _normalize_manifest_for_v3373(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest.update({
        "decision": DECISION,
        "cycle": CYCLE,
        "candidate_tag": INIT_BUILD,
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest = _normalize_manifest_for_v3373(json.loads(manifest_path.read_text(encoding="utf-8")))
    if base_main_error:
...
def _postprocess_manifest() -> dict[str, Any]:
    manifest = _normalize_manifest_for_v3373(ORIG_PREVIOUS_POSTPROCESS())
    manifest_path = OUT_DIR / "manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_PATH.write_text(
        render_report(
...
def _overlay_preserved_v3373_ramdisk() -> dict[str, Any]:
    overlay = ORIG_PREVIOUS_OVERLAY()
    overlay["mode"] = "preserve-v3335-ramdisk-overlay-v3373-init-helper-engine"
    return overlay


def _patch_v3368_module_for_v3373() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "v3368_adapter_source": v3373_adapter_source,
        "_rewrite_v3368_text": _rewrite_v3373_text,
        "_rewrite_v3368_bytes": _rewrite_v3373_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_overlay_preserved_v3368_ramdisk": _overlay_preserved_v3373_ramdisk,
