#!/usr/bin/env python3
"""Build V3094 native-init DOOM 1:1 scale comparison candidate."""
...
CYCLE = "V3094"
INIT_VERSION = "0.10.103"
INIT_BUILD = "v3094-doomgeneric-scale-1to1"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3094_DOOMGENERIC_SCALE_1TO1_SOURCE_BUILD_2026-06-23.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v3094_doomgeneric_scale_1to1.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v3094_doomgeneric_scale_1to1"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3094_doomgeneric_scale_1to1.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v515_doomgeneric_scale_1to1"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3094"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3094.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3094.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3094"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3094-scale-1to1"
...
FRAME_PATH = "/tmp/a90-doomgeneric-v3094-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3094-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3094-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3094-input.sock"
INPUT_UDP_PORT = v3086.INPUT_UDP_PORT
DEVICE_NCM_HOST = v3086.DEVICE_NCM_HOST
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3094-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3094-tick-telemetry.txt"
...
TICK_TELEMETRY_MARKER = "a90.doomgeneric.v3094.tick_telemetry=scale-1to1-fake-time-summary"
SCALE_1TO1_MARKER = "a90.doomgeneric.v3094.scale=large-frame-off-1to1"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.103 (v3094-doomgeneric-scale-1to1)",
    b"v3094-doomgeneric-scale-1to1",
    b"doomgeneric-private-link-v3094-scale-1to1",
    b"/bin/a90_doomgeneric_private_engine_v3094",
...
def _replace_required(source: str, old: str, new: str) -> str:
    if old not in source:
        raise RuntimeError(f"missing V3094 adapter source anchor: {old[:96]!r}")
    return source.replace(old, new, 1)


def v3094_adapter_source() -> str:
    ...
    source = _replace_required(
        ...
        'const char a90_doomgeneric_v3081_frame_ipc_policy[] =\n'
        '    "a90.doomgeneric.v3081.frame_ipc=shared-mmap-seq";\n'
        'const char a90_doomgeneric_v3094_tick_telemetry_policy[] =\n'
        f'    "{TICK_TELEMETRY_MARKER}";\n'
        'const char a90_doomgeneric_v3094_scale_policy[] =\n'
        f'    "{SCALE_1TO1_MARKER}";\n',
    )
    source = _replace_required(
        source,
    ...
    source = _replace_required(
        ...
        "        marker_checksum(a90_doomgeneric_v3079_pace_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3081_frame_ipc_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3094_tick_telemetry_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3094_scale_policy) == 0U) {\n",
    )
    source = _replace_required(
        source,
...
def apply_v3094_globals() -> None:
    ...
    v3086.v3084.v3083.v3081.v3081_adapter_source = v3094_adapter_source
    v3086.apply_v3086_globals()
    _set_large_frame(NATIVE_DASHBOARD_LARGE_FRAME)
    V3059.v3059_adapter_source = v3094_adapter_source
...
def render_report(
    ...
    return "\n".join([
        ...
        *marker_lines,
        ...
        f"- Scope: flash exact V3094 boot image via `native_init_flash.py`, health-check, require `large_frame=0` and `frame_scale=1:1`, run bounded DOOM loop, then read `{TICK_TELEMETRY_PATH}` to compare scaling-off frame timing with V3091.",
        "",
        "## Metadata",
        "",
...
def main() -> int:
    ...
    doom.update({
        ...
        "cadence_experiment": "scale-1to1-vs-large-fastscale",
        "tick_telemetry_marker": TICK_TELEMETRY_MARKER,
        "scale_1to1_marker": SCALE_1TO1_MARKER,
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-scale-1to1-candidate",
        "adoption_state": "pending-scale-1to1-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-scale-1to1-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-scale-1to1-candidate",
