#!/usr/bin/env python3
"""Verify P3.14 against its exact frozen P3.13 parent."""
...
def verify(root: Path, source: Path, intent_path: Path, patch_path: Path):
    ...
    if source != root / overlay.PARENT_SOURCE:
        raise ContractError("P3.14 parent source path differs")
    if patch_path != root / overlay.PARENT_PATCH:
        raise ContractError("P3.14 parent patch path differs")
