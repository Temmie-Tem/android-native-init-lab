#!/usr/bin/env python3
"""Build V2921 native-init candidate with the DEMO > Bad Apple menu entry."""
...
CYCLE = "V2921"
INIT_VERSION = "0.10.39"
INIT_BUILD = "v2921-demo-menu-badapple"
BUILD_TAG = INIT_BUILD
DECISION = "v2921-demo-menu-badapple-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2921_DEMO_MENU_BADAPPLE_SOURCE_BUILD_2026-06-20.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2921_demo_menu_badapple.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2921_demo_menu_badapple"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2921_demo_menu_badapple.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v467_demo_menu_badapple"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.39 (v2921-demo-menu-badapple)",
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2921 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    return "\n".join([
        "# Native Init V2921 DEMO Bad Apple Menu Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "- Parent live unit: V2920 proved the Bad Apple Player HUD A/V path; V2921 adds the bounded `DEMO > Bad Apple` menu launcher over the existing SD SHA-addressed cache.",
        "",
        "## Included Delta",
        "",
        "- Packages the `DEMO > Bad Apple` bounded Player HUD menu launcher into a flashable test image.",
        "- Keeps the full Bad Apple stream and PCM outside the boot image; boot carries only player/HUD code.",
        "- Adds the `DEMO > Bad Apple` menu entry as a bounded 120-frame video-only Player HUD preview that restores the menu after completion.",
        ...
        *marker_lines,
        ...
        "## Validation",
        "",
        "- `py_compile`: V2921 builder.",
        "- Build: AArch64 static native-init compile, helper compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V2921 identity, real Bad Apple preset SHA, player-hud layout strings, read-only telemetry HUD strings, and retained pageflip/mono1 markers.",
        "- Device validation is deferred to V2922: flash this exact image, run `version`/`status`/`selftest`, inspect `video status`, then exercise the bounded menu preview if cache is present.",
        "",
        ...
        "- Candidate type: `demo-menu-badapple-candidate`.",
        "",
    ])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "demo-menu-badapple-candidate",
        "parent_test_artifact": "v2920-badapple-player-hud-av-live",
        "badapple_player_hud": {
            "version": 1,
            "source_unit": "V2921",
            "parent_unit": "V2920",
            ...
            "commands": [
                ...
                "DEMO > BAD APPLE HUD menu preview: video demo badapple play --trust-cache --frames 120 --present pageflip --layout player-hud",
            ],
    ...
    (OUT_DIR / "demo-menu-badapple-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "demo-menu-badapple-candidate",
