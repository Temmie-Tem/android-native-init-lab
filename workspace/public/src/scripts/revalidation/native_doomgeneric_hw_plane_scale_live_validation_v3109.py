#!/usr/bin/env python3
"""V3109 rollback-gated live validation for V3108 DOOM hardware plane scale."""
...
RUN_ID = "V3109"
BUILD_TAG = "v3109-doomgeneric-hw-plane-scale-live"
DECISION_PREFIX = "v3109-doomgeneric-hw-plane-scale"
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V3109_DOOMGENERIC_HW_PLANE_SCALE_LIVE_2026-06-23.md"

CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v3108_doomgeneric_hw_plane_scale.img"
CANDIDATE_VERSION = "0.10.109"
CANDIDATE_TAG = "v3108-doomgeneric-hw-plane-scale"
CANDIDATE_SHA256 = "58affe427e1f9417f7c89f539528a3f693f5f38ae47ed3fae16124fc64055001"
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not result["pass"]:
            raise RuntimeError(f"V3108 hardware plane scale live validation did not classify cleanly: {classification}")
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"flash exact V3108 image {CANDIDATE_IMAGE}",
            ...
            "parse hw_plane.presented/fallback, timing, seq, and pageflip markers",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--live", action="store_true", help="flash V3108, run bounded DOOM loop, then rollback")
