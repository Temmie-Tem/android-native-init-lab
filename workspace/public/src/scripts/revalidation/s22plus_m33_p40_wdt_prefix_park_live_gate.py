#!/usr/bin/env python3
"""Guarded S22+ M33 P40 watchdog-prefix park native-init live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M33 P40 is not a Download beacon and does not expose USB ACM. It loads the
watchdog-managed full-M32-module-closure-without-configfs prefix, parks, and
lets the host/operator decide whether that module boundary survives past the
PMIC/PON reset window.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M33-P40-WDT-PREFIX-PARK-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M33-P40-WDT-PREFIX-PARK-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_VARIANT = "P40"
EXPECTED_PREFIX_COUNT = 40
EXPECTED_M33_MARKER = "S22_NATIVE_INIT_M33_WDT_PREFIX_PARK_P40"
EXPECTED_M33_AP_SHA256 = "420986c447df5cd155aee1ea32ece8ec5a7b021793dd9058d4fe6bc3744b7c34"
EXPECTED_M33_BOOT_SHA256 = "b07bbc97a36f63c92db915829b322ef8200ceb5944a244b0a8406780b46a9621"
EXPECTED_M33_INIT_SHA256 = "d2500dcf739d5d72c705ff24edbe6332c5a3d51defe08651c6cd517f14607274"
EXPECTED_M33_MODULE_LIST_SHA256 = "2291dc1c72add131c42d0b4ed6649880c20316d0598e0a2af942cc774949062c"
EXPECTED_M33_SOURCE_SHA256 = "cfd3bb3f510a53d07a1e9f21846953ff13ecf83680debf0aa839c79507eb50da"
...
EXPECTED_MODULE_ENTRY = "s22plus_m33_p40_wdt_prefix_park.modules"
...
DEFAULT_M33_AP = Path("workspace/private/outputs/s22plus_native_init/m33_wdt_prefix_park_matrix_v0_1/P40/odin4/AP.tar.md5")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m33_p40_wdt_prefix_park_live_gate_{utc_stamp()}")
...
def policy_required_markers() -> list[str]:
    return [
        "S22+ M33 P40 watchdog-prefix park native-init boot-only",
        ...
        "prefix_targets=40",
        "module_load_only=1",
        "full-M32-module-closure-without-configfs prefix",
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M33 P40 watchdog-prefix authorization markers: {missing}")


def find_p40_variant(data: dict[str, Any]) -> dict[str, Any]:
    for variant in data.get("variants", []):
        if variant.get("label") == EXPECTED_VARIANT:
            return variant
    raise SystemExit("M33 manifest does not contain P40 variant")
...
def verify_m33_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m33_p40_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m33_p40_manifest_closure={json.dumps(closure, sort_keys=True)}")
    append_log(log_path, f"m33_p40_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")
    append_log(log_path, f"m33_p40_manifest_init_file={init_info.get('file', '')}")
    ...
    if variant.get("prefix_count") != EXPECTED_PREFIX_COUNT:
        raise SystemExit(f"M33 P40 prefix count mismatch: {variant.get('prefix_count')!r}")
    required_hashes = {
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M33 P40 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    ...
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M33 P40 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    if closure.get("modules") != EXPECTED_MODULES:
        raise SystemExit(f"M33 P40 module closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M33 P40 module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M33_MODULE_LIST_SHA256:
        raise SystemExit(f"M33 P40 module list SHA mismatch: {closure.get('module_sha256')!r}")
    if any(module in closure.get("modules", []) for module in ("phy-msm-ssusb-qmp.ko", "eud.ko")):
        raise SystemExit("M33 P40 excluded QMP/EUD module leaked into closure")
    ...
    for key, expected in expected_boundaries.items():
        if boundaries.get(key) is not expected:
            raise SystemExit(f"M33 P40 boundary {key} mismatch: {boundaries.get(key)!r} != {expected!r}")
    for key in (
        "includes_arm_smmu",
        "includes_hs_phy",
        "includes_eusb2_phy",
        ...
        if key not in boundaries:
            raise SystemExit(f"M33 P40 boundary {key} missing")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M33 P40 manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M33 P40 module list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M33 P40 must not inject module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 1:
        raise SystemExit("M33 P40 must inject exactly one module-list text file into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M33 P40 required string missing from manifest: {required}")

    objdump_path = path.parent / EXPECTED_VARIANT / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M33 P40 objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M33 P40 /init does not load arm64 __NR_finit_module (273)")
    if any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M33 P40 /init must not load arm64 __NR_reboot (142)")
...
def verify_m33_artifacts(
    *,
    ...
    verify_ap(m33_ap, EXPECTED_M33_AP_SHA256, "m33_p40_candidate", log_path)
...
def observe_park_survival(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m33_p40_observe_start_utc={utc_now()}")
    append_log(log_path, f"m33_p40_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m33_p40_park_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m33_p40_result=unexpected_odin_before_survival_window elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "unexpected-odin-before-survival-window", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M33 P40 observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m33_p40_result=unexpected_adb_before_survival_window elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-survival-window", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m33_p40_survival_window_pass=1")
    append_log(log_path, "m33_p40_result=survived-observation-window-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m33_p40_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m33_p40_{label}_boot_restore_hash_check=skipped rollback_target=stock")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m33_p40_{label}_rollback", android, marker=EXPECTED_M33_MARKER)
    append_log(log_path, f"m33_p40_{label}_retained_marker_found={int(marker_found)}")
    return (0, android)
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m33-p40-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M33 P40 rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    if args.observe_sec < 60:
        raise SystemExit("--observe-sec must be at least 60 for the M33 P40 watchdog-prefix discriminator")
    if args.snapshot_interval_sec < 1.0:
        raise SystemExit("--snapshot-interval-sec must be at least 1.0")

    root = repo_root()
    run_dir = resolve_run_dir(root, args.run_dir)
    log_path = run_dir / "s22plus_m33_p40_wdt_prefix_park_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M33 P40 watchdog-prefix park live gate ===")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M33 P40 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M33 P40 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M33 P40 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M33 P40 live gate starting. This candidate should park; do not press keys during "
    ...
    if odin_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print("download mode did not appear for M33 P40 candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        record_timeline_event(run_dir, "live_session_end")
        print(f"M33 P40 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m33_p40_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M33 P40 candidate flashed. Observing for {args.observe_sec}s survival window.", flush=True)
    ...
    if result != "survived-observation-window-manual-download-required":
        if rollback_device is None:
            record_timeline_event(run_dir, "live_session_end")
            print(
                f"M33 P40 stopped before survival proof ({result}). If the device is not in Android, "
    ...
    print(
        "M33 P40 survived the observation window. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print(
            f"M33 P40 survival window passed, but manual Download mode did not appear. "
    ...
    record_timeline_event(run_dir, "live_session_end")
    print(f"M33 P40 live gate completed rc={rc}; result={result}; log={log_path}")
    return rc
