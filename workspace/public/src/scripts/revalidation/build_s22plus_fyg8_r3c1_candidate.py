#!/usr/bin/env python3
"""Build the host-only FYG8 R3C1 unpatched rebuilt-kernel candidate.

The builder starts from the exact, live-proven R3C0 raw boot bytes and replaces
only the fixed kernel region with the exact R2 Full-LTO Image. It preserves the
...
SCHEMA = "s22plus_fyg8_r3c1_candidate_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_r3c1_candidate/reproduction-a")
...
def build_candidate_bytes(control: bytes, r2_image: bytes) -> bytes:
    if len(control) != BOOT_SIZE:
        raise BuildError(f"R3C0 boot size mismatch: {len(control)}")
    if len(r2_image) != KERNEL_SIZE:
        raise BuildError(f"R2 Image size mismatch: {len(r2_image)}")
    ...
    if parse_arm64_header(r2_image) != parse_arm64_header(control_kernel):
        raise BuildError("R2 ARM64 Image header differs from R3C0 kernel header")
    candidate = bytearray(control)
    candidate[KERNEL_START:KERNEL_END] = r2_image
...
def changed_summary(before: bytes, after: bytes) -> dict[str, int]:
    ...
    if first is None or last is None:
        raise BuildError("R3C1 unexpectedly equals R3C0")
    if outside:
        raise BuildError(f"R3C1 changed {outside} bytes outside kernel region")
    return {
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    candidate = build_candidate_bytes(control, r2_image)
    difference = changed_summary(control, candidate)

    if sha256_bytes(candidate[KERNEL_START:KERNEL_END]) != EXPECTED_R2_IMAGE_SHA256:
        raise BuildError("candidate kernel does not equal exact R2 Image")
    ...
    with tempfile.TemporaryDirectory(prefix=f".{out.name}.", dir=out.parent) as temporary:
        ...
        if boot_path.read_bytes() != candidate:
            raise BuildError("staged R3C1 bytes changed after write")

        boot_lz4 = odin_dir / "boot.img.lz4"
        require_ok(
            run([lz4, "--content-size", "-B6", "-f", "-q", boot_path, boot_lz4]),
            "LZ4 compression",
        )
        ...
        manifest = {
            "schema": SCHEMA,
            "target": TARGET,
            "purpose": "R3C1 exact R2 unpatched rebuilt-kernel candidate in live-proven R3C0 carrier",
            "inputs": input_pins,
            "construction": {
                ...
                "kernel_equals_exact_r2_image": hashes["kernel"] == EXPECTED_R2_IMAGE_SHA256,
                ...
                "arm64_header_exact_r3c0_match": parse_arm64_header(r2_image)
            ...
            "verdict": "PASS_R3C1_ARTIFACT_BUILT_HOST_ONLY",
        }
