#!/usr/bin/env python3
"""One-use attended corrected-order TWRP-fastbootd Q2 census for exact SM-G986N T2."""
...
import s20plus_g986n_twrp_fastbootd_prep_q1 as q1
import s20plus_g986n_twrp_t2_profile_h0 as t2_profile


VERSION = "s20plus-g986n-twrp-fastbootd-census-q2-v1"
SCHEMA = "s20plus_g986n_twrp_fastbootd_q2_census_result_v1"
PENDING_VERDICT = "PASS_S20PLUS_G986N_TWRP_FASTBOOTD_Q2_CENSUS_RETURN_PENDING"
FINAL_PASS = "PASS_S20PLUS_G986N_TWRP_FASTBOOTD_Q2_CENSUS_RETURN_HEALTHY"
FINAL_NO_PROOF = "NO_PROOF_S20PLUS_G986N_TWRP_FASTBOOTD_Q2_RETURNED_HEALTHY"

# Rotated only after independent review of the exact dormant closure.
LIVE_ACTIVE = True
EXPECTED_REVIEWED_NORMALIZED_SHA256 = "00d8853eb8d43a24424dffff5e5b5edbb9326c6cfad03b5e348a3996397ef328"
...
RUN_ROOT_REL = Path("workspace/private/runs/s20plus-g986n-twrp-fastbootd-census-q2")
SHARED_GUARD_REL = Path(
    "workspace/private/runs/s20plus-g986n-routine-actions/active-action.json"
)
CONSUMED_NAME = "consumed-enable-q2-intent.json"
MAX_COMMAND_BYTES = 32 * 1024
MAX_FASTBOOT_WAIT_SECONDS = 30.0
SHA256_RE = re.compile(r"[0-9a-f]{64}")
Q1_FINAL_REL = Path(
    "workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-q1/"
    "run-20260902T185938Z-1788375578765597442/final-result.json"
)
Q1_FINAL_SIZE = 1_273
Q1_FINAL_SHA256 = "afdabd50be11120dafb73793c08bbb8b78e69a108a029c13134f6825ba53cc53"
Q1_CONSUMED_REL = Path(
    "workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-q1/"
    "consumed-prep-q1-intent.json"
)
Q1_CONSUMED_SIZE = 654
Q1_CONSUMED_SHA256 = "360bffd3918aa5b693eef723e130009ff6199cbf3ac95add80e40d5990b68b9c"
Q1_RUNNER_SIZE = 42_079
Q1_RUNNER_SHA256 = "0f7d3c5ed460ce70ceb37aca0bc369f5ec7cb65d14695067f33c5f039a3fdc3f"
...
[ -d "$g/functions/ffs.fastboot" ] && [ ! -L "$g/functions/ffs.fastboot" ] || exit 121
/system/bin/mount -t functionfs -o rmode=0770,fmode=0660,uid=1000,gid=1000 fastboot /dev/usb-ffs/fastboot
/system/bin/grep -q ' /dev/usb-ffs/fastboot functionfs ' /proc/mounts || exit 122
...
printf '%s\n' enabled > /tmp/s20plus_g986n_fastbootd_enabled_q2_v1
"""
...
LAUNCH_SCRIPT = (
    ...
    + " </dev/null >/tmp/s20plus_g986n_fastbootd_enable_q2_v1.log 2>&1 &\n"
    "printf 'armed\\n'\n"
)
...
def validate_q1_predecessor(root: Path) -> dict[str, Any]:
    runner = file_receipt(
        Path(q1.__file__).resolve(), Q1_RUNNER_SIZE, Q1_RUNNER_SHA256, "Q1 runner"
    )
    terminal = file_receipt(
        root / Q1_FINAL_REL, Q1_FINAL_SIZE, Q1_FINAL_SHA256, "Q1 terminal"
    )
    consumed = file_receipt(
        root / Q1_CONSUMED_REL,
        Q1_CONSUMED_SIZE,
        Q1_CONSUMED_SHA256,
        "Q1 consumed marker",
    )
    run_dir = (root / Q1_FINAL_REL).parent
    prepared, proved = q1.validate_journal(root, run_dir)
    value = classic.load_json(root / Q1_FINAL_REL, "Q1 final result")
    q1.validate_final_result(run_dir, prepared, proved, value)
    returned = value.get("returned_android")
    if (
        value.get("verdict")
        != "PASS_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_RETURNED_HEALTHY"
        or value.get("prep_proved") is not True
        or value.get("replay_permitted") is not False
        or value.get("usb_role_switch_attempts") != 0
        or value.get("fastboot_commands") != 0
        or value.get("persistent_writes") != 0
        or value.get("partition_operations") != 0
        or not isinstance(returned, dict)
    ):
        raise FastbootdCensusError("Q1 terminal differs")
    return {
        "runner": runner,
        "terminal": terminal,
        "consumed": consumed,
        "target": value["target"],
        "returned_boot_id_sha256": returned["boot_id_sha256"],
    }
...
def require_support(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "q1_predecessor": validate_q1_predecessor(root),
        "preflight_script": {
...
def acquire_guard(root: Path, run_dir: Path) -> None:
    ...
    try:
        base.durable_write(
            path,
            {
                "schema": "s20plus_g986n_twrp_fastbootd_q2_guard_v1",
                "version": VERSION,
                "action": "attended-t2-volatile-fastbootd-census-q2",
                "run_dir": str(run_dir),
                "unresolved": True,
                "at": now(),
            },
...
def check_guard(root: Path, run_dir: Path) -> None:
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_guard_v1"
        or value.get("version") != VERSION
        or value.get("action") != "attended-t2-volatile-fastbootd-census-q2"
...
def create_enable_intent(root: Path, run_dir: Path, prepared: dict[str, Any]) -> None:
    intent = {
        "schema": "s20plus_g986n_twrp_fastbootd_q2_enable_intent_v1",
        "version": VERSION,
        "run_dir": str(run_dir),
...
def validate_enable_intent(
    ...
    if (
        ...
        != "s20plus_g986n_twrp_fastbootd_q2_enable_intent_v1"
...
def connected_census(
    ...
    q1_predecessor = support["q1_predecessor"]
    recovery = recovery_snapshot(
        command,
        predecessor,
        expected_other=q1_predecessor["target"]["other_serial_sha256"],
    )
    if (
        recovery["serial_sha256"] != q1_predecessor["target"]["serial_sha256"]
        or recovery["topology_sha256"]
        != q1_predecessor["target"]["topology_sha256"]
        or recovery["boot_id_sha256"] == q1_predecessor["returned_boot_id_sha256"]
    ):
        raise FastbootdCensusError("current recovery does not continue Q1")
    target = {
        "model": "SM-G986N",
        "device": "y2q",
        "product": "y2qksx",
    ...
    prepared = {
        "schema": "s20plus_g986n_twrp_fastbootd_q2_prepared_v1",
        "version": VERSION,
        "run_dir": str(run_dir),
        "target": target,
        ...
        "q1_predecessor_sha256": classic.object_sha256(q1_predecessor),
    ...
    base.durable_write(
        ...
        {
            "schema": "s20plus_g986n_twrp_fastbootd_q2_enable_result_v1",
            "version": VERSION,
            "armed": armed,
    ...
    base.durable_write(
        ...
        {
            "schema": "s20plus_g986n_twrp_fastbootd_q2_entry_observed_v1",
            "version": VERSION,
            "serial_sha256": recovery["serial_sha256"],
            "topology_sha256": recovery["topology_sha256"],
            "vid": endpoint["vid"],
    ...
    for ordinal, variable in enumerate(GETVARS, start=1):
        ...
        intent = {
            "schema": "s20plus_g986n_twrp_fastbootd_q2_getvar_intent_v1",
            "version": VERSION,
            "ordinal": ordinal,
            "variable": variable,
            "attempt": 1,
...
def validate_prepared(run_dir: Path, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        prepared.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_prepared_v1"
        ...
        or not _is_sha256(prepared.get("q1_predecessor_sha256"))
...
def validate_enable_result(run_dir: Path) -> bool:
    ...
    if (
        result.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_enable_result_v1"
...
def validate_entry_observation(run_dir: Path, target: dict[str, Any]) -> bool:
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_entry_observed_v1"
        or value.get("version") != VERSION
...
def validate_query_prefix(run_dir: Path, entry_present: bool) -> int:
    ...
    for ordinal, variable in enumerate(GETVARS, start=1):
        ...
        if (
            ...
            or intent.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_getvar_intent_v1"
            or intent.get("version") != VERSION
...
def validate_final_result(
    ...
    if (
        ...
        or result.get("schema") != "s20plus_g986n_twrp_fastbootd_q2_final_result_v1"
        or result.get("version") != VERSION
...
def abort_pre_effect(root: Path) -> dict[str, Any]:
    ...
    if lexists(prepared_path):
        ...
        prepared_sha256 = classic.object_sha256(prepared)
    result: dict[str, Any] = {
        "schema": "s20plus_g986n_twrp_fastbootd_q2_pre_effect_abort_v1",
        "version": VERSION,
        "prepared_sha256": prepared_sha256,
        "device_effects": 0,
        "persistent_writes": 0,
        "partition_operations": 0,
        "verdict": "ABORTED_S20PLUS_G986N_TWRP_FASTBOOTD_Q2_BEFORE_EFFECT",
        "at": now(),
    }
    ...
    if lexists(abort_path):
        ...
        if (
            ...
            or result.get("schema")
            != "s20plus_g986n_twrp_fastbootd_q2_pre_effect_abort_v1"
            or result.get("version") != VERSION
            ...
            != "ABORTED_S20PLUS_G986N_TWRP_FASTBOOTD_Q2_BEFORE_EFFECT"
            or not isinstance(result.get("at"), str)
        ):
...
def finalize(
    ...
    result = {
        "schema": "s20plus_g986n_twrp_fastbootd_q2_final_result_v1",
        "version": VERSION,
        "target": target,
        "returned_android": {
...
def plan() -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_twrp_fastbootd_q2_plan_v1",
        "version": VERSION,
...
def main() -> int:
    ...
    if args.render_plan:
        ...
        return 0
    if not LIVE_ACTIVE:
        print("STOP_S20PLUS_TWRP_FASTBOOTD_Q2: reviewed activation is absent")
        return 2
    if args.abort_pre_effect:
        try:
            result = abort_pre_effect(root)
        except Exception:
            print("FAIL_S20PLUS_TWRP_FASTBOOTD_Q2_PRE_EFFECT_ABORT_CLOSED")
            return 1
        print(result["verdict"])
        return 0
    if not args.operator_attended:
        print("STOP_S20PLUS_TWRP_FASTBOOTD_Q2: operator attendance is required")
        return 2
    if args.connected:
        ...
        try:
            ...
            base.durable_write(
                ...
                {
                    "schema": "s20plus_g986n_twrp_fastbootd_q2_failure_v1",
                    "version": VERSION,
            ...
            print("RECOVERY_PENDING_S20PLUS_TWRP_FASTBOOTD_Q2_PHYSICAL_REBOOT")
            return 1
        except Exception as exc:
            ...
            if not consumed and lexists(guard_path(root)):
                try:
                    ...
                    pass
            print(
                "RECOVERY_PENDING_S20PLUS_TWRP_FASTBOOTD_Q2_PHYSICAL_REBOOT"
                if consumed
                else "FAIL_S20PLUS_TWRP_FASTBOOTD_Q2_PRE_EFFECT_CLOSED"
            )
            return 1
        print(result["verdict"])
    ...
    try:
        ...
        print("RETURN_REQUIRED_S20PLUS_TWRP_FASTBOOTD_Q2: reboot System physically")
        return 1
    except Exception:
        print("FAIL_S20PLUS_TWRP_FASTBOOTD_Q2_FINALIZE_CLOSED")
        return 1
    print(result["verdict"])
