#!/usr/bin/env python3
"""V2776 live validation for the native-init audio prerequisite API.

This validates the V2775 behavior change: `audio prereq [profile]` must expose the speaker preparation contract as a read-only API on device, then roll back to V2321.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2775-audio-prereq-api/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2775_audio_prereq_api.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2776_AUDIO_PREREQ_API_DEVICE_VALIDATION_2026-06-19.md"

CANDIDATE_VERSION = "0.9.296"
CANDIDATE_TAG = "v2775-audio-prereq-api"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2776-audio-prereq-api-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2776-audio-prereq-api-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if (
            ...
            result["decision"] = "v2776-audio-prereq-api-device-pass"
        elif result["prereq_error_observed"]:
            result["decision"] = "v2776-audio-prereq-api-profile-error"
        elif not result["prereq_version_observed"]:
            result["decision"] = "v2776-audio-prereq-api-missing-command"
        else:
            result["decision"] = "v2776-audio-prereq-api-device-validation-incomplete"
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2776-audio-prereq-api-device-validation-dry-run",
