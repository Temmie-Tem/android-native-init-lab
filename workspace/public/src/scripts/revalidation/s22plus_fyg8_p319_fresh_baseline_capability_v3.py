#!/usr/bin/env python3
"""Host-only P3.19 V3 fresh-baseline capability.
...
The H0 V3 baseline-design identity binds the exact target/profile and the V3 reducer,
D1 V3 design, and current D0 runtime source bytes. The active candidate intent and
...
D1_SUCCESSOR = SCRIPT_DIR / "s22plus_fyg8_p319_d1_fresh_baseline_v3.py"
...
D1_ADB_SNAPSHOT = PRIVATE / (
    "outputs/s22plus_fyg8_p319/d1-fresh-baseline-v3/adb-"
...
D0_SUCCESSOR = SCRIPT_DIR / "s22plus_fyg8_p319_d0_fresh_baseline_v3.py"
...
D1_SCHEMA = "s22plus_fyg8_p319_d1_fresh_baseline_v3_result"
D0_SCHEMA = "s22plus_fyg8_p319_d0_fresh_baseline_v3_result"
SCHEMA = "s22plus_fyg8_p319_fresh_baseline_v3"
VERDICT = "PASS_P319_FRESH_BASELINE_V3_H0_NORMALIZED"
RUN_DIR = ROOT / (
    "workspace/private/runs/device-action-d1-p319-fresh-baseline-v3/"
    "p319-fresh-baseline-3"
)
RUN_ARM = Path(str(RUN_DIR) + ".arm.json")
D1_RAW_ROOT = RUN_DIR.parent / (RUN_DIR.name + "-raw")
D1_RAW_ADB_DIR = D1_RAW_ROOT / "raw-adb"
D0_RUN_DIR = ROOT / (
    "workspace/private/runs/device-action-d0-p319-fresh-baseline-v3/"
    "d0-p319-fresh-baseline-3"
)
...
BASELINE_DESIGN_SCHEMA = "s22plus_fyg8_p319_fresh_baseline_design_v3"
BASELINE_DESIGN_ID = "s22plus-fyg8-p319-fresh-baseline-design-v3"
...
def _typed_equal(left: Any, right: Any) -> bool:
    """Compare JSON values without allowing bool/int substitution."""
...
def _d1_identity() -> dict[str, Any]:
    ...
    return {"path": _relative(D1_SUCCESSOR), **_identity(payload),
            "schema": "s22plus_fyg8_p319_d1_fresh_baseline_execution_binding_v3"}
...
def _compile_d1_v3(payload: bytes) -> Any:
    module = types.ModuleType("p319_fresh_bound_d1_v3")
    ...
    try:
        ...
        raise FreshBaselineError(
            f"pinned D1 V3 validator rejected: {type(exc).__name__}"
        ) from exc
    if _stable(
        D1_SUCCESSOR,
        "post-import P3.19 D1 V3 source",
        maximum=max(len(payload), 1),
    ) != payload:
        raise FreshBaselineError("P3.19 D1 V3 source changed after import")
    return module


def _d1_v3_context() -> tuple[Any, dict[str, Any], dict[str, Any], str]:
    source_payload = _stable(
        D1_SUCCESSOR, "P3.19 D1 V3 source", maximum=1024 * 1024
    )
    binding_payload = _stable(
        D1_EXECUTION_MANIFEST,
        "P3.19 D1 V3 execution binding",
        maximum=64 * 1024,
        mode=None,
        nlink=1,
    )
    module = _compile_d1_v3(source_payload)
    try:
        static = module._validated_static_inputs()
        inputs = module._validated_execution_inputs(static)
    except BaseException as exc:
        raise FreshBaselineError(
            f"P3.19 D1 V3 execution closure rejected: {type(exc).__name__}"
        ) from exc
    review = {
        "status": "pass-go",
        "verdict": (
    ...
    if _stable(
        D1_EXECUTION_MANIFEST,
        "post-load P3.19 D1 V3 execution binding",
...
def _validate_d1(
    ...
    if (
        ...
        raise FreshBaselineError("D1 V3 result path is outside the fixed namespace")
    if not isinstance(value, dict):
    ...
    try:
        # D1 V3 owns the exact arm/start/result/raw namespace and its
        # _post_validate routine reopens every pinned input and receipt.  Keep
        # this call as the sole semantic authority instead of duplicating a
        # weaker result grammar here.
        canonical_result = module._post_validate(inputs)
    except BaseException as exc:
        raise FreshBaselineError(
    ...
    # both execution-critical D1 inputs once more.  This closes the result and
    # source/binding TOCTOU window without widening the observer grammar.
    try:
        final_result_payload = _stable(
            result_path,
            "final D1 V3 result",
            maximum=4 * 1024 * 1024,
            mode=0o400,
        ...
        final_result = json.loads(
            final_result_payload.decode("utf-8"),
            object_pairs_hook=_unique,
            parse_constant=lambda item: (_ for _ in ()).throw(
                FreshBaselineError(
                    f"final D1 V3 result contains non-finite JSON: {item}"
                )
            ),
        )
        if (
            not isinstance(final_result, dict)
            or final_result_payload != _canonical(final_result)
            or not _typed_equal(final_result, canonical_result)
        ):
            raise FreshBaselineError("final D1 V3 result changed after validation")
        final_source_payload = _stable(
            D1_SUCCESSOR,
            "final D1 V3 source",
            maximum=1024 * 1024,
        )
        final_binding_payload = _stable(
            D1_EXECUTION_MANIFEST,
            "final D1 V3 binding",
            maximum=64 * 1024,
            mode=None,
    ...
    if not isinstance(raw_evidence, dict) or raw_evidence.get("complete") is not True:
        raise FreshBaselineError("D1 V3 canonical raw evidence is incomplete")
    before = _health(canonical_result.get("before"), "D1 V3 before", profile)
    after = _health(canonical_result.get("after"), "D1 V3 after", profile)
    if before["boot_id_sha256"] == after["boot_id_sha256"]:
        raise FreshBaselineError("D1 V3 reboot did not change boot identity")
    selection = canonical_result.get("selection")
    if not module._selection_complete(selection):
        raise FreshBaselineError("D1 V3 selection differs")
    return {
...
def _compile_d0_producer(payload: bytes) -> Any:
    module = types.ModuleType("p319_fresh_bound_d0_producer_v3")
...
def _d0_execution_binding(
    ...
    if (
        ...
        or value.get("schema")
        != "s22plus_fyg8_p319_d0_fresh_baseline_execution_binding_v3"
        or value.get("binding_id")
        != "s22plus-fyg8-p319-d0-fresh-baseline-v3"
        or value.get("action")
    ...
    expected_d1 = {
        ...
        "consumed_predecessor_binding_sha256": (
...
def _validate_d0(value: Any, design: dict[str, Any], d1: dict[str, Any], candidate: dict[str, Any], profile: dict[str, Any], result_path: Path) -> tuple[dict[str, Any], bytes, Any, bytes]:
    ...
    if item["schema"] != D0_SCHEMA or item["version"] != "device-action-d0-p319-v3" or item["mode"] != "connected-read-only":
    ...
    if item["verdict"] != "PASS_P319_D0_FRESH_BASELINE_RAW_V3":
...
def validate_result(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    d1_raw = value["d1"].get("raw_evidence")
    if (
        not isinstance(d1_raw, dict)
        or set(d1_raw)
        != {
            "schema", "root", "directory", "root_present",
            "directory_present", "complete", "children", "handles",
            "invalid_receipts", "unclaimed_children", "aggregate_sha256",
        }
        or d1_raw.get("schema")
        != "s22plus_fyg8_p319_d1_fresh_baseline_v3_raw_inventory"
        or d1_raw.get("root") != _relative(D1_RAW_ROOT)
        or d1_raw.get("directory") != _relative(D1_RAW_ADB_DIR)
        or d1_raw.get("root_present") is not True
        or d1_raw.get("directory_present") is not True
