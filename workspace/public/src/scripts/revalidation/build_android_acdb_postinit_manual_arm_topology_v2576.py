#!/usr/bin/env python3
"""Build V2576 ARM32 post-init manual-arm manual-arm ACDB topology capture artifacts.
...
The key V2576 delta is that acdb_ioctl stays completely silent until the helper explicitly arms capture after acdb_loader_init_v3() returns.  All init-time calls pass through with no dump/hash/file I/O.  Once armed, every out_len>0 ACDB call is dumped and the process exits immediately after the first ret==0 non-all-zero 4916-byte topology payload.  The preload still fakes the audio calibration allocate/dealloc/SET ioctl path when A90_ACDB_FAKE_ALLOCATE=1, so this remains measurement-only.
"""
...
RUN_ID = "V2576"
BUILD_TAG = "v2576-acdb-postinit-manual-arm-topology-capture-host-only"
...
HELPER_ARTIFACT_NAME = "a90_acdb_postinit_manual_arm_topology_exec_linked_v2576"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_postinit_manual_arm_topology_capture_v2576.so"
...
def vendor_lib_state(readelf: str) -> dict[str, Any]:
    ...
    state["required_for_v2576_ok"] = all(state["required_for_v2576"].values())
    return state
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    helper_obj = obj_dir / "a90_acdb_postinit_manual_arm_topology_exec_linked_v2576.o"
    ...
    return {
        ...
        "compile": {"helper": helper_compile, "acdbtap_postinit_manual_arm": tap_compile, "ioctl_trace": ioctl_compile},
...
def manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            ...
            "unarmed_policy": "all acdb_ioctl calls before helper a90_arm_capture() pass through with no dump/hash/file I/O",
            "arm_point": "manual-arm after init_v3: helper calls a90_arm_capture() only after acdb_loader_init_v3() returns 0 and before acdb_loader_send_common_custom_topology()",
    ...
    payload["ok"] = bool(
        ...
        and payload["vendor_lib_state"].get("required_for_v2576_ok")
