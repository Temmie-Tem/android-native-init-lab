#!/usr/bin/env python3
"""V2791 live handoff for native-init integrated audio playback.

This is the audio core closure gate: flash the V2791 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2791-audio-integrated-play/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2791_audio_integrated_play.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2791_AUDIO_INTEGRATED_PLAY_LIVE_2026-06-19.md"

CYCLE = "V2791"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.304"
CANDIDATE_TAG = "v2791-audio-integrated-play"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2791-audio-integrated-play-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2791-audio-integrated-play-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if not result["play_output_pass"] or play.get("rc") != 0:
            result["decision"] = "v2791-audio-integrated-play-live-command-failed-before-rollback"
        ...
        result["decision"] = "v2791-audio-integrated-play-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2791-audio-integrated-play-live-blocked")
        if result["decision"] == "v2791-audio-integrated-play-live-started":
            result["decision"] = "v2791-audio-integrated-play-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2791-audio-integrated-play-live-dry-run",
