#!/usr/bin/env python3
"""V2798 live handoff for native-init audio msm_audio_cal devnode.

This is the audio core closure gate: flash the V2798 native-init image, stage the
...
V2798 specifically verifies that `/dev/msm_audio_cal` is materialized before
the SET-cal executor opens it, so execution can progress beyond the V2797
`open_ok=0 errno=2` blocker.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2798-audio-msm-audio-cal-devnode/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2798_audio_msm_audio_cal_devnode.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2798_AUDIO_MSM_AUDIO_CAL_DEVNODE_LIVE_2026-06-19.md"

CYCLE = "V2798"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.311"
CANDIDATE_TAG = "v2798-audio-msm-audio-cal-devnode"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2798-audio-msm-audio-cal-devnode-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2798-audio-msm-audio-cal-devnode-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2798-audio-msm-audio-cal-devnode-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio msm_audio_cal devnode command did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-msm-audio-cal-devnode-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2798-audio-msm-audio-cal-devnode-live-worker-failed-before-rollback"
            raise RuntimeError("native audio msm_audio_cal devnode command did not emit all required pass markers")
        ...
        result["decision"] = "v2798-audio-msm-audio-cal-devnode-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2798-audio-msm-audio-cal-devnode-live-blocked")
        if result["decision"] == "v2798-audio-msm-audio-cal-devnode-live-started":
            result["decision"] = "v2798-audio-msm-audio-cal-devnode-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2798-audio-msm-audio-cal-devnode-live-dry-run",
