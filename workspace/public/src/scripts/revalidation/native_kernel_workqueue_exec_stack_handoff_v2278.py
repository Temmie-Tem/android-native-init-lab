#!/usr/bin/env python3
"""V2278 rollbackable live validation for the V2277 workqueue execute stack oracle.

Default mode is host-only dry-run. Live mode flashes the V2277 workqueue
execute_start stack + codeword boot image, waits for the helper-owned
post-FWREADY window, collects both sampler logs, rolls back to V2237, then
classifies workqueue execute_start stack/callsite evidence with the same-boot
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2278_WORKQUEUE_EXEC_STACK_LIVE_2026-06-12.md"
)
BUILD_MANIFEST = (
    REPO_ROOT
    / "workspace"
    ...
    / "builds"
    / "native-init"
    / "v2277-workqueue-exec-stack"
    / "manifest.json"
)
TEST_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2277_workqueue_exec_stack.img", legacy_fallback=False
)
...
TEST_EXPECT_VERSION = "A90 Linux init 0.9.275 (v2277-workqueue-exec-stack)"
ROLLBACK_EXPECT_VERSION = "A90 Linux init 0.9.268 (v2237-supplicant-terminate-poll)"
REQUIRED_CONFIRM = "I_UNDERSTAND_V2278_FLASHES_V2277_AND_ROLLS_BACK_TO_V2237"
REMOTE_ARTIFACTS = {
    "helper_result": "/cache/native-init-wifi-test-boot-v2277-helper.result",
    "summary": "/cache/native-init-wifi-test-boot-v2277.summary",
    "log": "/cache/native-init-wifi-test-boot-v2277.log",
    "workqueue_stack_log": "/cache/native-init-v2277-workqueue-exec-stack.log",
    "codeword_log": "/cache/native-init-v2277-tail-perf-regs-codeword.log",
}
TARGET_SYMBOLS = [
...
def parse_workqueue_stack_log(text: str) -> dict[str, Any]:
...
def classify_artifacts(paths: dict[str, Path]) -> dict[str, Any]:
    stack_text = paths.get("workqueue_stack_log", Path()).read_text(encoding="utf-8", errors="replace") if paths.get("workqueue_stack_log") and paths["workqueue_stack_log"].exists() else ""
    ...
    workqueue = parse_workqueue_stack_log(stack_text)
    ...
    if not stack_text:
        classification = "workqueue-stack-log-missing"
    elif workqueue.get("result") != "v2277-workqueue-exec-stack-sample-ring-complete":
        classification = "workqueue-stack-sampler-incomplete"
        ...
        classification = "workqueue-stack-no-activity"
    elif function_hits or stack_hits:
        classification = "workqueue-exec-stack-target-hit"
    elif overflow > 0 or printed < stored:
        classification = "workqueue-exec-stack-no-target-hit-partial-coverage"
    else:
        classification = "workqueue-exec-stack-no-target-hit"
...
def classify_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    if not manifest.get("execute"):
        ...
        return {
            "decision": "v2278-workqueue-exec-stack-dry-run-ready" if ok else "v2278-workqueue-exec-stack-dry-run-blocked",
            "pass": bool(ok),
            "reason": "host-only flash/capture/rollback plan is complete; live execution requires --execute and confirmation" if ok else "required V2277 build artifact or rollback image is missing/mismatched",
        }
    rollback_result = manifest.get("rollback") or {}
    if not rollback_result.get("selftest_ok"):
        return {"decision": "v2278-workqueue-exec-stack-rollback-selftest-failed", "pass": False, "reason": "rollback did not finish with selftest fail=0"}
    if manifest.get("live_block") == "preflight-current-baseline-failed":
        return {"decision": "v2278-workqueue-exec-stack-preflight-failed-no-flash", "pass": False, "reason": "current baseline verification failed before flashing V2277"}
    if manifest.get("live_block") == "test-flash-failed":
        return {"decision": "v2278-workqueue-exec-stack-test-flash-failed-rollback-pass", "pass": False, "reason": "V2277 flash or health verification failed, but rollback selftest fail=0 passed"}
    classification = ((manifest.get("collect") or {}).get("classification") or {}).get("classification", "unknown")
    if classification in {"workqueue-stack-log-missing", "workqueue-stack-sampler-incomplete", "codeword-log-missing", "codeword-slide-unusable"}:
        return {"decision": f"v2278-workqueue-exec-stack-live-{classification}-rollback-pass", "pass": False, "reason": "V2277 booted and rollback passed, but paired stack/codeword evidence was not classifiable"}
    return {"decision": f"v2278-workqueue-exec-stack-live-pass-{classification}", "pass": True, "reason": "V2277 workqueue execute-start stack and codeword artifacts were collected and classified; rollback selftest fail=0 passed"}
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    if manifest.get("execute"):
        ...
        lines.extend([
            ...
            f"- V2277 flash OK: `{(manifest.get('test_flash') or {}).get('ok')}`",
            f"- V2277 health: version=`{health.get('version_ok')}` status=`{health.get('status_ok')}` selftest_fail0=`{health.get('selftest_ok')}`",
        ...
        if classification.get("classification") == "workqueue-exec-stack-target-hit":
            lines.extend([
                "## Interpretation",
                "",
                "- The same-boot stack/callsite oracle intersects the firmware_class/qcacld-HDD target set.",
            ...
            "workqueue-exec-stack-no-target-hit",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", default="v2278-workqueue-exec-stack-live")
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "safety": {
            ...
            "partition_write_scope": "boot image flash to V2277 and rollback to V2237 only" if args.execute else "none",
            "wifi_scan_connect": False,
            "credentials": False,
            "dhcp_routes_ping": False,
    ...
    if not args.execute:
        ...
        manifest["result"] = {"decision": "v2278-workqueue-exec-stack-live-confirmation-missing", "pass": False, "reason": "live mode requires the exact confirmation token"}
    else:
        ...
        if not (current.get("verify_ok") and current.get("selftest_ok")):
            ...
            v2275.write_step(store, steps, "flash-v2277-from-native", test_flash)
