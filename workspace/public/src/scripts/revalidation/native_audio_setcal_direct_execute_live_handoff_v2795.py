#!/usr/bin/env python3
"""V2795 live handoff for native-init audio SET-cal direct execute.

This is the audio core closure gate: flash the V2795 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2795-audio-setcal-direct-execute/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2795_audio_setcal_direct_execute.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2795_AUDIO_SETCAL_DIRECT_EXECUTE_LIVE_2026-06-19.md"

CYCLE = "V2795"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.308"
CANDIDATE_TAG = "v2795-audio-setcal-direct-execute"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2795-audio-setcal-direct-execute-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2795-audio-setcal-direct-execute-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2795-audio-setcal-direct-execute-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio SET-cal direct execute did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-setcal-direct-execute-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2795-audio-setcal-direct-execute-live-worker-failed-before-rollback"
            raise RuntimeError("native audio SET-cal direct execute did not emit all required pass markers")
        ...
        result["decision"] = "v2795-audio-setcal-direct-execute-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2795-audio-setcal-direct-execute-live-blocked")
        if result["decision"] == "v2795-audio-setcal-direct-execute-live-started":
            result["decision"] = "v2795-audio-setcal-direct-execute-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2795-audio-setcal-direct-execute-live-dry-run",
