#!/usr/bin/env python3
"""Build V2917 native-init candidate with the Bad Apple player HUD preset."""
...
CYCLE = "V2917"
INIT_VERSION = "0.10.38"
INIT_BUILD = "v2917-badapple-player-hud"
BUILD_TAG = INIT_BUILD
DECISION = "v2917-badapple-player-hud-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2917_BADAPPLE_PLAYER_HUD_SOURCE_BUILD_2026-06-20.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2917_badapple_player_hud.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2917_badapple_player_hud"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2917_badapple_player_hud.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v466_badapple_player_hud"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.38 (v2917-badapple-player-hud)",
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2917 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    markers = manifest.get("v2917_marker_strings", [])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "badapple-player-hud-candidate",
        "parent_test_artifact": "v2916-badapple-player-hud-host-only",
        "badapple_player_hud": {
            "version": 1,
            "source_unit": "V2917",
            "parent_unit": "V2916",
        ...
        "v2917_marker_strings": marker_strings,
    ...
    (OUT_DIR / "badapple-player-hud-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "badapple-player-hud-candidate",
