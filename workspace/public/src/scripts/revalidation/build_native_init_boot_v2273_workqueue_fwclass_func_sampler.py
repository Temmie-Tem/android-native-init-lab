#!/usr/bin/env python3
"""Build V2273 workqueue firmware_class function-pointer sampler test boot.

This source/build wrapper keeps the V2237 Wi-Fi route and adds only a
ramdisk-local read-only BPF tracepoint sampler for workqueue function pointers
around the post-FWREADY boot_wlan / firmware_class fallback window.
"""
...
OUT_DIR = workspace_private_build_path("native-init", "v2273-workqueue-fwclass-func-sampler")
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2273_WORKQUEUE_FWCLASS_FUNC_SAMPLER_SOURCE_BUILD_2026-06-12.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2273_workqueue_fwclass_func_sampler.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2273_workqueue_fwclass_func_sampler"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2273_workqueue_fwclass_func_sampler.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v431_workqueue_fwclass_func_sampler"
...
EXPECTED_HELPER_MARKER = "a90_android_execns_probe v431"
EXTRA_INIT_FLAGS = v2237.EXTRA_INIT_FLAGS
HELPER_MODE = v2237.HELPER_MODE
HELPER_RUNTIME_MODE = v2237.HELPER_RUNTIME_MODE
WORKQUEUE_LOG_PATH = "/cache/native-init-v2273-workqueue-fwclass.log"
...
def with_workqueue_flags(flags: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for flag in WORKQUEUE_SAMPLER_FLAGS:
...
def configure_base() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--init-build": "v2273-workqueue-fwclass-func-sampler",
        "--out-dir": str(OUT_DIR),
        "--init-binary": str(INIT_BINARY),
        "--helper-binary": str(HELPER_BINARY),
        "--ramdisk-cpio": str(RAMDISK_CPIO),
        "--boot-image": str(BOOT_IMAGE),
        "--wifi-test-klog-prefix": "A90v2273",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2273.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2273.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2273.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2273-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2273.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2273-supervisor.pid",
...
def build_bpf_helper() -> str:
    ...
    return sha256(BPF_HELPER_BINARY)
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...], bpf_helper_sha: str) -> str:
    ...
    return "\n".join([
        ...
        f"- Helper marker: `a90_android_execns_probe v431` (binary marker string: `{manifest['helper_marker']}`)",
        f"- Helper SHA256: `{manifest['helper_sha256']}`",
        f"- Workqueue sampler ramdisk path: `{BPF_HELPER_RUNTIME_PATH}`",
        f"- Workqueue sampler SHA256: `{bpf_helper_sha}`",
        "",
        "## Route",
        "",
        ...
        f"- Capture contract: start before `append_post_fw_ready_boot_wlan_trigger`, observe `workqueue_queue_work` and `workqueue_execute_start` for {WORKQUEUE_DURATION_MS} ms, print up to {WORKQUEUE_PRINT_LIMIT} samples, write `{WORKQUEUE_LOG_PATH}`, then classify samples offline after the live handoff.",
...
def main() -> int:
    ...
    bpf_helper_sha = build_bpf_helper()
    ...
    base.render_report = lambda manifest: render_report(manifest, helper_flags, bpf_helper_sha)
    rc = base.main()
    manifest_path = OUT_DIR / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["candidate_tag"] = "v2273-workqueue-fwclass-func-sampler"
    ...
    live_candidate_path.write_text(json.dumps({
        "candidate_tag": "v2273-workqueue-fwclass-func-sampler",
        "boot_image": str(BOOT_IMAGE.relative_to(REPO_ROOT)),
        "boot_sha256": manifest["boot_sha256"],
        "init_version": manifest["init_version"],
        "init_build": manifest["init_build"],
        "helper_sha256": manifest["helper_sha256"],
        "workqueue_sampler_sha256": bpf_helper_sha,
        "source_report": str(REPORT_PATH.relative_to(REPO_ROOT)),
        "next_live_cycle": "V2274",
        "note": "V2273 keeps the V2237 WLAN route and adds a read-only workqueue function-pointer sampler around the post-FWREADY boot_wlan/firmware_class window.",
