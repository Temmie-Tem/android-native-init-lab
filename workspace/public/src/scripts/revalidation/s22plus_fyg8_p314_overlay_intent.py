#!/usr/bin/env python3
"""Materialize one immutable P3.14 userspace overlay intent."""
...
def create(root: Path, output: Path) -> dict:
    if output.exists() or output.is_symlink():
        raise IntentError(f"P3.14 overlay intent output exists: {output}")
    ...
    try:
        if os.write(descriptor, payload) != len(payload):
            raise IntentError("short P3.14 overlay intent write")
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
