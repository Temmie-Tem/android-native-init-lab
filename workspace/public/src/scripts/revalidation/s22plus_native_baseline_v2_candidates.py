...
def declaration(namespace, run_id, version, image_sha256):
    ...
    runtime.build_helper = lambda:source.helper_template(selected)
    runtime.materialize_helper = lambda key:source.materialize_helper(selected,key)
    runtime.audit_binding = lambda:dict(source.profile_contract(),run_id_hex=run_id,
                                        target=runtime.TARGET,live_authorized=False)
    observer = resident.Observer(selected,CONTROL)
    ...
    adapter.NATIVE_SOURCE_PROFILE = source.PROFILE
    return SimpleNamespace(__file__=__file__,IDENTITY=selected,PROFILE=source.PROFILE,OWNER_PROFILE=POLICY,
