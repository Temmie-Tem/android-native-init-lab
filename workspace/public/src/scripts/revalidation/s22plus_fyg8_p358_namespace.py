"""Fixed P358 namespace over sealed P353 H0 machinery; no device authority."""
...
def load(namespace):
    name = Path(namespace['__file__']).name.replace('p358', 'p353')
    relative, expected = PREDECESSORS[name]
    path = ROOT / relative
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != expected:
        raise ValueError('P358 sealed predecessor changed: ' + name)
    raw = raw.replace(b'P353', b'P358').replace(b'p353', b'p358')
    raw = raw.replace(b'c353f1e0a90b5e6d7c8a9b0c1d2e3f0b', b'c358f1e0a90b5e6d7c8a9b0c1d2e3f0b')
    raw = raw.replace(b'68f6aa66cb29cf94cd7c857545cffd54fc0a250f9bb0a98208a7a153a32e84d9',
                      b'06366eb6660e9de4c8edfac68e4baf21aae911f5b15c9220cae1cfefeee2812b')
    tree = ast.parse(raw)
    tree.body = [n for n in tree.body if not (isinstance(n, ast.If) and '__name__' in ast.unparse(n.test))]
    entry = namespace['__name__']
    try:
        namespace['__name__'] = entry if entry != '__main__' else 'p358_h0_template'
        exec(compile(ast.fix_missing_locations(tree), str(path) + '#p358', 'exec'), namespace)
    finally:
