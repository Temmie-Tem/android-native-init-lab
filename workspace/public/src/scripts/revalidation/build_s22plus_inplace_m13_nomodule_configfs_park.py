#!/usr/bin/env python3
"""Build the S22+ M13 no-module configfs/role-force park candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M13 shrinks below the M12 live boot loop by removing all runtime module
insertion and removing the boot-ramdisk module-list payload. It intentionally
keeps the M12 freestanding raw-syscall PID1, minimal filesystem setup,
configfs ss_acm.0 gadget attempt, role-force attempt, and no-reboot park loop.
This separates "module insertion is enough to loop" from "configfs/role-force
or the freestanding runtime still loops without any module work."
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m13_nomodule_configfs_park_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m13_nomodule_configfs_park.c")
MARKER = "S22_NATIVE_INIT_USB_ACM_M13"
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M13 no-module configfs park init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M13 no-module configfs park init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M13 no-module configfs park init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M13 no-module configfs park init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M13 no-module configfs park init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M13 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M13 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M13 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M13 /init: {required}")
    ...
    if reboot_nr_lines:
        raise SystemExit("M13 init unexpectedly contains arm64 __NR_reboot (142)")
    finit_nr_lines = [
        line
        for line in objdump_text.splitlines()
    ...
    if finit_nr_lines:
        raise SystemExit("M13 init unexpectedly contains arm64 __NR_finit_module (273)")
    ...
    (build_dir / "m13_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m13_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m13_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m13_init = build_dir / "s22plus_init_usb_acm_m13"
    m13_init_info = compile_init(source, m13_init, build_dir)
    ...
    patch_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m13_init}"],
        work_dir,
        "replace /init with M13 no-module configfs park init",
    )
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M13 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m13_init):
        raise SystemExit("replaced /init does not match compiled M13 init")
    ...
    sizes = {
        ...
        "m13_init": m13_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M13 in-place no-module configfs/role-force park native-init",
        "safety": {
            "boot_only": True,
