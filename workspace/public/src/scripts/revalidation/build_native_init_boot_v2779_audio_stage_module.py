#!/usr/bin/env python3
"""Build V2779 native-init audio stage module boot image.

V2779 splits the explicit audio stage contract into `a90_audio_stage.{h,c}` while preserving the validated audio command behavior.
...
CYCLE = "V2779"
INIT_VERSION = "0.9.298"
INIT_BUILD = "v2779-audio-stage-module"
BUILD_TAG = "v2779-audio-stage-module"
DECISION = "v2779-audio-stage-module-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2779_AUDIO_STAGE_MODULE_SOURCE_BUILD_2026-06-19.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2779_audio_stage_module.img", legacy_fallback=False
)
BASE_BOOT = workspace_private_input_path(
    "boot_images", "boot_linux_v2334_audio_snd_nodes_preflight.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2779_audio_stage_module"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2779_audio_stage_module.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v443_audio_stage_module"
...
def configure() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--wifi-test-klog-prefix": "A90v2779",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2779.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2779.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2779.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2779-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2779.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2779-supervisor.pid",
    }
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    return "\n".join([
        "# Native Init V2779 Audio Stage Module Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        "- Track: audio stage contract module source build.",
        ...
        "## Purpose",
        "",
        "- This compiles the V2779 source change that moves the explicit stage contract into `a90_audio_stage.{h,c}` while preserving the existing `audio` command surface.",
        "- Expected future live validation should reuse the V2778-validated `audio prereq internal-speaker-safe` smoke surface; this unit is source/link validation for the stage module split.",
        "- Current source keeps stage constants/types in `a90_audio_stage.h`, stage contract data in `a90_audio_stage.c`, profile/route data in `a90_audio_profile.c`, and command/IO orchestration in `a90_audio.c`.",
        "",
        "## Scope Boundary",
        "",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "parent_baseline": "v2777-audio-profile-module",
        ...
        "audio_stage_module_source_validation": {
            "native_stage_module_compiled": True,
            ...
            "known_source_boundary": "stage contract data are owned by a90_audio_stage.{h,c}; profile and route data remain in a90_audio_profile.{h,c}; command orchestration remains in a90_audio.c",
        },
    ...
    (OUT_DIR / "promotion-candidate.json").write_text(json.dumps({
        ...
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "note": "V2779 is a test image for the audio stage contract module split, not a promoted rollback baseline.",
