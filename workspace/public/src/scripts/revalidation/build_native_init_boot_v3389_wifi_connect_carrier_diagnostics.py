#!/usr/bin/env python3
"""Build V3389 native-init boot image with Wi-Fi connect carrier diagnostics."""
...
import build_native_init_boot_v3388_wifi_autoconnect_scan_recovery as previous

base = previous.base
ORIG_PREVIOUS_REWRITE_TEXT = previous._rewrite_v3388_text
ORIG_PREVIOUS_REWRITE_BYTES = previous._rewrite_v3388_bytes
ORIG_PREVIOUS_REQUIRED_STRINGS = previous.REQUIRED_STRINGS
ORIG_PREVIOUS_BOOT_AUDIT_MANIFEST = previous._boot_audit_manifest
ORIG_PREVIOUS_NORMALIZE_MANIFEST = previous._normalize_manifest_for_v3388

CYCLE = "V3389"
INIT_VERSION = "0.11.145"
INIT_BUILD = "v3389-wifi-connect-carrier-diagnostics"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3389_WIFI_CONNECT_CARRIER_DIAGNOSTICS_SOURCE_BUILD_2026-07-04.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3389_wifi_connect_carrier_diagnostics.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3389_wifi_connect_carrier_diagnostics"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3389_wifi_connect_carrier_diagnostics.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v634_wifi_connect_carrier_diagnostics"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3389"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3389.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3389.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3389"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3389-wifi-connect-carrier-diagnostics"

FRAME_PATH = "/tmp/a90-doomgeneric-v3389-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3389-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3389-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3389-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3389-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3389-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3389-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3389.c"
...
def _rewrite_v3389_text(text: str) -> str:
    text = ORIG_PREVIOUS_REWRITE_TEXT(text)
    replacements = (
        ("v3388-wifi-autoconnect-scan-recovery", INIT_BUILD),
        ("0.11.144", INIT_VERSION),
        ("V3388", CYCLE),
        ("v3388", "v3389"),
        ("a90-doomgeneric-v3388", "a90-doomgeneric-v3389"),
        ("a90.doomgeneric.v3388", "a90.doomgeneric.v3389"),
    )
...
def _rewrite_v3389_bytes(item: bytes) -> bytes:
    return _rewrite_v3389_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3389_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3389_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3389_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3389_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3389_bytes(marker) for marker in ORIG_PREVIOUS_REQUIRED_STRINGS)
...
OBSOLETE_RAMDISK_ENGINES = tuple(dict.fromkeys([
    *previous.OBSOLETE_RAMDISK_ENGINES,
    "a90_doomgeneric_private_engine_v3388",
]))


def _boot_audit_manifest() -> dict[str, Any]:
    manifest = ORIG_PREVIOUS_BOOT_AUDIT_MANIFEST()
    manifest["rung"] = "wifi-connect-carrier-diagnostics"
    manifest["scope"] = "native-owned-wifi-connect-carrier-diagnostics"
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "wifi-connect-carrier-diagnostics.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "wifi-connect-carrier-diagnostics",
...
def _normalize_manifest_for_v3389(manifest: dict[str, Any]) -> dict[str, Any]:
...
def _patch_v3388_module_for_v3389() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "_rewrite_v3388_text": _rewrite_v3389_text,
        "_rewrite_v3388_bytes": _rewrite_v3389_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_normalize_manifest_for_v3388": _normalize_manifest_for_v3389,
    }
