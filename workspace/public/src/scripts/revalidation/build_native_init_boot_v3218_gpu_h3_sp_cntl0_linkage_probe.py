#!/usr/bin/env python3
"""Build V3218 GPU H3 SP CNTL0 linkage probe."""
...
import build_native_init_boot_v3216_gpu_h3_minimal_ir3_mov_shader_probe as previous

base = previous.base

CYCLE = "V3218"
INIT_VERSION = "0.11.36"
INIT_BUILD = "v3218-gpu-h3-sp-cntl0-linkage-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3218_GPU_H3_SP_CNTL0_LINKAGE_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3218_gpu_h3_sp_cntl0_linkage_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3218_gpu_h3_sp_cntl0_linkage_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v568_gpu_h3_sp_cntl0_linkage_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3218"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3218.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3218.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3218"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3218-gpu-h3-sp-cntl0-linkage-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3218-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3218-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3218-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3218-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3218-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3218-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3218-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-sp-cntl0-linkage-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-sp-cntl0-linkage-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3218")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3218")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3218")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3218")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3218")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3218")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3218")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3218")
SFX_STREAM_MARKER = "a90.doomgeneric.v3218.audio=real-sfx-pcm-stream-gpu-h3-sp-cntl0-linkage-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-sp-cntl0-linkage-probe-v3218"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3218.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3218_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3216": b"a90-doomgeneric-v3218",
        b"a90.doomgeneric.v3216": b"a90.doomgeneric.v3218",
        b"v3216": b"v3218",
        b"V3216": b"V3218",
        b"first-triangle-h3-minimal-ir3-mov-f32-shader":
            b"first-triangle-h3-sp-cntl0-linkage-mov-f32-shader",
    }
...
GPU_H3_SP_CNTL0_LINKAGE_MARKERS = (
...
REQUIRED_STRINGS = tuple(_rewrite_v3218_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3216-gpu-h3-minimal-ir3-mov-shader-probe",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-sp-cntl0-linkage-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3218-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-sp-cntl0-linkage-mov-f32-shader-timeout-guard",
...
def v3218_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-sp-cntl0-linkage-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-sp-cntl0-linkage-probe")
        .replace("v3208", "v3218")
        .replace("V3208", "V3218")
    )


def _overlay_preserved_v3218_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3216 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3218 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3218 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3218 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3218-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            ...
            raise RuntimeError("V3216 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3218 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3218 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3216-ramdisk-overlay-v3218-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-sp-cntl0-linkage-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-sp-cntl0-linkage-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-sp-cntl0-linkage-shader-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3216-ramdisk-overlay-v3218-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-sp-cntl0-linkage-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-sp-cntl0-linkage-shader-live-validation"
...
def _apply_v3218_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-sp-cntl0-linkage-probe")
        ...
        "v3210_adapter_source": v3218_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3218_ramdisk,
