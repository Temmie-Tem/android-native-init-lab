#!/usr/bin/env python3
"""Register-independent linked audit interface for P3.00 telemetry."""
...
import s22plus_fyg8_p298_linked_audit as inherited
import s22plus_fyg8_p300_source_contract as p300


ADAPTER_ID = "s22plus-fyg8-p300-linked-audit-v1"
EXPECTED_SOURCE_CONTRACT_ID = p300.CONTRACT_ID
ADAPTER_MODULE = "s22plus_fyg8_p300_postbuild_linked_audit"
LINKED_VALIDATOR_SYMBOLS = p300.LINKED_VALIDATOR_SYMBOLS
...
CALLSITE_AUDIT_SYMBOLS = (
    *p300.PROBE_TARGET_SYMBOLS,
    *p300.GADGET_START_CALLSITE_SYMBOLS,
)
...
def _instructions(disassembly: str) -> tuple[_Instruction, ...]:
    ...
    if not result:
        raise AuditError("P3.00 linked function has no instructions")
    if len({instruction.address for instruction in result}) != len(result):
        raise AuditError("P3.00 linked function repeats an instruction address")
    return tuple(result)
...
def _require_exact_direct_call(
    ...
    if len(targeted) != count or any(
        ...
        raise AuditError(
            f"P3.00 linked {symbol} edge is not {count} exact direct call(s)"
        )
...
def _audit_ep0_enable_chain(
    ...
    if first + 1 >= len(instructions) or second + 2 >= len(instructions):
        raise AuditError("P3.00 EP0 enable return checks are truncated")
    ...
    if (
        ...
        raise AuditError("P3.00 EP0 enable hit order/control flow differs")
    return {
        "direct_call_count": 2,
...
def _audit_pullup_discard(
    ...
    if start + 3 >= len(instructions):
        raise AuditError("P3.00 pullup gadget-start sequence is truncated")
    ...
    if (
        ...
        raise AuditError(
            "P3.00 pullup does not overwrite gadget-start w0 before run-stop"
        )
    return {
...
def _audit_resume_check(
    ...
    if start + 1 >= len(instructions):
        raise AuditError("P3.00 resume gadget-start sequence is truncated")
    ...
    if (
        ...
        raise AuditError("P3.00 resume does not test gadget-start signed return")
    return {
        "direct_gadget_start_call_count": 1,
...
def audit_gadget_start_callsites(
    ...
    if missing:
        raise AuditError(f"P3.00 call-site disassembly is missing: {missing}")
    parsed = {
    ...
    proof = {
        ...
        "probe_targets_out_of_line": all(
            len(parsed[symbol]) > 0 for symbol in p300.PROBE_TARGET_SYMBOLS
        ),
...
def audit_gadget_start_callsite_pair(
    ...
    if canonical_a != canonical_b or proof_a != proof_b:
        raise AuditError("P3.00 Full-LTO A/B gadget-start call sites diverge")
    encoded = json.dumps(
...
def _validate_tables(logical_tables: dict[str, bytes]) -> None:
    expected = p300.linked_table_bytes()
    if (
        ...
        raise AuditError("P3.00 logical linked table set is invalid")
...
def normalize_linked_table_storage(
    ...
    if actual_storage != logical_tables:
        raise AuditError("P3.00 physical linked table bytes differ")
