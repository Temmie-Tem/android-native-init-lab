#!/usr/bin/env python3
"""Build V3320 GPU M3 shared GPU monitor extraction."""
...
import build_native_init_boot_v3319_gpu_m2_monitor_live_graphs as previous

base = previous.base

CYCLE = "V3320"
INIT_VERSION = "0.11.91"
INIT_BUILD = "v3320-gpu-m3-monitor-extraction"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3320_GPU_M3_MONITOR_EXTRACTION_SOURCE_BUILD_2026-06-27.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3320_gpu_m3_monitor_extraction"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3320_gpu_m3_monitor_extraction.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v613_gpu_m3_monitor_extraction"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3320"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3320.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3320.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3320"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3320-gpu-m3-monitor-extraction"

FRAME_PATH = "/tmp/a90-doomgeneric-v3320-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3320-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3320-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3320-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3320-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3320-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3320-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-m3-monitor-extraction"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-m3-monitor-extraction"

SFX_STREAM_MARKER = "a90.doomgeneric.v3320.audio=real-sfx-pcm-stream-gpu-m3-monitor-extraction"
SOUND_MODE = "native-doom-sfx-gpu-m3-monitor-extraction-v3320"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3320.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = "gpu-m3-shared-kgsl-2d-present-extraction"
M3_COMMAND = "gpu m3-monitor-extraction-probe --frames 12 --interval-ms 200 --timeout-ms 60000 --hold-ms 5000 --materialize-devnode"
...
def _rewrite_v3320_text(text: str) -> str:
    replacements = (
        ...
        ("a90-doomgeneric-v3319", "a90-doomgeneric-v3320"),
        ("a90.doomgeneric.v3319", "a90.doomgeneric.v3320"),
        ("v3319", "v3320"),
        ("V3319", "V3320"),
        ("gpu-m2-monitor-live-graphs", "gpu-m3-monitor-extraction"),
    )
...
def _rewrite_v3320_bytes(item: bytes) -> bytes:
    return _rewrite_v3320_text(item.decode("utf-8")).encode("utf-8")
...
def _minimal_gpu_m3_manifest() -> dict[str, Any]:
    return {
        ...
        "scope": SCOPE,
        "command": M3_COMMAND,
        "candidate_type": "gpu-m3-monitor-extraction-candidate",
        "data_sources": [
        ...
        "next_live_validation": [
            "flash-v3320-through-native-init-flash",
            "post-flash-health-check",
            "gpu-m3-monitor-extraction-probe-default",
...
def v3320_adapter_source() -> str:
    return _rewrite_v3320_text(previous.v3319_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-m3-monitor-extraction-candidate.json").write_text(json.dumps({
        ...
        "adoption_state": "pending-gpu-m3-monitor-extraction-live-validation",
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def _overlay_preserved_v3320_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3320 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3320 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3320 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3320 DOOM engine binary: {ENGINE_BINARY}")

    with tempfile.TemporaryDirectory(prefix="a90-v3320-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            ...
            raise RuntimeError("V3320 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3320 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3320 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3319-ramdisk-overlay-v3320-init-helper-engine",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-m3-monitor-extraction-candidate"
    manifest["adoption_state"] = "pending-gpu-m3-monitor-extraction-live-validation"
...
def _apply_v3320_overrides() -> None:
    previous._apply_v3319_overrides()
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3320_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3320_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3320_ramdisk,
