#!/usr/bin/env python3
"""Build P3.12 userspace twice against the fixed P3.10 Image."""
...
SCHEMA = "s22plus_fyg8_p312_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p312/userspace")
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("P3.12 userspace profile/source contract differs")
    return VERDICT


def _configure_base() -> None:
    p310_contract._configure()
...
def _source_contract(exact: dict) -> dict:
    return {
        ...
        "frozen_p311_baseline": exact["frozen_p311_baseline"],
        "callsite_audit": exact["callsite_audit"],
        "delayed_arm_qemu": exact["delayed_arm_qemu"],
        "tracefs_abi": exact["tracefs_abi"],
        "cross_gate_audit": exact["cross_gate_audit"],
        "carrier_decoder_authority": exact["carrier_decoder_authority"],
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.12 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p312-userspace-{label}-") as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.12 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
