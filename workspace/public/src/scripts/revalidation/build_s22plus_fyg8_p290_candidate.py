#!/usr/bin/env python3
"""Build one deterministic boot-only P2.90 FYG8 candidate host-only."""
...
import s22plus_fyg8_p290_boot_only_packager as packager
import s22plus_fyg8_p290_build_repro_check as repro
import s22plus_fyg8_p290_candidate_contract as candidate_contract
import s22plus_fyg8_p290_contract_spec as p290_spec
import s22plus_fyg8_p290_e2_stock_closure as p290_closure
import s22plus_fyg8_p290_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p290_candidate_artifact_result_v1"
VERDICT = "PASS_P290_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P290_SOURCE_CONTRACT_ID = repro.P290_SOURCE_CONTRACT_ID
P286_SOURCE_CONTRACT_ID = P290_SOURCE_CONTRACT_ID
DEFAULT_IMAGE = repro.DEFAULT_BUILD_A / "Image"
DEFAULT_REPRO_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p290/"
...
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p290/candidate-a"
)
...
def _configure() -> None:
    ...
    base.p286_spec = p290_spec
    base.p286_closure = p290_closure
    base.SCHEMA = SCHEMA
...
def verify_repro_result(
    ...
    *,
    ...
    value, result_receipt = base._read_json(  # noqa: SLF001
        result_path, "P2.90 build reproducibility result"
    )
    if (
        ...
        raise BuildError(
            "P2.90 build reproducibility result is not accepted"
        )
    ...
    if (
        ...
        raise BuildError("P2.90 linked audit adapter mismatch")
    expected_image = (
        value.get("build_a", {}).get("artifacts", {}).get("Image")
    )
    if expected_image != image_receipt:
        raise BuildError(
            "P2.90 supplied Image differs from reproducibility closure"
        )
    if intent_path is None or patch_path is None:
        raise BuildError("P2.90 selected build-input paths are missing")
    try:
        qualification = repro.verify_p290_qualification_file(
