#!/usr/bin/env python3
"""Derive and parse the exact P2.82 cycle and bind trace contract."""
...
def derive_contract(
    *,
    ...
    return {
        **result,
        "schema": "s22plus_fyg8_p282_derived_trace_contract_v1",
...
def _parent_window(
    ...
    all_entries = _values(records, "worker_in")
    entries = tuple(
        record
    ...
    returns = tuple(
        record
        for record in _values(records, "worker_out")
        if (
...
def render_c_header(derived: dict[str, Any]) -> bytes:
    if derived.get("schema") not in {
        ...
        "s22plus_fyg8_p282_derived_trace_contract_v1",
    }:
        raise TraceContractError("derived P2.82 trace schema is invalid")
    modules = derived.get("modules")
    if not isinstance(modules, list) or not modules:
        raise TraceContractError("derived P2.82 modules are invalid")
    offsets = modules[0].get("parent_pm_post_call_offsets")
    if (
        not isinstance(offsets, list)
        or len(offsets) != 2
    ...
    lines = [
        "/* Generated from s22plus_fyg8_p282_contract_spec.py. */",
