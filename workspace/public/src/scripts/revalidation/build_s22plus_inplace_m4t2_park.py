#!/usr/bin/env python3
"""Build the S22+ M4T2 in-place raw-park PID1 boot candidate.

Host-only.  This script does not reboot, flash, or touch a connected device.

M4T2 is a smaller discriminator after M4T1: replace Magisk ramdisk `/init`
with a raw AArch64 `_start` that has no libc startup, makes no syscalls, and
parks forever.  If a future live test still fast-loops, the failure is likely
before or during exec of `/init`; if it stops fast-looping, the kernel reached
and ran the custom PID1.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m4t2_park_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_park_m4t2.S")
MARKER = "S22_NATIVE_INIT_PARK_M4T2"
...
def compile_raw_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str]:
    ...
    require_ok(result, "compile raw park init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip raw park init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file raw park init")
    readelf_headers = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf_headers, "readelf raw park init")
    readelf_text = readelf_headers.stdout.decode("utf-8", errors="replace")
    if "INTERP" in readelf_text:
        raise SystemExit("raw park init unexpectedly has a PT_INTERP segment")
    if "AArch64" not in readelf_text:
        raise SystemExit("raw park init is not AArch64")
    binary = out_path.read_bytes()
    if MARKER.encode("ascii") not in binary:
        raise SystemExit(f"required static marker missing from raw init: {MARKER}")
    if b"/lib" in binary or b"ld-linux" in binary:
        raise SystemExit("raw park init unexpectedly contains dynamic loader strings")
...
def main(argv: list[str]) -> int:
    ...
    raw_init = build_dir / "s22plus_init_park_m4t2"
    ...
    if sha256_file(extracted_replaced) != sha256_file(raw_init):
        raise SystemExit("replaced /init does not match compiled raw park init")
    ...
    hashes = {
        ...
        "raw_park_init": sha256_file(raw_init),
    ...
    sizes = {
        ...
        "raw_park_init": raw_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M4T2 in-place raw non-returning PID1 park candidate",
        "safety": {
            "boot_only": True,
    ...
    (out_dir / "required_strings.txt").write_text(MARKER + "\n", encoding="ascii")
