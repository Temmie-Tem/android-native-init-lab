#!/usr/bin/env python3
"""P3.08 retained-record model with candidate-specific terminal ranges."""
...
SCHEMA = "s22plus_fyg8_p308_loss_resistant_telemetry_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P3.08 profile: {profile}")
    ordinal = generation - 1
    p308_wide_semantics = ordinal == spec.SUMMARY_ORDINAL
    if not p308_wide_semantics and repair.is_publication_error_detail(
        outcome, detail
    ):
        position = spec.position_for_generation(generation)
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P3.08 publication-error record is at the wrong position"
            )
        return
    try:
        spec.validate_slot(
