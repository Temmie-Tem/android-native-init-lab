#!/usr/bin/env python3
"""Build V3265 GPU H3 CP_SET_MODE restore-mode probe."""
...
import build_native_init_boot_v3263_gpu_h3_window_offset_cmdroom_probe as previous

base = previous.base

CYCLE = "V3265"
INIT_VERSION = "0.11.59"
INIT_BUILD = "v3265-gpu-h3-cp-set-mode-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3265_GPU_H3_CP_SET_MODE_SOURCE_BUILD_2026-06-26.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3265_gpu_h3_cp_set_mode_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3265_gpu_h3_cp_set_mode_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v590_gpu_h3_cp_set_mode_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3265"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3265.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3265.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3265"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3265-gpu-h3-cp-set-mode-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3265-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3265-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3265-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3265-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3265-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3265-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3265-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-cp-set-mode-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-cp-set-mode-probe"

INPUT_THREAD_MARKER = previous.INPUT_THREAD_MARKER.replace("v3263", "v3265")
TIME_MODEL_MARKER = previous.TIME_MODEL_MARKER.replace("v3263", "v3265")
DEMO_HUD_MARKER = previous.DEMO_HUD_MARKER.replace("v3263", "v3265")
PACED_TIME_MARKER = previous.PACED_TIME_MARKER.replace("v3263", "v3265")
TICK_TELEMETRY_MARKER = previous.TICK_TELEMETRY_MARKER.replace("v3263", "v3265")
SCALE_MARKER = previous.SCALE_MARKER.replace("v3263", "v3265")
PHASE_TELEMETRY_MARKER = previous.PHASE_TELEMETRY_MARKER.replace("v3263", "v3265")
GAMETIC_FRAME_TELEMETRY_MARKER = previous.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3263", "v3265")
SFX_STREAM_MARKER = "a90.doomgeneric.v3265.audio=real-sfx-pcm-stream-gpu-h3-cp-set-mode-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-cp-set-mode-probe-v3265"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3265.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = "first-triangle-h3-cp-set-mode-window-offset-visibility-packets-vpc-so-override-off-sysmem-bin-control-sp-update-cntl-compiler-vs-instrlen-cache-invalidate-rb-render-cntl-r0-output-shader"
PREVIOUS_SCOPE = previous.SCOPE
SHADER_PAYLOAD = previous.SHADER_PAYLOAD
CP_SET_MODE_OPCODE = "0x00000063"
CP_SET_MODE_VALUE = "0x00000000"
CMD_MAX_DWORDS = 320
STALE_V3263_ENGINE_RAMDISK_PATH = previous.ENGINE_RAMDISK_PATH


def _rewrite_v3265_bytes(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3263": b"a90-doomgeneric-v3265",
        b"a90.doomgeneric.v3263": b"a90.doomgeneric.v3265",
        b"v3263": b"v3265",
        b"V3263": b"V3265",
        b"gpu-h3-window-offset-cmdroom-probe": b"gpu-h3-cp-set-mode-probe",
...
def _rewrite_v3265_text(text: str) -> str:
    for old, new in (
        ...
        ("a90-doomgeneric-v3263", "a90-doomgeneric-v3265"),
        ("a90.doomgeneric.v3263", "a90.doomgeneric.v3265"),
        ("v3263", "v3265"),
        ("V3263", "V3265"),
        ("gpu-h3-window-offset-cmdroom-probe", "gpu-h3-cp-set-mode-probe"),
...
GPU_H3_CP_SET_MODE_MARKERS = (
    ...
    b"gpu.h3.draw.cp_set_mode=0x%x",
    b"gpu.h3.draw.cp_set_mode_value=0x%x",
)

REQUIRED_STRINGS = tuple(_rewrite_v3265_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        ...
        "scope": SCOPE,
        "cp_set_mode_source": "Mesa freedreno A6xx fd6_emit_restore CP_SET_MODE(0)",
        "cp_set_mode_opcode": CP_SET_MODE_OPCODE,
        "cp_set_mode_value": CP_SET_MODE_VALUE,
        "cmd_max_dwords": CMD_MAX_DWORDS,
        "state_reg_writes_expected": 98,
        "pm4_dwords_expected": 262,
        "shader_payload": SHADER_PAYLOAD,
        "readback": "expect changed pixels if missing CP_SET_MODE restore-mode packet blocked draw-state execution",
        "next_live_validation": [
            "flash-v3265-through-native-init-flash",
            "post-flash-health-check",
            "gpu-h3-cp-set-mode-timeout-guard",
...
def v3265_adapter_source() -> str:
    return _rewrite_v3265_text(previous.v3263_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-cp-set-mode-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-cp-set-mode-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-cp-set-mode-live-validation",
...
def _patch_previous_overlay_globals() -> list[tuple[Any, str, Any]]:
    replacements = {
        ...
        "STALE_V3259_ENGINE_RAMDISK_PATH": STALE_V3263_ENGINE_RAMDISK_PATH,
    }
...
def _overlay_preserved_v3265_ramdisk() -> dict[str, Any]:
    saved = _patch_previous_overlay_globals()
    try:
        overlay = previous._overlay_preserved_v3263_ramdisk()
    finally:
        _restore_previous_overlay_globals(saved)
    overlay["mode"] = "preserve-v3263-ramdisk-overlay-v3265-init-helper-engine"
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-h3-cp-set-mode-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-cp-set-mode-live-validation"
...
def _apply_v3265_overrides() -> None:
    previous._apply_v3263_overrides()
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3265_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3265_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3265_ramdisk,
