#!/usr/bin/env python3
"""V3124 rollback-gated live validation for V3123 summary-only direct blit."""
...
RUN_ID = "V3124"
BUILD_TAG = "v3124-doomgeneric-summary-only-direct-blit-live"
DECISION_PREFIX = "v3124-doomgeneric-summary-only-direct-blit"
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V3124_DOOMGENERIC_SUMMARY_ONLY_DIRECT_BLIT_LIVE_2026-06-23.md"

CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v3123_doomgeneric_summary_only_direct_blit.img"
CANDIDATE_VERSION = "0.10.116"
CANDIDATE_TAG = "v3123-doomgeneric-summary-only-direct-blit"
CANDIDATE_SHA256 = "248af9a65bb9d4c30cd1c0cd11db642c1cbcb332697b0387372bd7465175b808"
...
def preflight_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state.update({
        ...
        "expected_summary_marker": "video.demo.doom.loop.foreground_frame_log=0",
...
def parse_loop_output(text: str) -> dict[str, Any]:
    ...
    foreground_frame_log = _last_int(values, "video.demo.doom.loop.foreground_frame_log")
    read_avg_us = parsed.get("timing_read_avg_us")
    draw_avg_us = parsed.get("timing_draw_avg_us")
    parsed.update({
        "presenter_reader": presenter_reader,
        "foreground_frame_log": foreground_frame_log,
        "direct_shared_blit_markers_ok": presenter_reader == EXPECTED_READER,
        "summary_only_markers_ok": foreground_frame_log == EXPECTED_FOREGROUND_FRAME_LOG,
        "v3119_read_avg_us": V3119_READ_AVG_US,
        "v3119_draw_avg_us": V3119_DRAW_AVG_US,
        "read_improved_threshold_us": READ_IMPROVED_THRESHOLD_US,
        "read_improved_vs_v3119": isinstance(read_avg_us, int) and read_avg_us < READ_IMPROVED_THRESHOLD_US,
        "read_delta_vs_v3119_us": read_avg_us - V3119_READ_AVG_US if isinstance(read_avg_us, int) else None,
        "draw_delta_vs_v3119_us": draw_avg_us - V3119_DRAW_AVG_US if isinstance(draw_avg_us, int) else None,
    })
    return parsed
...
def loop_classification(loop: dict[str, Any], requested_frames: int) -> str:
    ...
    if not loop.get("summary_only_markers_ok"):
        return "summary-only-marker-missing"
    return base_classification
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Expected foreground frame log: `{EXPECTED_FOREGROUND_FRAME_LOG}`",
        ...
        *_marker_lines(loop.get("markers", {}) if isinstance(loop.get("markers"), dict) else {}),
        ...
        "## Host Validation",
        "",
        "- `py_compile`: V3124 live runner and focused tests.",
        "- `unittest`: V3124 live parser/report/preflight contract.",
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"flash exact V3123 image {CANDIDATE_IMAGE}",
            ...
            "parse summary-only marker, direct-shared-blit marker, timing, seq, and pageflip markers",
