#!/usr/bin/env python3
"""Build P3.02-M0 userspace twice against the fixed P3.00 run ID."""
...
import s22plus_fyg8_p302_candidate_contract as candidate_contract
import s22plus_fyg8_p302_overlay_contract as contract


SCHEMA = "s22plus_fyg8_p302_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p302/userspace")
...
def verdict_for_profile(
    ...
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("unsupported P3.02 userspace profile or parent contract")
    return VERDICT
...
def _compile(
    ...
    result = base._compile_once(  # noqa: SLF001
        root,
        directory,
    ...
    carrier = binary_carrier.apply(directory / "init")
    data = (directory / "init").read_bytes()
    result["init"].update(base.receipt(data))
    result["init"]["p302_carrier"] = carrier
    return result
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.02 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p302-userspace-{label}-") as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.02 userspace two-build reproducibility mismatch")
    carrier = exact["carrier"]["id"].encode("ascii")
    if builds[0]["init"].count(carrier) != 1 or carrier in builds[0]["child"]:
        raise BuildError("P3.02 retained carrier identity missing or duplicated")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
