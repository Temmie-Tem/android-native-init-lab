#!/usr/bin/env python3
"""Build V3268 GPU H3 raster polygon-mode probe."""
...
import build_native_init_boot_v3265_gpu_h3_cp_set_mode_probe as previous

base = previous.base

CYCLE = "V3268"
INIT_VERSION = "0.11.60"
INIT_BUILD = "v3268-gpu-h3-raster-mode-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3268_GPU_H3_RASTER_MODE_SOURCE_BUILD_2026-06-26.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3268_gpu_h3_raster_mode_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3268_gpu_h3_raster_mode_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v600_gpu_h3_raster_mode_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3268"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3268.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3268.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3268"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3268-gpu-h3-raster-mode-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3268-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3268-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3268-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3268-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3268-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3268-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3268-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-raster-mode-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-raster-mode-probe"

INPUT_THREAD_MARKER = previous.INPUT_THREAD_MARKER.replace("v3265", "v3268")
TIME_MODEL_MARKER = previous.TIME_MODEL_MARKER.replace("v3265", "v3268")
DEMO_HUD_MARKER = previous.DEMO_HUD_MARKER.replace("v3265", "v3268")
PACED_TIME_MARKER = previous.PACED_TIME_MARKER.replace("v3265", "v3268")
TICK_TELEMETRY_MARKER = previous.TICK_TELEMETRY_MARKER.replace("v3265", "v3268")
SCALE_MARKER = previous.SCALE_MARKER.replace("v3265", "v3268")
PHASE_TELEMETRY_MARKER = previous.PHASE_TELEMETRY_MARKER.replace("v3265", "v3268")
GAMETIC_FRAME_TELEMETRY_MARKER = previous.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3265", "v3268")
SFX_STREAM_MARKER = "a90.doomgeneric.v3268.audio=real-sfx-pcm-stream-gpu-h3-raster-mode-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-raster-mode-probe-v3268"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3268.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = "first-triangle-h3-raster-mode-cp-set-mode-window-offset-visibility-packets-vpc-so-override-off-sysmem-bin-control-sp-update-cntl-compiler-vs-instrlen-cache-invalidate-rb-render-cntl-r0-output-shader"
PREVIOUS_SCOPE = previous.SCOPE
SHADER_PAYLOAD = previous.SHADER_PAYLOAD
RASTER_MODE_SOURCE = "Mesa freedreno A6xx fd6_rasterizer POLYMODE6_TRIANGLES"
RASTER_MODE_VALUE = "0x00000003"
CMD_MAX_DWORDS = 320
STALE_V3265_ENGINE_RAMDISK_PATH = previous.ENGINE_RAMDISK_PATH


def _rewrite_v3268_bytes(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3265": b"a90-doomgeneric-v3268",
        b"a90.doomgeneric.v3265": b"a90.doomgeneric.v3268",
        b"v3265": b"v3268",
        b"V3265": b"V3268",
        b"gpu-h3-cp-set-mode-probe": b"gpu-h3-raster-mode-probe",
...
def _rewrite_v3268_text(text: str) -> str:
    for old, new in (
        ...
        ("a90-doomgeneric-v3265", "a90-doomgeneric-v3268"),
        ("a90.doomgeneric.v3265", "a90.doomgeneric.v3268"),
        ("v3265", "v3268"),
        ("V3265", "V3268"),
        ("gpu-h3-cp-set-mode-probe", "gpu-h3-raster-mode-probe"),
...
GPU_H3_RASTER_MODE_MARKERS = (
    ...
    b"gpu.h3.draw.vpc_rast_cntl=0x%x",
    b"gpu.h3.draw.pc_dgen_rast_cntl=0x%x",
)

REQUIRED_STRINGS = tuple(_rewrite_v3268_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        ...
        "scope": SCOPE,
        "raster_mode_source": RASTER_MODE_SOURCE,
        "raster_mode_value": RASTER_MODE_VALUE,
        "raster_mode_registers": {
            "VPC_RAST_CNTL": "0x00009108",
            "PC_DGEN_RAST_CNTL": "0x00009981",
        },
        "cmd_max_dwords": CMD_MAX_DWORDS,
        "state_reg_writes_expected": 100,
        "pm4_dwords_expected": 266,
        "shader_payload": SHADER_PAYLOAD,
        "readback": "expect changed pixels if missing polygon raster-mode triangle state blocked primitive rasterization",
        "next_live_validation": [
            "flash-v3268-through-native-init-flash",
            "post-flash-health-check",
            "gpu-h3-raster-mode-timeout-guard",
...
def v3268_adapter_source() -> str:
    return _rewrite_v3268_text(previous.v3265_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-raster-mode-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-raster-mode-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-raster-mode-live-validation",
...
def _patch_previous_overlay_globals() -> list[tuple[Any, str, Any]]:
    replacements = {
        ...
        "STALE_V3263_ENGINE_RAMDISK_PATH": STALE_V3265_ENGINE_RAMDISK_PATH,
    }
...
def _overlay_preserved_v3268_ramdisk() -> dict[str, Any]:
    saved = _patch_previous_overlay_globals()
    try:
        overlay = previous._overlay_preserved_v3265_ramdisk()
    finally:
        _restore_previous_overlay_globals(saved)
    overlay["mode"] = "preserve-v3265-ramdisk-overlay-v3268-init-helper-engine"
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-h3-raster-mode-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-raster-mode-live-validation"
...
def _apply_v3268_overrides() -> None:
    previous._apply_v3265_overrides()
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3268_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3268_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3268_ramdisk,
