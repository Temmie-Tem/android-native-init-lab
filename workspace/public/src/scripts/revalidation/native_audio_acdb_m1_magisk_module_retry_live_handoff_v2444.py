#!/usr/bin/env python3
"""V2444 source/test M1 Magisk-module retry runner.
...
V2444 additionally hardens the post-module-reboot settle window from V2439:
...
RUN_ID = "V2444"
BUILD_TAG = "v2444-audio-acdb-m1-magisk-module-retry"
ROOT = v2429.ROOT
...
def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "module_lifecycle": {
            ...
            "v2444_post_module_reboot_retry": True,
        },
        "commands": commands,
    ...
    if not safety.get("ok"):
        blockers.append("V2444 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2444 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2444 command safety failed: {plan.get('command_safety')}")
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2444 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2444 M1 Magisk-module retry")
