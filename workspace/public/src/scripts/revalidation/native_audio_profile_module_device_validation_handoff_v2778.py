#!/usr/bin/env python3
"""V2778 live validation for the native-init audio profile module split.

This validates the V2777 behavior change: profile and route data moved into a90_audio_profile.{h,c} while the read-only `audio prereq [profile]` contract still works on device, then rolls back to V2321.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2777-audio-profile-module/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2777_audio_profile_module.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2778_AUDIO_PROFILE_MODULE_DEVICE_VALIDATION_2026-06-19.md"

CANDIDATE_VERSION = "0.9.297"
CANDIDATE_TAG = "v2777-audio-profile-module"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2778-audio-profile-module-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2778-audio-profile-module-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if (
            ...
            result["decision"] = "v2778-audio-profile-module-device-pass"
        elif result["prereq_error_observed"]:
            result["decision"] = "v2778-audio-profile-module-profile-error"
        elif not result["prereq_version_observed"]:
            result["decision"] = "v2778-audio-profile-module-missing-command"
        else:
            result["decision"] = "v2778-audio-profile-module-device-validation-incomplete"
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2778-audio-profile-module-device-validation-dry-run",
