#!/usr/bin/env python3
"""Build the S22+ M10A mkdir-dev then reboot native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M10A is the first post-M9A side-effect discriminator. It preserves the M9A
freestanding C native-init shape, adds only mkdirat("/dev", 0755), then requests
Samsung download mode. It intentionally avoids kmsg, mknodat, mount, sleep,
module insertion, configfs, USB, Android handoff, and Magisk handoff.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a_mkdir_dev_reboot_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m10a_mkdir_dev_reboot.c")
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | int | list[str]]:
    ...
    require_ok(result, "compile M10A mkdir-dev reboot init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M10A mkdir-dev reboot init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M10A mkdir-dev reboot init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", "-S", out_path])
    require_ok(readelf, "readelf M10A mkdir-dev reboot init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M10A mkdir-dev reboot init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M10A init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M10A init is not AArch64")
    ...
    if svc_count != 2:
        raise SystemExit(f"M10A init expected exactly two svc instructions, saw {svc_count}: {svc_lines}")
    ...
    if not mkdir_nr_lines:
        raise SystemExit("M10A init does not load arm64 __NR_mkdirat (34) into x8")
    if not reboot_nr_lines:
        raise SystemExit("M10A init does not load arm64 __NR_reboot (142) into x8")
    if min(reboot_nr_lines) < min(mkdir_nr_lines):
        raise SystemExit("M10A reboot syscall appears before mkdirat syscall in disassembly")

    binary = out_path.read_bytes()
    for required in REQUIRED_STRINGS:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"M10A /init missing required string: {required}")
    for forbidden in FORBIDDEN_STRINGS:
        if forbidden in binary:
            raise SystemExit(f"M10A /init contains forbidden string: {forbidden!r}")

    (build_dir / "m10a_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m10a_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m10a_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m10a_init = build_dir / "s22plus_init_m10a_mkdir_dev_reboot"
    m10a_init_info = compile_init(source, m10a_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m10a_init}"], work_dir, "replace /init with M10A init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M10A patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m10a_init):
        raise SystemExit("replaced /init does not match compiled M10A init")
    ...
    hashes = {
        ...
        "m10a_init": sha256_file(m10a_init),
    ...
    sizes = {
        ...
        "m10a_init": m10a_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M10A in-place freestanding C mkdirat-dev then reboot discriminator",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns": "mkdirat-/dev is survivable before reboot; add the next single M8A side effect",
            "no_download_mode": "the first VFS syscall or pathname access is the failing boundary",
        },
    }
