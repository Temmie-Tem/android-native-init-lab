#!/usr/bin/env python3
"""Materialize one immutable P3.16 userspace overlay intent."""
...
def create(root: Path, output: Path) -> dict:
    if output.exists() or output.is_symlink():
        raise IntentError(f"P3.16 overlay intent output exists: {output}")
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise IntentError("short P3.16 overlay intent write")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
