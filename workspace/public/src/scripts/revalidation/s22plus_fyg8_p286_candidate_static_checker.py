...
import build_s22plus_fyg8_p286_candidate as candidate  # noqa: E402
...
import s22plus_fyg8_p286_candidate_contract as contract  # noqa: E402
...
SCHEMA = "s22plus_fyg8_p286_candidate_static_checker_v1"
VERDICT = "PASS_P286_INDEPENDENT_ARTIFACT_CLOSURE_HOST_ONLY"
TARGET = contract.TARGET
DEFAULT_CANDIDATE = candidate.DEFAULT_OUT
DEFAULT_CANDIDATE_B = Path("workspace/private/outputs/s22plus_fyg8_p286/candidate-b")
...
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p286/static-check-result.json"
)
...
def verify_artifact_result(
    ...
    *,
    ...
    package_result = value.get("outputs", {}).get("packager")
    if (
        not isinstance(package_result, dict)
        or package_result.get("schema") != candidate.packager.SCHEMA
        or package_result.get("verdict") != candidate.packager.VERDICT
        or package_result.get("boot_img") != outputs["boot_img"]
        or package_result.get("boot_img_lz4") != outputs["boot_img_lz4"]
        or package_result.get("ap_tar_md5") != outputs["ap_tar_md5"]
        or package_result.get("ap_structure")
        != value.get("outputs", {}).get("ap_structure")
        or package_result.get("verified") is not True
        or package_result.get("safety", {}).get("boot_only") is not True
    ):
        raise CheckError("candidate boot-only packager result mismatch")
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="s22-p286-static-") as name:
        work = Path(name)
        tools = work / "tools"
