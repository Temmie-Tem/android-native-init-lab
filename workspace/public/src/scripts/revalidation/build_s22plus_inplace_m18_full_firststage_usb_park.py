#!/usr/bin/env python3
"""Build the S22+ M18 full-firststage USB add-back native-init candidate.
...
first-stage modules.load set minus reset/anomaly modules, then a USB tail in
the operator-directed order. The boot ramdisk gets only a text module list;
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m18_full_firststage_usb_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m18_full_firststage_park.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M18_FULL"
...
RUNTIME_MODULES_FULL_FIRSTSTAGE_USB_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_FULL_FIRSTSTAGE_USB_RAMDISK = "s22plus_m18_full_firststage_usb.modules"
...
EXPECTED_FULL_FIRSTSTAGE_USB_COUNT = 141
EXPECTED_FIRSTSTAGE_UNIQUE_AFTER_BLOCKLIST = 129
EXPECTED_FIRSTSTAGE_REQUIRED_PROVIDERS = [
    "qcom-spmi-pmic.ko",
    "spmi-pmic-arb.ko",
    "regmap-spmi.ko",
    "pmic_class.ko",
...
def is_blocked_module(name: str) -> bool:
    return name in RESET_ANOMALY_BLOCKLIST
...
def derive_full_firststage_usb(
    modules_load_lines: list[str],
    recovery_lines: list[str],
    modules_dep_lines: list[str],
) -> dict[str, object]:
    ...
    firststage_unique: list[str] = []
    firststage_duplicates: list[str] = []
    for name in modules_load_lines:
        if name in firststage_unique:
            firststage_duplicates.append(name)
            continue
        firststage_unique.append(name)

    firststage_after_blocklist = [name for name in firststage_unique if not is_blocked_module(name)]
    excluded_present = sorted({name for name in modules_load_lines if is_blocked_module(name)})
    missing_excluded = sorted(RESET_ANOMALY_BLOCKLIST - set(modules_load_lines))
    if missing_excluded:
        raise SystemExit(f"expected reset/anomaly modules missing from modules.load: {missing_excluded}")
    if len(firststage_after_blocklist) != EXPECTED_FIRSTSTAGE_UNIQUE_AFTER_BLOCKLIST:
        raise SystemExit(
            "first-stage module count after blocklist changed: "
            f"{len(firststage_after_blocklist)} != {EXPECTED_FIRSTSTAGE_UNIQUE_AFTER_BLOCKLIST}"
        )

    missing_providers = sorted(module for module in EXPECTED_FIRSTSTAGE_REQUIRED_PROVIDERS if module not in firststage_after_blocklist)
    if missing_providers:
        raise SystemExit(f"first-stage provider module missing after blocklist: {missing_providers}")

    recovery_set = set(recovery_unique)
    missing_tail_recovery = sorted(module for module in USB_TAIL_MODULES if module not in recovery_set)
    missing_tail_dep = sorted(module for module in USB_TAIL_MODULES if module not in dep_map)
    if missing_tail_recovery:
        raise SystemExit(f"USB tail module missing from modules.load.recovery: {missing_tail_recovery}")
    if missing_tail_dep:
        raise SystemExit(f"USB tail module missing from modules.dep: {missing_tail_dep}")

    ordered = list(firststage_after_blocklist)
    tail_overlap_firststage = [module for module in USB_TAIL_MODULES if module in ordered]
    for module in USB_TAIL_MODULES:
        if module not in ordered:
            ordered.append(module)

    if len(ordered) != EXPECTED_FULL_FIRSTSTAGE_USB_COUNT:
        raise SystemExit(f"full-firststage USB module count changed: {len(ordered)} != {EXPECTED_FULL_FIRSTSTAGE_USB_COUNT}")
    if len(set(ordered)) != len(ordered):
        raise SystemExit("full-firststage USB common module list contains duplicates")
    if not ordered:
        raise SystemExit("derived M18 full firststage USB is empty")

    blocked_in_ordered = sorted(name for name in ordered if is_blocked_module(name))
    if blocked_in_ordered:
        raise SystemExit(f"reset/anomaly module leaked into M18 list: {blocked_in_ordered}")

    missing_tail_deps = {
        module: [dep for dep in dep_map.get(module, []) if dep not in ordered]
        for module in USB_TAIL_MODULES
        if any(dep not in ordered for dep in dep_map.get(module, []))
    }
    non_reset_missing_tail_deps = {
        module: [dep for dep in deps if dep not in RESET_ANOMALY_BLOCKLIST]
        for module, deps in missing_tail_deps.items()
    }
    non_reset_missing_tail_deps = {module: deps for module, deps in non_reset_missing_tail_deps.items() if deps}

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_FULL_FIRSTSTAGE_USB_BUF:
        raise SystemExit(
            "M18 full firststage USB does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_FULL_FIRSTSTAGE_USB_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M18 full firststage USB basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "order_source": "modules.load first-stage order minus reset/anomaly modules, then operator USB tail",
        "modules_load_count": len(modules_load_lines),
        "modules_load_unique_count": len(firststage_unique),
        "modules_load_duplicate_count": len(firststage_duplicates),
        "modules_load_duplicates": sorted(set(firststage_duplicates)),
        "firststage_after_blocklist_count": len(firststage_after_blocklist),
        "reset_anomaly_blocklist": sorted(RESET_ANOMALY_BLOCKLIST),
        "excluded_present": excluded_present,
        "required_firststage_providers": EXPECTED_FIRSTSTAGE_REQUIRED_PROVIDERS,
        "usb_tail_modules": USB_TAIL_MODULES,
        "usb_tail_count": len(USB_TAIL_MODULES),
        "usb_tail_overlap_firststage": tail_overlap_firststage,
        "missing_tail_deps": missing_tail_deps,
        "non_reset_missing_tail_deps": non_reset_missing_tail_deps,
        "subset_count": len(ordered),
        "subset_bytes": len(subset_text.encode("utf-8")),
        "runtime_modules_full_firststage_usb_buffer": RUNTIME_MODULES_FULL_FIRSTSTAGE_USB_BUF,
        "runtime_module_name_buffer": RUNTIME_MODULE_NAME_BUF,
        "subset_max_basename_len": max(len(name) for name in ordered),
        "subset": ordered,
        "subset_recovery_positions": {module: recovery_lines.index(module) + 1 for module in ordered if module in recovery_set},
        "subset_modules_load_positions": {
            module: modules_load_lines.index(module) + 1 for module in firststage_after_blocklist if module in modules_load_lines
        },
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    full_firststage_usb = derive_full_firststage_usb(modules_load_lines, recovery_lines, modules_dep_lines)
    full_subset = list(full_firststage_usb["subset"])

    missing_ko = sorted(module for module in full_subset if module not in ko_names)
    missing_recovery = sorted(module for module in full_subset if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"full-firststage USB vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"full-firststage USB module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M18 runtime parser buffer: "
    ...
    (build_dir / "modules.load.firststage.txt").write_text("\n".join(modules_load_lines) + "\n", encoding="utf-8")
    (build_dir / "modules.softdep.dwc3.txt").write_text(expected_softdep + "\n", encoding="utf-8")
    (build_dir / MODULES_FULL_FIRSTSTAGE_USB_RAMDISK).write_text(str(full_firststage_usb["subset_text"]), encoding="ascii")
    (build_dir / "m18_full_firststage_usb.txt").write_text(str(full_firststage_usb["subset_text"]), encoding="ascii")
    return {
        ...
        "m18_full_firststage_usb": {key: value for key, value in full_firststage_usb.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M18_FULL full-firststage USB init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M18_FULL full-firststage USB init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M18_FULL full-firststage USB init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M18_FULL full-firststage USB init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M18_FULL full-firststage USB init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M18_FULL init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M18_FULL init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M18_FULL init disassembly does not contain svc")
    ...
    required_strings = [
        ...
        "raw_syscalls=1",
        "/s22plus_m18_full_firststage_usb.modules",
        "module_list=boot_ramdisk_full_firststage_usb",
        ...
        "ttyGS0",
        "S22M18FULL0001",
        "S22_NATIVE_INIT_USB_ACM_M18_FULL READY",
        "S22_NATIVE_INIT_USB_ACM_M18_FULL ACK status park",
    ]
    binary = out_path.read_bytes()
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M18_FULL /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M18_FULL init unexpectedly contains arm64 __NR_reboot (142)")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"s22plus-m5", b"modules.load.recovery", b"download"):
        if forbidden in binary:
            raise SystemExit(f"M18_FULL /init contains forbidden string: {forbidden!r}")

    (build_dir / "M18_FULL_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "M18_FULL_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "M18_FULL_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m18_full_init = build_dir / "s22plus_init_usb_acm_m18_full"
    m18_full_init_info = compile_init(source, m18_full_init, build_dir)
    ...
    subset_file = build_dir / MODULES_FULL_FIRSTSTAGE_USB_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M18_FULL subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m18_full_init}"],
        work_dir,
        "replace /init with M18_FULL init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_FULL_FIRSTSTAGE_USB_RAMDISK} {subset_file}"],
        work_dir,
        "add M18_FULL full firststage USB list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M18_FULL patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m18_full_init):
        raise SystemExit("replaced /init does not match compiled M18_FULL init")
    extracted_subset = build_dir / MODULES_FULL_FIRSTSTAGE_USB_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_FULL_FIRSTSTAGE_USB_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M18_FULL full firststage USB list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M18_FULL full firststage USB list does not match builder output")
    ...
    hashes = {
        ...
        "m18_full_firststage_usb": sha256_file(subset_file),
        "nochange_repack_boot": nochange_sha,
        "original_magisk_init": original_init_sha,
        "m18_full_init": sha256_file(m18_full_init),
    ...
    sizes = {
        ...
        "m18_full_firststage_usb": subset_file.stat().st_size,
        "m18_full_init": m18_full_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M18 full-firststage USB in-place park-based native-init: vendor first-stage modules.load minus reset/anomaly modules, then the operator USB tail",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_FULL_FIRSTSTAGE_USB_RAMDISK}",
            "module_subset": "141-module modules.load first-stage minus reset/anomaly modules, then USB tail including PHYs, dwc3-msm, usb_f_ss_acm, max77705 role chain",
            ...
            "watchdog": "not-touched-by-init-source; gh_virt_wdt/qcom_wdt_core reset path blocklisted; softdog retained because it is in first-stage modules.load",
        ...
        "hashes": hashes,
        "sizes": sizes,
        "m18_full_init": m18_full_init_info,
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_FULL_FIRSTSTAGE_USB_RAMDISK,
    ...
    (out_dir / "required_strings.txt").write_text("\n".join(m18_full_init_info["required_strings"]) + "\n", encoding="ascii")
