#!/usr/bin/env python3
"""V2438 source/test M1 Magisk-module retry runner.
...
RUN_ID = "V2438"
BUILD_TAG = "v2438-audio-acdb-m1-magisk-module-retry"
ROOT = v2429.ROOT
...
def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not safety.get("ok"):
        blockers.append("V2438 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2438 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2438 command safety failed: {plan.get('command_safety')}")
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2438 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2438 M1 Magisk-module retry")
