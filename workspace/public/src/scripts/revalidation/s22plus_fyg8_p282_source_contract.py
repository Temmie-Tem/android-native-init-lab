#!/usr/bin/env python3
"""Versioned P2.82 parent-worker and pull-up source contract."""
...
import s22plus_fyg8_p282_e1_decoder as decoder
import s22plus_fyg8_p282_trace_contract as trace_contract


CONTRACT_ID = "s22plus-fyg8-p282-prebind-child-reinit-decision-v1"
PROFILE = spec.PROFILE
RUN_ID_DOMAIN = b"S22PLUS-FYG8-P282-PREBIND-CHILD-REINIT-RUN-ID-V1\0"
INTENT_SCHEMA = "s22plus_fyg8_p282_candidate_intent_v1"
PREIMAGE_SCHEMA = "s22plus_fyg8_p282_candidate_identity_preimage_v1"
INTENT_VERDICT = "PASS_P282_CANDIDATE_INTENT_HOST_ONLY"
CONTRACT_SCHEMA = "s22plus_fyg8_p282_candidate_contract_v1"
CONTRACT_VERDICT = "PASS_P282_CANDIDATE_CONTRACT_HOST_ONLY"
USERSPACE_VERDICT = "PASS_P282_E3_USERSPACE_TWO_BUILD_REPRO_HOST_ONLY"
IMPLEMENTATION_VERDICT = "PASS_P282_REINIT_DECISION_IMPLEMENTATION_HOST_ONLY"
SOURCE_CHECK_RUN_ID = hashlib.sha256(
    b"S22PLUS-FYG8-P282-SOURCE-CHECK-V1"
).digest()[:16]
...
DEFAULT_HSPHY_MODULE = Path(
    "workspace/private/inputs/s22plus_firmware/"
    "S906NKSS7FYG8_SKC/extracted-images/vendor_dlkm/lib/modules/"
    "phy-msm-snps-hs.ko"
)
...
MATERIALIZED_FILENAMES = {
    "checkpoint_client": "s22plus_fyg8_p282_checkpoint.c",
    "runtime_wrapper": "s22plus_fyg8_p282_e3_runtime.c",
    "plan_header": "s22plus_fyg8_p282_e3_plan.h",
    "e3_runtime_include": "s22plus_fyg8_p282_e3_runtime.inc.c",
    "classifier_include": "s22plus_fyg8_p282_classifier.inc.c",
    "p260_e3_runtime_include": "s22plus_fyg8_p260_e3_runtime.inc.c",
    "trace_descriptor_header": "s22plus_fyg8_p282_trace_descriptor.h",
}
...
COMMON_SOURCE_PATHS.update(
    {
        ...
        "decoder_adapter": Path(
            "workspace/public/src/scripts/revalidation/"
            "s22plus_fyg8_p282_e1_decoder.py"
        ),
        "trace_contract": Path(
            "workspace/public/src/scripts/revalidation/"
            "s22plus_fyg8_p282_trace_contract.py"
        ),
        "p280_contract_spec": Path(
            "workspace/public/src/scripts/revalidation/"
            "s22plus_fyg8_p280_contract_spec.py"
        ),
        "p280_trace_contract": Path(
            "workspace/public/src/scripts/revalidation/"
...
def _validate_runtime_authority_source(include: bytes) -> None:
    for name, expected_value in spec.RUNTIME_EXTERNAL_CONSTANTS:
        ...
        if len(matches) != 1:
            raise SourceContractError(
                f"P2.82 runtime constant {name} cardinality drifted"
            )
        ...
        if actual_value != expected_value:
            raise SourceContractError(
                f"P2.82 runtime constant {name} value drifted"
            )
    ...
    for forbidden in (
        ...
        if forbidden in include:
            raise SourceContractError(
                "P2.82 runtime broadened beyond exact Kprobe authority"
            )
    ...
    if tracefs_paths != set(spec.TRACEFS_ABSOLUTE_PATHS):
        raise SourceContractError("P2.82 tracefs absolute path set drifted")

    for name, token, expected_count in spec.RUNTIME_OPERATION_TOKENS:
        count = include.count(token.encode("ascii"))
        if count != expected_count:
            raise SourceContractError(
                f"P2.82 runtime operation {name} cardinality drifted"
            )
...
def _validate_trace_descriptor_source(header: bytes) -> None:
    for name, value in spec.RUNTIME_STRING_CONSTANTS:
        ...
        if header.count(token) != 1:
            raise SourceContractError(
                f"P2.82 generated runtime string {name} drifted"
            )
    if b'"host\\n"' in header:
        raise SourceContractError("P2.82 generated a forbidden host role")
    for token in (
        b"#define P282_CLASSIFIER_CONTRACT_DEFINED 1",
        b"#define P282_ROLE_EVENT_COUNT 4U",
        b"#define P282_CYCLE_EVENT_COUNT 14U",
        ...
        if header.count(token) != 1:
            raise SourceContractError(
                "P2.82 generated trace/classifier contract drifted"
            )
...
def _transform_checkpoint(data: bytes) -> bytes:
    value = _replace_exact(
        ...
        label="P2.82 checkpoint descriptor table",
    )
    value = _replace_exact(
        value,
        p260._checkpoint_detail_helper(),
        _checkpoint_detail_helper(),
        label="P2.82 checkpoint detail validator",
    )
    anchor = (
    ...
    return _replace_exact(
        ...
        label="P2.82 progress-detail publisher",
    )
...
def _transform_patch(data: bytes) -> bytes:
    value = _replace_exact(
        ...
        label="P2.82 kernel descriptor tables",
    )
    value = _replace_exact(
        value,
        p260._kernel_detail_validator(),
        _kernel_detail_validator(),
        label="P2.82 kernel detail validator",
    )
...
def _transform_runtime(data: bytes) -> bytes:
    value = _replace_exact(
        ...
        b'#include "s22plus_fyg8_p282_e3_plan.h"',
        label="P2.82 runtime plan include",
    )
    value = _replace_exact(
        value,
        b'#include "s22plus_fyg8_p260_e3_runtime.inc.c"',
        b'#include "s22plus_fyg8_p282_e3_runtime.inc.c"',
        label="P2.82 runtime include",
    )
    return _replace_exact(
        value,
        b"    p260_e3_run();\n",
        b"    p282_e3_run();\n",
        label="P2.82 runtime handoff",
    )
...
def source_bytes(root: Path) -> dict[str, bytes]:
    ...
    if set(result) != SOURCE_KEYS:
        raise SourceContractError("P2.82 source inventory changed")
    return result
...
def _generated_semantics(
    ...
    if generated["plan"] != historical["plan"]:
        raise SourceContractError("P2.82 changed the P2.60 module plan")
    ...
    if any(runtime.count(token) != 1 for token in required_runtime):
        raise SourceContractError("P2.82 runtime handoff drifted")
    required_checkpoint = (
    ...
    if any(
        ...
        raise SourceContractError("P2.82 checkpoint contract drifted")
    required_patch = (
    ...
    if any(
        ...
        raise SourceContractError("P2.82 kernel contract drifted")
    if b"p260_e3_run();" in runtime:
        raise SourceContractError("P2.82 retained the P2.60 handoff")
    ...
    if (
        ...
        raise SourceContractError("P2.82 linked stage geometry drifted")
    return {
...
def implementation_result(root: Path) -> dict[str, Any]:
    ...
    if first != second:
        raise SourceContractError("P2.82 generation is not deterministic")
    semantics = _generated_semantics(first, historical)
    source = source_bytes(root)
    with tempfile.TemporaryDirectory(prefix="s22-p282-") as temporary:
    ...
    return {
        "schema": "s22plus_fyg8_p282_implementation_v1",
...
def validate_reachable_records(run_id: bytes) -> dict[str, Any]:
    ...
    def check_progress(stage: int, detail: int) -> None:
        ...
        if (
            ...
            raise SourceContractError(
                "P2.82 progress record decode drifted"
            )
        checked += 1
    ...
    if checked != PROGRESS_DETAIL_VARIANTS + PROGRESS_TUPLE_VARIANTS:
        raise SourceContractError("P2.82 progress variant count mismatch")
    ...
    if result["reachable_slot_variants"] != REACHABLE_VARIANTS:
        raise SourceContractError("P2.82 reachable total drifted")
    return result
...
def audit_linked_tables(actual: dict[str, bytes]) -> dict[str, Any]:
    ...
    if actual != expected:
        raise SourceContractError("P2.82 linked descriptor tables differ")
    return {
