#!/usr/bin/env python3
"""P2.94 retained model with value-telemetry position semantics."""
...
SCHEMA = "s22plus_fyg8_p294_value_telemetry_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P2.94 profile: {profile}")
    if repair.is_publication_error_detail(outcome, detail):
        ...
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P2.94 publication-error record is at the wrong position"
            )
        return
    try:
        spec.validate_slot(
...
def base_context() -> Iterator[None]:
    with base.base_context():
        ...
        replacements = {
            "SCHEMA": SCHEMA,
            "spec": spec,
            "PROFILE_POSITION_SEQUENCES": {
                PROFILE: spec.POSITION_SEQUENCE
            },
            "PROFILE_TERMINAL_POSITIONS": {
                PROFILE: spec.TERMINAL_POSITION
            },
            "PROFILE_TERMINALS": {PROFILE: spec.TERMINAL_STAGE},
            "_validate_position_semantics": _validate_position_semantics,
        }
        previous = {
            name: getattr(inherited, name) for name in replacements
        }
