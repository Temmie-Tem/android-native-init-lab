#!/usr/bin/env python3
"""Build V2897 native-init candidate with a Bad-Apple-scale mono1 stream ceiling."""
...
CYCLE = "V2897"
INIT_VERSION = "0.10.33"
INIT_BUILD = "v2897-video-long-mono1-stream"
BUILD_TAG = INIT_BUILD
DECISION = "v2897-video-long-mono1-stream-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2897_VIDEO_LONG_MONO1_STREAM_SOURCE_BUILD_2026-06-19.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2897_video_long_mono1_stream.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2897_video_long_mono1_stream"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2897_video_long_mono1_stream.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v462_video_long_mono1_stream"
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2897 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    markers = manifest.get("v2897_marker_strings", [])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "video-long-mono1-stream-candidate",
        "parent_test_artifact": "v2896-video-mono1-stream-live",
        "long_mono1_stream": {
            "version": 1,
            "source_unit": "V2897",
            "command": "video stream --manifest PATH --video-only --present pageflip",
            "formats": ["xbgr8888-raw-stride", "gray8", "mono1"],
            "previous_max_frames": 600,
            "max_frames": 7200,
            "v2898_validation_frames": 601,
            "mono1_payload": "one bit per pixel, MSB-first, stride equals ceil(width / 8) for packed fixtures",
            "mono1_expand_target": "XBGR8888 KMS dumb buffer",
            "large_asset_policy": "pre-rendered frames stay in the SHA-addressed SD-card cache, not inside the boot image",
            "pageflip_telemetry": "flip_delta_count/min/max/avg/target_us from DRM page-flip event timestamps",
            "live_validation": "pending",
        },
        "v2897_marker_strings": marker_strings,
    ...
    (OUT_DIR / "video-long-mono1-stream-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "video-long-mono1-stream-candidate",
