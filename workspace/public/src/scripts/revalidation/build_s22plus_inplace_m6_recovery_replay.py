#!/usr/bin/env python3
"""Build the S22+ M6 recovery-module-replay USB-ACM native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M6 keeps the freestanding raw-syscall PID1 runtime from M5, but fixes the module
bring-up method: it does not inject a 26-module symbol closure into boot.
Instead, the runtime reads stock vendor_boot's /lib/modules/modules.load.recovery
and finit_module()s that ordered list from /lib/modules.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m6_recovery_replay_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m6_recovery_replay.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M6"
...
RUNTIME_MODULES_LOAD_RECOVERY_BUF = 32768
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    missing_ko = sorted(module for module in REQUIRED_RECOVERY_MODULES if module not in ko_names)
    missing_recovery = sorted(module for module in REQUIRED_RECOVERY_MODULES if module not in recovery_lines)
    if missing_ko:
    ...
    if recovery_bytes >= RUNTIME_MODULES_LOAD_RECOVERY_BUF:
        raise SystemExit(
            "modules.load.recovery does not fit M6 runtime buffer: "
            f"{recovery_bytes} >= {RUNTIME_MODULES_LOAD_RECOVERY_BUF}"
        )
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M6 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in REQUIRED_RECOVERY_MODULES}
    ...
    return {
        ...
        "runtime_modules_load_recovery_buffer": RUNTIME_MODULES_LOAD_RECOVERY_BUF,
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M6 recovery-replay init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M6 recovery-replay init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M6 recovery-replay init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M6 recovery-replay init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M6 recovery-replay init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M6 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M6 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M6 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M6 /init: {required}")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"s22plus-m5"):
        if forbidden in binary:
            raise SystemExit(f"M6 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m6_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m6_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m6_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m6_init = build_dir / "s22plus_init_usb_acm_m6"
    m6_init_info = compile_init(source, m6_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m6_init}"], work_dir, "replace /init with M6 init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M6 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m6_init):
        raise SystemExit("replaced /init does not match compiled M6 init")
    ...
    manifest = {
        ...
        "purpose": "M6 in-place USB-ACM native-init using vendor_boot modules.load.recovery replay",
        "safety": {
            "boot_only": True,
