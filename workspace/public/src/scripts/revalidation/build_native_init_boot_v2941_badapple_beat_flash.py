#!/usr/bin/env python3
"""Build V2941 native-init candidate with Bad Apple BEAT FLASH HUD."""
...
CYCLE = "V2941"
INIT_VERSION = "0.10.47"
INIT_BUILD = "v2941-badapple-beat-flash"
BUILD_TAG = INIT_BUILD
DECISION = "v2941-badapple-beat-flash-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2941_BADAPPLE_BEAT_FLASH_SOURCE_BUILD_2026-06-20.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2941_badapple_beat_flash.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2941_badapple_beat_flash"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2941_badapple_beat_flash.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v477_badapple_beat_flash"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.47 (v2941-badapple-beat-flash)",
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2941 boot-image markers: {missing}")
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "badapple-beat-flash-candidate",
    ...
    (OUT_DIR / "badapple-beat-flash-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "badapple-beat-flash-candidate",
