#!/usr/bin/env python3
"""Build the V2174 Wi-Fi connect test image.

V2174 is a test-boot candidate on top of the promoted V2169 transport contract.
It adds native-init `wifi connect [profile]` for bounded association/carrier
validation without DHCP, routes, external ping, boot autoconnect, or raw
credential logging.
"""
...
OUT_DIR = workspace_private_build_path("native-init", "v2174-wifi-urandom-connect-test-boot")
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2174_WIFI_URANDOM_CONNECT_SOURCE_BUILD_2026-06-08.md"
)
...
INIT_BINARY = OUT_DIR / "init_v2174_wifi_urandom_connect"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2174_wifi_urandom_connect.cpio"
...
def configure_base() -> None:
    ...
    replacements = {
        ...
        "--cycle-label": "v2174",
        "--init-version": "0.9.251",
        "--init-build": "v2174-wifi-urandom-connect",
        "--out-dir": str(OUT_DIR),
        "--init-binary": str(INIT_BINARY),
        "--helper-binary": str(OUT_DIR / "a90_android_execns_probe_v427_wifi_urandom_connect"),
        "--ramdisk-cpio": str(RAMDISK_CPIO),
        "--boot-image": str(BOOT_IMAGE),
        "--wifi-test-klog-prefix": "A90v2174",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2174.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2174.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2174.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2174-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2174.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2174-supervisor.pid",
...
def normalize_manifest_axes() -> None:
    ...
    manifest["candidate_tag"] = "v2174-wifi-urandom-connect"
    manifest["parent_baseline"] = "v2169-transport-contract"
    manifest["version_axes"] = {
        "candidate_tag": "v2174-wifi-urandom-connect",
        "parent_baseline": "v2169-transport-contract",
        "helper_version": "helper-v427",
        "run_id": "V2174",
        "note": "V2174 is a test-boot candidate for Wi-Fi association/carrier, not a promoted baseline.",
    }
