#!/usr/bin/env python3
"""V2673 Android handoff wrapper for V2672 lower-hidden-node ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2672 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2672
...
The own-process send path is the V2672 lower-hidden-node preload:
...
The V2672 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
ROOT = v2672.ROOT
RUN_ID = "V2673"
BUILD_TAG = "v2673-audio-acdb-lower-hidden-node-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2673_AUDIO_ACDB_LOWER_HIDDEN_NODE_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2673-acdb-lower-hidden-node-setcal-capture-{stamp}"
...
def build_v2672_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2672_build_root,
        manifest=args.v2672_manifest_path,
        report=v2672.DEFAULT_REPORT,
        clang=v2672.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2672.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    payload = v2672.make_payload(build_args)
    args.v2672_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2672_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2672_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    required_target_types = list(v2672.TARGET_CAL_TYPES)
    setcal_contract_ok = bool(
        ...
        and contract.get("get_commands") == {str(key): value for key, value in v2672.TARGET_GET_COMMANDS.items()}
        and required.get("helper_imports_init_v3")
        and required.get("helper_imports_arm_capture")
        and required.get("helper_imports_lower_runner")
        and required.get("helper_calls_init_v3_open_audconf")
        and required.get("helper_arms_after_init_before_lower")
        ...
        and required.get("preload_returns_to_init_without_send")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2672_artifacts:
        build_v2672_artifacts(args)
    manifest = read_v2672_manifest(args.v2672_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2673-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2673-preflash-native-bridge-unavailable"
...
def read_v2672_jsonl(path: Path) -> list[dict[str, Any]]:
    """Read V2672 event JSONL, tolerating C-emitted unquoted hex scalars."""
    if not path.exists():
        return []
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    lower_path = artifact_dir / "acdb-v2672-lower-hidden-events.jsonl"
    helper_path = artifact_dir / "acdb-v2672-lower-hidden-helper-events.jsonl"
    setcal_events = [
        event for event in read_jsonl(setcal_path)
        if event.get("event") == "setcal_capture"
    ]
    lower_events = _event_rows(read_v2672_jsonl(lower_path), "v2672_lower_hidden")
    helper_events = _event_rows(read_v2672_jsonl(helper_path), "v2672_lower_hidden_helper")
    ...
    lower_return_codes = helper_codes_by_stage.get("lower_hidden_nodes_return", [])
    ...
    common_hook_skipped = (
        ...
        and "return_to_init_v3_no_arm_no_send" in lower_stages
    )
    ...
    lower_runner_entered = bool(
        "loader_base_resolved" in lower_stages
    ...
    if real_set_events:
        classification = "v2673-boundary-violation-real-audio-set-passthrough"
    elif target_complete and target_payloads_dumped:
        classification = "v2673-lower-hidden-custom-setcal-captured"
    elif target_rows:
        classification = "v2673-lower-hidden-custom-setcal-partial"
    elif lower_fake_set_seen and not rows:
        classification = "v2673-lower-hidden-fake-set-called-no-setcal-records"
    elif lower_sequence_complete and not rows:
        classification = "v2673-lower-hidden-sequence-complete-no-setcal"
    elif lower_get_seen and not rows:
        classification = "v2673-lower-hidden-get-no-setcal"
    elif lower_runner_entered and not helper_started_lower and not rows:
        classification = "v2673-init-common-skip-loader-base-then-helper-sigsegv-before-arm"
    elif lower_runner_entered and not rows:
        classification = "v2673-lower-hidden-runner-entered-no-setcal"
    elif helper_started_lower and not rows:
        classification = "v2673-helper-started-lower-runner-no-lower-events"
    elif lower_runner_missing:
        classification = "v2673-helper-lower-runner-missing"
    elif common_hook_skipped and not rows:
        classification = "v2673-init-common-skip-no-lower-runner"
    elif rows:
        classification = "v2673-setcal-records-no-target-custom-topology"
    else:
        classification = f"v2673-{base_summary.get('classification', 'no-setcal-records')}"

    success = classification == "v2673-lower-hidden-custom-setcal-captured"
    partial_success = classification in {
        "v2673-lower-hidden-custom-setcal-partial",
        "v2673-setcal-records-no-target-custom-topology",
        "v2673-lower-hidden-fake-set-called-no-setcal-records",
        "v2673-lower-hidden-sequence-complete-no-setcal",
        "v2673-lower-hidden-get-no-setcal",
        "v2673-lower-hidden-runner-entered-no-setcal",
        "v2673-init-common-skip-loader-base-then-helper-sigsegv-before-arm",
        "v2673-helper-started-lower-runner-no-lower-events",
        "v2673-helper-lower-runner-missing",
        "v2673-init-common-skip-no-lower-runner",
    }
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2673-acdb-lower-hidden-node-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2672_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2672 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2673 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2672_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2673-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2672_artifacts", {})
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2672 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2673-init-common-skip-no-lower-runner":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2672 common-topology skip hook ran during init;",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2672-artifacts", action="store_true")
    parser.add_argument("--v2672-build-root", type=Path, default=v2672.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2672-manifest-path", type=Path, default=v2672.DEFAULT_MANIFEST)
