#!/usr/bin/env python3
"""Build P3.05 userspace twice against the fixed P3.00 Image."""
...
import s22plus_fyg8_p305_candidate_contract as candidate_contract
import s22plus_fyg8_p305_overlay_contract as contract


SCHEMA = "s22plus_fyg8_p305_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p305/userspace")
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if (
        ...
        raise BuildError("unsupported P3.05 userspace profile or parent contract")
    return VERDICT
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.05 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p305-userspace-{label}-") as name:
            directory = Path(name)
            result = parent._compile(  # noqa: SLF001
                root, directory, exact, materialized, tools
            )
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.05 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
