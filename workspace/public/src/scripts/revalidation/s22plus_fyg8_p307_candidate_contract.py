#!/usr/bin/env python3
"""Verify P3.07 against exact P3.05 and the fixed P3.00 Image."""

from pathlib import Path

import s22plus_fyg8_p305_candidate_contract as parent
...
def verify(root: Path, source: Path, intent_path: Path, patch_path: Path):
    if source != root / overlay.PARENT_SOURCE:
        raise ContractError("P3.07 parent source path differs")
    if patch_path != root / overlay.PARENT_PATCH:
        raise ContractError("P3.07 parent patch path differs")
