#!/usr/bin/env python3
"""P3.00 retained model with event-ingress/IRQ semantics."""
...
SCHEMA = "s22plus_fyg8_p300_event_ingress_irq_telemetry_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P3.00 profile: {profile}")
    if repair.is_publication_error_detail(outcome, detail):
        ...
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P3.00 publication-error record is at the wrong position"
            )
        return
    try:
        spec.validate_slot(
