#!/usr/bin/env python3
"""Build P3.15 userspace twice against the fixed Image."""
...
SCHEMA = "s22plus_fyg8_p315_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p315/userspace")
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if (
        ...
        raise BuildError("P3.15 userspace profile/source contract differs")
    return VERDICT
...
def _source_contract(exact: dict) -> dict:
    return {
        ...
        "frozen_p314_baseline": exact["frozen_p314_baseline"],
        "design_requirements_sha256": exact["design_requirements_sha256"],
        "restart_source_geometry": exact["restart_source_geometry"],
        "cross_gate_audit": exact["cross_gate_audit"],
        "runtime_fixture": exact["runtime_fixture"],
        "tracefs_abi": exact["tracefs_abi"],
        "prepackaging_closure": exact["prepackaging_closure"],
        "matrix_fixture": exact["matrix_fixture"],
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.15 userspace output exists: {output}")
    ...
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(
            prefix=f"s22-p315-userspace-{label}-"
        ) as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.15 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
