#!/usr/bin/env python3
"""V2446 source/test M1 Magisk-module post-reboot wait-budget runner.
...
V2446 hardens the post-module-reboot settle window from V2445: after the module
activation reboot, `adb wait-for-device` uses a dedicated longer budget instead
of the generic ADB command timeout. V2445 measured ADB returning about 206s
after the module-activation reboot; the V2446 default is 300s.
"""
...
def post_module_reboot_settle_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "v2446_observed_v2445_adb_return_sec": 206.359,
        "classification": "bounded adb reacquire plus Magisk-root retry after module activation reboot with V2446 extended wait budget",
    }
...
def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "module_lifecycle": {
            ...
            "v2446_post_module_adb_wait_budget": True,
        },
        "commands": commands,
    ...
    if not safety.get("ok"):
        blockers.append("V2446 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2446 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2446 command safety failed: {plan.get('command_safety')}")
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2446 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2446 M1 Magisk-module retry")
