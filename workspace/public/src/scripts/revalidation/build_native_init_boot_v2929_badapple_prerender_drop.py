#!/usr/bin/env python3
"""Build V2929 native-init candidate with Bad Apple pre-render late-frame drop skip."""
...
CYCLE = "V2929"
INIT_VERSION = "0.10.43"
INIT_BUILD = "v2929-badapple-prerender-drop"
BUILD_TAG = INIT_BUILD
DECISION = "v2929-badapple-prerender-drop-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2929_BADAPPLE_PRERENDER_DROP_SOURCE_BUILD_2026-06-20.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2929_badapple_prerender_drop.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2929_badapple_prerender_drop"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2929_badapple_prerender_drop.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v473_badapple_prerender_drop"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.43 (v2929-badapple-prerender-drop)",
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2929 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    return "\n".join([
        ...
        "- Parent live unit: V2927/V2929 probing showed the long preview collapsed when late drops still paid render cost; V2929 keeps the offset and skips late payloads before render/KMS work.",
        "",
        "## Included Delta",
        "",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "badapple-prerender-drop-candidate",
    ...
    (OUT_DIR / "badapple-prerender-drop-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "badapple-prerender-drop-candidate",
