#!/usr/bin/env python3
"""Run the complete P3.14 value-position matrix through Process-v2."""
...
SCHEMA = "s22plus_fyg8_p314_value_position_matrix_v1"
VERDICT = "PASS_P314_251450_REAL_PROCESS_V2_MATRIX_HOST_ONLY"
...
def _prefixes(run_id: bytes) -> tuple[bytes, ...]:
    ...
    if len(rows) != len(spec.POSITIONS):
        raise MatrixError("P3.14 progress prefix extent differs")
    return tuple(rows)
...
def _force_apply(
    ...
    *,
    ...
    """Encode one test cell without weakening the production semantic gate."""
    ...
    if active.generation + 1 != generation or active.outcome != model.OUTCOME_PROGRESS:
        raise MatrixError("P3.14 matrix prefix generation differs")
    position = spec.position_for_generation(generation)
    ...
    *,
    ...
    try:
        ...
        if persisted.get("accepted_by_adapter") is not False:
            raise MatrixError("P3.14 rejected cell persistence differs")
    else:
        persisted = json.loads(
        ...
        if (
            ...
            raise MatrixError("P3.14 accepted cell record geometry differs")
        ...
        if observed and family == "a":
            ...
            if semantics.get("detail_kind") != "p314-cycle-state-speed":
                raise MatrixError("P3.14 A semantics differ")
    ...
    if observed is not expected:
        raise MatrixError(
            f"P3.14 matrix decision differs: {family}/{generation}/{detail:#x}"
        )
...
def audit(root: Path | None = None) -> dict[str, Any]:
    ...
    if gates.get("verified") is not True:
        raise MatrixError("P3.14 encoder gates are not closed")
    ...
    if (
        ...
        raise MatrixError("P3.14 matrix cardinality differs")
    return {
        "schema": SCHEMA,
        "verdict": VERDICT,
