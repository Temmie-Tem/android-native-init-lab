#!/usr/bin/env python3
"""Build the S22+ M7 USB-subset USB-ACM native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M7 keeps the freestanding raw-syscall PID1 runtime from M6, but fixes M6's
over-broad module bring-up method. The builder derives a USB bring-up subset
from stock vendor_boot's modules.load.recovery + modules.dep, removes
watchdog/reset-prone modules, injects only that small text list into the boot
ramdisk, and leaves all module binaries in stock vendor_boot /lib/modules.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m7_usb_subset_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m7_usb_subset.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M7"
...
RUNTIME_MODULES_USB_SUBSET_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_USB_SUBSET_RAMDISK = "s22plus_m7_usb_subset.modules"
...
def derive_usb_subset(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    ...
    if not ordered:
        raise SystemExit("derived M7 USB subset is empty")
    if any(name in WATCHDOG_BLOCKLIST for name in ordered):
        raise SystemExit("watchdog module survived M7 USB subset blocklist")
    if any("audio" in name.lower() or "snd" in name.lower() for name in ordered):
        raise SystemExit("audio module survived M7 USB subset blocklist")
    ...
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_USB_SUBSET_BUF:
        raise SystemExit(
            "M7 USB subset does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_USB_SUBSET_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M7 USB subset basename exceeds runtime parser buffer: {too_long[:5]}")
    ...
    return {
        ...
        "runtime_modules_usb_subset_buffer": RUNTIME_MODULES_USB_SUBSET_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    usb_subset = derive_usb_subset(recovery_lines, modules_dep_lines)
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M7 runtime parser buffer: "
    ...
    (build_dir / MODULES_USB_SUBSET_RAMDISK).write_text(str(usb_subset["subset_text"]), encoding="ascii")
    (build_dir / "m7_usb_subset.txt").write_text(str(usb_subset["subset_text"]), encoding="ascii")
    return {
        ...
        "m7_usb_subset": {key: value for key, value in usb_subset.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M7 usb-subset init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M7 usb-subset init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M7 usb-subset init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M7 usb-subset init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M7 usb-subset init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M7 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M7 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M7 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M7 /init: {required}")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"s22plus-m5", b"modules.load.recovery"):
        if forbidden in binary:
            raise SystemExit(f"M7 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m7_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m7_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m7_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m7_init = build_dir / "s22plus_init_usb_acm_m7"
    m7_init_info = compile_init(source, m7_init, build_dir)
    ...
    subset_file = build_dir / MODULES_USB_SUBSET_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M7 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m7_init}"],
        work_dir,
        "replace /init with M7 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_USB_SUBSET_RAMDISK} {subset_file}"],
        work_dir,
        "add M7 USB subset list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M7 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m7_init):
        raise SystemExit("replaced /init does not match compiled M7 init")
    extracted_subset = build_dir / MODULES_USB_SUBSET_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_USB_SUBSET_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M7 USB subset list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M7 USB subset list does not match builder output")
    ...
    sizes = {
        ...
        "m7_usb_subset": subset_file.stat().st_size,
        "m7_init": m7_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M7 in-place USB-ACM native-init using vendor_boot recovery-order USB subset minus watchdogs",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_USB_SUBSET_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_USB_SUBSET_RAMDISK,
