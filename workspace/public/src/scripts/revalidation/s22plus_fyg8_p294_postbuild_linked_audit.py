#!/usr/bin/env python3
"""P2.94 post-build proof with linked telemetry and host closure."""
...
import s22plus_fyg8_p294_linked_audit as linked
...
IMPLEMENTATION_ID = "s22plus-fyg8-p294-linked-value-telemetry-v1"
...
def host_validator_tu(patch: bytes) -> bytes:
    ...
    if source.count(assignment) != 1 or source.count(b"request.detail = 0;") != 1:
        raise AuditError("P2.94 host validator detail injection differs")
...
def host_native_exhaustive(root: Path) -> dict[str, Any]:
    patch = p294.generate(root)["patch"]
    result = run_host_validator_tu(host_validator_tu(patch))
    result["identity_patch"] = p294.receipt(patch)
    result["production_validator_source"] = p294.receipt(
...
def linked_table_data(args, result: dict[str, Any]) -> dict[str, Any]:  # noqa: ANN001
    ...
    vmlinux = repro.candidate_contract.stable_read(
        directory / "vmlinux",
        "P2.94 direct-ELF linked vmlinux",
    ...
    if receipt != expected_receipt:
        raise AuditError("P2.94 linked vmlinux changed after reproducibility audit")
    proof = verify_linked_table_data(vmlinux, p294.linked_table_bytes())
...
def check(args):  # noqa: ANN001, ANN201
    ...
    if proof.get("verdict") != closure.VERDICT or proof.get("verified") is not True:
        raise AuditError("P2.94 telemetry postbuild closure differs")
    linked_result = result["linked_audit"]
    linked_result["postbuild_audit"]["value_telemetry"] = proof
    linked_result["source_contract_validator"]["accept_to_resume_pending_postbuild"] = False
    return result
