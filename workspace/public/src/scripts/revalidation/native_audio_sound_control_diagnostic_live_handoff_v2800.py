#!/usr/bin/env python3
"""V2800 live handoff for native-init audio sound-control diagnostics.
...
V2800 re-runs the V2799 candidate with extra pre/post ADSP and sound diagnostics because V2799 timed out before sound_control became ready and did not reach SET-cal.
"""
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2800_AUDIO_SOUND_CONTROL_DIAGNOSTIC_LIVE_2026-06-19.md"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2800-audio-sound-control-diagnostic-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2800-audio-sound-control-diagnostic-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2800-audio-sound-control-diagnostic-start-failed-before-rollback"
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2800-audio-sound-control-diagnostic-worker-failed-before-rollback"
        ...
        result["decision"] = "v2800-audio-sound-control-diagnostic-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2800-audio-sound-control-diagnostic-live-blocked")
        if result["decision"] == "v2800-audio-sound-control-diagnostic-live-started":
            result["decision"] = "v2800-audio-sound-control-diagnostic-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2800-audio-sound-control-diagnostic-live-dry-run",
