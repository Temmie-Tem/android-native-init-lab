#!/usr/bin/env python3
"""Verify P3.05 against exact P3.04 and the fixed P3.00 Image."""

from pathlib import Path

import s22plus_fyg8_p304_candidate_contract as parent
...
def verify(root: Path, source: Path, intent_path: Path, patch_path: Path):
    if source != root / overlay.PARENT_SOURCE:
        raise ContractError("P3.05 parent source path differs")
    if patch_path != root / overlay.PARENT_PATCH:
        raise ContractError("P3.05 parent patch path differs")
