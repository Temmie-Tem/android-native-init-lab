#!/usr/bin/env python3
"""Independently verify two candidate-bound P2.86 clean kernel builds."""
...
import s22plus_fyg8_p286_build as build  # noqa: E402
import s22plus_fyg8_p286_candidate_contract as candidate_contract  # noqa: E402


SCHEMA = "s22plus_fyg8_p286_build_repro_check_v1"
VERDICT = "PASS_P286_TWO_CLEAN_BUILD_REPRO_AND_LINKED_AUDIT_HOST_ONLY"
...
P286_QUALIFICATION_MODULE = "s22plus_fyg8_p286_pre_lto_qualification"
P286_QUALIFICATION_PROVENANCE_KEY = "p286_pre_lto_qualification"
DEFAULT_BUILD_A = Path("workspace/private/outputs/s22plus_fyg8_p286/artifacts-a")
DEFAULT_BUILD_B = Path("workspace/private/outputs/s22plus_fyg8_p286/artifacts-b")
...
LINKED_VALIDATOR_ADAPTERS = {
    ...
    P286_SOURCE_CONTRACT_ID: "s22plus_fyg8_p286_linked_audit",
}
...
def verify_p286_qualification_file(
    value: Any,
    exact_contract: dict[str, Any],
    *,
    intent_path: Path,
    ...
    return _verify_versioned_qualification_file(
        ...
        intent_path=intent_path,
        patch_path=patch_path,
        source_contract_id=P286_SOURCE_CONTRACT_ID,
        qualification_module=P286_QUALIFICATION_MODULE,
        contract_attribute="p286",
        label="P2.86",
        root=root,
    )
...
def _bundle_pre_lto_qualification(
    ...
    *,
    ...
    verify = {
        P282_SOURCE_CONTRACT_ID: verify_p282_qualification_file,
        P284_SOURCE_CONTRACT_ID: verify_p284_qualification_file,
        P286_SOURCE_CONTRACT_ID: verify_p286_qualification_file,
    }[source_contract_id]
    provenance_key = {
        P282_SOURCE_CONTRACT_ID: P282_QUALIFICATION_PROVENANCE_KEY,
        P284_SOURCE_CONTRACT_ID: P284_QUALIFICATION_PROVENANCE_KEY,
        P286_SOURCE_CONTRACT_ID: P286_QUALIFICATION_PROVENANCE_KEY,
    }[source_contract_id]
    return verify(
...
def _classify_wrapper_result(result: dict[str, Any]) -> str:
    ...
    if (
        ...
        and result.get("p286_build_pass") is True
    ...
    legacy_false_negative = (
        ...
        and result.get("p286_build_pass") is False
...
def verify_bundle(
    ...
    *,
    ...
    if result.get("p286_candidate_contract") != exact_contract:
        raise CheckError("P2.86 build does not embed the exact candidate contract")
...
def audit_linked(
    ...
    with tempfile.TemporaryDirectory(prefix="s22-p286-linked-") as temporary:
        staged = {}
        for name, data in captured.items():
