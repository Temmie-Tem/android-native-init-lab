#!/usr/bin/env python3
"""Create one P2.98 candidate intent through the versioned contract."""
...
SCHEMA = p298.INTENT_SCHEMA
PREIMAGE_SCHEMA = p298.PREIMAGE_SCHEMA
VERDICT = p298.INTENT_VERDICT
RUN_ID_DOMAIN = p298.RUN_ID_DOMAIN
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p298/intent")
TARGET = base.TARGET
PROFILE = p298.PROFILE
PROFILE_NUMBER = base.PROFILE_NUMBER
SUPPORTED_PROFILES = base.SUPPORTED_PROFILES
DEFAULT_SOURCE = p298.DRIVER_SOURCE_REFERENCE
DEFAULT_BASE_PATCH = base.DEFAULT_BASE_PATCH
DEFCONFIG = base.DEFCONFIG
BASE_FILES = dict(base.BASE_FILES)
IntentError = base.IntentError
_INHERITED_BUILD_PATCH = base.build_patch
_INHERITED_CONTRACT_IDS = base.candidate_contract_ids
_INHERITED_SELECTION = base.selected_source_contract_for_candidate
_SELECTOR_SELECT = selector.select
_SELECTOR_CONTRACT_IDS = selector.contract_ids
RUN_ID_DOMAINS = {**base.RUN_ID_DOMAINS, "E2": RUN_ID_DOMAIN}
SUPERSEDED_FOR_NEW_CANDIDATES = {
    **base.SUPERSEDED_FOR_NEW_CANDIDATES,
    base.p296.CONTRACT_ID: p298.CONTRACT_ID,
}
...
def build_patch(
    ...
    if profile != p298.PROFILE:
        raise IntentError(f"unsupported P2.98 candidate profile: {profile}")
    replacements = (
        (
            (
                "+CONFIG_S22PLUS_FYG8_E1_RUN_ID_HEX="
                f'"{p298.SOURCE_CHECK_RUN_ID.hex()}"'
            ).encode("ascii"),
            (
                "+CONFIG_S22PLUS_FYG8_E1_RUN_ID_HEX="
                f'"{run_id.hex()}"'
        ...
        (
            (
                ...
                f'"{p298.SOURCE_CHECK_UNSAT_TAG.hex()}"'
            ).encode("ascii"),
            (
                "+CONFIG_S22PLUS_FYG8_E1_UNSAT_TAG_HEX="
                f'"{unsat_tag.hex()}"'
    ...
    if counts != (1, 1):
        raise IntentError("P2.98 candidate config source binding differs")
    value = base_patch
...
def selected_source_contract_for_candidate(
    ...
    if source_contract_id == p298.CONTRACT_ID:
        try:
        ...
        return selector.SelectedSourceContract(
            module=p298,
            contract=contract,
            implementation_verdict=p298.IMPLEMENTATION_VERDICT,
            source_check_run_id=p298.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p298.USERSPACE_VERDICT,
        )
...
def _selector_contract_ids() -> tuple[str, ...]:
    return (*_SELECTOR_CONTRACT_IDS(), p298.CONTRACT_ID)
