#!/usr/bin/env python3
"""V2657 Android handoff wrapper for V2656 custom-topology ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2656 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2656
...
The own-process send path is the V2553 full-manifest helper selected by V2656:
...
The V2656 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
import build_android_acdb_custom_topology_real_common_setcal_capture_v2656 as v2656
...
ROOT = v2656.ROOT
RUN_ID = "V2657"
BUILD_TAG = "v2657-audio-acdb-custom-topology-real-common-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2657_AUDIO_ACDB_CUSTOM_TOPOLOGY_REAL_COMMON_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2657-acdb-custom-topology-real-common-setcal-capture-{stamp}"
...
def build_v2656_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2656_build_root,
        manifest=args.v2656_manifest_path,
        report=v2656.DEFAULT_REPORT,
    ...
    payload = v2656.make_payload(build_args)
    args.v2656_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2656_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2656_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    setcal_contract_ok = bool(
        ...
        and required.get("helper_calls_common_topology_before_send_v5")
        and required.get("preinit_has_real_common_compile_gate")
        and required.get("preinit_records_real_common_call")
        and required.get("build_defines_real_common_topology")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2656_artifacts:
        build_v2656_artifacts(args)
    manifest = read_v2656_manifest(args.v2656_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2657-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2657-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    preinit_path = artifact_dir / "acdb-v2608-preinit-no-send-events.jsonl"
    setcal_events = [
        event for event in read_jsonl(setcal_path)
        if event.get("event") == "setcal_capture"
    ]
    preinit_events = read_jsonl(preinit_path)
    rows = [_setcal_row(event) for event in setcal_events]
    preinit_stages = [
        str(event.get("stage"))
        for event in preinit_events
        if event.get("event") == "v2608_preinit_no_send"
    ]
    real_common_return_codes = [
        int32_or_none(event.get("code"))
        for event in preinit_events
        if event.get("event") == "v2608_preinit_no_send"
        and event.get("stage") == "real_common_topology_return"
    ]
    real_common_return_codes = [code for code in real_common_return_codes if code is not None]
    skipped_common_topology = "skip_real_common_topology" in preinit_stages
    real_common_topology_called = (
        "before_real_common_topology" in preinit_stages
        and "real_common_topology_return" in preinit_stages
    )
    ...
    if real_set_events:
        classification = "v2657-boundary-violation-real-audio-set-passthrough"
    elif skipped_common_topology:
        classification = "v2657-common-topology-hook-skipped-real-call-no-setcal"
    elif real_common_topology_called and not rows:
        classification = "v2657-real-common-returned-before-setcal-no-setcal"
    elif not rows:
        classification = f"v2657-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2657-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2657-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2657-setcal-records-no-custom-topology"
    else:
        classification = "v2657-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2657-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2657-custom-topology-setcal-partial",
            "v2657-setcal-records-no-custom-topology",
            "v2657-common-topology-hook-skipped-real-call-no-setcal",
            "v2657-real-common-returned-before-setcal-no-setcal",
        },
        "operator_valuable": bool(rows or skipped_common_topology or real_common_topology_called),
        "counts_toward_fails_twice": (
            False
            if skipped_common_topology or real_common_topology_called
            else base_summary.get("counts_toward_fails_twice")
        ),
        "setcal_events_path": rel(setcal_path),
        "preinit_events_path": rel(preinit_path),
        "preinit_stage_count": len(preinit_stages),
        "preinit_stages": preinit_stages,
        "skipped_common_topology": skipped_common_topology,
        "real_common_topology_called": real_common_topology_called,
        "real_common_return_codes": real_common_return_codes,
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2657-acdb-custom-topology-real-common-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2656_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "send_path": "init_v3 -> a90_arm_capture -> send_common_custom_topology() -> send_audio_cal_v5(15,1,0x11135,48000,0,48000,1)",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2656 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2657 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2656_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2657-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2656_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
        ...
        f"- skipped_common_topology: `{summary.get('skipped_common_topology')}`",
        f"- real_common_topology_called: `{summary.get('real_common_topology_called')}`",
        f"- preinit_stage_count: `{summary.get('preinit_stage_count')}`",
        f"- preinit_stages: `{summary.get('preinit_stages')}`",
        f"- setcal_record_count: `{summary.get('setcal_record_count')}`",
        f"- cal_types_seen: `{summary.get('cal_types_seen')}`",
        f"- real_common_return_codes: `{summary.get('real_common_return_codes')}`",
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2656 SET-calibration capture path.",
        ])
    if summary.get("skipped_common_topology"):
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed, but the selected V2656 combined preload did not reach",
    ...
    if summary.get("classification") == "v2657-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2656-artifacts", action="store_true")
    parser.add_argument("--v2656-build-root", type=Path, default=v2656.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2656-manifest-path", type=Path, default=v2656.DEFAULT_MANIFEST)
