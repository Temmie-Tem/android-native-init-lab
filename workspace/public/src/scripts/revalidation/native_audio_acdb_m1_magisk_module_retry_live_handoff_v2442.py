#!/usr/bin/env python3
"""V2442 source/test M1 Magisk-module retry runner.
...
V2442 additionally hardens the post-module-reboot settle window from V2439:
...
RUN_ID = "V2442"
BUILD_TAG = "v2442-audio-acdb-m1-magisk-module-retry"
ROOT = v2429.ROOT
...
def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "module_lifecycle": {
            ...
            "v2442_post_module_reboot_retry": True,
        },
        "commands": commands,
    ...
    if not safety.get("ok"):
        blockers.append("V2442 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2442 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2442 command safety failed: {plan.get('command_safety')}")
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2442 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2442 M1 Magisk-module retry")
