#!/usr/bin/env python3
"""Build V2887 native-init candidate with audio-sync late-frame drop policy."""
...
CYCLE = "V2887"
INIT_VERSION = "0.10.29"
INIT_BUILD = "v2887-av-sync-drop"
BUILD_TAG = INIT_BUILD
DECISION = "v2887-av-sync-drop-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2887_AV_SYNC_DROP_SOURCE_BUILD_2026-06-19.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2887_av_sync_drop.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2887_av_sync_drop"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2887_av_sync_drop.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v459_av_sync_drop"
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2887 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    markers = manifest.get("v2887_marker_strings", [])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "av-sync-drop-candidate",
        "parent_test_artifact": "v2886-av-sync-stream",
        "av_sync_stream": {
            "version": 1,
            "source_unit": "V2887",
        ...
        "v2887_marker_strings": marker_strings,
    ...
    (OUT_DIR / "av-sync-drop-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "av-sync-drop-candidate",
