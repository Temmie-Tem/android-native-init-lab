#!/usr/bin/env python3
"""Build the S22+ M10A1 stat-dev then reboot native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M10A1 is the post-M10A split. M10A bootlooped after adding
mkdirat("/dev", 0755), but the observed later download endpoint was manually
entered by the operator. M10A1 keeps the M9A/M10A freestanding C shape and
performs only a read-only pathname VFS lookup, newfstatat("/dev"), before
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a1_stat_dev_reboot_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m10a1_stat_dev_reboot.c")
REQUIRED_STRINGS = ["/dev", "download"]
FORBIDDEN_STRINGS = [
    b"ld-linux",
    b"libc.so",
    b"S22_NATIVE_INIT",
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | int | list[str]]:
    ...
    require_ok(result, "compile M10A1 stat-dev reboot init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M10A1 stat-dev reboot init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M10A1 stat-dev reboot init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", "-S", out_path])
    require_ok(readelf, "readelf M10A1 stat-dev reboot init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M10A1 stat-dev reboot init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M10A1 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M10A1 init is not AArch64")
    ...
    if svc_count != 2:
        raise SystemExit(f"M10A1 init expected exactly two svc instructions, saw {svc_count}: {svc_lines}")
    stat_nr_lines = [index for index, line in enumerate(objdump_lines) if "mov" in line and "x8" in line and "#0x4f" in line]
    reboot_nr_lines = [index for index, line in enumerate(objdump_lines) if "mov" in line and "x8" in line and "#0x8e" in line]
    mkdir_nr_lines = [index for index, line in enumerate(objdump_lines) if "mov" in line and "x8" in line and "#0x22" in line]
    if not stat_nr_lines:
        raise SystemExit("M10A1 init does not load arm64 __NR_newfstatat (79) into x8")
    if not reboot_nr_lines:
        raise SystemExit("M10A1 init does not load arm64 __NR_reboot (142) into x8")
    if mkdir_nr_lines:
        raise SystemExit("M10A1 init unexpectedly loads arm64 __NR_mkdirat (34)")
    if min(reboot_nr_lines) < min(stat_nr_lines):
        raise SystemExit("M10A1 reboot syscall appears before newfstatat syscall in disassembly")

    binary = out_path.read_bytes()
    for required in REQUIRED_STRINGS:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"M10A1 /init missing required string: {required}")
    for forbidden in FORBIDDEN_STRINGS:
        if forbidden in binary:
            raise SystemExit(f"M10A1 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m10a1_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m10a1_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m10a1_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m10a1_init = build_dir / "s22plus_init_m10a1_stat_dev_reboot"
    m10a1_init_info = compile_init(source, m10a1_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m10a1_init}"], work_dir, "replace /init with M10A1 init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M10A1 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m10a1_init):
        raise SystemExit("replaced /init does not match compiled M10A1 init")
    ...
    hashes = {
        ...
        "m10a1_init": sha256_file(m10a1_init),
    ...
    sizes = {
        ...
        "m10a1_init": m10a1_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M10A1 in-place freestanding C read-only stat-dev then reboot discriminator",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns_without_manual_entry": "pathname lookup/VFS read is survivable; mkdir mutation or directory-create path remains suspect",
            "no_download_mode": "pathname VFS access itself is suspect before Samsung reboot",
        },
    }
