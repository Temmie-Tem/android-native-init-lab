#!/usr/bin/env python3
"""V2801 live handoff for V2798 sound-control readiness replay.
...
V2801 re-runs the last known sound-control-good V2798 candidate with the V2800 diagnostic capture set to split artifact regression from runtime/environment drift.
"""
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2801_AUDIO_V2798_READINESS_REPLAY_LIVE_2026-06-19.md"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2801-audio-v2798-readiness-replay-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2801-audio-v2798-readiness-replay-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2801-audio-v2798-readiness-replay-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio V2798 readiness replay command did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-v2798-readiness-replay-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2801-audio-v2798-readiness-replay-worker-failed-before-rollback"
            raise RuntimeError("native audio V2798 readiness replay command did not emit all required pass markers")
        ...
        result["decision"] = "v2801-audio-v2798-readiness-replay-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2801-audio-v2798-readiness-replay-live-blocked")
        if result["decision"] == "v2801-audio-v2798-readiness-replay-live-started":
            result["decision"] = "v2801-audio-v2798-readiness-replay-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2801-audio-v2798-readiness-replay-live-dry-run",
