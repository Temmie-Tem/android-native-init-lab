#!/usr/bin/env python3
"""Execute every P3.11 output family against inherited actual gates."""
...
import s22plus_fyg8_p311_telemetry_decoder as decoder
import s22plus_fyg8_p311_telemetry_model as model
import s22plus_fyg8_p311_telemetry_spec as spec


SCHEMA = "s22plus_fyg8_p311_cross_gate_audit_v1"
VERDICT = "PASS_P311_ACTUAL_ENCODER_OUTPUTS_SUBSET_ALL_GATES_HOST_ONLY"
...
def _contradiction_array() -> bytes:
    return inherited._array(  # noqa: SLF001
        "p311_contradictions", sorted(spec.CONTRADICTION_DETAIL_NAMES)
    )
...
def _checkpoint_contradiction_tu(checkpoint: bytes, header: bytes) -> bytes:
    ...
    addition = (
        ...
        b"        for (size_t index = 0; index < sizeof(p311_contradictions) / sizeof(p311_contradictions[0]); ++index) {\n"
        b"            if (!p288_detail_allowed(ordinal, 2U, p311_contradictions[index])) rc = 40;\n"
        b"        }\n"
        b"    }\n"
        + needle
    )
    if source.count(needle) != 1:
        raise AuditError("P3.11 checkpoint gate insertion anchor differs")
...
def _kernel_contradiction_tu(patch: bytes) -> bytes:
    ...
    addition = (
        b"    for (size_t ordinal = 0; !rc && ordinal <= 106U; ++ordinal) {\n"
        b"        for (size_t index = 0; index < sizeof(p311_contradictions) / sizeof(p311_contradictions[0]); ++index) {\n"
        b"            if (!s22_fyg8_e1_detail_allowed(3U, ordinal, 107U, 2U, p311_contradictions[index])) rc = 40;\n"
        b"        }\n"
        b"    }\n"
        + needle
    )
    if source.count(needle) != 1:
        raise AuditError("P3.11 kernel gate insertion anchor differs")
    return source.replace(needle, addition, 1)
...
def audit(root: Path) -> dict[str, Any]:
    ...
    if not first <= set(p308.clock_outputs()):
        raise AuditError("P3.11 A outputs exceed executed inherited A gate set")
    if not summary <= set(p308.summary_outputs()):
        raise AuditError("P3.11 B outputs exceed executed inherited B gate set")
