#!/usr/bin/env python3
"""Build V3293 GPU H5 A2D-linearized triangle KMS presentation probe."""
...
CYCLE = "V3293"
INIT_VERSION = "0.11.72"
INIT_BUILD = "v3293-gpu-h5-linear-triangle-kms-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3293_GPU_H5_LINEAR_TRIANGLE_KMS_SOURCE_BUILD_2026-06-26.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3293_gpu_h5_linear_triangle_kms_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3293_gpu_h5_linear_triangle_kms_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v603_gpu_h5_linear_triangle_kms_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3293"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3293.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3293.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3293"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3293-gpu-h5-linear-triangle-kms-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3293-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3293-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3293-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3293-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3293-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3293-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3293-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h5-linear-triangle-kms-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h5-linear-triangle-kms-probe"

INPUT_THREAD_MARKER = previous.INPUT_THREAD_MARKER.replace("v3291", "v3293")
TIME_MODEL_MARKER = previous.TIME_MODEL_MARKER.replace("v3291", "v3293")
DEMO_HUD_MARKER = previous.DEMO_HUD_MARKER.replace("v3291", "v3293")
PACED_TIME_MARKER = previous.PACED_TIME_MARKER.replace("v3291", "v3293")
TICK_TELEMETRY_MARKER = previous.TICK_TELEMETRY_MARKER.replace("v3291", "v3293")
SCALE_MARKER = previous.SCALE_MARKER.replace("v3291", "v3293")
PHASE_TELEMETRY_MARKER = previous.PHASE_TELEMETRY_MARKER.replace("v3291", "v3293")
GAMETIC_FRAME_TELEMETRY_MARKER = previous.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3291", "v3293")
SFX_STREAM_MARKER = "a90.doomgeneric.v3293.audio=real-sfx-pcm-stream-gpu-h5-linear-triangle-kms-probe"
SOUND_MODE = "native-doom-sfx-gpu-h5-linear-triangle-kms-probe-v3293"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3293.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = "first-triangle-h5-a2d-linearized-h3-readback-to-kms-probe"
H5_PRESENTATION = "H3 tile6_3+flag color target A2D-linearized into parent-owned KMS dumb framebuffer"
H5_COMMAND = "gpu h5-triangle-kms-probe --timeout-ms 5000 --materialize-devnode"


def _rewrite_v3293_text(text: str) -> str:
    replacements = (
        ...
        ("a90-doomgeneric-v3291", "a90-doomgeneric-v3293"),
        ("a90.doomgeneric.v3291", "a90.doomgeneric.v3293"),
        ("v3291", "v3293"),
        ("V3291", "V3293"),
        ("gpu-h5-triangle-kms-probe", "gpu-h5-linear-triangle-kms-probe"),
        ...
        ("raw tile6_3 readback visualization scaled into parent-owned KMS dumb framebuffer",
         "A2D-linearized tile6_3 readback scaled into parent-owned KMS dumb framebuffer"),
        ("h3-readback-kms-presented", "h3-linear-readback-kms-presented"),
        ("gpu-h5-triangle-kms-shader-byte-audit",
         "gpu-h5-linear-triangle-kms-shader-byte-audit"),
    )
...
def _rewrite_v3293_bytes(item: bytes) -> bytes:
    return _rewrite_v3293_text(item.decode("utf-8")).encode("utf-8")
...
GPU_H5_LINEAR_KMS_MARKERS = (
    ...
    b"gpu.h5.kms.h3_linear_readback_changed_count=%u",
    b"gpu.h5.kms.result=h3-linear-readback-failed",
    b"h3-linear-readback-kms-presented",
    b"A2D LINEARIZED H3 COLOR",
    b"gpu-h5-triangle-kms",
)

REQUIRED_STRINGS = tuple(_rewrite_v3293_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h5_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        "source_baseline": "v3292-h5-raw-tile-readback-kms-live-telemetry-proof",
        "scope": SCOPE,
        "h5_presentation": H5_PRESENTATION,
        "command": H5_COMMAND,
        "candidate_type": "gpu-h5-linear-triangle-kms-probe-candidate",
        "h3_snapshot_words_expected": 128 * 128,
        "h3_snapshot_bytes_expected": 128 * 128 * 4,
        "presentation_mode": "A2D-linearized tile6_3 readback scaled into parent-owned KMS dumb framebuffer",
        ...
        "next_live_validation": [
            "flash-v3293-through-native-init-flash",
            "post-flash-health-check",
            "gpu-h5-linear-triangle-kms-timeout-guard",
...
def v3293_adapter_source() -> str:
    return _rewrite_v3293_text(previous.v3291_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h5-linear-triangle-kms-probe-candidate.json").write_text(json.dumps({
        ...
        "adoption_state": "pending-gpu-h5-linear-triangle-kms-live-validation",
...
def _overlay_preserved_v3293_ramdisk() -> dict[str, Any]:
    ...
    overlay["mode"] = "preserve-v3291-ramdisk-overlay-v3293-init-helper-engine"
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-h5-linear-triangle-kms-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h5-linear-triangle-kms-live-validation"
...
def _apply_v3293_overrides() -> None:
    ...
    replacements = {
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3293_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3293_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3293_ramdisk,
