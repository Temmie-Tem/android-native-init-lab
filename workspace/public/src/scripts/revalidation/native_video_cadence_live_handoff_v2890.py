#!/usr/bin/env python3
"""V2890 live handoff for V2889 page-flip cadence telemetry.

This runner flashes the V2889 candidate, stages a private 30fps synthetic full-geometry A90VSTR1 stream under SD runtime storage, runs the bounded `video stream --manifest ... --video-only --present pageflip` command, records DRM page-flip interval telemetry, then rolls back to v2321 and verifies selftest fail=0.
"""
...
ROOT = repo_root()
RUN_ID = "V2890"
BUILD_TAG = "v2890-video-cadence-live"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2890_VIDEO_CADENCE_LIVE_2026-06-19.md"
PREPARE_STREAM = ROOT / "workspace/public/src/scripts/revalidation/prepare_video_stream_v2874.py"
REMOTE_DIR = "/mnt/sdext/a90/runtime/video/v2890"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        "decision": "v2890-video-cadence-live-dry-run" if preflight_ok(state) else "v2890-video-cadence-live-blocked",
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "decision": "v2890-video-cadence-live-started",
        "pass": False,
        "preflight": state,
        "steps": steps,
    ...
    try:
        ...
        if not (result["candidate_version_ok"] and result["candidate_selftest_fail0"] and result["candidate_video_status_ok"]):
            result["decision"] = "v2890-video-cadence-candidate-health-failed-before-stream"
        ...
        if stream.get("rc") != 0 or not result["stream_summary"].get("pass"):
            result["decision"] = "v2890-video-cadence-stream-failed-before-rollback"
        ...
        if not result["candidate_selftest_after_stream_fail0"]:
            result["decision"] = "v2890-video-cadence-post-stream-selftest-failed"
            raise RuntimeError("candidate post-stream selftest did not report fail=0")
        result["decision"] = "v2890-video-cadence-live-pass-before-rollback"
        result["pass"] = True
    except Exception as exc:
        result.setdefault("decision", "v2890-video-cadence-live-blocked")
        if result["decision"] == "v2890-video-cadence-live-started":
            result["decision"] = "v2890-video-cadence-live-blocked"
...
def main() -> int:
    args = parse_args()
    out_dir = ROOT / f"workspace/private/runs/video/v2890-video-cadence-live-{now_slug()}"
    ...
    if not preflight_ok(state):
        payload = {
            "decision": "v2890-video-cadence-live-preflight-failed-no-flash",
            "pass": False,
            "preflight": state,
        }
