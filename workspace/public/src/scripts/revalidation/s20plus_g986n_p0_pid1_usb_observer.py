...
def read_attr(path: Path, label: str, maximum: int = 128) -> str:
    try:
        descriptor = os.open(path, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW)
    except OSError as exc:
        raise ObserverError(f"P0 sysfs attribute is unavailable: {label}") from exc
    try:
        info = os.fstat(descriptor)
        if not stat.S_ISREG(info.st_mode):
            raise ObserverError(f"P0 sysfs attribute is indirect: {label}")
        payload = os.read(descriptor, maximum + 1)
        if len(payload) > maximum or os.read(descriptor, 1):
            raise ObserverError(f"P0 sysfs attribute is oversized: {label}")
    finally:
        os.close(descriptor)
    if payload.endswith(b"\n"):
        payload = payload[:-1]
    if not payload or any(byte < 0x20 or byte > 0x7E for byte in payload):
        raise ObserverError(f"P0 sysfs attribute is malformed: {label}")
    try:
        return payload.decode("ascii", "strict")
    except UnicodeError as exc:
        raise ObserverError(f"P0 sysfs attribute is not ASCII: {label}") from exc


def optional_attr(path: Path, label: str) -> str | None:
    try:
        return read_attr(path, label)
...
def usb_device_nodes(usb_root: Path) -> list[tuple[str, Path]]:
    try:
        entries = sorted(usb_root.iterdir(), key=lambda path: path.name)
    except OSError as exc:
        raise ObserverError("P0 USB inventory is unavailable") from exc
    ...
    for entry in entries:
        ...
        try:
            ...
            raise ObserverError("P0 USB device link is invalid") from exc
        if not resolved.is_dir():
            raise ObserverError("P0 USB device node is not a directory")
...
def ttys_for_usb(
    ...
    try:
        ...
        raise ObserverError("P0 TTY inventory is unavailable") from exc
    ...
    for entry in entries:
        ...
        try:
            ...
            dev_value = read_attr(entry / "dev", "tty dev")
...
def scan_inventory(
    *,
    ...
    pending: list[str] = []
    conflicts: list[str] = []
    for usb_node, usb_path in usb_device_nodes(usb_root):
        try:
            vendor = read_attr(usb_path / "idVendor", "idVendor")
            product = read_attr(usb_path / "idProduct", "idProduct")
        except ObserverError:
            continue
        if vendor != USB_VENDOR or product != USB_PRODUCT:
            continue
        manufacturer = optional_attr(usb_path / "manufacturer", "manufacturer")
        product_string = optional_attr(usb_path / "product", "product")
        serial = optional_attr(usb_path / "serial", "serial")
        if product_string != USB_PRODUCT_STRING:
            continue
        base = {
            "usb_node": usb_node,
            "vendor": vendor,
        ...
        if manufacturer != USB_MANUFACTURER or serial is not None:
            conflicts.append(digest(base))
            continue
        ttys = ttys_for_usb(usb_node, usb_path, tty_root)
        if not ttys:
            pending.append(digest({**base, "tty_count": 0}))
        elif len(ttys) != 1:
            conflicts.append(digest({**base, "tty_count": len(ttys)}))
        else:
            exact.append(ttys[0])
    return Inventory(
        exact=tuple(exact),
...
def baseline_value(expected_usb_node: str) -> dict[str, Any]:
    if USB_NODE_RE.fullmatch(expected_usb_node) is None:
        raise ObserverError("P0 expected USB topology is invalid")
    return {
        "schema": BASELINE_SCHEMA,
        "observer_schema": SCHEMA,
        "expected_topology_sha256": hash_text(expected_usb_node),
...
def capture_baseline(
    ...
    *,
    ...
    expected = baseline_value(expected_usb_node)
    ...
    if (
        ...
        raise ObserverError("P0 candidate identity is not absent at baseline")
    return expected
...
def validate_baseline(value: Any, expected_usb_node: str) -> dict[str, Any]:
    ...
    if (
        ...
        or value != expected
        or type(value.get("candidate_absent")) is not bool
    ):
        raise ObserverError("P0 baseline is malformed")
    return value
...
def select_arrival(
    ...
    *,
    ...
    if inventory.conflicting_identity_sha256:
        raise ObserverError("P0 candidate identity is conflicting")
    if inventory.pending_identity_sha256:
        if inventory.exact:
            raise ObserverError("P0 candidate endpoint is ambiguous")
        raise ObserverError("P0 candidate endpoint is pending")
    if len(inventory.exact) != 1:
        label = "absent" if not inventory.exact else "ambiguous"
        raise ObserverError(f"P0 candidate endpoint is {label}")
    candidate = inventory.exact[0]
    if candidate.usb_node != expected_usb_node:
        raise ObserverError("P0 candidate arrived on a foreign topology")
    return candidate


def verify_descriptor(candidate: Candidate, descriptor: int) -> None:
    info = os.fstat(descriptor)
    if (
        ...
        raise ObserverError("P0 TTY descriptor identity changed")
...
def read_exact_banner(
    ...
    *,
    ...
    if type(timeout_sec) is not int or timeout_sec != BANNER_TIMEOUT_SEC:
        raise ObserverError("P0 banner timeout is not the reviewed bound")
    try:
        fcntl.ioctl(descriptor, termios.TIOCEXCL)
        tty.setraw(descriptor, termios.TCSANOW)
    except (OSError, termios.error) as exc:
        raise ObserverError("P0 TTY could not be exclusively opened") from exc
    deadline = monotonic() + timeout_sec
    payload = bytearray()
    while len(payload) < len(BANNER):
        remaining = deadline - monotonic()
        if remaining <= 0:
            raise ObserverError("P0 banner timed out")
        ...
        if not BANNER.startswith(payload):
            raise ObserverError("P0 banner bytes differ")
...
def observe_selected(
    ...
    *,
    ...
    validate_baseline(baseline, expected_usb_node)
    if candidate.usb_node != expected_usb_node:
        raise ObserverError("P0 selected endpoint topology changed")
    verify_descriptor(candidate, descriptor)
    ...
    if repeated.identity_sha256 != candidate.identity_sha256:
        raise ObserverError("P0 endpoint changed after banner")
    return {
...
def open_live(candidate: Candidate, dev_root: Path = DEV_ROOT) -> int:
    path = dev_root / candidate.tty_name
    try:
        before = path.lstat()
    except OSError as exc:
        raise ObserverError("P0 TTY node is unavailable") from exc
    if (
        not stat.S_ISCHR(before.st_mode)
        or os.major(before.st_rdev) != candidate.major
        or os.minor(before.st_rdev) != candidate.minor
    ):
        raise ObserverError("P0 TTY node identity differs")
    try:
        descriptor = os.open(
            path,
            os.O_RDWR
        ...
        raise ObserverError("P0 TTY open failed") from exc
    try:
        verify_descriptor(candidate, descriptor)
    except Exception:
        os.close(descriptor)
...
def require_active() -> None:
    if OBSERVER_ACTIVE is not True:
        raise ObserverError("P0 USB observer is dormant")
...
def observe_attended(
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if str(exc) not in {
                "P0 candidate endpoint is absent",
                "P0 candidate endpoint is pending",
            }:
                raise
        time.sleep(POLL_INTERVAL_SEC)
    if candidate is None:
        raise ObserverError("P0 candidate arrival timed out")
    descriptor = open_live(candidate)
    try:
