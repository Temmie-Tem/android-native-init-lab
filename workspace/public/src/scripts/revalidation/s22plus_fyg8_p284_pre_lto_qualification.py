#!/usr/bin/env python3
"""Pre-Full-LTO qualification for the P2.84 ingestion correction."""
...
import s22plus_fyg8_p284_e2_stock_closure as closure
import s22plus_fyg8_p284_source_contract as p284
import s22plus_fyg8_p284_sysfs_ingestion_oracle as ingestion


SCHEMA = "s22plus_fyg8_p284_pre_lto_qualification_v1"
VERDICT = "PASS_P284_PRE_FULL_LTO_QUALIFICATION_HOST_ONLY"
LINKED_META_SCHEMA = "s22plus_fyg8_p284_linked_audit_meta_v1"
LINKED_META_VERDICT = "PASS_P284_LINKED_AUDIT_META_HOST_ONLY"
LINKED_AUDIT_MODULE_NAME = "s22plus_fyg8_p284_linked_audit"
LINKED_AUDIT_TEST = Path(
    "tests/test_s22plus_fyg8_p284_linked_audit.py"
)
DEFAULT_LINKED_AUDIT_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p284_pre_lto/"
...
FOCUSED_TESTS = (
    *base.FOCUSED_TESTS,
    Path("tests/test_s22plus_fyg8_p284_contract_source.py"),
    Path("tests/test_s22plus_fyg8_p284_sysfs_ingestion_oracle.py"),
    Path("tests/test_s22plus_fyg8_p284_pre_lto_qualification.py"),
    Path("tests/test_s22plus_fyg8_p284_build_registration.py"),
)
GATE_NAMES = (
    *base.GATE_NAMES,
...
GATE_IMPLEMENTATION_SOURCES = {
    **base.GATE_IMPLEMENTATION_SOURCES,
    "closure": (
        base.SCRIPT_DIR / "s22plus_fyg8_p284_e2_stock_closure.py"
    ),
    "p284_qualification": Path(__file__).resolve(),
    "p284_source_contract": (
        base.SCRIPT_DIR / "s22plus_fyg8_p284_source_contract.py"
    ),
    "p284_contract_spec": (
        base.SCRIPT_DIR / "s22plus_fyg8_p284_contract_spec.py"
    ),
    "p284_linked_audit": (
        base.SCRIPT_DIR / "s22plus_fyg8_p284_linked_audit.py"
    ),
...
candidate_contract = base.candidate_contract
...
def _load_linked_audit_module():
    try:
        ...
        raise QualificationError(
            "P2.84 linked-audit module is unavailable"
        ) from exc
    if (
        getattr(module, "EXPECTED_SOURCE_CONTRACT_ID", None)
        != p284.CONTRACT_ID
        or getattr(module, "ADAPTER_ID", None)
        != "s22plus-fyg8-p284-linked-audit-v1"
    ):
        raise QualificationError("P2.84 linked-audit identity drifted")
    return module
...
def create_linked_audit_receipt(root: Path) -> dict[str, Any]:
    ...
    test = base._run_test_command(
        root, (LINKED_AUDIT_TEST,), "P2.84 linked audit"
    )
    payload = base._portable_repo_paths(
        root,
        {
            ...
            "module": base._material(
                ...
                "s22plus_fyg8_p284_linked_audit.py",
                "P2.84 linked-audit module",
            ),
...
def _verify_linked_audit_receipt(path: Path) -> dict[str, Any]:
    ...
    module = _load_linked_audit_module()
    value, receipt = base._load_json(path, "P2.84 linked-audit receipt")
    if set(value) != {
        "adapter_id",
        "known_good",
        "module",
        ...
        raise QualificationError("P2.84 linked-audit receipt shape drifted")
    payload = dict(value)
    digest = payload.pop("payload_sha256", None)
    if digest != hashlib.sha256(base._canonical(payload)).hexdigest():
        raise QualificationError("P2.84 linked-audit receipt digest mismatch")
    ...
    if (
        ...
        raise QualificationError("P2.84 linked-audit receipt is stale")
    return {
        **base._result_binding(root, path, receipt, "P2.84 linked audit"),
        "semantics": {
            "adapter_id": module.ADAPTER_ID,
            "source_contract_id": p284.CONTRACT_ID,
...
def verify_receipt(
    ...
    *,
    ...
    value, qualification_receipt = base._load_json(
        path, "P2.84 pre-LTO qualification"
    )
    payload = dict(value)
    digest = payload.pop("payload_sha256", None)
    if (
        ...
        or exact_contract.get("source_contract_id") != p284.CONTRACT_ID
    ):
        raise QualificationError("P2.84 qualification header is invalid")
    expected_safety = {
        "host_only": True,
        "kernel_built": False,
    ...
    if value.get("safety") != expected_safety:
        raise QualificationError("P2.84 qualification safety differs")
    candidate = base._candidate_binding(
        exact_contract, intent_path, patch_path
    )
    if value.get("candidate") != candidate:
        raise QualificationError("P2.84 qualification inputs differ")
    implementation = p284.implementation_result(root)
    _data, receipts = p284.source_receipts(root)
    ...
    if value.get("implementation") != expected_implementation:
        raise QualificationError("P2.84 implementation binding is stale")
    if value.get("gate_implementation") != _gate_implementation():
        raise QualificationError("P2.84 gate implementation is stale")
    evidence = value.get("evidence")
    if (
        not isinstance(evidence, dict)
        or evidence != _current_evidence(exact_contract, evidence)
    ):
        raise QualificationError("P2.84 evidence is stale")
    if value.get("gates") != _gate_matrix(evidence):
        raise QualificationError("P2.84 gate matrix drifted")
    return {
        "schema": SCHEMA,
        "verdict": VERDICT,
        "build_allowed": True,
