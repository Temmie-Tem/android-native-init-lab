#!/usr/bin/env python3
"""Independently audit one P3.14 fixed-Image candidate pair."""
...
import build_s22plus_fyg8_p314_candidate as candidate
import s22plus_fyg8_p313_candidate_static_checker as inherited
import s22plus_fyg8_p314_candidate_contract as contract
...
SCHEMA = "s22plus_fyg8_p314_candidate_static_checker_v1"
VERDICT = "PASS_P314_INDEPENDENT_ARTIFACT_CLOSURE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p314/static-check-result.json")
...
def _base():
    _configure()
    return inherited.inherited.inherited.base
...
def audit(args):  # noqa: ANN001, ANN201
    ...
    payload = contract.stable_read(
        ...
        "P3.14 final qualification closure",
        64 * 1024 * 1024,
    )
    try:
    ...
    if (
        not isinstance(prepackaging, dict)
        or not isinstance(prepackaging_receipt, dict)
        or not isinstance(prepackaging_receipt.get("size"), int)
        or not isinstance(prepackaging_receipt.get("sha256"), str)
    ):
    ...
    if (
        ...
        or exact["matrix_fixture"].get("verified") is not True
    ):
        raise CheckError("P3.14 qualification inputs differ")
    result.update(
        {
            "p314_tracefs_abi": exact["tracefs_abi"],
            "p314_cross_gate_audit": exact["cross_gate_audit"],
            "p314_runtime_fixture": fixture,
            "p314_matrix_fixture": exact["matrix_fixture"],
            "p314_process_v2_adapter_fixture": adapter,
            "p314_hazard_closure": exact["hazard_closure"],
            "p314_qualification_closure": qualified,
            "p314_telemetry": exact["telemetry"],
            "p314_observer": exact["observer"],
        }
    )
    return result
