#!/usr/bin/env python3
"""Build V2666 init-time real-common custom-topology ACDB SET capture artifacts.

Host-only build unit. V2665 proved the V2664 post-init common-only helper never
reaches its post-init call because acdb_loader_init_v3 SIGSEGVs after the
init-time common hook fake-allocates the target 10/14/24 cal blocks. V2666
therefore moves the real common-topology call into the init-time hook and exits
immediately after that call, while keeping the V2630 fake-success SET capture
shim.
"""
...
ROOT = v2659.ROOT
RUN_ID = "V2666"
BUILD_TAG = "v2666-acdb-init-real-common-setcal-capture-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2666_AUDIO_ACDB_INIT_REAL_COMMON_SETCAL_CAPTURE_BUILD_2026-06-18.md"

HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_common_only_exec_linked_v2664.c"
PREINIT_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_phase_common_init_real_v2666.c"
HELPER_ARTIFACT_NAME = "a90_acdb_init_real_common_setcal_capture_exec_linked_v2666"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_init_real_common_setcal_capture_combined_preload_v2666.so"
...
def source_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    required = {
        ...
        "phase_hook_init_real_event_path_v2666": "acdb-v2666-init-real-common-events.jsonl" in preinit,
    ...
    if isinstance(preload, dict):
        ...
        if isinstance(checks, dict):
            if "soname_v2659" in checks:
                checks["soname_v2666"] = checks.pop("soname_v2659")
