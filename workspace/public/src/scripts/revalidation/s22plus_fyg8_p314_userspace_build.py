#!/usr/bin/env python3
"""Build P3.14 userspace twice against the fixed P3.10 Image."""
...
SCHEMA = "s22plus_fyg8_p314_userspace_build_v1"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p314/userspace")
...
def verdict_for_profile(profile: str, source_contract_id: str | None = None) -> str:
    if profile != contract.PROFILE or source_contract_id != contract.PARENT_SOURCE_CONTRACT_ID:
        raise BuildError("P3.14 userspace profile/source contract differs")
    return VERDICT
...
def _source_contract(exact: dict) -> dict:
    return {
        ...
        "frozen_p313_baseline": exact["frozen_p313_baseline"],
...
def build_userspace(args: argparse.Namespace) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise BuildError(f"P3.14 userspace output exists: {output}")
    ...
    metadata = []
    for label in ("a", "b"):
        with tempfile.TemporaryDirectory(prefix=f"s22-p314-userspace-{label}-") as name:
            directory = Path(name)
    ...
    if builds[0] != builds[1] or metadata[0] != metadata[1]:
        raise BuildError("P3.14 userspace two-build reproducibility mismatch")
    result = {
        "schema": SCHEMA,
        "target": TARGET,
        "verdict": VERDICT,
