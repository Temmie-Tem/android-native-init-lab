#!/usr/bin/env python3
"""Verify P2.90 Full-LTO A/B with the inherited path-leak gate."""
...
import s22plus_fyg8_p290_build as build
import s22plus_fyg8_p290_candidate_contract as candidate_contract
import s22plus_fyg8_p290_source_contract as p290


SCHEMA = "s22plus_fyg8_p290_build_repro_check_v1"
VERDICT = "PASS_P290_TWO_CLEAN_BUILD_REPRO_AND_LINKED_AUDIT_HOST_ONLY"
PATH_GATE_SCHEMA = "s22plus_fyg8_p290_full_lto_a_path_gate_v1"
PATH_GATE_VERDICT = "PASS_P290_FULL_LTO_A_PATH_LEAK_GATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P290_SOURCE_CONTRACT_ID = p290.CONTRACT_ID
P286_SOURCE_CONTRACT_ID = P290_SOURCE_CONTRACT_ID
P290_QUALIFICATION_MODULE = "s22plus_fyg8_p290_pre_lto_qualification"
P290_QUALIFICATION_PROVENANCE_KEY = "p290_pre_lto_qualification"
P286_QUALIFICATION_MODULE = P290_QUALIFICATION_MODULE
P286_QUALIFICATION_PROVENANCE_KEY = P290_QUALIFICATION_PROVENANCE_KEY
DEFAULT_BUILD_A = Path(
    "workspace/private/outputs/s22plus_fyg8_p290/artifacts-a"
)
DEFAULT_BUILD_B = Path(
    "workspace/private/outputs/s22plus_fyg8_p290/artifacts-b"
)
...
CLANG_RESOURCE_PATH_MARKERS = (b"/lib/clang/", b"/lib64/clang/")
LINKED_VALIDATOR_ADAPTERS = {
    **base.LINKED_VALIDATOR_ADAPTERS,
    P290_SOURCE_CONTRACT_ID: "s22plus_fyg8_p290_postbuild_linked_audit",
}
...
def _configure() -> None:
    ...
    base.TARGET = TARGET
    base.P286_SOURCE_CONTRACT_ID = P290_SOURCE_CONTRACT_ID
    base.P286_QUALIFICATION_MODULE = P290_QUALIFICATION_MODULE
    base.P286_QUALIFICATION_PROVENANCE_KEY = (
        P290_QUALIFICATION_PROVENANCE_KEY
    )
...
def verify_p290_qualification_file(*args, **kwargs):  # noqa: ANN002, ANN003, ANN201
...
def audit_a_path_leaks(directory: Path) -> dict[str, Any]:
    """Fail before build B if build A retains a host-specific clang path."""
    ...
    clang_paths = tuple(
        ...
        and any(marker in value for marker in CLANG_RESOURCE_PATH_MARKERS)
        and b"/include" in value
    )
    stable_paths = tuple(
        value for value in clang_paths if value.startswith(b"/private-repo/")
    )
    absolute_host_paths = tuple(
        value
        for value in clang_paths
    ...
    if any(random_counts.values()):
        raise CheckError(
            "P2.90 Full-LTO A leaks a random private namespace"
        )
    if absolute_host_paths:
        raise CheckError(
            "P2.90 Full-LTO A leaks an absolute clang resource path"
        )
    if not stable_paths:
        raise CheckError(
            "P2.90 Full-LTO A lacks the mapped clang resource path"
        )
    return {
