#!/usr/bin/env python3
"""V2720 Android handoff wrapper for V2719 in-hook route-first common-topology ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2719 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2719
...
The own-process send path is the V2719 route-first helper:
...
The V2719 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
import build_android_acdb_inhook_route_first_common_setcal_capture_v2719 as v2719
...
ROOT = v2719.ROOT
RUN_ID = "V2720"
BUILD_TAG = "v2720-audio-acdb-inhook-route-first-common-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2720_AUDIO_ACDB_INHOOK_ROUTE_FIRST_COMMON_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2720-acdb-inhook-route-first-common-setcal-capture-{stamp}"
...
def build_v2719_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2719_build_root,
        manifest=args.v2719_manifest_path,
        report=v2719.DEFAULT_REPORT,
    ...
    payload = v2719.make_payload(build_args)
    args.v2719_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2719_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2719_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    setcal_contract_ok = bool(
        ...
        and required.get("helper_imports_only_init_v3")
        and required.get("helper_does_not_call_send_v5")
        and required.get("helper_does_not_call_common_topology")
        and required.get("preload_arms_then_send_v5_then_real_common")
        and required.get("preload_uses_corrected_send_v5_args")
        and required.get("preload_neutralizes_reentry")
        and required.get("preload_exits_inside_hook")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2719_artifacts:
        build_v2719_artifacts(args)
    manifest = read_v2719_manifest(args.v2719_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2720-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2720-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    helper_path = artifact_dir / "acdb-v2719-inhook-route-first-helper-events.jsonl"
    phase_path = artifact_dir / "acdb-v2719-inhook-route-first-common-events.jsonl"
    setcal_events = [
    ...
    helper_stages = [
        ...
        if event.get("event") == "v2719_inhook_route_first_helper"
    ]
    phase_stages = [
        str(event.get("stage"))
        for event in phase_events
        if event.get("event") == "v2719_inhook_route_first_common"
    ]
    inhook_route_first_common_return_codes = [
        int32_or_none(event.get("code"))
        for event in phase_events
        if event.get("event") == "v2719_inhook_route_first_common"
        and event.get("stage") == "inhook_real_common_return"
    ]
    inhook_route_first_common_return_codes = [code for code in inhook_route_first_common_return_codes if code is not None]
    inhook_entered = "inhook_route_first_enter" in phase_stages
    inhook_patch_initialized = "inhook_patch_initialized_flag_return" in phase_stages
    inhook_armed_capture = "inhook_armed_capture" in phase_stages
    inhook_reached_send_v5 = "inhook_before_send_audio_cal_v5" in phase_stages
    inhook_returned_send_v5 = "inhook_send_audio_cal_v5_return" in phase_stages
    inhook_real_common_called = (
        "inhook_before_real_common" in phase_stages
        and "inhook_real_common_return" in phase_stages
    )
    inhook_exited_after_real_common = "inhook_exit_after_route_first_common" in phase_stages
    ...
    custom_payloads_dumped = bool(custom_payload_rows) and not custom_payload_failed_rows
    ...
    if real_set_events:
        classification = "v2720-boundary-violation-real-audio-set-passthrough"
    elif inhook_reached_send_v5 and not rows:
        classification = "v2720-inhook-send-v5-reached-no-setcal"
    elif reentry_neutralized and not rows:
        classification = "v2720-inhook-common-reentry-neutralized-no-setcal"
    elif inhook_real_common_called and not rows:
        classification = "v2720-inhook-real-common-returned-before-setcal-no-setcal"
    elif inhook_entered and not rows:
        classification = "v2720-inhook-entered-no-setcal"
    elif not rows:
        classification = f"v2720-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2720-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2720-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2720-setcal-records-no-custom-topology"
    else:
        classification = "v2720-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2720-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2720-custom-topology-setcal-partial",
            "v2720-setcal-records-no-custom-topology",
            "v2720-inhook-send-v5-reached-no-setcal",
            "v2720-inhook-common-reentry-neutralized-no-setcal",
            "v2720-inhook-real-common-returned-before-setcal-no-setcal",
            "v2720-inhook-entered-no-setcal",
        },
        "operator_valuable": bool(
            rows
            or helper_stages
            or inhook_entered
            or inhook_reached_send_v5
            or inhook_real_common_called
            or reentry_neutralized
        ),
        "counts_toward_fails_twice": (
            False
            if helper_stages or inhook_entered or inhook_reached_send_v5 or inhook_real_common_called or reentry_neutralized
        ...
        "inhook_entered": inhook_entered,
        "inhook_patch_initialized": inhook_patch_initialized,
        "inhook_armed_capture": inhook_armed_capture,
        "inhook_reached_send_v5": inhook_reached_send_v5,
        "inhook_returned_send_v5": inhook_returned_send_v5,
        "inhook_real_common_called": inhook_real_common_called,
        "inhook_exited_after_real_common": inhook_exited_after_real_common,
        "reentry_neutralized": reentry_neutralized,
        "inhook_route_first_common_return_codes": inhook_route_first_common_return_codes,
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2720-acdb-custom-topology-route-first-common-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2719_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "send_path": "init_v3 -> common hook -> patch initialized -> arm capture -> send_audio_cal_v5(15,1,0x11135,48000,0,48000,1) -> real common topology -> exit",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2719 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2720 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2719_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2720-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2719_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
        ...
        f"- inhook_entered: `{summary.get('inhook_entered')}`",
        f"- inhook_patch_initialized: `{summary.get('inhook_patch_initialized')}`",
        f"- inhook_armed_capture: `{summary.get('inhook_armed_capture')}`",
        f"- inhook_reached_send_v5: `{summary.get('inhook_reached_send_v5')}`",
        f"- inhook_returned_send_v5: `{summary.get('inhook_returned_send_v5')}`",
        f"- inhook_real_common_called: `{summary.get('inhook_real_common_called')}`",
        f"- inhook_exited_after_real_common: `{summary.get('inhook_exited_after_real_common')}`",
        ...
        f"- inhook_route_first_common_return_codes: `{summary.get('inhook_route_first_common_return_codes')}`",
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2719 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2720-inhook-entered-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2719 in-hook phase entered the real",
            "  common-topology hook, but no `AUDIO_SET_CALIBRATION` rows were emitted;",
            f"- phase_stages: `{summary.get('phase_stages')}`;",
            "- this is useful frontier evidence for the in-hook route-first path and does not",
            "  count as a dead retry against the capture theme.",
        ])
    if summary.get("classification") == "v2720-inhook-send-v5-reached-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
    ...
    if summary.get("classification") == "v2720-inhook-common-reentry-neutralized-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2719 in-hook phase neutralized nested common-topology",
    ...
    if summary.get("classification") == "v2720-inhook-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the corrected V2719 preload reached the real common-topology",
            "  function (`inhook_before_real_common` then `inhook_real_common_return`);",
            f"- inhook_route_first_common_return_codes: `{summary.get('inhook_route_first_common_return_codes')}`;",
            "- no `AUDIO_SET_CALIBRATION` rows were emitted before exit/crash;",
            ...
            "- this is useful frontier evidence: V2719 reached the real common-topology path,",
            "  but it returned before SET emission.",
        ])
    lines.extend([
        "",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2719-artifacts", action="store_true")
    parser.add_argument("--v2719-build-root", type=Path, default=v2719.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2719-manifest-path", type=Path, default=v2719.DEFAULT_MANIFEST)
