#!/usr/bin/env python3
"""Bind the unchanged stock module/rootfs closure to the P3.14 intent."""
...
from s22plus_fyg8_p313_e2_stock_closure import *  # noqa: F403
import s22plus_fyg8_p313_e2_stock_closure as parent
import s22plus_fyg8_p314_overlay_contract as overlay


SCHEMA = "s22plus_fyg8_p314_stock_closure_h0_v1"
VERDICT = "PASS_P314_STOCK_61_MODULE_CLOSURE_HOST_ONLY"
ClosureError = parent.ClosureError
INCIDENTAL_PATH = parent.INCIDENTAL_PATH
INCIDENTAL_PATH_OFFSET = 0x47E1
INCIDENTAL_INSTRUCTION_WINDOW_OFFSET = 0x47E0
...
def select(source_contract_id: str | None):
    if source_contract_id != overlay.PARENT_SOURCE_CONTRACT_ID:
        raise ClosureError("P3.14 source contract differs")
...
def derive_module_closure(
    ...
    if plan_header is None:
        raise ClosureError("P3.14 exact materialized plan is missing")
    intent_path = plan_header.parent.parent / "overlay-intent.json"
    exact = overlay.verify_intent(root, intent_path)
    supplied = parent.module_parent.p241.stable_read(
        plan_header, "P3.14 materialized plan", 1024 * 1024
    )
    expected = exact.get("generated_artifacts", {}).get("plan_header")
    if parent.module_parent.receipt(supplied) != expected:
        raise ClosureError("P3.14 supplied plan identity differs")
    result = parent.module_parent.derive_module_closure(
        root, vendor_ramdisk, lz4, plan_header=plan_header
    )
    if result.get("plan_header") != expected:
        raise ClosureError("P3.14 derived plan receipt differs")
    return result
...
def _scrub_exact_incidental_opcode_path(data: bytes) -> bytes:
    if (
        ...
        raise ClosureError("P3.14 incidental opcode-path receipt mismatch")
    return (
...
def _validate_p314_authority_strings(data: bytes) -> None:
    p310 = parent.parent.p310_parent
    p300 = p310.parent
    ...
    if (
        ...
        raise ClosureError("P3.14 candidate absolute-path authority mismatch")
...
def exact_init_authority(expected: bytes) -> Iterator[None]:
    def validate(data: bytes) -> None:
        if data != expected:
            raise ClosureError("P3.14 effective init differs from source-bound userspace")
        _validate_p314_authority_strings(data)

    p300 = parent.parent.p310_parent.parent
