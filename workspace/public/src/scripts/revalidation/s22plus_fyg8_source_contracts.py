...
def _selection_for(module: ModuleType, contract: Any) -> SelectedSourceContract:
    ...
    if module is p254:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p254.IMPLEMENTATION_VERDICT,
            source_check_run_id=p254.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p254.USERSPACE_VERDICT,
        )
    if module is p257:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p257.IMPLEMENTATION_VERDICT,
            source_check_run_id=p257.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p257.USERSPACE_VERDICT,
        )
    if module is p258:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p258.IMPLEMENTATION_VERDICT,
            source_check_run_id=p258.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p258.USERSPACE_VERDICT,
        )
    if module is p260:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p260.IMPLEMENTATION_VERDICT,
            source_check_run_id=p260.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p260.USERSPACE_VERDICT,
        )
    if module is p280:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p280.IMPLEMENTATION_VERDICT,
    ...
    if module is p284:
        return SelectedSourceContract(
            module=module,
            contract=contract,
            implementation_verdict=p284.IMPLEMENTATION_VERDICT,
            source_check_run_id=p284.SOURCE_CHECK_RUN_ID,
            userspace_verdict=p284.USERSPACE_VERDICT,
        )
...
def select(
    ...
    try:
        ...
        p254.SourceContractError,
        p257.SourceContractError,
