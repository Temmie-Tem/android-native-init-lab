"""Fixed P359 namespace over sealed P353 H0 machinery; no device authority."""
...
def load(namespace):
    name = Path(namespace['__file__']).name.replace('p359', 'p353')
    relative, expected = PREDECESSORS[name]
    path = ROOT / relative
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != expected:
        raise ValueError('P359 sealed predecessor changed: ' + name)
    raw = raw.replace(b'P353', b'P359').replace(b'p353', b'p359')
    raw = raw.replace(b'c353f1e0a90b5e6d7c8a9b0c1d2e3f0b', b'c359f1e0a90b5e6d7c8a9b0c1d2e3f0a')
    raw = raw.replace(b'68f6aa66cb29cf94cd7c857545cffd54fc0a250f9bb0a98208a7a153a32e84d9',
                      b'0143a7a1f0ca848779267aff68bafe8c50edeecbfe3adef8a118b922e4ad215b')
    tree = ast.parse(raw)
    tree.body = [n for n in tree.body if not (isinstance(n, ast.If) and '__name__' in ast.unparse(n.test))]
    entry = namespace['__name__']
    try:
        namespace['__name__'] = entry if entry != '__main__' else 'p359_h0_template'
        exec(compile(ast.fix_missing_locations(tree), str(path) + '#p359', 'exec'), namespace)
    finally:
