#!/usr/bin/env python3
"""Host-only readiness audit for the S22+ ramoops DTBO + M18 capture gate.
...
DEFAULT_DRAFT = Path("docs/operations/S22PLUS_RAMOOPS_DTBO_M18_CAPTURE_AGENTS_EXCEPTION_DRAFT_2026-07-08.md")
DEFAULT_AGENT_CONTRACT = Path("AGENTS.md")
DEFAULT_GATE = Path("workspace/public/src/scripts/revalidation/s22plus_ramoops_dtbo_m18_capture_live_gate.py")
...
def required_markers() -> list[str]:
    return [
        ...
        gate.EXPECTED_M18_AP_SHA256,
        gate.EXPECTED_M18_BOOT_SHA256,
        gate.EXPECTED_M18_BASE_BOOT_SHA256,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "purpose": "host-only readiness audit for S22+ ramoops DTBO + M18 capture",
        "device_action": False,
    ...
    if args.expect_agents_inactive and report["agents"]["complete"]:
        fail("AGENTS.md already contains all capture markers; this audit expected inactive policy", report)
    ...
    if args.assert_default_dryrun_policy_block:
        ...
        expected_message = "AGENTS.md missing ramoops DTBO + M18 capture authorization markers"
