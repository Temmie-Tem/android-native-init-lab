#!/usr/bin/env python3
"""Build V3249 GPU H3 cache invalidate probe."""
...
import build_native_init_boot_v3247_gpu_h3_rb_render_cntl_probe as previous

base = previous.base

CYCLE = "V3249"
INIT_VERSION = "0.11.51"
INIT_BUILD = "v3249-gpu-h3-cache-invalidate-probe"
BUILD_TAG = INIT_BUILD
DECISION = "v3249-gpu-h3-cache-invalidate-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3249_GPU_H3_CACHE_INVALIDATE_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3249_gpu_h3_cache_invalidate_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3249_gpu_h3_cache_invalidate_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v579_gpu_h3_cache_invalidate_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3249"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3249.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3249.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3249"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3249-gpu-h3-cache-invalidate-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3249-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3249-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3249-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3249-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3249-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3249-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3249-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-cache-invalidate-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-cache-invalidate-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3249")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3249")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3249")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3249")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3249")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3249")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3249")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3249")
SFX_STREAM_MARKER = "a90.doomgeneric.v3249.audio=real-sfx-pcm-stream-gpu-h3-cache-invalidate-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-cache-invalidate-probe-v3249"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3249.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3249_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3247": b"a90-doomgeneric-v3249",
        b"a90.doomgeneric.v3247": b"a90.doomgeneric.v3249",
        b"v3247": b"v3249",
        b"V3247": b"V3249",
...
GPU_H3_CACHE_INVALIDATE_MARKERS = (
    ...
    b"gpu.h3.draw.pre_draw_cache_invalidate=ccu-color,ccu-depth,cache",
...
REQUIRED_STRINGS = tuple(_rewrite_v3249_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        ...
        "pm4_dwords_expected": 240,
        "sp_vs_cntl0": "0x00100100",
        "sp_ps_cntl0": "0x81000100",
        "sp_cntl0_source": "Mesa A6xx SP_VS_CNTL_0/SP_PS_CNTL_0 fullregfootprint=2 retained while probing pre-draw cache invalidation",
        ...
        "pre_draw_cache_invalidate": "ccu-color,ccu-depth,cache",
        "pre_draw_cache_invalidate_events": "0x19,0x18,0x31",
        ...
        "next_live_validation": [
            "flash-v3249-through-native-init-flash",
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3249 GPU H3 Pre-Draw Cache Invalidate Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        "- Track: GPU H3 first-triangle pre-draw CCU/cache invalidation before H4 readback proof.",
        ...
        "## Included Delta",
        "",
        "- Uses V3247 RB_RENDER_CNTL image as the preserved-ramdisk base and keeps the V3246 verified hand-assembled ir3 bytes unchanged.",
        ...
        "- Keeps `RB_RENDER_CNTL=0x00000010`, matching Mesa A6xx `fd6_gmem.cc::update_render_cntl()` with `CCUSINGLECACHELINESIZE=2`.",
        "- Adds the Mesa A6xx restore-path `fd6_cache_inv()` sequence before H3 shader/state/draw packets: `PC_CCU_INVALIDATE_COLOR=0x19`, `PC_CCU_INVALIDATE_DEPTH=0x18`, `CACHE_INVALIDATE=0x31`, then `CP_WAIT_FOR_IDLE`.",
        "- Keeps VS/PS output regids at `0x00`, so telemetry should still show `sp_vs_output_reg0=0x00000f00`.",
        "- Expects H3 state emission to stay at 92 register writes and PM4 dwords to rise from 233 to 240 because the invalidate sequence adds three one-dword `CP_EVENT_WRITE` packets plus one WFI.",
        "- Removes stale preserved-ramdisk DOOM engines before packing V3249 and gates the final boot image at 64MiB to protect the boot partition.",
        "- This isolates whether the no-pixel result is caused by missing Mesa pre-draw CCU/UCHE invalidation before advancing to H4 interior/exterior readback proof.",
        "",
        "## Source Basis",
        "",
        ...
        "- Mesa/freedreno restore/cache invalidation helper: `https://gitlab.freedesktop.org/mesa/mesa/-/raw/main/src/gallium/drivers/freedreno/a6xx/fd6_emit.h` (`fd6_cache_inv`).",
        ...
        "## Validation",
        "",
        "- `py_compile`: V3249 builder and focused H3 source test.",
        "- `unittest`: V3249 GPU H3 pre-draw cache invalidate source contract.",
        "- Build: AArch64 helper/native-init compile, preserved-ramdisk overlay, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3249 identity plus cache invalidate H3 telemetry.",
        ...
        "- Candidate type: `gpu-h3-cache-invalidate-probe-candidate`.",
    ]) + "\n"


def v3249_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-cache-invalidate-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-cache-invalidate-probe")
        .replace("v3208", "v3249")
        .replace("V3208", "V3249")
    )


def _overlay_preserved_v3249_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3247 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3249 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3249 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3249 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3249-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            if item == "--ramdisk":
                ...
                raise RuntimeError("V3247 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3249 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3249 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3247-ramdisk-overlay-v3249-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-cache-invalidate-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-cache-invalidate-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-cache-invalidate-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3247-ramdisk-overlay-v3249-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-cache-invalidate-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-cache-invalidate-live-validation"
...
def _apply_v3249_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-cache-invalidate-probe")
        ...
        "v3210_adapter_source": v3249_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3249_ramdisk,
