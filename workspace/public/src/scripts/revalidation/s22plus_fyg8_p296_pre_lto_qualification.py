#!/usr/bin/env python3
"""Pre-Full-LTO qualification for P2.96 built-in DWC3 telemetry."""
...
import build_s22plus_fyg8_p296_candidate as candidate_builder
import s22plus_fyg8_p290_pre_lto_qualification as base
import s22plus_fyg8_p296_candidate_contract as candidate_contract
import s22plus_fyg8_p296_e2_stock_closure as closure
import s22plus_fyg8_p296_source_contract as p296
import s22plus_fyg8_p296_telemetry_spec as spec
import s22plus_fyg8_p296_userspace_build as userspace


p286 = p296
SCHEMA = "s22plus_fyg8_p296_pre_lto_qualification_v1"
VERDICT = "PASS_P296_PRE_FULL_LTO_QUALIFICATION_HOST_ONLY"
LINKED_META_SCHEMA = "s22plus_fyg8_p296_linked_audit_meta_v1"
LINKED_META_VERDICT = "PASS_P296_LINKED_AUDIT_META_HOST_ONLY"
LINKED_AUDIT_MODULE_NAME = "s22plus_fyg8_p296_linked_audit"
LINKED_AUDIT_TEST = Path("tests/test_s22plus_fyg8_p296_contract.py")
LINKED_AUDIT_PATH = Path(
    "workspace/public/src/scripts/revalidation/s22plus_fyg8_p296_linked_audit.py"
)
DEFAULT_LINKED_AUDIT_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p296_pre_lto/linked-audit-meta.json"
)
...
DEFAULT_LIFECYCLE_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p296_pre_lto/"
    "p280-trace-lifecycle-current/result.json"
)
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p296_pre_lto/qualification.json"
)
FOCUSED_TESTS = (
    *tuple(
        path
    ...
    Path("tests/test_s22plus_fyg8_p296_contract.py"),
    Path(
        "workspace/public/src/scripts/revalidation/"
        "test_s22plus_fyg8_p296_telemetry.py"
    ),
)
P296_GATE_NAME = "23-builtin-dwc3-value-telemetry"
GATE_IMPLEMENTATION_SOURCES = {
    **base.GATE_IMPLEMENTATION_SOURCES,
    "closure": Path(__file__).with_name("s22plus_fyg8_p296_e2_stock_closure.py"),
    "p296_qualification": Path(__file__).resolve(),
    "p296_source_contract": Path(__file__).with_name("s22plus_fyg8_p296_source_contract.py"),
    "p296_telemetry_spec": Path(__file__).with_name("s22plus_fyg8_p296_telemetry_spec.py"),
    "p296_telemetry_closure": Path(__file__).with_name("s22plus_fyg8_p296_telemetry_closure.py"),
    "p296_identity_tiers": Path(__file__).with_name("s22plus_fyg8_p296_identity_tiers.py"),
    "p296_linked_audit": Path(__file__).with_name("s22plus_fyg8_p296_linked_audit.py"),
    "p296_candidate_builder": Path(__file__).with_name("build_s22plus_fyg8_p296_candidate.py"),
    "p296_userspace_builder": Path(__file__).with_name("s22plus_fyg8_p296_userspace_build.py"),
    "p296_decoder": Path(__file__).with_name("s22plus_fyg8_p296_telemetry_decoder.py"),
}
...
def _load_linked_audit_module():
    try:
        ...
        raise QualificationError("P2.96 linked-audit module is unavailable") from exc
    if (
        getattr(module, "EXPECTED_SOURCE_CONTRACT_ID", None) != p296.CONTRACT_ID
        or getattr(module, "ADAPTER_ID", None)
        != "s22plus-fyg8-p296-linked-audit-v1"
    ):
        raise QualificationError("P2.96 linked-audit identity drifted")
    return module
...
def _configure() -> None:
    ...
    base.P290_GATE_NAME = P296_GATE_NAME
