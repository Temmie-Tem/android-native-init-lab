#!/usr/bin/env python3
"""Build one deterministic boot-only P2.96 FYG8 candidate host-only."""
...
import s22plus_fyg8_p296_boot_only_packager as packager
import s22plus_fyg8_p296_build_repro_check as repro
import s22plus_fyg8_p296_candidate_contract as candidate_contract
import s22plus_fyg8_p296_e2_stock_closure as p296_closure
import s22plus_fyg8_p296_telemetry_spec as p296_spec
import s22plus_fyg8_p296_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p296_candidate_artifact_result_v1"
VERDICT = "PASS_P296_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P296_SOURCE_CONTRACT_ID = repro.P296_SOURCE_CONTRACT_ID
P286_SOURCE_CONTRACT_ID = P296_SOURCE_CONTRACT_ID
DEFAULT_IMAGE = repro.DEFAULT_BUILD_A / "Image"
DEFAULT_REPRO_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p296/build-repro-result.json"
)
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p296/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    value, result_receipt = base._read_json(  # noqa: SLF001
        result_path,
        "P2.96 build reproducibility result",
    )
    if (
        ...
        raise BuildError("P2.96 build reproducibility result is not accepted")
    linked = value.get("linked_audit", {})
    if (
        linked.get("audit_adapter") != "s22plus-fyg8-p296-linked-audit-v1"
        or linked.get("postbuild_audit", {}).get("verified") is not True
    ):
        raise BuildError("P2.96 linked audit adapter mismatch")
    expected_image = value.get("build_a", {}).get("artifacts", {}).get("Image")
    if expected_image != image_receipt:
        raise BuildError("P2.96 supplied Image differs from reproducibility closure")
    if intent_path is None or patch_path is None:
        raise BuildError("P2.96 selected build-input paths are missing")
    try:
        qualification = repro.verify_p296_qualification_file(
