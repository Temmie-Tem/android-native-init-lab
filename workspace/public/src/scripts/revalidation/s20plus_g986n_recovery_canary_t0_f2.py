...
import s20plus_g986n_recovery_canary_t0_runner_h0 as h0  # noqa: E402
...
RUN_ROOT = ROOT / "workspace/private/runs/s20plus-g986n-recovery-canary-t0-f2"
...
APPROVAL_PREFIX = "S20PLUS-G986N-RECOVERY-CANARY-T0-F2-APPROVE:"
PHYSICAL_PREFIX = "S20PLUS-G986N-T0-PHYSICAL-ROLLBACK-CONFIRM:"
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise T0F2Error("value is not canonical JSON") from exc
...
def _exact(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != keys:
        raise T0F2Error(f"{label} schema differs")
    return value
...
def _text(value: Any, label: str, maximum: int = 4096) -> str:
    if (
        ...
        raise T0F2Error(f"{label} is not bounded text")
    return value
...
def _hex64(value: Any, label: str) -> str:
    ...
    if HEX64_RE.fullmatch(text) is None:
        raise T0F2Error(f"{label} is not SHA-256")
    return text


def _integer(value: Any, label: str, minimum: int = 0) -> int:
    if type(value) is not int or value < minimum:
        raise T0F2Error(f"{label} is not an integer")
    return value
...
def _validate_endpoint(value: Any, label: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise T0F2Error(f"{label} identity differs")
    return item
...
def _validate_capture_receipt(
    ...
    if path.parent != run_dir or not path.name.endswith(".capture.json"):
        raise T0F2Error(f"{label} path escapes the run")
...
def _require_capture_name(receipt: dict[str, Any], pattern: str, label: str) -> None:
    if re.fullmatch(pattern, Path(receipt["path"]).name) is None:
        raise T0F2Error(f"{label} capture name differs")
...
def _validate_health(value: Any, run_dir: Path, label: str) -> dict[str, Any]:
    ...
    if item["target"] != h0.EXPECTED_TARGET:
        raise T0F2Error(f"{label} target differs")
    ...
    if item["topology_sha256"] != b0.EXPECTED_ANDROID_TOPOLOGY_SHA256:
        raise T0F2Error(f"{label} topology differs")
    expected_public = {
    ...
    if item["public_health"] != expected_public or item["root_health"] != b0.EXPECTED_ROOT_OUTPUT:
        raise T0F2Error(f"{label} Android/root health differs")
    ...
    if (
        ...
        raise T0F2Error(f"{label} root capture differs")
    ...
    if (
        ...
        raise T0F2Error(f"{label} recovery is not exact stock")
    ...
    if (
        ...
        raise T0F2Error(f"{label} recovery capture differs")
    match = re.fullmatch(
        r"(prepare|execute-pre-download|pre-candidate-abort|final)-"
    ...
    if match is None:
        raise T0F2Error(f"{label} recovery capture phase differs")
    phase = match.group(1)
    intent = _exact(
        b0.read_json(
    ...
    if (
        intent["schema"]
        != "s20plus_g986n_recovery_canary_t0_recovery_read_intent_v1"
        ...
        raise T0F2Error(f"{label} recovery read intent differs")
...
def _validate_baseline(value: Any, label: str) -> dict[str, Any]:
    try:
        ...
        raise T0F2Error(f"{label} differs") from exc
...
def _file_receipt(path: Path, size: int, sha256: str, label: str) -> dict[str, Any]:
    try:
        ...
        raise T0F2Error(f"{label} is unavailable") from exc
    if (
        path.is_symlink()
...
def normalized_self_sha256() -> str:
    ...
    if active_count != 1 or hash_count != 1:
        raise T0F2Error("runner normalization grammar changed")
...
def self_receipt(*, enforce_reviewed: bool = True) -> dict[str, Any]:
    ...
    if (
        ...
        raise T0F2Error("runner source is indirect")
    if enforce_reviewed and normalized != EXPECTED_REVIEWED_NORMALIZED_SHA256:
        raise T0F2Error("runner source is not independently reviewed")
    return {
        "path": str(SCRIPT),
...
def source_closure(*, enforce_self: bool = True) -> dict[str, Any]:
    ...
    if any(current[name] is not expected for name, expected in _BOUND_APIS.items()):
        raise T0F2Error("an imported execution API was rebound")
    receipts = {
...
def validate_host_closure(*, enforce_self: bool = True) -> dict[str, Any]:
    ...
    if (
        ...
        raise T0F2Error("T0 H0 artifact closure differs")
    return {"sources": sources, "host": host}


def require_active() -> None:
    if not T0_F2_ACTIVE:
        raise T0F2Error("S20+ recovery-canary T0 F2 is not active")
    source_closure()
...
def _run_id(path: Path) -> str:
    if path.parent != RUN_ROOT or RUN_ID_RE.fullmatch(path.name) is None:
        raise T0F2Error("run path is outside the closed namespace")
    return path.name
...
def allocate_run_dir() -> Path:
    ...
    for _ in range(32):
        ...
        b0.fsync_dir(RUN_ROOT)
        return path
    raise T0F2Error("unable to allocate a unique T0 run")


def resolve_run(run_id: str) -> Path:
    _ensure_private_roots()
    if RUN_ID_RE.fullmatch(run_id) is None:
        raise T0F2Error("run ID grammar differs")
    ...
    if (
        ...
        raise T0F2Error("run directory is indirect")
...
def validate_namespace(run_dir: Path) -> set[str]:
    ...
    with os.scandir(run_dir) as entries:
        for entry in entries:
            ...
            if (
                ...
                raise T0F2Error("T0 journal contains an indirect node")
    ...
    if unknown:
        raise T0F2Error("T0 journal contains an unknown node")
    one_shot_prefixes = (
    ...
    for prefix in one_shot_prefixes:
        ...
        if len(ordinals) > 1:
            raise T0F2Error(f"T0 journal reuses one-shot raw prefix {prefix}")
    ...
    if (
        ...
        raise T0F2Error("T0 run directory changed during validation")
    return actual


def _guard_value(run_dir: Path) -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_recovery_canary_t0_guard_v1",
        "version": VERSION,
...
def acquire_guard(run_dir: Path) -> None:
    if os.path.lexists(SHARED_GUARD):
        raise T0F2Error("shared S20+ action guard is already held")
...
def require_guard(run_dir: Path) -> None:
    if b0.read_json(SHARED_GUARD, "shared S20+ guard") != _guard_value(run_dir):
        raise T0F2Error("shared S20+ action guard differs")
...
def validate_candidate_claim(
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_global_claim_v1"
        ...
        raise T0F2Error("global candidate claim differs")
...
def require_fresh_candidate_unclaimed() -> None:
    ...
    if os.path.lexists(path):
        try:
            ...
            raise T0F2Error("global candidate claim is malformed") from exc
        raise T0F2Error("this exact T0 candidate is permanently consumed")
...
def _next_capture_name(run_dir: Path, prefix: str) -> str:
    for ordinal in range(1, 10_000):
        ...
        if not any(
            ...
            return name
    raise T0F2Error("raw-capture ordinal space is exhausted")
...
def android_stock_recovery_health(
    ...
    if prefix not in {
        ...
        raise T0F2Error("unknown fixed recovery-health phase")
    ...
    if os.path.lexists(read_intent_path):
        ...
        if (
            ...
            raise T0F2Error(f"{prefix} recovery read was consumed without a receipt")
        ...
        if captures or any(
            ...
            raise T0F2Error(f"{prefix} recovery evidence exists without intent")
    ...
    if (
        ...
        raise T0F2Error("Android identity changed during recovery digest read")
    return {
        "target": dict(h0.EXPECTED_TARGET),
        "serial_sha256": before["serial_sha256"],
        "topology_sha256": before["topology_sha256"],
...
def _adb_reboot_download(
    ...
    *,
    ...
    return {
        "schema": "s20plus_g986n_recovery_canary_t0_reboot_result_v1",
        "version": VERSION,
        "phase": phase,
...
def prepare() -> dict[str, Any]:
    ...
    b0.durable_json(
        ...
        {
            "schema": "s20plus_g986n_recovery_canary_t0_allocation_v1",
            "version": VERSION,
            "run_id": _run_id(run_dir),
    ...
    try:
        ...
        return {
            "verdict": "PREPARED_S20PLUS_G986N_RECOVERY_CANARY_T0_AWAITING_APPROVAL",
            "run_id": _run_id(run_dir),
        ...
        b0.durable_json(
            ...
            {
                "schema": "s20plus_g986n_recovery_canary_t0_failure_v1",
                "version": VERSION,
...
def read_prepared(
    ...
    *,
    ...
    prepared = b0.read_json(run_dir / "prepared.json", "T0 prepared binding")
    allocation = b0.read_json(run_dir / "allocation.json", "T0 allocation")
    if (
        set(prepared) != {
            "schema", "version", "binding", "binding_sha256", "approval_token", "at"
        }
        or prepared.get("schema") != "s20plus_g986n_recovery_canary_t0_prepared_v1"
        ...
        raise T0F2Error("prepared T0 binding is malformed")
    binding = _exact(
        prepared["binding"],
        {
    ...
    if (
        binding["schema"] != "s20plus_g986n_recovery_canary_t0_binding_v1"
        ...
        raise T0F2Error("prepared T0 binding fields differ")
    ...
    if stored_baseline != baseline:
        raise T0F2Error("stored Download baseline differs from preparation")
    expected_allocation = {
        "schema": "s20plus_g986n_recovery_canary_t0_allocation_v1",
        "version": VERSION,
        "run_id": _run_id(run_dir),
    ...
    if allocation != expected_allocation:
        raise T0F2Error("T0 allocation differs")
    ...
    if type(expires) is not int or (require_unexpired and int(time.time()) > expires):
        raise T0F2Error("prepared T0 approval expired")
...
def _validate_cage_binding(
    ...
    if (
        ...
        raise T0F2Error(f"{kind} process cage differs")
...
def _validate_cage_quiescence(
    ...
    if (
        ...
        raise T0F2Error(f"{kind} cage quiescence differs")
...
def _validate_approval(value: Any, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_approval_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T0F2Error("T0 approval differs")
...
def _validate_reboot_result(
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_reboot_result_v1"
        or item["version"] != VERSION
        or item["phase"] != phase
        or item["binding_sha256"] != binding_sha256
        ...
        raise T0F2Error(f"{phase} reboot result differs")
    ...
    if capture is None:
        if (
            ...
            raise T0F2Error(f"{phase} cut result differs")
        return item
    ...
    if (
        ...
        raise T0F2Error(f"{phase} reboot result is not raw-derived")
    return item
...
def _validate_transfer_intent_record(
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_transfer_intent_v1"
        or item["version"] != VERSION
        or item["kind"] != kind
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["ap"] != {
        ...
        raise T0F2Error(f"{kind} transfer intent differs")
    ...
    if os.path.lexists(run_dir / arrival_name):
        ...
        if not b0.same_download_session(arrival.get("endpoint", {}), item["endpoint"]):
            raise T0F2Error(f"{kind} intent endpoint is not arrival-bound")
...
def _validate_transfer_result_record(
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_transfer_result_v1"
        or item["version"] != VERSION
        or item["kind"] != kind
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T0F2Error(f"{kind} transfer result differs")
    ...
    if item["endpoint_post_state"] in {"same", "changed"}:
        ...
        if (
            ...
            raise T0F2Error(f"{kind} endpoint post identity differs")
    elif item["endpoint_post_identity"] is not None:
        raise T0F2Error(f"{kind} endpoint post state differs")
    capture = item["raw_capture"]
    if capture is None:
        if (
            ...
            raise T0F2Error(f"{kind} missing capture gained a result")
        return item
    ...
    if (
        ...
        raise T0F2Error(f"{kind} transfer result is not raw-derived")
    return item
...
def _validate_observation(
    ...
    if (
        ...
        raise T0F2Error("candidate recovery observation schema differs")
    item = value
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_observation_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["claim_verdict"] not in {"PROVED", "NO_PROOF"}
    ):
        raise T0F2Error("candidate recovery observation differs")
    _text(item["at"], "candidate observation time", 128)
    if item["claim_verdict"] == "NO_PROOF":
        if set(item) != no_proof:
            raise T0F2Error("NO_PROOF observation gained evidence")
        _text(item["reason"], "NO_PROOF reason")
        return item
    if set(item) != proved:
        raise T0F2Error("PROVED observation lacks evidence")
    if (
        ...
        raise T0F2Error("PROVED recovery identity differs")
    ...
    if (
        ...
        raise T0F2Error("PROVED recovery observation is not raw-derived")
    return item
...
def _validate_physical_arm(value: Any, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_physical_arm_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        or item["mode"] not in {
        ...
        raise T0F2Error("physical rollback arm differs")
    ...
    if item["mode"] == "empty-baseline-physical-entry":
        ...
        if (
            ...
            raise T0F2Error("physical empty baseline contains an endpoint")
    else:
        if item["baseline"] is not None:
            raise T0F2Error("already-Download arm contains a baseline")
...
def _validate_physical_observation_record(
    ...
    if kind not in {"initial", "resume"}:
        raise T0F2Error("physical observation kind differs")
    ...
    if not os.path.lexists(intent_path):
        if os.path.lexists(miss_path):
            raise T0F2Error("physical observation miss lacks intent")
        return None
    intent = _exact(
    ...
    if (
        intent["schema"]
        != "s20plus_g986n_recovery_canary_t0_physical_observation_intent_v1"
        or intent["version"] != VERSION
        ...
        raise T0F2Error(f"physical {kind} observation intent differs")
    ...
    if (
        miss["schema"]
        != "s20plus_g986n_recovery_canary_t0_physical_observation_miss_v1"
        ...
        raise T0F2Error(f"physical {kind} observation miss differs")
...
def _validate_rollback_arrival(
    ...
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_rollback_arrival_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T0F2Error("rollback Download arrival differs")
    ...
    if item["branch"] == "candidate-outcome-unproved-already-download":
        ...
        if (
            ...
            raise T0F2Error("candidate-failure rollback arrival provenance differs")
        ...
        if (
            ...
            raise T0F2Error("automatic rollback arrival lacks its reboot result")
        baseline = _validate_baseline(
            b0.read_json(
        ...
        if (
            ...
            raise T0F2Error("automatic rollback arrival provenance differs")
    else:
        ...
        if (
            ...
            raise T0F2Error("physical rollback arrival provenance differs")
...
def validate_journal(run_dir: Path, prepared: dict[str, Any]) -> set[str]:
    ...
    for node, required in dependencies.items():
        if node in actual and not required <= actual:
            raise T0F2Error(f"T0 journal dependency is missing for {node}")
    for phase in (
        "prepare", "execute-pre-download", "pre-candidate-abort", "final"
    ):
        ...
        if (
            ...
            raise T0F2Error(f"{phase} recovery read intent differs")
    ...
    if "candidate-download-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_recovery_canary_t0_download_intent_v1"
            or item["version"] != VERSION
            ...
            raise T0F2Error("candidate Download intent differs")
    ...
    if "recovery-boot-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_recovery_canary_t0_recovery_boot_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            ...
            raise T0F2Error("recovery boot intent differs")
    ...
    if "candidate-observation-intent.json" in actual:
        ...
        if (
            item["schema"] != "s20plus_g986n_recovery_canary_t0_observation_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            or type(item["attempt"]) is not int
            or item["attempt"] != 1
            ...
            raise T0F2Error("observation intent differs")
    ...
    if "rollback-download-intent.json" in actual:
        ...
        if (
            item["schema"]
            != "s20plus_g986n_recovery_canary_t0_rollback_download_intent_v1"
            or item["version"] != VERSION
            or item["binding_sha256"] != prepared["binding_sha256"]
            ...
            raise T0F2Error("rollback Download intent differs")
    ...
    if "physical-rollback-arm.json" in actual:
        ...
        if "physical-rollback-confirmation.json" in actual:
            ...
            if (
                confirmation["schema"]
                != "s20plus_g986n_recovery_canary_t0_physical_confirmation_v1"
                ...
                raise T0F2Error("physical confirmation differs")
            _text(confirmation["at"], "physical confirmation time", 128)
        ...
        if (
            ...
            raise T0F2Error("physical observation has both miss and arrival")
    ...
    if os.path.lexists(claim_path):
        ...
        if "candidate-download-arrival.json" not in actual:
            raise T0F2Error("global candidate claim lacks candidate Download arrival")
    elif "candidate-intent.json" in actual:
        raise T0F2Error("candidate intent lacks the global no-replay claim")
...
def _validate_terminal(
    ...
    terminal = b0.read_json(run_dir / "terminal.json", "T0 terminal")
    if terminal.get("schema") == "s20plus_g986n_recovery_canary_t0_pre_candidate_abort_v1":
        item = _exact(
            terminal,
            {
        ...
        if (
            ...
            raise T0F2Error("pre-candidate terminal differs")
        ...
        if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
            raise T0F2Error("pre-candidate abort did not return through a new boot")
    ...
    if not {
        ...
        raise T0F2Error("T0 terminal lacks its complete result graph")
    candidate = _validate_transfer_result_record(
        run_dir,
        b0.read_json(run_dir / "candidate-result.json", "candidate result"),
        "candidate",
    ...
    expected_verdict = (
        "PROVED_T0_RETURNED_STOCK_RECOVERY_HEALTHY"
        if proved
        else (
            "NO_PROOF_T0_RETURNED_STOCK_RECOVERY_HEALTHY"
            if rollback_complete
            else "NO_PROOF_T0_STOCK_RECOVERY_HEALTHY_TRANSFER_UNPROVED"
        )
    )
    if (
        item["schema"] != "s20plus_g986n_recovery_canary_t0_terminal_v1"
        or item["version"] != VERSION
        or item["binding_sha256"] != prepared["binding_sha256"]
        ...
        raise T0F2Error("T0 terminal is not graph-derived")
...
def _same_endpoint(expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if not b0.same_download_session(current, expected):
        raise T0F2Error("Download endpoint session changed")
    return current
...
def _transfer(
    ...
    if kind == "candidate":
        ...
        raise T0F2Error("unknown recovery transfer kind")
    ...
    if os.path.lexists(intent_path) or os.path.lexists(result_path):
        raise T0F2Error(f"{kind} attempt is already consumed")
    ...
    try:
        with h0.pin_regular_file(
            ...
            if (
                ...
                raise T0F2Error("Download endpoint changed at Odin dispatch")
            handle, stdout, stderr = _capture(
                run_dir,
                f"{kind}-transfer",
                [
                    str(b0.DASH), "-c", b0.ODIN_CAGE_ENTRY_SCRIPT,
                    "s20plus-t0-odin-cage", cage_binding["cage"], *command,
                ],
                timeout=300,
            ...
            if (
                ...
                raise T0F2Error("an opened transfer input changed across Odin")
            try:
...
def recover_transfer_result(
    ...
    if len(captures) > 1:
        raise T0F2Error(f"{kind} transfer has multiple raw receipts")
    handle = None
    stdout = b""
    stderr = b""
    raw_receipt = None
...
def recover_reboot_result(
    ...
    if len(captures) > 1:
        raise T0F2Error(f"{phase} reboot has multiple raw receipts")
    handle = None
    stdout = b""
    stderr = b""
    raw_receipt = None
...
def _candidate_claim(run_dir: Path, prepared: dict[str, Any]) -> None:
    ...
    b0.durable_json(
        path,
        {
            "schema": "s20plus_g986n_recovery_canary_t0_global_claim_v1",
            "version": VERSION,
            "run_id": _run_id(run_dir),
            "binding_sha256": prepared["binding_sha256"],
            "candidate_sha256": h0.CANDIDATE_AP_SHA256,
...
def _rollback_from_download(
    ...
    if not os.path.lexists(arrival_path):
        raise T0F2Error("stock rollback requires a durable bound Download arrival")
    ...
    if (
        ...
        raise T0F2Error("rollback Download arrival differs from dispatch input")
...
def execute(run_dir: Path, approval: str) -> dict[str, Any]:
    ...
    if approval != prepared["approval_token"]:
        raise T0F2Error("approval token does not match the prepared T0 binding")
    if os.path.lexists(run_dir / "approval.json"):
        raise T0F2Error("prepared T0 approval is already consumed")
    if digest(validate_host_closure()) != prepared["binding"]["closure_sha256"]:
        raise T0F2Error("T0 execution closure changed after preparation")
    b0.durable_json(
        run_dir / "approval.json",
        {
            "schema": "s20plus_g986n_recovery_canary_t0_approval_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
    ...
    if (
        ...
        raise T0F2Error("prepared Android boot changed before candidate Download")
    require_fresh_candidate_unclaimed()
    if digest(validate_host_closure()) != prepared["binding"]["closure_sha256"]:
        raise T0F2Error("T0 execution closure changed before candidate Download")
    baseline = prepared["binding"]["initial_download_baseline"]
    if digest(baseline) != digest(
        ...
        raise T0F2Error("candidate Download baseline changed")
    intent = {
        "schema": "s20plus_g986n_recovery_canary_t0_download_intent_v1",
        "version": VERSION,
        "phase": "candidate",
        "binding_sha256": prepared["binding_sha256"],
        "source": preflight,
    ...
    try:
        ...
        if not b0.same_download_session(current, endpoint):
            raise T0F2Error("post-candidate Download endpoint differs from the bound session")
        devices, listing_sha256 = b0.enumerate_download()
        if devices != [current["device"]]:
            raise T0F2Error("post-candidate Download listing differs")
        b0.durable_json(
            run_dir / "rollback-download-arrival.json",
            {
                "schema": "s20plus_g986n_recovery_canary_t0_rollback_arrival_v1",
                "version": VERSION,
                "binding_sha256": prepared["binding_sha256"],
...
def abort_pre_candidate(run_dir: Path) -> dict[str, Any]:
    ...
    if any(os.path.lexists(run_dir / name) for name in forbidden):
        raise T0F2Error("pre-candidate abort is unavailable after a transfer intent")
    ...
    if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
        raise T0F2Error("pre-candidate abort requires a later Android boot")
    terminal = {
        "schema": "s20plus_g986n_recovery_canary_t0_pre_candidate_abort_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "verdict": (
            "ABORTED_AFTER_CANDIDATE_CLAIM_NO_TRANSFER_STOCK_RECOVERY_HEALTHY"
...
def _recovery_observation(run_dir: Path, prepared: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    if os.path.lexists(run_dir / "candidate-observation-intent.json"):
        raise T0F2Error("candidate recovery observation is already consumed")
    b0.durable_json(
        run_dir / "candidate-observation-intent.json",
        {
            "schema": "s20plus_g986n_recovery_canary_t0_observation_intent_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
            "attempt": 1,
            "no_replay": True,
    ...
    def no_proof(reason: str) -> tuple[dict[str, Any], None]:
        return {
            "schema": "s20plus_g986n_recovery_canary_t0_observation_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
    ...
    while time.monotonic() < deadline:
        ...
        if len(matches) > 1:
            return no_proof("exact-target-ambiguous")
        if not matches:
            if expected_rows:
                return no_proof("prepared-serial-present-without-exact-target-metadata")
            time.sleep(2)
            continue
        row = matches[0]
        if (
            hashlib.sha256(row["serial"].encode()).hexdigest() != expected_serial
            or row["state"] != "recovery"
            or not b0.EXPECTED_ADB_METADATA <= row["metadata"]
        ):
            return no_proof("exact-target-state-or-identity-mismatch")
        try:
            serial = row["serial"]
            devpath = b0.adb_devpath(serial)
            if hashlib.sha256(devpath.encode()).hexdigest() != prepared["binding"]["preflight"]["topology_sha256"]:
                raise T0F2Error("recovery ADB topology differs")
            handle, stdout, stderr = _capture(
                run_dir,
                "candidate-recovery-observer",
            ...
            final_row = b0.exact_adb_row(final, expected_serial, "recovery")
...
def _automatic_rollback_download(
    ...
    intent = {
        "schema": "s20plus_g986n_recovery_canary_t0_rollback_download_intent_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
...
def observe_recovery(run_dir: Path) -> dict[str, Any]:
    ...
    if result["classification"] != "odin_transfer_completed":
        raise T0F2Error("only a completed candidate may enter recovery observation")
    b0.read_json(run_dir / "recovery-boot-intent.json", "recovery boot intent")
    ...
    if serial is None:
        return {
            "verdict": "NO_PROOF_T0_PHYSICAL_ROLLBACK_REQUIRED",
            "run_id": _run_id(run_dir),
            "candidate_replay": False,
        }
    endpoint = _automatic_rollback_download(run_dir, prepared, serial, observation)
    if endpoint is None:
        return {
            "verdict": "T0_OBSERVED_AUTOMATIC_ROLLBACK_ENTRY_UNPROVED",
            "run_id": _run_id(run_dir),
            "candidate_replay": False,
        }
    rollback = _rollback_from_download(
        run_dir, prepared, endpoint, "automatic-recovery-adb"
    )
    return {
        "verdict": "T0_OBSERVED_STOCK_ROLLBACK_ATTEMPTED",
        "run_id": _run_id(run_dir),
...
def arm_physical_rollback(run_dir: Path) -> dict[str, Any]:
    ...
    if os.path.lexists(run_dir / "rollback-intent.json"):
        raise T0F2Error("stock recovery rollback is already consumed")
    if not os.path.lexists(run_dir / "candidate-intent.json"):
        raise T0F2Error("physical stock rollback requires a consumed candidate intent")
    path = run_dir / "physical-rollback-arm.json"
    if os.path.lexists(path):
        raise T0F2Error("physical rollback arm is already consumed")
    if os.path.lexists(run_dir / "rollback-download-arrival.json"):
        raise T0F2Error("rollback Download arrival is already bound")
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T0F2Error("physical rollback baseline is ambiguous")
    ...
    if not devices:
        ...
        if (
            ...
            raise T0F2Error("existing Download endpoint is not the candidate session")
        baseline = None
...
def _publish_physical_observation_intent(
    ...
    if os.path.lexists(path):
        ...
        if record is None or record.get("schema") != (
            ...
            raise T0F2Error(f"physical {kind} observation is already closed")
        return record
    value = {
        "schema": "s20plus_g986n_recovery_canary_t0_physical_observation_intent_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "kind": kind,
...
def _publish_physical_observation_miss(
    ...
    b0.durable_json(
        ...
        {
            "schema": "s20plus_g986n_recovery_canary_t0_physical_observation_miss_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
            "kind": kind,
...
def confirm_physical_rollback(
    ...
    if (
        ...
        raise T0F2Error("physical rollback confirmation differs or expired")
    if os.path.lexists(run_dir / "physical-rollback-confirmation.json"):
        raise T0F2Error("physical rollback confirmation is consumed; use --resume")
    b0.durable_json(
        run_dir / "physical-rollback-confirmation.json",
        {
            "schema": "s20plus_g986n_recovery_canary_t0_physical_confirmation_v1",
...
def _wait_android_health(
    ...
    while time.monotonic() < deadline:
        try:
            ...
            raise T0F2Error("final ADB inventory is malformed") from exc
        matches = b0.target_adb_rows(rows)
        expected_rows = tuple(
            row
            for row in rows
        ...
        if len(matches) > 1:
            raise T0F2Error("final exact-target ADB inventory is ambiguous")
        if not matches:
            if expected_rows:
                raise T0F2Error(
        ...
        if (
            ...
            raise T0F2Error("final exact target state or identity differs")
        break
    else:
        raise T0F2Error("final exact Android arrival timed out")
    health, _serial = android_stock_recovery_health(
        run_dir,
        "final",
        expected_serial,
...
def _publish_recovery_boot_intent(
    ...
    if os.path.lexists(run_dir / "recovery-boot-intent.json"):
        return
    if candidate["classification"] != "odin_transfer_completed":
        raise T0F2Error("unproved candidate cannot gain a recovery boot intent")
    b0.durable_json(
        run_dir / "recovery-boot-intent.json",
        {
            "schema": "s20plus_g986n_recovery_canary_t0_recovery_boot_intent_v1",
            "version": VERSION,
            "binding_sha256": prepared["binding_sha256"],
...
def _bind_current_candidate_download(
    ...
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T0F2Error("candidate Download resume state is ambiguous")
    if not devices:
        return None
    endpoint = b0.identify_download()
    if endpoint["device"] != devices[0]:
        raise T0F2Error("candidate Download listing changed")
    arrival = {
        "endpoint": endpoint,
...
def _bind_current_rollback_download(
    ...
    devices, listing_sha256 = b0.enumerate_download()
    if len(devices) > 1:
        raise T0F2Error("rollback Download resume state is ambiguous")
    if not devices:
        return None
    endpoint = b0.identify_download()
    if endpoint["device"] != devices[0]:
        raise T0F2Error("rollback Download listing changed")
    if branch == "automatic-recovery-adb":
        ...
        raise T0F2Error("rollback resume branch differs")
    arrival = {
        "schema": "s20plus_g986n_recovery_canary_t0_rollback_arrival_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
...
def _pending(run_dir: Path, prepared: dict[str, Any], verdict: str, **extra: Any) -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_recovery_canary_t0_pending_v1",
        "version": VERSION,
        "run_id": _run_id(run_dir),
        "binding_sha256": prepared["binding_sha256"],
        "verdict": verdict,
...
def resume_run(run_dir: Path) -> dict[str, Any]:
    ...
    if candidate["classification"] != "odin_transfer_completed":
        ...
        if observation["claim_verdict"] == "PROVED":
            if not os.path.lexists(run_dir / "rollback-download-intent.json"):
                rows = b0.adb_inventory()
                row = b0.exact_adb_row(
                    rows,
                    prepared["binding"]["preflight"]["serial_sha256"],
                    "recovery",
                )
                endpoint = _automatic_rollback_download(
                    run_dir, prepared, row["serial"], observation
                )
                if endpoint is None:
                    return _pending(
                        run_dir,
                        prepared,
                        "AUTOMATIC_ROLLBACK_ENTRY_UNPROVED_PHYSICAL_REQUIRED",
                    )
            else:
                if not os.path.lexists(run_dir / "rollback-download-result.json"):
                    recover_reboot_result(run_dir, prepared, "rollback")
                if not os.path.lexists(run_dir / "rollback-download-arrival.json"):
                    endpoint = _bind_current_rollback_download(
                        run_dir, prepared, "automatic-recovery-adb"
                    )
                    if endpoint is None:
                        return _pending(
                            run_dir,
            ...
            return _pending(
                run_dir, prepared, "NO_PROOF_T0_PHYSICAL_ROLLBACK_REQUIRED"
            )
...
def finalize(run_dir: Path) -> dict[str, Any]:
    ...
    if health["boot_id_sha256"] == prepared["binding"]["preflight"]["boot_id_sha256"]:
        raise T0F2Error("final Android reused the prepared source boot ID")
    observation = (
        _validate_observation(
            run_dir,
    ...
    if proved:
        verdict = "PROVED_T0_RETURNED_STOCK_RECOVERY_HEALTHY"
    elif rollback_complete:
        verdict = "NO_PROOF_T0_RETURNED_STOCK_RECOVERY_HEALTHY"
    else:
        verdict = "NO_PROOF_T0_STOCK_RECOVERY_HEALTHY_TRANSFER_UNPROVED"
    terminal = {
        "schema": "s20plus_g986n_recovery_canary_t0_terminal_v1",
        "version": VERSION,
        "binding_sha256": prepared["binding_sha256"],
        "verdict": verdict,
        "candidate_claim": (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Operate the exact S20+ recovery-canary T0 F2 owner"
    )
