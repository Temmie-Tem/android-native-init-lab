#!/usr/bin/env python3
"""Build V2895 native-init candidate with mono1 video stream support."""
...
CYCLE = "V2895"
INIT_VERSION = "0.10.32"
INIT_BUILD = "v2895-video-mono1-stream"
BUILD_TAG = INIT_BUILD
DECISION = "v2895-video-mono1-stream-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2895_VIDEO_MONO1_STREAM_SOURCE_BUILD_2026-06-19.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2895_video_mono1_stream.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2895_video_mono1_stream"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2895_video_mono1_stream.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v462_video_mono1_stream"
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2895 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    markers = manifest.get("v2895_marker_strings", [])
    ...
    return "\n".join([
        ...
        *marker_lines,
        "- Device validation is deferred to V2896: flash this exact image, run a 30fps mono1 page-flip stream and verify expansion/cadence telemetry, then rollback to `v2321`.",
        "",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "video-mono1-stream-candidate",
        "parent_test_artifact": "v2894-video-gray8-cache-hit",
        "mono1_stream": {
            "version": 1,
            "source_unit": "V2895",
        ...
        "v2895_marker_strings": marker_strings,
    ...
    (OUT_DIR / "video-mono1-stream-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "video-mono1-stream-candidate",
