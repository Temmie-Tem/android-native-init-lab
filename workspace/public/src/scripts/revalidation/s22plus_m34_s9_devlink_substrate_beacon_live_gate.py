#!/usr/bin/env python3
"""Guarded S22+ M34 S9 download-beacon state-probe live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M34 S9 keeps the S8B1A wide B1 beacon, closes the Waipio devlink supplier
substrate load-set, skips downstream configfs/UDC work, reads one predicate
after module load, then uses reboot(download) as a 1-bit host-visible state
probe:
...
from s22plus_m34_s7a2_geni_i2c_live_gate import M34_S7A2_I2C_ORDER_ACTUAL
...
LIVE_ACK_TOKEN = "S22PLUS-M34-S9-BEACON-PROBE-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M34-S9-BEACON-PROBE-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_STAGE = "S9"
DISPLAY_SERIAL_REDACTED = "<S22_SERIAL_REDACTED>"
EXPECTED_M34_MARKER = "S22_NATIVE_INIT_M34_RUNTIME_GADGET_SPLIT_S9"
EXPECTED_M34_AP_SHA256 = "41a76ac1404c99273e9ec3aeae591dbfc94e1aa83daf97de9a7068e3c155022f"
EXPECTED_M34_BOOT_SHA256 = "509a05e4ff97dad39ca52eae6c57169e20d3ddbf1524d292e8c91b9286a80414"
EXPECTED_M34_INIT_SHA256 = "9f231faff6154dc08b6b4d1b6cd169e82c81bfdc1e8d02cc92d1ea5a02dbd390"
EXPECTED_M34_MODULE_LIST_SHA256 = "c07425f4c738b53822e9f6783a142a2b5eafd72a15bd34c06fb3b49357c8fe26"
EXPECTED_M34_TEMPLATE_SOURCE_SHA256 = "8364aca94582fc325f89855b5cfd4e47ff8e41d2f18c341c99bd750ea3ebe3ae"
...
EXPECTED_MODULE_ENTRY = "s22plus_m34_s9_runtime_gadget_split.modules"
EXPECTED_STOCK_RECIPE_REPORT = "docs/reports/S22PLUS_STOCK_USB_GADGET_ACM_RECIPE_2026-07-09.md"
EXPECTED_PROBE = "typec_port_or_i2c_any_0066"
EXPECTED_PROBE_PATHS = ["/sys/class/typec/port0", "/sys/bus/i2c/devices/*-0066"]
ANDROID_MAX77705_I2C_ALIAS = "/sys/bus/i2c/devices/57-0066"
...
M34_S9_RISK_MODULES = M34_S7A_RISK_MODULES
M34_S9_DEVLINK_SUPPLIER_ORDER_ACTUAL = [
    "qcom_rpmh.ko",
    "clk-rpmh.ko",
    "gdsc-regulator.ko",
    "clk-qcom.ko",
...
DEFAULT_M34_AP = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_10/S9/odin4/AP.tar.md5")
DEFAULT_M34_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_10/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m34_s9_devlink_substrate_beacon_live_gate_{utc_stamp()}")
...
def agents_exception_draft() -> str:
    ...
    return f"""   **DRAFT ONLY - Narrow operator-authorized exception (2026-07-09, S22+ M34 S9 download-beacon state-probe boot-only live gate):**
   ...
   one bounded attended boot-partition-only M34 S9 live gate on the Samsung S22+
   ...
   `s8_beacon_probe={EXPECTED_PROBE}` / `predicate=typec_port_or_i2c_any_0066`,
...
def verify_agents_text(agents: str, log_path: Path, *, source_label: str) -> None:
    ...
    if draft_only:
        raise SystemExit(
            f"{source_label} contains draft-only M34 S9 authorization text; "
    ...
    if missing:
        raise SystemExit(f"{source_label} missing M34 S9 beacon-probe authorization markers: {missing}")
    ...
    if not active_template_present:
        raise SystemExit(
            f"{source_label} marker coverage is present but exact M34 S9 active authorization template is absent"
        )
...
ACTIVE_EXCEPTION_INSERT_ANCHOR = "   **Consumed exception (2026-07-09, S22+ M34 S8B1 download-beacon\n"
...
def agents_candidate_text(current_agents: str) -> str:
    ...
    if has_draft_only_m34_exception(current_agents):
        raise SystemExit("source AGENTS contains draft-only M34 S9 text; refuse to build active candidate over it")
    if has_exact_active_exception_template(current_agents):
        return current_agents
    if not missing_policy_markers(current_agents):
        raise SystemExit("source AGENTS is marker-complete but exact M34 S9 active template is absent")
    if ACTIVE_EXCEPTION_INSERT_ANCHOR not in current_agents:
        raise SystemExit("source AGENTS missing M34 S8B1 consumed-exception insertion anchor")
...
def verify_m34_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m34_s9_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m34_s9_manifest_runtime_steps={json.dumps(runtime_steps, sort_keys=True)}")
    append_log(log_path, f"m34_s9_manifest_closure={json.dumps(closure, sort_keys=True)}")
    ...
    if matrix.get("host_build_order") != ["S1", "S2", "S3", "S4", "S5", "S6", "S7A", "S7A2", "S8B1", "S8B1A", "S9"]:
        raise SystemExit(f"M34 host-build order mismatch: {matrix.get('host_build_order')!r}")
    if matrix.get("next_host_only_candidate") != "S9":
        raise SystemExit(f"M34 next host-only candidate mismatch: {matrix.get('next_host_only_candidate')!r}")
    if matrix.get("s9_download_beacon_probe") != EXPECTED_PROBE:
        raise SystemExit(f"M34 S9 probe mismatch: {matrix.get('s9_download_beacon_probe')!r}")
    if matrix.get("s9_true_action") != "reboot(download)" or matrix.get("s9_false_action") != "park":
        raise SystemExit("M34 S9 true/false action mismatch")
    if matrix.get("s9_probe_paths") != EXPECTED_PROBE_PATHS:
        raise SystemExit(f"M34 S9 probe paths mismatch: {matrix.get('s9_probe_paths')!r}")
    if matrix.get("s9_starts_from_s8b1a_b1_recipe") is not True:
        raise SystemExit("M34 S9 matrix does not prove S8B1A B1 recipe base")
    if matrix.get("s9_closes_devlink_supplier_substrate") is not True:
        raise SystemExit("M34 S9 matrix does not prove devlink supplier substrate closure")
    if matrix.get("s9_devlink_supplier_load_set") != M34_S9_DEVLINK_SUPPLIER_LOAD_SET:
        raise SystemExit(f"M34 S9 devlink load-set mismatch: {matrix.get('s9_devlink_supplier_load_set')!r}")
    if matrix.get("s9_devlink_supplier_new_modules") != M34_S9_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S9 devlink new-module set mismatch: {matrix.get('s9_devlink_supplier_new_modules')!r}")
    if matrix.get("s9_skips_downstream_configfs_and_udc_to_isolate_probe") is not True:
        raise SystemExit("M34 S9 matrix does not prove downstream USB isolation")

    if stage.get("stage_number") != 11:
        raise SystemExit(f"M34 S9 stage number mismatch: {stage.get('stage_number')!r}")
    expected_steps = {
    ...
    if runtime_steps != expected_steps:
        raise SystemExit(f"M34 S9 runtime steps mismatch: {runtime_steps!r}")
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M34 S9 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if stage.get("tar_members") != [EXPECTED_MEMBER]:
        raise SystemExit(f"M34 S9 manifest tar members mismatch: {stage.get('tar_members')!r}")
    ...
    required_safety = {
        ...
        "stage_s9_beacon_probe": EXPECTED_PROBE,
    ...
    if closure.get("module_count") != EXPECTED_MODULE_COUNT:
        raise SystemExit(f"M34 S9 module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M34_MODULE_LIST_SHA256:
        raise SystemExit(f"M34 S9 module list SHA mismatch: {closure.get('module_sha256')!r}")
    if closure.get("devlink_supplier_load_set") != M34_S9_DEVLINK_SUPPLIER_LOAD_SET:
        raise SystemExit(f"M34 S9 closure devlink load-set mismatch: {closure.get('devlink_supplier_load_set')!r}")
    if closure.get("devlink_supplier_new_modules") != M34_S9_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S9 closure devlink new-module set mismatch: {closure.get('devlink_supplier_new_modules')!r}")
    if closure.get("devlink_supplier_order_actual") != M34_S9_DEVLINK_SUPPLIER_ORDER_ACTUAL:
        raise SystemExit(f"M34 S9 closure devlink order mismatch: {closure.get('devlink_supplier_order_actual')!r}")
    if closure.get("geni_i2c_transport_targets") != M34_S7A2_I2C_GENI_TRANSPORT_TARGETS:
        raise SystemExit(f"M34 S9 closure GENI I2C targets mismatch: {closure.get('geni_i2c_transport_targets')!r}")
    if closure.get("geni_i2c_transport_order_actual") != M34_S7A2_I2C_ORDER_ACTUAL:
        raise SystemExit(f"M34 S9 closure GENI I2C order mismatch: {closure.get('geni_i2c_transport_order_actual')!r}")
    modules = closure.get("modules", [])
    if modules.index("i2c-msm-geni.ko") >= modules.index("pdic_max77705.ko"):
        raise SystemExit("M34 S9 module order must load i2c-msm-geni.ko before pdic_max77705.ko")
    if closure.get("session_producer_targets") != M34_S7A_SESSION_PRODUCER_TARGETS:
        raise SystemExit(f"M34 S9 session-producer targets mismatch: {closure.get('session_producer_targets')!r}")
    if closure.get("additional_new_modules") != M34_S9_EXPECTED_NEW_MODULES:
        raise SystemExit(f"M34 S9 additional module list mismatch: {closure.get('additional_new_modules')!r}")
    if closure.get("risk_modules") != M34_S9_RISK_MODULES:
        raise SystemExit(f"M34 S9 risk module list mismatch: {closure.get('risk_modules')!r}")
    if closure.get("contains_sec_debug_region") is not True:
        raise SystemExit("M34 S9 closure does not explicitly carry sec_debug_region")
    if closure.get("requires_live_risk_review") is not True:
        raise SystemExit("M34 S9 closure does not require live risk review")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M34 S9 manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M34 S9 module-list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M34 S9 must not inject module binaries into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M34 S9 required string missing from manifest: {required}")
    for forbidden in [
        "phase=configfs_done",
        ...
        if forbidden in required_strings:
            raise SystemExit(f"M34 S9 manifest unexpectedly requires forbidden string: {forbidden}")

    objdump_path = path.parent / EXPECTED_STAGE / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M34 S9 objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S9 /init does not load arm64 __NR_finit_module (273)")
    if not any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S9 /init does not load arm64 __NR_reboot (142)")
...
def verify_m34_artifacts(
    *,
    ...
    verify_ap(m34_ap, EXPECTED_M34_AP_SHA256, "m34_s9_candidate", log_path)
...
def android_predicate_baseline_path(run_dir: Path) -> Path:
    return run_dir / "s22plus_m34_s9_android_predicate_baseline.json"


def android_reset_context_baseline_path(run_dir: Path) -> Path:
    return run_dir / "s22plus_m34_s9_android_reset_context_baseline.json"
...
def read_android_root_text_value(serial: str, path: str, log_path: Path) -> str:
    ...
    append_log(log_path, f"s9_android_root_value_rc_{key}={proc.returncode}")
    if proc.returncode != 0:
        append_log(log_path, f"s9_android_root_value_stderr_{key}={proc.stderr.strip()!r}")
        return "<missing>"
    return proc.stdout.replace("\n", " ").strip()


def collect_android_s9_predicate_baseline(
    *,
    run_dir: Path,
    log_path: Path,
    serial: str,
) -> dict[str, Any]:
    probe_paths = " ".join(shlex.quote(path) for path in ["/sys/class/typec/port0", ANDROID_MAX77705_I2C_ALIAS, *ANDROID_S8B2_HINT_PATHS])
    class_value_paths = [
        "/sys/class/typec/port0/data_role",
        "/sys/class/typec/port0/power_role",
...
""".replace("__PROBE_PATHS__", probe_paths).replace("__CLASS_VALUE_PATHS__", class_values)
    ...
    append_log(log_path, f"s9_android_predicate_baseline_rc={proc.returncode}")
    append_log(log_path, f"s9_android_predicate_baseline_stdout={proc.stdout.strip()!r}")
    append_log(log_path, f"s9_android_predicate_baseline_stderr={proc.stderr.strip()!r}")
    if proc.returncode != 0:
        raise SystemExit(f"failed to collect Android S9 predicate baseline: rc={proc.returncode}")
    ...
    for raw_line in proc.stdout.splitlines():
        ...
        if kind == "exists":
            ...
            if path == ANDROID_MAX77705_I2C_ALIAS:
                alias_exists = value == "1"
            else:
                target.setdefault(path, {})["exists"] = value == "1"
        elif kind == "realpath":
            target = hint_paths if path in ANDROID_S8B2_HINT_PATHS else paths
            if path == ANDROID_MAX77705_I2C_ALIAS:
                alias_realpath = value
            else:
    ...
    paths["/sys/bus/i2c/devices/*-0066"] = {
        "exists": alias_exists,
        "matched": [ANDROID_MAX77705_I2C_ALIAS] if alias_exists else [],
        "realpath": alias_realpath,
    }
    predicate_true = any(bool(paths.get(path, {}).get("exists")) for path in EXPECTED_PROBE_PATHS)
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s9_android_predicate_baseline_v1",
        ...
        "future_b2_hints": {
            ...
            "max77705_i2c_device_exists": bool(paths.get("/sys/bus/i2c/devices/*-0066", {}).get("exists")),
            "max77705_i2c_matched_paths": paths.get("/sys/bus/i2c/devices/*-0066", {}).get("matched", []),
    ...
    append_log(log_path, f"s9_android_predicate_baseline_json={path}")
    append_log(log_path, f"s9_android_predicate_baseline={json.dumps(payload, sort_keys=True)}")
    if not predicate_true:
        raise SystemExit(f"Android S9 predicate baseline is false for {EXPECTED_PROBE_PATHS}")
    return payload
...
def collect_android_reset_context_baseline(
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s9_android_reset_context_baseline_v1",
        "timestamp_utc": utc_now(),
        "serial": serial,
        "summary": summary,
    }
    ...
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    append_log(log_path, f"s9_android_reset_context_baseline_json={path}")
    append_log(
        log_path,
        "s9_android_reset_context_baseline="
...
def run_android_readonly_preflight(
    *,
    ...
    collect_android_s9_predicate_baseline(run_dir=run_dir, log_path=log_path, serial=selected_serial)
...
def write_result_summary(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "s22plus_m34_s9_result_v1",
        "timestamp_utc": utc_now(),
...
def write_result_analysis(run_dir: Path, log_path: Path, result_json: Path) -> None:
    try:
        from analyze_s22plus_m34_s9_result import classify_result, load_json
        ...
        analysis_path = run_dir / "s22plus_m34_s9_result_analysis.json"
...
def live_runbook(args: argparse.Namespace, packet_path: Path | None = None) -> str:
    helper = "workspace/public/src/scripts/revalidation/s22plus_m34_s9_devlink_substrate_beacon_live_gate.py"
    analyzer = "workspace/public/src/scripts/revalidation/analyze_s22plus_m34_s9_result.py"
    ...
    lines = [
        "# S22+ M34 S9 live runbook (no command below inserts AGENTS.md by itself)",
...
def write_prelive_packet(
    *,
    ...
    runbook_path = run_dir / "s22plus_m34_s9_live_runbook.txt"
    active_template_path = run_dir / "s22plus_m34_s9_active_exception_template.txt"
    packet_path = run_dir / "s22plus_m34_s9_prelive_packet.json"
    runbook = live_runbook(packet_args, packet_path=packet_path)
    baseline_path = android_predicate_baseline_path(run_dir)
    if not baseline_path.is_file():
        raise SystemExit(f"prelive packet requires Android S9 predicate baseline: {baseline_path}")
    baseline_payload = json.loads(baseline_path.read_text(encoding="utf-8"))
    if not baseline_payload.get("predicate_true"):
        raise SystemExit("prelive packet refuses false Android S9 predicate baseline")
    ...
    if reset_baseline_payload.get("schema") != "s22plus_m34_s9_android_reset_context_baseline_v1":
        raise SystemExit(
    ...
    material_paths = {
        ...
        "android_s9_predicate_baseline_json": baseline_path,
        "android_reset_context_baseline_json": reset_baseline_path,
    }
    payload = {
        "schema": "s22plus_m34_s9_prelive_packet_v1",
        ...
        "android_s9_predicate_baseline": baseline_payload,
        "android_s9_predicate_baseline_json": str(baseline_path),
...
def verify_prelive_packet(
    *,
    ...
    baseline = payload.get("android_s9_predicate_baseline")
    if not isinstance(baseline, dict):
        raise SystemExit("prelive packet missing Android S9 predicate baseline")
    if baseline.get("schema") != "s22plus_m34_s9_android_predicate_baseline_v1":
        raise SystemExit(f"prelive packet Android S9 predicate baseline schema mismatch: {baseline.get('schema')!r}")
    if baseline.get("serial") != selected_serial:
        raise SystemExit("prelive packet Android S9 predicate baseline serial mismatch")
    if baseline.get("probe") != EXPECTED_PROBE or baseline.get("probe_paths") != EXPECTED_PROBE_PATHS:
        raise SystemExit("prelive packet Android S9 predicate baseline probe mismatch")
    if not baseline.get("predicate_true"):
        raise SystemExit("prelive packet Android S9 predicate baseline is false")
    hints = baseline.get("future_b2_hints")
    if not isinstance(hints, dict):
        raise SystemExit("prelive packet Android S9 predicate baseline missing future_b2_hints")
    if hints.get("candidate_paths") != ANDROID_S8B2_HINT_PATHS:
        raise SystemExit("prelive packet Android S9 future B2 candidate path mismatch")
    if hints.get("value_paths") != ANDROID_S8B2_HINT_VALUE_PATHS:
        raise SystemExit("prelive packet Android S9 future B2 value path mismatch")
    if not isinstance(hints.get("paths"), dict) or not isinstance(hints.get("values"), dict):
        raise SystemExit("prelive packet Android S9 future B2 hints are malformed")
    baseline_json = Path(str(payload.get("android_s9_predicate_baseline_json", "")))
    if baseline_json != android_predicate_baseline_path(packet_run_dir):
        raise SystemExit("prelive packet Android S9 predicate baseline path mismatch")
    if not baseline_json.is_file():
        raise SystemExit(f"prelive packet Android S9 predicate baseline JSON missing: {baseline_json}")
    if json.loads(baseline_json.read_text(encoding="utf-8")) != baseline:
        raise SystemExit("prelive packet Android S9 predicate baseline JSON is stale")
    ...
    if reset_baseline.get("schema") != "s22plus_m34_s9_android_reset_context_baseline_v1":
        raise SystemExit(
            "prelive packet Android reset-context baseline schema mismatch: "
    ...
    expected_material_paths = {
        "runbook": runbook_path,
        "active_exception_template": active_template_path,
        "android_s9_predicate_baseline_json": baseline_json,
...
def verify_prelive_packet_dryrun_evidence(dryrun_dir: Path, log_path: Path) -> None:
    dryrun_log = dryrun_dir / "s22plus_m34_s9_devlink_substrate_beacon_live_gate.txt"
...
def observe_download_beacon(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m34_s9_observe_start_utc={utc_now()}")
    append_log(log_path, f"m34_s9_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m34_s9_beacon_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m34_s9_result=download-beacon-hit elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "download-beacon-hit", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M34 S9 observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m34_s9_result=unexpected-adb-before-rollback elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-rollback", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m34_s9_result=download-beacon-miss-parked-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m34_s9_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if actual_rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m34_s9_{label}_boot_restore_hash_check=skipped rollback_target={actual_rollback_target}")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m34_s9_{label}_rollback", android, marker=EXPECTED_M34_MARKER)
    append_log(log_path, f"m34_s9_{label}_retained_marker_found={int(marker_found)}")
    return RollbackResult(
        rc=0,
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m34-s9-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M34 S9 rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    if args.observe_sec < 30:
        raise SystemExit("--observe-sec must be at least 30 for the M34 S9 download-beacon probe")
    ...
    if print_only_mode:
        with tempfile.TemporaryDirectory(prefix="s22plus_m34_s9_print_") as tmp:
            log_path = Path(tmp) / "s22plus_m34_s9_devlink_substrate_beacon_live_gate.txt"
            append_log(log_path, f"=== {utc_now()} s22plus M34 S9 download-beacon probe live gate ===")
            ...
            if args.write_agents_candidate is not None:
                ...
                print(
                    "write-agents-candidate ok: exact M34 S9 active exception inserted into candidate; "
            ...
            if args.verify_agents_candidate is not None:
                ...
                print(
                    "verify-agents-candidate ok: exact M34 S9 active exception is present; "
                    f"no AGENTS.md write, no device action; candidate={candidate_path}"
                )
                return 0
    ...
    log_path = run_dir / "s22plus_m34_s9_devlink_substrate_beacon_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M34 S9 download-beacon probe live gate ===")
    append_log(log_path, f"target={EXPECTED_TARGET}")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M34 S9 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.readonly_preflight:
        ...
        print(
            "readonly-preflight ok: M34 S9 candidate, rollback APs, Android stability, "
            f"and current boot hash verified for {DISPLAY_SERIAL_REDACTED}; no AGENTS exception required; log={log_path}"
        )
        return 0
    ...
    if args.prelive_packet:
        ...
        print(
            "prelive-packet ok: M34 S9 artifacts, rollback APs, Android stability, "
            f"current boot hash, active-template, and runbook captured for {DISPLAY_SERIAL_REDACTED}; "
    ...
    if args.verify_prelive_packet is not None:
        ...
        print(
            "verify-prelive-packet ok: packet matches current S9 helper contract, "
            f"selected_serial={DISPLAY_SERIAL_REDACTED}; no device action; log={log_path}"
        )
        return 0
    ...
    if args.verify_prelive_packet_after_dryrun is not None:
        ...
        print(
            "verify-prelive-packet-after-dryrun ok: packet matches current S9 helper contract, "
            f"dry-run evidence checked, live dir absent, selected_serial={DISPLAY_SERIAL_REDACTED}; "
    ...
    if args.rollback_from_download:
        ...
        print(f"M34 S9 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M34 S9 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M34 S9 live gate starting. HIT should self-enter Download mode if "
        "typec port0 or any i2c *-0066 device exists; MISS parks and needs manual Download rollback.",
        flush=True,
    )
    ...
    if odin_device is None:
        ...
        print("download mode did not appear for M34 S9 candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        ...
        print(f"M34 S9 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m34_s9_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M34 S9 candidate flashed. Observing download beacon for {args.observe_sec}s.", flush=True)
    ...
    if result == "download-beacon-hit":
        if rollback_device is None:
            ...
            print(f"M34 S9 HIT recorded but no rollback device is available. log={log_path}", file=sys.stderr)
            return 4
        rollback = rollback_boot_only_from_download(
            odin=odin,
            rollback_ap=rollback_ap,
        ...
        print(f"M34 S9 live gate completed rc={rollback.rc}; result={result}; log={log_path}")
        return rollback.rc
    ...
    if result != "download-beacon-miss-parked-manual-download-required":
        ...
        print(
            f"M34 S9 stopped before clean HIT/MISS proof ({result}). If the device is not in Android, "
    ...
    print(
        f"M34 S9 result={result}. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        ...
        print(
            f"M34 S9 MISS observed, but manual Download mode did not appear. "
