#!/usr/bin/env python3
"""P2.98 three-tier identity for gadget-start/event telemetry."""
...
import s22plus_fyg8_p296_identity_tiers as inherited_identity
import s22plus_fyg8_p296_source_contract as inherited_contract
...
SCHEMA = "s22plus_fyg8_p298_three_tier_identity_v1"
PROFILE = spec.PROFILE
SOURCE_CHECK_RUN_ID = hashlib.sha256(
    b"S22PLUS-FYG8-P298-SOURCE-CHECK-V1"
).digest()[:16]
...
TIER1_DIRECT_PATHS = {
    "p298_identity_tiers": PREFIX / "s22plus_fyg8_p298_identity_tiers.py",
    "p298_telemetry_spec": PREFIX / "s22plus_fyg8_p298_telemetry_spec.py",
    "p298_telemetry_transform": PREFIX / "s22plus_fyg8_p298_telemetry_transform.py",
    "p298_telemetry_generator": PREFIX / "s22plus_fyg8_p298_telemetry_generator.py",
    "p298_source_contract": PREFIX / "s22plus_fyg8_p298_source_contract.py",
    "p298_candidate_intent": PREFIX / "s22plus_fyg8_p298_candidate_intent.py",
    "p298_userspace_build": PREFIX / "s22plus_fyg8_p298_userspace_build.py",
    "p298_build": PREFIX / "s22plus_fyg8_p298_build.py",
    "p298_candidate_builder": PREFIX / "build_s22plus_fyg8_p298_candidate.py",
    "p298_boot_only_packager": PREFIX / "s22plus_fyg8_p298_boot_only_packager.py",
}
TIER2_DIRECT_PATHS = {
    "p298_telemetry_model": PREFIX / "s22plus_fyg8_p298_telemetry_model.py",
    "p298_telemetry_decoder": PREFIX / "s22plus_fyg8_p298_telemetry_decoder.py",
    "p298_telemetry_closure": PREFIX / "s22plus_fyg8_p298_telemetry_closure.py",
    "p298_telemetry_test": PREFIX / "test_s22plus_fyg8_p298_telemetry.py",
    "p298_change_freeze": PREFIX / "s22plus_fyg8_p298_change_freeze.py",
    "p298_candidate_contract": PREFIX / "s22plus_fyg8_p298_candidate_contract.py",
    "p298_build_repro_check": PREFIX / "s22plus_fyg8_p298_build_repro_check.py",
    "p298_candidate_static_checker": PREFIX / "s22plus_fyg8_p298_candidate_static_checker.py",
    "p298_e2_stock_closure": PREFIX / "s22plus_fyg8_p298_e2_stock_closure.py",
    "p298_linked_audit": PREFIX / "s22plus_fyg8_p298_linked_audit.py",
    "p298_postbuild_linked_audit": PREFIX / "s22plus_fyg8_p298_postbuild_linked_audit.py",
    "p298_pre_lto_qualification": PREFIX / "s22plus_fyg8_p298_pre_lto_qualification.py",
    "p298_contract_test": Path("tests/test_s22plus_fyg8_p298_contract.py"),
}
...
INHERITED_PAYLOAD_SOURCE_KEYS = {
    f"p296_input__{key}": key for key in sorted(inherited_contract.SOURCE_KEYS)
}
...
def descriptor() -> dict[str, Any]:
    return {
        ...
        "profile": PROFILE,
        "tier1": {
            "purpose": "kernel-embedded P2.98 payload identity",
        ...
        "tier2": {
            ...
            "inherited": "P2.96 tier-2 receipt closure",
            "direct_paths": {
                key: _canonical_relative(path)
...
def tier1_materials(root: Path) -> dict[str, bytes]:
    ...
    if set(result) != TIER1_SOURCE_KEYS:
        raise IdentityTierError("P2.98 Tier-1 source key set differs")
    return result
...
def payload_identity(materials: Mapping[str, bytes]) -> str:
    return _sha256(
        _canonical(
            {
                "domain": "S22PLUS-FYG8-P298-PAYLOAD-IDENTITY-V1",
...
def qualification_identity(
    ...
    return _sha256(
        _canonical(
            {
                "domain": "S22PLUS-FYG8-P298-QUALIFICATION-IDENTITY-V1",
...
def validate() -> dict[str, Any]:
    ...
    if (
        ...
        raise IdentityTierError("P2.98 identity descriptor scope differs")
    return {
        "schema": SCHEMA,
        "descriptor_sha256": descriptor_sha256(),
