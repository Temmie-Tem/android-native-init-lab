#!/usr/bin/env python3
"""V2804 live handoff for native-init audio no-wait ADSP kick.

This is the audio core closure gate: flash the V2804 native-init image, stage the
...
V2804 flashes the no-wait ADSP-kick candidate, stages the known-good ACDB SET replay artifacts, executes the integrated native audio play command, and rolls back to V2321.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2804-audio-adsp-kick-no-wait/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2804_audio_adsp_kick_no_wait.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2804_AUDIO_ADSP_KICK_NO_WAIT_LIVE_2026-06-19.md"

CYCLE = "V2804"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.314"
CANDIDATE_TAG = "v2804-audio-adsp-kick-no-wait"
...
def classify_play_output(text: str) -> dict[str, Any]:
    return {
        ...
        or "audio.play.integrated.adsp_prebooted=1" in text,
        "worker_adsp_boot_skipped": "audio.play.integrated.adsp.boot_skipped=1 reason=foreground_prime_no_wait" in text,
...
def play_output_pass(summary: dict[str, Any]) -> bool:
    required = [
        ...
        "foreground_prime_ok",
        "foreground_prime_no_wait",
        "worker_adsp_prebooted",
        "worker_adsp_boot_skipped",
        "integrated_done",
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Foreground ADSP kick seen/ok/no-wait/failed: `{int(bool(play_summary.get('foreground_prime_seen')))} / {int(bool(play_summary.get('foreground_prime_ok')))} / {int(bool(play_summary.get('foreground_prime_no_wait')))} / {int(bool(play_summary.get('foreground_prime_failed')))}`",
        f"- Worker ADSP prebooted / second boot skipped: `{int(bool(play_summary.get('worker_adsp_prebooted')))} / {int(bool(play_summary.get('worker_adsp_boot_skipped')))}`",
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2804-audio-adsp-kick-no-wait-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2804-audio-adsp-kick-no-wait-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2804-audio-adsp-kick-no-wait-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio no-wait ADSP-kick command did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-adsp-kick-no-wait-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2804-audio-adsp-kick-no-wait-worker-failed-before-rollback"
            raise RuntimeError("native audio no-wait ADSP-kick command did not emit all required pass markers")
        ...
        result["decision"] = "v2804-audio-adsp-kick-no-wait-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2804-audio-adsp-kick-no-wait-live-blocked")
        if result["decision"] == "v2804-audio-adsp-kick-no-wait-live-started":
            result["decision"] = "v2804-audio-adsp-kick-no-wait-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2804-audio-adsp-kick-no-wait-live-dry-run",
