#!/usr/bin/env python3
"""Independently audit one deterministic P2.90 boot-only candidate."""
...
import build_s22plus_fyg8_p290_candidate as candidate
import s22plus_fyg8_p286_candidate_static_checker as base
import s22plus_fyg8_p290_build_repro_check as repro
import s22plus_fyg8_p290_candidate_contract as contract
import s22plus_fyg8_p290_e2_stock_closure as p290_closure
import s22plus_fyg8_p290_postbuild_linked_audit as postbuild_audit
import s22plus_fyg8_p290_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p290_candidate_static_checker_v1"
VERDICT = "PASS_P290_INDEPENDENT_ARTIFACT_CLOSURE_HOST_ONLY"
TARGET = contract.TARGET
DEFAULT_CANDIDATE = candidate.DEFAULT_OUT
DEFAULT_CANDIDATE_B = Path(
    "workspace/private/outputs/s22plus_fyg8_p290/candidate-b"
)
...
def verify_repro(root: Path, args, exact_contract):  # noqa: ANN001, ANN201
    result, payload = base.read_json(
        ...
        "P2.90 build reproducibility result",
        16 * 1024 * 1024,
    )
    ...
    if (
        ...
        raise CheckError(
            "P2.90 reproducibility result differs from fresh post-build "
            "verification"
        )
