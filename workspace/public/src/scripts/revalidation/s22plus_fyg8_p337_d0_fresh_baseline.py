#!/usr/bin/env python3
"""P3.37 exact-target D0 fresh-baseline consumer.

This wrapper reuses the finalized P3.36/P3.20 raw-first state machine while
owning a new P3.37 D1 dependency and no-replay namespace.  The P3.37 stock
...
P336_SOURCE = SCRIPT_DIR / "s22plus_fyg8_p336_d0_fresh_baseline.py"
P320_SOURCE = SCRIPT_DIR / "s22plus_fyg8_p320_d0_fresh_baseline.py"
ADAPTER = SCRIPT_DIR / "s22plus_fyg8_p337_stock_process_v2_adapter.py"
D1_SOURCE = SCRIPT_DIR / "s22plus_fyg8_p337_d1_fresh_baseline.py"
D1_BINDING = ROOT / (
    "workspace/public/src/device-action/bindings/"
    "s22plus_fyg8_p337_d1_fresh_baseline.json"
)
D1_RESULT = ROOT / (
    "workspace/private/runs/device-action-d1-p337-fresh-baseline/"
    "p337-d1-fresh-baseline-1/result.json"
)
...
BINDING_MANIFEST = ROOT / (
    "workspace/public/src/device-action/bindings/"
    "s22plus_fyg8_p337_d0_fresh_baseline.json"
)

RUN_PARENT = ROOT / "workspace/private/runs/device-action-d0-p337-fresh-baseline"
RUN_DIR = RUN_PARENT / "p337-d0-fresh-baseline-1"
RUN_ARM = RUN_PARENT / "p337-d0-fresh-baseline-1.arm.json"
...
ORDINAL = "p337-d0-fresh-baseline-1"
BINDING_SCHEMA = "s22plus_fyg8_p337_d0_fresh_baseline_execution_binding_v1"
BINDING_ID = "s22plus-fyg8-p337-d0-fresh-baseline-v1"
AUTHORITY_PREFIX = "DEVICE-ACTION-D0-P337-FRESH-BASELINE-1-APPROVE:"
REVIEW_VERDICT = "PASS_GO_P337_D0_FRESH_BASELINE_H0_CAPABILITY_V1"
D1_REVIEW_VERDICT = "PASS_GO_P337_D1_FRESH_BASELINE_H0_CAPABILITY_V1"
D1_BINDING_SCHEMA = "s22plus_fyg8_p337_d1_fresh_baseline_execution_binding_v1"
D1_BINDING_ID = "s22plus-fyg8-p337-d1-fresh-baseline-v1"
D1_RESULT_SCHEMA = "s22plus_fyg8_p337_d1_fresh_baseline_result_v1"
D1_RESULT_VERDICT = "PASS_P337_D1_FRESH_BASELINE_EXACT_NORMAL_REBOOT_RETURN_HEALTH"
RESULT_SCHEMA = "s22plus_fyg8_p337_d0_fresh_baseline_result_v1"
RESULT_VERDICT = "PASS_P337_D0_FRESH_BASELINE_RAW_FIRST_V1"
STOP_SCHEMA = "s22plus_fyg8_p337_d0_fresh_baseline_stop_v1"
STOP_VERDICT = "STOP_P337_D0_FRESH_BASELINE_CONSUMED_NO_REPLAY"
HOST_ADB_SIZE = 716_968
HOST_ADB_SHA256 = "05a1a4435e436230931acd8737fd68f31542d652731d3ca8c464cab7a42be226"
RAW_SIZE = 2_097_136
ADAPTER_PLACEHOLDER = {
    "status": "P337_ADAPTER_FINAL_REPIN_REQUIRED",
    "path": "workspace/public/src/scripts/revalidation/s22plus_fyg8_p337_stock_process_v2_adapter.py",
    "size": 0,
    "sha256": "0" * 64,
}
...
class D0Error(RuntimeError):
    """The P3.37 D0 action failed closed."""
...
def _load_p336() -> types.ModuleType:
    payload = _stable(P336_SOURCE, "P3.36 D0 source", maximum=2 * 1024 * 1024)
    module = types.ModuleType("s22plus_fyg8_p336_d0_bound_for_p337")
    module.__file__ = str(P336_SOURCE)
    module.__package__ = ""
    try:
        exec(compile(payload, str(P336_SOURCE), "exec", dont_inherit=True), module.__dict__)
    except BaseException as exc:
        raise D0Error(f"P3.36 D0 source failed to load: {type(exc).__name__}") from exc
    replacements = {
        "ROOT": ROOT,
        "SCRIPT": SCRIPT,
    ...
    def expected_binding(inputs: Mapping[str, Any], review: Mapping[str, Any]) -> dict[str, Any]:
        ...
        value["action"] = "one exact connected read-only P3.37 fresh-baseline acquisition"
    ...
    def validated_static_inputs() -> dict[str, Any]:
        ...
        payload = module._stable(BINDING_MANIFEST, "P3.37 D0 binding", maximum=256 * 1024)
        try:
            binding = json.loads(payload.decode("ascii"))
        except (UnicodeError, json.JSONDecodeError) as exc:
            raise D0Error("P3.37 D0 binding is not JSON") from exc
        review = binding.get("independent_review")
        if review not in (
            {"status": "review-pending", "verdict": None},
            {"status": "pass-go", "verdict": REVIEW_VERDICT},
        ):
            raise module.D0Error("P3.37 D0 review state differs")
        ...
        if binding != expected:
            ...
            if binding != module._expected_binding(pending_inputs, review):
                raise module.D0Error("P3.37 D0 binding does not match exact inputs")
            inputs = pending_inputs
        d1_payload = module._stable(D1_BINDING, "P3.37 D1 binding", maximum=256 * 1024)
        try:
            d1 = json.loads(d1_payload.decode("ascii"))
        except (UnicodeError, json.JSONDecodeError) as exc:
            raise D0Error("P3.37 D1 binding is not JSON") from exc
        if (
            d1.get("schema") != D1_BINDING_SCHEMA
            or d1.get("binding_id") != D1_BINDING_ID
            or d1.get("target") != TARGET
            or d1.get("ordinal") != "p337-d1-fresh-baseline-1"
        ):
            raise module.D0Error("P3.37 D1 predecessor identity differs")
        return {
            "manifest": binding,
...
def self_test() -> dict[str, Any]:
    ...
    if not _adapter_ready(static):
        return {
            "schema": "s22plus_fyg8_p337_d0_fresh_baseline_fixture_h0",
            "verdict": "PENDING_P337_D0_ADAPTER_FINAL_REPIN_REQUIRED",
            "ordinal": ORDINAL,
            "run_id": P337_RUN_ID,
            "raw_first": True,
            "clean_baseline": None,
    ...
    if value.get("clean_baseline") is not True or value.get("device_contact") is not False:
        raise D0Error("P3.37 D0 fixture result differs")
    return {
        "schema": "s22plus_fyg8_p337_d0_fresh_baseline_fixture_h0",
        "verdict": "PASS_P337_D0_FRESH_BASELINE_FIXTURE_H0",
        "ordinal": ORDINAL,
        "run_id": P337_RUN_ID,
        "raw_first": True,
        "clean_baseline": True,
...
def run_live(approval: str) -> dict[str, Any]:
    if not isinstance(approval, str) or not approval:
        raise D0Error("P3.37 D0 approval is absent")
    static = _validated_static_inputs()
    if not _adapter_ready(static):
        raise D0Error("P3.37 adapter final repin is required before D0 arm")
    bound = _load_p336()
    try:
    ...
    result.update({
        ...
        "version": "device-action-d0-p337-v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"P3.37 D0 blocked: {exc}", file=sys.stderr)
        return 2
