#!/usr/bin/env python3
"""P2.92 retained model with exact-slot and publication-error semantics."""
...
SCHEMA = "s22plus_fyg8_p292_exact_slot_position_model_v1"
...
def _validate_position_semantics(
    ...
    if profile != PROFILE:
        raise DesignError(f"unsupported P2.92 profile: {profile}")
    if repair.is_publication_error_detail(outcome, detail):
        try:
            position = base.spec.position_for_generation(generation)
        except base.spec.SpecError as exc:
            raise DesignError(str(exc)) from exc
        if (stage, item_index) != position.pair:
            raise DesignError(
                "P2.92 publication-error record is at the wrong position"
            )
        return
    try:
        base.spec.validate_slot(
...
def base_context() -> Iterator[None]:
    with base.base_context():
        previous_schema = base.base.SCHEMA
        previous_validator = base.base._validate_position_semantics  # noqa: SLF001
        base.base.SCHEMA = SCHEMA
        base.base._validate_position_semantics = _validate_position_semantics  # noqa: SLF001
        try:
            yield
        finally:
            base.base.SCHEMA = previous_schema
            base.base._validate_position_semantics = previous_validator  # noqa: SLF001
