#!/usr/bin/env python3
"""Build a P3.01 boot-only candidate with the exact qualified P3.00 Image."""
...
import s22plus_fyg8_p301_boot_only_packager as packager
import s22plus_fyg8_p301_candidate_contract as candidate_contract
import s22plus_fyg8_p301_overlay_contract as contract
import s22plus_fyg8_p301_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p301_candidate_artifact_result_v1"
VERDICT = "PASS_P301_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p301/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    if result_path != root / contract.PARENT_REPRO_RESULT:
        raise BuildError("P3.01 parent reproducibility path differs")
    value, result_receipt = _read_json(
        result_path, "P3.01 fixed P3.00 reproducibility result"
    )
    ...
    if (
        ...
        or value.get("build_a", {}).get("artifacts", {}).get("Image")
        != image_receipt
        ...
        raise BuildError("P3.01 fixed P3.00 reproducibility closure differs")
    return {
