#!/usr/bin/env python3
"""Guarded S22+ M10A2 getpid reboot native-init live gate.
...
M10A2 package and rollback APs without touching any device.

M10A2 is a lower-layer discriminator, not a module or USB proof. It performs a
non-VFS getpid() syscall, then reboot("download"). A later Odin endpoint is
still manually ambiguous: the helper can observe and rollback from download
...
LIVE_ACK_TOKEN = "S22PLUS-M10A2-GETPID-REBOOT-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M10A2-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M10A2_AP_SHA256 = "108c0a5e2a1fd80efed5ae93ea01b4b98c4990f7d3d8b292ef35ccc0de2fdb60"
EXPECTED_M10A2_BOOT_SHA256 = "f0238a82cad63a3d8017a0892a3a85bfe79c8c503848a4ac0fa4a21a77a72c94"
EXPECTED_M10A2_BASE_BOOT_SHA256 = "2e541703951dc725bad35850faf7028c2d910dd5f21166449b63f1248c29967e"
EXPECTED_M10A2_KERNEL_SHA256 = "bceca73edbfca3499148e16741c939779157925949ef6bc8a8e31d6b68fc2cff"
EXPECTED_M10A2_INIT_SHA256 = "0839562fbef74328abb17646d957516154ae85ab954667782c809249cf8bde99"
EXPECTED_M10A2_SOURCE_SHA256 = "5b15166dfc405a7ee1297ac1cd0da3bd844779099748cf98ee3aca8e2e665d9a"
EXPECTED_M10A2_RETAINED_MARKER = "S22_NATIVE_INIT_M10A2_NO_RETAINED_MARKER_EXPECTED"

DEFAULT_M10A2_AP = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a2_getpid_reboot_v0_1/odin4/AP.tar.md5")
DEFAULT_M10A2_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a2_getpid_reboot_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    ...
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m10a2_getpid_reboot_live_gate_{stamp}")
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    required = [
        "S22+ M10A2 getpid reboot native-init boot-only",
        EXPECTED_M10A2_AP_SHA256,
        EXPECTED_M10A2_BOOT_SHA256,
        EXPECTED_M10A2_BASE_BOOT_SHA256,
        EXPECTED_M10A2_KERNEL_SHA256,
        EXPECTED_M10A2_INIT_SHA256,
        EXPECTED_M10A2_SOURCE_SHA256,
        LIVE_ACK_TOKEN,
        ROLLBACK_ACK_TOKEN,
        "freestanding C raw-syscall runtime",
        "`getpid()` non-VFS syscall",
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M10A2 live authorization markers: {missing}")


def verify_m10a2_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M10A2 manifest missing: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    hashes = data.get("hashes", {})
    safety = data.get("safety", {})
    ramdisk = data.get("ramdisk", {})
    init_info = data.get("m10a2_init", {})
    tar_members_seen = data.get("tar_members")
    append_log(log_path, f"m10a2_manifest_path={path}")
    append_log(log_path, f"m10a2_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    append_log(log_path, f"m10a2_manifest_safety={json.dumps(safety, sort_keys=True)}")
    append_log(log_path, f"m10a2_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")

    expected_hashes = {
        "ap_tar_md5": EXPECTED_M10A2_AP_SHA256,
        "boot_img": EXPECTED_M10A2_BOOT_SHA256,
        "base_boot": EXPECTED_M10A2_BASE_BOOT_SHA256,
        "kernel": EXPECTED_M10A2_KERNEL_SHA256,
        "m10a2_init": EXPECTED_M10A2_INIT_SHA256,
        "source": EXPECTED_M10A2_SOURCE_SHA256,
        "nochange_repack_boot": EXPECTED_M10A2_BASE_BOOT_SHA256,
    }
    for key, expected in expected_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M10A2 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M10A2 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    expected_safety: dict[str, Any] = {
        ...
        "first_externally_observable_action": "getpid-then-reboot-download",
        "intended_syscall_count": 2,
        "auto_reboot": "download-after-getpid",
        "marker_write": False,
        "kmsg_write": False,
        "vfs_setup": "none",
        "vfs_mutation": False,
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise SystemExit(f"M10A2 manifest safety {key} mismatch: {safety.get(key)!r} != {expected!r}")
    if safety.get("intended_syscalls") != ["getpid", "reboot"]:
        raise SystemExit(f"M10A2 intended syscalls mismatch: {safety.get('intended_syscalls')!r}")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M10A2 manifest did not replace /init")
    if ramdisk.get("only_intended_entry_change") != "init":
        raise SystemExit("M10A2 manifest intended ramdisk change is not only /init")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A2 must not inject vendor module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A2 must not inject module-list files into boot ramdisk")

    required_strings = set(init_info.get("required_strings", []))
    if "download" not in required_strings:
        raise SystemExit("M10A2 required download string missing from manifest")
    if init_info.get("svc_count") != 2:
        raise SystemExit(f"M10A2 expected two svc instructions, got {init_info.get('svc_count')!r}")
    ...
    for forbidden in [
        ...
        if forbidden not in forbidden_strings:
            raise SystemExit(f"M10A2 forbidden-string gate missing from manifest: {forbidden}")
...
def observe_until_odin(run_dir: Path, log_path: Path, seconds: int, odin: Path) -> str | None:
    ...
    while time.monotonic() < deadline:
        iteration += 1
        label = f"m10a2_download_observation_{iteration:03d}"
        host_snapshot(run_dir, log_path, label, odin)
        devices = odin_devices(odin, log_path, f"{label}-extra")
        if len(devices) == 1:
            append_log(log_path, "m10a2_download_endpoint_seen=1")
            append_log(log_path, "m10a2_manual_download_ambiguity=operator-confirmation-required")
            return devices[0]
        if len(devices) > 1:
            raise SystemExit(f"refusing ambiguous Odin devices during M10A2 observation: {devices}")
        rows = adb_rows(log_path, f"{label}-extra")
        if rows:
            append_log(log_path, f"m10a2_candidate_adb_rows={rows}")
        time.sleep(1.0)
    append_log(log_path, "m10a2_download_endpoint_seen=0")
    return None
...
def rollback_from_download(
    ...
    if serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", serial, marker=EXPECTED_M10A2_RETAINED_MARKER)
    return 0


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--m10a2-ap", type=Path, default=DEFAULT_M10A2_AP)
    parser.add_argument("--m10a2-manifest", type=Path, default=DEFAULT_M10A2_MANIFEST)
    ...
    log_path = run_dir / "s22plus_m10a2_getpid_reboot_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus m10a2 getpid reboot live gate ===")
    ...
    m10a2_ap = resolve(root, args.m10a2_ap)
    m10a2_manifest = resolve(root, args.m10a2_manifest)
    ...
    verify_ap(m10a2_ap, EXPECTED_M10A2_AP_SHA256, "m10a2_candidate", log_path)
    verify_m10a2_manifest(m10a2_manifest, log_path)
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M10A2 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M10A2 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M10A2 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    if odin_device is None:
        print("download mode did not appear for M10A2 candidate flash", file=sys.stderr)
        return 2

    candidate_rc = flash_ap(odin, m10a2_ap, odin_device, log_path, "candidate")
    if candidate_rc != 0:
        print(f"M10A2 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        print(
            "M10A2 candidate flash completed but the original Odin device did not disconnect; "
        ...
        if post_rollback_serial is None:
            return 6
        append_log(log_path, "m10a2_result=no-proof-original-download-never-disconnected")
        collect_android_pstore(
            run_dir,
            log_path,
            "post_rollback_no_disconnect",
            post_rollback_serial,
            marker=EXPECTED_M10A2_RETAINED_MARKER,
        )
        return 7

    print("M10A2 candidate flashed. Waiting for later download endpoint; operator manual entry would make it ambiguous.")
    rollback_device = observe_until_odin(run_dir, log_path, args.download_observation_wait_sec, odin)
    if rollback_device is None:
        print("M10A2 download endpoint did not appear; enter download mode manually and run --rollback-from-download.", file=sys.stderr)
        return 4
    ...
    if post_rollback_serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", post_rollback_serial, marker=EXPECTED_M10A2_RETAINED_MARKER)
    print(
        "M10A2 live gate observed a later download endpoint and rollback ok; "
