#!/usr/bin/env python3
"""Build one deterministic boot-only P2.88 FYG8 candidate host-only."""
...
import s22plus_fyg8_p288_boot_only_packager as packager
import s22plus_fyg8_p288_build_repro_check as repro
import s22plus_fyg8_p288_candidate_contract as candidate_contract
import s22plus_fyg8_p288_contract_spec as p288_spec
import s22plus_fyg8_p288_e2_stock_closure as p288_closure
import s22plus_fyg8_p288_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p288_candidate_artifact_result_v1"
VERDICT = "PASS_P288_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P288_SOURCE_CONTRACT_ID = repro.P288_SOURCE_CONTRACT_ID
...
DEFAULT_OUT = Path(
    "workspace/private/outputs/s22plus_fyg8_p288/candidate-a"
)
...
def verify_repro_result(
    ...
    *,
    ...
    value, result_receipt = base._read_json(  # noqa: SLF001
        result_path, "P2.88 build reproducibility result"
    )
    if (
        ...
        raise BuildError(
            "P2.88 build reproducibility result is not accepted"
        )
    ...
    if (
        ...
        raise BuildError("P2.88 linked audit adapter mismatch")
    expected_image = (
        value.get("build_a", {}).get("artifacts", {}).get("Image")
    )
    if expected_image != image_receipt:
        raise BuildError(
            "P2.88 supplied Image differs from reproducibility closure"
        )
    if intent_path is None or patch_path is None:
        raise BuildError("P2.88 selected build-input paths are missing")
    try:
        pre_lto_qualification = repro.verify_p288_qualification_file(
