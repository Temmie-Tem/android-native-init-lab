#!/usr/bin/env python3
"""V2797 live handoff for native-init audio dmabuf msync-nonfatal.

This is the audio core closure gate: flash the V2797 native-init image, stage the
...
V2797 specifically verifies that the V2796 dmabuf `msync(EINVAL)` blocker is
non-fatal and that execution can progress to the next audio SET/route/PCM stage.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2797-audio-dmabuf-msync-nonfatal/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2797_audio_dmabuf_msync_nonfatal.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2797_AUDIO_DMABUF_MSYNC_NONFATAL_LIVE_2026-06-19.md"

CYCLE = "V2797"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.310"
CANDIDATE_TAG = "v2797-audio-dmabuf-msync-nonfatal"
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- DMABUF msync nonfatal / SET entries prepared / msm_audio_cal missing: `{int(bool(play_summary.get('dmabuf_msync_nonfatal')))} / {int(bool(play_summary.get('setcal_prepared_all')))} / {int(bool(play_summary.get('msm_audio_cal_missing')))}`",
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2797-audio-dmabuf-msync-nonfatal-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2797-audio-dmabuf-msync-nonfatal-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2797-audio-dmabuf-msync-nonfatal-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio dmabuf msync-nonfatal command did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-dmabuf-msync-nonfatal-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2797-audio-dmabuf-msync-nonfatal-live-worker-failed-before-rollback"
            raise RuntimeError("native audio dmabuf msync-nonfatal command did not emit all required pass markers")
        ...
        result["decision"] = "v2797-audio-dmabuf-msync-nonfatal-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2797-audio-dmabuf-msync-nonfatal-live-blocked")
        if result["decision"] == "v2797-audio-dmabuf-msync-nonfatal-live-started":
            result["decision"] = "v2797-audio-dmabuf-msync-nonfatal-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2797-audio-dmabuf-msync-nonfatal-live-dry-run",
