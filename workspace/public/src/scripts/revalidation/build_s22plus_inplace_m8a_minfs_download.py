#!/usr/bin/env python3
"""Build the S22+ M8A minimal-fs timed-download native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M8A is the first post-M8 discriminator.  It preserves the M8 freestanding
runtime shape but removes every module, configfs, and USB-gadget variable:
replace Magisk ramdisk `/init`, mount only dev/proc/sys/run, emit a kmsg marker,
then request Samsung download mode.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m8a_minfs_download_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m8a_minfs_download.c")
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M8A minimal-fs timed-download init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M8A minimal-fs timed-download init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M8A minimal-fs timed-download init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M8A minimal-fs timed-download init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M8A minimal-fs timed-download init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M8A init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M8A init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M8A init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M8A /init: {required}")
    for forbidden in forbidden_strings:
        if forbidden in binary:
            raise SystemExit(f"M8A /init contains forbidden string: {forbidden!r}")

    (build_dir / "m8a_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m8a_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m8a_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m8a_init = build_dir / "s22plus_init_m8a_minfs_download"
    m8a_init_info = compile_init(source, m8a_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m8a_init}"], work_dir, "replace /init with M8A init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M8A patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m8a_init):
        raise SystemExit("replaced /init does not match compiled M8A init")
    ...
    sizes = {
        ...
        "m8a_init": m8a_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M8A in-place native-init minimal-fs timed-download discriminator with no module or gadget path",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns": "direct PID1 survived minimal dev/proc/sys/run setup and reached reboot download",
            "no_download_mode": "failure is before or during minimal freestanding PID1/fs/reboot path, not module/configfs/USB gadget",
            "retained_marker_found": "candidate reached M8A marker before reboot request",
        },
    }
