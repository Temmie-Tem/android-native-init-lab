"""Fixed P363 namespace over sealed P353 H0 machinery; no device authority."""
...
def load(namespace):
    name = Path(namespace['__file__']).name.replace('p363', 'p353')
    relative, expected = PREDECESSORS[name]
    path = ROOT / relative
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != expected:
        raise ValueError('P363 sealed predecessor changed: ' + name)
    raw = raw.replace(b'P353', b'P363').replace(b'p353', b'p363')
    raw = raw.replace(b'c353f1e0a90b5e6d7c8a9b0c1d2e3f0b', b'c363f1e0a90b5e6d7c8a9b0c1d2e3f0b')
    ...
    try:
        namespace['__name__'] = entry if entry != '__main__' else 'p363_h0_template'
        exec(compile(ast.fix_missing_locations(tree), str(path) + '#p363', 'exec'), namespace)
    finally:
