#!/usr/bin/env python3
"""Materialize one P3.04 stock USB-notifier bridge intent."""
...
def create(root: Path, output: Path) -> dict:
    if output.exists() or output.is_symlink():
        raise IntentError(f"P3.04 overlay intent output exists: {output}")
    ...
    try:
        if os.write(descriptor, payload) != len(payload):
            raise IntentError("short P3.04 overlay intent write")
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
