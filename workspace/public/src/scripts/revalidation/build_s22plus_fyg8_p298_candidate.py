#!/usr/bin/env python3
"""Build one deterministic boot-only P2.98 FYG8 candidate host-only."""
...
import s22plus_fyg8_p298_boot_only_packager as packager
...
import s22plus_fyg8_p298_telemetry_spec as p298_spec
import s22plus_fyg8_p298_userspace_build as userspace


SCHEMA = "s22plus_fyg8_p298_candidate_artifact_result_v1"
VERDICT = "PASS_P298_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
TARGET = candidate_contract.TARGET
P298_SOURCE_CONTRACT_ID = repro.P298_SOURCE_CONTRACT_ID
P286_SOURCE_CONTRACT_ID = P298_SOURCE_CONTRACT_ID
DEFAULT_IMAGE = repro.DEFAULT_BUILD_A / "Image"
DEFAULT_REPRO_RESULT = Path(
    "workspace/private/outputs/s22plus_fyg8_p298/build-repro-result.json"
)
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p298/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    if (
        ...
        raise BuildError("P2.98 build reproducibility result is not accepted")
    linked = value.get("linked_audit", {})
    if (
        linked.get("audit_adapter") != "s22plus-fyg8-p298-linked-audit-v1"
        or linked.get("postbuild_audit", {}).get("verified") is not True
    ):
        raise BuildError("P2.98 linked audit adapter mismatch")
    expected_image = value.get("build_a", {}).get("artifacts", {}).get("Image")
    if expected_image != image_receipt:
        raise BuildError("P2.98 supplied Image differs from reproducibility closure")
    if intent_path is None or patch_path is None:
        raise BuildError("P2.98 selected build-input paths are missing")
    try:
        qualification = repro.verify_p298_qualification_file(
