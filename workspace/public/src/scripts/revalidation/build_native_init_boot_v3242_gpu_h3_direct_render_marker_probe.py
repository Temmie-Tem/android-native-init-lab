#!/usr/bin/env python3
"""Build V3242 GPU H3 direct-render marker probe."""
...
CYCLE = "V3242"
INIT_VERSION = "0.11.48"
INIT_BUILD = "v3242-gpu-h3-direct-render-marker-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3242_GPU_H3_DIRECT_RENDER_MARKER_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3242_gpu_h3_direct_render_marker_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3242_gpu_h3_direct_render_marker_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v578_gpu_h3_direct_render_marker_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3242"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3242.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3242.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3242"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3242-gpu-h3-direct-render-marker-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3242-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3242-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3242-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3242-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3242-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3242-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3242-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-direct-render-marker-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-direct-render-marker-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3242")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3242")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3242")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3242")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3242")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3242")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3242")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3242")
SFX_STREAM_MARKER = "a90.doomgeneric.v3242.audio=real-sfx-pcm-stream-gpu-h3-direct-render-marker-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-direct-render-marker-probe-v3242"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3242.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3242_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3232": b"a90-doomgeneric-v3242",
        b"a90.doomgeneric.v3232": b"a90.doomgeneric.v3242",
        b"v3232": b"v3242",
        b"V3232": b"V3242",
...
REQUIRED_STRINGS = tuple(_rewrite_v3242_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        ...
        "scope": "first-triangle-h3-direct-render-marker-r1-footprint2-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3242-through-native-init-flash",
            "post-flash-health-check",
            "verify-fresh-boot-fwclass-path-before-h3",
            "gpu-h3-direct-render-marker-r1-footprint2-mov-f32-shader-timeout-guard",
...
def v3242_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-direct-render-marker-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-direct-render-marker-probe")
        .replace("v3208", "v3242")
        .replace("V3208", "V3242")
    )


def _overlay_preserved_v3242_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3232 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3242 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3242 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3242 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3242-overlay-") as temp_name:
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3242 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3242 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3232-ramdisk-overlay-v3242-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-direct-render-marker-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-direct-render-marker-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-direct-render-marker-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3232-ramdisk-overlay-v3242-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-direct-render-marker-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-direct-render-marker-live-validation"
...
def _apply_v3242_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-direct-render-marker-probe")
        ...
        "v3210_adapter_source": v3242_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3242_ramdisk,
