#!/usr/bin/env python3
"""V2793 live handoff for native-init audio play worker.

This is the audio core closure gate: flash the V2793 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2793-audio-play-worker/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2793_audio_play_worker.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2793_AUDIO_PLAY_WORKER_LIVE_2026-06-19.md"

CYCLE = "V2793"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.306"
CANDIDATE_TAG = "v2793-audio-play-worker"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2793-audio-play-worker-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2793-audio-play-worker-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2793-audio-play-worker-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio play worker did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-play-worker-log",
            ["run", "/bin/cat", REMOTE_PLAY_LOG],
            timeout=45.0,
            retry_unsafe=True,
            allow_error=True,
        )
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2793-audio-play-worker-live-worker-failed-before-rollback"
            raise RuntimeError("native audio play worker did not emit all required pass markers")
        ...
        result["decision"] = "v2793-audio-play-worker-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2793-audio-play-worker-live-blocked")
        if result["decision"] == "v2793-audio-play-worker-live-started":
            result["decision"] = "v2793-audio-play-worker-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2793-audio-play-worker-live-dry-run",
        ...
        "commands": {
            ...
            "play_worker_log": ["run", "/bin/cat", REMOTE_PLAY_LOG],
