#!/usr/bin/env python3
"""Verify a P3.01 overlay against the exact P3.00 parent candidate."""
...
def verify(root: Path, source: Path, intent_path: Path, patch_path: Path):
    if source != root / overlay.PARENT_SOURCE:
        raise ContractError("P3.01 parent source path differs")
    if patch_path != root / overlay.PARENT_PATCH:
        raise ContractError("P3.01 parent patch path differs")
