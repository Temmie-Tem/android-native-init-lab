#!/usr/bin/env python3
"""Build V3287 GPU H3 cffdump-shaped VFD/VS contract probe."""
...
CYCLE = "V3287"
INIT_VERSION = "0.11.69"
INIT_BUILD = "v3287-gpu-h3-vfd-vs-contract-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3287_GPU_H3_VFD_VS_CONTRACT_SOURCE_BUILD_2026-06-26.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3287_gpu_h3_vfd_vs_contract_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3287_gpu_h3_vfd_vs_contract_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v600_gpu_h3_vfd_vs_contract_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3287"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3287.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3287.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3287"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3287-gpu-h3-vfd-vs-contract-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3287-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3287-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3287-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3287-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3287-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3287-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3287-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-vfd-vs-contract-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-vfd-vs-contract-probe"

INPUT_THREAD_MARKER = previous.INPUT_THREAD_MARKER.replace("v3284", "v3287")
TIME_MODEL_MARKER = previous.TIME_MODEL_MARKER.replace("v3284", "v3287")
DEMO_HUD_MARKER = previous.DEMO_HUD_MARKER.replace("v3284", "v3287")
PACED_TIME_MARKER = previous.PACED_TIME_MARKER.replace("v3284", "v3287")
TICK_TELEMETRY_MARKER = previous.TICK_TELEMETRY_MARKER.replace("v3284", "v3287")
SCALE_MARKER = previous.SCALE_MARKER.replace("v3284", "v3287")
PHASE_TELEMETRY_MARKER = previous.PHASE_TELEMETRY_MARKER.replace("v3284", "v3287")
GAMETIC_FRAME_TELEMETRY_MARKER = previous.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3284", "v3287")
SFX_STREAM_MARKER = "a90.doomgeneric.v3287.audio=real-sfx-pcm-stream-gpu-h3-vfd-vs-contract-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-vfd-vs-contract-probe-v3287"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3287.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = (
    "first-triangle-h3-vfd-vs-contract-replay-a640-nonzero-init-magic-block-"
    "flag-mrt-cffdump-color-target-varying-ij-vpc-linkage-clip-guardband-su-"
    "rasterizer-a6xx-output-routing-sp-frontend-prog-id-state-sp-const-fs-"
    "output-cntl-raster-mode-cp-set-mode-window-offset-visibility-packets-"
    "vpc-so-override-off-sysmem-bin-control-sp-update-cntl-compiler-vs-"
    "instrlen-cache-invalidate-rb-render-cntl"
)
PREVIOUS_SCOPE = previous.SCOPE
VFD_VS_SOURCE = "V3286 cffdump diff A640 draw2 VFD/VS input-contract candidate"
...
def _rewrite_v3287_text(text: str) -> str:
    replacements = (
        ...
        ("a90-doomgeneric-v3284", "a90-doomgeneric-v3287"),
        ("a90.doomgeneric.v3284", "a90.doomgeneric.v3287"),
        ("v3284", "v3287"),
        ("V3284", "V3287"),
        ("gpu-h3-a640-magic-block-probe", "gpu-h3-vfd-vs-contract-probe"),
        ...
        ("mesa-ir3-disasm-verified-h3-mov-r2-plus-a640-cffdump-bary-f-frag-shader",
         "constant-free-cffdump-shaped-vfd-vs-contract-plus-a640-cffdump-bary-f-frag-shader"),
...
def _rewrite_v3287_bytes(item: bytes) -> bytes:
    return _rewrite_v3287_text(item.decode("utf-8")).encode("utf-8")


GPU_H3_VFD_VS_CONTRACT_MARKERS = (
    b"gpu.h3.draw.scope=" + SCOPE.encode("ascii"),
    b"gpu.h3.draw.shader_payload=" + SHADER_PAYLOAD.encode("ascii"),
    b"gpu.h3.draw.shader_payload_source=constant-free-cffdump-shaped-vfd-vs-contract-plus-a640-cffdump-bary-f-frag-shader",
...
REQUIRED_STRINGS = tuple(_rewrite_v3287_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        ...
        "sp_vs_const_config_reference_deferred": "0x101 requires VS constant-buffer replay; V3287 remains constant-free",
        "readback": "expect changed color or flag buffer words if the previous single-fetch VFD/VS input contract blocked raster output",
        "next_live_validation": [
            "flash-v3287-through-native-init-flash",
            "post-flash-health-check",
            "gpu-h3-vfd-vs-contract-timeout-guard",
...
def render_report(
    ...
    return "\n".join([
        ...
        f"- Expected PM4 size is `{PM4_DWORDS_EXPECTED}` dwords; state register writes stay `{STATE_REG_WRITES_EXPECTED}`; VFD draw-local writes become `{VFD_REG_WRITES_EXPECTED}`.",
        "",
        "## Source Basis",
        "",
...
def v3287_adapter_source() -> str:
    return _rewrite_v3287_text(previous.v3284_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-vfd-vs-contract-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-vfd-vs-contract-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-vfd-vs-contract-live-validation",
...
def _overlay_preserved_v3287_ramdisk() -> dict[str, Any]:
    ...
    overlay["mode"] = "preserve-v3284-ramdisk-overlay-v3287-init-helper-engine"
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-h3-vfd-vs-contract-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-vfd-vs-contract-live-validation"
...
def _apply_v3287_overrides() -> None:
    ...
    replacements = {
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3287_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3287_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3287_ramdisk,
