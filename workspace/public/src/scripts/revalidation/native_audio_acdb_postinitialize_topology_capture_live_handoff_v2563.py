#!/usr/bin/env python3
"""V2563 live handoff for post-initialize target-only ACDB topology capture.

This wrapper reuses the V2490 checked Android boot/stage/pull/rollback engine
with V2563 artifacts:

- helper initializes ACDB, then calls acdb_loader_send_common_custom_topology()
  as a fallback path;
- combined preload keeps acdb_ioctl silent until ACDB_CMD_INITIALIZE_V2 returns
  success, then captures only custom-topology size/data commands, fakes audio
  calibration ioctls when A90_ACDB_FAKE_ALLOCATE=1, and exits after the first
  ret==0 non-zero 4916-byte topology buffer.
"""
...
import build_android_acdb_postinitialize_topology_capture_v2563 as v2563
import native_audio_acdb_ownprocess_get_live_handoff_v2490 as v2490

RUN_ID = "V2563"
BUILD_TAG = "v2563-audio-acdb-postinitialize-topology-capture-live"
ROOT = v2563.ROOT
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2563_AUDIO_ACDB_POSTINITIALIZE_TOPOLOGY_CAPTURE_LIVE_HANDOFF_2026-06-16.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2563-acdb-postinitialize-topology-capture-{stamp}"
...
def read_v2563_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    target_only_after_initialize = (
        "-DA90_ACDBTAP_AUTO_ARM_ON_INITIALIZE=1" in cflags
        and "-DA90_ACDBTAP_CUSTOM_TOPOLOGY_ONLY=1" in cflags
    )
    return {
        "ok": bool(payload.get("ok") and helper.get("ok") and preload.get("ok") and target_only_after_initialize),
        "path": rel(path),
        "manifest": payload,
        "helper": helper,
        "preload": preload,
        "target_only_after_initialize": target_only_after_initialize,
...
def build_v2563_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        build_root=args.v2563_build_root,
        manifest_path=args.v2563_manifest_path,
        clang=v2563.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2563.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    return v2563.manifest(build_args)


def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2563_artifacts:
        build_v2563_artifacts(args)
    manifest = read_v2563_manifest(args.v2563_manifest_path)
...
def summarize_postinit_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    if pass_through_set_events:
        classification = "v2563-boundary-violation-real-audio-set-passthrough"
    elif topology:
        ...
        classification = "v2563-init-internal-topology-before-target-only-sigsegv"
    elif not init_ok:
        classification = "v2563-init-v3-failed-before-arm"
    elif not rows:
        classification = "v2563-target-only-no-acdbtap-events"
    elif size_queries:
        classification = "v2563-postinitialize-topology-size-query-only"
    elif successful:
        classification = "v2563-postinitialize-topology-partial-nonzero-capture"
    elif not helper_fallback_armed:
        classification = "v2563-init-internal-target-only-no-topology-capture"
    else:
        classification = f"v2563-{base_summary.get('classification', 'no-acdbtap-capture')}"
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2563-acdb-postinitialize-topology-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2563_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "target_only_after_initialize": bool(artifacts.get("manifest", {}).get("target_only_after_initialize")),
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2563 post-initialize target-only helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2563 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2563_artifacts"]
    ...
    capture_summary = summarize_postinit_capture(pulled_dir) if pulled_dir else {
        "classification": "v2563-no-pulled-artifacts",
        "success": False,
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2563-result.json", wrapper)
    return wrapper
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines.extend([
        ...
        "## Artifacts",
        "",
        f"- helper_sha256: `{payload.get('v2563_artifacts', {}).get('helper', {}).get('sha256')}`",
        f"- preload_sha256: `{payload.get('v2563_artifacts', {}).get('preload', {}).get('sha256')}`",
        "",
        "## Boundary",
        "",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2563-artifacts", action="store_true")
    parser.add_argument("--v2563-build-root", type=Path, default=v2563.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2563-manifest-path", type=Path, default=v2563.DEFAULT_MANIFEST)
