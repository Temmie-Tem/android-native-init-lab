#!/usr/bin/env python3
"""Build V2664 common-only custom-topology ACDB SET capture artifacts.

Host-only build unit. V2663 proved the exported
acdb_loader_send_common_custom_topology() already contains the missing 10/14/24
custom-topology SET paths. V2664 therefore removes the unrelated per-device
send_audio_cal_v5 call from the own-process helper, while reusing the V2659
phase-common hook and the V2630 fake-success SET capture shim.
"""
...
ROOT = v2659.ROOT
RUN_ID = "V2664"
BUILD_TAG = "v2664-acdb-common-only-setcal-capture-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2664_AUDIO_ACDB_COMMON_ONLY_SETCAL_CAPTURE_BUILD_2026-06-18.md"

HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_common_only_exec_linked_v2664.c"
PREINIT_SOURCE_REL = v2659.PREINIT_SOURCE_REL
HELPER_ARTIFACT_NAME = "a90_acdb_common_only_setcal_capture_exec_linked_v2664"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_common_only_setcal_capture_combined_preload_v2664.so"
...
def source_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    required = {
        ...
        "helper_calls_common_after_arm": common_pos > arm_pos,
        ...
        "phase_hook_init_short_success": "init_common_return_success" in preinit,
        "phase_hook_calls_real_common_postinit": "postinit_before_real_common" in preinit and "a90_real_common_topology()" in preinit,
    ...
    if isinstance(preload, dict):
        ...
        if isinstance(checks, dict):
            if "soname_v2659" in checks:
                checks["soname_v2664"] = checks.pop("soname_v2659")
...
def make_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            "call_order": "acdb_loader_init_v3 -> init-short common hook -> a90_arm_capture -> post-init real common topology only",
            "target_cal_types": TARGET_CAL_TYPES,
            "removed_from_v2659": "send_audio_cal_v5 is intentionally not imported or called",
            "success_discriminator": "future live run must capture byte-exact SET records for cal_types 10/14/24; helper exit is not tied to common return code",
