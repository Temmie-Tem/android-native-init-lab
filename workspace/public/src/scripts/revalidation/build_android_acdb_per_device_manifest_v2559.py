#!/usr/bin/env python3
"""Build V2559 ARM32 ACDB per-device manifest capture helper.

Host-only unit.  It builds a helper that initializes ACDB, arms the acdb_ioctl
preload, deliberately skips the already-pinned common topology payload, and calls
acdb_loader_send_audio_cal_v5(15, 0, 0x11135, 48000, 48000, 0, 1) so the preload
can capture per-device GET out-buffers.
...
ROOT = v2553.ROOT
RUN_ID = "V2559"
BUILD_TAG = "v2559-acdb-per-device-manifest-capture-host-only"
...
HELPER_ARTIFACT_NAME = "a90_acdb_per_device_manifest_exec_linked_v2559"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_per_device_manifest_preload_v2559.so"
...
def manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            ...
            "common_topology_call": "skipped: payload already pinned by V2548/V2557 SHA-256",
            "per_device_call": "acdb_loader_send_audio_cal_v5(15, 0, 0x11135, 48000, 48000, 0, 1)",
            "preload_policy": "silent post-INITIALIZE_V2 auto-arm plus helper manual-arm, no exit on 4916, fake audio calibration ioctls when A90_ACDB_FAKE_ALLOCATE=1",
