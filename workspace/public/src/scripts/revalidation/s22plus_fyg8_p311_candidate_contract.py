#!/usr/bin/env python3
"""Verify P3.11 against its exact fixed P3.10 parent."""
...
def verify(root: Path, source: Path, intent_path: Path, patch_path: Path):
    ...
    if source != root / overlay.PARENT_SOURCE:
        raise ContractError("P3.11 parent source path differs")
    if patch_path != root / overlay.PARENT_PATCH:
        raise ContractError("P3.11 parent patch path differs")
