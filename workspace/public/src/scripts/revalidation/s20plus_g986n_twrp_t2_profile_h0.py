#!/usr/bin/env python3
"""Exact host closure and recovery observer profile for S20+ TWRP T2.

This module has no connected entrypoint.  It pins the IYC2-stock-substrate,
AstroForge-ramdisk T2 candidate and exact-stock rollback, validates their
...
VERSION = "s20plus-g986n-twrp-t2-profile-h0-v1"
HOST_RESULT_SCHEMA = "s20plus_g986n_twrp_t2_host_validation_v1"
HOST_PASS_VERDICT = "PASS_S20PLUS_G986N_TWRP_T2_HOST_CLOSURE"
...
OUTPUT_ROOT = ROOT / "workspace/private/outputs/s20plus_g986n/twrp_port_t2_v1"
CANDIDATE_AP = OUTPUT_ROOT / "candidate/AP.tar.md5"
CANDIDATE_AP_SIZE = 52_101_161
CANDIDATE_AP_SHA256 = (
    "6d10b3154f2e899ee64305f3f3279d7d5243eb917f1fedb413b8f88d22ec88cb"
)
CANDIDATE_MEMBER_SIZE = 52_096_466
...
CANDIDATE_TAR_MD5 = "133d8d11763e7271a1aed2f395c25eae"
...
BUILDER = HERE / "build_s20plus_g986n_twrp_port_t2_h0.py"
BUILDER_SIZE = 25_475
BUILDER_SHA256 = (
    "505f4fabc5e9684464b3c266453734db9e5f089507576df5dac00785683e635d"
)

MARKER_PATH = "/init.s20plus_g986n_twrp_port_t2"
MARKER_SIZE = 398
MARKER_SHA256 = "19a1a617fd2e97feda6362a2435e3d2bcce600fb37f983dd131e21c5ecf9c246"
...
class TWRPT2ProfileError(RuntimeError):
    """The exact T2 host or observer closure changed."""
...
def _strict_json(payload: bytes, label: str) -> dict[str, Any]:
    def unique(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise TWRPT2ProfileError(f"{label} has a duplicate key")
            result[key] = value
        return result
    ...
    try:
        value = json.loads(
            ...
            parse_constant=lambda token: (_ for _ in ()).throw(
                TWRPT2ProfileError(f"{label} has a non-finite number")
            ),
        )
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise TWRPT2ProfileError(f"{label} is malformed") from exc
    if type(value) is not dict:
        raise TWRPT2ProfileError(f"{label} root differs")
    return value
...
def _read_exact(descriptor: int, size: int) -> bytes:
    ...
    while offset < size:
        ...
        if not block:
            raise TWRPT2ProfileError("short manifest read")
        payload.extend(block)
        offset += len(block)
    if os.pread(descriptor, 1, size):
        raise TWRPT2ProfileError("manifest grew while read")
...
def validate_manifest() -> dict[str, Any]:
    with pin_regular_file(
        ...
        value = _strict_json(_read_exact(descriptor, MANIFEST_SIZE), "T2 manifest")
    ...
    if (
        value.get("schema") != "s20plus_g986n_twrp_port_t2_build_v1"
        or value.get("verdict")
        != "PASS_S20PLUS_G986N_TWRP_PORT_T2_HOST_BUILT_REVIEW_PENDING"
        ...
        raise TWRPT2ProfileError("T2 manifest semantics changed")
...
def validate_host_closure() -> dict[str, Any]:
    ...
    if any(current[name] is not value for name, value in _BOUND_BASE_APIS.items()):
        raise TWRPT2ProfileError("T0 H0 base API was rebound")
    with pin_regular_file(
        BASE_SOURCE,
    ...
    with pin_regular_file(
        BUILDER, label="T2 builder", expected_size=BUILDER_SIZE,
...
def parse_recovery_observation(
    ...
    if type(result) is not tuple or len(result) != 3:
        raise TWRPT2ProfileError("T2 observer envelope differs")
    ...
    if (
        ...
        raise TWRPT2ProfileError("T2 observer command failed or exceeded bounds")
    try:
        text = stdout.decode("utf-8", "strict")
    except UnicodeError as exc:
        raise TWRPT2ProfileError("T2 observer output is not UTF-8") from exc
    if "\r" in text or "\x00" in text or not text.endswith("\n"):
        raise TWRPT2ProfileError("T2 observer framing differs")
    lines = text.splitlines()
    if len(lines) != len(RECOVERY_OUTPUT_KEYS):
        raise TWRPT2ProfileError("T2 observer field count differs")
    values: dict[str, str] = {}
    for expected, line in zip(RECOVERY_OUTPUT_KEYS, lines, strict=True):
        ...
        if separator != "=" or key != expected or key in values or not value:
            raise TWRPT2ProfileError("T2 observer grammar differs")
        values[key] = value
    if any(values.get(key) != value for key, value in EXPECTED_RECOVERY_FIXED_OUTPUT.items()):
        raise TWRPT2ProfileError("T2 observer fixed values differ")
    if BOOT_ID_RE.fullmatch(values["boot_id"]) is None:
        raise TWRPT2ProfileError("T2 recovery boot ID differs")
    return values
