#!/usr/bin/env python3
"""Build the S22+ M10A2 getpid then reboot native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M10A2 is the post-M10A1 split. M10A1 bootlooped after a read-only
newfstatat("/dev") before reboot. M10A2 removes pathname/VFS access and tests
whether one non-VFS syscall before reboot is survivable.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a2_getpid_reboot_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m10a2_getpid_reboot.c")
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | int | list[str]]:
    ...
    require_ok(result, "compile M10A2 getpid reboot init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M10A2 getpid reboot init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M10A2 getpid reboot init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", "-S", out_path])
    require_ok(readelf, "readelf M10A2 getpid reboot init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M10A2 getpid reboot init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M10A2 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M10A2 init is not AArch64")
    ...
    if svc_count != 2:
        raise SystemExit(f"M10A2 init expected exactly two svc instructions, saw {svc_count}: {svc_lines}")
    getpid_nr_lines = [index for index, line in enumerate(objdump_lines) if "mov" in line and "x8" in line and "#0xac" in line]
    ...
    if not getpid_nr_lines:
        raise SystemExit("M10A2 init does not load arm64 __NR_getpid (172) into x8")
    if not reboot_nr_lines:
        raise SystemExit("M10A2 init does not load arm64 __NR_reboot (142) into x8")
    if forbidden_nr_lines:
        raise SystemExit(f"M10A2 init unexpectedly loads a VFS-ish syscall number: {forbidden_nr_lines}")

    getpid_func_start = function_start_for_instruction(objdump_lines, getpid_nr_lines[0])
    ...
    try:
        getpid_call_index = branch_targets.index(getpid_func_start)
        reboot_call_index = branch_targets.index(reboot_func_start)
    except ValueError as exc:
        raise SystemExit(
            "M10A2 _start does not branch to both getpid and reboot helpers "
            f"(branches={branch_targets!r}, getpid=0x{getpid_func_start:x}, reboot=0x{reboot_func_start:x})"
        ) from exc
    if reboot_call_index < getpid_call_index:
        raise SystemExit("M10A2 _start calls reboot before getpid")

    binary = out_path.read_bytes()
    for required in REQUIRED_STRINGS:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"M10A2 /init missing required string: {required}")
    for forbidden in FORBIDDEN_STRINGS:
        if forbidden in binary:
            raise SystemExit(f"M10A2 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m10a2_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m10a2_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m10a2_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
        "file": (file_info.stdout + file_info.stderr).decode("utf-8", errors="replace").strip(),
        "readelf": readelf_text,
        "objdump": objdump_text,
        "svc_count": svc_count,
        "getpid_func_start": f"0x{getpid_func_start:x}",
...
def main(argv: list[str]) -> int:
    ...
    m10a2_init = build_dir / "s22plus_init_m10a2_getpid_reboot"
    m10a2_init_info = compile_init(source, m10a2_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m10a2_init}"], work_dir, "replace /init with M10A2 init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M10A2 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m10a2_init):
        raise SystemExit("replaced /init does not match compiled M10A2 init")
    ...
    hashes = {
        ...
        "m10a2_init": sha256_file(m10a2_init),
    ...
    sizes = {
        ...
        "m10a2_init": m10a2_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M10A2 in-place freestanding C non-VFS getpid then reboot discriminator",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns_without_manual_entry": "one non-VFS pre-reboot syscall is survivable; M10A1 points at pathname VFS access",
            "no_download_mode": "failure is broader than pathname VFS; inspect pre-reboot syscall/timing/state",
        },
    }
