#!/usr/bin/env python3
"""Build a P3.02-M0 carrier with the exact qualified P3.00 Image."""
...
import s22plus_fyg8_p302_boot_only_packager as packager
...
SCHEMA = "s22plus_fyg8_p302_candidate_artifact_result_v1"
VERDICT = "PASS_P302_DETERMINISTIC_BOOT_ONLY_CANDIDATE_HOST_ONLY"
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_fyg8_p302/candidate-a")
...
def verify_repro_result(
    ...
    *,
    ...
    if result_path != root / contract.PARENT_REPRO_RESULT:
        raise BuildError("P3.02 parent reproducibility path differs")
    value, result_receipt = _read_json(
        result_path, "P3.02 fixed P3.00 reproducibility result"
    )
    ...
    if (
        ...
        raise BuildError("P3.02 fixed P3.00 reproducibility closure differs")
    return {
    ...
    result.update(
        {
            ...
            "p302_kernel_rebuild": False,
            "p302_carrier_id": exact_contract["carrier"]["id"],
