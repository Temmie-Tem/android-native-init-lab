#!/usr/bin/env python3
"""Build the S22+ M10A4 inline probe then reboot native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M10A4 is the post-M10A3 split. M10A3 bootlooped after a no-syscall helper call
before reboot. M10A4 removes that pre-reboot function call/return but keeps
inline stack work in _start before the reboot helper.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a4_inline_probe_reboot_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m10a4_inline_probe_reboot.c")
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | int | list[str]]:
    ...
    require_ok(result, "compile M10A4 inline probe reboot init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M10A4 inline probe reboot init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M10A4 inline probe reboot init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", "-S", out_path])
    require_ok(readelf, "readelf M10A4 inline probe reboot init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M10A4 inline probe reboot init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M10A4 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M10A4 init is not AArch64")
    ...
    if svc_count != 1:
        raise SystemExit(f"M10A4 init expected exactly one svc instruction, saw {svc_count}: {svc_lines}")
    ...
    if not reboot_nr_lines:
        raise SystemExit("M10A4 init does not load arm64 __NR_reboot (142) into x8")
    if getpid_nr_lines:
        raise SystemExit(f"M10A4 init unexpectedly loads arm64 __NR_getpid (172): {getpid_nr_lines}")
    if forbidden_nr_lines:
        raise SystemExit(f"M10A4 init unexpectedly loads a VFS-ish syscall number: {forbidden_nr_lines}")
    ...
    if branch_targets != [reboot_func_start]:
        raise SystemExit(f"M10A4 expected exactly one branch to reboot helper, got {branch_targets!r}")

    binary = out_path.read_bytes()
    for required in REQUIRED_STRINGS:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"M10A4 /init missing required string: {required}")
    for forbidden in FORBIDDEN_STRINGS:
        if forbidden in binary:
            raise SystemExit(f"M10A4 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m10a4_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m10a4_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m10a4_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m10a4_init = build_dir / "s22plus_init_m10a4_inline_probe_reboot"
    m10a4_init_info = compile_init(source, m10a4_init, build_dir)
    ...
    patch_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {m10a4_init}"], work_dir, "replace /init with M10A4 init")
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M10A4 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m10a4_init):
        raise SystemExit("replaced /init does not match compiled M10A4 init")
    ...
    hashes = {
        ...
        "m10a4_init": sha256_file(m10a4_init),
    ...
    sizes = {
        ...
        "m10a4_init": m10a4_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M10A4 in-place freestanding C inline stack-probe then reboot discriminator",
        "safety": {
            "boot_only": True,
        ...
        "future_live_interpretation": {
            "download_mode_returns_without_manual_entry": "pre-reboot inline stack work is survivable; M10A3 points at the helper call/return boundary",
            "no_download_mode": "any pre-reboot stack work or instruction delay before reboot helper is suspect",
        },
    }
