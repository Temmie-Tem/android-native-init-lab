#!/usr/bin/env python3
"""Attended one-use corrected-order T2 fastbootd preparation probe without USB role switch."""
...
import s20plus_g986n_twrp_fastbootd_census as census
import s20plus_g986n_twrp_fastbootd_prep_probe as predecessor
import s20plus_g986n_twrp_t2_profile_h0 as t2_profile


VERSION = "s20plus-g986n-twrp-fastbootd-prep-q1-v1"
PENDING = "PASS_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_READY_RETURN_PENDING"
FINAL_PASS = "PASS_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_RETURNED_HEALTHY"
FINAL_NO_PROOF = "NO_PROOF_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_RETURNED_HEALTHY"

LIVE_ACTIVE = True
EXPECTED_REVIEWED_NORMALIZED_SHA256 = "53d297e007b2a7246a7a1c8cbecb743ec51d53ce5d64eb66789678a25e7a4bdd"

RUN_ROOT_REL = Path("workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-q1")
SHARED_GUARD_REL = Path(
    "workspace/private/runs/s20plus-g986n-routine-actions/active-action.json"
)
CONSUMED_NAME = "consumed-prep-q1-intent.json"
OLD_FINAL_REL = Path(
    "workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-probe/"
    "run-20260902T184407Z-1788374647061721372/final-result.json"
)
OLD_FINAL_SIZE = 1_275
OLD_FINAL_SHA256 = "ea9a201bf1bd9a093c70a3791ebf4fb1b1b4f489788bfe2660ee2c1d5372221d"
OLD_CONSUMED_REL = Path(
    "workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-probe/"
    "consumed-prep-intent.json"
)
OLD_CONSUMED_SIZE = 657
OLD_CONSUMED_SHA256 = "b0349cb99202bb5997f196fc74db7656bf25342f068e5b955339807c0847a4c0"
OLD_COMMAND_REL = Path(
    "workspace/private/runs/s20plus-g986n-twrp-fastbootd-prep-probe/"
    "run-20260902T184407Z-1788374647061721372/prep-command-result.json"
)
OLD_COMMAND_SIZE = 688
OLD_COMMAND_SHA256 = "50042ef670fd700d907773eee4f046039cdb693f1127d9878049a2909d97903e"
OLD_RAW_STDOUT_SIZE = 70
OLD_RAW_STDOUT_SHA256 = "6dd9ce2743aaca177c1194e88b0ec4e88493c72f0d59ab1496163855b926ae3a"
OLD_RUNNER_SIZE = 41_056
OLD_RUNNER_SHA256 = "60b3e72996c87eb884ade79112301a541d5af400bbed102dd1e7a46fd71703c4"
MAX_BYTES = 32 * 1024
...
[ -d "$g/functions/ffs.fastboot" ] && [ ! -L "$g/functions/ffs.fastboot" ] || exit 211
...
/system/bin/grep -q ' /dev/usb-ffs/fastboot functionfs ' /proc/mounts || exit 212
...
def require_support(root: Path) -> dict[str, Any]:
    ...
    old_source = pin_file(old_path, OLD_RUNNER_SIZE, OLD_RUNNER_SHA256, "old prep runner")
...
def validate_predecessor(root: Path) -> dict[str, Any]:
    pin_file(root / OLD_FINAL_REL, OLD_FINAL_SIZE, OLD_FINAL_SHA256, "old prep terminal")
    pin_file(
        root / OLD_CONSUMED_REL,
        OLD_CONSUMED_SIZE,
        OLD_CONSUMED_SHA256,
        "old prep consumed marker",
    )
    pin_file(
        root / OLD_COMMAND_REL,
        OLD_COMMAND_SIZE,
        OLD_COMMAND_SHA256,
        "old prep command result",
    )
    if lexists(guard_path(root)):
        raise PrepProbeError("shared S20+ guard is not empty")
    value = predecessor.finalize(root, no_device_command)
    if (
        value.get("verdict")
        != "NO_PROOF_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_RETURNED_HEALTHY"
        or value.get("replay_permitted") is not False
        or value.get("prep_proved") is not False
        ...
        raise PrepProbeError("old prep terminal differs")
    old_run = (root / OLD_FINAL_REL).parent
    old_rc, old_stdout, old_stderr = predecessor.raw_result(old_run, "connected-06")
    if (
        old_rc != 0
        or len(old_stdout) != OLD_RAW_STDOUT_SIZE
        or hashlib.sha256(old_stdout).hexdigest() != OLD_RAW_STDOUT_SHA256
        or old_stderr != b""
    ):
        raise PrepProbeError("old prep mount failure differs")
    return value
...
def check_guard(root: Path, run_dir: Path) -> None:
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_guard_v1"
        or value.get("version") != VERSION
        or value.get("action") != "attended-t2-fastbootd-prep-q1"
...
def create_intent(root: Path, run_dir: Path, prepared: dict[str, Any]) -> dict[str, Any]:
    intent = {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_intent_v1",
        "version": VERSION,
        "run_dir": str(run_dir),
...
def classify_prepare(result: tuple[int, bytes, bytes]) -> dict[str, Any]:
    ...
    return {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_command_result_v1",
        "version": VERSION,
        "success": success,
...
def connected_probe(root: Path, run_dir: Path, command: Command) -> dict[str, Any]:
    ...
    recovery = census.recovery_snapshot(command, t2)
    target = {
        "model": "SM-G986N",
        "device": "y2q",
        "product": "y2qksx",
    ...
    result = {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_probe_result_v1",
        "version": VERSION,
        "target": target,
...
def validate_prepared(run_dir: Path, value: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        set(value) != expected
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_prepared_v1"
        or value.get("version") != VERSION
        or value.get("run_dir") != str(run_dir)
        ...
        or value.get("starting_preflight") != census.PREFLIGHT_EXPECTED
        or any(
...
def validate_intent(
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_intent_v1"
        or value.get("version") != VERSION
        or value.get("run_dir") != str(run_dir)
...
def validate_probe(
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_probe_result_v1"
        or value.get("version") != VERSION
...
def validate_final_result(
    ...
    if (
        ...
        or value.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_final_result_v1"
        or value.get("version") != VERSION
...
def finalize(root: Path, command: Command) -> dict[str, Any]:
    ...
    result = {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_final_result_v1",
        "version": VERSION,
        "target": target,
        "returned_android": {
...
def abort_pre_effect(root: Path) -> dict[str, Any]:
    ...
    result = {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_abort_v1",
        "version": VERSION,
        "device_effects": 0,
        "persistent_writes": 0,
        "partition_operations": 0,
        "verdict": "ABORTED_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_BEFORE_EFFECT",
        "at": now(),
    }
    ...
    if lexists(path):
        ...
        if (
            ...
            or current.get("schema") != "s20plus_g986n_twrp_fastbootd_prep_q1_abort_v1"
            ...
            != "ABORTED_S20PLUS_G986N_TWRP_FASTBOOTD_PREP_Q1_BEFORE_EFFECT"
...
def plan() -> dict[str, Any]:
    return {
        "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_plan_v1",
        "version": VERSION,
...
def main() -> int:
    ...
    if args.render_plan:
        ...
        return 0
    if not LIVE_ACTIVE:
        print("STOP_S20PLUS_TWRP_FASTBOOTD_PREP_Q1: reviewed activation is absent")
        return 2
    if args.abort_pre_effect:
        try:
            ...
            print("FAIL_S20PLUS_TWRP_FASTBOOTD_PREP_Q1_ABORT_CLOSED")
            return 1
    if not args.operator_attended:
        print("STOP_S20PLUS_TWRP_FASTBOOTD_PREP_Q1: attendance is required")
        return 2
    if args.connected:
        ...
        try:
            ...
            base.durable_write(
                ...
                {
                    "schema": "s20plus_g986n_twrp_fastbootd_prep_q1_failure_v1",
                    "version": VERSION,
            ...
            print("RECOVERY_PENDING_S20PLUS_TWRP_FASTBOOTD_PREP_Q1_PHYSICAL_REBOOT")
            return 1
            ...
            if not consumed and lexists(guard_path(root)):
                try:
                    ...
                    pass
            print(
                "RECOVERY_PENDING_S20PLUS_TWRP_FASTBOOTD_PREP_Q1_PHYSICAL_REBOOT"
                if consumed
                else "FAIL_S20PLUS_TWRP_FASTBOOTD_PREP_Q1_PRE_EFFECT_CLOSED"
            )
            return 1
    ...
    try:
        ...
        print("FAIL_S20PLUS_TWRP_FASTBOOTD_PREP_Q1_FINALIZE_CLOSED")
        return 1
    print(result["verdict"])
