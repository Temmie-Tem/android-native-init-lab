#!/usr/bin/env python3
"""Exact boot-only A90 H17 resident installer for the existing read-only UFS root.

This runner deliberately has no SD-rootfs staging mode.  It binds one H17 boot
...
SCHEMA = "a90-h17-ufs-f1-manifest-v1"
RESULT_SCHEMA = "a90-h17-ufs-f1-result-v1"
JOURNAL_SCHEMA = "a90-h17-ufs-f1-journal-v1"
QUALIFICATION_SCHEMA = "a90-h17-ufs-execution-qualification-v1"
INVENTORY_SCHEMA = "a90-h17-ufs-readonly-inventory-v1"
APPROVAL_SCHEMA = "a90-h17-ufs-f1-approval-prepared-v1"
APPROVAL_BINDING_SCHEMA = "a90-h17-ufs-f1-approval-binding-v1"
APPROVAL_PREFIX = "A90-H17-F1-APPROVE:"
APPROVAL_TTL_SEC = 1800
CAPABILITY = "A90_H17_PERSISTENT_UFS_SERVER_V1"
RUN_ID_RE = re.compile(r"^a90-h17-ufs-f1-[0-9]{8}-[0-9]{2}$")
HEX64_RE = re.compile(r"^[0-9a-f]{64}$")

CURRENT_VERSION = "0.11.184"
CURRENT_BUILD = "phase3-minimal-h16-direct-ufs-ro-async-wifi-auto-benchmark"
...
ENABLE_PATH = "/cache/a90-auto-handoff-phase3-minimal-h17.enable"
LATCH_PATH = "/cache/a90-auto-handoff-phase3-minimal-h17.done"
CONTENT_REL = (
...
VERSION_MANIFEST_REL = (
    "workspace/public/src/scripts/revalidation/a90_flat_builder/versions/"
    "phase3-minimal-h17/manifest.toml"
)
...
NATIVE_CLOSURE_SHA256 = (
    "ab5ce09b1464be5bd3f69026522a85678d53f852462f0a7a222608b605c42016"
)
HOST_CAPABILITY_CLOSURE_SHA256 = (
    "45aa74c13ca8a11b4bb06b192436b8416fa294f5c200fa0d2326cec6bca076ee"
)
HOST_QUALIFICATION_REL = (
    "workspace/public/src/scripts/revalidation/a90_flat_builder/versions/"
    "phase3-minimal-h17/capability-qualification.json"
)
...
class ContractError(RuntimeError):
    """Raised before widening, replaying, or misclassifying an H17 effect."""
...
def execution_closure() -> dict[str, Any]:
    ...
    if not isinstance(init, dict):
        raise ContractError("H17 native manifest init binding is absent")
    ...
    if (
        ...
        raise ContractError("H17 native transitive closure changed")
...
def validate_host_capability_qualification() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema")
        != "a90-h17-persistent-ufs-server-capability-qualification-v1"
        or value.get("capability") != CAPABILITY
        or value.get("verdict") != "PASS_GO"
        or value.get("execution_closure_sha256")
        != HOST_CAPABILITY_CLOSURE_SHA256
        or value.get("review_scope")
        != "h17-public-builder-native-overlay-hud-and-observer-capability"
        or value.get("new_hazard_or_incident") is not True
        or value.get("ordinal_requalification_required") is not False
        ...
        raise ContractError("H17 host capability qualification changed")
    return value
...
def validate_ab_receipt(
    ...
    if (
        ...
        raise ContractError("H17 deterministic build receipt is not exact")
    return binding
...
def _baseline_inputs(manifest: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    if (
        manifest.get("schema") != "a90-h16-ufs-f1-manifest-v1"
        or manifest.get("capability") != "A90_DIRECT_UFS_READONLY_ROOT_V2"
        ...
        or result.get("schema") != "a90-h16-ufs-f1-result-v1"
        or result.get("status") != "PASS_A90_H16_UFS_RESIDENT_INSTALLED"
        ...
        raise ContractError("H16 resident predecessor is not exact healthy 0,0")


def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    if RUN_ID_RE.fullmatch(args.run_id) is None:
        raise ContractError("H17 run id is not exact")
    run_dir = (PRIVATE_RUN_BASE / args.run_id).resolve()
    if run_dir.parent != PRIVATE_RUN_BASE:
        raise ContractError("H17 run directory escapes private run base")
    ...
    if not isinstance(baseline_manifest, dict) or not isinstance(baseline_result, dict):
        raise ContractError("H16 resident predecessor JSON shape changed")
    _baseline_inputs(baseline_manifest, baseline_result)
    if baseline_result.get("manifest_sha256") != sha256_file(
        baseline_manifest_path
    ):
        raise ContractError("H16 resident predecessor manifest binding changed")
    ...
    if not isinstance(target, dict) or not isinstance(rollback_value, dict):
        raise ContractError("H16 target/rollback binding is absent")
    ...
    if bridge_device != EXACT_BRIDGE_DEVICE:
        raise ContractError("H16 exact A90 bridge path is absent")
    ...
    if (
        ...
        raise ContractError("H16 observer keypair changed")

    candidate = resolve_regular_input(Path(args.candidate), "candidate boot")
    receipt_path = resolve_regular_input(Path(args.ab_receipt), "AB receipt")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if not isinstance(receipt, dict):
        raise ContractError("H17 AB receipt is not an object")
...
def load_manifest(path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise ContractError("H17 F1 manifest header changed")
    ...
    if (
        ...
        raise ContractError("H17 authority widened")

    closure = execution_closure()
    if value.get("execution_closure") != closure:
        raise ContractError("H17 execution-critical closure changed")
    validate_host_capability_qualification()
    ...
    if (
        ...
        raise ContractError("H17 UFS binding changed")

    predecessor = value.get("predecessor")
    if not isinstance(predecessor, dict):
        raise ContractError("H16 resident predecessor binding is absent")
    ...
    if not isinstance(predecessor_target, dict) or not isinstance(target, dict):
        raise ContractError("H17 exact target predecessor binding is absent")
    ...
    if (
        ...
        raise ContractError("H17 exact target or recovery binding changed")
    ...
    if (
        ...
        raise ContractError("H17 observer or network scope changed")
...
def _approval_path(manifest: dict[str, Any]) -> Path:
    ...
    if run_dir.parent != PRIVATE_RUN_BASE:
        raise ContractError("H17 approval path escapes private run base")
    return run_dir / "h17-f1-approval-prepared.json"
...
def validate_approval(
    ...
    if not stat.S_ISREG(info.st_mode) or path.is_symlink() or info.st_mode & 0o077:
        raise ContractError("H17 F1 approval is not a private regular file")
    value = json.loads(path.read_text(encoding="utf-8"))
    binding = value.get("approval_binding") if isinstance(value, dict) else None
    if not isinstance(binding, dict):
        raise ContractError("H17 F1 approval binding is absent")
    binding_sha = json_sha256(binding)
    ...
    if (
        ...
        raise ContractError("H17 F1 approval is not fresh and exact")
    return value
...
def require_consumed_approval(
    ...
    if len(matches) != 1:
        raise ContractError("H17 F1 approval consumption is not durable and unique")
    record = matches[0]
    binding = record.get("approval_binding")
    if not isinstance(binding, dict):
        raise ContractError("H17 F1 consumed approval binding is absent")
    expected = approval_binding(
        manifest,
        manifest_sha,
        created_utc=str(binding.get("created_utc") or ""),
    ...
    if (
        ...
        raise ContractError("H17 F1 consumed approval changed")
    return record
...
def _journal_dir(manifest: dict[str, Any]) -> Path:
    ...
    return run_dir / "h17-f1-live" / "journal"
...
def read_journal(path: Path, manifest: dict[str, Any], manifest_sha: str) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(sorted(path.glob("*.json"))):
        ...
        if (
            ...
            raise ContractError("durable H17 journal is inconsistent")
...
def exact_preflight(
    ...
    script = "\n".join(
        (
            ...
            "echo A90H17_F1_PRE exact=1 devt=$DEVT devt_policy=same-session-only ufs_mounted=0 enable_absent=1 latch_absent=1 userdata_write=0",
        )
    )
    record = base.run_f1_shell(args, script)
    marker = re.compile(
        r"^A90H17_F1_PRE exact=1 devt=[0-9]+:[0-9]+ "
    ...
    if len(marker.findall(str(record.get("text") or "").replace("\r", ""))) != 1:
        raise ContractError("fresh H17 connected preflight is not exact")
...
def exact_candidate_health(spec: Any, args: argparse.Namespace, guard: Any) -> dict[str, Any]:
    ...
    if not isinstance(first_boot, dict) or first_boot.get("proof") is not True:
        raise ContractError("H17 first boot is not exact unarmed resident health")
...
def _require_launch_quiesced(
    ...
    if (
        ...
        or value.get("schema") != "a90-h17-flash-process-group-v1"
...
def _flash_record(
    ...
    *,
    ...
    try:
        ...
        launch = {
            "schema": "a90-h17-flash-process-group-v1",
            "kind": kind,
            "leader_pid": pid,
            "pgid": pgid,
...
def recover(
    ...
    if args.operator_attended is not True:
        raise ContractError("A90 H17 recovery is attended-only")
...
def _publish_reconciled_result(
    ...
    if result_path.exists():
        ...
        if existing != result:
            raise ContractError("existing H17 result differs from durable closure")
    else:
...
def reconcile_health(
    ...
    if args.operator_attended is not True:
        raise ContractError("A90 H17 F1 health reconciliation is attended-only")
    validate_live_args(args)
    manifest = load_manifest(manifest_path, manifest_sha)
    spec = _spec(manifest, manifest_path, manifest_sha)
    journal = _journal_dir(manifest)
    ...
    if actions.count("closed") > 1 or ("closed" in actions and actions[-1] != "closed"):
        raise ContractError("H17 closed record is not terminal")
    if "closed" in actions:
        ...
        if (
            ...
            raise ContractError("durable H17 closed result is invalid")
    ...
    if candidate_count == 1:
        ...
        result = {
            "schema": RESULT_SCHEMA,
            "status": "PASS_A90_H17_UFS_RESIDENT_INSTALLED",
            "run_id": manifest["run_id"],
            "manifest_sha256": manifest_sha,
            "device_safety_state": "RESIDENT_HEALTHY",
            "candidate_attempt_count": 1,
...
def execute(manifest_path: Path, manifest_sha: str, args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("A90 H17 F1 is attended-only")
    validate_live_args(args)
    manifest = load_manifest(manifest_path, manifest_sha)
    spec = _spec(manifest, manifest_path, manifest_sha)
    journal = _journal_dir(manifest)
    ...
    records = read_journal(journal, manifest, manifest_sha)
    if records:
        raise ContractError("fresh H17 execution requires an empty durable journal")
...
def audit(manifest_path: Path, manifest_sha: str) -> dict[str, Any]:
    manifest = load_manifest(manifest_path, manifest_sha)
    return {
        "schema": "a90-h17-ufs-f1-audit-v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"H17_UFS_F1_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
