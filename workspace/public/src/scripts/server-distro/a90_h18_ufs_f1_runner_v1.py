#!/usr/bin/env python3
"""Exact boot-only A90 H18 resident installer for the existing read-only UFS root.

This runner deliberately has no SD-rootfs staging mode.  It binds one H18 boot
...
RESULT_SCHEMA = "a90-h18-ufs-f1-result-v1"
JOURNAL_SCHEMA = "a90-h18-ufs-f1-journal-v1"
QUALIFICATION_SCHEMA = "a90-h18-ufs-execution-qualification-v1"
EXECUTION_REVIEW_SCHEMA = (
    "a90-h18-ufs-f1-d1-execution-independent-review-v1"
)
INVENTORY_SCHEMA = "a90-h18-ufs-readonly-inventory-v1"
APPROVAL_SCHEMA = "a90-h18-ufs-f1-approval-prepared-v1"
APPROVAL_BINDING_SCHEMA = "a90-h18-ufs-f1-approval-binding-v1"
APPROVAL_PREFIX = "A90-H18-F1-APPROVE:"
APPROVAL_TTL_SEC = 1800
D0_MAX_AGE_SEC = 900
CAPABILITY = "A90_H18_POST_ROOT_FAILURE_ATTRIBUTION_V1"
RUN_ID_RE = re.compile(r"^a90-h18-ufs-f1-[0-9]{8}-[0-9]{2}$")
HEX64_RE = re.compile(r"^[0-9a-f]{64}$")
H18_AUTO_STATUS_RE = re.compile(
    r"^A90AUTO_STATUS binding=(?P<binding>[01]) "
    r"enable=(?P<enable>[01]) latch=(?P<latch>[01]) "
    r"build=(?P<build>[a-z0-9._-]+)$"
)

CURRENT_VERSION = "0.11.185"
CURRENT_BUILD = "phase3-minimal-h17-ufs-ro-observer-auth-persistent-hud"
...
H17_D1_TERMINAL_RESULT_SHA256 = (
    "e4a831ac64f79164f33d8adec0c7f63a133641ffe8471d4a330756a2d5eafeb3"
)
H17_D1_RECORDS = (
    (
        "0000-open.json",
    ...
    (
        ...
        318655,
        "fbf053f9adc803e57ac8d9f6f4654e29cada16634262684e271964b8f904badc",
    ),
    (
        "0006-closed.json",
        318649,
        "a801c7b0cbba4fcc54ed3de1c358a540f7dab335723451b2627a0e0bf881d36a",
    ),
)
CANDIDATE_VERSION = "0.11.186"
CANDIDATE_BUILD = "phase3-minimal-h18-post-root-failure-attribution"
CANDIDATE_BOOT_SIZE = 58384384
...
CANDIDATE_INIT_SHA256 = (
    "3e32373f71c2ebedf9d2e0454268a701089b0df54f30aeb5d718a9920d2ef135"
)
CANDIDATE_RAMDISK_SIZE = 8550912
CANDIDATE_RAMDISK_SHA256 = (
    "655a38756f492045c19d103ebfc180bd428852ea415fad3f33429b792e6760e3"
)
...
CANDIDATE_AB_RECEIPT_SHA256 = (
    "6227da6f1238cf09bd7c9e80e6a30d947bb3857a5680a7ac13ef05c711b947a9"
)
CANDIDATE_AB_RECEIPT_SIZE = 4927
...
ENABLE_PATH = "/cache/a90-auto-handoff-phase3-minimal-h18.enable"
LATCH_PATH = "/cache/a90-auto-handoff-phase3-minimal-h18.done"
CONTENT_REL = (
...
VERSION_MANIFEST_REL = (
    "workspace/public/src/scripts/revalidation/a90_flat_builder/versions/"
    "phase3-minimal-h18/manifest.toml"
)
...
HOST_CAPABILITY_CLOSURE_SHA256 = (
    "88f9aa7a38549a379223fc08ccb84792a4848122f32bb1d62abbe8be03800534"
)
HOST_QUALIFICATION_REL = (
    "workspace/public/src/scripts/revalidation/a90_flat_builder/versions/"
    "phase3-minimal-h18/capability-qualification.json"
)
HOST_REVIEW_REPORT_REL = (
    "docs/reports/"
    "A90_H18_POST_ROOT_FAILURE_ATTRIBUTION_INDEPENDENT_REVIEW_2026-08-11.json"
)
EXECUTION_REVIEW_REPORT_REL = (
    "docs/reports/A90_H18_UFS_F1_D1_EXECUTION_INDEPENDENT_REVIEW_2026-08-12.json"
)
EXECUTION_REVIEWER = "/root/a90_h18_execution_review"
EXECUTION_REVIEW_SCOPE = (
    "h18-boot-only-f1-and-diagnostic-ufs-d1-execution-critical-closure"
)
EXECUTION_REVIEW_INCIDENT = "H18_POST_ROOT_FAILURE_ATTRIBUTION"
EXECUTION_REVIEW_REQUIRED_INVARIANTS = (
    "exact H17 terminal predecessor and exact reviewed H18 A/B artifacts",
    "one boot-only H18 candidate or exact V2321 rollback transfer with no replay",
    "fresh F1 and D1 approvals do not derive live authority from qualification",
    "one attended arm-plus-reboot dispatch with no D1 effect replay",
    "exact whole-log prefix and ordered appended diagnostic and cleanup records",
    "missing diagnostic remains NO_PROOF while clean native return proves health only",
...
EXECUTION_SOURCE_RELS = (
    "AGENTS.md",
    "workspace/public/src/scripts/server-distro/a90_h18_ufs_f1_runner_v1.py",
    "workspace/public/src/scripts/server-distro/a90_h18_ufs_d1_runner_v1.py",
    "workspace/public/src/scripts/server-distro/a90_h18_persistent_server_observer_v1.py",
    FIRSTBOOT_REL,
...
class ContractError(RuntimeError):
    """Raised before widening, replaying, or misclassifying an H18 effect."""
...
def load_reviewed_ab_receipt(path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if (
        ...
        raise ContractError("H18 reviewed AB receipt changed")
    try:
        value = json.loads(resolved.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ContractError("H18 AB receipt is not exact JSON") from exc
    if not isinstance(value, dict):
        raise ContractError("H18 AB receipt is not an object")
...
def require_reviewed_ab_receipt_binding(value: Any) -> None:
    if (
        ...
        raise ContractError("H18 live AB receipt is not the reviewed artifact")
...
def execution_closure() -> dict[str, Any]:
    ...
    if not isinstance(init, dict):
        raise ContractError("H18 native manifest init binding is absent")
    ...
    if (
        ...
        raise ContractError("H18 native transitive closure changed")
...
def validate_host_capability_qualification() -> dict[str, Any]:
    ...
    reviewed_rels = (
        "tests/test_a90_h17_persistent_server_source_v1.py",
        "workspace/public/src/native-init/a90_server_distro.c",
        VERSION_MANIFEST_REL,
    )
    expected_hashes = {
        ...
        for relative in reviewed_rels
    }
    review_report = (REPO_ROOT / HOST_REVIEW_REPORT_REL).resolve(strict=True)
    if (
        not isinstance(value, dict)
        or value.get("schema")
        != "a90-h18-post-root-failure-attribution-capability-qualification-v1"
        or value.get("capability") != CAPABILITY
        or value.get("verdict") != "PASS_GO"
        or value.get("execution_closure_sha256")
        != HOST_CAPABILITY_CLOSURE_SHA256
        or value.get("execution_hashes") != expected_hashes
        or value.get("review_scope")
        != "h18-post-root-failure-attribution-native-capability"
        or value.get("new_hazard_or_incident") is not True
        or value.get("ordinal_requalification_required") is not False
        or value.get("f1_runner_qualified") is not False
        or value.get("d1_runner_qualified") is not False
        or value.get("record_semantics")
        != "observed-a90-log-required-for-attribution"
        or value.get("missing_record_verdict") != "NO_PROOF_NO_REPLAY"
        or value.get("review_report") != HOST_REVIEW_REPORT_REL
        or value.get("review_report_sha256") != sha256_file(review_report)
        or value.get("live_authority") is not False
    ):
        raise ContractError("H18 host capability qualification changed")
    return value
...
def validate_ufs_inventory(
    ...
    *,
    ...
    expected_marker = (
        f"A90H18_D0 exact=1 devt={observed_devt} "
...
def validate_ab_receipt(
    ...
    if (
        ...
        or input_pins.get("h17_firstboot_sha256")
        != sha256_file(REPO_ROOT / FIRSTBOOT_REL)
        ...
        raise ContractError("H18 deterministic build receipt is not exact")
    return binding
...
def _baseline_inputs(manifest: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    if (
        manifest.get("schema") != "a90-h17-ufs-f1-manifest-v1"
        or manifest.get("capability") != "A90_H17_PERSISTENT_UFS_SERVER_V1"
        ...
        or result.get("schema") != "a90-h17-ufs-f1-result-v1"
        or result.get("status") != "PASS_A90_H17_UFS_RESIDENT_INSTALLED"
        ...
        raise ContractError("H17 resident predecessor is not exact healthy 0,0")


def validate_h17_d1_terminal(
    value: Any,
    ...
    if (
        ...
        or len(value["records"]) != len(H17_D1_RECORDS)
    ):
        raise ContractError("H17 D1 terminal binding is not exact")
    actions = (
    ...
    for sequence, (filename, size, sha) in enumerate(H17_D1_RECORDS):
        ...
        if (
            ...
            raise ContractError("H17 D1 journal artifact changed")
        ...
        if path.name != filename or (parent is not None and path.parent != parent):
            raise ContractError("H17 D1 journal path set changed")
        parent = path.parent
        if (
            record.get("schema") != "a90-h17-ufs-d1-journal-v1"
            or record.get("sequence") != sequence
            or record.get("action") != actions[sequence]
        ):
            raise ContractError("H17 D1 journal sequence changed")
    ...
    if (
        ...
        or final.get("result_sha256") != H17_D1_TERMINAL_RESULT_SHA256
        or json_sha256(result) != H17_D1_TERMINAL_RESULT_SHA256
        or closed.get("result_sha256") != H17_D1_TERMINAL_RESULT_SHA256
        or closed.get("result") != result
        or result.get("schema") != "a90-h17-ufs-d1-result-v1"
        ...
        raise ContractError("H17 D1 terminal HEALTHY barrier changed")
    return value


def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    if RUN_ID_RE.fullmatch(args.run_id) is None:
        raise ContractError("H18 run id is not exact")
    run_dir = (PRIVATE_RUN_BASE / args.run_id).resolve()
    if run_dir.parent != PRIVATE_RUN_BASE:
        raise ContractError("H18 run directory escapes private run base")
    ...
    predecessor_d1 = {
        ...
        "records": [
            ...
            for filename, _, _ in H17_D1_RECORDS
        ],
        "terminal_result_sha256": H17_D1_TERMINAL_RESULT_SHA256,
    }
    validate_h17_d1_terminal(
        predecessor_d1,
    ...
    if not isinstance(target, dict) or not isinstance(rollback_value, dict):
        raise ContractError("H17 target/rollback binding is absent")
    ...
    if bridge_device != EXACT_BRIDGE_DEVICE:
        raise ContractError("H17 exact A90 bridge path is absent")
    ...
    if not isinstance(observer, dict):
        raise ContractError("H17 observer binding is absent")
    ...
    if (
        ...
        raise ContractError("H17 observer keypair changed")
...
def load_manifest(path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise ContractError("H18 F1 manifest header changed")
    ...
    if (
        ...
        raise ContractError("H18 authority widened")

    closure = execution_closure()
    if value.get("execution_closure") != closure:
        raise ContractError("H18 execution-critical closure changed")
    validate_host_capability_qualification()
    ...
    if (
        ...
        raise ContractError("H18 UFS binding changed")

    predecessor = value.get("predecessor")
    if not isinstance(predecessor, dict):
        raise ContractError("H17 resident predecessor binding is absent")
    ...
    if not isinstance(predecessor_target, dict) or not isinstance(target, dict):
        raise ContractError("H18 exact target predecessor binding is absent")
    ...
    if (
        ...
        raise ContractError("H18 exact target or recovery binding changed")
    ...
    if (
        ...
        raise ContractError("H18 observer or network scope changed")
...
def _approval_path(manifest: dict[str, Any]) -> Path:
    ...
    if run_dir.parent != PRIVATE_RUN_BASE:
        raise ContractError("H18 approval path escapes private run base")
    return run_dir / "h18-f1-approval-prepared.json"
...
def validate_approval(
    ...
    if not stat.S_ISREG(info.st_mode) or path.is_symlink() or info.st_mode & 0o077:
        raise ContractError("H18 F1 approval is not a private regular file")
    value = json.loads(path.read_text(encoding="utf-8"))
    binding = value.get("approval_binding") if isinstance(value, dict) else None
    if not isinstance(binding, dict):
        raise ContractError("H18 F1 approval binding is absent")
    binding_sha = json_sha256(binding)
    ...
    if (
        ...
        raise ContractError("H18 F1 approval is not fresh and exact")
    return value
...
def require_consumed_approval(
    ...
    if len(matches) != 1:
        raise ContractError("H18 F1 approval consumption is not durable and unique")
    record = matches[0]
    binding = record.get("approval_binding")
    if not isinstance(binding, dict):
        raise ContractError("H18 F1 consumed approval binding is absent")
    expected = approval_binding(
        manifest,
        manifest_sha,
        created_utc=str(binding.get("created_utc") or ""),
    ...
    if (
        ...
        raise ContractError("H18 F1 consumed approval changed")
    return record
...
def _journal_dir(manifest: dict[str, Any]) -> Path:
    ...
    return run_dir / "h18-f1-live" / "journal"
...
def read_journal(path: Path, manifest: dict[str, Any], manifest_sha: str) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(sorted(path.glob("*.json"))):
        ...
        if (
            ...
            raise ContractError("durable H18 journal is inconsistent")
...
def _validate_closed_result(
    ...
    if (
        ...
        raise ContractError("durable H18 closed result is invalid")
    ...
    if candidate_health is not None:
        ...
        valid = (
            result.get("status") == "PASS_A90_H18_UFS_RESIDENT_INSTALLED"
    ...
    if not valid:
        raise ContractError("durable H18 closed result contradicts journal state")
...
def _validate_f1_journal(
    ...
    if any(action not in allowed or actions.count(action) != 1 for action in actions):
        raise ContractError("durable H18 journal action set changed")
    opening = ("approval-consumed", "guard-armed", "candidate-intent")
    if tuple(actions[: min(3, len(actions))]) != opening[: min(3, len(actions))]:
        raise ContractError("durable H18 journal opening order changed")
    if "closed" in actions and actions[-1] != "closed":
        raise ContractError("durable H18 closed record is not terminal")
    approval = records[0]
    if (
        ...
        raise ContractError("durable H18 consumed approval changed")
    if len(actions) >= 2 and (
        records[1].get("candidate_replay") is not False
        or not isinstance(records[1].get("guard"), dict)
    ):
        raise ContractError("durable H18 guard arm changed")
    if len(actions) >= 3:
        intent = records[2]
        if (
            ...
            raise ContractError("durable H18 candidate intent changed")
    if len(actions) <= 3:
        return
    ...
    if candidate_launch:
        ...
        if (
            ...
            or launch.get("schema") != "a90-h18-flash-process-group-v1"
    ...
    if actions[result_index] != "candidate-result":
        raise ContractError("durable H18 candidate launch/result order changed")
    ...
    if (
        ...
        raise ContractError("durable H18 candidate result changed")
    ...
    if tuple(core) not in valid_cores:
        raise ContractError("durable H18 journal state transition changed")
...
def append_journal(
    ...
    if not validated or validated[-1] != value:
        raise ContractError("durable H18 journal append did not revalidate")
    return value
...
def validate_live_args(args: argparse.Namespace) -> None:
    ...
    for name, expected in exact.items():
        ...
        if (
            ...
            raise ContractError(f"H18 live argument {name} changed")


def require_current_native_health(args: argparse.Namespace) -> dict[str, Any]:
    """Read and validate the exact H17 predecessor without a legacy allowlist."""
    health = {
...
def exact_preflight(
    ...
    script = "\n".join(
        (
            ...
            "echo A90H18_F1_PRE exact=1 devt=$DEVT devt_policy=same-session-only ufs_mounted=0 enable_absent=1 latch_absent=1 userdata_write=0",
        )
    )
    record = base.run_f1_shell(args, script)
    marker = re.compile(
        r"^A90H18_F1_PRE exact=1 devt=[0-9]+:[0-9]+ "
    ...
    if len(marker.findall(str(record.get("text") or "").replace("\r", ""))) != 1:
        raise ContractError("fresh H18 connected preflight is not exact")
...
def validate_candidate_native_health(
    ...
    if (
        ...
        raise ContractError("H18 candidate native health identity changed")
    version = base.require_exact_f1_command_receipt(
        native.get("version"), ["version"], "H18 candidate version"
    )
    selftest = base.require_exact_f1_command_receipt(
        native.get("selftest"), ["selftest"], "H18 candidate selftest"
    )
    ...
    if (
        ...
        raise ContractError("H18 candidate native health facts changed")
    return native


def validate_h18_auto_status_record(
    record: Any,
    *,
    enable: int,
    latch: int,
    ...
    if len(lines) != 1:
        raise ContractError("H18 auto-handoff status is not unique")
    match = H18_AUTO_STATUS_RE.fullmatch(lines[0])
    if match is None:
        raise ContractError("H18 auto-handoff status shape changed")
    facts = {
    ...
    if facts != {
        ...
        raise ContractError("H18 auto-handoff status facts changed")
    return facts
...
def exact_candidate_health(
    ...
    if not isinstance(first_boot, dict) or first_boot.get("proof") is not True:
        raise ContractError("H18 first boot is not exact unarmed resident health")
...
def validate_stored_candidate_health(
    ...
    if (
        ...
        raise ContractError("durable H18 candidate health changed")
    validate_candidate_native_health(native, manifest)
    validate_h18_auto_status_record(first_boot.get("status"), enable=0, latch=0)
    log = base.require_exact_f1_command_receipt(
        first_boot.get("log"), ["logcat"], "durable H18 first-boot log"
    )
    base.require_auto_handoff_log_exclusively_unarmed(
        str(log.get("text") or ""),
        "durable H18 first-boot log",
    )
    return health
...
def _require_launch_quiesced(
    ...
    if (
        ...
        or value.get("schema") != "a90-h18-flash-process-group-v1"
...
def _flash_record(
    ...
    *,
    ...
    try:
        ...
        launch = {
            "schema": "a90-h18-flash-process-group-v1",
            "kind": kind,
            "leader_pid": pid,
            "pgid": pgid,
...
def recover(
    ...
    if args.operator_attended is not True:
        raise ContractError("A90 H18 recovery is attended-only")
...
def _publish_reconciled_result(
    ...
    if result_path.exists():
        ...
        if existing != result:
            raise ContractError("existing H18 result differs from durable closure")
    else:
...
def reconcile_health(
    ...
    if args.operator_attended is not True:
        raise ContractError("A90 H18 F1 health reconciliation is attended-only")
    validate_live_args(args)
    manifest = load_manifest(manifest_path, manifest_sha)
    spec = _spec(manifest, manifest_path, manifest_sha)
    journal = _journal_dir(manifest)
    ...
    if actions.count("closed") > 1 or ("closed" in actions and actions[-1] != "closed"):
        raise ContractError("H18 closed record is not terminal")
    if "closed" in actions:
    ...
    if not candidate_results:
        ...
        try:
            ...
            if not _reconstructed_candidate_transfer_is_proven(candidate_record):
                ...
                _park_recovery(manifest, manifest_sha, journal, pending)
                raise ContractError(
                    "candidate boot bytes are ambiguous despite running H18 health; "
                    "resume bound rollback only"
                ) from pending
...
def execute(manifest_path: Path, manifest_sha: str, args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("A90 H18 F1 is attended-only")
    validate_live_args(args)
    manifest = load_manifest(manifest_path, manifest_sha)
    _, inventory = load_json_bound(
    ...
    records = read_journal(journal, manifest, manifest_sha)
    if records:
        raise ContractError("fresh H18 execution requires an empty durable journal")
...
def audit(manifest_path: Path, manifest_sha: str) -> dict[str, Any]:
    manifest = load_manifest(manifest_path, manifest_sha)
    return {
        "schema": "a90-h18-ufs-f1-audit-v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"H18_UFS_F1_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
