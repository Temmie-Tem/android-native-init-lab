#!/usr/bin/env python3
"""One-shot attended A90 H17 persistent Debian server handoff.

The runner arms one already-installed H17 resident, reboots once, observes the
...
import a90_h17_persistent_server_observer_v1 as persistent_observer  # noqa: E402
import a90_v3403_f1_orchestrator as base  # noqa: E402


SCHEMA = "a90-h17-ufs-d1-journal-v1"
RESULT_SCHEMA = "a90-h17-ufs-d1-result-v1"
RECONCILE_SCHEMA = "a90-h17-ufs-d1-reconciliation-v1"
APPROVAL_SCHEMA = "a90-h17-ufs-d1-approval-prepared-v1"
APPROVAL_BINDING_SCHEMA = "a90-h17-ufs-d1-approval-binding-v1"
APPROVAL_PREFIX = "A90-H17-D1-APPROVE:"
...
UNMOUNTED_RE = re.compile(
    r"^A90H17_POST_PHYSICAL_RETURN devt=(?P<major>[0-9]+):"
...
class ContractError(RuntimeError):
    """Raised before replaying or widening the exact H17 D1 ordinal."""
...
def _load_install_result(
    ...
    if f1.sha256_file(path) != expected_sha256:
        raise ContractError("H17 install result SHA256 changed")
    ...
    if (
        ...
        raise ContractError("H17 resident install terminal is not exact")
    return result
...
def load_inputs(args: argparse.Namespace) -> tuple[dict[str, Any], Any, dict[str, Any]]:
    ...
    if args.expect_execution_closure_sha256 != closure["sha256"]:
        raise ContractError("H17 capability execution closure changed")
...
def parse_status(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(lines) != 1:
        raise ContractError("H17 auto-handoff status is not unique")
    match = STATUS_RE.fullmatch(lines[0])
    if match is None:
        raise ContractError("H17 auto-handoff status shape changed")
    value = {
    ...
    if value["binding"] != 1 or value["build"] != f1.CANDIDATE_BUILD:
        raise ContractError("auto-handoff status is not the exact H17 binding")
    return value
...
def require_status(
    ...
    *,
    ...
    if (status["enable"], status["latch"]) != (enable, latch):
        raise ContractError(
            f"H17 state is {status['enable']},{status['latch']}; "
...
def require_cancellation_fsync_proof(
    ...
    before = _exact_log_text(opening_log, "H17 cancellation opening log")
    after = _exact_log_text(final_log, "H17 cancellation final log")
    pattern = re.compile(
    ...
    if before_matches or len(after_matches) != 1:
        raise ContractError(
            "H17 no-effect close lacks one post-intent cancellation fsync proof"
        )
    return {
        "proof": True,
...
def _expected_h17_state(intent_sha256: str, state: str) -> bytes:
    if f1.HEX64_RE.fullmatch(intent_sha256) is None:
        raise ContractError("H17 state intent SHA256 is invalid")
    if state not in {
        "armed-after-native-health",
        "automatic-handoff-dispatched-no-replay",
    }:
        raise ContractError("H17 state name is invalid")
    lines = (
...
def require_same_intent_state(
    ...
    exact = base.require_exact_f1_command_receipt(
        record,
        command,
        "H17 exact enable/latch/evidence intent binding",
    )
    lines = [
        line.strip()
        for line in str(exact.get("text") or "").replace("\r", "").splitlines()
...
def _write_record(
    ...
    if index >= len(JOURNAL_NAMES):
        raise ContractError("H17 D1 journal overflow")
    value = {
        "schema": SCHEMA,
        "sequence": index,
        "action": action,
    ...
    for index, name in enumerate(JOURNAL_NAMES):
        ...
        if not path.exists():
            if any((transaction_dir / later).exists() for later in JOURNAL_NAMES[index + 1 :]):
                raise ContractError("H17 D1 journal has a gap")
            break
        ...
        if (
            ...
            raise ContractError("H17 D1 journal record changed")
...
def approval_binding(
    ...
    *,
    ...
    return {
        ...
        "workflow": "A90_H17_ATTENDED_PERSISTENT_SERVER_D1_V1",
...
def validate_approval(
    ...
    if not path.is_file() or path.is_symlink() or info.st_mode & 0o077:
        raise ContractError("H17 D1 approval is not a private regular file")
    value = json.loads(path.read_text(encoding="utf-8"))
    binding = value.get("approval_binding") if isinstance(value, dict) else None
    if not isinstance(binding, dict):
        raise ContractError("H17 D1 approval binding is absent")
    ...
    if (
        ...
        raise ContractError("H17 D1 approval is not fresh and exact")
    return value
...
def _validate_records(
    ...
    if (
        ...
        raise ContractError("H17 D1 opening binding changed")
    opening_log = opening.get("opening_log")
    if not isinstance(opening_log, dict):
        raise ContractError("H17 D1 opening log is absent")
    _exact_log_text(opening_log, "H17 D1 durable opening log")
    ...
    if approval_value != expected_approval:
        raise ContractError("H17 D1 consumed approval binding changed")
    if len(records) == 1:
        return None
    intent = records[1]
    if (
        ...
        raise ContractError("H17 D1 arm-reboot intent changed")
    ...
    if any(
        ...
        raise ContractError("H17 D1 continuation intent binding changed")
    if len(records) >= 3 and (
        ...
        raise ContractError("H17 D1 dispatch result changed")
    if len(records) >= 4 and (
        ...
        raise ContractError("H17 D1 persistent observation changed")
    if len(records) >= 5:
        ...
        no_effect = (
            ...
            == "ABORTED_H17_ARM_REBOOT_NO_PERSISTENT_EFFECT"
        ...
        if (
            ...
            or result.get("candidate_replay") is not False
            or not (live_pending or no_effect)
            or records[4].get("result_sha256") != f1.json_sha256(result)
        ):
            raise ContractError("H17 D1 current-state result changed")
    if len(records) >= 6:
        result = records[5].get("result")
        same_intent = (
            result.get("same_intent_binding")
        ...
        physical_return = (
            ...
            and same_intent.get("proof") is True
            and same_intent.get("intent_sha256") == intent_sha256
        )
        no_effect_final = (
            isinstance(result, dict)
            and result.get("status")
            == "ABORTED_H17_ARM_REBOOT_NO_PERSISTENT_EFFECT"
            and result.get("arm_persistent_effect_count") == 0
            and result.get("cancellation_fsync", {}).get("proof") is True
        )
        if (
            not isinstance(result, dict)
            or result.get("schema") != RESULT_SCHEMA
...
def _live_result(
    ...
    if server_proven and guard_released and visible_confirmed == "yes":
        status = "PASS_A90_H17_PERSISTENT_SERVER_LIVE"
    elif visible_confirmed == "no":
        status = "REFUTED_A90_H17_PERSISTENT_SERVER_DISPLAY"
    else:
        status = "NO_PROOF_A90_H17_PERSISTENT_SERVER_LIVE"
    return {
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H17 automatic UFS handoff is attended-only")
...
def _unmounted_script() -> str:
    return "\n".join(
        (
            ...
            '[ "$C" = 0 ]',
            'echo "A90H17_POST_PHYSICAL_RETURN devt=$MAJ:$MIN '
...
def _prove_userdata_unmounted(effect_args: argparse.Namespace) -> dict[str, Any]:
    ...
    exact = base.require_exact_f1_command_receipt(
        record,
        command,
        "H17 post-physical-return unmounted userdata",
    )
    lines = [
        line.strip()
        for line in str(exact.get("text") or "").replace("\r", "").splitlines()
        if line.strip().startswith("A90H17_POST_PHYSICAL_RETURN ")
    ]
    match = UNMOUNTED_RE.fullmatch(lines[0]) if len(lines) == 1 else None
    if match is None or int(match.group("count"), 10) != 0:
        raise ContractError("H17 userdata is not one exact unmounted identity")
...
def finalize_physical_return(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ContractError(
            "H17 physical-return finalization requires attended confirmation"
        )
    ...
    live_proven = live_result.get("status") == "PASS_A90_H17_PERSISTENT_SERVER_LIVE"
    result = {
        "schema": RESULT_SCHEMA,
        "status": (
            "PASS_A90_H17_PERSISTENT_SERVER_PHYSICAL_RETURN_HEALTHY"
            if live_proven
            else "NO_PROOF_A90_H17_PERSISTENT_SERVER_PHYSICAL_RETURN_HEALTHY"
        ),
        "intent_sha256": intent_sha256,
...
def finalize_no_effect(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H17 no-effect finalization is attended-only")
    manifest, spec, _ = load_inputs(args)
    transaction_dir = _require_transaction_dir(
        manifest,
        args.transaction_dir,
    ...
    current = {
        "schema": RESULT_SCHEMA,
        "status": "ABORTED_H17_ARM_REBOOT_NO_PERSISTENT_EFFECT",
        "intent_sha256": intent_sha256,
        "device_safety_state": "RESIDENT_HEALTHY",
        "resident_healthy": True,
        "ordinal_closed": False,
...
def reconcile(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(records) == 5:
        current = records[4]["result"]
        terminal = (
            "PERSISTENT_DEBIAN_LIVE_HEALTH_PENDING_NO_REPLAY"
            if current.get("device_safety_state")
            == "HEALTH_PENDING_PERSISTENT_DEBIAN"
            else "NO_PERSISTENT_EFFECT_READY_FOR_FINALIZE"
        )
        return {
            "schema": RECONCILE_SCHEMA,
            "terminal": terminal,
    ...
    try:
        if args.prepare_approval:
            ...
            if args.approval is None:
                raise ContractError("H17 D1 execute requires fresh exact approval")
        ...
        print(f"H17_UFS_D1_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
