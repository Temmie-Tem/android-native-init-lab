#!/usr/bin/env python3
"""Bounded authenticated observer for the live A90 H24 Debian server.
...
the H24 persistent-server result.
"""
...
SCHEMA = "a90-h24-persistent-server-observer-v1"
HOST = "192.168.7.2"
PORT = 2222
BEGIN = "A90H24_OBSERVER_BEGIN"
END = "A90H24_OBSERVER_END"
...
class ObserverError(RuntimeError):
    """Raised when H24 live facts are absent, ambiguous, or malformed."""
...
LC_ALL=C
export LC_ALL
marker_snapshot=$(cat /run/a90-d3-marker 2>/dev/null || true)
marker_value() {
  key=$1
  printf '%s\n' "$marker_snapshot" | /usr/bin/awk -F= -v wanted="$key" '
...
hud_exe=$(readlink /proc/$hud_pid/exe 2>/dev/null || true)
hud_start_ticks_before=$(/usr/bin/awk '{ print $22 }' /proc/$hud_pid/stat 2>/dev/null || true)
status_snapshot=$(cat /run/a90-dpublic/hud-presenter.status 2>/dev/null || true)
hud_drm_fd_count=0
hud_drm_fd_target=
hud_directory_fd_count=0
for fd in /proc/$hud_pid/fd/*; do
  target=$(readlink "$fd" 2>/dev/null || true)
  kind=$(/usr/bin/stat -Lc '%F' "$fd" 2>/dev/null || true)
  [ "$kind" = "directory" ] && hud_directory_fd_count=$((hud_directory_fd_count + 1))
  case "$target" in
    *dri*|*card0*|*drm*)
      hud_drm_fd_count=$((hud_drm_fd_count + 1))
      hud_drm_fd_target=$target
      ;;
  esac
done
hud_stdio_targets=$(printf '%s|%s|%s' \
  "$(readlink /proc/$hud_pid/fd/0 2>/dev/null || true)" \
  "$(readlink /proc/$hud_pid/fd/1 2>/dev/null || true)" \
  "$(readlink /proc/$hud_pid/fd/2 2>/dev/null || true)")
pid1_mnt_ns=$(readlink /proc/1/ns/mnt 2>/dev/null || true)
hud_mnt_ns=$(readlink /proc/$hud_pid/ns/mnt 2>/dev/null || true)
hud_mnt_ns_distinct=0
[ -n "$pid1_mnt_ns" ] && [ -n "$hud_mnt_ns" ] && \
  [ "$pid1_mnt_ns" != "$hud_mnt_ns" ] && hud_mnt_ns_distinct=1
dir_entries() {
  /bin/ls -1A "$1" 2>/dev/null | /usr/bin/sort | /usr/bin/awk '
    BEGIN { separator = "" }
    { printf "%s%s", separator, $0; separator = "," }
    END { print "" }
  '
}
debian_dev_entries_before=$(dir_entries /dev)
debian_dev_core_meta=$(
  for leaf in console tty ptmx null zero random urandom; do
    printf '%s=' "$leaf"
    /usr/bin/stat -Lc '%F|%t:%T|%a' "/dev/$leaf" 2>/dev/null || printf missing
    printf '\n'
  done | /usr/bin/awk '
    BEGIN { separator = "" }
    { printf "%s%s", separator, $0; separator = ";" }
...
debian_dev_pts_meta=$(/usr/bin/stat -Lc '%F|%a' /dev/pts 2>/dev/null || true)
debian_dev_ttygs0_meta=$(/usr/bin/stat -Lc '%F|%t:%T|%a' /dev/ttyGS0 2>/dev/null || printf absent)
debian_dev_block_count=0
for entry in /dev/*; do
  [ -e "$entry" ] || continue
  kind=$(/usr/bin/stat -Lc '%F' "$entry" 2>/dev/null || true)
  [ "$kind" = "block special file" ] && \
    debian_dev_block_count=$((debian_dev_block_count + 1))
done
debian_dev_entries_after=$(dir_entries /dev)
debian_dev_mount=$(/usr/bin/awk '$2 == "/dev" { count += 1; value = $1 "|" $3 "|" $4 } END { if (count == 1) print value; else print "__invalid_count_" count + 0 }' /proc/mounts 2>/dev/null)
debian_dev_pts_mount=$(/usr/bin/awk '$2 == "/dev/pts" { count += 1; value = $1 "|" $3 "|" $4 } END { if (count == 1) print value; else print "__invalid_count_" count + 0 }' /proc/mounts 2>/dev/null)
hud_root=/proc/$hud_pid/root
hud_root_entries=$(dir_entries "$hud_root")
hud_dev_entries=$(dir_entries "$hud_root/dev")
hud_dri_entries=$(dir_entries "$hud_root/dev/dri")
hud_run_entries=$(dir_entries "$hud_root/run")
hud_card_meta=$(/usr/bin/stat -Lc '%F|%t:%T' "$hud_root/dev/dri/card0" 2>/dev/null || true)
hud_exe_after=$(readlink /proc/$hud_pid/exe 2>/dev/null || true)
hud_start_ticks_after=$(/usr/bin/awk '{ print $22 }' /proc/$hud_pid/stat 2>/dev/null || true)
status_value() {
  key=$1
  printf '%s\n' "$status_snapshot" | /usr/bin/awk -F= -v wanted="$key" '
    $1 == wanted { count += 1; value = $2 }
    END { if (count == 1) print value; else print "__invalid_count_" count + 0 }
  '
}
echo A90H24_OBSERVER_BEGIN
...
echo debian_dev_entries_before=$debian_dev_entries_before
echo debian_dev_entries_after=$debian_dev_entries_after
echo debian_dev_core_meta=$debian_dev_core_meta
echo debian_dev_pts_meta=$debian_dev_pts_meta
echo debian_dev_ttygs0_meta=$debian_dev_ttygs0_meta
echo debian_dev_block_count=$debian_dev_block_count
echo debian_dev_mount=$debian_dev_mount
echo debian_dev_pts_mount=$debian_dev_pts_mount
...
echo firstboot_mount_count=$(/usr/bin/awk '$2 == "/etc/a90-d3-firstboot" { count += 1 } END { print count + 0 }' /proc/mounts 2>/dev/null)
...
echo hud_exe=$hud_exe
echo hud_exe_after=$hud_exe_after
echo hud_start_ticks_before=$hud_start_ticks_before
echo hud_start_ticks_after=$hud_start_ticks_after
echo hud_drm_fd_count=$hud_drm_fd_count
echo hud_drm_fd_target=$hud_drm_fd_target
echo hud_directory_fd_count=$hud_directory_fd_count
echo hud_stdio_targets=$hud_stdio_targets
echo hud_mnt_ns_distinct=$hud_mnt_ns_distinct
echo hud_root_entries=$hud_root_entries
echo hud_dev_entries=$hud_dev_entries
echo hud_dri_entries=$hud_dri_entries
echo hud_run_entries=$hud_run_entries
echo hud_card_meta=$hud_card_meta
echo hud_status_state=$(status_value state)
echo hud_status_pid=$(status_value pid)
echo hud_present_rc=$(status_value present_rc)
echo hud_last_sequence=$(status_value last_sequence)
echo hud_status_intent=$(status_value intent)
echo hud_status_owner=$(status_value owner)
echo hud_status_process_model=$(status_value process_model)
...
def exact_section(text: str) -> str:
    ...
    if lines.count(BEGIN) != 1 or lines.count(END) != 1:
        raise ObserverError("H24 observer transcript markers are not unique")
    begin = lines.index(BEGIN)
    end = lines.index(END)
    if end <= begin:
        raise ObserverError("H24 observer transcript marker order is invalid")
...
def parse_facts(text: str) -> dict[str, str]:
    ...
    for line in section.splitlines():
        if "=" not in line:
            raise ObserverError("H24 observer fact line is malformed")
        key, value = line.split("=", 1)
        if KEY_RE.fullmatch(key) is None or key in facts:
            raise ObserverError("H24 observer fact keys are not exact")
        facts[key] = value
    if set(facts) != EXPECTED_KEYS:
        raise ObserverError("H24 observer fact set is not exact")
    return facts


def classify(text: str, returncode: int, visible_hud: bool) -> dict[str, Any]:
    if returncode != 0:
        raise ObserverError("authenticated H24 SSH observation failed")
    ...
    dev_parts = facts["debian_dev_mount"].split("|")
    devpts_parts = facts["debian_dev_pts_mount"].split("|")
    ...
    dev_options = set(dev_parts[2].split(",")) if len(dev_parts) == 3 else set()
    devpts_options = (
        set(devpts_parts[2].split(",")) if len(devpts_parts) == 3 else set()
    )
    auth_key_size = (
    ...
    exact = {
        ...
        "debian_minimal_dev": (
            facts["debian_dev_entries_before"]
            == facts["debian_dev_entries_after"]
            and facts["debian_dev_entries_before"]
            in {
                "console,null,ptmx,pts,random,tty,urandom,zero",
                "console,null,ptmx,pts,random,tty,ttyGS0,urandom,zero",
            }
            and facts["debian_dev_core_meta"]
            == (
            ...
            and facts["debian_dev_pts_meta"] == "directory|755"
            and (
                facts["debian_dev_ttygs0_meta"] == "absent"
                or re.fullmatch(
                    r"character special file\|[0-9a-f]+:[0-9a-f]+\|600",
                    facts["debian_dev_ttygs0_meta"],
                )
                is not None
            )
            and (
                (facts["debian_dev_ttygs0_meta"] == "absent")
                == ("ttyGS0" not in facts["debian_dev_entries_before"].split(","))
            )
            and facts["debian_dev_block_count"] == "0"
            and len(dev_parts) == 3
            and dev_parts[0] == "tmpfs"
            and dev_parts[1] == "tmpfs"
            and {"rw", "nosuid", "noexec"} <= dev_options
            and "devtmpfs" not in facts["debian_dev_mount"]
            and len(devpts_parts) == 3
            and devpts_parts[0] == "devpts"
            and devpts_parts[1] == "devpts"
            and "rw" in devpts_options
        ),
        "observer_auth_tmpfs": (
            len(auth_parts) == 3
        ...
        "firstboot_overlay_disabled": facts["firstboot_mount_count"] == "0",
        ...
        "native_hud_presenter": (
            ...
            and facts["hud_exe_after"] == facts["hud_exe"]
            and POSITIVE_RE.fullmatch(facts["hud_start_ticks_before"])
            is not None
            and facts["hud_start_ticks_after"]
            == facts["hud_start_ticks_before"]
            and facts["hud_drm_fd_count"] == "1"
            and facts["hud_drm_fd_target"] == "/dev/dri/card0"
            and facts["hud_directory_fd_count"] == "0"
            and facts["hud_stdio_targets"]
            == "/dev/null|/dev/null|/dev/null"
            and facts["hud_mnt_ns_distinct"] == "1"
            and facts["hud_root_entries"] == "dev,run"
            and facts["hud_dev_entries"] == "dri"
            and facts["hud_dri_entries"] == "card0"
            and facts["hud_run_entries"] == "a90-dpublic"
            and facts["hud_card_meta"] == "character special file|e2:0"
            and facts["hud_status_state"] == "running"
            and facts["hud_status_pid"] == facts["hud_pid"]
            and facts["hud_present_rc"] == "0"
            and POSITIVE_RE.fullmatch(facts["hud_last_sequence"]) is not None
            and facts["hud_status_intent"]
            == "/run/a90-dpublic/hud-intent.json"
            and facts["hud_status_owner"] == "native-init"
            and facts["hud_status_process_model"]
            == "forked-native-child-survives-switch-root"
            and facts["marker_hud_intent_written"] == "1"
            # The immutable UFS firstboot script probes before publishing the
            # intent.  H24 deliberately owns no DRM FD at that point, so its
            # legacy H17 marker trio must stay zero.  Current success comes
            # only from the exact PID-coupled status and private card root
            # facts above, observed after the intent has been consumed.
            and facts["marker_hud_presenter_pid_valid"] == "0"
            and facts["marker_hud_presenter_started"] == "0"
            and facts["marker_hud_started"] == "0"
        ),
        "wifi": (
    ...
    return {
        "schema": SCHEMA,
        "status": "PASS_A90_H24_PERSISTENT_SERVER_LIVE" if proof else "NO_PROOF_A90_H24_PERSISTENT_SERVER",
        "proof": proof,
...
def observe(
    ...
    *,
    ...
    last: dict[str, Any] = {
        "schema": SCHEMA,
        "status": "NO_PROOF_A90_H24_PERSISTENT_SERVER",
        "proof": False,
        "device_safety": "HEALTH_PENDING_PERSISTENT_DEBIAN",
        "new_device_effect_authority": False,
        "automatic_native_return_expected": False,
