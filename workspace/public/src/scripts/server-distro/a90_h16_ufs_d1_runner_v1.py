#!/usr/bin/env python3
"""One-shot attended A90 H16 direct-UFS automatic handoff and benchmark.

The runner arms one already-installed H16 resident, reboots once, observes the
existing UFS appliance as Debian PID 1 over the same A90 USB/NCM identity, and
requires a later exact H16 native return.  It transfers no payload, flashes no
...
SCHEMA = "a90-h16-ufs-d1-journal-v1"
RESULT_SCHEMA = "a90-h16-ufs-d1-result-v1"
RECONCILE_SCHEMA = "a90-h16-ufs-d1-reconciliation-v1"
APPROVAL_SCHEMA = "a90-h16-ufs-d1-approval-prepared-v1"
APPROVAL_BINDING_SCHEMA = "a90-h16-ufs-d1-approval-binding-v1"
APPROVAL_PREFIX = "A90-H16-D1-APPROVE:"
...
class ContractError(RuntimeError):
    """Raised before replaying or widening the exact H16 D1 ordinal."""
...
def _load_install_result(
    ...
    if f1.sha256_file(path) != expected_sha256:
        raise ContractError("H16 install result SHA256 changed")
    ...
    if (
        ...
        raise ContractError("H16 resident install terminal is not exact")
    return result
...
def load_inputs(args: argparse.Namespace) -> tuple[dict[str, Any], Any, dict[str, Any]]:
    ...
    if args.expect_execution_closure_sha256 != closure["sha256"]:
        raise ContractError("H16 capability execution closure changed")
...
def parse_status(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(lines) != 1:
        raise ContractError("H16 auto-handoff status is not unique")
    match = STATUS_RE.fullmatch(lines[0])
    if match is None:
        raise ContractError("H16 auto-handoff status shape changed")
    value = {
    ...
    if value["binding"] != 1 or value["build"] != f1.CANDIDATE_BUILD:
        raise ContractError("auto-handoff status is not the exact H16 binding")
    return value
...
def require_status(
    ...
    *,
    ...
    if (status["enable"], status["latch"]) != (enable, latch):
        raise ContractError(
            f"H16 state is {status['enable']},{status['latch']}; "
...
def require_cancellation_fsync_proof(
    ...
    before = _exact_log_text(opening_log, "H16 cancellation opening log")
    after = _exact_log_text(final_log, "H16 cancellation final log")
    pattern = re.compile(
    ...
    if before_matches or len(after_matches) != 1:
        raise ContractError(
            "H16 no-effect close lacks one post-intent cancellation fsync proof"
        )
    return {
        "proof": True,
...
def native_failure_cleanup_disposition(
    ...
    *,
    ...
    before = _exact_log_text(opening_log, "H16 cleanup opening log")
    after = _exact_log_text(final_log, "H16 cleanup final log")
    pattern = re.compile(
...
def _write_record(
    ...
    if index >= len(JOURNAL_NAMES):
        raise ContractError("H16 D1 journal overflow")
    value = {
        "schema": SCHEMA,
        "sequence": index,
        "action": action,
    ...
    for index, name in enumerate(JOURNAL_NAMES):
        ...
        if not path.exists():
            if any((transaction_dir / later).exists() for later in JOURNAL_NAMES[index + 1 :]):
                raise ContractError("H16 D1 journal has a gap")
            break
        ...
        if (
            ...
            raise ContractError("H16 D1 journal record changed")
...
def validate_approval(
    ...
    if not path.is_file() or path.is_symlink() or info.st_mode & 0o077:
        raise ContractError("H16 D1 approval is not a private regular file")
    value = json.loads(path.read_text(encoding="utf-8"))
    binding = value.get("approval_binding") if isinstance(value, dict) else None
    if not isinstance(binding, dict):
        raise ContractError("H16 D1 approval binding is absent")
    ...
    if (
        ...
        raise ContractError("H16 D1 approval is not fresh and exact")
    return value
...
def _validate_records(
    ...
    if (
        ...
        raise ContractError("H16 D1 opening binding changed")
    opening_log = opening.get("opening_log")
    if not isinstance(opening_log, dict):
        raise ContractError("H16 D1 opening log is absent")
    _exact_log_text(opening_log, "H16 D1 durable opening log")
    ...
    if approval_value != expected_approval:
        raise ContractError("H16 D1 consumed approval binding changed")
    if len(records) == 1:
        return None
    intent = records[1]
    if (
        ...
        raise ContractError("H16 D1 arm-reboot intent changed")
    ...
    if any(
        ...
        raise ContractError("H16 D1 continuation intent binding changed")
    if len(records) >= 3 and (
        ...
        raise ContractError("H16 D1 dispatch result changed")
    if len(records) >= 4 and (
        ...
        raise ContractError("H16 D1 observation changed")
    if len(records) >= 5:
...
def _finalize(
    ...
    if benchmark.get("native_handoff_failed") is True:
        terminal = "REFUTED_H16_NATIVE_HANDOFF_FAILED_RESIDENT_HEALTHY"
    elif durable.get("proof") is True and wifi and host_link and guard_released:
        terminal = (
            "PASS_H16_UFS_AUTO_HANDOFF_VISIBLE"
            if visible_confirmed == "yes"
            else "REFUTED_H16_UFS_DISPLAY_VISIBILITY"
            if visible_confirmed == "no"
            else "PASS_H16_UFS_AUTO_HANDOFF_NO_PROOF_VISIBILITY"
        )
    else:
        terminal = "NO_PROOF_H16_UFS_OBSERVER_RESIDENT_HEALTHY"
    return {
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H16 automatic UFS handoff is attended-only")
...
def finalize_return(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.operator_attended is not True:
        raise ContractError("H16 return finalization is attended-only")
...
def finalize_no_effect(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H16 no-effect finalization is attended-only")
    manifest, spec, _ = load_inputs(args)
    transaction_dir = _require_transaction_dir(
        manifest,
        args.transaction_dir,
    ...
    result = {
        "schema": RESULT_SCHEMA,
        "terminal": "ABORTED_H16_ARM_REBOOT_NO_PERSISTENT_EFFECT",
        "intent_sha256": intent_sha256,
        "resident_healthy": True,
        "candidate_replay": False,
    ...
    try:
        if args.prepare_approval:
            ...
            if args.approval is None:
                raise ContractError("H16 D1 execute requires fresh exact approval")
            value = execute(args)
        ...
        print(f"H16_UFS_D1_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
