#!/usr/bin/env python3
"""One-shot attended A90 H18 persistent Debian server handoff.

The runner arms one already-installed H18 resident, reboots once, observes the
...
import a90_h18_persistent_server_observer_v1 as persistent_observer  # noqa: E402
import a90_v3403_f1_orchestrator as base  # noqa: E402


SCHEMA = "a90-h18-ufs-d1-journal-v1"
RESULT_SCHEMA = "a90-h18-ufs-d1-result-v1"
RECONCILE_SCHEMA = "a90-h18-ufs-d1-reconciliation-v1"
APPROVAL_SCHEMA = "a90-h18-ufs-d1-approval-prepared-v1"
APPROVAL_BINDING_SCHEMA = "a90-h18-ufs-d1-approval-binding-v1"
APPROVAL_PREFIX = "A90-H18-D1-APPROVE:"
...
NATIVE_FALLBACK_CURRENT_STATUSES = frozenset(
    {
        "REFUTED_H18_POST_ROOT_FAILURE_ATTRIBUTED_NATIVE_FALLBACK_CURRENT",
        "REFUTED_H18_OTHER_FAILURE_ATTRIBUTED_NATIVE_FALLBACK_CURRENT",
        "NO_PROOF_H18_FAILURE_ATTRIBUTION_NATIVE_FALLBACK_CURRENT",
    }
)
NATIVE_FALLBACK_FINAL_STATUSES = frozenset(
    {
        "REFUTED_H18_POST_ROOT_FAILURE_ATTRIBUTED_NATIVE_FALLBACK_HEALTHY",
        "REFUTED_H18_OTHER_FAILURE_ATTRIBUTED_NATIVE_FALLBACK_HEALTHY",
        "NO_PROOF_H18_FAILURE_ATTRIBUTION_NATIVE_FALLBACK_HEALTHY",
    }
)
JOURNAL_NAMES = (
    "0000-open.json",
...
class ContractError(RuntimeError):
    """Raised before replaying or widening the exact H18 D1 ordinal."""
...
def _load_install_result(
    ...
    if f1.sha256_file(path) != expected_sha256:
        raise ContractError("H18 install result SHA256 changed")
    ...
    if (
        ...
        raise ContractError("H18 resident install terminal is not exact")
...
def load_inputs(args: argparse.Namespace) -> tuple[dict[str, Any], Any, dict[str, Any]]:
    ...
    if args.expect_execution_closure_sha256 != closure["sha256"]:
        raise ContractError("H18 capability execution closure changed")
...
def parse_status(record: dict[str, Any]) -> dict[str, Any]:
    exact = base.require_exact_f1_command_receipt(
        record, ["auto-handoff-status"], "H18 auto-handoff status"
    )
    lines = [
        line.strip()
    ...
    if enable not in (0, 1) or latch not in (0, 1):
        raise ContractError("H18 auto-handoff status values are not binary")
    return f1.validate_h18_auto_status_record(
...
def require_status(
    ...
    *,
    ...
    if (status["enable"], status["latch"]) != (enable, latch):
        raise ContractError(
            f"H18 state is {status['enable']},{status['latch']}; "
...
def require_cancellation_fsync_proof(
    ...
    before = _exact_log_text(opening_log, "H18 cancellation opening log")
    after = _exact_log_text(final_log, "H18 cancellation final log")
    pattern = re.compile(
    ...
    if not after.startswith(before):
        raise ContractError("H18 cancellation log history is not an exact prefix")
    appended = after[len(before) :]
    before_matches = pattern.findall(before)
    appended_matches = pattern.findall(appended)
    if before_matches or len(appended_matches) != 1:
        raise ContractError(
            "H18 no-effect close lacks one post-intent cancellation fsync proof"
        )
    return {
        "proof": True,
...
def native_fallback_attribution(
    ...
    before = _exact_log_text(opening_log, "H18 attribution opening log")
    after = _exact_log_text(final_log, "H18 attribution final log")
    ...
    if not after.startswith(before):
        raise ContractError("H18 native log history is not an exact prefix")
    appended = after[len(before) :]
    diagnostics = list(diagnostic_pattern.finditer(appended))
    cleanups = list(cleanup_pattern.finditer(appended))
    if len(cleanups) != 1:
        raise ContractError("H18 native fallback cleanup evidence is not unique")
    cleanup = cleanups[0]
    if cleanup.groupdict() != {"clean": "1", "mounted": "0", "recovery": "0"}:
        raise ContractError("H18 native fallback cleanup is recovery-pending")
    if len(diagnostics) > 1:
        raise ContractError("H18 native fallback diagnostic is not unique")
    if not diagnostics:
        return {
            "proof": False,
            "status": "NO_PROOF_H18_FAILURE_ATTRIBUTION",
    ...
    if rc >= 0 or error != -rc or diagnostic.start() >= cleanup.start():
        raise ContractError("H18 native fallback diagnostic facts changed")
    stage = facts["stage"]
...
def _expected_h18_state(intent_sha256: str, state: str) -> bytes:
    if f1.HEX64_RE.fullmatch(intent_sha256) is None:
        raise ContractError("H18 state intent SHA256 is invalid")
    if state not in {
        "armed-after-native-health",
        "automatic-handoff-dispatched-no-replay",
    }:
        raise ContractError("H18 state name is invalid")
    lines = (
...
def _same_intent_script() -> str:
    return "\n".join(
        (
            ...
            'echo "A90H18_INTENT_BINDING intent=$EINT enable_sha256=$ES '
...
def require_same_intent_state(
    ...
    exact = base.require_exact_f1_command_receipt(
        record,
        command,
        "H18 exact enable/latch/evidence intent binding",
    )
    lines = [
        line.strip()
        for line in str(exact.get("text") or "").replace("\r", "").splitlines()
    ...
    expected = {
        ...
        "enable": hashlib.sha256(
            _expected_h18_state(intent_sha256, "armed-after-native-health")
        ).hexdigest(),
    ...
    if match is None or match.groupdict() != expected:
        raise ContractError("H18 enable/latch/evidence intent binding changed")
    return {
        "proof": True,
        "intent_sha256": intent_sha256,
...
def _write_record(
    ...
    if index >= len(JOURNAL_NAMES):
        raise ContractError("H18 D1 journal overflow")
    value = {
        "schema": SCHEMA,
        "sequence": index,
        "action": action,
    ...
    for index, name in enumerate(JOURNAL_NAMES):
        ...
        if not path.exists():
            if any((transaction_dir / later).exists() for later in JOURNAL_NAMES[index + 1 :]):
                raise ContractError("H18 D1 journal has a gap")
            break
        ...
        if (
            ...
            raise ContractError("H18 D1 journal record changed")
...
def approval_binding(
    ...
    *,
    ...
    return {
        ...
        "workflow": "A90_H18_ATTENDED_PERSISTENT_SERVER_D1_V1",
...
def validate_approval(
    ...
    if not path.is_file() or path.is_symlink() or info.st_mode & 0o077:
        raise ContractError("H18 D1 approval is not a private regular file")
    value = json.loads(path.read_text(encoding="utf-8"))
    binding = value.get("approval_binding") if isinstance(value, dict) else None
    if not isinstance(binding, dict):
        raise ContractError("H18 D1 approval binding is absent")
    ...
    if (
        ...
        raise ContractError("H18 D1 approval is not fresh and exact")
    return value
...
def _valid_same_intent(value: Any, intent_sha256: str) -> bool:
    ...
    try:
        base.require_exact_f1_command_receipt(
            ...
            command,
            "durable H18 same-intent binding",
        )
    except Exception:
        return False
    return bool(
...
def _valid_unmounted_userdata(value: Any) -> bool:
    ...
    try:
        base.require_exact_f1_command_receipt(
            value.get("record") if isinstance(value, dict) else None,
            command,
            "durable H18 unmounted userdata",
        )
    except Exception:
        return False
    return bool(
...
def _valid_auto_handoff_11(result: dict[str, Any]) -> bool:
    ...
    if status != {
        ...
        return False
    try:
        f1.validate_h18_auto_status_record(
...
def _valid_no_effect_result(
    ...
    try:
        ...
        return False
    return bool(
        result.get("status") == "ABORTED_H18_ARM_REBOOT_NO_PERSISTENT_EFFECT"
...
def _valid_physical_return_result(
    ...
    if not isinstance(result, dict) or not isinstance(current, dict):
        return False
    live_proven = current.get("status") == "PASS_A90_H18_PERSISTENT_SERVER_LIVE"
    expected_status = (
        "PASS_A90_H18_PERSISTENT_SERVER_PHYSICAL_RETURN_HEALTHY"
        if live_proven
        else "NO_PROOF_A90_H18_PERSISTENT_SERVER_PHYSICAL_RETURN_HEALTHY"
    )
    return bool(
...
def _valid_native_fallback_result(
    ...
    *,
    ...
    if attribution_common and attribution.get("proof") is False:
        attribution_exact = (
            attribution.get("status") == "NO_PROOF_H18_FAILURE_ATTRIBUTION"
        ...
        attribution_exact = bool(
            attribution.get("status") == "PROVED_H18_FAILURE_ATTRIBUTION"
...
def _validate_records(
    ...
    if (
        ...
        raise ContractError("H18 D1 opening binding changed")
    opening_log = opening.get("opening_log")
    if not isinstance(opening_log, dict):
        raise ContractError("H18 D1 opening log is absent")
    _exact_log_text(opening_log, "H18 D1 durable opening log")
    ...
    if approval_value != expected_approval:
        raise ContractError("H18 D1 consumed approval binding changed")
    if len(records) == 1:
        return None
    intent = records[1]
    if (
        ...
        raise ContractError("H18 D1 arm-reboot intent changed")
    ...
    if any(
        ...
        raise ContractError("H18 D1 continuation intent binding changed")
    if len(records) >= 3 and (
        ...
        raise ContractError("H18 D1 dispatch result changed")
    if len(records) >= 4 and (
        ...
        raise ContractError("H18 D1 persistent observation changed")
    if len(records) >= 5:
        ...
        if (
            ...
            raise ContractError("H18 D1 current-state result changed")
    if len(records) >= 6:
...
def _live_result(
    ...
    if server_proven and guard_released and visible_confirmed == "yes":
        status = "PASS_A90_H18_PERSISTENT_SERVER_LIVE"
    elif visible_confirmed == "no":
        status = "REFUTED_A90_H18_PERSISTENT_SERVER_DISPLAY"
    else:
        status = "NO_PROOF_A90_H18_PERSISTENT_SERVER_LIVE"
    return {
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H18 automatic UFS handoff is attended-only")
...
def _unmounted_script() -> str:
    return "\n".join(
        (
            ...
            '[ "$C" = 0 ]',
            'echo "A90H18_POST_PHYSICAL_RETURN devt=$MAJ:$MIN '
...
def _prove_userdata_unmounted(effect_args: argparse.Namespace) -> dict[str, Any]:
    ...
    exact = base.require_exact_f1_command_receipt(
        record,
        command,
        "H18 post-physical-return unmounted userdata",
    )
    lines = [
        line.strip()
        for line in str(exact.get("text") or "").replace("\r", "").splitlines()
    ...
    if match is None or int(match.group("count"), 10) != 0:
        raise ContractError("H18 userdata is not one exact unmounted identity")
...
def finalize_physical_return(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ContractError(
            "H18 physical-return finalization requires attended confirmation"
        )
    ...
    live_proven = live_result.get("status") == "PASS_A90_H18_PERSISTENT_SERVER_LIVE"
    result = {
        "schema": RESULT_SCHEMA,
        "status": (
            "PASS_A90_H18_PERSISTENT_SERVER_PHYSICAL_RETURN_HEALTHY"
...
def finalize_no_effect(args: argparse.Namespace) -> dict[str, Any]:
    if args.operator_attended is not True:
        raise ContractError("H18 no-effect finalization is attended-only")
    manifest, spec, _ = load_inputs(args)
    transaction_dir = _require_transaction_dir(
        manifest,
        args.transaction_dir,
    ...
    if len(records) >= 5 and (
        ...
        != "ABORTED_H18_ARM_REBOOT_NO_PERSISTENT_EFFECT"
    ):
    ...
    current = {
        "schema": RESULT_SCHEMA,
        "status": "ABORTED_H18_ARM_REBOOT_NO_PERSISTENT_EFFECT",
        "intent_sha256": intent_sha256,
        "device_safety_state": "RESIDENT_HEALTHY",
        "resident_healthy": True,
        "ordinal_closed": False,
...
def _native_fallback_status(attribution: dict[str, Any], *, final: bool) -> str:
    ...
    if attribution.get("proof") is not True:
        return f"NO_PROOF_H18_FAILURE_ATTRIBUTION_NATIVE_FALLBACK_{suffix}"
    if attribution.get("incident_window_match") is True:
        return (
            "REFUTED_H18_POST_ROOT_FAILURE_ATTRIBUTED_"
            f"NATIVE_FALLBACK_{suffix}"
        )
    return f"REFUTED_H18_OTHER_FAILURE_ATTRIBUTED_NATIVE_FALLBACK_{suffix}"


def finalize_native_fallback(args: argparse.Namespace) -> dict[str, Any]:
    """Close one returned H18 diagnostic handoff without replaying its effect."""
    if args.operator_attended is not True:
        raise ContractError("H18 native-fallback finalization is attended-only")
    manifest, spec, _ = load_inputs(args)
    transaction_dir = _require_transaction_dir(
        manifest,
        args.transaction_dir,
    ...
    try:
        if args.prepare_approval:
            ...
            if args.approval is None:
                raise ContractError("H18 D1 execute requires fresh exact approval")
        ...
        print(f"H18_UFS_D1_ERROR {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
