#!/usr/bin/env python3
"""Bounded authenticated observer for the live A90 H17 Debian server.
...
the H17 persistent-server result.
"""
...
SCHEMA = "a90-h17-persistent-server-observer-v1"
HOST = "192.168.7.2"
PORT = 2222
BEGIN = "A90H17_OBSERVER_BEGIN"
END = "A90H17_OBSERVER_END"
...
class ObserverError(RuntimeError):
    """Raised when H17 live facts are absent, ambiguous, or malformed."""
...
def exact_section(text: str) -> str:
    ...
    if lines.count(BEGIN) != 1 or lines.count(END) != 1:
        raise ObserverError("H17 observer transcript markers are not unique")
    begin = lines.index(BEGIN)
    end = lines.index(END)
    if end <= begin:
        raise ObserverError("H17 observer transcript marker order is invalid")
...
def parse_facts(text: str) -> dict[str, str]:
    ...
    for line in section.splitlines():
        if "=" not in line:
            raise ObserverError("H17 observer fact line is malformed")
        key, value = line.split("=", 1)
        if KEY_RE.fullmatch(key) is None or key in facts:
            raise ObserverError("H17 observer fact keys are not exact")
        facts[key] = value
    if set(facts) != EXPECTED_KEYS:
        raise ObserverError("H17 observer fact set is not exact")
    return facts


def classify(text: str, returncode: int, visible_hud: bool) -> dict[str, Any]:
    if returncode != 0:
        raise ObserverError("authenticated H17 SSH observation failed")
    ...
    return {
        "schema": SCHEMA,
        "status": "PASS_A90_H17_PERSISTENT_SERVER_LIVE" if proof else "NO_PROOF_A90_H17_PERSISTENT_SERVER",
        "proof": proof,
...
def observe(
    ...
    *,
    ...
    last: dict[str, Any] = {
        "schema": SCHEMA,
        "status": "NO_PROOF_A90_H17_PERSISTENT_SERVER",
        "proof": False,
        "device_safety": "HEALTH_PENDING_PERSISTENT_DEBIAN",
        "new_device_effect_authority": False,
        "automatic_native_return_expected": False,
