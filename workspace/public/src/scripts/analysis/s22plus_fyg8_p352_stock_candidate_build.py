...
_source = _source.replace(b'outputs/s22plus_fyg8_p352/h0-work/executability-final.json', b'outputs/s22plus_fyg8_p351/h0-work/executability-final.json')
...
_p352_source_files = source_files

def source_files():
    result = _p352_source_files()
...
_p352_audit_existing = audit_existing


def audit_existing(output_root=DEFAULT_OUTPUT_ROOT):
    result = _p352_audit_existing(output_root)
    if result.get('display_kms_contract') != kms_identity():
        raise AuditError('P352 KMS build join differs')
    return result
