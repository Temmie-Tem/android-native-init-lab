#!/usr/bin/env python3
"""Prepare the P3.34 Process-v2 bundle in a host-only rehearsal."""
...
import s22plus_fyg8_p334_first_read_rc_acm_observer as framed_observer  # noqa: E402
import s22plus_fyg8_p334_first_read_rc_runtime as framed_runtime  # noqa: E402
import s22plus_fyg8_p334_process_v2_candidate_static as candidate_static  # noqa: E402
...
SCHEMA = "s22plus_fyg8_p334_process_v2_promotion_v1"
VERDICT = "PASS_P334_PROCESS_V2_PROMOTION_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p334_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P334_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P334_PROCESS_V2_READY_MANIFEST_REHEARSAL_HOST_ONLY"
...
DEFAULT_PROMOTION = ROOT / (
    ...
    "process-v2-promotion-20260904-01"
)
...
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p334-process-v2-ready-1"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p334-live-1"
DEFAULT_TIMEOUT_SEC = 300
PRIVATE_PARENT = ROOT / "workspace/private/outputs/s22plus_fyg8_p334"
AUTH_KEY_IDENTITY = dict(evidence.P334_AUTH_EXEC_AUTH_KEY_IDENTITY)


class PromotionError(ValueError):
    """The exact P3.34 promotion closure or manifest is invalid."""
...
def _load() -> types.ModuleType:
    ...
    module = types.ModuleType("prepare_s22plus_fyg8_p330_bound_for_p334")
...
class _EvidenceProxy:
    P328_RUN_MANIFEST_SCHEMA = evidence.P334_RUN_MANIFEST_SCHEMA
    P328_STATIC_RESULT_SCHEMA = evidence.P334_STATIC_RESULT_SCHEMA
    P328_STATIC_RESULT_VERDICT = evidence.P334_STATIC_RESULT_VERDICT
    ...
    def p328_authenticated_framed_observer_spec() -> dict[str, Any]:
        return evidence.p334_authenticated_logical_resident_observer_spec()
...
def verify_manifest(
    ...
    *,
    ...
    if payload != _INNER.manifest_bytes(value):
        raise PromotionError("P3.34 ready manifest bytes differ")
    private_root = ROOT / "workspace/private"
    try:
        private_root.lstat()
    except OSError as exc:
        raise PromotionError("P3.34 private workspace is unavailable") from exc
    with tempfile.TemporaryDirectory(prefix=".p334-ready-", dir=private_root) as name:
        temporary = Path(name)
        staged_value = value
