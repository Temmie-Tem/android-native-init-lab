#!/usr/bin/env python3
"""Build the P3.37 boot-only open-read-diagnostic candidate (host-only).

The exact P3.36 builder is loaded into a private module graph and retained as
the packaging engine.  P3.37 reopens only the verified P3.36 result/source
closure, transforms its one runtime include through the P3.37 runtime, and
binds the P3.37 observer.  The package remains A/B-equal and boot-only; this
...
P336_BUILDER_SOURCE = ANALYSIS / "s22plus_fyg8_p336_stock_candidate_build.py"
P336_BUILDER_IDENTITY = {
    "size": 28_118,
    "sha256": "d926685bb143790ae557f684eb5ccd090275e85d56d991c069b9c08a63ddeff5",
}
P336_OUTPUT = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p336/"
    "stock-candidate-build-v1-20260904-09"
)
P336_RESULT = P336_OUTPUT / "result.json"
P336_RESULT_IDENTITY = {
    "size": 56_894,
...
P331_RUN_ID_HEX = P337_RUN_ID_HEX
P331_RUN_ID = P337_RUN_ID
P330_RUN_ID_HEX = P337_RUN_ID_HEX
P330_RUN_ID = P337_RUN_ID
P328_RUN_ID_HEX = P337_RUN_ID_HEX
P328_RUN_ID = P337_RUN_ID
P336_AP_IDENTITY = {
    "size": 28_631_081,
    "sha256": "d19f978109100f3c87a5ca92dd2aaf0aaf8af0e228f0eebb6d7716c95008361b",
}
P334_AP_IDENTITY = {
    "size": 28_631_081,
...
P337_RUNTIME_SOURCE = REVALIDATION / "s22plus_fyg8_p337_open_read_diag_runtime.py"
P337_OBSERVER_SOURCE = REVALIDATION / "s22plus_fyg8_p337_open_read_diag_acm_observer.py"
P337_ACTION_SOURCE = REVALIDATION / "s22plus_fyg8_p336_long_idle_action.py"
P337_RUNTIME_SOURCE_IDENTITY = dict(runtime.identity(P337_RUNTIME_SOURCE.read_bytes()))
P337_OBSERVER_SOURCE_IDENTITY = dict(observer.identity(P337_OBSERVER_SOURCE.read_bytes()))
P337_ACTION_SOURCE_IDENTITY = {
    "size": P337_ACTION_SOURCE.stat().st_size,
    "sha256": hashlib.sha256(P337_ACTION_SOURCE.read_bytes()).hexdigest(),
}
# Compatibility labels consumed by the exact-loaded P3.36 static seam.
P336_RUNTIME_SOURCE_IDENTITY = dict(P337_RUNTIME_SOURCE_IDENTITY)
P336_OBSERVER_SOURCE_IDENTITY = dict(P337_OBSERVER_SOURCE_IDENTITY)
P335_RUNTIME_SOURCE_IDENTITY = dict(P337_RUNTIME_SOURCE_IDENTITY)
P335_OBSERVER_SOURCE_IDENTITY = dict(P337_OBSERVER_SOURCE_IDENTITY)
...
SCHEMA = "s22plus-fyg8-p337-stock-candidate-build-v1"
VERDICT = "PASS_P337_STOCK_CANDIDATE_BUILD_H0_OPEN_READ_DIAGNOSTIC"
STATUS = "IMPLEMENTED_H0_OPEN_READ_DIAGNOSTIC_REVIEW_PENDING"
TARGET = {"model": "SM-S906N", "codename": "g0q", "build": "S906NKSS7FYG8"}


class AuditError(RuntimeError):
    """The exact P3.36 lineage or P3.37 closure differs."""
...
def _load_p336_builder() -> types.ModuleType:
    payload = _stable(
        P336_BUILDER_SOURCE,
        "P3.36 builder source",
        2 << 20,
        P336_BUILDER_IDENTITY,
    )
    module = types.ModuleType("s22plus_fyg8_p336_builder_bound_for_p337")
    module.__file__ = str(P336_BUILDER_SOURCE)
    module.__package__ = ""
    aliases = {
        "s22plus_fyg8_p336_artifact_identity": artifact,
        "s22plus_fyg8_p336_stock_process_v2_adapter": adapter,
        "s22plus_fyg8_p336_long_idle_runtime": runtime,
        "s22plus_fyg8_p336_long_idle_acm_observer": observer,
    ...
    try:
        ...
        exec(compile(payload, str(P336_BUILDER_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
    except Exception as exc:
        raise AuditError("P3.36 builder source failed to load") from exc
    finally:
    ...
    if getattr(module, "P336_RUN_ID_HEX", None) != P336_RUN_ID_HEX:
        raise AuditError("P3.36 builder binding differs")
    return module


_P336 = _load_p336_builder()
_P328 = _P336._P328
_ENGINE = _P336._ENGINE
_P336_LOAD_PACKAGER = _P336._load_packager
...
def _rebind_engine() -> None:
    ...
    for module in modules:
        for name, value in tuple(vars(module).items()):
            if value == P336_RUN_ID:
                setattr(module, name, P337_RUN_ID)
            elif value == P336_RUN_ID_HEX:
                ...
                for key, current in tuple(defaults.items()):
                    if current == P336_RUN_ID:
                        defaults[key] = P337_RUN_ID
                        changed = True
                    elif current == P336_RUN_ID_HEX:
    ...
    _ENGINE.P321_OUTPUT = P336_OUTPUT
    _ENGINE.P322_RUN_ID = P337_RUN_ID
    _ENGINE.P322_RUN_ID_HEX = P337_RUN_ID_HEX
    _ENGINE.P322_ARTIFACT_SOURCE = artifact.P337_ARTIFACT_SOURCE
    _ENGINE.P322_REPAIR_SOURCE = P337_RUNTIME_SOURCE
    _ENGINE.P322_ADAPTER_SOURCE = adapter.P337_ADAPTER_SOURCE
...
def _predecessor() -> tuple[dict[str, Any], bytes, bytes]:
    ...
    value = _strict_json(payload, "P3.36 result")
    candidate = value.get("phase2", {}).get("candidate")
    if (
        value.get("schema") != "s22plus-fyg8-p336-stock-candidate-build-v1"
        or value.get("run_id_hex") != P336_RUN_ID_HEX
        or value.get("target") != TARGET
        or not isinstance(candidate, dict)
        or candidate.get("a") != candidate.get("b")
        or candidate.get("a", {}).get("ap_tar_md5") != P336_AP_IDENTITY
        ...
        raise AuditError("P3.36 predecessor result differs")
    for name, expected in sorted(value["source_closure"].items()):
        _stable(
            P336_OUTPUT / "stock-sources" / name,
            f"P3.36 source {name}",
            2 << 20,
            expected,
            mode=0o400,
    ...
    for label in ("a", "b"):
        _stable(
            P336_OUTPUT / f"candidate-{label}/odin4/AP.tar.md5",
            f"P3.36 candidate {label}",
            128 << 20,
            P336_AP_IDENTITY,
        )
    source = _stable(
...
def _current_sources() -> dict[str, bytes]:
    paths = {
        ...
        "p328_artifact_identity.py": artifact.P337_ARTIFACT_SOURCE,
        "p328_stock_process_v2_adapter.py": adapter.P337_ADAPTER_SOURCE,
        "p328_authenticated_exec_runtime.py": P337_RUNTIME_SOURCE,
        "p328_authenticated_acm_observer.py": P337_OBSERVER_SOURCE,
    }
    expected = {
        "p328_authenticated_exec_runtime.py": P337_RUNTIME_SOURCE_IDENTITY,
        "p328_authenticated_acm_observer.py": P337_OBSERVER_SOURCE_IDENTITY,
    }
    return {
        name: _stable(path, f"P3.37 source {name}", 2 << 20, expected.get(name))
...
def _active_key() -> bytes:
    ...
    if type(key) is not bytes or len(key) != artifact.AUTH_KEY_SIZE:
        raise AuditError("P3.37 auth key is not bound")
    return key
...
def _runtime_transform(before: bytes, key: bytes) -> tuple[bytes, dict[str, Any]]:
    _stable(
        P337_RUNTIME_SOURCE,
        "P3.37 runtime source",
        2 << 20,
        P337_RUNTIME_SOURCE_IDENTITY,
    )
    ...
    receipt.update(
        {
            ...
            "input_lineage": "p336-runtime-include",
...
def _copy_source_closure(
    ...
    if repair is None:
        raise AuditError("P3.36 runtime source is absent")
    _ENGINE.p321._fsync_directory(source_root)
    return receipts, repair


def _load_packager() -> tuple[Any, Any, bytes]:
    helper, packager, source = _P336_LOAD_PACKAGER()
    packager.RUN_ID = P337_RUN_ID
...
def _normalize_result(value: dict[str, Any]) -> dict[str, Any]:
    try:
        result = copy.deepcopy(_P336._normalize_result(value))
    except Exception as exc:
        raise AuditError(f"P3.36 inherited result normalization failed: {exc}") from exc
    candidate = result.get("phase2", {}).get("candidate")
    if not isinstance(candidate, dict):
        raise AuditError("P3.37 candidate projection is absent")
    ...
    if (
        ...
        raise AuditError("P3.37 candidate repeats a consumed predecessor or differs A/B")
    for label in ("a", "b"):
        package = candidate[label]["package"]
        package.update(
            {
                "schema": "s22plus_fyg8_p337_boot_only_open_read_diagnostic_package_v1",
                "verdict": "PASS_P337_DETERMINISTIC_BOOT_ONLY_OPEN_READ_DIAGNOSTIC_H0",
            }
        )
    result.update(
        {
            ...
            "status": STATUS,
            "run_id_hex": P337_RUN_ID_HEX,
            "differs_from_consumed_p336": True,
        }
    )
    lineage = result.setdefault("lineage", {})
    lineage.update(
        {
            "construction_base_run_id": P336_RUN_ID_HEX,
            "construction_base_result": P336_RESULT_IDENTITY,
            "construction_base_ap": P336_AP_IDENTITY,
            "predecessor_run_id": P336_RUN_ID_HEX,
            "predecessor_result": P336_RESULT_IDENTITY,
            "predecessor_ap": P336_AP_IDENTITY,
            "fresh_run_id": P337_RUN_ID_HEX,
            "p336_source_closure_reopened": True,
        }
    )
    preservation = result.setdefault("preservation", {})
    preservation.update(
        {
            "p336_consumed_candidate_distinct": True,
            "p336_source_closure_reopened": True,
    ...
    result["compatibility_labels"] = {
        "engine": "P336 exact-loaded",
        "runtime_input": "exact P336 runtime include",
        "wire_namespace": "unchanged P336 authenticated framing and fixed command ABI",
    ...
    result["limitations"] = [
        "P337 reuses the exact P336 packaging engine and changes only the runtime include.",
        "Initial proof retains the P336 three-session same-FD lease and one host close/reopen.",
        "Successful P336 framing is unchanged; only first-OPEN failures emit one best-effort stage-3 diagnostic.",
...
for module in (_P336, _P328, _ENGINE):
...
def _validate_repair(before: bytes, after: bytes) -> dict[str, Any]:
    ...
    if expected != after:
        raise AuditError("P3.37 transformed runtime differs")
    return receipt


def _require_builder() -> None:
    if _P336 is None or _P328 is None or _ENGINE is None:
        raise AuditError("P3.37 builder dependencies are unavailable")
    adapter.audit()
