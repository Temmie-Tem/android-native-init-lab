#!/usr/bin/env python3
"""Prepare the P3.32 logical-resident F1 bundle (host-only rehearsal).

This wrapper reopens the exact P3.30 Process-v2 preparation engine and binds
the fresh P3.32 resident observer/runtime, candidate, and output identities.
...
import s22plus_fyg8_p332_logical_resident_acm_observer as framed_observer  # noqa: E402
import s22plus_fyg8_p332_logical_resident_exec_runtime as framed_runtime  # noqa: E402
import s22plus_fyg8_p332_process_v2_candidate_static as candidate_static  # noqa: E402
...
SCHEMA = "s22plus_fyg8_p332_process_v2_promotion_v1"
VERDICT = "PASS_P332_PROCESS_V2_PROMOTION_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p332_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P332_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P332_PROCESS_V2_READY_MANIFEST_REHEARSAL_HOST_ONLY"
DEFAULT_BUILDER_OUTPUT = ROOT / (
...
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p332-process-v2-ready-1"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p332-live-1"
DEFAULT_TIMEOUT_SEC = 300
PRIVATE_PARENT = ROOT / "workspace/private/outputs/s22plus_fyg8_p332"
AUTH_KEY_IDENTITY = dict(evidence.P332_AUTH_EXEC_AUTH_KEY_IDENTITY)


class PromotionError(ValueError):
    """The exact P3.32 promotion closure or manifest is not valid."""
...
def _load() -> types.ModuleType:
    ...
    module = types.ModuleType("prepare_s22plus_fyg8_p330_bound_for_p332")
    ...
    # loaded P3.32 modules while executing the exact pinned source.  This
...
class _EvidenceProxy:
    """Bind the inherited P3.30 engine to P3.32 evidence schemas and role."""

    P328_RUN_MANIFEST_SCHEMA = evidence.P332_RUN_MANIFEST_SCHEMA
    P328_STATIC_RESULT_SCHEMA = evidence.P332_STATIC_RESULT_SCHEMA
    P328_STATIC_RESULT_VERDICT = evidence.P332_STATIC_RESULT_VERDICT
    ...
    def p328_authenticated_framed_observer_spec() -> dict[str, Any]:
        return evidence.p332_authenticated_logical_resident_observer_spec()
...
def _promotion_payloads(
    ...
    # The P3.32 static projection intentionally keeps only the public key
...
def verify_manifest(
    ...
    *,
    ...
    """Recheck a manifest in a P3.32-only private temporary namespace."""

    if payload != _INNER.manifest_bytes(value):
        raise PromotionError("P3.32 ready manifest bytes differ")
    private_root = ROOT / "workspace/private"
    try:
        private_root.lstat()
    except OSError as exc:
        raise PromotionError("P3.32 private workspace is unavailable") from exc
    with tempfile.TemporaryDirectory(prefix=".p332-ready-", dir=private_root) as name:
        temporary = Path(name)
        staged_value = value
