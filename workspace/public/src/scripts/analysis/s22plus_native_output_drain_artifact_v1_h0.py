"""Qualify the drain successor; keep consumed UFS exporter receipts immutable."""
...
def qualify(namespace,output,*,key_path,build_directory=None):
    ...
    require(namespace in catalog.DECLARATIONS,'UFS build declaration is absent')
    selected=catalog.DECLARATIONS[namespace]
    require(catalog.research_profile(selected)=='thermal-v3-reconnect-ufs-drain-v1','runtime is outside UFS scope')
    ...
    require(built['verdict']=='PASS_NATIVE_THERMAL_BUILD_H0' and built['byte_identical'] is True
        and built['candidate']['a']==built['candidate']['b'],'UFS A/B producer did not qualify')
    key=pin(private_path(ROOT,key_path),maximum=32)
    require({k:key[k] for k in ('size','sha256')}==selected.artifact.auth_key_identity(),
        'UFS key does not match the audited producer')
    ap=dict(path=str(builder.DEFAULT_OUTPUT_ROOT/'candidate-a/odin4/AP.tar.md5'),
        **built['candidate']['a']['ap_tar_md5'])
    with transport.pin_boot_only_ap(Path(ap['path']),label='UFS qualification',
            expected_size=ap['size'],expected_sha256=ap['sha256']) as bound:
        member=transport.boot_only_member_receipt(bound,label='UFS qualification')
    output=private_path(ROOT,output,exists=False);require(not output.exists(),'UFS qualification already exists')
    output.mkdir(mode=0o700)
    image=dict(schema='s22plus-native-image-v3',namespace=namespace,run_id_hex=selected.IDENTITY.run_id_hex,
        profile='thermal-v3-reconnect-ufs-drain-v1',version=selected.IDENTITY.display_version,
        ap=ap,member=member,key=key,runtime_sources=built['source_inputs'])
