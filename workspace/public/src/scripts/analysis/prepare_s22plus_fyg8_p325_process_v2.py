#!/usr/bin/env python3
"""Prepare the P3.25 static promotion and ready manifest, host-only."""
...
import s22plus_fyg8_p325_process_v2_candidate_static as p325_static  # noqa: E402
import s22plus_fyg8_p325_stock_candidate_build as p325_build  # noqa: E402
import s22plus_fyg8_p325_stock_process_v2_adapter as p325_adapter  # noqa: E402


P324_PREPARE_SOURCE = ANALYSIS / "prepare_s22plus_fyg8_p324_process_v2.py"
P324_PREPARE_IDENTITY = {
    "size": 14_117,
    "sha256": "62c3c330dde26bb33d2f7d1c988784ba400224bcc76788a440bfb0a8bdcbb154",
}

SCHEMA = "s22plus_fyg8_p325_process_v2_promotion_v1"
VERDICT = "PASS_P325_PROCESS_V2_PROMOTION_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p325_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P325_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P325_PROCESS_V2_READY_MANIFEST_REHEARSAL_HOST_ONLY"
DEFAULT_BUILDER_OUTPUT = p325_build.DEFAULT_OUTPUT_ROOT
DEFAULT_STATIC_OUTPUT = p325_static.DEFAULT_OUTPUT
...
DEFAULT_PROMOTION = ROOT / (
    ...
    "process-v2-promotion-20260901-03"
)
DEFAULT_MANIFEST = ROOT / (
    "workspace/public/src/device-action/manifests/"
    "s22plus_fyg8_p325_process_v2_ready_2.json"
)
DEFAULT_TARGET_PROFILE = ROOT / "workspace/public/src/device-action/profiles/s22plus_fyg8.json"
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p325-process-v2-ready-2"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p325-live-2"
DEFAULT_TIMEOUT_SEC = 300
ROLLBACK_IDENTITY = dict(p325_static.ROLLBACK_IDENTITY)
TARGET = dict(p325_static.TARGET)
PRIVATE_PARENT = ROOT / "workspace/private/outputs/s22plus_fyg8_p325"


class WrapperError(ValueError):
    """The exact P324 prepare delegate or P325 binding differs."""
...
def _stable_source(path: Path, expected: dict[str, Any]) -> bytes:
    ...
    try:
        ...
        raise WrapperError("P324 prepare source is unavailable") from exc
    ...
    if (
        ...
        raise WrapperError("P324 prepare source identity differs")
    return payload
...
def _load_delegate() -> tuple[types.ModuleType, types.ModuleType]:
    payload = _stable_source(P324_PREPARE_SOURCE, P324_PREPARE_IDENTITY)
    module = types.ModuleType("prepare_s22plus_fyg8_p324_bound_for_p325")
    module.__file__ = str(P324_PREPARE_SOURCE)
    module.__package__ = ""
    try:
        exec(  # noqa: S102
            compile(payload, str(P324_PREPARE_SOURCE), "exec", dont_inherit=True),
            module.__dict__,
        )
    except Exception as exc:
        raise WrapperError("P324 prepare source failed to load") from exc
    base = module._BASE
    build_proxy = _proxy(
        p325_build,
    ...
    adapter_proxy = _proxy(
        p325_adapter,
        P321_RUN_ID_HEX=p325_adapter.P325_RUN_ID_HEX,
        P321_OVERLAY_CONTRACT_ID=p325_adapter.P325_OVERLAY_CONTRACT_ID,
    )
    evidence_proxy = _proxy(
        current_evidence,
        P321_RUN_MANIFEST_SCHEMA=current_evidence.P325_RUN_MANIFEST_SCHEMA,
        P321_STATIC_RESULT_SCHEMA=current_evidence.P325_STATIC_RESULT_SCHEMA,
        P321_STATIC_RESULT_VERDICT=current_evidence.P325_STATIC_RESULT_VERDICT,
    )
    base.candidate_static = p325_static
...
_P324, _BASE = _load_delegate()
PromotionError = _BASE.PromotionError
adapter = p325_adapter
...
_BASE_PROMOTION_PAYLOADS = _P324._BASE_PROMOTION_PAYLOADS
_BASE_DERIVE_MANIFEST = _P324._BASE_DERIVE_MANIFEST
_BASE_BUILD = _P324._BASE_BUILD
...
def _promotion_payloads(
    ...
    run_manifest = decode(payloads["run_manifest"], "P325 run manifest")
    observation = run_manifest.get("observation_contract")
    if (
        not isinstance(observation, dict)
        or observation.get("accepted_identity") != "P321_STOCK_OBSERVER_V4_RETAINED"
    ):
        raise PromotionError("P321 promotion seam differs")
    observation["accepted_identity"] = "P325_STOCK_OBSERVER_V4_RETAINED"
    run_payload = canonical(run_manifest)
    static_result = decode(payloads["static_check"], "P325 static result")
...
def derive_manifest(**kwargs: Any) -> dict[str, Any]:
    ...
    run_id = p325_adapter.P325_RUN_ID_HEX
...
def verify_manifest(
    ...
    *,
    ...
    if payload != manifest_bytes(value):
        raise PromotionError("P325 ready manifest bytes differ")
    staged_value = value
    PRIVATE_PARENT.mkdir(mode=0o700, parents=True, exist_ok=True)
    PRIVATE_PARENT.chmod(0o700)
    with tempfile.TemporaryDirectory(prefix=".p325-ready-", dir=PRIVATE_PARENT) as name:
        temporary = Path(name)
