#!/usr/bin/env python3
"""Publish the P3.37 host-only candidate-static receipt.

This validator reopens the fresh P3.37 builder result and exact-loads the
reviewed P3.36 static seam only for canonical JSON/no-clobber publication.
The changed closure is explicit: P337 artifact identity, adapter, runtime,
observer, builder, the P336 static predecessor, and the unchanged exact rollback.
The consumed P336 later-action lane is intentionally absent. No device, ADB, Odin, approval, transfer,
...
import s22plus_fyg8_p337_stock_candidate_build as candidate_build  # noqa: E402
import s22plus_fyg8_p337_stock_process_v2_adapter as adapter  # noqa: E402


SELF_SOURCE = Path(__file__).resolve()
P336_STATIC_SOURCE = ANALYSIS / "s22plus_fyg8_p336_process_v2_candidate_static.py"
P336_STATIC_SOURCE_IDENTITY = {
    "size": 20_374,
    "sha256": "3fdfaafeeea7f69afd4d0e1fdbb2021e3c8ded8f342163122b699c61cd6db675",
}
...
SCHEMA = "s22plus_fyg8_p337_process_v2_candidate_static_v1"
VERDICT = "PASS_P337_PROCESS_V2_CANDIDATE_STATIC_HOST_ONLY"
...
class StaticContractError(ValueError):
    """The exact P3.37 static closure is incomplete or changed."""
...
def _load_p336_static() -> types.ModuleType:
    payload = _stable(
        P336_STATIC_SOURCE,
        "P3.36 candidate-static source",
        2 << 20,
        P336_STATIC_SOURCE_IDENTITY,
    )
    module = types.ModuleType("s22plus_fyg8_p336_static_bound_for_p337")
    module.__file__ = str(P336_STATIC_SOURCE)
    module.__package__ = ""
    aliases = {
        "s22plus_fyg8_p336_artifact_identity": artifact,
        "s22plus_fyg8_p336_stock_candidate_build": candidate_build,
    ...
    try:
        ...
        exec(compile(payload, str(P336_STATIC_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
    except Exception as exc:
        raise StaticContractError("P3.36 static source failed to load") from exc
    finally:
...
_P336 = _load_p336_static()
canonical = _P336.canonical
publish = _P336.publish
...
def _observer_projection() -> dict[str, Any]:
    ...
    value.update(
        {
            "source": {
                "path": _relative(P337_OBSERVER_SOURCE),
                **P337_OBSERVER_SOURCE_IDENTITY,
            },
            "runtime_source": {
                "path": _relative(P337_RUNTIME_SOURCE),
...
P337_RUNTIME_SOURCE = Path(runtime.__file__).resolve()
P337_OBSERVER_SOURCE = Path(observer.__file__).resolve()
...
def _candidate_projection(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(phase2, dict):
        raise StaticContractError("P3.37 phase-2 projection is absent")
    ...
    if not isinstance(userspace, dict) or not isinstance(candidate, dict):
        raise StaticContractError("P3.37 candidate/userspace projection is absent")
    a = userspace.get("a")
    if not isinstance(a, dict) or not isinstance(a.get("init"), dict) or not isinstance(a.get("child"), dict):
        raise StaticContractError("P3.37 userspace identities are absent")
...
def _validate_candidate(value: dict[str, Any], built: dict[str, Any]) -> None:
    ...
    if not isinstance(candidate, dict):
        raise StaticContractError("P3.37 candidate is absent")
    ...
    if (
        ...
        != "s22plus_fyg8_p337_boot_only_open_read_diagnostic_package_v1"
    ):
        raise StaticContractError("P3.37 candidate identity differs")
...
def _validate_runtime(value: dict[str, Any], built: dict[str, Any]) -> None:
    ...
    if not isinstance(repair, dict) or repair != expected:
        raise StaticContractError("P3.37 runtime repair receipt differs")
    checks = {
    ...
    for key, expected_value in checks.items():
        if repair.get(key) != expected_value:
            raise StaticContractError(f"P3.37 runtime field differs: {key}")


def _validate_safety(value: dict[str, Any]) -> None:
    safety = value.get("safety")
    if not isinstance(safety, dict) or safety.get("host_only") is not True:
        raise StaticContractError("P3.37 static authority projection differs")
    for name in (
        "device_contact",
        "device_write",
        "odin_invoked",
        ...
        if safety.get(name) is not False:
            raise StaticContractError(f"P3.37 safety flag differs: {name}")


def _project(built: dict[str, Any]) -> dict[str, Any]:
    _stable(BUILDER_RESULT, "P3.37 builder result", 4 << 20, BUILDER_RESULT_IDENTITY, mode=0o400)
    ...
    source_closure = {
        ...
        "p337_artifact_identity": _source_receipt(
            Path(artifact.__file__).resolve(), "P3.37 artifact identity"
        ),
        "p337_open_read_diag_acm_observer": _source_receipt(
            P337_OBSERVER_SOURCE, "P3.37 observer", 2 << 20, P337_OBSERVER_SOURCE_IDENTITY
        ),
        "p337_open_read_diag_runtime": _source_receipt(
            P337_RUNTIME_SOURCE, "P3.37 runtime", 2 << 20, P337_RUNTIME_SOURCE_IDENTITY
        ),
        "p337_stock_candidate_build": _source_receipt(
            Path(candidate_build.__file__).resolve(), "P3.37 builder"
        ),
        "p337_stock_process_v2_adapter": _source_receipt(
            Path(adapter.__file__).resolve(), "P3.37 adapter"
        ),
        "p336_candidate_static": _source_receipt(
            P336_STATIC_SOURCE, "P3.36 candidate static", 2 << 20, P336_STATIC_SOURCE_IDENTITY
        ),
        "rollback_ap": _source_receipt(ROLLBACK_AP, "P3.37 exact rollback AP", 64 << 20, ROLLBACK_IDENTITY),
    }
    value = {
        "schema": SCHEMA,
        "verdict": VERDICT,
...
def build_result() -> dict[str, Any]:
    try:
        ...
        if built.get("schema") != "s22plus-fyg8-p337-stock-candidate-build-v1" or built.get("run_id_hex") != RUN_ID:
            raise StaticContractError("P3.37 builder header differs")
        return _project(built)
    except StaticContractError:
        raise
    except Exception as exc:
        raise StaticContractError(f"P3.37 static result did not regenerate: {exc}") from exc


def validate_result(value: Any) -> dict[str, Any]:
    expected = build_result()
    if type(value) is not dict or value != expected:
        raise StaticContractError("P3.37 static result does not regenerate")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.audit_only:
            stored = _stable(output, "P3.37 stored static result", 4 << 20)
            if stored != payload:
                raise StaticContractError("P3.37 stored static result differs")
        else:
