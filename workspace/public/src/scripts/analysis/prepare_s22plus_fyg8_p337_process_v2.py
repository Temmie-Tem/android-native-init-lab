#!/usr/bin/env python3
"""Prepare the P3.37 Process-v2 bundle in a host-only rehearsal.

The reviewed P3.36 preparation seam is exact-loaded into an isolated module
graph.  This wrapper changes only the P337 source/result/static identities and
...
import s22plus_fyg8_p337_open_read_diag_acm_observer as framed_observer  # noqa: E402
import s22plus_fyg8_p337_open_read_diag_runtime as framed_runtime  # noqa: E402
import s22plus_fyg8_p337_process_v2_candidate_static as candidate_static  # noqa: E402
...
P336_PREPARE_SOURCE = ANALYSIS / "prepare_s22plus_fyg8_p336_process_v2.py"
P336_PREPARE_IDENTITY = {
    "size": 13_587,
    "sha256": "cbaecbf9f2ac214f2dc681e58d9e695906aefd4633ddc2893a6cf1c9f814e1bc",
}
SCHEMA = "s22plus_fyg8_p337_process_v2_promotion_v1"
VERDICT = "PASS_P337_PROCESS_V2_PROMOTION_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p337_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P337_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P337_PROCESS_V2_READY_MANIFEST_REHEARSAL_HOST_ONLY"
...
DEFAULT_PROMOTION = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p337/process-v2-promotion-20260904-07"
)
DEFAULT_MANIFEST = ROOT / "workspace/public/src/device-action/manifests/s22plus_fyg8_p337_process_v2_ready_7.json"
DEFAULT_TARGET_PROFILE = ROOT / "workspace/public/src/device-action/profiles/s22plus_fyg8.json"
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p337-process-v2-ready-7"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p337-live-7"
DEFAULT_TIMEOUT_SEC = 300
PRIVATE_PARENT = ROOT / "workspace/private/outputs/s22plus_fyg8_p337"
...
class PromotionError(ValueError):
    """The exact P3.37 promotion closure or manifest is invalid."""
...
def _stable_source() -> bytes:
    direct = P336_PREPARE_SOURCE.absolute()
    try:
        before = direct.lstat()
        resolved = direct.resolve(strict=True)
        with direct.open("rb") as stream:
            payload = stream.read(P336_PREPARE_IDENTITY["size"] + 1)
            inside = __import__("os").fstat(stream.fileno())
        after = direct.lstat()
    except OSError as exc:
        raise PromotionError("P3.36 prepare source is unavailable") from exc
    if (
        direct != resolved
        ...
        or identity(payload) != P336_PREPARE_IDENTITY
    ):
        raise PromotionError("P3.36 prepare source identity differs")
    return payload


def _load_p336() -> types.ModuleType:
    payload = _stable_source()
    module = types.ModuleType("prepare_s22plus_fyg8_p336_bound_for_p337")
    module.__file__ = str(P336_PREPARE_SOURCE)
    module.__package__ = ""
    aliases = {
        "s22plus_fyg8_p336_artifact_identity": artifact,
        "s22plus_fyg8_p336_long_idle_acm_observer": framed_observer,
        "s22plus_fyg8_p336_long_idle_runtime": framed_runtime,
        "s22plus_fyg8_p336_first_read_rc_acm_observer": framed_observer,
        "s22plus_fyg8_p336_first_read_rc_runtime": framed_runtime,
        "s22plus_fyg8_p336_process_v2_candidate_static": candidate_static,
    ...
    try:
        ...
        exec(compile(payload, str(P336_PREPARE_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
    except Exception as exc:
        raise PromotionError("P3.36 prepare source failed to load") from exc
    finally:
...
def _p337_observer_spec() -> dict[str, Any]:
    ...
    return dict(evidence.p337_authenticated_open_read_diagnostic_observer_spec())


class _P337PrepareAdapterProxy:
    ...
    P328_RUN_ID = bytes.fromhex(P337_RUN_ID_HEX)
...
_PREPARE_ADAPTER = _P337PrepareAdapterProxy()


class _EvidenceProxy:
    P328_RUN_MANIFEST_SCHEMA = getattr(
        evidence, "P337_RUN_MANIFEST_SCHEMA", "s22plus_fyg8_p337_process_v2_run_manifest_v1"
    )
    P328_STATIC_RESULT_SCHEMA = getattr(
        evidence, "P337_STATIC_RESULT_SCHEMA", "s22plus_fyg8_p337_process_v2_static_result_v1"
    )
    P328_STATIC_RESULT_VERDICT = getattr(
        evidence, "P337_STATIC_RESULT_VERDICT", "PASS_P337_PROCESS_V2_STATIC_RESULT_HOST_ONLY"
    )
    # Reuse the established logical-resident Process-v2 role.  P337 changes
...
def _patch_prepare_graph() -> None:
    # The inherited P336 prepare functions resolve these names through their
    # own module dictionaries; no imported P336 module is modified.
    for module in (_P336, _P330, _INNER):
        ...
        for name, value in {
            ...
            "P337_RUN_ID_HEX": P337_RUN_ID_HEX,
        }.items():
...
def verify_manifest(
    ...
    *,
    ...
    if payload != _INNER.manifest_bytes(value):
        raise PromotionError("P3.37 ready manifest bytes differ")
    private_root = ROOT / "workspace/private"
    try:
        private_root.lstat()
    except OSError as exc:
        raise PromotionError("P3.37 private workspace is unavailable") from exc
    with tempfile.TemporaryDirectory(prefix=".p337-ready-", dir=private_root) as name:
        temporary = Path(name)
        staged_value = value
