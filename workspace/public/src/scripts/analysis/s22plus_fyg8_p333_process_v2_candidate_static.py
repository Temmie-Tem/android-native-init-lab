#!/usr/bin/env python3
"""Re-derive and publish the P3.33 host-only candidate-static receipt.

P3.33 is an exact-loaded P3.32 static seam.  The only fresh closure is the
P3.33 builder output and its pre-OPEN diagnostic observer/runtime/adapter;
...
import s22plus_fyg8_p333_open_entry_diag_acm_observer as resident_observer  # noqa: E402
import s22plus_fyg8_p333_open_entry_diag_runtime as resident_runtime  # noqa: E402
...
P332_STATIC_SOURCE = ANALYSIS / "s22plus_fyg8_p332_process_v2_candidate_static.py"
P332_STATIC_SOURCE_IDENTITY = {
    "size": 22_210,
    "sha256": "2dc883a897b3b5cb682d993856a33600942912be9237370688baba367c0f300b",
}
BUILDER_OUTPUT = ROOT / (
...
P333_BOOT_LZ4_IDENTITY = {
    "size": 28_622_656,
    "sha256": "6b4b726f47d8e9e0fdd3c8c1b5b79d68b98d74b787a32d365e2830d8944677b1",
}
...
DEFAULT_OUTPUT = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p333/"
    "process-v2-candidate-static-20260904-02.json"
)
SCHEMA = "s22plus_fyg8_p333_process_v2_candidate_static_v1"
VERDICT = "PASS_P333_PROCESS_V2_CANDIDATE_STATIC_HOST_ONLY"
RUN_ID = resident_runtime.P333_RUN_ID_HEX
PREDECESSOR_RUN_ID = candidate_build.P332_RUN_ID_HEX
OVERLAY = adapter.P333_OVERLAY_CONTRACT_ID
...
class StaticContractError(ValueError):
    """The exact P3.33 static closure is incomplete or changed."""
...
def _p332_static_source_receipt() -> dict[str, Any]:
    payload = _stable(
        P332_STATIC_SOURCE,
        "P3.32 candidate-static source",
        2 << 20,
        P332_STATIC_SOURCE_IDENTITY,
        nlink=1,
    )
    return {"path": _relative(P332_STATIC_SOURCE), **identity(payload)}


def _validate_header(value: Any) -> dict[str, Any]:
    if type(value) is not dict:
        raise StaticContractError("P3.33 static result is not an object")
    if (
        ...
        raise StaticContractError("P3.33 static header differs")
    return value
...
def _candidate_projection(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(candidate, dict) or not isinstance(userspace, dict):
        raise StaticContractError("P3.33 candidate/userspace projection is absent")
    init = userspace.get("a", {}).get("init")
    child = userspace.get("a", {}).get("child")
    if not isinstance(init, dict) or not isinstance(child, dict):
        raise StaticContractError("P3.33 userspace identity is absent")
    ...
    package_schema = "s22plus_fyg8_p333_boot_only_open_entry_diag_package_v1"
    for label in ("a", "b"):
...
def _validate_candidate(value: dict[str, Any], built: dict[str, Any]) -> None:
    ...
    if (
        ...
        != "s22plus_fyg8_p333_boot_only_open_entry_diag_package_v1"
        or candidate.get("image") != P333_IMAGE_IDENTITY
        or candidate.get("init") != P333_INIT_IDENTITY
        or candidate.get("a", {}).get("boot_img_lz4") != P333_BOOT_LZ4_IDENTITY
    ):
        raise StaticContractError("P3.33 candidate identity differs")
...
def _validate_observer(value: dict[str, Any]) -> None:
    ...
    if not isinstance(observer, dict):
        raise StaticContractError("P3.33 observer projection is absent")
    for key, expected_value in expected.items():
        if observer.get(key) != expected_value:
            raise StaticContractError(f"P3.33 observer field differs: {key}")
...
def _validate_runtime(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise StaticContractError("P3.33 runtime binding differs")


def _validate_safety(value: dict[str, Any]) -> None:
    safety = value.get("safety")
    if not isinstance(safety, dict) or safety.get("host_only") is not True:
        raise StaticContractError("P3.33 static authority projection differs")
    for name in (
        "device_contact",
        "device_write",
        "odin_invoked",
        ...
        if safety.get(name) is not False:
            raise StaticContractError(f"P3.33 safety flag differs: {name}")


def _project(built: dict[str, Any]) -> dict[str, Any]:
    _stable(BUILDER_RESULT, "P3.33 builder result", 4 << 20, BUILDER_RESULT_IDENTITY, mode=0o400, nlink=1)
    ...
    source_closure = {
        ...
        "p333_artifact_identity": _source_receipt(
            Path(artifact.__file__).resolve(), "P3.33 artifact identity"
        ),
        "p333_open_entry_diag_acm_observer": _source_receipt(
            Path(resident_observer.__file__).resolve(), "P3.33 observer"
        ),
        "p333_open_entry_diag_runtime": _source_receipt(
            Path(resident_runtime.__file__).resolve(), "P3.33 runtime"
        ),
        "p333_stock_candidate_build": _source_receipt(
            Path(candidate_build.__file__).resolve(), "P3.33 builder"
        ),
        "p333_stock_process_v2_adapter": _source_receipt(
            Path(adapter.__file__).resolve(), "P3.33 adapter"
        ),
        "p332_candidate_static": _p332_static_source_receipt(),
        "rollback_ap": _source_receipt(ROLLBACK_AP, "P3.33 exact rollback AP", 64 << 20),
    }
    return {
        "schema": SCHEMA,
        "verdict": VERDICT,
...
def build_result() -> dict[str, Any]:
    try:
        ...
        raise StaticContractError(f"P3.33 static result did not regenerate: {exc}") from exc


def validate_result(value: Any) -> dict[str, Any]:
    expected = build_result()
    if type(value) is not dict or value != expected:
        raise StaticContractError("P3.33 static result does not regenerate")
    return value
...
def validate_bound_result(value: Any) -> dict[str, Any]:
    ...
    if type(value) is not dict or value != expected:
        raise StaticContractError("P3.33 bound static result does not regenerate")
    return value
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise StaticContractError("P3.33 value is not canonical JSON") from exc


def publish(path: Path, payload: bytes) -> None:
    direct = path.absolute()
    if direct.exists() or direct.is_symlink():
        raise StaticContractError("P3.33 static output already exists")
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise StaticContractError("P3.33 static publication was short")
            offset += written
