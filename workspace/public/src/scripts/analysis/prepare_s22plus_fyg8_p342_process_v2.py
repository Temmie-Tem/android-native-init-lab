#!/usr/bin/env python3
"""Prepare the P342 Process-v2 promotion projection on the host only.

This module reopens the P342 static receipt, exact boot-only AP and exact
...
import s22plus_fyg8_p342_open_read_branch_acm_observer as framed_observer  # noqa: E402
import s22plus_fyg8_p342_open_read_branch_runtime as framed_runtime  # noqa: E402
import s22plus_fyg8_p342_process_v2_candidate_static as candidate_static  # noqa: E402
import s22plus_fyg8_p342_stock_candidate_build as candidate_build  # noqa: E402
import s22plus_fyg8_p342_stock_process_v2_adapter as adapter  # noqa: E402


SCHEMA = "s22plus_fyg8_p342_process_v2_promotion_v1"
VERDICT = "PASS_P342_PROCESS_V2_PROMOTION_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P342_PROCESS_V2_PROMOTION_REHEARSAL_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p342_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P342_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
RUN_MANIFEST_SCHEMA = evidence.P342_RUN_MANIFEST_SCHEMA
STATIC_RESULT_SCHEMA = evidence.P342_STATIC_RESULT_SCHEMA
STATIC_RESULT_VERDICT = evidence.P342_STATIC_RESULT_VERDICT
...
DEFAULT_MANIFEST = ROOT / (
    ...
    "s22plus_fyg8_p342_process_v2_ready_1.json"
)
DEFAULT_TARGET_PROFILE = ROOT / "workspace/public/src/device-action/profiles/s22plus_fyg8.json"
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p342-process-v2-ready-1"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p342-live-1"
...
class PromotionError(ValueError):
    """The exact P342 promotion closure or manifest is invalid."""
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise PromotionError("P342 promotion value is not canonical JSON") from exc
...
def manifest_bytes(value: dict[str, Any]) -> bytes:
    try:
        ...
        raise PromotionError("P342 ready manifest is not canonical JSON") from exc
...
def relative(path: Path) -> str:
    ...
    try:
        ...
        raise PromotionError("P342 path escaped the repository") from exc
...
def _candidate_inputs(
    ...
    if static_path.exists() or static_path.is_symlink():
        static_payload = _stable_bytes(static_path, "P342 candidate-static", 2 << 20, mode=0o400)
        static_value = _decode(static_payload, "P342 candidate-static")
        if static_payload != candidate_static.canonical(static_value):
            raise PromotionError("P342 candidate-static bytes are not canonical")
    else:
    ...
    try:
        ...
        raise PromotionError("P342 candidate-static did not reopen") from exc
    builder_path = DEFAULT_BUILDER_OUTPUT / "result.json"
    builder_payload = _stable_bytes(builder_path, "P342 builder result", 4 << 20, mode=0o400)
    builder_value = _decode(builder_payload, "P342 builder result")
    try:
        audited = candidate_build.audit_existing(DEFAULT_BUILDER_OUTPUT)
    except Exception as exc:
        raise PromotionError("P342 builder result did not reopen") from exc
    if audited != builder_value:
        raise PromotionError("P342 builder result changed")
    candidate = static_value.get("candidate")
    if not isinstance(candidate, dict) or not isinstance(candidate.get("a"), dict):
        raise PromotionError("P342 candidate-static package is incomplete")
    image = _stable_bytes(
        DEFAULT_BUILDER_OUTPUT / "inputs/fixed-Image",
        "P342 candidate Image",
        64 << 20,
    ...
    child = _stable_bytes(
        ...
        "P342 candidate child",
        2 << 20,
        expected=candidate.get("child"),
        mode=0o400,
    )
    ap_payload = _stable_bytes(candidate_ap_path, "P342 candidate AP", 64 << 20)
    try:
        ap_info = artifact.inspect_ap(
            ...
            label="P342 candidate AP",
        )
    except Exception as exc:
        raise PromotionError("P342 candidate AP is invalid") from exc
    if ap_info["ap_structure"]["member"]["name"] != "boot.img.lz4":
        raise PromotionError("P342 candidate AP is not boot-only")
    ...
    if candidate["a"].get("ap_tar_md5") != {
        ...
        raise PromotionError("P342 candidate-static AP identity differs")
...
def _acceptance(payloads: dict[str, bytes], output: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise PromotionError(f"P342 acceptance identity differs: {exc}") from exc
    return value
...
def _promotion_payloads(
    ...
    candidate = static_value.get("candidate")
    if not isinstance(candidate, dict) or not isinstance(candidate.get("a"), dict):
        raise PromotionError("P342 candidate package is incomplete")
    ...
    if static_value.get("authentication") != expected_authentication:
        raise PromotionError("P342 authentication receipt differs")
    ...
    run_manifest = {
        ...
        "observation_contract": {
            "accepted_identity": "P342_STOCK_OBSERVER_V4_IDLE_REUSE",
...
def _observer_spec() -> dict[str, Any]:
    return dict(evidence.p342_authenticated_open_read_branch_observer_spec())
...
def _offline_verification(
    ...
    try:
        ...
        raise PromotionError(f"P342 common offline verification failed: {exc}") from exc
...
def _manifest(
    *,
    ...
    run_manifest = _decode(payloads["run_manifest"], "P342 run manifest")
    expected = {
        "source_contract_id": adapter.PARENT_SOURCE_CONTRACT_ID,
        "userspace_overlay_contract_id": adapter.OVERLAY_CONTRACT_ID,
        "profile": adapter.PROFILE,
    ...
    if any(run_manifest.get(key) != value for key, value in expected.items()):
        raise PromotionError("P342 promotion run manifest differs")
    if type(timeout_sec) is not int or not 1 <= timeout_sec <= 600:
        raise PromotionError("P342 observation timeout is invalid")
...
def _write_exclusive(path: Path, payload: bytes, mode: int) -> None:
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise PromotionError("P342 publication was short")
            written += count
...
def publish_promotion(output: Path, payloads: dict[str, bytes]) -> None:
    ...
    if direct.exists() or direct.is_symlink():
        raise PromotionError("P342 promotion output already exists")
...
def publish_manifest(path: Path, payload: bytes) -> None:
    direct = path.absolute()
    if direct.exists() or direct.is_symlink():
        raise PromotionError("P342 ready manifest already exists")
...
def verify_manifest(
    ...
    *,
    ...
    if payload != manifest_bytes(value):
        raise PromotionError("P342 ready manifest bytes differ")
    private_root = ROOT / "workspace/private"
    try:
        private_root.lstat()
    except OSError as exc:
        raise PromotionError("P342 private workspace is unavailable") from exc
    import tempfile

    with tempfile.TemporaryDirectory(prefix=".p342-ready-", dir=private_root) as name:
        temporary = Path(name)
...
def build(
    *,
    ...
    rollback_payload = _stable_bytes(rollback_path, "P342 exact rollback AP", 64 << 20)
    ...
    if {key: rollback_ap[key] for key in ("size", "sha256")} != ROLLBACK_IDENTITY:
        raise PromotionError("P342 rollback identity differs")
    acceptance = _acceptance(payloads, promotion_root)
    ...
    try:
        ...
        if args.publish:
            if verification.get("common_offline_verified") is not True:
                raise PromotionError("P342 common verification is pending; publication refused")
            ...
            raise PromotionError("P342 audit-only refuses existing publication paths")
