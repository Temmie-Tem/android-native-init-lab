#!/usr/bin/env python3
"""Prepare P3.26 static promotion and ready manifest (host-only)."""
...
import s22plus_fyg8_p326_process_v2_candidate_static as p326_static  # noqa: E402
import s22plus_fyg8_p326_stock_candidate_build as p326_build  # noqa: E402
import s22plus_fyg8_p326_stock_process_v2_adapter as p326_adapter  # noqa: E402


P325_PREPARE_SOURCE = ANALYSIS / "prepare_s22plus_fyg8_p325_process_v2.py"
P325_PREPARE_IDENTITY = {
    "size": 14_433,
    "sha256": "6fb9a323ef12bbc14f63abfe168ebebe1c55d20545612ca969fd285a6bd82a9b",
}
SCHEMA = "s22plus_fyg8_p326_process_v2_promotion_v1"
VERDICT = "PASS_P326_PROCESS_V2_PROMOTION_HOST_ONLY"
READY_SCHEMA = "s22plus_fyg8_p326_ready_manifest_builder_v1"
READY_VERDICT = "PASS_P326_PROCESS_V2_READY_MANIFEST_HOST_ONLY"
REHEARSAL_VERDICT = "PASS_P326_PROCESS_V2_READY_MANIFEST_REHEARSAL_HOST_ONLY"
DEFAULT_BUILDER_OUTPUT = p326_build.DEFAULT_OUTPUT_ROOT
DEFAULT_STATIC_OUTPUT = p326_static.DEFAULT_OUTPUT
...
DEFAULT_MANIFEST_ID = "s22plus-fyg8-p326-process-v2-ready-1"
DEFAULT_LIVE_RUN_ID = "s22plus-fyg8-p326-live-1"
DEFAULT_TIMEOUT_SEC = 300
ROLLBACK_IDENTITY = dict(p326_static.ROLLBACK_IDENTITY)
TARGET = dict(p326_static.TARGET)
PRIVATE_PARENT = ROOT / "workspace/private/outputs/s22plus_fyg8_p326"
...
def _stable_source(path: Path, expected: dict[str, Any]) -> bytes:
    ...
    try:
        ...
        raise WrapperError("P325 prepare source is unavailable") from exc
    ...
    if (
        ...
        raise WrapperError("P325 prepare source identity differs")
    return payload
...
def _load_delegate() -> tuple[types.ModuleType, types.ModuleType]:
    payload = _stable_source(P325_PREPARE_SOURCE, P325_PREPARE_IDENTITY)
    module = types.ModuleType("prepare_s22plus_fyg8_p325_bound_for_p326")
    module.__file__ = str(P325_PREPARE_SOURCE)
    module.__package__ = ""
    try:
        exec(compile(payload, str(P325_PREPARE_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
    except Exception as exc:
        raise WrapperError("P325 prepare source failed to load") from exc
    base = module._BASE
    build_proxy = _proxy(
        p326_build,
    ...
    adapter_proxy = _proxy(
        p326_adapter,
        P321_RUN_ID_HEX=p326_adapter.P326_RUN_ID_HEX,
        P321_OVERLAY_CONTRACT_ID=p326_adapter.P326_OVERLAY_CONTRACT_ID,
    )
    evidence_proxy = _proxy(
        current_evidence,
        P321_RUN_MANIFEST_SCHEMA=current_evidence.P326_RUN_MANIFEST_SCHEMA,
        P321_STATIC_RESULT_SCHEMA=current_evidence.P326_STATIC_RESULT_SCHEMA,
        P321_STATIC_RESULT_VERDICT=current_evidence.P326_STATIC_RESULT_VERDICT,
    )
    base.candidate_static = p326_static
...
_P325, _BASE = _load_delegate()
PromotionError = _BASE.PromotionError
adapter = p326_adapter
...
_BASE_PROMOTION_PAYLOADS = _P325._BASE_PROMOTION_PAYLOADS
_BASE_DERIVE_MANIFEST = _P325._BASE_DERIVE_MANIFEST
_BASE_BUILD = _P325._BASE_BUILD
...
def _promotion_payloads(
    ...
    run_manifest = decode(payloads["run_manifest"], "P326 run manifest")
    observation = run_manifest.get("observation_contract")
    if not isinstance(observation, dict):
        raise PromotionError("P326 promotion observer is absent")
    observation["accepted_identity"] = "P326_STOCK_OBSERVER_V4_RETAINED"
    run_payload = canonical(run_manifest)
    static_result = decode(payloads["static_check"], "P326 static result")
    artifacts = static_result.get("candidate", {}).get("artifacts")
    if not isinstance(artifacts, dict):
        raise PromotionError("P326 static artifacts are absent")
...
def derive_manifest(**kwargs: Any) -> dict[str, Any]:
    ...
    run_id = p326_adapter.P326_RUN_ID_HEX
    value["observation"]["candidate_observer"] = {
        ...
        "banner_hex": console.DEVICE_TRANSCRIPT.hex(),
    }
    value["observation"][current_evidence.CANDIDATE_ARRIVAL_PROOF_ROLE_KEY] = (
        current_evidence.CANDIDATE_BIDIRECTIONAL_CONSOLE_ROLE
    )
    return value
...
def verify_manifest(
    ...
    *,
    ...
    if payload != manifest_bytes(value):
        raise PromotionError("P326 ready manifest bytes differ")
    staged_value = value
    PRIVATE_PARENT.mkdir(mode=0o700, parents=True, exist_ok=True)
    PRIVATE_PARENT.chmod(0o700)
    with tempfile.TemporaryDirectory(prefix=".p326-ready-", dir=PRIVATE_PARENT) as name:
        temporary = Path(name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "manifest_id": manifest["manifest_id"], "run_id": p326_adapter.P326_RUN_ID_HEX,
