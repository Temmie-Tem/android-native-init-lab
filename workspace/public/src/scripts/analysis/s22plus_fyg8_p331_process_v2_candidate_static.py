#!/usr/bin/env python3
"""Validate and publish the P3.31 host-only candidate-static receipt.

The checker reopens the single P3.31 build, both boot-only APs, the exact
...
P330_IMAGE_IDENTITY = candidate_build.P330_IMAGE_IDENTITY
P330_INIT_IDENTITY = candidate_build.P330_INIT_IDENTITY
...
RUN_ID = candidate_build.P331_RUN_ID_HEX
PREDECESSOR_RUN_ID = candidate_build.P330_RUN_ID_HEX
OVERLAY = adapter.P331_OVERLAY_CONTRACT_ID
...
class StaticContractError(ValueError):
    """The exact P3.31 static closure is incomplete or changed."""
...
def _validate_static_header(value: Any) -> dict[str, Any]:
    if type(value) is not dict:
        raise StaticContractError("P3.31 static result is not an object")
    if (
        ...
        raise StaticContractError("P3.31 static header differs")
    return value
...
def _validate_candidate(value: dict[str, Any], built: dict[str, Any]) -> None:
    ...
    if not isinstance(candidate, dict) or candidate.get("a") != candidate.get("b"):
        raise StaticContractError("P3.31 A/B candidate differs")
    ...
    if (
        ...
        != "s22plus_fyg8_p331_boot_only_resident_package_v1"
    ):
        raise StaticContractError("P3.31 candidate identity differs")
    if candidate.get("image") == P330_IMAGE_IDENTITY or candidate.get("init") == P330_INIT_IDENTITY:
        raise StaticContractError("P3.31 candidate repeats the consumed P3.30 image")
...
def _validate_observer(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        or observer.get("proof_command_count") != 1
        or observer.get("session_cap") != resident_runtime.MAX_SESSIONS
        or observer.get("reconnect_cap") != resident_runtime.MAX_RECONNECTS
        or observer.get("fixed_heartbeat_only") is not True
    ):
        raise StaticContractError("P3.31 resident observer binding differs")
    ...
    if (
        ...
        raise StaticContractError("P3.31 observer source identity differs")
...
def _validate_runtime(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        or repair.get("command_policy") != "fixed_heartbeat_status_v1"
        or repair.get("caller_selected_command") is not False
        or repair.get("session_cap") != resident_runtime.MAX_SESSIONS
        or repair.get("reconnect_cap") != resident_runtime.MAX_RECONNECTS
        or repair.get("banner_scope") != "resident_loop_per_session"
        or repair.get("auth_key_path_published") is not False
    ):
        raise StaticContractError("P3.31 resident runtime binding differs")
    if value.get("authentication", {}).get("path_published") is not False:
        raise StaticContractError("P3.31 authentication path leaked")


def _validate_safety(value: dict[str, Any]) -> None:
    safety = value.get("safety")
    if not isinstance(safety, dict) or safety.get("host_only") is not True:
        raise StaticContractError("P3.31 static authority projection differs")
    for name in (
        "device_contact",
        "device_write",
        "odin_invoked",
        ...
        if safety.get(name) is not False:
            raise StaticContractError(f"P3.31 safety flag differs: {name}")
...
def _project(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(phase2, dict):
        raise StaticContractError("P3.31 build phase is absent")
    ...
    if not isinstance(candidate, dict) or not isinstance(userspace, dict):
        raise StaticContractError("P3.31 candidate/userspace projection is absent")
    init = userspace.get("a", {}).get("init")
    child = userspace.get("a", {}).get("child")
    if not isinstance(init, dict) or not isinstance(child, dict):
        raise StaticContractError("P3.31 userspace identity is absent")
    ...
    return {
        ...
        "source_closure": {
            ...
            "p331_artifact_identity": _source_receipt(artifact.P331_ARTIFACT_SOURCE, "P3.31 artifact"),
            "p331_authenticated_acm_observer": _source_receipt(Path(resident_observer.__file__), "P3.31 observer"),
            "p331_resident_exec_runtime": _source_receipt(Path(resident_runtime.__file__), "P3.31 runtime"),
            "p331_stock_candidate_build": _source_receipt(candidate_build.SELF_SOURCE, "P3.31 builder"),
            "p331_stock_process_v2_adapter": _source_receipt(adapter.P331_ADAPTER_SOURCE, "P3.31 adapter"),
            "p330_candidate_static": {
                "path": _relative(P330_STATIC_SOURCE),
                **P330_STATIC_SOURCE_IDENTITY,
            },
            "rollback_ap": _source_receipt(ROLLBACK_AP, "P3.31 exact rollback AP", 64 << 20),
        },
        "authentication": {
            "required": True,
            "scheme": "auth-key-v1",
            "key": artifact.auth_key_identity(),
...
def build_result() -> dict[str, Any]:
    try:
        ...
        raise StaticContractError(f"P3.31 static result did not regenerate: {exc}") from exc
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise StaticContractError("P3.31 value is not canonical JSON") from exc


def publish(path: Path, payload: bytes) -> None:
    direct = path.absolute()
    if direct.exists() or direct.is_symlink():
        raise StaticContractError("P3.31 static output already exists")
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise StaticContractError("P3.31 static publication was short")
            offset += written
