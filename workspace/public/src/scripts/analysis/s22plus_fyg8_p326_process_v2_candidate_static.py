#!/usr/bin/env python3
"""Build the P3.26 Process-v2 candidate-static receipt (host-only)."""
...
import s22plus_fyg8_p326_bidirectional_acm_observer as observer_adapter  # noqa: E402
import s22plus_fyg8_p326_bidirectional_console_runtime as console  # noqa: E402
import s22plus_fyg8_p326_stock_candidate_build as candidate_build  # noqa: E402
...
P325_RUNTIME = candidate_build.P325_OUTPUT / "stock-sources/s22plus_fyg8_p290_e3_runtime.inc.c"
P326_RUNTIME = BUILDER_OUTPUT / "stock-sources/s22plus_fyg8_p290_e3_runtime.inc.c"
...
DEFAULT_OUTPUT = ROOT / (
    ...
    "process-v2-candidate-static-20260902-06.json"
)

SCHEMA = "s22plus_fyg8_p326_process_v2_candidate_static_v1"
VERDICT = "PASS_P326_PROCESS_V2_CANDIDATE_STATIC_HOST_ONLY"
TARGET = {"model": "SM-S906N", "codename": "g0q", "build": "S906NKSS7FYG8"}
RUN_ID = adapter.P326_RUN_ID_HEX
PREDECESSOR_RUN_ID = adapter.P325_RUN_ID_HEX
OVERLAY = adapter.P326_OVERLAY_CONTRACT_ID
...
def _builder(*, runtime_bound: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    payload = stable_bytes(BUILDER_RESULT, "P326 builder result", 2 << 20, mode=0o400)
    value = decode(payload, "P326 builder result")
    if not runtime_bound:
        try:
            if candidate_build.audit_existing(BUILDER_OUTPUT) != value:
                raise StaticContractError("P326 builder audit differs")
        except StaticContractError:
            raise
        except Exception as exc:
            raise StaticContractError("P326 builder did not reopen") from exc
    ...
    if (
        ...
        or candidate.get("differs_from_consumed_p325") is not True
        ...
        raise StaticContractError("P326 builder contract differs")
    for label in ("a", "b"):
        ...
        if (
            ...
            raise StaticContractError("P326 boot-only BusyBox package differs")
...
def _artifacts(builder: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    image = stable_bytes(
        BUILDER_OUTPUT / "inputs/fixed-Image", "P326 Image", 64 << 20,
        candidate["fixed_image_identity"], mode=0o400,
    )
    init = stable_bytes(
        BUILDER_OUTPUT / "userspace-a/init", "P326 init", 2 << 20,
        userspace["a"]["init"], mode=0o400,
    )
    child = stable_bytes(
        BUILDER_OUTPUT / "userspace-a/s22-e1-child", "P326 child", 2 << 20,
...
def _runtime(builder: dict[str, Any]) -> dict[str, Any]:
    before = stable_bytes(P325_RUNTIME, "P325 runtime", 2 << 20, console.P325_RUNTIME_IDENTITY, mode=0o400)
    after = stable_bytes(
        P326_RUNTIME, "P326 runtime", 2 << 20,
    ...
    except console.ConsoleRuntimeError as exc:
        raise StaticContractError(str(exc)) from exc
...
def build_result(*, runtime_bound: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        raise StaticContractError("P326 adapter did not reopen") from exc
    if (
        ...
        raise StaticContractError("P326 adapter binding differs")
    candidate = builder["phase2"]["candidate"]
    userspace = builder["phase2"]["userspace"]
    sources = {
        "p326_candidate_static": receipt(SELF_SOURCE, "P326 static", 2 << 20),
        "p326_stock_candidate_build": receipt(candidate_build.SELF_SOURCE, "P326 builder", 2 << 20),
        "p326_stock_process_v2_adapter": receipt(candidate_build.P326_ADAPTER_SOURCE, "P326 adapter", 2 << 20),
        "p326_artifact_identity": receipt(candidate_build.P326_ARTIFACT_SOURCE, "P326 artifact", 2 << 20),
        "p326_bidirectional_console_runtime": receipt(candidate_build.P326_RUNTIME_SOURCE, "P326 runtime helper", 2 << 20),
        "p326_bidirectional_acm_observer": receipt(candidate_build.P326_OBSERVER_SOURCE, "P326 observer", 2 << 20),
        "busybox": receipt(candidate_build.BUSYBOX, "P326 BusyBox", 4 << 20, candidate_build.BUSYBOX_IDENTITY),
        "rollback_ap": receipt(ROLLBACK_AP, "P326 rollback", 64 << 20, ROLLBACK_IDENTITY),
    }
    return {
        "schema": SCHEMA,
        "verdict": VERDICT,
        "authority_source": sources["p326_candidate_static"],
        "target": TARGET,
        ...
        "adapter": {"lineage": lineage, "acceptance": acceptance, "source": sources["p326_stock_process_v2_adapter"]},
        "observer_adapter": {
            "schema": observer_adapter.SCHEMA,
            "contract_id": observer_adapter.CONTRACT_ID,
            "source": sources["p326_bidirectional_acm_observer"],
...
def validate_result(value: Any) -> dict[str, Any]:
    if type(value) is not dict or value != build_result():
        raise StaticContractError("P326 static result does not regenerate")
    return value


def validate_bound_result(value: Any) -> dict[str, Any]:
    if type(value) is not dict or value != build_result(runtime_bound=True):
        raise StaticContractError("P326 bound static result differs")
    return value


def publish(path: Path, payload: bytes) -> None:
    if path.exists() or path.is_symlink():
        raise StaticContractError("P326 static output already exists")
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise StaticContractError("P326 static publication was short")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
