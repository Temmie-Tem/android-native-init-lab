#!/usr/bin/env python3
"""Reopen the P342 boot-only candidate as an H0 static receipt.

The P342 builder owns byte production.  This wrapper reopens its immutable
...
SCHEMA = evidence.P342_CANDIDATE_STATIC_SCHEMA
VERDICT = evidence.P342_CANDIDATE_STATIC_VERDICT
RUN_ID = candidate_build.P342_RUN_ID_HEX
PREDECESSOR_RUN_ID = candidate_build.P341_RUN_ID_HEX
TARGET = dict(candidate_build.TARGET)
AUTH_KEY_IDENTITY = dict(evidence.P342_AUTH_EXEC_AUTH_KEY_IDENTITY)


class StaticContractError(ValueError):
    """The exact P342 static closure is incomplete or changed."""
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise StaticContractError("P342 static value is not canonical JSON") from exc
...
def _candidate_projection(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(phase2, dict):
        raise StaticContractError("P342 builder phase-2 projection is absent")
    ...
    if not isinstance(candidate, dict) or not isinstance(userspace, dict):
        raise StaticContractError("P342 candidate/userspace projection is absent")
    ...
    if (
        ...
        raise StaticContractError("P342 userspace identities are absent")
    ...
    if (
        ...
        or candidate.get("a", {}).get("ap_tar_md5") == artifact.P341_AP_IDENTITY
        or candidate.get("a", {}).get("package", {}).get("members")
        != ["boot.img.lz4"]
    ):
        raise StaticContractError("P342 candidate identity differs")
    ...
    result.update(
        {
            ...
            "init": user_a["init"],
            "differs_from_consumed_p341": True,
    ...
    if not isinstance(result.get("image"), dict):
        raise StaticContractError("P342 fixed Image identity is absent")
    for label in ("a", "b"):
        package = result[label].get("package")
        if not isinstance(package, dict):
            raise StaticContractError("P342 package projection is absent")
        package.update(
            {
                "schema": "s22plus_fyg8_p342_idle_reuse_boot_only_package_v1",
                "verdict": "PASS_P342_IDLE_REUSE_BOOT_ONLY_H0",
            }
        )
    return result
    ...
    value.update(
        {
            ...
            "total_command_count": adapter.TOTAL_COMMANDS,
            "physical_reopen_count": observer.PHYSICAL_REOPEN_COUNT,
            "physical_reopen_indexes": list(evidence.P342_PHYSICAL_REOPEN_INDEXES),
...
def _validate_runtime(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(repair, dict) or not isinstance(expected, dict):
        raise StaticContractError("P342 runtime repair/source receipt is absent")
    source = _stable(
        BUILDER_OUTPUT / "stock-sources/s22plus_fyg8_p290_e3_runtime.inc.c",
        "P342 runtime include",
        2 << 20,
        expected,
        mode=0o400,
    )
    try:
        checked = runtime.validate_p342_runtime(source)
    except Exception as exc:
        raise StaticContractError("P342 runtime include did not validate") from exc
    for name, required in {
        ...
        if checked.get(name) != required or repair.get(name) != required:
            raise StaticContractError(f"P342 runtime field differs: {name}")
...
def _validate_safety(value: dict[str, Any]) -> None:
    ...
    if not isinstance(safety, dict) or safety.get("host_only") is not True:
        raise StaticContractError("P342 static safety projection differs")
    for name in (
        ...
        if safety.get(name) is not False:
            raise StaticContractError(f"P342 safety flag differs: {name}")
...
def build_result() -> dict[str, Any]:
    try:
        ...
        if (
            ...
            raise StaticContractError("P342 builder header or scope differs")
        ...
        source_closure = {
            ...
            "p342_artifact_identity": _source_receipt(
                Path(artifact.__file__).resolve(), "P342 artifact identity"
            ),
            "p342_host_first_open": _source_receipt(
                runtime.HOST_FIRST_SOURCE, "P342 host-first OPEN helper"
            ),
            "p342_open_failure_capture": _source_receipt(
                capture_path, "P342 initial failure capture"
            ),
            "p342_open_read_branch_acm_observer": _source_receipt(
                Path(observer.__file__).resolve(), "P342 observer"
            ),
            "p342_open_read_branch_runtime": _source_receipt(
                Path(runtime.__file__).resolve(), "P342 runtime"
            ),
            "p342_idle_reuse_probe": _source_receipt(
                idle_path, "P342 idle-reuse probe"
            ),
            "p342_stock_candidate_build": _source_receipt(
                Path(candidate_build.__file__).resolve(), "P342 builder"
            ),
            "p342_stock_process_v2_adapter": _source_receipt(
                Path(adapter.__file__).resolve(), "P342 adapter"
            ),
            "rollback_ap": _source_receipt(
                ROLLBACK_AP, "P342 exact rollback AP", 64 << 20, ROLLBACK_IDENTITY
            ),
        }
        if observer_value.get("capture_source") != {
            key: source_closure["p342_open_failure_capture"][key]
            for key in ("size", "sha256")
        }:
            raise StaticContractError("P342 failure-capture source identity differs")
        if observer_value.get("idle_reuse_source") != source_closure["p342_idle_reuse_probe"]:
            raise StaticContractError("P342 idle-reuse source identity differs")
        ...
        value = {
            ...
            "artifact_identity": {
                ...
                "predecessor_ap_identity_rejected": dict(artifact.P341_AP_IDENTITY),
                "boot_only": True,
            ...
            "idle_reuse": {
                ...
                "elapsed_seconds_min": evidence.P342_IDLE_REUSE_MIN_ELAPSED_SECONDS,
                "elapsed_seconds_max": evidence.P342_IDLE_REUSE_MAX_ELAPSED_SECONDS,
            },
            "safety": {
                "host_only": True,
                "device_contact": False,
        ...
        raise StaticContractError(f"P342 static result did not regenerate: {exc}") from exc
...
def validate_result(value: Any) -> dict[str, Any]:
    ...
    if type(value) is not dict or canonical(value) != canonical(expected):
        raise StaticContractError("P342 static result differs from current closure")
    return value
...
def _publish(path: Path, payload: bytes) -> None:
    ...
    if direct.exists() or direct.is_symlink():
        raise StaticContractError("P342 static result already exists")
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise StaticContractError("P342 static publication was short")
            written += count
