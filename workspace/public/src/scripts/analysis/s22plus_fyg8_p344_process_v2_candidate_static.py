#!/usr/bin/env python3
"""Reopen the P344 boot-only candidate as an H0 static receipt.

The P344 builder owns byte production.  This wrapper reopens its immutable
...
import s22plus_fyg8_p344_stock_candidate_build as candidate_build  # noqa: E402
import s22plus_fyg8_p344_stock_process_v2_adapter as adapter  # noqa: E402
...
SCHEMA = evidence.P344_CANDIDATE_STATIC_SCHEMA
VERDICT = evidence.P344_CANDIDATE_STATIC_VERDICT
RUN_ID = candidate_build.P344_RUN_ID_HEX
PREDECESSOR_RUN_ID = candidate_build.P343_RUN_ID_HEX
TARGET = dict(candidate_build.TARGET)
AUTH_KEY_IDENTITY = dict(evidence.P344_AUTH_EXEC_AUTH_KEY_IDENTITY)


class StaticContractError(ValueError):
    """The exact P344 static closure is incomplete or changed."""
...
def canonical(value: Any) -> bytes:
    try:
        ...
        raise StaticContractError("P344 static value is not canonical JSON") from exc
...
def _candidate_projection(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(phase2, dict):
        raise StaticContractError("P344 builder phase-2 projection is absent")
    ...
    if not isinstance(candidate, dict) or not isinstance(userspace, dict):
        raise StaticContractError("P344 candidate/userspace projection is absent")
    ...
    if (
        ...
        raise StaticContractError("P344 userspace identities are absent")
    ...
    if (
        ...
        or candidate.get("a", {}).get("ap_tar_md5") == artifact.P343_AP_IDENTITY
        or candidate.get("a", {}).get("package", {}).get("members")
        != ["boot.img.lz4"]
    ):
        raise StaticContractError("P344 candidate identity differs")
    ...
    result.update(
        {
            ...
            "init": user_a["init"],
            "differs_from_consumed_p343": True,
    ...
    if not isinstance(result.get("image"), dict):
        raise StaticContractError("P344 fixed Image identity is absent")
    for label in ("a", "b"):
        package = result[label].get("package")
        if not isinstance(package, dict):
            raise StaticContractError("P344 package projection is absent")
        package.update(
            {
                "schema": "s22plus_fyg8_p344_named_exploration_boot_only_package_v1",
                "verdict": "PASS_P344_NAMED_EXPLORATION_BOOT_ONLY_H0",
            }
        )
    return result
    ...
    value.update(
        {
            ...
            "total_command_count": adapter.TOTAL_COMMANDS,
            "physical_reopen_count": observer.PHYSICAL_REOPEN_COUNT,
            "physical_reopen_indexes": list(evidence.P344_PHYSICAL_REOPEN_INDEXES),
...
def _validate_runtime(built: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(repair, dict) or not isinstance(expected, dict):
        raise StaticContractError("P344 runtime repair/source receipt is absent")
    source = _stable(
        BUILDER_OUTPUT / "stock-sources/s22plus_fyg8_p290_e3_runtime.inc.c",
        "P344 runtime include",
        2 << 20,
        expected,
        mode=0o400,
    )
    try:
        checked = runtime.validate_p344_runtime(source)
    except Exception as exc:
        raise StaticContractError("P344 runtime include did not validate") from exc
    for name, required in {
        ...
        if checked.get(name) != required or repair.get(name) != required:
            raise StaticContractError(f"P344 runtime field differs: {name}")
...
def _validate_safety(value: dict[str, Any]) -> None:
    ...
    if not isinstance(safety, dict) or safety.get("host_only") is not True:
        raise StaticContractError("P344 static safety projection differs")
    for name in (
        ...
        if safety.get(name) is not False:
            raise StaticContractError(f"P344 safety flag differs: {name}")
...
def build_result() -> dict[str, Any]:
    try:
        ...
        if (
            ...
            raise StaticContractError("P344 builder header or scope differs")
        ...
        source_closure = {
            ...
            "p344_artifact_identity": _source_receipt(
                Path(artifact.__file__).resolve(), "P344 artifact identity"
            ),
            "p344_host_first_open": _source_receipt(
                runtime.HOST_FIRST_SOURCE, "P344 host-first OPEN helper"
            ),
            "p344_open_failure_capture": _source_receipt(
                capture_path, "P344 initial failure capture"
            ),
            "p344_open_read_branch_acm_observer": _source_receipt(
                Path(observer.__file__).resolve(), "P344 observer"
            ),
            "p344_open_read_branch_runtime": _source_receipt(
                Path(runtime.__file__).resolve(), "P344 runtime"
            ),
            "p344_idle_reuse_probe": _source_receipt(
                idle_path, "P344 idle-reuse probe"
            ),
            "p344_stock_candidate_build": _source_receipt(
                Path(candidate_build.__file__).resolve(), "P344 builder"
            ),
            "p344_stock_process_v2_adapter": _source_receipt(
                Path(adapter.__file__).resolve(), "P344 adapter"
            ),
            "p344_exploration_session": _source_receipt(
                REVALIDATION / "s22plus_fyg8_p344_exploration_session.py",
                "P344 exploration session",
            ),
            "p344_exploration_action": _source_receipt(
                REVALIDATION / "s22plus_fyg8_p344_exploration_action.py",
                "P344 exploration action",
            ),
            "p344_readonly_exploration": _source_receipt(
                REVALIDATION / "s22plus_fyg8_readonly_exploration.py",
                "P344 read-only exploration catalog",
            ),
            ...
            "rollback_ap": _source_receipt(
                ROLLBACK_AP, "P344 exact rollback AP", 64 << 20, ROLLBACK_IDENTITY
            ),
        }
        if observer_value.get("capture_source") != {
            key: source_closure["p344_open_failure_capture"][key]
            for key in ("size", "sha256")
        }:
            raise StaticContractError("P344 failure-capture source identity differs")
        if observer_value.get("idle_reuse_source") != source_closure["p344_idle_reuse_probe"]:
            raise StaticContractError("P344 idle-reuse source identity differs")
        ...
        value = {
            ...
            "artifact_identity": {
                ...
                "predecessor_ap_identity_rejected": dict(artifact.P343_AP_IDENTITY),
                "boot_only": True,
            ...
            "idle_reuse": {
                ...
                "elapsed_seconds_min": evidence.P344_IDLE_REUSE_MIN_ELAPSED_SECONDS,
                "elapsed_seconds_max": evidence.P344_IDLE_REUSE_MAX_ELAPSED_SECONDS,
            },
            "safety": {
                "host_only": True,
                "device_contact": False,
        ...
        raise StaticContractError(f"P344 static result did not regenerate: {exc}") from exc
...
def validate_result(value: Any) -> dict[str, Any]:
    ...
    if type(value) is not dict or canonical(value) != canonical(expected):
        raise StaticContractError("P344 static result differs from current closure")
    return value
...
def _publish(path: Path, payload: bytes) -> None:
    ...
    if direct.exists() or direct.is_symlink():
        raise StaticContractError("P344 static result already exists")
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise StaticContractError("P344 static publication was short")
            written += count
