...
The changed closure is explicit: P338 artifact identity, adapter, runtime,
observer, builder, the P337 static predecessor, and the unchanged exact rollback.
The consumed P337 later-action lane is intentionally absent. No device, ADB, Odin, approval, transfer,
...
P337_STATIC_SOURCE = ANALYSIS / "s22plus_fyg8_p337_process_v2_candidate_static.py"
P337_STATIC_SOURCE_IDENTITY = {
    "size": 19_522,
    "sha256": "fba6c13a26cfbb5183393b7f1356e3c7f7595cd7b6cc5c0e118214100a8f7bc9",
}
...
SCHEMA = "s22plus_fyg8_p338_process_v2_candidate_static_v1"
VERDICT = "PASS_P338_PROCESS_V2_CANDIDATE_STATIC_HOST_ONLY"
...
# P337's exact-loaded P336 static seam imports the consumed long-idle action at
# module load.  Keep that historical import inert here: P338 deliberately has
...
def _load_p337_static() -> types.ModuleType:
    payload = _stable(
        P337_STATIC_SOURCE,
    ...
    module = types.ModuleType("s22plus_fyg8_p337_static_bound_for_p338")
    module.__file__ = str(P337_STATIC_SOURCE)
    module.__package__ = ""
    aliases = {
    ...
    # Keep an already-imported consumed P337/P336 graph from leaking globals
    # into this exact-loaded static seam.  The fresh P338 modules remain
    ...
    try:
        ...
        exec(compile(payload, str(P337_STATIC_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
...
_P337 = _load_p337_static()
canonical = _P337.canonical
publish = _P337.publish
...
def _observer_projection() -> dict[str, Any]:
    ...
    value.update(
        {
            "source": {
                "path": _relative(P338_OBSERVER_SOURCE),
                **P338_OBSERVER_SOURCE_IDENTITY,
            },
            "runtime_source": {
                "path": _relative(P338_RUNTIME_SOURCE),
...
P338_RUNTIME_SOURCE = Path(runtime.__file__).resolve()
P338_OBSERVER_SOURCE = Path(observer.__file__).resolve()
...
def _validate_candidate(value: dict[str, Any], built: dict[str, Any]) -> None:
    ...
    if (
        ...
        != "s22plus_fyg8_p338_boot_only_open_read_branch_package_v1"
    ):
...
def _project(built: dict[str, Any]) -> dict[str, Any]:
    ...
    source_closure = {
        ...
        "p338_artifact_identity": _source_receipt(
            Path(artifact.__file__).resolve(), "P3.38 artifact identity"
        ),
        "p338_open_read_branch_acm_observer": _source_receipt(
            P338_OBSERVER_SOURCE, "P3.38 observer", 2 << 20, P338_OBSERVER_SOURCE_IDENTITY
        ),
        "p338_open_read_branch_runtime": _source_receipt(
            P338_RUNTIME_SOURCE, "P3.38 runtime", 2 << 20, P338_RUNTIME_SOURCE_IDENTITY
        ),
        "p338_stock_candidate_build": _source_receipt(
            Path(candidate_build.__file__).resolve(), "P3.38 builder"
        ),
        "p338_stock_process_v2_adapter": _source_receipt(
            Path(adapter.__file__).resolve(), "P3.38 adapter"
        ),
        "p337_candidate_static": _source_receipt(
            P337_STATIC_SOURCE, "P3.37 candidate static", 2 << 20, P337_STATIC_SOURCE_IDENTITY
        ),
...
def build_result() -> dict[str, Any]:
    try:
        ...
        if built.get("schema") != "s22plus-fyg8-p338-stock-candidate-build-v1" or built.get("run_id_hex") != RUN_ID:
