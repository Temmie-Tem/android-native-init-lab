#!/usr/bin/env python3
"""Build the P3.36 boot-only long-idle candidate (host-only).

The exact P3.35 builder is loaded into a private module graph and retained as
the packaging engine.  P3.36 reopens only the verified P3.35 result/source
closure, transforms its one runtime include through the P3.36 runtime, and
binds the P3.36 observer.  The package remains A/B-equal and boot-only; this
...
P335_BUILDER_SOURCE = ANALYSIS / "s22plus_fyg8_p335_stock_candidate_build.py"
P335_BUILDER_IDENTITY = {
    "size": 25_891,
    "sha256": "902ed12163a8f3c73f4dc6df6548e860cdbb8a85d23724c480711c2771fe136f",
}
P335_OUTPUT = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p335/"
    "stock-candidate-build-v1-20260904-03"
)
P335_RESULT = P335_OUTPUT / "result.json"
P335_RESULT_IDENTITY = {
    "size": 56_292,
...
P331_RUN_ID_HEX = P336_RUN_ID_HEX
P331_RUN_ID = P336_RUN_ID
P330_RUN_ID_HEX = P336_RUN_ID_HEX
P330_RUN_ID = P336_RUN_ID
...
P336_RUNTIME_SOURCE = REVALIDATION / "s22plus_fyg8_p336_long_idle_runtime.py"
P336_OBSERVER_SOURCE = REVALIDATION / "s22plus_fyg8_p336_long_idle_acm_observer.py"
P336_ACTION_SOURCE = REVALIDATION / "s22plus_fyg8_p336_long_idle_action.py"
...
P336_ACTION_SOURCE_IDENTITY = {
    "size": P336_ACTION_SOURCE.stat().st_size,
    "sha256": hashlib.sha256(P336_ACTION_SOURCE.read_bytes()).hexdigest(),
}
# Compatibility labels consumed by the exact-loaded P3.35 static seam.
P335_RUNTIME_SOURCE_IDENTITY = dict(P336_RUNTIME_SOURCE_IDENTITY)
P335_OBSERVER_SOURCE_IDENTITY = dict(P336_OBSERVER_SOURCE_IDENTITY)
...
SCHEMA = "s22plus-fyg8-p336-stock-candidate-build-v1"
VERDICT = "PASS_P336_STOCK_CANDIDATE_BUILD_H0_LONG_IDLE"
STATUS = "IMPLEMENTED_H0_LONG_IDLE_REVIEW_PENDING"
TARGET = {"model": "SM-S906N", "codename": "g0q", "build": "S906NKSS7FYG8"}


class AuditError(RuntimeError):
    """The exact P3.35 lineage or P3.36 closure differs."""
...
def _load_p335_builder() -> types.ModuleType:
    payload = _stable(
        P335_BUILDER_SOURCE,
        "P3.35 builder source",
        2 << 20,
        P335_BUILDER_IDENTITY,
    )
    module = types.ModuleType("s22plus_fyg8_p335_builder_bound_for_p336")
    module.__file__ = str(P335_BUILDER_SOURCE)
    module.__package__ = ""
    aliases = {
    ...
    try:
        ...
        exec(compile(payload, str(P335_BUILDER_SOURCE), "exec", dont_inherit=True), module.__dict__)  # noqa: S102
    except Exception as exc:
        raise AuditError("P3.35 builder source failed to load") from exc
    finally:
    ...
    if getattr(module, "P335_RUN_ID_HEX", None) != P335_RUN_ID_HEX:
        raise AuditError("P3.35 builder binding differs")
    return module


_P335 = _load_p335_builder()
_P328 = _P335._P328
_ENGINE = _P335._ENGINE
_P335_LOAD_PACKAGER = _P335._load_packager
...
def _rebind_engine() -> None:
    ...
    for module in modules:
        for name, value in tuple(vars(module).items()):
            if value == P335_RUN_ID:
                setattr(module, name, P336_RUN_ID)
            elif value == P335_RUN_ID_HEX:
                ...
                for key, current in tuple(defaults.items()):
                    if current == P335_RUN_ID:
                        defaults[key] = P336_RUN_ID
                        changed = True
                    elif current == P335_RUN_ID_HEX:
    ...
    _ENGINE.P321_OUTPUT = P335_OUTPUT
    _ENGINE.P322_RUN_ID = P336_RUN_ID
    _ENGINE.P322_RUN_ID_HEX = P336_RUN_ID_HEX
    _ENGINE.P322_ARTIFACT_SOURCE = artifact.P336_ARTIFACT_SOURCE
    _ENGINE.P322_REPAIR_SOURCE = P336_RUNTIME_SOURCE
    _ENGINE.P322_ADAPTER_SOURCE = adapter.P336_ADAPTER_SOURCE
...
def _predecessor() -> tuple[dict[str, Any], bytes, bytes]:
    ...
    value = _strict_json(payload, "P3.35 result")
    candidate = value.get("phase2", {}).get("candidate")
    if (
        value.get("schema") != "s22plus-fyg8-p335-stock-candidate-build-v1"
        or value.get("run_id_hex") != P335_RUN_ID_HEX
        or value.get("target") != TARGET
        or not isinstance(candidate, dict)
        or candidate.get("a") != candidate.get("b")
        or candidate.get("a", {}).get("ap_tar_md5") != P335_AP_IDENTITY
        ...
        raise AuditError("P3.35 predecessor result differs")
    for name, expected in sorted(value["source_closure"].items()):
        _stable(
            P335_OUTPUT / "stock-sources" / name,
            f"P3.35 source {name}",
            2 << 20,
            expected,
            mode=0o400,
    ...
    for label in ("a", "b"):
        _stable(
            P335_OUTPUT / f"candidate-{label}/odin4/AP.tar.md5",
            f"P3.35 candidate {label}",
            128 << 20,
            P335_AP_IDENTITY,
        )
    source = _stable(
...
def _current_sources() -> dict[str, bytes]:
    paths = {
        ...
        "p328_artifact_identity.py": artifact.P336_ARTIFACT_SOURCE,
        "p328_stock_process_v2_adapter.py": adapter.P336_ADAPTER_SOURCE,
        "p328_authenticated_exec_runtime.py": P336_RUNTIME_SOURCE,
        "p328_authenticated_acm_observer.py": P336_OBSERVER_SOURCE,
    }
    expected = {
        "p328_authenticated_exec_runtime.py": P336_RUNTIME_SOURCE_IDENTITY,
        "p328_authenticated_acm_observer.py": P336_OBSERVER_SOURCE_IDENTITY,
    }
    return {
        name: _stable(path, f"P3.36 source {name}", 2 << 20, expected.get(name))
...
def _active_key() -> bytes:
    ...
    if type(key) is not bytes or len(key) != artifact.AUTH_KEY_SIZE:
        raise AuditError("P3.36 auth key is not bound")
    return key
...
def _copy_source_closure(
    ...
    if repair is None:
        raise AuditError("P3.35 runtime source is absent")
    _ENGINE.p321._fsync_directory(source_root)
    return receipts, repair


def _load_packager() -> tuple[Any, Any, bytes]:
    helper, packager, source = _P335_LOAD_PACKAGER()
    packager.RUN_ID = P336_RUN_ID
...
def _normalize_result(value: dict[str, Any]) -> dict[str, Any]:
    try:
        result = copy.deepcopy(_P335._normalize_result(value))
    except Exception as exc:
        raise AuditError(f"P3.35 inherited result normalization failed: {exc}") from exc
    candidate = result.get("phase2", {}).get("candidate")
    if not isinstance(candidate, dict):
        raise AuditError("P3.36 candidate projection is absent")
    ...
    if (
        ...
        raise AuditError("P3.36 candidate repeats a consumed predecessor or differs A/B")
    for label in ("a", "b"):
    ...
    result.update(
        {
            ...
            "status": STATUS,
            "run_id_hex": P336_RUN_ID_HEX,
            "differs_from_consumed_p335": True,
        }
    )
    lineage = result.setdefault("lineage", {})
    lineage.update(
        {
            "construction_base_run_id": P335_RUN_ID_HEX,
            "construction_base_result": P335_RESULT_IDENTITY,
            "construction_base_ap": P335_AP_IDENTITY,
            "predecessor_run_id": P335_RUN_ID_HEX,
            "predecessor_result": P335_RESULT_IDENTITY,
            "predecessor_ap": P335_AP_IDENTITY,
            "fresh_run_id": P336_RUN_ID_HEX,
            "p335_source_closure_reopened": True,
        }
    )
    preservation = result.setdefault("preservation", {})
    preservation.update(
        {
            "p335_consumed_candidate_distinct": True,
            "p335_source_closure_reopened": True,
            "runtime_delta_long_idle_host_resync_only": True,
            "long_idle_host_resync": True,
            "later_action_open_before_resync": True,
            "later_action_max_preamble_pairs": observer.MAX_PREAMBLE_PAIRS,
            "later_action_max_resync_bytes": observer.MAX_RESYNC_BYTES,
    ...
    result["compatibility_labels"] = {
        "engine": "P335 exact-loaded",
        "runtime_input": "exact P335 runtime include",
        "wire_namespace": "unchanged P335 authenticated framing and fixed command ABI",
    ...
    result["limitations"] = [
        "P336 reuses the exact P335 packaging engine and transforms only its runtime include.",
        "Initial proof retains the P335 three-session same-FD lease and one host close/reopen.",
        "A later action sends one OPEN before a bounded exact P336 preamble resynchronization.",
...
# The inherited P3.35 engine resolves these callbacks through its own module
# globals.  Rebinding only the isolated graph keeps P3.35 imports untouched.
for module in (_P335, _P328, _ENGINE):
...
def _validate_repair(before: bytes, after: bytes) -> dict[str, Any]:
    ...
    if expected != after:
        raise AuditError("P3.36 transformed runtime differs")
    return receipt


def _require_builder() -> None:
    if _P335 is None or _P328 is None or _ENGINE is None:
        raise AuditError("P3.36 builder dependencies are unavailable")
    adapter.audit()
