...
/*
 * Samsung S22+ native-init M12 M5-floor USB-ACM candidate.
 *
 * M12 keeps the M11 freestanding PID1/configfs/role-force/park shape but
 * lowers the module list to the 24 modules common to M5 and M11. Runtime
 * module binaries are read from stock vendor_boot /lib/modules; only the text
 * module list is added to boot.
 */
...
#define MODULES_M5_FLOOR_BUF 8192
...
static const char k_marker[] =
    "S22_NATIVE_INIT_USB_ACM_M12 version=0.1 pid1=direct "
    "runtime=freestanding raw_syscalls=1 "
    "modules_m5_floor=/s22plus_m12_m5_floor.modules "
    "module_source=stock_vendor_boot_ramdisk module_list=boot_ramdisk_m5_floor "
...
static void setup_minimal_fs(void) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=mounts proc_rc=");
...
static void emit_module(size_t index, const char *name, long open_rc, long finit_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=module index=");
...
static void load_m5_floor_modules(void) {
    static char text[MODULES_M5_FLOOR_BUF];
    long fd = sys_openat(AT_FDCWD, "/s22plus_m12_m5_floor.modules", O_RDONLY | O_CLOEXEC, 0);
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=modules_m5_floor open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    }
    long n = sys_read((int)fd, text, MODULES_M5_FLOOR_BUF - 1);
    (void)sys_close((int)fd);
    if (n < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=modules_m5_floor read_rc=");
        sb_put_i64(&sb, n);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=modules_m5_floor_done count=");
    sb_put_u64(&sb, (uint64_t)index);
    sb_putc(&sb, '\n');
    emit_buf(&sb);
}
...
static long write_attr(const char *path, const char *value) {
    ...
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=write path=");
        sb_puts(&sb, path);
        sb_puts(&sb, " open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
...
static void force_usb_roles_device(void) {
    ...
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=usb_role open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
        ...
        while (pos < nread) {
            ...
            if (!cstr_eq(name, ".") && !cstr_eq(name, "..")) {
                ...
                if (base + nlen + sizeof("/role") < sizeof(role_path)) {
                    ...
                    struct sbuf sb = {.data = {0}, .len = 0};
                    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=usb_role name=");
                    sb_puts(&sb, name);
                    sb_puts(&sb, " rc=");
                    sb_put_i64(&sb, rc);
                    sb_putc(&sb, '\n');
    ...
    (void)sys_close((int)fd);
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=usb_role_done count=");
    sb_put_u64(&sb, count);
    sb_putc(&sb, '\n');
    emit_buf(&sb);
}
...
static void emit_gadget(long symlink_rc, int have_udc, const char *udc, long udc_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=acm_gadget symlink_rc=");
...
static int create_acm_gadget(void) {
    ...
    if (read_trim("/config/usb_gadget/g1/UDC", current_udc, sizeof(current_udc)) == 0 && current_udc[0] != '\0') {
        emit("S22_NATIVE_INIT_USB_ACM_M12 phase=acm_gadget already_bound=1\n");
        return 1;
    }
    ...
    (void)write_attr("/config/usb_gadget/g1/strings/0x409/product", "S22 Native Init M12 ACM");
    (void)write_attr("/config/usb_gadget/g1/strings/0x409/serialnumber", "S22M12ACM0001");
...
static void handle_command(const char *cmd, int fd) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=acm_command cmd=");
    sb_puts(&sb, cmd);
    sb_putc(&sb, '\n');
    emit_buf(&sb);

    const char ack[] = "S22_NATIVE_INIT_USB_ACM_M12 ACK status park\n";
...
static void read_serial_commands(int fd, char *cmd_buf, size_t *cmd_len, size_t cmd_cap) {
    ...
    for (;;) {
        ...
        if (n < 0) {
            struct sbuf sb = {.data = {0}, .len = 0};
            sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=acm_read rc=");
            sb_put_i64(&sb, n);
            sb_putc(&sb, '\n');
            emit_buf(&sb);
            return;
        ...
        for (long i = 0; i < n; ++i) {
            ...
            if (*cmd_len + 1 < cmd_cap) {
                ...
                cmd_buf[cmd_cap - 1] = '\0';
                emit("S22_NATIVE_INIT_USB_ACM_M12 phase=acm_command_overflow\n");
                *cmd_len = 0;
            }
        }
    }
...
static void emit_park(unsigned int tick, int bound, int tty_ready, long open_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M12 phase=park tick=");
    sb_put_u64(&sb, tick);
    sb_puts(&sb, " bound=");
...
static void serial_probe_loop(void) {
    ...
    for (unsigned int tick = 0;; ++tick) {
        ...
        long fd = sys_openat(AT_FDCWD, "/dev/ttyGS0", O_RDWR | O_NOCTTY | O_NONBLOCK | O_CLOEXEC, 0);
        if (fd >= 0) {
            const char banner[] = "S22_NATIVE_INIT_USB_ACM_M12 READY\n";
...
static void m12_main(void) {
    setup_minimal_fs();
    emit(k_marker);
    load_m5_floor_modules();
    force_usb_roles_device();
