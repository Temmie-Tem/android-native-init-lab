...
static const char *decision_for_results(const struct child_result *a,
                                        ...
                                        int have_b,
                                        int parent_timeout) {
    if (parent_timeout || (have_a && a->timeout)) {
        return "bbt-timeout";
    }
    if (!have_a || !have_b) {
    ...
    if (a->set_context_mgr_rc != 0) {
        return a->set_context_mgr_errno == EBUSY ?
               "bbt-context-mgr-ebusy" : "bbt-context-mgr-fail";
    }
...
static void print_result(const struct child_result *a,
                         ...
                         const char *decision,
                         int parent_timeout) {
    printf("bbt.helper=a90_binder_target_bbt\n");
    printf("bbt.helper_version=1\n");
    printf("bbt.mode=two-process-oneway-zero-object\n");
    printf("bbt.path=%s\n", path);
    ...
    printf("bbt.exit_rc=%d\n", strcmp(decision, "bbt-target-ok") == 0 ? 0 : 1);
}
...
int main(int argc, char **argv) {
    ...
    if (map_length == 0 || map_length > MAX_MAP_LENGTH || (map_length % 4096UL) != 0) {
        printf("bbt.helper=a90_binder_target_bbt\n");
        printf("bbt.decision=bbt-helper-invalid-length\n");
    ...
    if (read_full(ready_pipe[0], &ready, sizeof(ready)) != 0 ||
        ...
        return strcmp(decision, "bbt-target-ok") == 0 ? 0 : 1;
    }
    result_a = ready.result;
    have_a = 1;
