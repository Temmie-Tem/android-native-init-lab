/*
 * a90_bpf_workqueue_exec_stack_sample_ring — bounded workqueue execute stack sampler.
 *
 * Captures workqueue:workqueue_execute_start tracepoint records and the BPF
 * stackid for the executing worker context. The tracepoint record contributes
 * only scalar fields already copied by the kernel tracepoint; the stack frames
 * come from the sanctioned bpf_get_stackid helper and STACK_TRACE map.
 *
...
#define A90_VERSION "a90_bpf_workqueue_exec_stack_sample_ring v2277"
...
int main(int argc, char **argv) {
    ...
    printf("%s\n", A90_VERSION);
    printf("allow_attach=%d duration_ms=%d print_limit=%d verbose=%d max_samples=%d stack_depth=%d stack_max_entries=%d\n",
    ...
    printf("result=v2277-workqueue-exec-stack-sample-ring-complete\n");
    close(prog);
    close(stack_map);
    close(samples_map);
    close(stats_map);
