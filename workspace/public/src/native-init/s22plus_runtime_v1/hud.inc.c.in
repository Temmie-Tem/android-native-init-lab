...
struct hud1_state {
    long pid,collector;
    int collector_reaped,collector_signalled,collector_wait_unknown,collector_status;
...
static void hud1_stop(struct hud1_state *h) {
    hud1_close(&h->input);
    if(h->collector>0 && !h->collector_reaped && !h->collector_wait_unknown && !h->collector_signalled) {
        h->collector_signalled=1;long rc=syscall6(129,h->collector,9,0,0,0,0);
        hud1_number(h,rc<0?"METRICS_SIGNAL_ERROR ":"METRICS_SIGNAL_ATTEMPT ",rc<0?(unsigned long)-rc:(unsigned long)h->collector);
    }
...
static void hud1_start(struct hud1_state *h) {
    int pair[2]={-1,-1},out[2]={-1,-1},metrics[2]={-1,-1};
    ...
    rc=syscall6(199,1,5|O_NONBLOCK|O_CLOEXEC,0,(long)(uintptr_t)metrics,0,0);
    if(rc<0)goto fail;
    if(metrics[0]<=2 || metrics[1]<=2){rc=-P260_EPROTO;goto fail;}
    long collector=sys_clone();if(collector<0){rc=collector;goto fail;}
    if(!collector) {
        if(p328_setsid()<0)sys_exit(126);
        long null=sys_openat("/dev/null",O_CLOEXEC,0);if(null<0)sys_exit(126);
        if(p328_dup_to((int)null,0)!=0 || p328_dup_to(metrics[0],1)!=1 || p328_dup_to(out[1],2)!=2)sys_exit(126);
        if(syscall6(436,3,0xffffffffU,0,0,0,0)<0)sys_exit(126);
        char *argv[]={"/s22-display","--collect-status",(char *)rc1_run_id_ascii,NULL};
        char *env[]={"PATH=/bin","HOME=/","TERM=dumb",NULL};
        (void)sys_execve("/s22-display",argv,env);sys_exit(126);
    }
    h->collector=collector;hud1_number(h,"METRICS_START ",(unsigned long)collector);
    ...
    if(!pid) {
        ...
        if(p328_dup_to(metrics[1],3)!=3)sys_exit(126);
        if(syscall6(436,4,0xffffffffU,0,0,0,0)<0)sys_exit(126);
        char *argv[]={"/s22-display","--supervised-drm",(char *)rc1_run_id_ascii,NULL};
        char *env[]={"PATH=/bin","HOME=/","TERM=dumb",NULL};
        (void)sys_execve("/s22-display",argv,env);sys_exit(126);
    }
    (void)sys_close(metrics[0]);(void)sys_close(metrics[1]);
...
fail:
    for(unsigned i=0;i<2;i++){if(pair[i]>=0)(void)sys_close(pair[i]);if(out[i]>=0)(void)sys_close(out[i]);if(metrics[i]>=0)(void)sys_close(metrics[i]);}
    h->failed=1;hud1_number(h,"HUD_START_ERROR ",(unsigned long)-rc);hud1_stop(h);
}
static void hud1_tick(struct hud1_state *h,uint64_t now,unsigned console_state) {
    if(h->collector>0 && !h->collector_reaped && !h->collector_wait_unknown) {
        long n=sys_wait4(h->collector,&h->collector_status,WNOHANG);
        if(n==h->collector){h->collector_reaped=1;hud1_number(h,"METRICS_EXIT ",(unsigned)h->collector_status);}
        else if(n!=0){h->collector_wait_unknown=1;hud1_number(h,"METRICS_WAIT_ERROR ",(unsigned long)(n<0?-n:n));}
    }
    if(h->pid<=0)return;
    if(h->output>=0) {
