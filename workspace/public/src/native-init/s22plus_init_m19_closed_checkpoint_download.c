...
/*
 * Samsung S22+ native-init M19 dependency-closed checkpoint/download probe.
 *
 * This is an observation-channel candidate, not an ACM bring-up candidate.
 * It loads the first N modules from a dependency-closed USB-tail list, then
...
#ifndef M19_PREFIX_LIMIT
#define M19_PREFIX_LIMIT 0
#endif

#ifndef M19_PREFIX_LABEL
#define M19_PREFIX_LABEL "C000"
#endif
...
static const char k_marker[] =
    "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT version=0.1 pid1=direct "
    "runtime=freestanding raw_syscalls=1 "
    "prefix_label=" M19_PREFIX_LABEL " prefix_limit=" STR(M19_PREFIX_LIMIT) " "
    "modules_closed=/s22plus_m19_closed_usb.modules module_count=150 "
    "module_source=stock_vendor_boot_ramdisk module_list=boot_ramdisk_closed_usb "
    "module_injection=list_only observation=checkpoint-download "
...
static void setup_minimal_fs(void) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=mounts prefix_label=");
    sb_puts(&sb, M19_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M19_PREFIX_LIMIT);
    sb_puts(&sb, " proc_rc=");
...
static void emit_module(size_t index, const char *name, long open_rc, long finit_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=module prefix_label=");
    sb_puts(&sb, M19_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M19_PREFIX_LIMIT);
    sb_puts(&sb, " index=");
...
static void load_prefix_modules(void) {
    const size_t prefix_limit = (size_t)M19_PREFIX_LIMIT;
    if (prefix_limit == 0U) {
        emit("S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=modules_prefix_skipped prefix_label="
             M19_PREFIX_LABEL " prefix_limit=0\n");
        return;
    }

    static char text[MODULES_PREFIX_BUF];
    long fd = sys_openat(AT_FDCWD, "/s22plus_m19_closed_usb.modules", O_RDONLY | O_CLOEXEC, 0);
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=modules_prefix open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    ...
    if (n < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=modules_prefix read_rc=");
        sb_put_i64(&sb, n);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=modules_prefix_done prefix_label=");
    sb_puts(&sb, M19_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M19_PREFIX_LIMIT);
...
__attribute__((noreturn)) static void reboot_download_then_park(void) {
    emit("S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=checkpoint prefix_label="
         M19_PREFIX_LABEL " prefix_limit=" STR(M19_PREFIX_LIMIT) " action=reboot_download\n");
    ...
    long rc = sys_reboot(
        ...
        k_download_arg);
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M19_CLOSED_CHECKPOINT phase=reboot_returned prefix_label=");
    sb_puts(&sb, M19_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M19_PREFIX_LIMIT);
    sb_puts(&sb, " rc=");
