 ...
 * plus the F0/F1/F2/F3 content-changing ladder preparation commands (design §12).
 *
 ...
 * the captured before.full image before any reboot. F2 is token-gated and writes/verifies the
 * planned target full image for a host-controlled reboot into the candidate, restoring before.full
 * only if the target write fails. F3 uses the same guarded leave-target primitive for a staged
 * rollback image. Any anomaly is reported as a STOP. E/F rungs are token-gated. Output lines are
 * "A90BWE*" / "A90BWF*" key=value.
