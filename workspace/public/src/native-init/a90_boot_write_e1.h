 ...
 * PARTNAME=boot, picks one E1 target or four E2 targets spread across the observed all-zero sector
 * population in tail slack (past the boot-image content, 1 MiB before the partition end to avoid
 * any AVB footer), REFUSES unless every selected sector reads all-zero, writes the exact bytes it
 * just read back to the same offset,
