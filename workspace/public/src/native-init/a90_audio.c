...
static void audio_pcm_apply_gain(int16_t *buffer,
                                 size_t sample_count,
                                 int pcm_gain_milli) {
    size_t index;

    if (buffer == NULL || sample_count == 0 || pcm_gain_milli >= AUDIO_PCM_GAIN_MILLI_DEFAULT) {
        return;
    }
    if (pcm_gain_milli < 0) {
        pcm_gain_milli = 0;
    ...
    for (index = 0; index < sample_count; ++index) {
        int scaled = ((int)buffer[index] * pcm_gain_milli) / 1000;
        if (scaled > 32767) {
            scaled = 32767;
        } else if (scaled < -32768) {
            scaled = -32768;
        }
        buffer[index] = (int16_t)scaled;
    }
}
...
static bool audio_pcm_file_path_allowed(const char *path) {
    if (path == NULL || path[0] != '/' || audio_setcal_path_has_dotdot(path)) {
        return false;
    }
    return audio_setcal_path_has_prefix(path, AUDIO_SETCAL_RUNTIME_PREFIX);
...
static bool audio_pcm_file_is_badapple_full_song(const char *path) {
    return path != NULL &&
           (strcmp(path, AUDIO_BADAPPLE_FULL_PCM_PATH) == 0 ||
...
static int audio_read_exact_fd(int fd, void *buffer, size_t bytes) {
    size_t done = 0;
...
static int audio_pcm_peak_limit(int amplitude_milli) {
    if (amplitude_milli < 0) {
        return 0;
    }
    if (amplitude_milli >= 1000) {
        return 32767;
    }
    return (32767 * amplitude_milli) / 1000;
}

static int audio_pcm_file_open_validated(const struct audio_speaker_profile *profile,
                                         const char *pcm_file_path,
    ...
    a90_console_printf("audio.play.pcm_file.expected_bytes=%lld\r\n", expected_bytes);
    if (profile == NULL || pcm_file_path == NULL || expected_bytes <= 0 || out_fd == NULL) {
        a90_console_printf("audio.play.pcm_file.error=bad-argument\r\n");
        return -EINVAL;
    }
    if (!path_allowed) {
        a90_console_printf("audio.play.pcm_file.error=path-not-allowed\r\n");
        return -EPERM;
    }

    errno = 0;
    fd = open(pcm_file_path, O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
    a90_console_printf("audio.play.pcm_file.open.rc=%d errno=%d\r\n", fd >= 0 ? 0 : -1, fd < 0 ? errno : 0);
    if (fd < 0) {
        return -errno;
    }
    if (fstat(fd, &st) < 0) {
        rc = -errno;
        a90_console_printf("audio.play.pcm_file.error=fstat-failed errno=%d\r\n", errno);
        close(fd);
        return rc;
    }
    a90_console_printf("audio.play.pcm_file.regular=%d\r\n", S_ISREG(st.st_mode) ? 1 : 0);
    a90_console_printf("audio.play.pcm_file.size=%lld\r\n", (long long)st.st_size);
    if (!S_ISREG(st.st_mode)) {
        a90_console_printf("audio.play.pcm_file.error=not-regular\r\n");
        close(fd);
        return -EINVAL;
    }
    ...
    if (pcm_gain_milli < 0 || pcm_gain_milli > AUDIO_PCM_GAIN_MILLI_MAX) {
        a90_console_printf("audio.play.pcm_file.error=bad-pcm-gain\r\n");
        close(fd);
        return -EINVAL;
    }
    ...
    while (remaining > 0) {
        ...
        if ((want & 1U) != 0U) {
            a90_console_printf("audio.play.pcm_file.error=unaligned-read\r\n");
            close(fd);
            return -EINVAL;
        }
        rc = audio_read_exact_fd(fd, scan_buffer, want);
        if (rc < 0) {
            a90_console_printf("audio.play.pcm_file.error=read-failed rc=%d\r\n", rc);
            close(fd);
            return rc;
        }
        sample_count = want / sizeof(int16_t);
        for (index = 0; index < sample_count; ++index) {
            int raw = (int)scan_buffer[(index * 2U)] | ((int)scan_buffer[(index * 2U) + 1U] << 8);
            int sample = raw >= 0x8000 ? raw - 0x10000 : raw;
    ...
    scaled_peak_abs = (int)(((long long)peak_abs * (long long)pcm_gain_milli + 999LL) / 1000LL);
    a90_console_printf("audio.play.pcm_file.peak_abs=%d\r\n", peak_abs);
    a90_console_printf("audio.play.pcm_file.pcm_gain_milli=%d\r\n", pcm_gain_milli);
    a90_console_printf("audio.play.pcm_file.scaled_peak_abs=%d\r\n", scaled_peak_abs);
    a90_console_printf("audio.play.pcm_file.peak_limit=%d\r\n", peak_limit);
    a90_console_printf("audio.play.pcm_file.amplitude_within_cap=%d\r\n", scaled_peak_abs <= peak_limit ? 1 : 0);
    if (scaled_peak_abs > peak_limit) {
        a90_console_printf("audio.play.pcm_file.error=amplitude-cap-exceeded\r\n");
        close(fd);
        return -EPERM;
    }
    if (lseek(fd, 0, SEEK_SET) < 0) {
        rc = -errno;
        a90_console_printf("audio.play.pcm_file.error=rewind-failed errno=%d\r\n", errno);
        close(fd);
        return rc;
    }
    a90_console_printf("audio.play.pcm_file.validated=1\r\n");
    *out_fd = fd;
    return 0;
}
...
static int audio_play_execute_pcm(const struct audio_speaker_profile *profile,
    ...
    a90_console_printf("audio.play.execute.total_frames=%lld\r\n", total_frames);
    ...
    if (use_pcm_file) {
        rc = audio_pcm_file_open_validated(profile,
                                           pcm_file_path,
                                           (long long)total_frames * (long long)frame_bytes,
                                           amplitude_milli,
                                           pcm_gain_milli,
    ...
    audio_play_async_statusf("audio.play.worker.total_frames=%lld\n", total_frames);
    ...
    while (frames_done < total_frames) {
        int frames_this_chunk = (int)(total_frames - frames_done);
        ...
        if (use_pcm_file) {
            ...
            if (rc < 0) {
                ...
                a90_console_printf("A90_LISTEN_WINDOW_END rc=%d chunks=%d frames=%lld bytes=%lld\r\n",
                                   rc,
                                   chunk_count,
                                   frames_done,
                                   (long long)frames_done * (long long)frame_bytes);
            ...
            audio_pcm_apply_gain(buffer, (size_t)frames_this_chunk * (size_t)profile->channels, pcm_gain_milli);
        } else {
    ...
    a90_console_printf("A90_LISTEN_WINDOW_END rc=%d chunks=%d frames=%lld bytes=%lld\r\n",
                       ...
                       frames_done,
                       (long long)frames_done * (long long)frame_bytes);
    a90_console_printf("audio.play.execute.done=%d chunks=%d frames=%lld bytes=%lld\r\n",
                       rc == 0 ? 1 : 0,
                       chunk_count,
                       frames_done,
                       (long long)frames_done * (long long)frame_bytes);
...
static void audio_play_async_reset_status(const struct audio_speaker_profile *profile,
    ...
    audio_play_async_statusf("audio.play.worker.pcm_gain_milli=%d\n", pcm_gain_milli);
...
static int audio_play_start_worker(const struct audio_speaker_profile *profile,
    ...
    if (pid == 0) {
        ...
        rc = audio_play_execute_integrated(profile,
                                           mode,
                                           amplitude_milli,
                                           duration_ms,
                                           manifest_path,
...
static int audio_play_cmd(char **argv, int argc) {
    ...
    int requested_pcm_gain_milli = AUDIO_PCM_GAIN_MILLI_DEFAULT;
    int amplitude_milli = 0;
    int duration_ms = 0;
    int effective_duration_cap_ms = 0;
    int pcm_gain_milli = AUDIO_PCM_GAIN_MILLI_DEFAULT;
    char pcm_path[64];
    bool pcm_node_ready;
    int argi;
    ...
    for (argi = 2; argi < argc; ++argi) {
        if (argv == NULL || argv[argi] == NULL) {
            a90_console_printf("usage: audio play [profile] [--mode probe|listen] [--amplitude-milli N] [--duration-ms N] [--manifest PATH] [--pcm-file PATH] [--pcm-gain-milli N] [--dry-run|--execute]\r\n");
            return -EINVAL;
        }
        if (strcmp(argv[argi], "--dry-run") == 0) {
            execute_mode = false;
            ...
            ++argi;
        } else if (strcmp(argv[argi], "--pcm-gain-milli") == 0) {
            if (argi + 1 >= argc || !audio_parse_nonnegative_int(argv[argi + 1], &requested_pcm_gain_milli)) {
                a90_console_printf("usage: audio play [profile] [--mode probe|listen] [--amplitude-milli N] [--duration-ms N] [--manifest PATH] [--pcm-file PATH] [--pcm-gain-milli N] [--dry-run|--execute]\r\n");
                return -EINVAL;
            }
            ++argi;
            ...
            manifest_path = argv[++argi];
        } else if (strcmp(argv[argi], "--pcm-file") == 0) {
            if (argi + 1 >= argc || argv[argi + 1] == NULL) {
                a90_console_printf("usage: audio play [profile] [--mode probe|listen] [--amplitude-milli N] [--duration-ms N] [--manifest PATH] [--pcm-file PATH] [--pcm-gain-milli N] [--dry-run|--execute]\r\n");
                return -EINVAL;
            }
    ...
    a90_console_printf("audio.play.pcm_file=%s\r\n",
                       pcm_file_requested ? pcm_file_path : "-");
    ...
    a90_console_printf("audio.play.pcm_gain_milli=%d\r\n", pcm_gain_milli);
    a90_console_printf("audio.play.pcm_gain.attenuation_only=1\r\n");
    ...
    a90_console_printf("audio.play.safety.duration_within_cap=%d\r\n",
                       duration_ms <= effective_duration_cap_ms ? 1 : 0);
    ...
    if (pcm_gain_milli < 0 || pcm_gain_milli > AUDIO_PCM_GAIN_MILLI_MAX) {
        a90_console_printf("audio.play.refused=pcm-gain-out-of-range\r\n");
        return -EPERM;
    }
    if (amplitude_milli > profile->amplitude_cap_milli || duration_ms > effective_duration_cap_ms) {
        a90_console_printf("audio.play.refused=safety-cap-exceeded\r\n");
        return -EPERM;
    }
    if (pcm_file_requested && !pcm_file_allowed) {
...
static void audio_print_core_promotion_status(const struct audio_speaker_profile *profile) {
    ...
    a90_console_printf("audio.status.productization.version=1\r\n");
    a90_console_printf("audio.status.productization.latest_run=%s\r\n", AUDIO_PRODUCTIZATION_LATEST_RUN);
    ...
    a90_console_printf("audio.status.feature.boot_chime.blocks_boot=0\r\n");
    a90_console_printf("audio.status.feature.boot_chime.validation_run=%s\r\n", AUDIO_BOOT_CHIME_VALIDATION_RUN);
