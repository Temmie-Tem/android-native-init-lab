...
_Static_assert(sizeof(struct status_metrics)==72,"metrics wire size");
...
static int status_collect(const char *identity) {
    ...
    for(unsigned sequence=1;sequence<=601;sequence++) {
        ...
        struct status_metrics m={.magic=0x31545353U,.sequence=sequence,.collected_ms=(uint64_t)stamp.tv_sec*1000+(uint64_t)stamp.tv_nsec/1000000};
