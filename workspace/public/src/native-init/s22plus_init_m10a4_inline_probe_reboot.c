...
/*
 * Samsung S22+ native-init M10A4 inline probe then reboot discriminator.
 *
 * M10A4 splits the M10A3 bootloop boundary. It removes the pre-reboot helper
 * call/return, but keeps inline stack work in _start before the reboot helper.
 * The first externally observable action remains exactly one reboot(2) syscall
...
void _start(void) {
    volatile uintptr_t stack_probe[2];
    stack_probe[0] = 0x346130316d323273ULL;
    stack_probe[1] = 0;
    asm volatile("" : : "r"(stack_probe[0]), "r"(stack_probe[1]) : "memory");
