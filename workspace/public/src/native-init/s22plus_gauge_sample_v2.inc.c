...
enum gauge_parse_result {
    GAUGE_PARSE_OK, GAUGE_PARSE_FORMAT, GAUGE_PARSE_SOURCE_ERROR,
    GAUGE_PARSE_SEQUENCE, GAUGE_PARSE_TIMESTAMP, GAUGE_PARSE_RAW_RANGE,
    GAUGE_PARSE_SOC, GAUGE_PARSE_VOLTAGE, GAUGE_PARSE_CURRENT,
    GAUGE_PARSE_VOLTAGE_RANGE
};
static enum gauge_parse_result gauge_parse_reason(const char *p,struct gauge_sample *out) {
    ...
    if(!gauge_unsigned(&p,"S22FG1 seq=",&s.sequence)||
       ...
       !gauge_signed(&p," current_ua=",&s.current_ua)||strcmp(p,"\n"))return GAUGE_PARSE_FORMAT;
    if(s.error)return GAUGE_PARSE_SOURCE_ERROR;
    if(!s.sequence||s.sequence>601)return GAUGE_PARSE_SEQUENCE;
    if(!s.start_ms)return GAUGE_PARSE_TIMESTAMP;
    if(s.valid>7||s.soc_raw>65535||s.voltage_raw>65535||s.current_raw>65535)return GAUGE_PARSE_RAW_RANGE;
    ...
    if(s.soc_permille!=soc)return GAUGE_PARSE_SOC;
    if(s.voltage_uv!=s.voltage_raw*625/8)return GAUGE_PARSE_VOLTAGE;
    if(s.current_ua!=current)return GAUGE_PARSE_CURRENT;
    if((s.valid&2)&&(s.voltage_uv<2000000||s.voltage_uv>5000000))return GAUGE_PARSE_VOLTAGE_RANGE;
    *out=s;return GAUGE_PARSE_OK;
}
