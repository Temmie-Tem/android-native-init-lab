/* P2.82 parent-worker and pull-up discriminator over the P2.60 E3 path. */

#include "s22plus_fyg8_p260_e3_runtime.inc.c"
#include "s22plus_fyg8_p282_trace_descriptor.h"
...
static long p282_run_cycle_role_helper(
    uint16_t operation,
    int unrelated_fd,
    ...
    for (;;) {
        ...
        if (malformed || p282_deadline_expired(deadline)) {
            if (!child_reaped) {
                (void)sys_kill(pid, SIGKILL);
                (void)sys_wait4(pid, &child_status, 0);
            }
            (void)sys_close(pipe_fds[0]);
            return malformed
    ...
    return rc != 0 ? rc : (long)record.result;
}
...
static long p282_cycle_stop(
    ...
    int unrelated_fd,
    const struct timespec64 *deadline) {
    long rc = p282_run_cycle_role_helper(
        P282_HELPER_OPERATION_NONE_WRITE,
        unrelated_fd,
        deadline);
    if (rc == P282_CONTROL_HELPER_SOURCE_CONTRADICTION) {
        struct p282_classification classification = {0};
        int classified = p282_control_classification(
            P282_STAGE_STOP, (unsigned int)rc, &classification);
        if (classified > 0) {
            p282_cycle_abort(
                cycle,
                P282_STAGE_STOP,
...
static unsigned int p282_cycle_restart(
    ...
    if (rc == P282_CONTROL_HELPER_SOURCE_CONTRADICTION) {
        struct p282_classification classification = {0};
        int classified = p282_control_classification(
            P282_STAGE_RESTART, (unsigned int)rc, &classification);
        if (classified > 0) {
            p282_cycle_abort(
                cycle,
                P282_STAGE_RESTART,
    ...
    if (!mode_peripheral) {
        p282_restart_exact_failure(
            ...
            P282_DETAIL_PARENT_MODE_NOT_PERIPHERAL_STAGE_MASK);
    }
    if (!exact_udc) {
        p282_restart_exact_failure(
            cycle,
...
static __attribute__((noreturn)) void p282_e3_run(void) {
