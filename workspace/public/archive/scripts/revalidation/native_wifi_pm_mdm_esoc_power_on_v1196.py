#!/usr/bin/env python3
"""V1196 PM observer: subsys_esoc0 open + RELATED restart_level + periodic mdm3 status.

V1195 result: restart_level=RELATED confirmed — device did NOT reboot unexpectedly.
However GPIO 142 did not fire in 3-minute window. GPIO polling was silent (no
changes visible). Needed periodic status output to diagnose MDM boot progress.

v230 adds 10s-interval status output in the GPIO polling loop:
  pm_observer_mdm_power_on.status.elapsed_ms=N
  pm_observer_mdm_power_on.status.gpio142_count=N
  pm_observer_mdm_power_on.status.mdm3_state=OFFLINING|ONLINE|...
  pm_observer_mdm_power_on.status.mdm3_crash_count=N

This lets us observe:
- Does mdm3 state change during the window?
- Does crash_count increment (MDM SSR with RELATED restart)?
- Does GPIO 142 count change?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1196-pm-mdm-esoc-power-on-related-status")
LATEST_POINTER = Path(
    "tmp/wifi/latest-v1196-pm-mdm-esoc-power-on-related-status.txt"
)
DEFAULT_EXECNS_HELPER_SHA256 = (
    "fe63241a12696140ac0a13e501722b510bc8b37f55bc117b3b7285d60b717504"
)
DEFAULT_EXECNS_HELPER_MARKER = "a90_android_execns_probe v236"
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1196"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1196/pm-mdm-esoc-power-on-child.sh"
DEFAULT_COLLECTOR_SCRIPT = (
    "/cache/a90-runtime/v1196/pm-mdm-esoc-power-on-collector.sh"
)
DEFAULT_CHILD_OUTPUT = (
    "/cache/a90-runtime/v1196/pm-mdm-esoc-power-on-output.txt"
)
PROOF_PREFIX = "/tmp/a90-v1196-"
...
PCIE_ENUMERATE_FLAG = "--pm-observer-trigger-pcie-enumerate"


def pm_mdm_esoc_power_on_v1196_child_command(args: Any) -> list[str]:
    ...
    if PCIE_ENUMERATE_FLAG not in result:
        result.append(PCIE_ENUMERATE_FLAG)
    return result
...
def _mdm_power_on(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    # collect status entries (periodic 10s snapshots)
    ...
    for step in steps:
        ...
        for line in text.splitlines():
            ...
            for skey in ("elapsed_ms", "gpio142_count", "mdm3_state", "mdm3_crash_count"):
...
def decide_v1196(args: Any, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1196-pm-mdm-esoc-power-on-related-status-plan-ready",
            True,
            "plan-only; no device mutation",
            "deploy helper v230, then run V1196 live gate",
        )
    ...
    if gate.get("begin") != "1":
        return (
            "v1196-vndservice-gate-not-activated",
            False,
            "vndservice gate not activated; verify helper v230 and flags",
    ...
    if not esoc0_found:
        return (
            "v1196-mdm-helper-esoc0-not-found",
            False,
    ...
    if not power_begin:
        return (
            "v1196-mdm-power-on-not-triggered",
            False,
    ...
    if not rl_ok:
        return (
            "v1196-restart-level-set-failed",
            False,
    ...
    # Analyze periodic status entries for MDM boot progress
    mdm3_states = [e.get("mdm3_state", "") for e in status_entries]
    crash_counts = [e.get("mdm3_crash_count", "") for e in status_entries]
    gpio_counts = [e.get("gpio142_count", "") for e in status_entries]

    state_transitions = list(dict.fromkeys(s for s in mdm3_states if s))
    max_crash = max((int(c) for c in crash_counts if c.isdigit()), default=0)

    if not status_entries and not power_on.get("end", ""):
        return (
            "v1196-device-rebooted-before-polling",
            False,
            "no status entries and no end marker; device likely rebooted "
            f"(mss restart_level=SYSTEM cascaded?)",
            "check mss restart_level; consider changing both esoc0 and mss to RELATED",
        )
    ...
    if max_crash > 0:
        return (
            "v1196-mdm3-crash-count-nonzero",
            False,
            (
                f"mdm3 crash_count={max_crash}; MDM SSR fired with RELATED restart; "
                f"mdm_helper likely failed to complete ESOC_IMG_XFER_DONE; "
                f"state_transitions={state_transitions}; "
                f"gpio_counts={gpio_counts[:5]}"
            ),
            (
                "mdm_helper failed firmware transfer; "
                "check mdm_helper SELinux domain and firmware file accessibility"
            ),
        )

    # GPIO silent, no crash, subsys_esoc0 still blocked
    return (
        "v1196-subsys-esoc0-blocked-gpio142-silent",
        False,
        (
            f"subsys_esoc0 open blocking; GPIO 142 silent after "
            f"{len(status_entries)*10}s; "
            f"mdm3 states={state_transitions}; "
            f"crash_count=0; MDM powerup stuck in mdm_subsys_powerup"
        ),
        (
            "MDM powerup stuck; check AP2MDM GPIO toggle and MDM hardware state; "
            "consider pstore/last_kmsg for previous boot diagnostics"
        ),
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# V1196 PM Observer: subsys_esoc0 + restart_level=RELATED + Periodic Status",
        "",
...
def patch_defaults() -> None:
    ...
    v1183.pm_per_proxy_vndservice_gate_child_command = pm_mdm_esoc_power_on_v1196_child_command
    v1106.pm_cnss_child_command = pm_mdm_esoc_power_on_v1196_child_command
...
def main() -> int:
    ...
    decision, passed, reason, next_step = decide_v1196(args, manifest)
    manifest.update(
