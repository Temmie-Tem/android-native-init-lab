#!/usr/bin/env python3
"""V1573 local-only sanity verifier for the service-window mdm_helper launch-contract test boot."""
...
DEFAULT_MANIFEST = (
    ...
    / "tmp"
    / "wifi"
    / "v1573-mdm-helper-launch-contract-crashfix-test-boot"
    / "manifest.json"
)
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1573-mdm-helper-launch-contract-crashfix-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1573_MDM_HELPER_LAUNCH_CONTRACT_CRASHFIX_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1573-mdm-helper-launch-contract-crashfix-test-boot-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v290"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.69 (v1573-mdm-helper-launch-contract-crashfix)",
    "a90_android_execns_probe v290",
    "v1573",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1573-mdm-helper-launch-contract-crashfix-artifact-sanity-pass",
            True,
            "V1573 service-window mdm_helper launch-contract crashfix test boot artifact passed local sanity",
        )
    return (
        "v1573-mdm-helper-launch-contract-crashfix-artifact-sanity-blocked",
        False,
        "V1573 crashfix artifact sanity failed; fix local artifact before any live handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1573 manifest: `{manifest['_path']}`",
        f"- V1573 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
