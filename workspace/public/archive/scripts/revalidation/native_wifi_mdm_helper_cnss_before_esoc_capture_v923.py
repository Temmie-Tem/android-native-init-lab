#!/usr/bin/env python3
"""V923 bounded CNSS-before-eSoC mdm_helper trigger capture proof.

Runs deployed helper v152 in wifi-companion-mdm-helper-cnss-before-subsys-trigger-capture
mode. Permitted live actions are selinuxfs mount/cleanup, private property shim,
`/vendor/bin/pm-service`, `/vendor/bin/mdm_helper`, `/vendor/bin/cnss_diag`,
`/vendor/bin/cnss-daemon -n -l`, and a bounded child open of `/dev/subsys_esoc0`
only after a WLFW precondition marker is observed. No service-manager, Wi-Fi HAL,
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v923-mdm-helper-cnss-before-esoc-capture-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v923-mdm-helper-cnss-before-esoc-capture-live.txt")
base.DEFAULT_LOCAL_HELPER = Path("tmp/wifi/v921-execns-helper-v152-build/a90_android_execns_probe")
base.DEFAULT_HELPER_SHA256 = "cdaa1adde9774e90e1d1e9f5f4eca43be4643b7ff0be2c8a0a08da5bf3e52105"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v152"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--allow-mdm-helper-cnss-before-subsys-trigger-capture", action="store_true")
...
def required_flags(args: argparse.Namespace) -> list[str]:
    ...
    for flag, enabled in (
        ...
        (
            ...
            args.allow_mdm_helper_cnss_before_subsys_trigger_capture,
        ),
...
def helper_surface(text: str) -> dict[str, Any]:
    ...
    execns_rc0 = "A90_EXECNS_END rc=0" in text
    open_markers_absent = not any(
        key.startswith(f"{prefix}.subsys_trigger.") or key == f"{prefix}.subsys_esoc0_open_attempting"
        for key in keys
    )
    if (
        not interesting.get("result")
        and stdout_truncated
        and execns_rc0
        and interesting.get("wlfw_precondition_observed") == "0"
        and open_markers_absent
    ):
        interesting["subsys_esoc0_open_attempted"] = "0"
        interesting["result"] = "wlfw-precondition-missing-no-open-output-truncated"
        interesting["reason"] = "execns-stdout-truncated-before-final-result"
        interesting["output_truncated_before_final_result"] = "1"
        interesting["end"] = "1"
    return {
...
def execute(args: argparse.Namespace, store: base.EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    helper_step = v857.run_device(args, store, steps, "mdm-helper-cnss-before-esoc", helper_command(args), timeout=args.toybox_timeout_sec + 25.0)
    ...
    if cleanup_needed and args.allow_cleanup_reboot:
        analysis["reboot_cleanup"] = base.reboot_cleanup(args, store, "cnss-before-esoc actor not proven stopped")
    elif cleanup_needed:
...
def decide(args: argparse.Namespace,
    ...
    if args.command == "plan":
        if not (local["exists"] and local["sha256"] == args.helper_sha256 and local["marker"] and local["mode"]):
            return "v923-plan-helper-v152-missing", False, f"local={local}", "build and deploy helper v152 before V923"
        return "v923-mdm-helper-cnss-before-esoc-plan-ready", True, "plan-only; no device command executed", "run bounded V923 wait-gated subsys trigger capture proof"
    missing = required_flags(args)
    if missing:
        return "v923-mdm-helper-cnss-before-esoc-approval-required", False, f"missing flags: {', '.join(missing)}", "rerun with explicit V923 flags"
    helper = analysis.get("helper") or {}
    failed_steps = step_failures(steps, helper)
    if failed_steps:
        return "v923-step-failed", False, f"failed_steps={failed_steps}", "inspect V923 evidence before retry"
    remote = analysis.get("remote_helper") or {}
    if not (remote.get("sha_ok") and remote.get("marker_ok") and remote.get("mode_ok")):
        return "v923-helper-v152-remote-mismatch", False, f"remote={remote}", "redeploy helper v152 before V923"
    if helper.get("forbidden_true"):
        return "v923-forbidden-action-detected", False, f"forbidden={helper.get('forbidden_true')}", "stop and audit helper before retry"
    contract = helper.get("contract") or {}
    if contract.get("begin") != "1" or contract.get("allowed") != "1":
        return "v923-helper-mode-not-executed", False, f"contract={contract}", "fix V923 helper command before retry"
    if contract.get("per_mgr_start_attempted") != "1" or contract.get("mdm_helper_start_attempted") != "1":
        return "v923-runtime-actors-not-attempted", False, f"contract={contract}", "repair actor ordering before retry"
    if contract.get("subsys_esoc0_open_attempted") == "1" and contract.get("wlfw_precondition_observed") != "1":
        return "v923-open-gate-violation", False, f"contract={contract}", "audit helper ordering before retry"
    cleanup = analysis.get("reboot_cleanup") or {}
    if analysis.get("cleanup_needed") and not cleanup.get("healthy"):
        return "v923-reboot-cleanup-review", False, f"cleanup={cleanup}", "verify or rerun recovery reboot before continuing"
    result = contract.get("result")
    if result == "wlfw-precondition-observed-trigger-clean":
        return "v923-wlfw-precondition-observed-trigger-clean", True, f"contract={contract}", "inspect WLFW/BDF/wlan0 and mdm3 deltas before any HAL/scan/connect work"
    if result in {"wlfw-precondition-missing-no-open", "wlfw-precondition-missing-no-open-output-truncated"}:
        return "v923-wlfw-precondition-missing-no-open", True, f"contract={contract}", "classify why CNSS/WLFW precondition does not appear before eSoC trigger"
    if result == "mdm-helper-esoc-fd-missing-no-open":
        return "v923-mdm-helper-esoc-fd-missing-no-open", True, f"contract={contract}", "classify why bounded gate did not catch /dev/esoc-0 under native runtime inputs"
    if result == "reboot-required":
        return "v923-reboot-required-cleaned", True, f"contract={contract}", "inspect pre-reboot subsys trigger evidence"
    if result in {"property-shim-setup-failed", "manual-review-required", "blocked"}:
        return "v923-subsys-trigger-setup-failed", False, f"contract={contract}", "repair property/per_mgr/mdm_helper trigger setup before retry"
    return "v923-mdm-helper-cnss-before-esoc-review", False, f"contract={contract}", "inspect V923 helper output before continuing"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V923 mdm_helper Subsys Trigger Capture",
        "",
        ...
        "## Guardrails",
        "",
        "- Permits only selinuxfs mount/cleanup, private property shim, `/vendor/bin/pm-service`, `/vendor/bin/mdm_helper`, `/vendor/bin/cnss_diag`, `/vendor/bin/cnss-daemon -n -l`, and the WLFW-gated `/dev/subsys_esoc0` child open.",
        "- `pm_proxy_helper`, service-manager, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, controller eSoC notify, and controller BOOT_DONE are forbidden.",
