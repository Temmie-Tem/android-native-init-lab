#!/usr/bin/env python3
"""V1096 tracefs-only CNSS libperipheral_client voter classifier.
...
libperipheral_client.so.  It answers one narrow question: does cnss-daemon
actually enter the PM client/vote path before mdm3 stays OFFLINING?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1096-pm-cnss-peripheral-tracefs-live")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1096"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1096/pm-cnss-voter-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1096/peripheral-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1096/pm-cnss-voter-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1096-pm-cnss-peripheral-tracefs-live.txt")

EVENT_SPECS = (
    ("pm_find_lock", "6078"),
    ("pm_register_connect", "612c"),
    ("pm_client_register", "6ec8"),
    ("pm_client_unregister", "71bc"),
    ("pm_client_connect", "7544"),
    ("pm_client_disconnect", "761c"),
    ("pm_client_ack", "76f0"),
)
...
def write_child_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    script = "\n".join([
        ...
        f"echo v1096.child_full_output={shlex.quote(args.child_output)}",
        "echo v1096.child_rc=$rc",
        "exit $rc",
        "",
    ])
...
echo tracefs_uprobe_collector=v1096
...
$BB grep -E '^(A90_EXECNS_(END|STDOUT_END)|pm_service_trigger_observer\\.|wifi_vndservice_query\\.|wifi_companion_qrtr_readback\\.|v1096\\.)' "$CHILD_LOG" 2>/dev/null || true
...
def write_collector_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    store.write_text("host/peripheral-tracefs-collector-script.txt", script)
...
def parse_tracefs_output(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "child_rc": keys.get("child.rc", keys.get("v1096.child_rc", "")),
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    collector_step = base.run_a90ctl(
        ...
        store,
        steps,
        "pm-cnss-peripheral-tracefs-observer",
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    ...
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1096-execns-helper-sha-mismatch", False, "remote execns helper is not v206", "redeploy helper v206")
    usage = analysis.get("execns_usage") or {}
    if not (usage.get("marker_ok") and usage.get("mode_ok") and usage.get("start_cnss_flag_ok")):
        return ("v1096-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v206")
    if not analysis.get("peripheral_client_visible"):
        return ("v1096-peripheral-client-not-visible", False, "read-only vendor mount did not expose libperipheral_client.so", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1096-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1096-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1096-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1096-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if contract.get("cnss_daemon_start_executed") != "1":
        return ("v1096-cnss-daemon-not-started", False, "child did not execute V1095 CNSS phase", "repair child command before retry")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1096-peripheral-client-uprobe-no-hit", False, "no libperipheral_client trace lines were captured", "verify shared-library inode mapping or offsets")
    ...
    if cnss_hits > 0:
        return (
            ...
            True,
            f"cnss_hits={cnss_hits} mdm3_state={mdm3_state} counts={tracefs.get('cnss_daemon_hits')}",
            "classify PM service response/vote semantics or lower eSoC trigger",
        )
    return (
        "v1096-cnss-peripheral-client-not-called",
        True,
        f"total_hits={tracefs.get('hit_count')} by_comm={tracefs.get('by_comm')}",
        "classify cnss-daemon internal gating before PM client request",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1096 PM CNSS Peripheral Tracefs Live",
        "",
