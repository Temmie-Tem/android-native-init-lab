#!/usr/bin/env python3
"""V1163 bounded late pm-proxy eSoC trigger live gate.

This V1143 derivative keeps the global-firmware outer holder, post-policy
CNSS PM path, post-PM mdm_helper lower trace, and helper cleanup.  It adds
the helper v216 late trigger flag:
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1163-late-per-proxy-esoc-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1163-late-per-proxy-esoc-live.txt")
DEFAULT_EXECNS_HELPER_SHA256 = "b9518555ef53f8e721f8a057c8145085b3ba91899c34609c59cb1885e8b71241"
DEFAULT_EXECNS_HELPER_MARKER = "a90_android_execns_probe v216"
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1163"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1163/late-per-proxy-esoc-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1163/late-per-proxy-esoc-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1163/late-per-proxy-esoc-output.txt"
PROOF_PREFIX = "/tmp/a90-v1163-"
...
def serial_remote_marker_check_v1163(
    args: Any,
    store: EvidenceStore,
...
def tracefs_collector_script_v1163(args: Any) -> str:
...
def parse_tracefs_output_v1163(text: str) -> dict[str, Any]:
...
def patch_defaults() -> None:
    ...
    v1143.serial_remote_marker_check_v1143 = serial_remote_marker_check_v1163
    v1143.tracefs_collector_script_v1143 = tracefs_collector_script_v1163
    v1143.parse_tracefs_output_v1143 = parse_tracefs_output_v1163
...
def decide_v1163(args: Any, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1163-late-per-proxy-esoc-plan-ready",
            True,
            "plan-only; no device mutation, PM actor, mdm_helper, CNSS daemon, reboot, or Wi-Fi action executed",
            "run bounded late pm-proxy live with helper v216 and explicit allow flags",
        )
    ...
    if not usage.get("late_per_proxy_flag_ok"):
        return (
            "v1163-helper-late-per-proxy-flag-missing",
            False,
            f"usage={usage}",
            "redeploy helper v216 or inspect helper usage output",
        )
    if not base_pass and not deferred_per_proxy_expected:
        return (
            base_decision.replace("v1143", "v1163", 1).replace("v1139", "v1163", 1),
            False,
            base_reason,
            base_next,
        )
    if late.get("begin") != "1":
        if post.get("mdm_helper_observable") == "0" or values.get("child.per_mgr.exit_code") == "0":
            return (
                "v1163-pm-service-exited-before-late-per-proxy",
                True,
                f"per_mgr_exit={values.get('child.per_mgr.exit_code')} post={post} late={late}",
                "classify why pm-service exits before mdm_helper /dev/esoc-0 readiness in the v216 late gate",
            )
        return (
            "v1163-late-per-proxy-not-reached",
            True,
    ...
    if late.get("gate_positive") != "1":
        return (
            "v1163-late-per-proxy-gate-not-opened",
            True,
            f"late={late}",
    ...
    if late.get("started") != "1":
        return (
            "v1163-late-per-proxy-start-missing",
            False,
            f"late={late}",
    ...
    if late.get("snapshot_captured") != "1":
        return (
            "v1163-late-per-proxy-snapshot-incomplete",
            False,
            f"late={late}",
    ...
    if lower_values:
        return (
            "v1163-late-per-proxy-lower-artifact-observed",
            True,
    ...
    if per_mgr_esoc or queue_esoc:
        return (
            "v1163-late-per-proxy-esoc-trigger-observed",
            True,
    ...
    return (
        "v1163-late-per-proxy-no-esoc-trigger",
        True,
        f"late={late} polls={polls}",
        "compare pm-proxy Binder transaction preconditions against Android V1159",
    )
...
def main() -> int:
    ...
    manifest["cycle"] = "v1163"
    manifest["generated_at"] = now_iso()
    decision, passed, reason, next_step = decide_v1163(args, manifest)
