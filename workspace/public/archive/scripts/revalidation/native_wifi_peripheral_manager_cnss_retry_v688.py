#!/usr/bin/env python3
"""V688 service74-gated PeripheralManager provider + CNSS retry proof.

This proof replays the current service74-positive CNSS retry path with helper
v114. It inserts the Android PeripheralManager provider pair (`pm-service` and
...
base.DEFAULT_OUT_DIR = base.Path("tmp/wifi/v688-peripheral-manager-cnss-retry")
base.DEFAULT_HELPER_SHA256 = "6de2ee5efc65441f18ee587f54b525c4006553cae54363c2e036aa21f976a5f4"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v114"
base.DEFAULT_V490_MANIFEST = base.Path("tmp/wifi/v688-v490-current-run/manifest.json")
base.APPROVAL_PHRASE = (
    "approve v688 PeripheralManager provider CNSS retry proof only; "
...
V688_MODE = "wifi-companion-service74-gated-peripheral-manager-cnss-retry-start-only"
EXPECTED_ORDER = (
...
def build_checks(args: base.argparse.Namespace,
    ...
    helper_ready = (
        ...
        and all(token in usage for token in V688_USAGE_TOKENS)
    )
    base.add_check(
        checks,
        "helper-v114-peripheral-manager-cnss-retry-contract",
        "pass" if helper_ready else "blocked",
        "blocker",
        "remote helper must expose v114 service74-gated PeripheralManager/CNSS retry mode",
        [
            line
            for line in (sha_text + "\n" + usage).splitlines()
            if args.helper_sha256 in line
            or args.helper_marker in line
            or V688_MODE in line
        ...
        "deploy helper v114 before V688 live proof",
    )
    return checks
...
def companion_command(args: base.argparse.Namespace) -> list[str]:
    ...
    _set_option(command, "--mode", V688_MODE)
    _set_option(command, "--property-root", V535_PROPERTY_ROOT)
    if len(command) > MAX_CMDV1_COMMAND_ARGS:
        raise RuntimeError(f"V688 helper command has {len(command)} args; max safe args={MAX_CMDV1_COMMAND_ARGS}")
    return command
...
def run_live(args: base.argparse.Namespace,
    ...
    live["v688_peripheral_manager_surface"] = surface
    live["v688_peripheral_manager_ready"] = _provider_ready(surface)
    return live
...
def decide(args: base.argparse.Namespace,
    ...
    if args.command == "plan":
        return (
            "v688-peripheral-manager-cnss-retry-plan-ready",
            True,
            "plan-only; no device command executed",
            "deploy helper v114, refresh current-boot prerequisites, then run V688 preflight/live",
            False,
        )
    blocked = base.blockers(checks)
    if blocked:
        return "v688-peripheral-manager-cnss-retry-blocked", False, "blocked by " + ", ".join(blocked), "resolve blockers before V688", False
    ...
    if not base.approved(args):
        return "v688-peripheral-manager-cnss-retry-approval-required", True, "exact approval phrase required; no live command executed", "rerun with exact V688 approval", False
    ...
    if not reboot.get("version_seen") or not reboot.get("status_healthy"):
        return "v688-cleanup-review", False, f"reboot_cleanup={reboot}", "verify device manually before continuing", live_executed
    if not live.get("holder_started") or not (live.get("qrtr_rx_wait") or {}).get("seen"):
        return "v688-lower-modem-blocked", False, "subsys_modem holder did not reproduce QRTR RX", "restore lower modem readiness before V688", live_executed
    if not live.get("companion_executed"):
        return "v688-companion-skipped", False, "companion was skipped by QRTR gate", "inspect QRTR wait evidence", live_executed

    surface = live.get("v688_peripheral_manager_surface") or {}
    ...
    if surface.get("order") != EXPECTED_ORDER or surface.get("peripheral_manager_enabled") != "1":
        return (
            "v688-helper-order-contract-gap",
            False,
            f"surface={surface}",
            "fix helper v114 provider order contract before another live attempt",
            live_executed,
        )
    if vnd_ready.get("ready") != "1" or cnss_retry.get("initial_cleanup_safe") != "1":
        return (
            "v688-vnd-or-initial-cnss-cleanup-gap",
            True,
    ...
    if not _provider_ready(surface):
        return (
            "v688-provider-start-gap-classified",
            True,
            f"surface={surface}",
            "context repair held; inspect pm-service/pm-proxy runtime output before another CNSS retry or Wi-Fi HAL start",
            live_executed,
        )
    if not cnss_retry.get("retry_start_order"):
        return (
            "v688-provider-ready-cnss-retry-withheld",
            True,
            f"surface={surface}",
    ...
    if _advanced(counts, markers):
        return (
            "v688-peripheral-manager-wifi-surface-advanced",
            True,
    ...
    return (
        "v688-peripheral-manager-cnss-retry-gap-persists",
        True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    text = _v668_render_summary(manifest).replace(
        ...
        "# V688 PeripheralManager Provider CNSS Retry Proof",
        1,
    )
    ...
    return "\n".join([
        text,
        "",
        "## V688 PeripheralManager Contract",
        "",
        ...
        f"- peripheral_manager_ready: `{live.get('v688_peripheral_manager_ready', '')}`",
...
def build_manifest(args: base.argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    ...
    manifest["cycle"] = "v688"
    manifest["helper_version"] = "v114"
    manifest["property_root"] = V535_PROPERTY_ROOT
    manifest["peripheral_manager_mode"] = V688_MODE
    manifest["expected_order"] = EXPECTED_ORDER
    manifest["peripheral_manager_surface"] = live.get("v688_peripheral_manager_surface") if live else {}
    manifest["peripheral_manager_ready"] = bool(live.get("v688_peripheral_manager_ready")) if live else False
    manifest["context_repair_regressed"] = _context_repair_regressed(live.get("v688_peripheral_manager_surface") or {}) if live else False
    ...
    manifest["explicitly_approved"] = [
        "helper v114 service74-gated PeripheralManager provider start-only mode",
