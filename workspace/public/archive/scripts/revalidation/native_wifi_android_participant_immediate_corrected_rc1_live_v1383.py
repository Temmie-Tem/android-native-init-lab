#!/usr/bin/env python3
"""V1383 bounded Android participant parity + immediate corrected RC1 enumerate live gate."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1383-android-participant-immediate-corrected-rc1-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1383-android-participant-immediate-corrected-rc1-live.txt")
PLAN_OUT_DIR = Path("tmp/wifi/v1383-android-participant-immediate-corrected-rc1-plan")
PLAN_LATEST_POINTER = Path("tmp/wifi/latest-v1383-android-participant-immediate-corrected-rc1-plan.txt")
REPORT_PATH = Path("docs/reports/NATIVE_INIT_V1383_ANDROID_PARTICIPANT_IMMEDIATE_CORRECTED_RC1_LIVE_2026-06-01.md")
...
CORRECTED_RC1_FLAG = "--pm-observer-late-per-proxy-immediate-corrected-rc1-enumerate"
...
def _force_v1383_child_command(original):
...
def configure() -> None:
    ...
    base.CYCLE_LABEL = "v1383"
    base.CYCLE_NAME = "V1383"
    base.SUMMARY_HEADING = "V1383 Android Participant Immediate Corrected RC1 Live"
    base.EVIDENCE_FILE_PREFIX = "v1383"
    base._force_response_sampler_child_command = _force_v1383_child_command
...
def decide_v1383(manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if manifest.get("command") == "plan":
        return (
            "v1383-android-participant-immediate-corrected-rc1-plan-ready",
            True,
            "plan-only; no device command or live action executed",
            "run V1383 bounded live gate with helper v284",
        )
    ...
    if not corrected.get("emitted") or not corrected.get("end"):
        return (
            "v1383-corrected-rc1-summary-missing",
            False,
    ...
    if not corrected.get("triggered"):
        return (
            "v1383-corrected-rc1-not-triggered",
            False,
    ...
    if _int_value(corrected.get("gate_pm_service_powerup_thread_count"), -1) <= 0:
        return (
            "v1383-corrected-rc1-triggered-without-powerup-thread",
            False,
            f"corrected RC1 enumerate triggered without the v284 powerup-thread gate: gate_pm_service_powerup_thread_count={corrected.get('gate_pm_service_powerup_thread_count')}",
            "stop and inspect corrected RC1 gate evidence before retry",
        )
    if corrected.get("rc_sel_rc") != 0 or corrected.get("case_rc") != 0:
        return (
            "v1383-corrected-rc1-write-failed",
            False,
    ...
    if _timing_downstream_progress(sampler):
        return (
            "v1383-corrected-rc1-downstream-progress",
            True,
            "immediate corrected RC1 enumerate ran in the Android participant powerup-thread window and downstream GPIO/PCI/MHI/WLFW/wlan0 progress appeared",
    ...
    if bool(sampler.get("timing_pcie_rc1_transition_seen")) or bool(dmesg_rc1.get("transition_seen")):
        ...
        return (
            "v1383-corrected-rc1-ltssm-no-downstream-clean",
            True,
            "immediate corrected RC1 enumerate fired at the first v284 powerup-thread poll"
    ...
    return (
        "v1383-corrected-rc1-no-rc1-transition",
        True,
        "immediate corrected RC1 enumerate write returned inside the Android participant powerup-thread window, but timing summary saw no RC1/downstream transition",
...
def augment_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    decision, passed, reason, next_step = decide_v1383(manifest)
...
def render_summary(manifest: dict[str, Any]) -> str:
    return "\n".join([
        "# V1383 Android Participant Immediate Corrected RC1 Live",
        "",
...
def print_result(manifest: dict[str, Any]) -> None:
    print(f"v1383_decision: {manifest.get('decision')}")
    print(f"v1383_pass: {manifest.get('pass')}")
    print(f"v1383_reason: {manifest.get('reason')}")
    print(f"v1383_next: {manifest.get('next_step')}")
    print(f"v1383_manifest: {_run_dir() / 'manifest.json'}")
