#!/usr/bin/env python3
"""V908 bounded mdm_helper runtime-contract capture proof.

Runs deployed helper v148 in wifi-companion-mdm-helper-runtime-contract-capture
mode. The newly allowed live actions are the helper's private property shim,
`/vendor/bin/pm-service` as `per_mgr_light`, and `/vendor/bin/mdm_helper`.
It does not start service-manager/CNSS/Wi-Fi HAL, scan/connect, use
credentials, configure DHCP/routes, external ping, or open `/dev/subsys_esoc0`
as a controller trigger.
"""
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v908-mdm-helper-runtime-contract-capture-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v908-mdm-helper-runtime-contract-capture-live.txt")
base.DEFAULT_LOCAL_HELPER = Path("tmp/wifi/v906-execns-helper-v148-build/a90_android_execns_probe")
base.DEFAULT_HELPER_SHA256 = "12633b3c21292cf547393abce972bc7b1e855144dcdaea3975a45e943228cae6"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v148"
base.MODE = "wifi-companion-mdm-helper-runtime-contract-capture"
base.PREFIX = "mdm_helper_runtime_contract"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--toybox-timeout-sec", type=int, default=28)
    parser.add_argument("--allow-mountsystem-ro", action="store_true")
    parser.add_argument("--allow-selinuxfs-mount", action="store_true")
    parser.add_argument("--allow-mdm-helper-runtime-contract-capture", action="store_true")
...
def required_flags(args: argparse.Namespace) -> list[str]:
    ...
    for flag, enabled in (
        ...
        ("--allow-mdm-helper-runtime-contract-capture", args.allow_mdm_helper_runtime_contract_capture),
...
def helper_surface(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "mhi_mirror": {
            key: value
            for key, value in keys.items()
            if key.startswith(f"{prefix}.mhi_mirror.")
        },
        "snapshots": {
            key: value
            for key, value in keys.items()
...
def execute(args: argparse.Namespace, store: base.EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    helper_step = v857.run_device(args, store, steps, "mdm-helper-runtime-contract", helper_command(args), timeout=args.toybox_timeout_sec + 25.0)
    ...
    if cleanup_needed and args.allow_cleanup_reboot:
        analysis["reboot_cleanup"] = base.reboot_cleanup(args, store, "mdm_helper runtime-contract actor not proven stopped")
    elif cleanup_needed:
...
def decide(args: argparse.Namespace,
    ...
    if args.command == "plan":
        if not (local["exists"] and local["sha256"] == args.helper_sha256 and local["marker"] and local["mode"]):
            return "v908-plan-helper-v148-missing", False, f"local={local}", "build and deploy helper v148 before V908"
        return "v908-mdm-helper-runtime-contract-plan-ready", True, "plan-only; no device command executed", "run bounded V908 runtime-contract capture proof"
    missing = required_flags(args)
    if missing:
        return "v908-mdm-helper-runtime-contract-approval-required", False, f"missing flags: {', '.join(missing)}", "rerun with explicit V908 flags"
    helper = analysis.get("helper") or {}
    failed_steps = base.step_failures(steps, helper)
    if failed_steps:
        return "v908-step-failed", False, f"failed_steps={failed_steps}", "inspect V908 evidence before retry"
    remote = analysis.get("remote_helper") or {}
    if not (remote.get("sha_ok") and remote.get("marker_ok") and remote.get("mode_ok")):
        return "v908-helper-v148-remote-mismatch", False, f"remote={remote}", "redeploy helper v148 before V908"
    if helper.get("forbidden_true"):
        return "v908-forbidden-action-detected", False, f"forbidden={helper.get('forbidden_true')}", "stop and audit helper before retry"
    contract = helper.get("contract") or {}
    if contract.get("begin") != "1" or contract.get("allowed") != "1":
        return "v908-helper-mode-not-executed", False, f"contract={contract}", "fix V908 helper command before retry"
    if contract.get("per_mgr_start_attempted") != "1" or contract.get("mdm_helper_start_attempted") != "1":
        return "v908-runtime-actors-not-attempted", False, f"contract={contract}", "repair runtime-contract actor ordering before retry"
    cleanup = analysis.get("reboot_cleanup") or {}
    if analysis.get("cleanup_needed") and not cleanup.get("healthy"):
        return "v908-reboot-cleanup-review", False, f"cleanup={cleanup}", "verify or rerun recovery reboot before continuing"
    result = contract.get("result")
    if result == "mdm-helper-produced-ks-mhi":
        return "v908-ks-mhi-observed", True, f"contract={contract}", "inspect mdm3/GPIO142/WLFW/BDF/wlan0 deltas before HAL or scan work"
    if result == "mdm-helper-esoc-fd-observed":
        return "v908-mdm-helper-esoc-fd-observed", True, f"contract={contract}", "classify why fd observation does not progress to ks/MHI/WLFW"
    if result == "mdm-helper-runtime-no-esoc-fd":
        return "v908-mdm-helper-runtime-no-esoc-fd", True, f"contract={contract}", "compare Android runtime inputs beyond property/per_mgr_light"
    if result == "mdm-helper-not-observable":
        return "v908-mdm-helper-not-observable-clean", True, f"contract={contract}", "classify mdm_helper startup/runtime dependency before retry"
    if result == "reboot-required":
        return "v908-reboot-required-cleaned", True, f"contract={contract}", "inspect pre-reboot runtime-contract evidence"
    if result in {"property-shim-setup-failed", "manual-review-required"}:
        return "v908-runtime-contract-setup-failed", False, f"contract={contract}", "repair property/per_mgr/mdm_helper runtime setup before retry"
    return "v908-mdm-helper-runtime-contract-review", False, f"contract={contract}", "inspect V908 helper output before continuing"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V908 mdm_helper Runtime Contract Capture",
        "",
        ...
        "## Guardrails",
        "",
        "- Only selinuxfs mount/cleanup, private property shim, `/vendor/bin/pm-service`, and `/vendor/bin/mdm_helper` are permitted.",
        "- `pm_proxy_helper` and controller `/dev/subsys_esoc0` open are explicitly forbidden.",
...
def build_manifest(args: argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    ...
    contract = helper.get("contract") or {}
    cleanup = analysis.get("reboot_cleanup") or {}
    ks_window = int(contract.get("ks_count.window") or "0") if str(contract.get("ks_count.window") or "0").lstrip("-").isdigit() else 0
    ks_final = int(contract.get("ks_count.final") or "0") if str(contract.get("ks_count.final") or "0").lstrip("-").isdigit() else 0
    mhi_window = int(contract.get("mhi_pipe_cmdline_count.window") or "0") if str(contract.get("mhi_pipe_cmdline_count.window") or "0").lstrip("-").isdigit() else 0
    mhi_final = int(contract.get("mhi_pipe_cmdline_count.final") or "0") if str(contract.get("mhi_pipe_cmdline_count.final") or "0").lstrip("-").isdigit() else 0
    return {
        ...
        "ks_start_executed": ks_window > 0 or ks_final > 0 or mhi_window > 0 or mhi_final > 0,
        "subsys_esoc0_controller_open_attempted": contract.get("subsys_esoc0_controller_open_attempted") == "1",
        "subsys_esoc0_open_attempted": contract.get("subsys_esoc0_controller_open_attempted") == "1",
