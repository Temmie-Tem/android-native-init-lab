#!/usr/bin/env python3
"""V1379 bounded Android participant parity + corrected RC1 enumerate live rerun."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1379-android-participant-corrected-rc1-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1379-android-participant-corrected-rc1-live.txt")
PLAN_OUT_DIR = Path("tmp/wifi/v1379-android-participant-corrected-rc1-plan")
PLAN_LATEST_POINTER = Path("tmp/wifi/latest-v1379-android-participant-corrected-rc1-plan.txt")
REPORT_PATH = Path("docs/reports/NATIVE_INIT_V1379_ANDROID_PARTICIPANT_CORRECTED_RC1_LIVE_2026-06-01.md")
...
def _force_v1379_child_command(original):
...
def configure() -> None:
    ...
    base.CYCLE_LABEL = "v1379"
    base.CYCLE_NAME = "V1379"
    base.SUMMARY_HEADING = "V1379 Android Participant Corrected RC1 Live"
    base.EVIDENCE_FILE_PREFIX = "v1379"
    base._force_response_sampler_child_command = _force_v1379_child_command
...
def decide_v1379(manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if manifest.get("command") == "plan":
        return (
            "v1379-android-participant-corrected-rc1-plan-ready",
            True,
            "plan-only; no device command or live action executed",
            "run V1379 bounded live gate with helper v283",
        )
    ...
    if not corrected.get("emitted") or not corrected.get("end"):
        return (
            "v1379-corrected-rc1-summary-missing",
            False,
    ...
    if not corrected.get("triggered"):
        return (
            "v1379-corrected-rc1-not-triggered",
            False,
    ...
    if _int_value(corrected.get("gate_pm_service_powerup_thread_count"), -1) <= 0:
        return (
            "v1379-corrected-rc1-triggered-without-powerup-thread",
            False,
            f"corrected RC1 enumerate triggered without the v283 powerup-thread gate: gate_pm_service_powerup_thread_count={corrected.get('gate_pm_service_powerup_thread_count')}",
            "stop and inspect corrected RC1 gate evidence before retry",
        )
    if corrected.get("rc_sel_rc") != 0 or corrected.get("case_rc") != 0:
        return (
            "v1379-corrected-rc1-write-failed",
            False,
    ...
    if _timing_downstream_progress(sampler):
        return (
            "v1379-corrected-rc1-downstream-progress",
            True,
    ...
    if bool(sampler.get("timing_pcie_rc1_transition_seen")):
        return (
            "v1379-corrected-rc1-ltssm-no-downstream-clean",
            True,
    ...
    return (
        "v1379-corrected-rc1-no-rc1-transition",
        True,
...
def augment_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    decision, passed, reason, next_step = decide_v1379(manifest)
...
def render_summary(manifest: dict[str, Any]) -> str:
    return "\n".join([
        "# V1379 Android Participant Corrected RC1 Live",
        "",
...
def print_result(manifest: dict[str, Any]) -> None:
    print(f"v1379_decision: {manifest.get('decision')}")
    print(f"v1379_pass: {manifest.get('pass')}")
    print(f"v1379_reason: {manifest.get('reason')}")
    print(f"v1379_next: {manifest.get('next_step')}")
    print(f"v1379_manifest: {_run_dir() / 'manifest.json'}")
