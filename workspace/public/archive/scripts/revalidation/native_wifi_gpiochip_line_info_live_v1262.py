#!/usr/bin/env python3
"""V1262 bounded read-only PMIC GPIO9 line-info proof."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1262-gpiochip-line-info-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1262-gpiochip-line-info-live.txt")
DEFAULT_HELPER = "/cache/bin/a90_android_execns_probe"
DEFAULT_TOYBOX = "/cache/bin/toybox"
DEFAULT_HELPER_SHA256 = "32ac877a165a266d96589387d9974dfea38c81d0adb368bf17ff15de77a9f9fb"
DEFAULT_HELPER_MARKER = "a90_android_execns_probe v263"
MODE = "wifi-companion-pmic-gpiochip-line-info-preflight"
ALLOW_FLAG = "--allow-pmic-gpiochip-line-info-preflight"
PREFIX = "pmic_gpiochip_line_info_preflight."
APPROVAL_PHRASE = (
    "approve v1262 PMIC GPIO9 line-info proof only; "
...
def run_live_steps(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    steps.append(capture_native(args, store, "gpiochip-line-info-proof", helper_command(args), timeout=args.timeout))
...
def analyze(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    live_text = step_text(store, steps, "gpiochip-line-info-proof")
    ...
    return {
        ...
        "lineinfo_executed": values.get("gpio_get_lineinfo_executed") == "1",
        "lineinfo_ok": values.get("gpio_get_lineinfo_ok") == "1",
        "cleanup_ok": values.get("cleanup_ok") == "1",
        "ready": values.get("line_info_preflight_ready") == "1",
        "result": values.get("result", ""),
        "chip_name": values.get("chip_name", ""),
        "chip_label": values.get("chip_label", ""),
        "chip_lines": values.get("chip_lines", ""),
        "line_offset": values.get("line_offset", ""),
        "line_global": values.get("line_global", ""),
        "line_flags": values.get("line_flags", ""),
        "line_flag_kernel": values.get("line_flag_kernel", ""),
        "line_flag_is_out": values.get("line_flag_is_out", ""),
        "line_flag_active_low": values.get("line_flag_active_low", ""),
        "line_flag_open_drain": values.get("line_flag_open_drain", ""),
        "line_flag_open_source": values.get("line_flag_open_source", ""),
        "line_flag_bias_pull_up": values.get("line_flag_bias_pull_up", ""),
        "line_flag_bias_pull_down": values.get("line_flag_bias_pull_down", ""),
        "line_flag_bias_disable": values.get("line_flag_bias_disable", ""),
        "line_name": values.get("line_name", ""),
        "line_consumer": values.get("line_consumer", ""),
...
def build_checks(command: str, args: argparse.Namespace, analysis: dict[str, Any], steps: list[dict[str, Any]]) -> list[Check]:
    ...
    checks = [
        ...
        Check("helper-v263", "pass" if analysis["helper_sha_ok"] and analysis["helper_marker_ok"] and analysis["helper_mode_ok"] else "blocked", "blocker", f"sha={analysis['helper_sha_ok']} marker={analysis['helper_marker_ok']} mode={analysis['helper_mode_ok']}", "deploy V1261 helper v263 first"),
    ]
    if command == "run":
        checks.extend([
            Check("live-command", "pass" if step_ok.get("gpiochip-line-info-proof") and analysis["live_output_present"] else "blocked", "blocker", "helper produced prefixed live output", "inspect helper command output"),
            Check("sysfs-contract", "pass" if analysis["sysfs_dev_match"] and analysis["sysfs_label_match"] and analysis["sysfs_base_match"] and analysis["sysfs_ngpio_match"] else "blocked", "blocker", f"dev={analysis['sysfs_dev_match']} label={analysis['sysfs_label_match']} base={analysis['sysfs_base_match']} ngpio={analysis['sysfs_ngpio_match']}", "refresh gpiochip mapping before any line-info retry"),
            Check("line-info-proof", "pass" if analysis["ready"] else "blocked", "blocker", f"mknod={analysis['mknod_ok']} open={analysis['open_ok']} chipinfo={analysis['chipinfo_ok']} lineinfo={analysis['lineinfo_ok']} cleanup={analysis['cleanup_ok']} result={analysis['result']}", "do not proceed to GPIO line request until line-info proof is ready"),
...
def decide(command: str, checks: list[Check], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    ...
    if command == "plan":
        return ("v1262-gpiochip-line-info-plan-ready", True, "plan-only; no device command executed", "run preflight then approved V1262 live proof")
    if blockers:
        return ("v1262-gpiochip-line-info-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before continuing")
    if command == "preflight":
        return ("v1262-gpiochip-line-info-ready", True, "helper v263 and native health ready for bounded line-info proof", "run approved V1262 live proof")
    if analysis["ready"] and analysis["all_zero_markers_ok"]:
        return ("v1262-gpiochip-line-info-pass", True, "PMIC GPIO9 line-info ioctl passed without requesting the line", "plan bounded line request or line-hold only after reviewing flags/consumer")
    return ("v1262-gpiochip-line-info-incomplete", True, "live proof completed but readiness is incomplete", "inspect helper output before next gate")
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ["lineinfo_ok", analysis["lineinfo_ok"]],
