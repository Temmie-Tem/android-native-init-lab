#!/usr/bin/env python3
"""V649 Android handoff for same-boot full audio/Wi-Fi recapture.

This wrapper temporarily boots a known Android boot image, waits for
`sys.boot_completed=1`, runs the V649 read-only Android collector, then
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v649-android-full-audio-wifi-handoff")
...
def v649_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v649-android-full-audio-wifi-recapture-run"


def v649_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    ...
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            if args.serial:
                v649_command.extend(["--serial", args.serial])
            v649_command.append("run")
            plan.append(("v649-android-full-audio-wifi-recapture", v649_command, collector_timeout))
        else:
            plan.append((name, command, timeout))
    return plan


def load_v649_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v649_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v649 = load_v649_result(store)
    android_summary = v649.get("android_summary") or {}
    markers = android_summary.get("markers") or {}
    counts = markers.get("counts") or {}
    deltas = markers.get("deltas_ms") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v649-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass before V649"
    elif not v649.get("pass"):
        decision = "v649-handoff-v649-failed-rollback-complete"
        pass_ok = False
        reason = "V649 Android same-boot timing collector failed after rollback completed"
    else:
        decision = str(v649.get("decision") or "v649-handoff-review-required")
        pass_ok = bool(v649.get("pass"))
        reason = str(v649.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "state": {
            "service_notifier_74_count": counts.get("service_notifier_74"),
            "wlan_pd_count": counts.get("wlan_pd"),
            "asoc_probe_count": counts.get("asoc_probe"),
            "pm_qos_duplicate_count": counts.get("pm_qos_duplicate"),
            "qos_warning_count": counts.get("qos_warning"),
            "has_audio_context": android_summary.get("has_audio_context"),
            "has_wlan_pd": android_summary.get("has_wlan_pd"),
            "has_wifi_qmi": android_summary.get("has_wifi_qmi"),
            "warning_dirty": android_summary.get("warning_dirty"),
            "service74_to_wlan_pd_ms": deltas.get("service74_to_wlan_pd"),
            "service74_to_asoc_probe_ms": deltas.get("service74_to_asoc_probe"),
            "asoc_probe_to_pm_qos_ms": deltas.get("asoc_probe_to_pm_qos"),
        },
        "v649": {
            "path": v649.get("_path"),
            "decision": v649.get("decision"),
            "pass": v649.get("pass"),
            "reason": v649.get("reason"),
            "next_step": v649.get("next_step"),
        },
    }
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v649_out_dir": str(v649_out_dir(store)),
    ...
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v649-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v649-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v649-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v649-handoff-approval-required", False

    plan = v649_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_active_wifi(plan)
    if offenders:
        context["forbidden_active_wifi_steps"] = offenders
        return [], context, "v649-handoff-active-wifi-command-blocked", False
    ...
    if args.command == "plan":
        ...
        return steps, context, "v649-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v649-android-full-audio-wifi-recapture"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v649-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v649-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v649-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v649-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v649-android-full-audio-wifi-recapture":
            v649_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v649-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v649_step_ok:
            return steps, context, "v649-handoff-v649-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v649-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v649-handoff-plan-ready": "execution plan generated without device mutation",
        "v649-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v649-handoff-approval-required": "live run refused because approval flags are missing",
        "v649-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v649-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v649-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v649-handoff-active-wifi-command-blocked": "handoff plan contains a forbidden active Wi-Fi command pattern",
        "v649-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v649-handoff-v649-failed-rollback-complete": "V649 Android timing collector failed, and native rollback completed",
        "v649-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v649-handoff-readback-failed-rollback-attempted": "Android boot readback failed and native rollback was attempted from recovery",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V649 Android Full Audio/Wi-Fi Recapture Handoff",
        "",
        ...
        f"- v649_out_dir: `{manifest['context']['v649_out_dir']}`",
        "",
        "## Comparison",
        "",
        f"- boot_complete: `{comparison.get('boot_complete', '-')}`",
        f"- v649_decision: `{(comparison.get('v649') or {}).get('decision', '-')}`",
        f"- v649_next_step: `{(comparison.get('v649') or {}).get('next_step', '-')}`",
        "",
        ...
        "- Live mode temporarily flashes Android boot, runs V649 read-only collection, then restores native init.",
...
def main() -> int:
    ...
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v649-handoff-approval-required",
            "v649-handoff-missing-native-rollback",
            "v649-handoff-missing-android-boot",
            "v649-handoff-image-collision",
            "v649-handoff-active-wifi-command-blocked",
        },
