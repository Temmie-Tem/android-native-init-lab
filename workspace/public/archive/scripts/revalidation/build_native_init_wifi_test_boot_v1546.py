#!/usr/bin/env python3
"""Build the V1546 low-overhead endpoint observer Wi-Fi test boot artifact."""
...
OUT_DIR = base.REPO_ROOT / "tmp" / "wifi" / "v1546-low-overhead-endpoint-observer-test-boot"
REPORT_PATH = (
    base.REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1546_LOW_OVERHEAD_ENDPOINT_OBSERVER_SOURCE_BUILD_2026-06-02.md"
)
...
DEFAULT_ARGS = [
    ...
    "V1546",
    "--decision",
    "v1546-low-overhead-endpoint-observer-test-boot-source-build-pass",
    "--cycle-label",
    "v1546",
    "--init-version",
    "0.9.100",
    ...
    str(OUT_DIR),
    "--init-binary",
    str(OUT_DIR / "init_v1546_wifi_test"),
    "--helper-binary",
    str(OUT_DIR / "a90_android_execns_probe_v287"),
    "--ramdisk-cpio",
    str(OUT_DIR / "ramdisk_v1546_wifi_test.cpio"),
    "--boot-image",
    str(OUT_DIR / "boot_linux_v1546_wifi_test.img"),
    "--wifi-test-klog-prefix",
    "A90v1546",
    "--wifi-test-disable",
    "/cache/native-init-wifi-test-boot-v1546.disable",
    "--wifi-test-log",
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Low-overhead endpoint result path: `{wifi['rc1_window_result']}`",
        ...
        f"- immediate endpoint sampler: `{wifi['rc1_immediate_endpoint_sampler']}`",
