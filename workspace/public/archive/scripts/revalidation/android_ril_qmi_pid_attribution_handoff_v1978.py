#!/usr/bin/env python3
"""V1978 Android-good pre-UP libqmi PID attribution handoff.

V1974 proved normal Android has anonymous pre-UP DMS/WLFW libqmi lookups before
`msm/modem/wlan_pd` reaches UP, but the trace comm was not enough to attribute
that thread to `rild` or another process.  V1978 reruns the same rollbackable
Android-good measurement and adds live `/proc` attribution for libqmi trace PIDs
while those tasks are still alive.
"""
...
CYCLE = "V1978"
DEFAULT_OUT_DIR = Path("tmp/wifi/v1978-android-ril-qmi-pid-attribution-handoff")
DEFAULT_REPORT_PATH = Path(
    "docs/reports/NATIVE_INIT_V1978_ANDROID_RIL_QMI_PID_ATTRIBUTION_2026-06-04.md"
)
LATEST_POINTER = Path("tmp/wifi/latest-v1978-android-ril-qmi-pid-attribution.txt")
MODULE_NAME = "a90_v1978_ril_qmi_pid"
REMOTE_MODULE_DIR = f"/data/adb/modules/{MODULE_NAME}"
REMOTE_EVIDENCE_DIR = "/data/local/tmp/a90-v1978-ril-qmi-pid"
REMOTE_STAGE_PREFIX = "/data/local/tmp/a90_v1978_ril_qmi_pid"
TRACEFS_GROUP = "a90rilqmi1978"

V1974_POST_FS_DATA_SCRIPT = v1974.post_fs_data_script
v1934 = v1974.v1934

ATTR_BLOCK_RE = re.compile(
    r"^A90_V1978_PID_BEGIN label=(?P<label>\S+) pid=(?P<pid>\d+) prefix=(?P<prefix>\S+)$"
)
ATTR_END_RE = re.compile(r"^A90_V1978_PID_END pid=(?P<pid>\d+)$")
...
def module_prop() -> str:
    return "\n".join(
        [
            f"id={MODULE_NAME}",
            "name=A90 V1978 RIL QMI PID attribution observer",
            "version=1",
            "versionCode=1",
            "author=A90 native-init project",
            "description=Temporary read-only Android-good libqmi pre-UP PID attribution observer. Remove after capture.",
            "",
        ]
    )
  ...
  for proc in /proc/[0-9]*; do
    ...
    case "$comm $cmd" in
      *rild*|*RILD*|*qcril*|*QCRIL*|*cnss-daemon*|*pm-service*|*per_mgr*|*libqmi*|*time_daemon*|*netmgrd*|*qmuxd*|*vendor_ril*|*vendor_qti*)
        ...
        echo "A90_V1978_FOCUS|label=$label|uptime=$uptime|pid=$pid|comm=$comm|attr=$attr|cmdline=$cmd" >> "$out"
        ;;
    esac
  done
}
    ...
    case "$proc_comm $cmd" in
      *rild*|*RILD*|*qcril*|*QCRIL*|*cnss-daemon*|*pm-service*|*per_mgr*|*libqmi*|*time_daemon*|*netmgrd*|*qmuxd*|*vendor_ril*|*vendor_qti*|*qtidataservices*)
        ...
        for task in "$proc"/task/[0-9]*; do
          ...
          echo "A90_V1978_TASK|label=$label|uptime=$uptime|pid=$pid|tid=$tid|proc_comm=$proc_comm|thread_comm=$task_comm|attr=$attr|cmdline=$cmd" >> "$out"
        done
        ;;
    esac
  done
...
dump_process_census() {
  ...
  {
    echo "A90_V1978_CENSUS label=$label uptime=$(cat /proc/uptime 2>/dev/null | awk '{print $1}')"
    ps -A -T -o PID,TID,PPID,NAME,CMD 2>&1 || ps -AT 2>&1 || ps -A 2>&1 || true
    echo "A90_V1978_PROC_SCAN"
    for proc in /proc/[0-9]*; do
      [ -d "$proc" ] || continue
      pid="$(basename "$proc")"
      comm="$(cat "$proc/comm" 2>/dev/null)"
...
dump_one_pid_attr() {
  ...
  prefix="$3"
  out="$LIBQMI_ATTR"
  echo "A90_V1978_PID_BEGIN label=$label pid=$pid prefix=$prefix" >> "$out"
  if [ -d "/proc/$pid" ]; then
    echo "exists=1" >> "$out"
  ...
  echo "A90_V1978_PID_END pid=$pid" >> "$out"
}

dump_libqmi_pid_attribution() {
  label="$1"
  [ -r "$LIBQMI_UPROBE" ] || return 0
  echo "A90_V1978_ATTR_BEGIN label=$label uptime=$(cat /proc/uptime 2>/dev/null | awk '{print $1}')" >> "$LIBQMI_ATTR"
  ...
  echo "A90_V1978_ATTR_END label=$label" >> "$LIBQMI_ATTR"
}
'''
...
def post_fs_data_script(samples: int, delay_us: int) -> str:
    ...
    text = patch_once(
        ...
        'LIBQMI_UPROBE_SUMMARY="$OUT/libqmi-uprobe-summary.txt"\n'
        'LIBQMI_ATTR="$OUT/libqmi-pid-attribution.txt"\n'
    ...
    text = patch_once(
        ...
        "    dump_libqmi_pid_attribution \"sample-$i\"\n",
    )
    text = patch_once(
        text,
    ...
    text = patch_once(
        text,
        "  dump_cnss_qrtr_trace\n  summarize_requests\n",
        "  dump_cnss_qrtr_trace\n  dump_libqmi_pid_attribution final\n  dump_process_census late\n  summarize_requests\n",
    )
    text = patch_once(
        text,
    ...
    text = patch_once(
        ...
        "    printf 'libqmi_uprobe_trace_lines='\n    count_nonempty_lines \"$LIBQMI_UPROBE\"\n"
        "    printf 'libqmi_pid_attribution_lines='\n    count_nonempty_lines \"$LIBQMI_ATTR\"\n",
    )
    return text
...
def parse_focus_census(text: str) -> dict[int, dict[str, Any]]:
    ...
    for line in text.splitlines():
        if not line.startswith("A90_V1978_FOCUS|"):
            continue
        fields: dict[str, str] = {}
...
def parse_task_map(text: str) -> dict[int, dict[str, Any]]:
    by_pid: dict[int, dict[str, Any]] = {}
    for line in text.splitlines():
        if not line.startswith("A90_V1978_TASK|"):
            continue
        fields: dict[str, str] = {}
        for part in line.split("|")[1:]:
            if "=" in part:
...
def analyze_pulled_evidence(store: v1934.EvidenceStore) -> dict[str, Any]:
    ...
    attribution_text = read_file(evidence_dir / "libqmi-pid-attribution.txt")
    ...
    analysis["v1978_attribution"] = summarize_pid_attribution(events, wlan_pd_time, attribution)
    analysis["v1978_attribution"]["attribution_file_present"] = bool(attribution_text)
    analysis["v1978_attribution"]["focus_census_file_present"] = bool(focus_text)
    analysis["v1978_attribution"]["task_map_file_present"] = bool(task_text)
    return analysis
...
def classify_result(
    ...
    if not selftest_ok:
        return "v1978-rollback-selftest-failed", False, "native rollback did not prove selftest fail=0", "rollback-selftest-failed"
    if not base_pass:
        return f"v1978-base-handoff-failed-{base_decision}", False, "underlying Android handoff did not complete", "android-handoff-failed"
    analysis = context.get("analysis") or {}
    if contaminated(analysis):
        return (
            "v1978-android-capture-rejected-degraded-or-pcie-mhi",
            False,
    ...
    if not stateup_complete(analysis):
        return (
            "v1978-android-normal-stateup-incomplete-rollback-pass",
            False,
    ...
    if not parser_ok:
        return "v1978-parser-chain-failed-rollback-pass", False, "Android capture succeeded but V1894/V1888 parser chain did not pass", "parser-chain-failed"

    uprobe = analysis.get("v1974_uprobe") or {}
    attribution = analysis.get("v1978_attribution") or {}
    if int(uprobe.get("send_event_count") or 0) <= 0:
        return (
            "v1978-libqmi-uprobe-incomplete-rollback-pass",
            False,
    ...
    if int(attribution.get("lead_lookup_pre_count") or 0) <= 0:
        return (
            "v1978-no-preup-dms-nas-wds-lookup-regression-rollback-pass",
            False,
    ...
    if int(attribution.get("lead_lookup_pre_rild_count") or 0) > 0:
        return (
            "v1978-preup-dms-nas-wds-attributed-rild-rollback-pass",
            True,
            "pre-UP DMS/NAS/WDS libqmi lookup thread was attributed to rild",
            "preup-lead-lookup-attributed-rild",
        )
    if int(attribution.get("lead_lookup_pre_unresolved_count") or 0) < int(attribution.get("lead_lookup_pre_count") or 0):
        return (
            "v1978-preup-dms-nas-wds-attributed-non-rild-rollback-pass",
            True,
            "pre-UP DMS/NAS/WDS libqmi lookup thread was attributed, but not to rild",
            "preup-lead-lookup-attributed-non-rild",
        )
    if attribution.get("attribution_file_present"):
        return (
            "v1978-preup-dms-nas-wds-attribution-unresolved-rollback-pass",
            True,
            "pre-UP DMS/NAS/WDS lookup reproduced, but live PID attribution could not resolve the process",
            "preup-lead-lookup-attribution-unresolved",
        )
    return (
        "v1978-pid-attribution-file-missing-rollback-pass",
        False,
        "normal Android state-up and pre-UP lookup reproduced, but PID attribution file was absent",
        "pid-attribution-missing",
    )
...
def main() -> int:
    ...
    if execute:
        ...
        decision = (
            "v1978-android-ril-qmi-pid-attribution-plan-ready"
            if args.command == "plan"
            else "v1978-android-ril-qmi-pid-attribution-dryrun-ready"
        )
        pass_ok = bool(base_pass)
        reason = "plan/dry-run completed without Android-good live capture"
        label = "android-ril-qmi-pid-attribution-handoff-ready"
