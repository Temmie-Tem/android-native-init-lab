#!/usr/bin/env python3
"""V1141 source/build-only gate for post-PM mdm_helper lower tracing."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1141-execns-helper-v215-build")
...
DEFAULT_V1140_MANIFEST = Path("tmp/wifi/v1140-post-pm-lower-gap-classifier/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1141-execns-helper-v215-build.txt")
HELPER_MARKER = "a90_android_execns_probe v215"
EXPECTED_V1140_DECISION = "v1140-post-pm-esoc0-only-gap-classified"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v215"',
    "allow_post_pm_mdm_helper_lower_trace",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1140 = load_json(args.v1140_manifest)
    analysis: dict[str, Any] = {
        "v1140": {
            "decision": v1140.get("decision", ""),
            "pass": bool(v1140.get("pass")),
            "path": str(repo_path(args.v1140_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1140-input",
            "status": "pass" if analysis["v1140"]["decision"] == EXPECTED_V1140_DECISION and analysis["v1140"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1140']['decision']} pass={analysis['v1140']['pass']}",
            "next_step": "refresh V1140 before building helper v215",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source lower-trace support",
        },
    ]
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1141 build-only gate"})
        return rows
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1141-post-pm-lower-trace-helper-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, PM actor, mdm_helper, CNSS actor, or Wi-Fi action executed",
            "run V1141 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1141-post-pm-lower-trace-helper-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1141-post-pm-lower-trace-helper-build-pass",
        True,
        f"helper v215 built sha256={analysis['build']['output_sha256']}",
        "V1142 should deploy helper v215; V1143 should run bounded post-PM lower-trace live without Wi-Fi HAL or bring-up",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1141 Post-PM Lower Trace Helper Build",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1140-manifest", type=Path, default=DEFAULT_V1140_MANIFEST)
