#!/usr/bin/env python3
"""V1817 host-only classifier for the next lower publication target."""
...
CYCLE = "V1817"
SOURCE_DIR = REPO_ROOT / "tmp" / "wifi" / "v1816-lower-publication-precondition-handoff"
...
OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1817-lower-publication-target-classifier"
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1817_LOWER_PUBLICATION_TARGET_CLASSIFIER_2026-06-03.md"
)
...
def collect_details(source: dict[str, Any],
    ...
    return {
        ...
        "native_lower_publication_label": gate.get("lower_publication_precondition_label", ""),
        ...
        "native_pd_mapper_positive": gate.get("raw_pd_mapper_positive"),
        "native_subsys_positive": gate.get("raw_subsys_positive"),
        "native_pil_positive": gate.get("raw_pil_positive"),
        "native_qmi_positive": gate.get("raw_qmi_positive"),
        "native_wlfw_positive": gate.get("raw_wlfw_positive"),
        ...
        "native_precondition_samples": gate.get("precondition_samples", []),
...
def native_lower_publication_gap(details: dict[str, Any]) -> bool:
    return (
        bool(details.get("source_pass"))
        and bool(details.get("rollback_ok"))
        and details.get("native_lower_publication_label") == "service74-raw-absent-preconditions-visible"
        ...
        and not boolish(details.get("native_pd_mapper_positive"))
        and boolish(details.get("native_subsys_positive"))
        and boolish(details.get("native_pil_positive"))
        and boolish(details.get("native_qmi_positive"))
...
def android_locator_baseline_positive(details: dict[str, Any]) -> bool:
...
def classify(details: dict[str, Any]) -> tuple[str, str]:
    if not native_lower_publication_gap(details):
        return (
            "native-lower-publication-shape-incomplete",
            "V1816 evidence did not match the fixed service180-present/service74-wlanpd-absent lower publication gap",
        )
    if not android_locator_baseline_positive(details):
        return (
            "android-locator-baseline-incomplete",
            "Android-positive baselines do not prove service locator, service 74, wlan_pd, WLFW, qmi-server, and wlan0 progression",
        )
    return (
        "wlan-pd-service-locator-visibility-target",
        "Native has MSS/subsys/PIL/QMI context but no pd-mapper text, wlan_pd text, service-notifier 74, WLFW service 69, or wlan0 while Android-good has service locator, service 74, wlan_pd, qmi-server, WLFW, and wlan0",
    )
...
def render_samples(samples: list[dict[str, Any]]) -> list[str]:
    ...
    for sample in samples:
        lines.extend([
            f"- `{sample.get('phase')}` counts pd-mapper/subsys/pil/qmi/wlfw: `{sample.get('raw_count_pd_mapper_text')}` / `{sample.get('raw_count_subsys_text')}` / `{sample.get('raw_count_pil_text')}` / `{sample.get('raw_count_qmi_text')}` / `{sample.get('raw_count_wlfw_text')}`",
            f"- `{sample.get('phase')}` last pd-mapper/wlan_pd: `{sample.get('last_pd_mapper')}` / `{sample.get('last_wlan_pd')}`",
        ])
    return lines
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Native V1816 Shape",
        "",
        f"- V1816 decision: `{d['source_decision']}`",
        f"- labels lower-publication/service74/PM-client/handoff/lower-state: `{d['native_lower_publication_label']}` / `{d['native_service74_raw_label']}` / `{d['native_pm_client_label']}` / `{d['native_handoff_klog_label']}` / `{d['native_lower_state_label']}`",
        ...
        f"- raw service180/service74/wlan_pd counts: `{d['native_service180_counts']}` / `{d['native_service74_counts']}` / `{d['native_wlan_pd_counts']}`",
        f"- lower precondition counts pd-mapper/subsys/pil/qmi/wlfw: `{d['native_pd_mapper_counts']}` / `{d['native_subsys_counts']}` / `{d['native_pil_counts']}` / `{d['native_qmi_counts']}` / `{d['native_wlfw_counts']}`",
        f"- lower precondition positives pd-mapper/subsys/pil/qmi/wlfw: `{d['native_pd_mapper_positive']}` / `{d['native_subsys_positive']}` / `{d['native_pil_positive']}` / `{d['native_qmi_positive']}` / `{d['native_wlfw_positive']}`",
        "- Broad native WLFW text is treated only as context because WLFW service 69 and `wlan0` remain absent.",
        *render_samples(d["native_precondition_samples"]),
        "",
        "## Native Lower State",
        "",
        ...
        f"- Android V622 deltas sysmon→locator, sysmon→SN180, SN180→SN74, SN180→wlan_pd, SN180→WLFW, wlan_pd→qmi-server: `{deltas['sysmon_modem_to_service_locator']}` / `{deltas['sysmon_modem_to_service_notifier_180']}` / `{deltas['service_notifier_180_to_service_notifier_74']}` / `{deltas['service_notifier_180_to_wlan_pd']}` / `{deltas['service_notifier_180_to_wlfw_start']}` / `{deltas['wlan_pd_to_qmi_server_connected']}`",
...
def main() -> int:
    ...
    result = {
        "cycle": CYCLE,
        "decision": f"v1817-{label}-host-pass",
        "pass": True,
        "reason": reason,
        "out_dir": rel(OUT_DIR),
