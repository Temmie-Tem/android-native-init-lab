#!/usr/bin/env python3
"""V1251 temporary-debugfs PMIC soft-reset read-surface preflight.

This extends V1250 by mounting debugfs only when it is not already mounted, then
running the same fail-closed helper v260 PMIC soft-reset preflight.  The live
scope is read-only observation plus temporary debugfs mount/umount cleanup.
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1251-pmic-soft-reset-debugfs-preflight-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1251-pmic-soft-reset-debugfs-preflight-live.txt")
...
APPROVAL_PHRASE = (
    "approve v1251 temporary debugfs read-surface PMIC preflight only; "
...
def run_debugfs_steps(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not mounted_before:
        ...
        mounted_by_v1251 = bool(mount_step.get("raw_ok"))
    steps.append(capture_native(args, store, "debugfs-mounts-during", proc_mounts_command(args), timeout=15.0))
    steps.append(capture_native(args, store, "pmic-soft-reset-debugfs-preflight", helper_command(args), timeout=args.timeout))
    if mounted_by_v1251:
        steps.append(
            capture_native(
                args,
    ...
    return {"mounted_before": mounted_before, "mounted_by_v1251": mounted_by_v1251}
...
def analyze(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]], mount_info: dict[str, Any]) -> dict[str, Any]:
    ...
    live_text = step_text(store, steps, "pmic-soft-reset-debugfs-preflight")
    ...
    return {
        ...
        "mounted_by_v1251": bool(mount_info.get("mounted_by_v1251")),
...
def build_checks(args: argparse.Namespace, command: str, analysis: dict[str, Any], steps: list[dict[str, Any]]) -> list[Check]:
    ...
    if command == "run":
        checks.extend([
            Check("debugfs-mounted", "pass" if analysis["mounted_during"] else "blocked", "blocker", f"before={analysis['mounted_before']} by_v1251={analysis['mounted_by_v1251']} during={analysis['mounted_during']}", "inspect debugfs mount command"),
            Check("live-command", "pass" if step_ok.get("pmic-soft-reset-debugfs-preflight") and analysis["live_output_present"] else "blocked", "blocker", f"result={analysis['result']} output_present={analysis['live_output_present']}", "inspect helper output"),
            ...
            Check("debugfs-cleanup", "pass" if analysis["cleanup_ok"] else "blocked", "blocker", f"before={analysis['mounted_before']} after={analysis['mounted_after']}", "unmount debugfs if V1251 mounted it"),
...
def decide(command: str, checks: list[Check], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    ...
    if command == "plan":
        return (
            "v1251-pmic-soft-reset-debugfs-preflight-plan-ready",
            True,
            "plan-only; no device command executed",
            "run V1251 preflight then approved temporary-debugfs read-surface gate",
        )
    if blockers:
        return (
            "v1251-pmic-soft-reset-debugfs-preflight-blocked",
            False,
    ...
    if command == "preflight":
        return (
            "v1251-pmic-soft-reset-debugfs-preflight-ready",
            True,
            "remote helper v260 and native health are ready for temporary-debugfs read-surface preflight",
            "run approved V1251 temporary-debugfs gate",
        )
    if analysis["native_reproduction_candidate"]:
        return (
            "v1251-pmic-debugfs-native-reproduction-candidate",
            True,
            "debugfs read surface confirms native PMIC/GDSC state is a reproduction candidate and no forbidden action executed",
            "design the bounded write gate only after a separate source/plan cycle",
        )
    if analysis["read_contract_ready"]:
        return (
            "v1251-pmic-debugfs-read-contract-ready-no-candidate",
            True,
            "debugfs read surface completed, but helper did not classify native state as a reproduction candidate",
            "inspect PMIC/GDSC fields before deciding the next gate",
        )
    return (
        "v1251-pmic-debugfs-read-surface-incomplete",
        True,
        "temporary debugfs gate executed safely but PMIC/GDSC read contract remains incomplete",
        "inspect helper fields and debugfs mount surface before any write-gate design",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ["mounted_by_v1251", analysis["mounted_by_v1251"]],
    ...
    return "\n".join([
        ...
        "## Safety",
        "",
        "- temporary debugfs mount is the only live environment change, and it is cleaned up when V1251 mounted it",
...
def main() -> int:
    ...
    manifest = {
        ...
        "temporary_debugfs_mount_executed": bool(mount_info.get("mounted_by_v1251")),
