#!/usr/bin/env python3
"""V1123 source/build-only gate for PM observer private firmware mounts."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1123-execns-helper-v212-build")
...
DEFAULT_V1122_MANIFEST = Path("tmp/wifi/v1122-provider-namespace-delta-classifier/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1123-execns-helper-v212-build.txt")
HELPER_MARKER = "a90_android_execns_probe v212"
EXPECTED_V1122_DECISION = "v1122-provider-regression-is-global-firmware-vendor-surface"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v212"',
    "pm_observer_private_firmware_mounts",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1122 = load_json(args.v1122_manifest)
    analysis: dict[str, Any] = {
        "v1122": {
            "decision": v1122.get("decision", ""),
            "pass": bool(v1122.get("pass")),
            "path": str(repo_path(args.v1122_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1122-input",
            "status": "pass" if analysis["v1122"]["decision"] == EXPECTED_V1122_DECISION and analysis["v1122"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1122']['decision']} pass={analysis['v1122']['pass']}",
            "next_step": "refresh V1122 before building helper v212",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source private firmware mount support",
        },
    ]
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1123 build-only gate"})
        return rows
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1123-private-firmware-pm-observer-helper-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1123 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1123-private-firmware-pm-observer-helper-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1123-private-firmware-pm-observer-helper-build-pass",
        True,
        f"helper v212 built sha256={analysis['build']['output_sha256']}",
        "deploy helper v212, then run private-firmware PM observer live gate",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1123 Private Firmware PM Observer Helper Build",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1122-manifest", type=Path, default=DEFAULT_V1122_MANIFEST)
