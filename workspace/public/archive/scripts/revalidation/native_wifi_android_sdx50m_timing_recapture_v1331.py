#!/usr/bin/env python3
"""V1331 Android read-only SDX50M/eSoC/PCIe/MHI timing recapture.
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1331-android-sdx50m-timing-recapture")
...
DMESG_PATTERN = (
    ...
    "mdm_subsys_powerup|esoc0|SDX50M|ap2mdm|mdm2ap|errfatal|err_fatal|"
...
echo A90_V1331_PROCESS_FDS_BEGIN
for d in /proc/[0-9]*; do
  ...
  case "$cmd" in
    *A90_V1331_PROCESS_FDS*) wanted=0 ;;
  esac
  if [ "$wanted" = "1" ]; then
...
echo A90_V1331_PROCESS_FDS_END
"""

INTERRUPTS_COMMAND = (
    "echo A90_V1331_INTERRUPTS_BEGIN; "
    "cat /proc/interrupts 2>&1 | grep -Ei 'mdm|esoc|gpio|status|err|fatal|pcie|mhi|142|135' || true; "
    "echo A90_V1331_INTERRUPTS_END"
)
...
def prop_command() -> str:
    ...
    return "; ".join([
        "echo A90_V1331_PROPS_BEGIN",
        f"for p in {props}; do echo \"$p=$(getprop $p 2>/dev/null)\"; done",
        "echo A90_V1331_PROPS_END",
    ])


def dmesg_filtered_command() -> str:
    return f"dmesg 2>&1 | grep -Ei {DMESG_PATTERN!r} | tail -n 1800 || true"
...
def collect(args: argparse.Namespace, store: EvidenceStore) -> list[Capture]:
    ...
    return [
        capture_shell(args, store, "v1331-props", prop_command(), 15.0),
        capture_shell(args, store, "v1331-dmesg-filtered", dmesg_filtered_command(), 35.0),
        capture_shell(args, store, "v1331-dmesg-unfiltered-tail", dmesg_unfiltered_command(), 35.0),
        capture_shell(args, store, "v1331-process-fds", PROCESS_FD_COMMAND, 25.0),
        capture_shell(args, store, "v1331-interrupts", INTERRUPTS_COMMAND, 15.0),
    ]
...
def summarize(captures: list[Capture], store: EvidenceStore) -> dict[str, Any]:
    props = parse_key_values(capture_text(captures, "v1331-props"))
    dmesg_text = capture_text(captures, "v1331-dmesg-filtered", "v1331-dmesg-unfiltered-tail")
    process_text = capture_text(captures, "v1331-process-fds")
    interrupts_text = capture_text(captures, "v1331-interrupts")
    ...
    store.write_text("android-v1331-props-normalized.txt", normalized_props)
    return {
...
def decide(args: argparse.Namespace,
    ...
    if args.command == "plan":
        return (
            "v1331-android-sdx50m-timing-plan-ready",
            True,
    ...
    if devices["device_count"] == 0:
        return (
            "v1331-android-adb-unavailable",
            False,
            "no Android ADB device is currently visible",
            "run V1331 Android handoff from native/recovery state or verify adb",
        )
    if not selected_device_available(args, devices):
        return (
            "v1331-android-adb-selection-needed",
            False,
    ...
    if args.command == "preflight":
        return (
            "v1331-android-sdx50m-timing-preflight-ready",
            True,
            "one Android ADB device is visible",
            "run V1331 Android read-only timing recapture",
        )
    if not captures:
        return "v1331-capture-missing", False, "run command produced no captures", "inspect adb or collector failure"
    if not summary.get("boot_completed"):
        return (
            "v1331-android-not-boot-complete",
            False,
            "Android ADB is visible but sys.boot_completed=1 was not captured",
            "wait for boot-complete and rerun V1331",
        )
    if not summary.get("response_present"):
        return (
            "v1331-android-response-chain-missing",
            False,
    ...
    if subsys_time is not None and wlfw_time is not None and wlfw_time < subsys_time:
        return (
            "v1331-android-wlfw-before-subsys-esoc0",
            True,
    ...
    if subsys_time is not None and pcie_time is not None:
        if pcie_time > subsys_time:
            return (
                "v1331-android-pcie-after-subsys-esoc0",
                True,
        ...
        return (
            "v1331-android-pcie-before-captured-subsys-esoc0",
            True,
    ...
    if subsys_time is None:
        return (
            "v1331-android-esoc0-marker-missing-but-response-present",
            True,
    ...
    return (
        "v1331-android-clock-source-incomparable",
        True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1331 Android SDX50M Timing Recapture",
        "",
