#!/usr/bin/env python3
"""V526 Android handoff wrapper for V525 companion-service identity recapture.

Live mode temporarily flashes a known Android boot image, boots Android, waits
for boot-complete, runs the V525 read-only companion identity recapture, then
restores the native init boot image. It does not enable Wi-Fi, scan, connect,
link up interfaces, change credentials, start Wi-Fi daemons directly, or route
traffic.
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v526-android-companion-identity-handoff")
...
def v525_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v525-android-companion-identity-run"


def v526_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    ...
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            if args.serial:
                v525_command.extend(["--serial", args.serial])
            v525_command.append("run")
            plan.append(("v525-android-companion-identity", v525_command, max(args.timeout * 14, 600)))
        else:
...
def load_v525_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v525_out_dir(store) / "manifest.json"
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v525_out_dir": str(v525_out_dir(store)),
    }
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v526-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v526-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v526-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v526-handoff-approval-required", False

    plan = v526_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_plan_command(plan)
    if offenders:
        context["forbidden_plan_commands"] = offenders
        return [], context, "v526-handoff-forbidden-command-blocked", False

    steps: list[StepResult] = []
    if args.command == "plan":
        for name, command, _ in plan:
            steps.append(write_step(store, name, command, "[plan] not executed\n", "", 0, 0.0, skipped=True, ok_override=True))
        return steps, context, "v526-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v526-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v526-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v526-handoff-readback-failed-rollback-attempted", False
        if name == "v525-android-companion-identity":
            v525_ok = step.ok
        if execute and not step.ok and name not in {"wait-boot-complete"}:
            return steps, context, f"v526-handoff-failed-{name}", False

    context["boot_state"] = boot_state
    v525_result = load_v525_result(store)
    context["v525_result"] = {
        "path": v525_result.get("_path"),
        "decision": v525_result.get("decision"),
        "pass": v525_result.get("pass"),
        "reason": v525_result.get("reason"),
        "next_step": v525_result.get("next_step"),
    }
    if execute and not boot_state.get("boot_completed"):
        return steps, context, "v526-handoff-bootcomplete-missing-rollback-complete", False
    if execute and not v525_ok:
        return steps, context, "v526-handoff-v525-capture-failed-rollback-complete", False
    return steps, context, "v526-handoff-pass" if execute else "v526-handoff-dryrun-ready", True


def reason_for(decision: str) -> str:
    return {
        "v526-handoff-plan-ready": "execution plan generated without device mutation",
        "v526-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v526-handoff-approval-required": "live run refused because approval flags are missing",
        "v526-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v526-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v526-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v526-handoff-forbidden-command-blocked": "handoff plan contains a forbidden active Wi-Fi command pattern",
        "v526-handoff-pass": "Android handoff, V525 identity recapture, and native rollback completed",
        "v526-handoff-v525-capture-failed-rollback-complete": "V525 Android identity recapture failed, but native rollback steps completed",
        "v526-handoff-bootcomplete-missing-rollback-complete": "Android boot-complete was not observed, but rollback steps completed",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V526 Android Companion Identity Handoff",
        "",
        ...
        f"- v525_out_dir: `{manifest['context']['v525_out_dir']}`",
        "",
        ...
        "",
        "```json",
        json.dumps(manifest["context"].get("v525_result", {}), ensure_ascii=False, indent=2, sort_keys=True),
        "```",
        "",
        "## Guardrails",
        "",
        ...
        "- Android work is limited to V525 read-only companion identity recapture.",
...
def main() -> int:
    ...
    manifest = {
        ...
        "boot_partition_write_executed": args.command == "run" and decision != "v526-handoff-approval-required",
    ...
    print(f"v525: {manifest['context'].get('v525_result', {})}")
