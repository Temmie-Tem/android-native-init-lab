#!/usr/bin/env python3
"""V1577 local-only sanity verifier for the service-window PM proxy contract devnode test boot."""
...
DEFAULT_MANIFEST = (
    ...
    / "tmp"
    / "wifi"
    / "v1577-service-window-pm-proxy-contract-devnode-test-boot"
    / "manifest.json"
)
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1577-service-window-pm-proxy-contract-devnode-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1577_SERVICE_WINDOW_PM_PROXY_CONTRACT_DEVNODE_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1577-service-window-pm-proxy-contract-devnode-test-boot-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v292"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.69 (v1577-service-window-pm-proxy-contract-devnode)",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1577-service-window-pm-proxy-contract-devnode-artifact-sanity-pass",
            True,
            "V1577 service-window mdm_helper launch-contract PM proxy contract test boot artifact passed local sanity",
        )
    return (
        "v1577-service-window-pm-proxy-contract-devnode-artifact-sanity-blocked",
        False,
        "V1577 PM proxy contract artifact sanity failed; fix local artifact before any live handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1577 manifest: `{manifest['_path']}`",
        f"- V1577 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
