#!/usr/bin/env python3
"""V2029 rollbackable handoff for dual RFS wlanmdsp TFTP serving."""
...
import native_wifi_rfs_fallback_tftp_full_chain_handoff_v2027 as prev2027


prev2021 = prev2027.prev2021
CYCLE = "V2029"
OUT_DIR = prev2021.prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2029-dual-rfs-wlanmdsp-tftp-handoff"
)
HANDOFF_DIR = OUT_DIR / "v2028-handoff"
HANDOFF_REPORT = OUT_DIR / "v2028-handoff-report.md"
REPORT_PATH = prev2021.prev2013.prev1998.prev1992.prev.repo_path(
    "docs/reports/NATIVE_INIT_V2029_DUAL_RFS_WLANMDSP_TFTP_HANDOFF_2026-06-04.md"
)
V2028_OUT = prev2021.prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2028-dual-rfs-wlanmdsp-tftp-test-boot"
)
V2028_INIT = V2028_OUT / "init_v2028_dual_rfs_wlanmdsp_tftp"
V2028_BOOT = V2028_OUT / "boot_linux_v2028_dual_rfs_wlanmdsp_tftp.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2028/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.196 (v2028-dual-rfs-wlanmdsp-tftp)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2028.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2028.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2028-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v382"


ORIGINAL_COLLECT_DETAILS = prev2027.ORIGINAL_COLLECT_DETAILS


def rel(path: Path) -> str:
    return prev2027.rel(path)


def intish(value: object) -> int:
    return prev2027.intish(value)


def hit(events: dict[str, dict[str, str]], name: str) -> int:
    return prev2027.hit(events, name)


def is_zero(value: str) -> bool:
    return prev2027.is_zero(value)


def path_has(paths: dict[str, int], needle: str) -> bool:
    return prev2027.path_has(paths, needle)
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2028_INIT, init_required), (V2028_BOOT, boot_required)):
        forbidden_tokens = init_forbidden if path == V2028_INIT else ()
        if not path.exists():
...
def classify(handoff: dict[str, Any],
    ...
    if not route_ok:
        label = "dual-rfs-tftp-route-regression"
        reason = "V2028 did not preserve rollback, light observer, dual RFS bridges, full chain, or bounded TFTP trace"
        passed = False
    elif intish(cascade.get("wlan0")) > 0:
        label = "dual-rfs-wlan0-progress"
        reason = "dual RFS wlanmdsp serving reached wlan0; stop before credentials/scan/connect until gated"
        passed = True
...
def rows_to_md(rows: list[list[object]]) -> str:
    return prev2027.rows_to_md(rows)
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    matrix_rows = [
        ...
        ["rfs_probe", classification.get("probe_success"), f"path={bridge.get('probe_path')} exists={bridge.get('probe_exists')} size={bridge.get('probe_size')} open_rc={bridge.get('probe_open_rc')}"],
...
def configure_prev2021() -> None:
    ...
    prev2021.V2020_OUT = V2028_OUT
    prev2021.V2020_INIT = V2028_INIT
    prev2021.V2020_BOOT = V2028_BOOT
