#!/usr/bin/env python3
"""V1387 bounded Android participant parity + pre-poll corrected RC1 enumerate live gate."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1387-android-participant-prepoll-corrected-rc1-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1387-android-participant-prepoll-corrected-rc1-live.txt")
PLAN_OUT_DIR = Path("tmp/wifi/v1387-android-participant-prepoll-corrected-rc1-plan")
PLAN_LATEST_POINTER = Path("tmp/wifi/latest-v1387-android-participant-prepoll-corrected-rc1-plan.txt")
REPORT_PATH = Path("docs/reports/NATIVE_INIT_V1387_ANDROID_PARTICIPANT_PREPOLL_CORRECTED_RC1_LIVE_2026-06-01.md")
...
CORRECTED_RC1_FLAG = "--pm-observer-late-per-proxy-prepoll-corrected-rc1-enumerate"
CORRECTED_PREFIX = "pm_service_trigger_observer.corrected_rc1_enumerate."
PREPOLL_PREFIX = "pm_service_trigger_observer.prepoll_corrected_rc1."
...
def _force_v1387_child_command(original):
...
def configure() -> None:
    ...
    base.CYCLE_LABEL = "v1387"
    base.CYCLE_NAME = "V1387"
    base.SUMMARY_HEADING = "V1387 Android Participant Pre-Poll Corrected RC1 Live"
    base.EVIDENCE_FILE_PREFIX = "v1387"
    base._force_response_sampler_child_command = _force_v1387_child_command
...
def _read_global_dmesg_text(manifest: dict[str, Any]) -> str:
    run_dir = Path(str(manifest.get("_run_dir") or _run_dir()))
    candidate = run_dir / "native/global-dmesg-after-observer.txt"
    try:
        return candidate.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return ""
...
def _first_dmesg_time(lines: list[str], *needles: str) -> float | None:
    for line in lines:
...
def _first_dmesg_line(lines: list[str], *needles: str) -> str:
    for line in lines:
        if all(needle in line for needle in needles):
            return line
    return ""
...
def _dmesg_rc1_summary(text: str) -> dict[str, Any]:
    lines = ANSI_RE.sub("", text).splitlines()
    esoc0_time = _first_dmesg_time(lines, "__subsystem_get: esoc0")
    test11_time = _first_dmesg_time(lines, "PCIe: TEST: 11")
    assert_time = _first_dmesg_time(lines, "Assert the reset of endpoint of RC1")
    phy_time = _first_dmesg_time(lines, "PCIe RC1 PHY is ready")
    release_time = _first_dmesg_time(lines, "Release the reset of endpoint of RC1")
    l0_time = _first_dmesg_time(lines, "LTSSM_STATE: LTSSM_L0")
    link_failed_time = _first_dmesg_time(lines, "link initialization failed")
    poll_compliance_time = _first_dmesg_time(lines, "LTSSM_STATE: LTSSM_POLL_COMPLIANCE")
    transition_seen = test11_time is not None or assert_time is not None or phy_time is not None
    return {
        "transition_seen": transition_seen,
        "l0_seen": l0_time is not None,
        "link_failed_seen": link_failed_time is not None,
        "poll_compliance_seen": poll_compliance_time is not None,
        "esoc0_time": esoc0_time,
        "test11_time": test11_time,
        "assert_time": assert_time,
        "phy_ready_time": phy_time,
        "release_time": release_time,
        "poll_compliance_time": poll_compliance_time,
        "l0_time": l0_time,
        "link_failed_time": link_failed_time,
        "esoc0_to_assert_sec": None if esoc0_time is None or assert_time is None else assert_time - esoc0_time,
        "release_to_link_failed_sec": None if release_time is None or link_failed_time is None else link_failed_time - release_time,
        "line_esoc0": _first_dmesg_line(lines, "__subsystem_get: esoc0"),
        "line_test11": _first_dmesg_line(lines, "PCIe: TEST: 11"),
        "line_assert": _first_dmesg_line(lines, "Assert the reset of endpoint of RC1"),
        "line_phy_ready": _first_dmesg_line(lines, "PCIe RC1 PHY is ready"),
        "line_release": _first_dmesg_line(lines, "Release the reset of endpoint of RC1"),
        "line_poll_compliance": _first_dmesg_line(lines, "LTSSM_STATE: LTSSM_POLL_COMPLIANCE"),
        "line_link_failed": _first_dmesg_line(lines, "link initialization failed"),
    }
...
def _corrected_summary(raw: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "phase": raw.get("phase", ""),
        "monotonic_ms": _int_value(raw.get("monotonic_ms"), -1),
        "gate_per_mgr_subsys_esoc0_count": _int_value(raw.get("gate_per_mgr_subsys_esoc0_count"), -1),
        "gate_pm_service_powerup_thread_count": _int_value(raw.get("gate_pm_service_powerup_thread_count"), -1),
...
def _prepoll_summary(raw: dict[str, str]) -> dict[str, Any]:
    return {
        "emitted": bool(raw),
        "begin": _int_value(raw.get("begin"), 0) == 1,
        "end": _int_value(raw.get("end"), 0) == 1,
        "poll_max": _int_value(raw.get("poll_max"), -1),
        "poll_interval_us": _int_value(raw.get("poll_interval_us"), -1),
        "start_monotonic_ms": _int_value(raw.get("start_monotonic_ms"), -1),
        "poll_count": _int_value(raw.get("poll_count"), -1),
        "elapsed_ms": _int_value(raw.get("elapsed_ms"), -1),
        "last_gate_per_mgr_subsys_esoc0_count": _int_value(raw.get("last_gate_per_mgr_subsys_esoc0_count"), -1),
        "last_gate_pm_service_powerup_thread_count": _int_value(raw.get("last_gate_pm_service_powerup_thread_count"), -1),
        "triggered": _int_value(raw.get("triggered"), 0) == 1,
        "raw": raw,
    }
...
def decide_v1387(manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if manifest.get("command") == "plan":
        return (
            "v1387-android-participant-prepoll-corrected-rc1-plan-ready",
            True,
            "plan-only; no device command or live action executed",
            "run V1387 bounded live gate with helper v285",
        )

    sampler = manifest.get("response_sampler") or {}
    route = manifest.get("current_route") or {}
    corrected = manifest.get("corrected_rc1_enumerate") or {}
    prepoll = manifest.get("prepoll_corrected_rc1") or {}
    dmesg_rc1 = manifest.get("dmesg_rc1") or {}

    if route.get("corrected_rc1_flag_in_child_script") != 1:
        return (
            "v1387-corrected-rc1-flag-missing",
            False,
            "child command did not include the corrected RC1 enumerate flag",
            "repair V1387 command injection before live retry",
        )
    if not _forbidden_safety_clear(manifest):
        return (
            "v1387-safety-violation",
            False,
            "one or more forbidden Wi-Fi/network/PMIC/GPIO/eSoC safety markers was nonzero",
            "stop and audit V1387 output before retry",
        )
    if not sampler.get("timing_emitted") or not sampler.get("timing_end"):
        return (
            "v1387-timing-summary-missing",
            False,
    ...
    if not corrected.get("emitted") or not corrected.get("end"):
        return (
            "v1387-corrected-rc1-summary-missing",
            False,
            "corrected RC1 enumerate summary did not emit a complete block",
            "inspect helper output and debugfs availability before retry",
        )
    if not prepoll.get("emitted") or not prepoll.get("end"):
        return (
            "v1387-prepoll-summary-missing",
            False,
            "pre-poll corrected RC1 gate summary did not emit a complete block",
            "inspect helper v285 output before retry",
        )
    if not corrected.get("triggered"):
        return (
            "v1387-corrected-rc1-not-triggered",
            False,
    ...
    if not prepoll.get("triggered") or not str(corrected.get("phase", "")).startswith("late_per_proxy_prepoll_"):
        return (
            "v1387-corrected-rc1-not-prepoll-triggered",
            False,
            f"corrected RC1 enumerate did not fire from the v285 pre-poll gate: phase={corrected.get('phase')} prepoll_triggered={prepoll.get('triggered')}",
            "repair V1387 pre-poll ordering before another live retry",
        )
    if _int_value(corrected.get("gate_pm_service_powerup_thread_count"), -1) <= 0:
        return (
            "v1387-corrected-rc1-triggered-without-powerup-thread",
            False,
            f"corrected RC1 enumerate triggered without the v285 powerup-thread gate: gate_pm_service_powerup_thread_count={corrected.get('gate_pm_service_powerup_thread_count')}",
            "stop and inspect corrected RC1 gate evidence before retry",
        )
    if corrected.get("rc_sel_rc") != 0 or corrected.get("case_rc") != 0:
        return (
            "v1387-corrected-rc1-write-failed",
            False,
    ...
    if _timing_downstream_progress(sampler):
        return (
            "v1387-corrected-rc1-downstream-progress",
            True,
            "pre-poll corrected RC1 enumerate ran in the Android participant powerup-thread window and downstream GPIO/PCI/MHI/WLFW/wlan0 progress appeared",
            "classify first progressed surface before Wi-Fi HAL, scan/connect, DHCP, or external ping",
        )
    if bool(sampler.get("timing_pcie_rc1_transition_seen")) or bool(dmesg_rc1.get("transition_seen")):
        delta = dmesg_rc1.get("esoc0_to_assert_sec")
        delta_text = f" (esoc0-to-assert {delta:.3f}s)" if isinstance(delta, (int, float)) else ""
        return (
            "v1387-corrected-rc1-ltssm-no-downstream-clean",
            True,
            "pre-poll corrected RC1 enumerate fired from the v285 pre-poll gate"
            f"{delta_text}; dmesg shows RC1 LTSSM activity before link failure, but no GPIO142/PCI/MHI/WLFW/wlan0 appeared",
            "compare V1387 action timing and LTSSM phase against Android before any new participant or lower-branch mutation",
        )
    return (
        "v1387-corrected-rc1-no-rc1-transition",
        True,
        "pre-poll corrected RC1 enumerate write returned inside the Android participant powerup-thread window, but timing summary saw no RC1/downstream transition",
...
def augment_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    dmesg_text = _read_global_dmesg_text(manifest)
    corrected = _corrected_summary(_parse_prefixed_lines(run_text, CORRECTED_PREFIX))
    prepoll = _prepoll_summary(_parse_prefixed_lines(run_text, PREPOLL_PREFIX))
    route = manifest.get("current_route") or {}
    route.update({
    ...
    manifest["prepoll_corrected_rc1"] = prepoll
    manifest["dmesg_rc1"] = _dmesg_rc1_summary(dmesg_text)
    ...
    decision, passed, reason, next_step = decide_v1387(manifest)
...
def _key_rows(manifest: dict[str, Any]) -> list[list[Any]]:
    ...
    return [
        ...
        ["corrected_monotonic_ms", corrected.get("monotonic_ms")],
        ["corrected_gate_per_mgr_subsys_esoc0_count", corrected.get("gate_per_mgr_subsys_esoc0_count")],
        ["corrected_gate_pm_service_powerup_thread_count", corrected.get("gate_pm_service_powerup_thread_count")],
        ...
        ["prepoll_triggered", prepoll.get("triggered")],
        ["prepoll_poll_count", prepoll.get("poll_count")],
        ["prepoll_elapsed_ms", prepoll.get("elapsed_ms")],
        ["prepoll_last_gate_per_mgr_subsys_esoc0_count", prepoll.get("last_gate_per_mgr_subsys_esoc0_count")],
        ["prepoll_last_gate_pm_service_powerup_thread_count", prepoll.get("last_gate_pm_service_powerup_thread_count")],
        ...
        ["dmesg_pcie_rc1_transition_seen", dmesg_rc1.get("transition_seen")],
        ["dmesg_esoc0_to_assert_sec", dmesg_rc1.get("esoc0_to_assert_sec")],
        ["dmesg_release_to_link_failed_sec", dmesg_rc1.get("release_to_link_failed_sec")],
        ["dmesg_l0_seen", dmesg_rc1.get("l0_seen")],
        ["dmesg_link_failed_seen", dmesg_rc1.get("link_failed_seen")],
...
def render_summary(manifest: dict[str, Any]) -> str:
    return "\n".join([
        "# V1387 Android Participant Pre-Poll Corrected RC1 Live",
        "",
...
def render_report(manifest: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "## Dmesg RC1 Lines",
        "",
        *[f"- `{key}`: {value}" for key, value in (manifest.get("dmesg_rc1") or {}).items() if key.startswith("line_") and value],
        "",
    ])
...
def print_result(manifest: dict[str, Any]) -> None:
    print(f"v1387_decision: {manifest.get('decision')}")
    print(f"v1387_pass: {manifest.get('pass')}")
    print(f"v1387_reason: {manifest.get('reason')}")
    print(f"v1387_next: {manifest.get('next_step')}")
    print(f"v1387_manifest: {_run_dir() / 'manifest.json'}")
