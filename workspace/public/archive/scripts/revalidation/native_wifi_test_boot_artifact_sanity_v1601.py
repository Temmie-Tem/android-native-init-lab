#!/usr/bin/env python3
"""V1601 local-only sanity verifier for the V1600 PM-first late-per-proxy PPH-gate lower-marker test boot."""
...
base.DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1601-pm-first-late-per-proxy-pph-gate-lower-marker-artifact-sanity"
base.DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1601_PM_FIRST_LATE_PER_PROXY_PPH_GATE_LOWER_MARKER_ARTIFACT_SANITY_2026-06-02.md"
)
base.EXPECTED_DECISION = "v1600-pm-first-late-per-proxy-pph-gate-lower-marker-test-boot-source-build-pass"
base.EXPECTED_HELPER = "a90_android_execns_probe_v297"
base.EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.105 (v1600-pm-first-late-per-proxy-pph-gate-lower-marker)",
    "a90_android_execns_probe v297",
    "A90v1600",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "watch_sec": 120,
        "helper_mode": "android-service-window-pm-proxy-contract-pm-first-late-per-proxy-pph-gate-lower-marker",
        "helper_runtime_mode": "wifi-companion-android-wifi-service-window-subsys-trigger-capture",
        "helper_result": "/cache/native-init-wifi-test-boot-v1600-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if pass_ok:
        return (
            ...
            True,
            "V1600 PM-first late-per-proxy PPH-gate lower-marker test boot artifact passed local sanity",
        )
    return (
        "v1601-pm-first-late-per-proxy-pph-gate-lower-marker-artifact-sanity-blocked",
        False,
        f"V1600 artifact sanity failed; base decision was {decision}",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1600 manifest: `{manifest['_path']}`",
        f"- V1600 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
