#!/usr/bin/env python3
"""V2037 rollbackable handoff for dual-RFS + passive logdw TFTP transfer."""
...
OUT_DIR = prev2007.prev2000.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2037-dual-rfs-logdw-transfer-handoff"
)
HANDOFF_DIR = OUT_DIR / "v2036-handoff"
HANDOFF_REPORT = OUT_DIR / "v2036-handoff-report.md"
REPORT_PATH = prev2007.prev2000.prev1998.prev1992.prev.repo_path(
    "docs/reports/NATIVE_INIT_V2037_DUAL_RFS_LOGDW_TRANSFER_HANDOFF_2026-06-04.md"
)
V2036_OUT = prev2007.prev2000.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2036-dual-rfs-logdw-transfer-test-boot"
)
V2036_INIT = V2036_OUT / "init_v2036_dual_rfs_logdw_transfer"
V2036_BOOT = V2036_OUT / "boot_linux_v2036_dual_rfs_logdw_transfer.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2036/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.199 (v2036-dual-rfs-logdw-transfer)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2036.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2036.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2036-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v384"
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2036_INIT, init_required), (V2036_BOOT, boot_required)):
        forbidden_tokens = init_forbidden if path == V2036_INIT else boot_forbidden
        if not path.exists():
...
def classify(handoff: dict[str, Any],
    ...
    wlanmdsp_logdw = intish(logdw_summary.get("wlanmdsp")) > 0
    transfer_complete = (
        wlanmdsp_logdw
        and (
    ...
    if not route_ok:
        label = "dual-rfs-logdw-route-regression"
        reason = "V2036 did not preserve rollback, light observer, cnss-daemon, dual-RFS bridges, helper, or passive logdw prerequisites"
        passed = False
    elif intish(cascade.get("wlan0")) > 0:
        label = "dual-rfs-logdw-wlan0-progress"
        reason = "dual-RFS plus passive logdw route reached wlan0; stop before credentials/scan/connect until a dedicated gate"
        passed = True
    elif intish(cascade.get("fw_ready")) > 0:
        label = "dual-rfs-logdw-fw-ready-progress"
        reason = "dual-RFS plus passive logdw route crossed into FW-ready progress"
        passed = True
    elif transfer_complete and cap_bdf_cal_success:
        label = "dual-rfs-logdw-wlanmdsp-transfer-complete-post-cal-no-fw-ready"
        reason = "passive logdw shows wlanmdsp transfer completion and cap/BDF/cal success, but FW-ready/wlan0 did not follow"
        passed = True
    elif transfer_complete:
        label = "dual-rfs-logdw-wlanmdsp-transfer-complete-no-fw-ready"
        reason = "passive logdw shows wlanmdsp transfer completion, but FW-ready/wlan0 did not follow"
        passed = True
    elif wlanmdsp_logdw:
        label = "dual-rfs-logdw-wlanmdsp-request-no-complete"
        reason = "passive logdw saw wlanmdsp activity, but no transfer-complete marker"
        passed = True
    elif intish(logdw_summary.get("server_check")) > 0:
        label = "dual-rfs-logdw-initial-tftp-no-wlanmdsp"
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    matrix_rows = [
        ...
        ["rfs_probe", bridge.get("probe_exists") == 1, f"path={bridge.get('probe_path')} open_rc={bridge.get('probe_open_rc')} errno={bridge.get('probe_open_errno')}"],
...
def patch_prev_module() -> None:
    ...
    prev1998.V1997_OUT = V2036_OUT
    prev1998.V1997_INIT = V2036_INIT
    prev1998.V1997_BOOT = V2036_BOOT
