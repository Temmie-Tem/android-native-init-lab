#!/usr/bin/env python3
"""V1412 local-only sanity verifier for the V1411 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1411-wifi-test-boot-kmsg-fallback" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1412-wifi-test-boot-kmsg-fallback-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1412_WIFI_TEST_BOOT_KMSG_FALLBACK_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1411-wifi-test-boot-kmsg-fallback-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v286"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.74 (v1411-wifitest)",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1412-wifi-test-boot-kmsg-fallback-artifact-sanity-pass",
            True,
            "V1411 kmsg-fallback test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1412-wifi-test-boot-kmsg-fallback-artifact-sanity-blocked",
        False,
        "V1411 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1411 manifest: `{manifest['_path']}`",
        f"- V1411 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- Wi-Fi test kmsg fallback contract: `{checks['wifi_test_contract']['ok']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1411-rc1-watcher.result"
            ),
        },
    }
