#!/usr/bin/env python3
"""V1571 local-only sanity verifier for the service-window mdm_helper launch-contract test boot."""
...
DEFAULT_MANIFEST = (
    ...
    / "tmp"
    / "wifi"
    / "v1571-mdm-helper-launch-contract-test-boot"
    / "manifest.json"
)
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1571-mdm-helper-launch-contract-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1571_MDM_HELPER_LAUNCH_CONTRACT_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1571-mdm-helper-launch-contract-test-boot-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v289"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.69 (v1571-mdm-helper-launch-contract)",
    "a90_android_execns_probe v289",
    "v1571",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            ...
            True,
            "V1571 service-window mdm_helper launch-contract test boot artifact passed local sanity",
        )
    return (
        "v1571-mdm-helper-launch-contract-artifact-sanity-blocked",
        False,
        "V1571 artifact sanity failed; fix local artifact before any live handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1571 manifest: `{manifest['_path']}`",
        f"- V1571 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
