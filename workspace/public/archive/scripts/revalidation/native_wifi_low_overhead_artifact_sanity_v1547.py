#!/usr/bin/env python3
"""V1547 local-only sanity verifier for the V1546 low-overhead test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1547-low-overhead-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1547_LOW_OVERHEAD_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1546-low-overhead-endpoint-observer-test-boot-source-build-pass"
EXPECTED_BOOT_MARKERS = (
    ...
    "/cache/native-init-wifi-test-boot-v1546.pid",
    "/cache/native-init-wifi-test-boot-v1546-supervisor.pid",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1546-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1546-low-overhead-endpoint.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1547-low-overhead-artifact-sanity-pass",
            True,
            "V1546 low-overhead observer artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1547-low-overhead-artifact-sanity-blocked",
        False,
        "V1546 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1546 manifest: `{manifest['_path']}`",
        f"- V1546 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- absent slow/unsafe markers: `{checks['boot_absent_markers']['absent_ok']}`",
        f"- low-overhead contract: `{checks['wifi_test_contract']['ok']}`",
