#!/usr/bin/env python3
"""V1020 bounded after-fd upper-surface subsystem-window live gate.

Runs deployed helper v173 with
`--service-manager-order after-mdm-helper-esoc-fd-with-wifi-surface-subsys-window`
and `--subsys-trigger-gate post-upper-surface-no-wlfw`. This permits a bounded
`/dev/subsys_esoc0` child only after mdm_helper fd, service-manager, Wi-Fi HAL
legacy/ext, wificond, and CNSS are up. It still forbids IWifi.start,
qcwlanstate, scan/connect, credentials, DHCP/routes, external ping, raw eSoC
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v1020-after-fd-subsys-window-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v1020-after-fd-subsys-window-live.txt")
...
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v173"
...
DEFAULT_SERVICE_MANAGER_ORDER = "after-mdm-helper-esoc-fd-with-wifi-surface-subsys-window"
DEFAULT_SUBSYS_TRIGGER_GATE = "post-upper-surface-no-wlfw"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--subsys-trigger-gate",
        choices=("post-upper-surface-no-wlfw",),
...
def decide(
    ...
    if args.command == "plan":
        if not (local["exists"] and local["sha256"] == args.helper_sha256 and local["marker"] and local["mode"]):
            return "v1020-plan-helper-v173-missing", False, f"local={local}", "build/deploy helper v173 before V1020"
        return "v1020-after-fd-subsys-window-plan-ready", True, "plan-only; no device command executed", "run bounded V1020 live gate"
    missing = required_flags(args)
    if missing:
        return "v1020-after-fd-subsys-window-approval-required", False, f"missing flags: {', '.join(missing)}", "rerun with explicit V1020 flags"
    helper = analysis.get("helper") or {}
    failed_steps = v923.step_failures(steps, helper)
    if failed_steps:
        return "v1020-step-failed", False, f"failed_steps={failed_steps}", "inspect V1020 evidence before retry"
    remote = analysis.get("remote_helper") or {}
    if not (remote.get("sha_ok") and remote.get("marker_ok") and remote.get("mode_ok")):
        return "v1020-helper-v173-remote-mismatch", False, f"remote={remote}", "redeploy helper v173 before V1020"
    if helper.get("forbidden_true"):
        return "v1020-forbidden-action-detected", False, f"forbidden={helper.get('forbidden_true')}", "stop and audit helper before retry"
    contract = helper.get("contract") or {}
    if contract.get("begin") != "1" or contract.get("allowed") != "1":
        return "v1020-helper-mode-not-executed", False, f"contract={contract}", "fix V1020 helper command before retry"
    if contract.get("matrix_mode") != "1" or contract.get("service_manager_order") != args.service_manager_order:
        return "v1020-matrix-order-mismatch", False, f"contract={contract}", "fix V1020 helper order command before retry"
    if contract.get("subsys_trigger_gate") != args.subsys_trigger_gate:
        return "v1020-subsys-gate-mismatch", False, f"contract={contract}", "fix V1020 helper gate command before retry"
    if contract.get("per_mgr_start_attempted") != "1" or contract.get("mdm_helper_start_attempted") != "1":
        return "v1020-runtime-actors-not-attempted", False, f"contract={contract}", "repair actor ordering before retry"
    cleanup = analysis.get("reboot_cleanup") or {}
    if analysis.get("cleanup_needed") and not cleanup.get("healthy"):
        return "v1020-reboot-cleanup-review", False, f"cleanup={cleanup}", "verify or rerun recovery reboot before continuing"
    result = contract.get("result")
    if result == "mdm-helper-esoc-fd-missing-no-open":
        return "v1020-mdm-helper-esoc-fd-missing-no-open", True, f"contract={contract}", "classify fd regression before upper-surface retry"
    if contract.get("mdm_helper_esoc0_fd_seen") != "1":
        return "v1020-mdm-helper-esoc-fd-not-proven", False, f"contract={contract}", "inspect fd gate evidence"
    if contract.get("wifi_hal_started") != "1" or contract.get("wificond_started") != "1":
        return "v1020-upper-wifi-surface-not-started", False, f"contract={contract}", "inspect Wi-Fi HAL/wificond child output"
    if result == "wlfw-precondition-observed-trigger-clean":
        return "v1020-wlfw-precondition-observed-trigger-clean", True, f"contract={contract}", "inspect WLFW/BDF/wlan0 before scan/connect"
    if result == "post-upper-surface-no-wlfw-trigger-clean":
        return "v1020-post-upper-surface-subsys-trigger-clean", True, f"contract={contract}", "inspect WLFW/BDF/wlan0 after scoped subsystem window"
    if result in {"wlfw-precondition-missing-no-open", "wlfw-precondition-missing-no-open-output-truncated"}:
        return "v1020-subsys-window-did-not-open", False, f"contract={contract}", "inspect why post-upper-surface trigger gate did not fire"
    if result == "reboot-required":
        return "v1020-reboot-required-cleaned", True, f"contract={contract}", "inspect pre-reboot matrix evidence"
    return "v1020-after-fd-subsys-window-review", False, f"contract={contract}", "inspect V1016 helper output before continuing"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "## Guardrails",
        "",
        "- Permits only current-boot selinuxfs mount/cleanup, private property shim, `/vendor/bin/pm-service`, `/vendor/bin/mdm_helper`, service-manager trio, Wi-Fi HAL legacy/ext, `wificond`, `/vendor/bin/cnss_diag`, `/vendor/bin/cnss-daemon -n -l`, and the post-upper-surface no-WLFW-gated `/dev/subsys_esoc0` child open.",
