#!/usr/bin/env python3
"""V970 bounded live proof for helper v161 Android Wi-Fi service-window mode."""
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v970-android-service-window-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v970-android-service-window-live.txt")
...
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v161"
base.MODE = "wifi-companion-android-wifi-service-window-start-only"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--toybox-timeout-sec", type=int, default=44)
...
def decide(
    ...
    if args.command == "plan":
        if not (local["exists"] and local["sha256"] == args.helper_sha256 and local["marker"] and local["mode"]):
            return "v970-plan-helper-v161-missing", False, f"local={local}", "deploy helper v161 before V970"
        return "v970-android-service-window-plan-ready", True, "plan-only; no device command executed", "run bounded V970 service-window proof"
    missing = required_flags(args)
    if missing:
        return "v970-android-service-window-approval-required", False, f"missing flags: {', '.join(missing)}", "rerun with explicit V970 flags"
    helper = analysis.get("helper") or {}
    failed_steps = step_failures(steps, helper)
    if failed_steps:
        return "v970-step-failed", False, f"failed_steps={failed_steps}", "inspect V970 evidence before retry"
    remote = analysis.get("remote_helper") or {}
    if not (remote.get("sha_ok") and remote.get("marker_ok") and remote.get("mode_ok")):
        return "v970-helper-v161-remote-mismatch", False, f"remote={remote}", "redeploy helper v161 before V970"
    if helper.get("forbidden_true"):
        return "v970-forbidden-action-detected", False, f"forbidden={helper.get('forbidden_true')}", "stop and audit helper before retry"
    contract = helper.get("contract") or {}
    if contract.get("begin") != "1" or contract.get("allowed") != "1" or contract.get("exec_attempted") != "1":
        return "v970-helper-mode-not-executed", False, f"contract={contract}", "fix V970 helper command before retry"
    ...
    if not expected_started or contract.get("child_started") != "14":
        return "v970-service-window-actors-not-started", False, f"contract={contract}", "repair service-window actor order before retry"
    cleanup = analysis.get("reboot_cleanup") or {}
    if analysis.get("cleanup_needed") and not cleanup.get("healthy"):
        return "v970-reboot-cleanup-review", False, f"cleanup={cleanup}", "verify or rerun recovery cleanup before continuing"
    result = contract.get("result")
    if result == "wlfw-precondition-observed":
        return (
            "v970-android-service-window-wlfw-observed",
            True,
            f"contract={contract}",
            "classify WLFW/BDF/wlan0 evidence before scan/connect",
        )
    if result == "service-window-no-wlfw":
        return (
            "v970-android-service-window-no-wlfw",
            True,
            f"contract={contract}",
            "compare with V968 timing and decide whether an early GPIO sampler or a narrower Android actor delta is needed",
        )
    if result == "start-only-runtime-gap":
        return (
            "v970-android-service-window-runtime-gap",
            True,
            f"contract={contract}",
            "classify which Android service-window actor exited before timeout",
        )
    if result == "start-only-reboot-required":
        return (
            "v970-android-service-window-reboot-cleaned",
            True,
            f"contract={contract}",
            "inspect pre-cleanup service-window evidence",
        )
    return "v970-android-service-window-review", False, f"contract={contract}", "inspect V970 helper output before continuing"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "## Guardrails",
        "",
        "- Permits service-manager trio, QRTR companion stack, Wi-Fi HAL service processes, `wificond`, `mdm_helper`, `cnss_diag`, and `cnss-daemon` only as a start-only service window.",
        "- `qcwlanstate`, `IWifi.start`, `/dev/subsys_esoc0` open, eSoC ioctl, scan/connect, credentials, DHCP/routes, and external ping are forbidden.",
