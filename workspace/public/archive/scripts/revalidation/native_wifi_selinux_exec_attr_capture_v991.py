#!/usr/bin/env python3
"""V991 source/build verifier for post-exec SELinux attr capture."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v991-selinux-exec-attr-capture")
LATEST_POINTER = Path("tmp/wifi/latest-v991-selinux-exec-attr-capture.txt")
DEFAULT_HELPER_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_BUILD_ARTIFACT = Path("tmp/wifi/v991-execns-helper-v168-build/a90_android_execns_probe")
DEFAULT_BUILD_LOG = Path("tmp/wifi/v991-execns-helper-v168-build/build.log")
...
def classify(source: str, build_rc: int, build_log: str, artifact: Path) -> dict[str, Any]:
    capture_fn = function_body(source, "append_capture_snapshot_compact")
    attr_fn = function_body(source, "append_proc_attr_value_compact")
    strings_output = artifact_strings(artifact)
    checks = {
        "execns_version_v168": 'EXECNS_VERSION "a90_android_execns_probe v168"' in source,
        "attr_capture_helper_added": (
            "append_proc_attr_value_compact" in source
            and "capture.%s.%s.value=" in attr_fn
            and "append_escaped_ascii" in attr_fn
        ),
        "exec_snapshot_captures_current_attr": (
            'append_proc_attr_value_compact(buf, pid, label, "attr/current")' in capture_fn
        ),
        "exec_snapshot_captures_exec_attr": (
            'append_proc_attr_value_compact(buf, pid, label, "attr/exec")' in capture_fn
        ),
        "wificond_trace_path_preserved": all(
            token in source
            for token in (
        ...
        "strings_confirm_version": "a90_android_execns_probe v168" in strings_output,
    ...
    return {
        ...
        if passed
        else "v991-selinux-exec-attr-capture-incomplete",
        "pass": passed,
        "reason": (
            "helper v168 records attr/current and attr/exec at traced exec/crash stops"
            if passed
            else "missing checks: " + ", ".join(name for name, ok in checks.items() if not ok)
        ),
        "next_step": (
            "deploy helper v168, then rerun bounded Android service-window proof to confirm post-exec SELinux attrs"
            if passed
            else "repair attr capture before deploy"
        ),
        "checks": checks,
    }
