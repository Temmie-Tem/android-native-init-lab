#!/usr/bin/env python3
"""V1575 local-only sanity verifier for the service-window PM proxy contract test boot."""
...
DEFAULT_MANIFEST = (
    ...
    / "tmp"
    / "wifi"
    / "v1575-service-window-pm-proxy-contract-test-boot"
    / "manifest.json"
)
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1575-service-window-pm-proxy-contract-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1575_SERVICE_WINDOW_PM_PROXY_CONTRACT_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1575-service-window-pm-proxy-contract-test-boot-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v291"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.69 (v1575-service-window-pm-proxy-contract)",
    "a90_android_execns_probe v291",
    "v1575",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1575-service-window-pm-proxy-contract-artifact-sanity-pass",
            True,
            "V1575 service-window mdm_helper launch-contract PM proxy contract test boot artifact passed local sanity",
        )
    return (
        "v1575-service-window-pm-proxy-contract-artifact-sanity-blocked",
        False,
        "V1575 PM proxy contract artifact sanity failed; fix local artifact before any live handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1575 manifest: `{manifest['_path']}`",
        f"- V1575 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
