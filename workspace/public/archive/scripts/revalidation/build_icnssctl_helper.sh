...
SRC="${ROOT_DIR}/stage3/linux_init/helpers/a90_icnssctl.c"
OUT="${1:-${ROOT_DIR}/stage3/linux_init/helpers/a90_icnssctl}"
...
aarch64-linux-gnu-readelf -d "${OUT}" >/tmp/a90_icnssctl_dynamic_check.txt 2>&1 || true
cat /tmp/a90_icnssctl_dynamic_check.txt
if ! grep -q "There is no dynamic section" /tmp/a90_icnssctl_dynamic_check.txt; then
