#!/usr/bin/env python3
"""V1331 bounded Android handoff for SDX50M/eSoC timing recapture.

This wrapper temporarily boots a known Android boot image, waits for Android
boot completion, runs the V1331 read-only timing collector, then restores the
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1331-android-sdx50m-timing-handoff")
...
def v1331_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v1331-android-sdx50m-timing-recapture-run"


def v1331_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    ...
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            plan.append(("v1331-android-sdx50m-timing-recapture", collector, collector_timeout))
        else:
            plan.append((name, command, timeout))
    return plan


def load_v1331_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v1331_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v1331 = load_v1331_result(store)
    android_summary = v1331.get("android_summary") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v1331-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass before V1331"
    elif not v1331.get("pass"):
        decision = "v1331-handoff-v1331-failed-rollback-complete"
        pass_ok = False
        reason = "V1331 Android timing collector failed after rollback completed"
    else:
        decision = str(v1331.get("decision") or "v1331-handoff-review-required")
        pass_ok = bool(v1331.get("pass"))
        reason = str(v1331.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "v1331": {
            "path": v1331.get("_path"),
            "decision": v1331.get("decision"),
            "pass": v1331.get("pass"),
            "reason": v1331.get("reason"),
            "next_step": v1331.get("next_step"),
        },
    }
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v1331_out_dir": str(v1331_out_dir(store)),
    ...
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v1331-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v1331-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v1331-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v1331-handoff-approval-required", False

    plan = v1331_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_active_wifi(plan)
    if offenders:
        context["forbidden_active_wifi_steps"] = offenders
        return [], context, "v1331-handoff-active-wifi-command-blocked", False
    ...
    if args.command == "plan":
        ...
        return steps, context, "v1331-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v1331-android-sdx50m-timing-recapture"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v1331-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v1331-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v1331-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v1331-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v1331-android-sdx50m-timing-recapture":
            v1331_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v1331-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v1331_step_ok:
            return steps, context, "v1331-handoff-v1331-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v1331-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v1331-handoff-plan-ready": "execution plan generated without device mutation",
        "v1331-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v1331-handoff-approval-required": "live run refused because approval flags are missing",
        "v1331-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v1331-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v1331-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v1331-handoff-active-wifi-command-blocked": "handoff plan contains a forbidden active Wi-Fi command pattern",
        "v1331-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v1331-handoff-v1331-failed-rollback-complete": "V1331 Android timing collector failed, and native rollback completed",
        "v1331-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v1331-handoff-readback-failed-rollback-attempted": "Android boot readback failed or SHA did not match; native rollback was attempted",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- v1331_out_dir: `{manifest['context']['v1331_out_dir']}`",
        "",
        "## Comparison",
        "",
        f"- boot_complete: `{comparison.get('boot_complete', '-')}`",
        f"- v1331_decision: `{(comparison.get('v1331') or {}).get('decision', '-')}`",
        f"- v1331_next_step: `{(comparison.get('v1331') or {}).get('next_step', '-')}`",
        "",
        ...
        "- Live mode temporarily flashes Android boot, runs V1331 read-only collection, then restores native init.",
...
def main() -> int:
    ...
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v1331-handoff-approval-required",
            "v1331-handoff-missing-native-rollback",
            "v1331-handoff-missing-android-boot",
            "v1331-handoff-image-collision",
            "v1331-handoff-active-wifi-command-blocked",
        },
