#!/usr/bin/env python3
"""V1055 bounded PM full-contract live proof with helper v180 plain-open fallback modem pre-holder."""
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v1055-pm-full-contract-with-modem-holder-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v1055-pm-full-contract-with-modem-holder-live.txt")
base.DEFAULT_LOCAL_HELPER = Path("tmp/wifi/v1053-execns-helper-v180-build/a90_android_execns_probe")
base.DEFAULT_HELPER_SHA256 = HELPER_SHA256_V180
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v180"
...
def _map_v1055(value: str) -> str:
    return (
        value.replace("v1032", "v1055")
        .replace("V1032", "V1055")
        .replace("v175", "v180")
        .replace("v177", "v180")
        .replace("V1055", "V1055")
...
def decide(args, local: dict[str, Any], steps: list[dict[str, Any]], analysis: dict[str, Any]):
    ...
    if args.command == "plan":
        return "v1055-pm-full-contract-with-modem-holder-plan-ready", pass_ok, _map_v1055(reason), "refresh V401/V490, then run bounded V1055 live gate"
    ...
    if decision in {
        ...
        return _map_v1055(decision), pass_ok, _map_v1055(reason), _map_v1055(next_step)

    if contract.get("pm_full_contract_with_modem_holder_matrix") != "1":
        return (
            "v1055-modem-holder-matrix-missing",
            False,
            "pm_full_contract_with_modem_holder_matrix!=1",
            "fix helper command or v180 matrix selection before retry",
        )
    if contract.get("modem_pre_holder_confirmed") != "1":
        return (
            "v1055-modem-pre-holder-not-confirmed-clean",
            True,
            (
                "modem_pre_holder_confirmed=0 "
                f"child_chroot={contract.get('modem_pre_holder_child_chroot')} "
                f"nonblock_errno={contract.get('modem_pre_holder_nonblock_errno')} "
                f"plain_retry={contract.get('modem_pre_holder_plain_retry')} "
                f"first_errno={contract.get('modem_pre_holder_first_errno')} "
    ...
    if contract.get("pm_full_contract_seen") == "1":
        return (
            "v1055-modem-holder-pm-full-contract-seen",
            True,
            (
    ...
    return (
        "v1055-modem-holder-confirmed-pm-full-contract-missing",
        True,
        (
        ...
        "inspect v1055 PM fd snapshots before deciding whether holder timing or actor order needs repair",
    )
...
def render_summary(manifest):
    return (
        ...
        .replace("# V1032 PM Runtime-Domain Guard Live", "# V1055 PM Full-Contract with Modem Holder Live")
        .replace("V1032", "V1055")
        .replace("helper `v175`", "helper `v180`")
        .replace("helper v175", "helper v180")
        .replace("v175", "v180")
    )
...
def build_manifest(args, store):
    ...
    manifest["rerun_after_v1054_v180_deploy"] = True
    ...
    manifest["modem_pre_holder_nonblock_opened"] = contract.get("modem_pre_holder_nonblock_opened") == "1"
    manifest["modem_pre_holder_nonblock_errno"] = contract.get("modem_pre_holder_nonblock_errno", "")
    manifest["modem_pre_holder_plain_retry"] = contract.get("modem_pre_holder_plain_retry") == "1"
    manifest["modem_pre_holder_first_errno"] = contract.get("modem_pre_holder_first_errno", "")
