#!/usr/bin/env python3
"""V1516 local-only sanity verifier for the V1515 critical-source Wi-Fi test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1516-wifi-critical-source-pre-l0-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1516_WIFI_CRITICAL_SOURCE_PRE_L0_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1515-wifi-critical-source-pre-l0-test-boot-source-build-pass"
EXPECTED_BOOT_MARKERS = (
    ...
    "/cache/native-init-wifi-test-boot-v1515.pid",
    "/cache/native-init-wifi-test-boot-v1515-supervisor.pid",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1515-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1515-critical-source-pre-l0.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1516-wifi-critical-source-pre-l0-artifact-sanity-pass",
            True,
            "V1515 critical-source pre-L0 test boot artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1516-wifi-critical-source-pre-l0-artifact-sanity-blocked",
        False,
        "V1515 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1515 manifest: `{manifest['_path']}`",
        f"- V1515 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- critical-source pre-L0 contract: `{checks['wifi_test_contract']['ok']}`",
