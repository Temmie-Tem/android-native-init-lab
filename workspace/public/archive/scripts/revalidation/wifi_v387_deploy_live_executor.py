#!/usr/bin/env python3
"""Fail-closed executor for V387 helper v18 deploy and ptrace timeout cleanup.

This sequences the V387 tools:

* helper v18 deploy wrapper
* service-manager ptrace timeout cleanup live runner
* runtime-gap classifier

Approved actions remain split. Without exact V387 approval phrases and explicit
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v387-deploy-live-executor")
DEPLOY_APPROVAL_PHRASE = "approve v387 deploy execns helper v18 only; no daemon start and no Wi-Fi bring-up"
LIVE_APPROVAL_PHRASE = "approve v387 service-manager ptrace timeout cleanup only; no Wi-Fi HAL start and no Wi-Fi bring-up"
DEPLOY_APPROVAL_BLOCKER = "exact-v387-deploy-approval-phrase"
LIVE_APPROVAL_BLOCKER = "exact-v387-ptrace-timeout-cleanup-live-approval-phrase"
...
def deploy_ok(store: EvidenceStore) -> bool:
    ...
    return manifest.get("decision") == "execns-helper-v18-deploy-pass" and bool(manifest.get("pass"))
...
def refusal_manifest(args: argparse.Namespace, store: EvidenceStore, blockers: list[str]) -> dict[str, Any]:
    return base_result(
        ...
        store,
        [],
        "v387-deploy-live-executor-approval-required",
        True,
        "exact V387 approval phrase and explicit flags are required for requested live action",
        "provide exact V387 approval phrase only if accepting the scoped action",
        blockers,
        False,
        False,
        False,
...
def classify_if_needed(args: argparse.Namespace, store: EvidenceStore, steps: list[StepResult]) -> tuple[str, bool, str, str, list[str]]:
    ...
    if live.get("decision") == "service-manager-start-only-live-runtime-gap":
        ...
        return (
            str(classified.get("decision") or "v387-classify-missing"),
    ...
    return (
        str(live.get("decision") or "v387-live-missing"),
        bool(live.get("pass")),
        f"live decision={live.get('decision')} pass={live.get('pass')}",
        str(live.get("next_step") or "inspect V387 live evidence"),
        [],
    )
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    if args.command == "plan":
        ...
        return base_result(
            args, store, steps,
            "v387-deploy-live-executor-plan-ready",
            True,
            "plan-only; deploy/live remain approval-gated",
            "run V387 deploy with exact approval before ptrace timeout cleanup live capture",
    ...
    if args.command in {"deploy", "full"}:
        ...
        if not deploy_ok(store):
            return base_result(
                args, store, steps,
                "v387-deploy-live-executor-deploy-review",
                False,
        ...
        if args.command == "deploy":
            return base_result(
                args, store, steps,
                "v387-deploy-live-executor-deploy-pass",
                True,
                f"deploy decision={load_json(deploy_dir(store) / 'manifest.json').get('decision')}",
                "run V387 ptrace timeout cleanup live capture after exact live approval phrase",
    ...
    if not live_preflight_ok(store):
        return base_result(
            args, store, steps,
            "v387-deploy-live-executor-live-preflight-blocked",
            False,
    ...
    return base_result(
        args, store, steps,
        f"v387-deploy-live-executor-{decision_prefix}-{decision}",
        pass_ok,
        reason,
        next_step,
        remaining,
