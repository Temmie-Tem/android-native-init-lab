#!/usr/bin/env python3
"""V1281 source/build-only gate for PCIe/GDSC/kmsg response sampler support."""
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v1281-execns-helper-v268-build")
base.DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v268")
base.DEFAULT_V1268 = Path("tmp/wifi/v1280-v1279-response-gap-classifier/manifest.json")
base.LATEST_POINTER = Path("tmp/wifi/latest-v1281-execns-helper-v268-build.txt")
base.HELPER_MARKER = "a90_android_execns_probe v268"
base.EXPECTED_V1268_DECISION = "v1280-pcie-gdsc-response-sampler-selected"
base.REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v268"',
    "collect_response_kmsg_markers",
    "kmsg_line_interesting",
    "kmsg_pcie_count=%d",
    "kmsg_gdsc_count=%d",
    "kmsg_mhi_count=%d",
    "kmsg_esoc_count=%d",
    "kmsg_wlfw_count=%d",
    "kmsg_filtered_block=%s",
)
...
def decide(command: str, rows: list[dict[str, str]], analysis: dict) -> tuple[str, bool, str, str]:
    if command == "plan":
        return ("v1281-pcie-kmsg-sampler-build-plan-ready", True, "plan-only", "run V1281 source/build-only gate")
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1281-pcie-kmsg-sampler-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1281-pcie-kmsg-sampler-build-pass",
        True,
        f"helper v268 built sha256={analysis['build']['stage3']['sha256']}",
        "V1282 should deploy helper v268 only; V1283 should run the bounded PCIe/GDSC/kmsg sampler",
    )
...
def checks(command: str, analysis: dict) -> list[dict[str, str]]:
    ...
    for row in rows:
        if row.get("name") == "v1268-input":
            row["name"] = "v1280-input"
            row["next_step"] = "rerun V1280 before helper build"
        elif row.get("name") == "source-strings":
            row["next_step"] = "repair helper source PCIe/kmsg sampler support"
        elif row.get("name") == "binary-strings":
            row["next_step"] = "repair built helper PCIe/kmsg sampler strings"
    return rows
...
def render_summary(manifest: dict) -> str:
    ...
    return "\n".join([
        "# V1281 PCIe/GDSC/kmsg Sampler Build",
        "",
