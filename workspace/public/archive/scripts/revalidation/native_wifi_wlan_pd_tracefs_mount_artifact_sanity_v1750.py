#!/usr/bin/env python3
"""V1750 local-only sanity verifier for the V1749 tracefs mount-restore test boot."""
...
DEFAULT_MANIFEST = (
    REPO_ROOT / "tmp" / "wifi" / "v1749-wlan-pd-tracefs-mount-restore-test-boot" / "manifest.json"
)
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1750-wlan-pd-tracefs-mount-restore-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1750_WLAN_PD_TRACEFS_MOUNT_RESTORE_ARTIFACT_SANITY_2026-06-03.md"
)
EXPECTED_DECISION = "v1749-wlan-pd-tracefs-mount-restore-source-build-pass"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.143 (v1749-wlan-pd-tracefs-mount-restore)",
    "a90_android_execns_probe v329",
    "A90v1749",
    "native-init-wifi-test-boot-v1749",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            ...
            True,
            "V1749 tracefs mount-restore artifact passed local sanity; one rollbackable live handoff may be planned separately",
        )
    return (
        "v1750-wlan-pd-tracefs-mount-restore-artifact-sanity-blocked",
        False,
        "V1749 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1749 manifest: `{manifest['_path']}`",
        f"- V1749 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- mount_debugfs enabled: `{checks['route_contract']['mount_debugfs']}`",
