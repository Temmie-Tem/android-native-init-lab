#!/usr/bin/env python3
"""V1233 source/build-only gate for mdm_helper post-WAIT_FOR_REQ branch snapshots."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1233-execns-helper-v257-build")
DEFAULT_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_OUTPUT_NAME = "a90_android_execns_probe"
DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v257")
DEFAULT_BUILD_SCRIPT = Path("scripts/revalidation/build_android_execns_probe_helper.sh")
DEFAULT_V1232_MANIFEST = Path("tmp/wifi/v1232-mdm-helper-post-wait-req-ks-observer-live/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1233-execns-helper-v257-build.txt")
HELPER_MARKER = "a90_android_execns_probe v257"
EXPECTED_V1232_DECISION = "v1232-wait-req-returned-no-ks-mhi"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v257"',
    "pm_observer_mdm_helper_post_wait_req_branch_snapshot",
    "--pm-observer-mdm-helper-post-wait-req-branch-snapshot",
    "append_mdm_helper_post_wait_req_branch_snapshot",
    "emit_mdm_helper_post_wait_req_branch_snapshot",
    "post_wait_req.branch_snapshot_enabled=%d",
    "post_wait_branch.begin=1",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1232 = load_json(args.v1232_manifest)
    analysis: dict[str, Any] = {
        "v1232": {
            "decision": v1232.get("decision", ""),
            "pass": bool(v1232.get("pass")),
            "path": str(repo_path(args.v1232_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1232-input",
            "status": "pass" if analysis["v1232"]["decision"] == EXPECTED_V1232_DECISION and analysis["v1232"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1232']['decision']} pass={analysis['v1232']['pass']}",
            "next_step": "refresh V1232 before building helper v257",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source branch snapshot support",
        },
    ]
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1233 build-only gate"})
        return rows
    ...
    rows.extend([
        ...
        },
        {
            ...
            "next_step": "copy built helper to stage3/linux_init/helpers/a90_android_execns_probe_v257",
        },
        {
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1233-post-wait-req-branch-snapshot-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, eSoC ioctl, PM actor, mdm_helper, CNSS actor, or Wi-Fi action executed",
            "run V1233 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1233-post-wait-req-branch-snapshot-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1233-post-wait-req-branch-snapshot-build-pass",
        True,
        f"helper v257 built sha256={analysis['build']['output']['sha256']}",
        "V1234 should deploy helper v257 only; V1235 should run the bounded branch snapshot without Wi-Fi HAL or bring-up",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1232-manifest", type=Path, default=DEFAULT_V1232_MANIFEST)
