#!/usr/bin/env python3
"""V917 bounded mdm_helper /dev/subsys_esoc0 trigger capture proof.

Runs deployed helper v150 in wifi-companion-mdm-helper-runtime-subsys-trigger-capture
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v917-mdm-helper-subsys-trigger-capture-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v917-mdm-helper-subsys-trigger-capture-live.txt")
base.DEFAULT_LOCAL_HELPER = Path("tmp/wifi/v916-execns-helper-v150-build/a90_android_execns_probe")
base.DEFAULT_HELPER_SHA256 = "920283483ade4ce3a989cfe84ecc6094b3b45dcb1af323bbba374f6e22e93572"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v150"
...
def decide(args: argparse.Namespace,
    ...
    if args.command == "plan":
        if not (local["exists"] and local["sha256"] == args.helper_sha256 and local["marker"] and local["mode"]):
            return "v917-plan-helper-v150-missing", False, f"local={local}", "build and deploy helper v150 before V917"
        return "v917-mdm-helper-subsys-trigger-plan-ready", True, "plan-only; no device command executed", "run bounded V917 subsys trigger capture proof"
    missing = required_flags(args)
    if missing:
        return "v917-mdm-helper-subsys-trigger-approval-required", False, f"missing flags: {', '.join(missing)}", "rerun with explicit V917 flags"
    helper = analysis.get("helper") or {}
    failed_steps = base.step_failures(steps, helper)
    if failed_steps:
        return "v917-step-failed", False, f"failed_steps={failed_steps}", "inspect V917 evidence before retry"
    remote = analysis.get("remote_helper") or {}
    if not (remote.get("sha_ok") and remote.get("marker_ok") and remote.get("mode_ok")):
        return "v917-helper-v150-remote-mismatch", False, f"remote={remote}", "redeploy helper v150 before V917"
    if helper.get("forbidden_true"):
        return "v917-forbidden-action-detected", False, f"forbidden={helper.get('forbidden_true')}", "stop and audit helper before retry"
    contract = helper.get("contract") or {}
    if contract.get("begin") != "1" or contract.get("allowed") != "1":
        return "v917-helper-mode-not-executed", False, f"contract={contract}", "fix V917 helper command before retry"
    if contract.get("per_mgr_start_attempted") != "1" or contract.get("mdm_helper_start_attempted") != "1":
        return "v917-runtime-actors-not-attempted", False, f"contract={contract}", "repair actor ordering before retry"
    cleanup = analysis.get("reboot_cleanup") or {}
    if analysis.get("cleanup_needed") and not cleanup.get("healthy"):
        return "v917-reboot-cleanup-review", False, f"cleanup={cleanup}", "verify or rerun recovery reboot before continuing"
    result = contract.get("result")
    if result == "trigger-window-captured":
        return "v917-subsys-trigger-window-captured", True, f"contract={contract}", "inspect WLFW/BDF/wlan0 and mdm3 deltas before any HAL/scan/connect work"
    if result == "trigger-not-attempted-no-esoc-fd":
        return "v917-trigger-not-attempted-no-esoc-fd", True, f"contract={contract}", "classify why mdm_helper did not hold /dev/esoc-0 under native runtime inputs"
    if result == "mdm-helper-not-observable":
        return "v917-mdm-helper-not-observable-clean", True, f"contract={contract}", "classify mdm_helper startup/runtime dependency before retry"
    if result == "reboot-required":
        return "v917-reboot-required-cleaned", True, f"contract={contract}", "inspect pre-reboot subsys trigger evidence"
    if result in {"property-shim-setup-failed", "manual-review-required"}:
        return "v917-subsys-trigger-setup-failed", False, f"contract={contract}", "repair property/per_mgr/mdm_helper trigger setup before retry"
    return "v917-mdm-helper-subsys-trigger-review", False, f"contract={contract}", "inspect V917 helper output before continuing"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V917 mdm_helper Subsys Trigger Capture",
        "",
