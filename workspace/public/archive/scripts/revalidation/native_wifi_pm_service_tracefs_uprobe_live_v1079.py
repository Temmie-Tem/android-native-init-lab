#!/usr/bin/env python3
"""V1079 bounded tracefs-only PM-service uprobe collector."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1079-pm-service-tracefs-uprobe-live")
DEFAULT_V1078_MANIFEST = Path("tmp/wifi/v1078-pm-service-uprobe-active-live/manifest.json")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1079"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1079/pm-observer.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1079/tracefs-uprobe-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1079/pm-observer-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1079-pm-service-tracefs-uprobe-live.txt")
...
ALLOWED_V1078_DECISIONS = {
    "v1078-uprobe-active-failed",
...
echo tracefs_uprobe_collector=v1079
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1079-pm-service-tracefs-uprobe-plan-ready",
            True,
            "plan-only; no tracefs write, PM actor, BPF attach, or Wi-Fi action executed",
            "run V1079 with explicit allow flags",
        )
    v1078_decision = (manifest.get("v1078") or {}).get("decision")
    if v1078_decision not in ALLOWED_V1078_DECISIONS:
        return (
            "v1079-v1078-predecessor-missing",
            False,
            f"unexpected V1078 decision={v1078_decision!r}",
            "rerun or inspect V1078 before tracefs-only collector",
        )
    missing = required_flags(args)
    if missing:
        return (
            "v1079-pm-service-tracefs-uprobe-approval-required",
            False,
            "missing explicit flags: " + ", ".join(missing),
            "rerun with all V1079 allow flags",
        )
    analysis = manifest.get("analysis") or {}
    tracefs = analysis.get("tracefs_uprobe") or {}
    post = analysis.get("post_surface") or {}
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1079-execns-helper-sha-mismatch", False, "remote execns helper is not v196", "redeploy V1074 helper v196")
    if not analysis.get("pm_binary_visible"):
        return ("v1079-pm-binary-not-visible", False, "read-only vendor mount did not expose pm-service", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1079-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1079-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1079-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1079-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1079-pm-service-tracefs-uprobe-no-hit", False, "no pm-service uprobe trace lines were captured", "verify uprobe offset/file mapping or child runtime")
    return (
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    v1078 = load_json(args.v1078_manifest)
    manifest: dict[str, Any] = {
        "cycle": "v1079",
        "generated_at": now_iso(),
        "command": args.command,
        "v1078": {
            "manifest": str(repo_path(args.v1078_manifest)),
            "decision": v1078.get("decision", ""),
            "pass": bool(v1078.get("pass")),
        },
    ...
    if (
        ...
        and v1078.get("decision") in ALLOWED_V1078_DECISIONS
    ):
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1079 PM Service Tracefs Uprobe Live",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1078-manifest", type=Path, default=DEFAULT_V1078_MANIFEST)
