#!/usr/bin/env python3
"""V1426 local-only sanity verifier for the V1425 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1425-wifi-test-boot-rc1-retry" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1426-wifi-test-boot-rc1-retry-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1426_WIFI_TEST_BOOT_RC1_RETRY_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1425-wifi-test-boot-rc1-retry-source-build-pass"
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1426-wifi-test-boot-rc1-retry-artifact-sanity-pass",
            True,
            "V1425 RC1 retry test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1426-wifi-test-boot-rc1-retry-artifact-sanity-blocked",
        False,
        "V1425 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1425 manifest: `{manifest['_path']}`",
        f"- V1425 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- Wi-Fi test RC1 retry contract: `{checks['wifi_test_contract']['ok']}`",
        ...
        f"- RC1 retry delay ms: `{checks['wifi_test_contract']['rc1_retry_delay_ms']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1425-rc1-watcher.result"
                and wifi_test.get("rc1_window_sampler") is True
                and wifi_test.get("rc1_window_result") == "/cache/native-init-wifi-test-boot-v1425-rc1-window.result"
                and wifi_test.get("rc1_retry_count") == 2
                and wifi_test.get("rc1_retry_delay_ms") == 500
            ),
        },
    }
