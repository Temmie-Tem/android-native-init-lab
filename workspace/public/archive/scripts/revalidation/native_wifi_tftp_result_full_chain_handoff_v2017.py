#!/usr/bin/env python3
"""V2017 rollbackable handoff for native TFTP result/full-chain tracing."""
...
OUT_DIR = prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2017-tftp-result-full-chain-handoff"
)
HANDOFF_DIR = OUT_DIR / "v2016-handoff"
HANDOFF_REPORT = OUT_DIR / "v2016-handoff-report.md"
REPORT_PATH = prev2013.prev1998.prev1992.prev.repo_path(
    "docs/reports/NATIVE_INIT_V2017_TFTP_RESULT_FULL_CHAIN_HANDOFF_2026-06-04.md"
)
V2016_OUT = prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2016-tftp-result-full-chain-test-boot"
)
V2016_INIT = V2016_OUT / "init_v2016_tftp_result_full_chain"
V2016_BOOT = V2016_OUT / "boot_linux_v2016_tftp_result_full_chain.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2016/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.190 (v2016-tftp-result-full-chain)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2016.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2016.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2016-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v377"
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2016_INIT, init_required), (V2016_BOOT, boot_required)):
        forbidden_tokens = init_forbidden if path == V2016_INIT else ()
        if not path.exists():
...
def classify(handoff: dict[str, Any],
    ...
    if not route_ok:
        ...
        reason = "V2016 did not preserve rollback, light observer, bridges, full chain, or tftp result trace"
        passed = False
    elif intish(cascade.get("wlan0")) > 0:
        label = "tftp-result-wlan0-progress"
        reason = "V2016 reached wlan0; stop before credentials/scan/connect until a dedicated gated unit"
        passed = True
    elif intish(cascade.get("fw_ready")) > 0:
        label = "tftp-result-fw-ready-progress"
        reason = "V2016 crossed into visible FW-ready progress"
        passed = True
    elif (
        trace.get("requested")
    ...
    return {
        **result,
        "label": label,
        "decision": f"v2017-{label}-rollback-{'pass' if passed else 'blocked'}",
        "pass": passed,
        "reason": reason,
...
def configure_prev2013() -> None:
    ...
    prev2013.V2012_OUT = V2016_OUT
    prev2013.V2012_INIT = V2016_INIT
    prev2013.V2012_BOOT = V2016_BOOT
