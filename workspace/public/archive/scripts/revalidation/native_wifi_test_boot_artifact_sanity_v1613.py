#!/usr/bin/env python3
"""V1613 local-only sanity verifier for the V1612 per_mgr non-stopping context boot."""
...
base.DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1612-per-mgr-nonstop-context-test-boot" / "manifest.json"
base.DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1613-per-mgr-nonstop-context-artifact-sanity"
base.DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1613_PER_MGR_NONSTOP_CONTEXT_ARTIFACT_SANITY_2026-06-02.md"
)
base.EXPECTED_DECISION = "v1612-per-mgr-nonstop-context-test-boot-source-build-pass"
base.EXPECTED_HELPER = "a90_android_execns_probe_v300"
base.EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.108 (v1612-per-mgr-nonstop-context)",
    "a90_android_execns_probe v300",
    "A90v1612",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "watch_sec": 120,
        "helper_mode": "android-service-window-pm-proxy-contract-pm-first-late-per-proxy-pph-gate-per-mgr-nonstop-context-trace-lower-marker",
        "helper_runtime_mode": "wifi-companion-android-wifi-service-window-subsys-trigger-capture",
        "helper_result": "/cache/native-init-wifi-test-boot-v1612-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if pass_ok:
        return (
            "v1613-per-mgr-nonstop-context-artifact-sanity-pass",
            True,
            "V1612 per_mgr non-stopping context test boot artifact passed local sanity",
        )
    return (
        "v1613-per-mgr-nonstop-context-artifact-sanity-blocked",
        False,
        f"V1612 artifact sanity failed; base decision was {decision}",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1612 manifest: `{manifest['_path']}`",
        f"- V1612 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- boot non-stopping markers: `{checks['boot_markers']['ok']}`",
        f"- helper non-stopping markers: `{checks['helper_markers']['ok']}`",
