#!/usr/bin/env python3
"""V916 source/build verifier for corrected mdm_helper /dev/subsys_esoc0 trigger support."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v916-mdm-helper-subsys-trigger-support")
LATEST_POINTER = Path("tmp/wifi/latest-v916-mdm-helper-subsys-trigger-support.txt")
DEFAULT_HELPER_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_BUILD_ARTIFACT = Path("tmp/wifi/v916-execns-helper-v150-build/a90_android_execns_probe")
DEFAULT_BUILD_LOG = Path("tmp/wifi/v916-execns-helper-v150-build/build.log")
...
def classify(source: str, build_log: str, artifact: Path) -> dict[str, Any]:
    ...
    checks = {
        "execns_version_v150": 'EXECNS_VERSION "a90_android_execns_probe v150"' in source,
        ...
        "subsys_child_open": 'open("/dev/subsys_esoc0", O_RDONLY | O_NONBLOCK | O_CLOEXEC)' in trigger_function,
        "child_chroot": "subsys_trigger.child_chroot=1" in trigger_function and "chroot(paths->root)" in trigger_function,
    ...
    return {
        "decision": "v916-mdm-helper-subsys-trigger-support-pass" if passed else "v916-mdm-helper-subsys-trigger-support-incomplete",
        "pass": passed,
        "reason": (
            "helper v150 adds corrected mdm_helper runtime-subsys-trigger capture with no Notify/BOOT_DONE spoofing and upper Wi-Fi surface observation"
            if passed else
            "missing source/build checks: " + ", ".join(name for name, ok in checks.items() if not ok)
        ),
        "next_step": (
            "deploy helper v150, then run bounded V917 corrected native trigger gate"
            if passed else
            "repair helper v150 trigger support before deploy"
        ),
        "checks": checks,
    }
...
def render_summary(manifest: dict[str, Any]) -> str:
    return "\n".join([
        "# V916 mdm_helper Subsys Trigger Support",
        "",
