#!/usr/bin/env python3
"""V971 source/build verifier for Android service-window validation repair."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v971-android-service-window-validation-repair")
LATEST_POINTER = Path("tmp/wifi/latest-v971-android-service-window-validation-repair.txt")
DEFAULT_HELPER_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_BUILD_ARTIFACT = Path("tmp/wifi/v971-execns-helper-v162-build/a90_android_execns_probe")
DEFAULT_BUILD_LOG = Path("tmp/wifi/v971-execns-helper-v162-build/build.log")
...
ALLOWLIST_TOKEN = "is_wifi_companion_android_wifi_service_window_start_only_mode(cfg->mode)"
...
def service_defaults_validation_block(source: str) -> str:
    match = re.search(
...
def classify(source: str, build_rc: int, build_log: str, artifact: Path) -> dict[str, Any]:
    validation_block = service_defaults_validation_block(source)
    strings_output = artifact_strings(artifact)
    checks = {
        "execns_version_v162": 'EXECNS_VERSION "a90_android_execns_probe v162"' in source,
        "mode_still_present": MODE in source,
        "service_defaults_allowlist_repaired": ALLOWLIST_TOKEN in validation_block,
        "service_window_default_still_service_defaults": all(
            token in source
            for token in (
        ...
        "strings_confirm_version": "a90_android_execns_probe v162" in strings_output,
        "strings_confirm_mode": MODE in strings_output,
        "strings_confirm_allow_flag": "--allow-android-wifi-service-window" in strings_output,
    }
    passed = all(checks.values())
    return {
        "decision": "v971-android-service-window-validation-repair-pass"
        if passed
        else "v971-android-service-window-validation-repair-incomplete",
        "pass": passed,
        "reason": (
            "helper v162 allows the Android service-window mode to use its service-defaults SELinux context policy while preserving no-Wi-Fi guardrails"
            if passed
            else "missing checks: " + ", ".join(name for name, ok in checks.items() if not ok)
        ),
        "next_step": (
            "deploy helper v162, then rerun the bounded V970 Android service-window live proof"
            if passed
            else "repair helper v162 validation allowlist before deploy"
        ),
        "checks": checks,
    }
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- no device command",
            "- no actor start by the verifier",
            "- no `qcwlanstate`, eSoC open/ioctl, scan/connect, credentials, DHCP/routes, or external ping",
            "",
        ]
    )
