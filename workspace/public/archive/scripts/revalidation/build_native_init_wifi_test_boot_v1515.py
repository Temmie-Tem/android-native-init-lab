#!/usr/bin/env python3
"""Build the V1515 critical-source pre-L0 Wi-Fi test boot artifact."""
...
OUT_DIR = base.REPO_ROOT / "tmp" / "wifi" / "v1515-wifi-critical-source-pre-l0-test-boot"
REPORT_PATH = (
    base.REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1515_WIFI_CRITICAL_SOURCE_PRE_L0_SOURCE_BUILD_2026-06-01.md"
)
...
DEFAULT_ARGS = [
    ...
    "V1515",
    "--decision",
    "v1515-wifi-critical-source-pre-l0-test-boot-source-build-pass",
    "--cycle-label",
    "v1515",
    "--init-version",
    "0.9.97",
    ...
    str(OUT_DIR),
    "--init-binary",
    str(OUT_DIR / "init_v1515_wifi_test"),
    "--helper-binary",
    str(OUT_DIR / "a90_android_execns_probe_v287"),
    "--ramdisk-cpio",
    str(OUT_DIR / "ramdisk_v1515_wifi_test.cpio"),
    "--boot-image",
    str(OUT_DIR / "boot_linux_v1515_wifi_test.img"),
    "--wifi-test-klog-prefix",
    "A90v1515",
    "--wifi-test-disable",
    "/cache/native-init-wifi-test-boot-v1515.disable",
    "--wifi-test-log",
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Critical-source pre-L0 result path: `{wifi['rc1_window_result']}`",
