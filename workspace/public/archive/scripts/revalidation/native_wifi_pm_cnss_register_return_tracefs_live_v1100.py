#!/usr/bin/env python3
"""V1100 tracefs-only CNSS PM register-return classifier.
...
libperipheral_client.so with ARM64 register fetchargs. It answers one narrow
question: whether `cnss-daemon` receives a successful `pm_client_register` return and then calls the actionable PM connect path before mdm3 stays OFFLINING?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1100-pm-cnss-register-return-tracefs-live")
DEFAULT_V1095_MANIFEST = Path("tmp/wifi/v1095-pm-cnss-voter-surface-live/manifest.json")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1100"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1100/pm-cnss-voter-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1100/pm-cnss-register-return-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1100/pm-cnss-voter-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1100-pm-cnss-register-return-tracefs-live.txt")

EVENT_SPECS = (
    ("pm_client_register_entry", "6ec8", "client=+0(%x2):string peripheral=+0(%x3):string out=%x5"),
    ("pm_client_register_ret", "6ec8", "ret=$retval"),
    ("pm_register_connect_entry", "612c", ""),
    ("pm_client_connect_entry", "7544", ""),
    ("pm_client_connect_ret", "7544", "ret=$retval"),
    ("pm_client_ack_entry", "76f0", ""),
    ("pm_server_ontransact", "85bc", "code=%x1 flags=%x4"),
    ("pm_server_ontransact_thunk", "8858", "code=%x1 flags=%x4"),
)

SERVER_EVENT_LABELS = {"pm_server_ontransact", "pm_server_ontransact_thunk"}
RETURN_EVENT_LABELS = {"pm_client_register_ret", "pm_client_connect_ret"}
TRANSACTION_METHODS = {
    "0x1": "register",
    "0x2": "unregister",
    "0x3": "connect",
...
def write_child_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    script = "\n".join([
        ...
        f"echo v1100.child_full_output={shlex.quote(args.child_output)}",
        "echo v1100.child_rc=$rc",
        "exit $rc",
        "",
    ])
...
def tracefs_collector_script(args: argparse.Namespace) -> str:
    ...
    grep_pattern = "|".join(re.escape(label) for label, _offset, _fetch in EVENT_SPECS)
    register_calls = "\n".join(
        f"register_event {shlex.quote(label)} {shlex.quote(offset)} {shlex.quote(fetch)}"
        for label, offset, fetch in EVENT_SPECS
    )
    enable_calls = "\n".join(
        f"enable_event {shlex.quote(label)}"
        for label, _offset, _fetch in EVENT_SPECS
    )
    count_calls = "\n".join(
...
GROUP=a90pm1100
BIN={shlex.quote(args.peripheral_client)}
...
echo tracefs_uprobe_collector=v1100
...
if ! $BB test -r "$BIN"; then
  echo result=tracefs-uprobe-binary-missing
  exit 1
fi
...
$BB grep -E '^(A90_EXECNS_(END|STDOUT_END)|pm_service_trigger_observer\\.|wifi_vndservice_query\\.|wifi_companion_qrtr_readback\\.|v1100\\.)' "$CHILD_LOG" 2>/dev/null || true
...
def write_collector_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    store.write_text("host/pm-cnss-register-return-tracefs-collector-script.txt", script)
...
def parse_tracefs_output(text: str) -> dict[str, Any]:
    ...
    transaction_flag_counts: dict[str, int] = {}
    transaction_codes_by_comm: dict[str, dict[str, int]] = {}
    return_values_by_comm: dict[str, dict[str, list[str]]] = {}
    register_args_by_comm: dict[str, list[dict[str, str]]] = {}
    for line in trace_lines:
        ...
        if label in SERVER_EVENT_LABELS:
            ...
            if code_match:
                ...
                transaction_code_counts[code] = transaction_code_counts.get(code, 0) + 1
                transaction_codes_by_comm.setdefault(comm, {})
                transaction_codes_by_comm[comm][code] = transaction_codes_by_comm[comm].get(code, 0) + 1
            if flags_match:
                flags = flags_match.group(1)
                transaction_flag_counts[flags] = transaction_flag_counts.get(flags, 0) + 1
        ...
        if label == "pm_client_register_entry":
            ...
            register_args_by_comm.setdefault(comm, [])
            register_args_by_comm[comm].append({
    ...
    return {
        ...
        "transaction_code_counts": transaction_code_counts,
        "transaction_flag_counts": transaction_flag_counts,
        "transaction_codes_by_comm": transaction_codes_by_comm,
        "transaction_methods": TRANSACTION_METHODS,
        "return_values_by_comm": return_values_by_comm,
        "register_args_by_comm": register_args_by_comm,
        ...
        "child_rc": keys.get("child.rc", keys.get("v1100.child_rc", "")),
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    collector_step = base.run_a90ctl(
        ...
        store,
        steps,
        "pm-server-pm-cnss-register-return-tracefs-observer",
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1100-cnss-register-return-tracefs-plan-ready",
            True,
            "plan-only; no tracefs write, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1100 with explicit allow flags",
        )
    v1095_decision = (manifest.get("v1095") or {}).get("decision")
    if v1095_decision not in ALLOWED_V1095_DECISIONS:
        return (
            "v1100-v1095-predecessor-missing",
            False,
            f"unexpected V1095 decision={v1095_decision!r}",
            "rerun or inspect V1095 before PM pm-cnss-register-return tracefs classifier",
        )
    missing = required_flags(args)
    if missing:
        return (
            "v1100-cnss-register-return-tracefs-approval-required",
            False,
            "missing explicit flags: " + ", ".join(missing),
            "rerun with all V1100 allow flags",
        )
    ...
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1100-execns-helper-sha-mismatch", False, "remote execns helper is not v206", "redeploy helper v206")
    usage = analysis.get("execns_usage") or {}
    if not (usage.get("marker_ok") and usage.get("mode_ok") and usage.get("start_cnss_flag_ok")):
        return ("v1100-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v206")
    if not analysis.get("peripheral_client_visible"):
        return ("v1100-peripheral-client-not-visible", False, "read-only vendor mount did not expose libperipheral_client.so", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1100-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1100-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1100-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1100-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if contract.get("cnss_daemon_start_executed") != "1":
        return ("v1100-cnss-daemon-not-started", False, "child did not execute V1095 CNSS phase", "repair child command before retry")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1100-pm-server-uprobe-no-hit", False, "no libperipheral_client trace lines were captured", "verify shared-library inode mapping or offsets")
    server_hits = int(tracefs.get("pm_server_ontransact_hit_count") or 0)
    ...
    code_counts = tracefs.get("transaction_code_counts") or {}
    ...
    if not code_counts:
        return ("v1100-transaction-code-fetch-missing", False, "server onTransact code fetcharg missing", "return to V1099 fetcharg gate")
    if cnss_register_entries <= 0:
        return ("v1100-cnss-register-entry-missing", False, "cnss-daemon did not enter pm_client_register", "return to V1096 CNSS PM client path gate")
    if not cnss_register_returns:
        return (
            ...
            (
                ...
                f"codes={code_counts} mdm3_state={mdm3_state}"
            ),
    ...
    if nonzero_cnss_register_returns and cnss_connect_entries <= 0:
        return (
            "v1100-cnss-pm-register-fails-before-connect-mdm3-still-offline",
            True,
            (
    ...
    if zero_cnss_register_returns and cnss_connect_entries <= 0:
        return (
            "v1100-cnss-pm-register-succeeds-but-connect-not-called",
            True,
            (
                f"cnss_register_ret={cnss_register_returns} cnss_connect_entries={cnss_connect_entries} "
                f"codes={code_counts} mdm3_state={mdm3_state}"
    ...
    if cnss_connect_entries > 0:
        if any(value not in ("0", "0x0") for value in cnss_connect_returns):
            return (
                "v1100-cnss-pm-connect-fails-mdm3-still-offline",
                True,
                (
        ...
        return (
            "v1100-cnss-pm-connect-succeeds-mdm3-still-offline",
            True,
            (
                f"cnss_register_ret={cnss_register_returns} cnss_connect_ret={cnss_connect_returns} "
                f"codes={code_counts} per_mgr_server_hits={per_mgr_server_hits} "
    ...
    if server_hits > 0:
        return (
            "v1100-pm-server-ontransact-hit-unmapped-binder-thread",
            True,
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    v1095_manifest = load_json(args.v1095_manifest)
    manifest: dict[str, Any] = {
        "cycle": "v1100",
        "generated_at": now_iso(),
        "command": args.command,
        "v1095": {
            "manifest": str(repo_path(args.v1095_manifest)),
            "decision": v1095_manifest.get("decision", ""),
            "pass": bool(v1095_manifest.get("pass")),
        },
        "event_specs": [
            f"{label}:0x{offset}" + (f":{fetch}" if fetch else "")
            for label, offset, fetch in EVENT_SPECS
        ],
        "steps": [],
        "analysis": {},
    ...
    if (
        ...
        and v1095_manifest.get("decision") in ALLOWED_V1095_DECISIONS
    ):
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1100 PM CNSS Register Return Tracefs Live",
        "",
        ...
        f"- pm_server_ontransact_hit_count: `{tracefs.get('pm_server_ontransact_hit_count', '')}`",
        f"- per_mgr_binder_server_hit_count: `{tracefs.get('per_mgr_binder_server_hit_count', '')}`",
        f"- per_mgr_pid: `{tracefs.get('per_mgr_pid', '')}`",
        f"- transaction_code_counts: `{tracefs.get('transaction_code_counts', {})}`",
        f"- return_values_by_comm: `{tracefs.get('return_values_by_comm', {})}`",
        f"- register_args_by_comm: `{tracefs.get('register_args_by_comm', {})}`",
        f"- transaction_flag_counts: `{tracefs.get('transaction_flag_counts', {})}`",
        ...
        json.dumps({
            "counts": counts,
            "by_comm": by_comm,
            "transaction_code_counts": tracefs.get("transaction_code_counts") or {},
            "transaction_codes_by_comm": tracefs.get("transaction_codes_by_comm") or {},
            "transaction_methods": tracefs.get("transaction_methods") or {},
            "return_values_by_comm": tracefs.get("return_values_by_comm") or {},
            "register_args_by_comm": tracefs.get("register_args_by_comm") or {},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1095-manifest", type=Path, default=DEFAULT_V1095_MANIFEST)
