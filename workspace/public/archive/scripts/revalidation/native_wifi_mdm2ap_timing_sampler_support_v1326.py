#!/usr/bin/env python3
"""V1326 source/build-only gate for MDM2AP/errfatal/PCIe timing sampler support."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1326-mdm2ap-timing-sampler-support")
DEFAULT_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v276")
DEFAULT_BUILD_SCRIPT = Path("scripts/revalidation/build_android_execns_probe_helper.sh")
LATEST_POINTER = Path("tmp/wifi/latest-v1326-mdm2ap-timing-sampler-support.txt")
HELPER_MARKER = "a90_android_execns_probe v276"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v276"',
    "pm_observer_late_per_proxy_mdm2ap_errfatal_pcie_timing_sampler",
    "--pm-observer-late-per-proxy-mdm2ap-errfatal-pcie-timing-sampler",
    "collect_mdm_errfatal_irq_snapshot",
    "mdm2ap_timing_summary",
    "late-per-proxy-mdm2ap-errfatal-pcie-timing",
    "mdm2ap_timing.gpio142_irq_delta=%lu",
    "mdm2ap_timing.errfatal_irq_delta=%lu",
    "mdm2ap_timing.pcie_rc1_transition_seen=%d",
    "mdm2ap_timing.mhi_bus_max=%d",
    "mdm2ap_timing.wlfw_kmsg_max=%d",
    ...
    rows = [
        {
            ...
            "next_step": "repair helper source MDM2AP timing sampler support",
        },
        {
...
def decide(command: str, rows: list[dict[str, str]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if command == "plan":
        return (
            "v1326-mdm2ap-timing-sampler-build-plan-ready",
            True,
            "plan-only",
            "run V1326 source/build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return (
            "v1326-mdm2ap-timing-sampler-build-blocked",
            False,
    ...
    return (
        "v1326-mdm2ap-timing-sampler-build-pass",
        True,
        f"helper v276 built sha256={stage3['sha256']}",
        "V1327 should deploy helper v276 only; V1328 should run the bounded MDM2AP timing sampler live",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1326 MDM2AP Timing Sampler Build",
        "",
        ...
        "- new opt-in flag: `--pm-observer-late-per-proxy-mdm2ap-errfatal-pcie-timing-sampler`",
        "- response mode: `late-per-proxy-mdm2ap-errfatal-pcie-timing`",
        "- intended cadence: `120` samples at `50ms` after late `per_proxy` start",
        "- output contract: compact `mdm2ap_timing.*` aggregate keys for GPIO142, MDM errfatal, PCIe RC1, MHI/ks, WLFW, and `wlan0` timing",
        "- safety markers remain explicit: no PMIC write, GPIO request, direct eSoC ioctl, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, or external ping",
        "",
        "## Safety",
        "",
