#!/usr/bin/env python3
"""V1446 local-only sanity verifier for the V1445 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1446-wifi-test-boot-case-aligned-micro-endpoint-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1446_WIFI_TEST_BOOT_CASE_ALIGNED_MICRO_ENDPOINT_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1445-wifi-test-boot-case-aligned-micro-endpoint-source-build-pass"
...
EXPECTED_BOOT_MARKERS = (
    ...
    "/cache/native-init-wifi-test-boot-v1445.pid",
    "/cache/native-init-wifi-test-boot-v1445-supervisor.pid",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1445-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1445-rc1-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1446-wifi-test-boot-case-aligned-micro-endpoint-artifact-sanity-pass",
            True,
            "V1445 case-aligned micro endpoint test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1446-wifi-test-boot-case-aligned-micro-endpoint-artifact-sanity-blocked",
        False,
        "V1445 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1445 manifest: `{manifest['_path']}`",
            f"- V1445 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- retry/immediate/v1441 markers absent: `{checks['boot_absent_markers']['absent_ok']}`",
            f"- case-aligned micro endpoint sampler contract: `{checks['wifi_test_contract']['ok']}`",
...
def main() -> int:
    ...
    result = {
        ...
        "next_gate": "V1447 rollbackable live handoff for only the V1445 image",
    }
