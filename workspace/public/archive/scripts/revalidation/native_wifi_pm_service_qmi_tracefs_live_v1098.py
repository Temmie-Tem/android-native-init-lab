#!/usr/bin/env python3
"""V1098 tracefs-only PM service QMI classifier.

This gate replays the V1095 bounded PM provider + pm-proxy + cnss-daemon
observer, while arming dynamic tracefs uprobes on vendor pm-service PLT
entries. It answers one narrow question: after Binder delivery is proven, does
pm-service enter QMI/register/send/handle paths before mdm3 stays OFFLINING?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1098-pm-service-qmi-tracefs-live")
...
DEFAULT_TRACE_BINARY = "/mnt/vendor/bin/pm-service"
DEFAULT_TRACEFS_ROOT = "/sys/kernel/tracing"
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1098"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1098/pm-cnss-voter-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1098/pm-service-qmi-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1098/pm-cnss-voter-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1098-pm-service-qmi-tracefs-live.txt")

EVENT_SPECS = (
    ("pm_log_print", "9e60"),
    ("pm_property_set", "9ec0"),
    ("pm_get_system_info", "9f40"),
    ("pm_qmi_send_resp", "9f60"),
    ("pm_qmi_send_ind", "9f70"),
    ("pm_qmi_register", "9fb0"),
    ("pm_qmi_select", "9fd0"),
    ("pm_qmi_handle_event", "9ff0"),
    ("pm_qmi_unregister", "a000"),
)
...
QMI_SEND_LABELS = {"pm_qmi_send_resp", "pm_qmi_send_ind"}
...
def write_child_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    script = "\n".join([
        ...
        f"echo v1098.child_full_output={shlex.quote(args.child_output)}",
        "echo v1098.child_rc=$rc",
        "exit $rc",
        "",
    ])
...
GROUP=a90pm1097
BIN={shlex.quote(args.trace_binary)}
...
echo tracefs_uprobe_collector=v1098
...
$BB grep -E '^(A90_EXECNS_(END|STDOUT_END)|pm_service_trigger_observer\\.|wifi_vndservice_query\\.|wifi_companion_qrtr_readback\\.|v1098\\.)' "$CHILD_LOG" 2>/dev/null || true
...
def write_collector_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    store.write_text("host/pm-service-qmi-tracefs-collector-script.txt", script)
...
def parse_tracefs_output(text: str) -> dict[str, Any]:
    ...
    qmi_hits_by_comm = {
        comm: {
            ...
            if label in QMI_EVENT_LABELS
        }
        for comm, labels in by_comm.items()
        if any(label in QMI_EVENT_LABELS for label in labels)
    }
    qmi_send_hits_by_comm = {
        comm: {
            label: count
            for label, count in labels.items()
            if label in QMI_SEND_LABELS
        }
        for comm, labels in by_comm.items()
        if any(label in QMI_SEND_LABELS for label in labels)
    }
    per_mgr_qmi_hits = sum(
        count
        for comm, labels in qmi_hits_by_comm.items()
        if comm == "pm-service"
    ...
    return {
        ...
        "pm_service_qmi_hit_count": sum(
            count
            for labels in qmi_hits_by_comm.values()
            for count in labels.values()
        ),
        "pm_service_qmi_send_hit_count": sum(
            count
            for labels in qmi_send_hits_by_comm.values()
            for count in labels.values()
        ),
        "pm_service_qmi_hits_by_comm": qmi_hits_by_comm,
        "pm_service_qmi_send_hits_by_comm": qmi_send_hits_by_comm,
        "per_mgr_pid": per_mgr_pid,
        "per_mgr_qmi_hit_count": per_mgr_qmi_hits,
        ...
        "child_rc": keys.get("child.rc", keys.get("v1098.child_rc", "")),
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    lib_step = base.run_a90ctl(args, store, steps, "trace-binary-stat", ["run", args.busybox, "stat", args.trace_binary], timeout=15.0, allow_error=True)
    analysis["trace_binary_visible"] = bool(lib_step.get("ok"))
    ...
    collector_step = base.run_a90ctl(
        ...
        store,
        steps,
        "pm-service-qmi-tracefs-observer",
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    ...
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1098-execns-helper-sha-mismatch", False, "remote execns helper is not v206", "redeploy helper v206")
    usage = analysis.get("execns_usage") or {}
    if not (usage.get("marker_ok") and usage.get("mode_ok") and usage.get("start_cnss_flag_ok")):
        return ("v1098-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v206")
    if not analysis.get("trace_binary_visible"):
        return ("v1098-pm-service-binary-not-visible", False, "read-only vendor mount did not expose pm-service", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1098-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1098-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1098-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1098-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if contract.get("cnss_daemon_start_executed") != "1":
        return ("v1098-cnss-daemon-not-started", False, "child did not execute V1095 CNSS phase", "repair child command before retry")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1098-pm-service-qmi-uprobe-no-hit", False, "no pm-service trace lines were captured", "verify binary inode mapping or PLT offsets")
    qmi_hits = int(tracefs.get("pm_service_qmi_hit_count") or 0)
    qmi_send_hits = int(tracefs.get("pm_service_qmi_send_hit_count") or 0)
    per_mgr_qmi_hits = int(tracefs.get("per_mgr_qmi_hit_count") or 0)
    mdm3_state = contract.get("post_provider_surface.after_cnss_daemon.mdm3_state") or ""
    if qmi_send_hits > 0:
        return (
            "v1098-pm-service-qmi-send-hit-mdm3-still-offline",
            True,
            (
                f"qmi_send_hits={qmi_send_hits} qmi_hits={qmi_hits} "
                f"mdm3_state={mdm3_state} hits={tracefs.get('pm_service_qmi_send_hits_by_comm')}"
            ),
    ...
    if qmi_hits > 0:
        return (
            ...
            True,
            f"qmi_hits={qmi_hits} per_mgr_qmi_hits={per_mgr_qmi_hits} hits={tracefs.get('pm_service_qmi_hits_by_comm')}",
            "classify why PM Binder requests do not produce QMI send/indication",
        )
    if tracefs.get("counts", {}).get("pm_log_print", 0) > 0:
        return (
            "v1098-pm-service-log-only-no-qmi",
            True,
            f"log_print={tracefs.get('counts', {}).get('pm_log_print')} by_comm={tracefs.get('by_comm')}",
            "classify PM service onTransact cases or log message identity",
        )
    return (
        "v1098-pm-service-qmi-not-called",
        True,
        f"total_hits={tracefs.get('hit_count')} by_comm={tracefs.get('by_comm')}",
        "inspect PM service PLT offsets and child sequence before retry",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1098 PM Service QMI Tracefs Live",
        "",
        ...
        f"- pm_service_qmi_hit_count: `{tracefs.get('pm_service_qmi_hit_count', '')}`",
        f"- pm_service_qmi_send_hit_count: `{tracefs.get('pm_service_qmi_send_hit_count', '')}`",
        f"- per_mgr_qmi_hit_count: `{tracefs.get('per_mgr_qmi_hit_count', '')}`",
        f"- per_mgr_pid: `{tracefs.get('per_mgr_pid', '')}`",
        f"- trace_line_count: `{tracefs.get('trace_line_count', '')}`",
        f"- child_rc: `{tracefs.get('child_rc', '')}`",
        "",
        "```json",
        json.dumps({"counts": counts, "by_comm": by_comm}, indent=2, sort_keys=True),
        "```",
        "",
        "## Steps",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--trace-binary", default=DEFAULT_TRACE_BINARY)
