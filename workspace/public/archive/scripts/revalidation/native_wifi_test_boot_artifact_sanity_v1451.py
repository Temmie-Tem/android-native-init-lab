#!/usr/bin/env python3
"""V1451 local-only sanity verifier for the V1450 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1451-wifi-test-boot-provider-trigger-micro-endpoint-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1451_WIFI_TEST_BOOT_PROVIDER_TRIGGER_MICRO_ENDPOINT_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1450-wifi-test-boot-provider-trigger-micro-endpoint-source-build-pass"
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1450-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1450-rc1-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            ...
            True,
            "V1450 provider-trigger micro endpoint test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1451-wifi-test-boot-provider-trigger-micro-endpoint-artifact-sanity-blocked",
        False,
        "V1450 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1450 manifest: `{manifest['_path']}`",
            f"- V1450 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- retry/immediate/case-writer markers absent: `{checks['boot_absent_markers']['absent_ok']}`",
            f"- provider-trigger micro endpoint sampler contract: `{checks['wifi_test_contract']['ok']}`",
...
def main() -> int:
    ...
    result = {
        ...
        "next_gate": "V1452 rollbackable live handoff for only the V1450 image",
    }
