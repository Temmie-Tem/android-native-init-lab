#!/usr/bin/env python3
"""V687 current-boot orchestrator for PeripheralManager/CNSS retry proof."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v687-peripheral-manager-cnss-retry-orchestrated")
V687_SCRIPT = "scripts/revalidation/native_wifi_peripheral_manager_cnss_retry_v687.py"
V687_APPROVAL = (
...
HELPER_SHA256 = "60ed7a14d3b33b2f700fb644fd1ccd7a037ac8d9c50db082fa0dea7646965ce9"
HELPER_MARKER = "a90_android_execns_probe v113"

ALLOWED_LIVE_ACTIONS = (
    "V641 one-shot clean-DSP reboot",
    "V401 SELinuxfs mount surface",
    "V490 Android SELinux policy-load proof",
    "bounded helper v113 PeripheralManager provider/CNSS retry proof",
...
def peripheral_for(arm: dict[str, Any] | None) -> dict[str, Any]:
    live = (arm or {}).get("live") or {}
    surface = live.get("v687_peripheral_manager_surface")
...
def decide(command: str, prep: dict[str, Any] | None, arm: dict[str, Any] | None) -> tuple[str, bool, str, str]:
    if command == "plan":
        return (
            "v687-peripheral-manager-orchestrator-plan-ready",
            True,
            "plan-only; no device command executed",
            "deploy helper v113, then run V687 orchestrated live proof",
        )
    if not prep or not prep.get("ready"):
        return "v687-current-boot-prep-blocked", False, f"prep={prep}", "restore current-boot prerequisites before V687"
    if not arm or not arm.get("decision"):
        return "v687-arm-missing", False, "V687 arm did not produce a manifest", "inspect arm evidence"
    if "blocked" in str(arm.get("decision")):
        return "v687-arm-blocked", False, f"arm={summarize_arm(arm)}", "resolve V687 arm blocker before retry"
    counts = counts_for(arm)
    markers = markers_for(arm)
    surface = peripheral_for(arm)
    ...
    if provider_ready(surface):
        return (
            "v687-provider-ready-no-wlfw-advance",
            True,
    ...
    return (
        "v687-provider-start-gap-classified",
        True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V687 PeripheralManager CNSS Retry Orchestrator",
        "",
...
def main() -> int:
    ...
    if args.command == "run":
        root = store.run_dir
        arm_root = root / "arm-v687-v113-peripheral-manager"
        prep = v673.prep_current_boot(args, store, "v687", arm_root)
        if prep.get("ready"):
            arm = v673.run_arm(
                args,
                store,
