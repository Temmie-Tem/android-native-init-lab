#!/usr/bin/env python3
"""V487 native SELinux init-domain handoff matrix.

This runner uses `a90_android_execns_probe v45` in `selinux-domain-proof` mode.
Unlike V478's dynamic toybox post-exec check, v45 re-execs the static helper via
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v487-native-selinux-init-handoff")
DEFAULT_HELPER = "/cache/bin/a90_android_execns_probe"
DEFAULT_TOYBOX = "/cache/bin/toybox"
DEFAULT_EXPECT_VERSION = "A90 Linux init 0.9.61 (v319)"
DEFAULT_HELPER_SHA256 = "387b4f10a3f6e11805b1680cdf18f25f3be8b939826739e06edc7ea6b3c07770"
APPROVAL_PHRASE = (
    "approve v487 native SELinux init handoff proof only; "
...
def build_checks(args: argparse.Namespace, steps: dict[str, Step],
    ...
    helper_ready = (
        ...
        and "a90_android_execns_probe v45" in helper_usage
        and "selinux-domain-proof" in helper_usage
        and "--selinux-print-current" in helper_usage
    )
    add_check(checks, "helper-v45", "pass" if helper_ready else "blocked", "blocker",
              f"sha_match={args.helper_sha256 in helper_sha} marker={'a90_android_execns_probe v45' in helper_usage} static_probe={'--selinux-print-current' in helper_usage}",
              [line for line in helper_sha.splitlines() if args.helper in line][:2],
              "deploy helper v45 before V487 run")
...
def decide(args: argparse.Namespace, checks: list[Check], cases: list[dict[str, Any]] | None) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return "v487-selinux-init-handoff-plan-ready", True, "plan-only; no device command executed", "deploy v45 and run preflight"
    ...
    if args.command == "preflight":
        if pre_run_blockers:
            return "v487-selinux-init-handoff-blocked", False, "blocked before run by " + ", ".join(pre_run_blockers), "resolve blockers before V487 run"
        return "v487-selinux-init-handoff-preflight-ready", True, "preflight passed; run still needs exact approval", "run approved init handoff matrix"
    if not approved(args):
        return "v487-selinux-init-handoff-approval-required", False, "missing exact approval phrase or apply flags", "rerun with exact V487 approval"
    if pre_run_blockers:
        return "v487-selinux-init-handoff-blocked", False, "blocked before run by " + ", ".join(pre_run_blockers), "resolve blockers before retry"
    if cases and any(case["postexec_match"] for case in cases):
        return "v487-selinux-init-handoff-handoff-present", True, "at least one requested domain survived static re-exec", "try HAL registration with the passing domain path only"
    return "v487-selinux-init-handoff-kernel-stuck", True, "static re-exec/current proof still reports kernel for init and Android domains", "treat missing kernel-to-init/domain handoff as the primary Wi-Fi HAL registration blocker"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V487 Native SELinux Init Handoff Matrix",
            "",
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    return {
        ...
        "plan": {
            "helper": args.helper,
            "helper_version": "a90_android_execns_probe v45",
