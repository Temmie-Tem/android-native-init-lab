#!/usr/bin/env python3
"""V1118 global-holder live gate with zero-delay CNSS after per_mgr start."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1118-global-holder-zero-delay-cnss-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1118-global-holder-zero-delay-cnss-live.txt")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1118"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1118/pm-zero-delay-cnss-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1118/pm-zero-delay-cnss-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1118/pm-zero-delay-cnss-output.txt"
PROOF_PREFIX = "/tmp/a90-v1118-"
...
def decide_v1118(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1118-global-holder-zero-delay-cnss-plan-ready",
            True,
            "plan-only; no deploy, tracefs write, PM actor, CNSS actor, or Wi-Fi action executed",
            "deploy helper v211, then run V1118 with explicit allow flags",
        )
    ...
    if blockers:
        return ("v1118-global-preflight-blocked", False, f"blockers={blockers}", "clear global firmware preflight blockers")
    if not global_fw.get("observer_executed"):
        return ("v1118-observer-not-executed", False, f"global={global_fw}", "run only after global holder and QRTR RX are true")
    if not all((global_fw.get("mounted_hits") or {}).values()):
        return ("v1118-global-firmware-mount-missing", False, f"mounted_hits={global_fw.get('mounted_hits')}", "repair global firmware mounts")
    if not global_fw.get("holder_opened"):
        return ("v1118-global-modem-holder-missing", False, "global /dev/subsys_modem holder did not open", "repair holder precondition")
    cleanup = global_fw.get("reboot_cleanup") or {}
    if not (cleanup.get("version_seen") and cleanup.get("status_healthy")):
        return ("v1118-reboot-cleanup-unhealthy", False, f"cleanup={cleanup}", "verify native health before continuing")
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1118-execns-helper-sha-mismatch", False, "remote execns helper is not v211", "deploy helper v211")
    if not (
        usage.get("marker_ok")
        ...
        return ("v1118-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v211")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1118-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("forbidden_true"):
        return ("v1118-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if contract.get("start_cnss_zero_delay_after_per_mgr") != "1":
        return (
            "v1118-zero-delay-cnss-contract-missing",
            False,
            f"contract_value={contract.get('start_cnss_zero_delay_after_per_mgr')}",
            "repair child command flags before retry",
        )
    if contract.get("child.per_mgr.post_start_probe_wait_ms") != "0":
        return (
            "v1118-per-mgr-zero-delay-marker-missing",
            False,
            f"wait_ms={contract.get('child.per_mgr.post_start_probe_wait_ms')}",
            "repair helper v211 zero-delay branch",
        )
    if contract.get("child.per_mgr.post_start_probe_deferred_until_after_cnss") != "1":
        return (
            "v1118-per-mgr-deferred-probe-marker-missing",
            False,
            f"deferred={contract.get('child.per_mgr.post_start_probe_deferred_until_after_cnss')}",
            "repair helper v211 zero-delay branch",
        )
    if contract.get("per_proxy_start_executed") != "0" or contract.get("child.per_proxy.start_skipped") != "1":
        return (
            "v1118-pre-cnss-per-proxy-not-skipped",
            False,
    ...
    if services.get("69") or marker_counts.get("wlfw") or marker_counts.get("wlan0"):
        return (
            "v1118-zero-delay-cnss-wlfw-advance",
            True,
    ...
    if "0x0" in connect_ret:
        return (
            "v1118-zero-delay-cnss-pm-connect-returned-ok",
            True,
    ...
    if connect_entries > 0 or connect_ret:
        return (
            "v1118-zero-delay-cnss-pm-connect-path-reached",
            True,
    ...
    if register_entries > 0 or register_ret:
        return (
            "v1118-zero-delay-cnss-pm-register-path-reached",
            True,
            f"register_entries={register_entries} register_ret={register_ret} cnss_hits={cnss_hits}",
            "trace transition from PM register to PM connect under zero-delay order",
        )
    if cnss_hits > 0:
        return (
            "v1118-cnss-hit-no-pm-client-entry",
            True,
            f"cnss_hits={cnss_hits}",
            "inspect cnss-daemon pre-PM-client startup path under zero-delay order",
        )
    return (
        "v1118-zero-delay-cnss-no-pm-client-entry",
        True,
        (
            f"cnss_hits={cnss_hits} "
        ...
        "inspect child output and PM service/provider lifetime under zero-delay order",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1118 Global Holder Zero-delay CNSS Live",
        "",
        ...
        json.dumps({
            ...
            "per_mgr_post_start_probe_deferred_until_after_cnss": contract.get("child.per_mgr.post_start_probe_deferred_until_after_cnss", ""),
...
def main() -> int:
    ...
    manifest["cycle"] = "v1118"
    manifest["generated_at"] = now_iso()
    decision, passed, reason, next_step = decide_v1118(args, manifest)
