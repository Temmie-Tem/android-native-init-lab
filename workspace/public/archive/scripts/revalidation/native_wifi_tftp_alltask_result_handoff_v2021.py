#!/usr/bin/env python3
"""V2021 rollbackable handoff for native TFTP all-task result tracing."""
...
OUT_DIR = prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2021-tftp-alltask-result-handoff"
)
HANDOFF_DIR = OUT_DIR / "v2020-handoff"
HANDOFF_REPORT = OUT_DIR / "v2020-handoff-report.md"
REPORT_PATH = prev2013.prev1998.prev1992.prev.repo_path(
    "docs/reports/NATIVE_INIT_V2021_TFTP_ALLTASK_RESULT_HANDOFF_2026-06-04.md"
)
V2020_OUT = prev2013.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2020-tftp-alltask-result-test-boot"
)
V2020_INIT = V2020_OUT / "init_v2020_tftp_alltask_result"
V2020_BOOT = V2020_OUT / "boot_linux_v2020_tftp_alltask_result.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2020/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.192 (v2020-tftp-alltask-result)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2020.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2020.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2020-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v379"
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2020_INIT, init_required), (V2020_BOOT, boot_required)):
        forbidden_tokens = init_forbidden if path == V2020_INIT else ()
        if not path.exists():
...
def parse_records(fields: dict[str, str], kind: str) -> dict[tuple[str, int], dict[str, str]]:
    pattern = re.compile(rf"^wlan_pd_tftp_server_trace\.{kind}\.([^.]+)\.record_(\d+)\.(.+)$")
    ...
    for key, value in fields.items():
        ...
        if match:
            ...
            record = records.setdefault((task, record_index), {"task": task, "record_index": str(record_index)})
            record[match.group(3)] = value
    return records
...
def collect_tftp_syscall_trace() -> dict[str, Any]:
    ...
    for index in sorted(packets):
        ...
        task = record.get("task", "tftp_server")
        record_index = intish(record.get("record_index"))
        ...
        if len(first_source_records) < 12:
            first_source_records.append(
                f"{task}.packet_{record_index:03d} dir={direction or 'none'} op={op or 'none'} "
        ...
        if path and len(first_data_records) < 12:
            first_data_records.append(
                f"{task}.packet_{record_index:03d} {direction or 'none'} {op or 'none'} node={node or 'none'} "
        ...
        if len(first_packet_records) < 16:
            ...
            first_packet_records.append(f"{task}.packet_{record_index:03d} {direction or 'none'} {op or 'none'} {detail}")
        if op == "ERROR" and len(first_error_records) < 12:
            first_error_records.append(
                f"{task}.packet_{record_index:03d} code={record.get('error_code', '')} msg={message or 'none'}"
            )
    ...
    for index in sorted(fs_records):
        ...
        if len(first_fs_records) < 16:
            first_fs_records.append(
                f"{task}.fs_{record_index:03d} {record.get('name', '')} ret={record.get('ret', '')} "
    ...
    base.update({
        ...
        "task_counts": {record.get("task", "tftp_server"): sum(1 for item in packet_values if item.get("task", "tftp_server") == record.get("task", "tftp_server")) for record in packet_values},
        "fs_task_counts": {record.get("task", "tftp_server"): sum(1 for item in fs_values if item.get("task", "tftp_server") == record.get("task", "tftp_server")) for record in fs_values},
...
def classify(handoff: dict[str, Any],
    ...
    if not route_ok:
        ...
        reason = "V2020 did not preserve rollback, light observer, bridges, full chain, or tftp all-task result trace"
        passed = False
    elif intish(cascade.get("wlan0")) > 0:
        label = "tftp-result-wlan0-progress"
        reason = "V2020 reached wlan0; stop before credentials/scan/connect until a dedicated gated unit"
        passed = True
    elif intish(cascade.get("fw_ready")) > 0:
        label = "tftp-result-fw-ready-progress"
        reason = "V2020 crossed into visible FW-ready progress"
        passed = True
    elif (
        trace.get("requested")
        ...
        reason = "native modem repeatedly requests mcfg.tmp, but no compact ACK/DATA/ERROR or focused filesystem result was captured even with all-task sendmsg decoding"
        passed = True
    ...
    return {
        **result,
        "label": label,
        "decision": f"v2021-{label}-rollback-{'pass' if passed else 'blocked'}",
        "pass": passed,
        "reason": reason,
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    if tftp_trace.get("mcfg_transfer_seen"):
        ...
        interpretation_lines = [
            ...
            "- No compact `sendmsg`/`sendto` ACK/DATA/ERROR and no focused `openat`/stat path result appeared in any traced `tftp_server` task.",
            "- This localizes the next blocker to the TFTP result side: the traced `tftp_server` tasks receive the modem request but do not emit a visible TFTP response or focused filesystem result in this window.",
            "- Next bounded unit: pivot from AP-side producer work to why stock `tftp_server` does not answer `/readwrite/mcfg.tmp` after QRTR receive, using source or a narrower server-side probe.",
        ]
    else:
        interpretation_lines = [
            "- The full-chain route stayed safe and rollbackable, but no named modem TFTP request was observed in this capture.",
            "- Next bounded unit: re-check `tftp_server` registration/reachability and preserve the all-task trace parser before changing the route.",
        ]
    matrix_rows = [
...
def configure_prev2013() -> None:
    ...
    prev2013.V2012_OUT = V2020_OUT
    prev2013.V2012_INIT = V2020_INIT
    prev2013.V2012_BOOT = V2020_BOOT
