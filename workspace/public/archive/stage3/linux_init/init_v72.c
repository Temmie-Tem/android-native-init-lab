...
#define INIT_VERSION "0.8.3"
#define INIT_BUILD "v72"
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v72: boot splash applied\n");
        ...
        klogf("<6>A90v72: boot splash skipped (%d)\n", saved_errno);
...
static int draw_screen_display_test(void) {
    ...
    const char *wrap_sample = "LONG LOG LINE WRAPS INTO SAFE WIDTH WITHOUT CLIPPING";
    char line[96];
    char chunk[96];
    const char *cursor;
    uint32_t scale;
    ...
    uint32_t cutout_w;
    uint32_t cutout_x;
    uint32_t cutout_center_x;
    uint32_t cutout_center_y;
    uint32_t pocket_w;
    uint32_t line_height;
    uint32_t gap;
    ...
    cutout_y = scale * 4;
    cutout_h = top > cutout_y + gap ? top - cutout_y - gap : line_height;
    cutout_w = kms_state.width / 5;
    ...
    cutout_center_x = kms_state.width / 2;
    cutout_center_y = cutout_y + cutout_h / 2;
    pocket_w = cutout_x > left + gap ? cutout_x - left - gap : 0;

    if (cutout_h > line_height && pocket_w > scale * 16) {
        uint32_t right_x = cutout_x + cutout_w + gap;
        uint32_t right_w = left + content_width > right_x ?
                           left + content_width - right_x : 0;
        uint32_t hole = scale * 9;
        ...
        kms_draw_text_fit(&kms_state, left + scale * 2, cutout_y + scale * 2,
                          "TOP LEFT SLOT", 0x66ddff, small_scale,
                          pocket_w - scale * 4);
        ...
        kms_draw_text_fit(&kms_state, cutout_x + scale * 2, cutout_y + scale * 2,
                          "PUNCH HOLE", 0xffcc33, small_scale,
                          cutout_w - scale * 4);
        if (hole >= scale * 4) {
            kms_fill_rect(&kms_state,
        ...
        if (right_w > scale * 16) {
            ...
            kms_draw_text_fit(&kms_state, right_x + scale * 2, cutout_y + scale * 2,
                              "TOP RIGHT SLOT", 0x66ddff, small_scale,
                              right_w - scale * 4);
        }
    }
    ...
    kms_draw_text_fit(&kms_state, left, top, "TOOLS / DISPLAY TEST",
                      0xffcc33, title_scale, content_width);
    top += line_height + gap;

    kms_draw_text_fit(&kms_state, left, top, "COLOR PALETTE",
                      0x88aaff, scale, content_width);
    top += line_height;

    swatch_width = (content_width - gap) / 2;
    swatch_height = line_height + gap;
    for (index = 0; index < SCREEN_MENU_COUNT(palette); ++index) {
        ...
        uint32_t swatch_y = top + row * (swatch_height + gap);
    ...
    top += ((uint32_t)(SCREEN_MENU_COUNT(palette) + 1) / 2) * (swatch_height + gap) + gap;

    kms_draw_text_fit(&kms_state, left, top, "FONT SCALE",
                      0x88aaff, scale, content_width);
    top += line_height;
    for (index = 1; index <= 7; ++index) {
        ...
        if (top + row_height >= footer_y - line_height) {
            break;
        }
        snprintf(line, sizeof(line), "SCALE %u ABC123 0.8.3", row_scale);
        kms_fill_rect(&kms_state, left, top,
                      content_width, row_height, index % 2 ? 0x101620 : 0x182030);
    ...
    wrap_offset = 0;
    for (index = 0; cursor[wrap_offset] != '\0' && index < 3; ++index) {
        size_t next_offset;
    ...
    if (grid_h >= line_height * 3) {
        uint32_t center_x = left + content_width / 2;
        uint32_t center_y = grid_y + grid_h / 2;
        ...
        kms_draw_rect_outline(&kms_state, left, grid_y, content_width, grid_h,
                              scale, 0x4060a0);
        kms_fill_rect(&kms_state, center_x, grid_y, scale, grid_h, 0x203050);
        kms_fill_rect(&kms_state, left, center_y, content_width, scale, 0x203050);
        kms_draw_text_fit(&kms_state, left + scale * 2, grid_y + scale * 2,
                          "SAFE AREA GRID", 0x88ee88, small_scale,
                          content_width - scale * 4);
    }

    kms_draw_text_fit(&kms_state, left, footer_y, "ANY BUTTON BACK",
...
static int reattach_console(const char *reason, bool announce) {
    ...
    native_logf("console", "reattach requested reason=%s old_fd=%d",
                reason, old_fd);
    klogf("<6>A90v72: console reattach requested reason=%s old_fd=%d\n",
          reason, old_fd);
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v72: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v72: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    klogf("<6>A90v72: console reattached reason=%s fd=%d\n", reason, console_fd);
    if (announce) {
...
static int handle_displaytest(char **argv, int argc) {
    (void)argv;
    (void)argc;
    cprintf("displaytest: drawing color/font/grid screen\r\n");
    return draw_screen_display_test();
}
...
static const struct shell_command command_table[] = {
    ...
    { "displaytest", handle_displaytest, "displaytest", CMD_DISPLAY },
...
int main(void) {
    ...
    klogf("<6>A90v72: base mounts ready\n");
    ...
    klogf("<6>A90v72: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v72\n");
        klogf("<6>A90v72: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v72: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v72: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v72\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                mark_step("5_netservice_ok_v72\n");
                cprintf("# Netservice: NCM %s %s, tcpctl port %s.\r\n",
                        NETSERVICE_IFNAME,
                        NETSERVICE_DEVICE_IP,
                        NETSERVICE_TCP_PORT);
                klogf("<6>A90v72: netservice started\n");
            } else {
                ...
                timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v72: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v72: console attach failed (%d)\n", saved_errno);
    }
