...
static bool test_key_bit(const char *bitmap, unsigned int code) {
    ...
    for (token = strtok_r(copy, " \t", &saveptr);
         token != NULL;
         token = strtok_r(NULL, " \t", &saveptr), ++word_index) {
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v11: base mounts ready\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v11\n");
        klogf("<6>A90v11: cache mounted\n");
    } else {
        klogf("<6>A90v11: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v11\n");
        klogf("<6>A90v11: ACM gadget configured\n");
    } else {
        klogf("<6>A90v11: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v11\n");
        klogf("<6>A90v11: ttyGS0 ready\n");
    } else {
        klogf("<6>A90v11: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v11\n");
        cprintf("\r\nA90 Linux init v11\r\n");
