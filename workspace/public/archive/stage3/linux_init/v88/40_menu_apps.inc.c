    ...
    while (1) {
        ...
        if (active_app == SCREEN_APP_LOG) {
            ...
            draw_screen_cutout_calibration(&cutout_cal, true);
                   ...
                   active_app == SCREEN_APP_CHANGELOG_0819 ||
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v88: boot splash applied\n");
        ...
        klogf("<6>A90v88: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_menu(const struct screen_menu_page *page,
    ...
    a90_hud_draw_log_tail_panel(a90_kms_framebuffer(),
                            x,
...
static int draw_screen_display_test_page(unsigned int page_index) {
    ...
    if (page_index == 0) {
        ...
        for (index = 1; index <= 8; ++index) {
            ...
            snprintf(line, sizeof(line), "SCALE %u ABC123 0.8.19", row_scale);
...
static int draw_screen_changelog_v0819(void) {
    ...
    return draw_screen_about_lines("CHANGELOG / 0.8.19", lines, SCREEN_MENU_COUNT(lines));
}
