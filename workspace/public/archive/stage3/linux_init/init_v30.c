...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v30: automatic giant TEST probe applied\n");
        }
    } else {
        klogf("<6>A90v30: automatic giant TEST probe skipped (%d)\n", errno);
    }
}
...
static void cmd_status(void) {
    ...
    if (read_long_value("/sys/class/power_supply/battery/capacity", &value) == 0) {
        snprintf(battery_pct, sizeof(battery_pct), "%ld%%", value);
    }
    if (read_trimmed_text_file("/sys/class/power_supply/battery/status",
                               battery_status,
                               sizeof(battery_status)) < 0) {
        strcpy(battery_status, "?");
    }
    if (read_long_value("/sys/class/power_supply/battery/temp", &value) == 0) {
        format_temp_tenths(battery_temp, sizeof(battery_temp), value * 100);
    }
    if (read_long_value("/sys/class/power_supply/battery/voltage_now", &value) == 0) {
        snprintf(battery_voltage, sizeof(battery_voltage), "%ldmV", value / 1000);
    }
    if (read_average_thermal_temp("cpu-", "cpuss", &value) == 0) {
        format_temp_tenths(cpu_temp, sizeof(cpu_temp), value);
    }
    if (read_average_thermal_temp("gpuss", NULL, &value) == 0) {
        format_temp_tenths(gpu_temp, sizeof(gpu_temp), value);
    }
    if (read_meminfo_kb("MemTotal", &mem_total_kb) == 0 &&
        read_meminfo_kb("MemAvailable", &mem_avail_kb) == 0) {
        snprintf(memory, sizeof(memory), "%ld/%ldMB used",
                 (mem_total_kb - mem_avail_kb) / 1024,
                 mem_total_kb / 1024);
    }
    read_first_token("/proc/loadavg", loadavg, sizeof(loadavg));
    read_first_token("/proc/uptime", uptime, sizeof(uptime));

    cprintf("init: %s\r\n", INIT_BANNER);
    cprintf("uptime: %ss  load=%s\r\n", uptime, loadavg);
    cprintf("battery: %s %s temp=%s voltage=%s\r\n",
            battery_pct, battery_status, battery_temp, battery_voltage);
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v30: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v30: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v30\n");
        klogf("<6>A90v30: cache mounted\n");
    } else {
        klogf("<6>A90v30: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v30\n");
        klogf("<6>A90v30: ACM gadget configured\n");
    } else {
        klogf("<6>A90v30: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v30\n");
        klogf("<6>A90v30: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v30: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
