...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v41: automatic giant TEST probe applied\n");
            native_logf("boot", "display test probe applied");
        }
    } else {
        klogf("<6>A90v41: automatic giant TEST probe skipped (%d)\n", errno);
...
int main(void) {
    ...
    if (mount_cache() == 0) {
        ...
        klogf("<6>A90v41: cache mount failed (%d)\n", saved_errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v41\n");
        native_logf("boot", "ACM gadget configured");
        klogf("<6>A90v41: ACM gadget configured\n");
    } else {
        int saved_errno = errno;
        native_logf("boot", "ACM gadget failed errno=%d error=%s",
                    saved_errno, strerror(saved_errno));
        klogf("<6>A90v41: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v41\n");
        native_logf("boot", "ttyGS0 ready");
        klogf("<6>A90v41: ttyGS0 ready\n");
        boot_auto_frame();
        ...
        native_logf("boot", "ttyGS0 missing errno=%d error=%s",
                    saved_errno, strerror(saved_errno));
        klogf("<6>A90v41: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v41\n");
        ...
        native_logf("boot", "console attach failed errno=%d error=%s",
                    saved_errno, strerror(saved_errno));
        klogf("<6>A90v41: console attach failed (%d)\n", saved_errno);
    }
