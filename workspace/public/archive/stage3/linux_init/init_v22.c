...
static void boot_auto_frame(void) {
    if (kms_prepare_buffer(0x000000) == 0) {
        kms_draw_boot_frame(&kms_state);
        klogf("<6>A90v22: automatic display frame applied\n");
    } else {
        klogf("<6>A90v22: automatic display frame skipped (%d)\n", errno);
    }
}
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v22: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v22: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v22\n");
        klogf("<6>A90v22: cache mounted\n");
    } else {
        klogf("<6>A90v22: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v22\n");
        klogf("<6>A90v22: ACM gadget configured\n");
    } else {
        klogf("<6>A90v22: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v22\n");
        klogf("<6>A90v22: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v22: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v22\n");
        cprintf("\r\nA90 Linux init v22\r\n");
