/* Included by stage3/linux_init/init_v103.c. Do not compile standalone. */
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v103: boot splash applied\n");
        ...
        klogf("<6>A90v103: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_changelog_v0830(void) {
    const char *lines[] = {
        "0.9.3 v103 WIFI INVENTORY",
        "Adds wifiinv summary/full command",
        "Inventories WLAN/rfkill paths",
        "Keeps Wi-Fi inventory read-only",
        "Defers bring-up to v104 evidence",
        "Preserves service/USB state",
    };

    return draw_screen_about_lines("CHANGELOG / 0.9.3", lines, SCREEN_MENU_COUNT(lines));
}
