...
static void kms_draw_status_overlay(struct kms_display_state *state,
    ...
    kms_draw_text(state, x, y + slot * 4, storage_line, storage_color, scale > 3 ? scale - 2 : scale);
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v79: boot splash applied\n");
        ...
        klogf("<6>A90v79: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_display_test_page(unsigned int page_index) {
    ...
    if (page_index == 0) {
        ...
        for (index = 1; index <= 8; ++index) {
            ...
            snprintf(line, sizeof(line), "SCALE %u ABC123 0.8.10", row_scale);
...
static int reattach_console(const char *reason, bool announce) {
    ...
    if (!quiet_success) {
        native_logf("console", "reattach requested reason=%s old_fd=%d",
                    reason, old_fd);
        klogf("<6>A90v79: console reattach requested reason=%s old_fd=%d\n",
              reason, old_fd);
    }
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v79: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v79: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (!quiet_success) {
        ...
        klogf("<6>A90v79: console reattached reason=%s fd=%d\n", reason, console_fd);
    }
    if (announce) {
...
static void boot_storage_use_cache(const char *reason, int rc, int saved_errno) {
    ...
    native_logf("storage", "fallback root=%s reason=%s detail=%s",
                boot_storage.root,
                reason,
                boot_storage.detail);
    timeline_record(rc, saved_errno, "storage", "%s", boot_storage.warning);
}
...
static void boot_storage_use_sd(void) {
    ...
    timeline_record(0, 0, "storage", "sd main root=%s uuid=%s",
                    boot_storage.root,
                    boot_storage.sd_uuid);
}
...
static void boot_storage_probe(void) {
    ...
    boot_auto_frame();
    native_logf("storage", "boot sd probe start expected_uuid=%s", SD_EXPECTED_UUID);
...
static int cmd_mountsd_status(void) {
    ...
    if (read_ext4_uuid(node_path, uuid, sizeof(uuid)) == 0) {
        cprintf("mountsd: uuid=%s expected=%s match=%s\r\n",
                uuid,
                SD_EXPECTED_UUID,
...
static int cmd_storage(void) {
    cprintf("storage: backend=%s root=%s fallback=%s\r\n",
            boot_storage.backend,
            boot_storage.root,
            boot_storage.fallback ? "yes" : "no");
    cprintf("storage: sd present=%s mounted=%s expected=%s rw=%s uuid=%s\r\n",
            boot_storage.sd_present ? "yes" : "no",
            boot_storage.sd_mounted ? "yes" : "no",
            boot_storage.sd_expected ? "yes" : "no",
            boot_storage.sd_rw_ok ? "yes" : "no",
            boot_storage.sd_uuid);
    cprintf("storage: expected_uuid=%s id_file=%s\r\n", SD_EXPECTED_UUID, SD_ID_FILE);
    cprintf("storage: detail=%s\r\n", boot_storage.detail);
    if (boot_storage.warning[0] != '\0') {
        cprintf("storage: warning=%s\r\n", boot_storage.warning);
    }
    cprintf("storage: log=%s\r\n", native_log_current_path());
    return 0;
}
...
int main(void) {
    ...
    klogf("<6>A90v79: base mounts ready\n");
    ...
    klogf("<6>A90v79: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v79\n");
        klogf("<6>A90v79: cache mounted\n");
    } else {
        int saved_errno = errno;
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno),
                        native_log_current_path());
        klogf("<6>A90v79: cache mount failed (%d)\n", saved_errno);
    }
    boot_auto_frame();
    boot_storage_probe();

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v79\n");
        ...
        klogf("<6>A90v79: ACM gadget configured\n");
    } else {
        int saved_errno = errno;
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v79: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v79: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v79\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                mark_step("5_netservice_ok_v79\n");
                cprintf("# Netservice: NCM %s %s, tcpctl port %s.\r\n",
                        NETSERVICE_IFNAME,
                        NETSERVICE_DEVICE_IP,
                        NETSERVICE_TCP_PORT);
                klogf("<6>A90v79: netservice started\n");
            } else {
                ...
                timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v79: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v79: console attach failed (%d)\n", saved_errno);
    }
