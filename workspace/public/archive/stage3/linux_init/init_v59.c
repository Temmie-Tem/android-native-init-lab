...
static void kms_draw_status_overlay(struct kms_display_state *state,
    ...
    snprintf(footer, sizeof(footer), "A90v59 UP %.8s", snapshot.uptime);
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v59: automatic giant TEST probe applied\n");
        ...
        klogf("<6>A90v59: automatic giant TEST probe skipped (%d)\n", saved_errno);
...
static int reattach_console(const char *reason, bool announce) {
    ...
    native_logf("console", "reattach requested reason=%s old_fd=%d",
                reason, old_fd);
    klogf("<6>A90v59: console reattach requested reason=%s old_fd=%d\n",
          reason, old_fd);
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v59: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v59: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    klogf("<6>A90v59: console reattached reason=%s fd=%d\n", reason, console_fd);
    if (announce) {
...
int main(void) {
    ...
    klogf("<6>A90v59: base mounts ready\n");
    ...
    klogf("<6>A90v59: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v59\n");
        klogf("<6>A90v59: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v59: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v59: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v59\n");
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v59: console attach failed (%d)\n", saved_errno);
    }
