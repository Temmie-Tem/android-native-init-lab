...
static void kms_draw_status_overlay(struct kms_display_state *state,
    ...
    snprintf(footer, sizeof(footer), "A90v64 UP %.8s", snapshot.uptime);
...
static void kms_draw_boot_splash(struct kms_display_state *state) {
    uint32_t width = state->width;
    uint32_t height = state->height;
    uint32_t scale = width >= 1080 ? 7 : 4;
    uint32_t title_scale = scale + 2;
    uint32_t x = width / 14;
    uint32_t y = height / 9;
    uint32_t card_w = width - x * 2;
    uint32_t line_h = scale * 12;
    uint32_t card_y;
    uint32_t row_y;
    uint32_t row_gap = scale * 13;
    uint32_t footer_scale = scale > 2 ? scale - 1 : scale;
    ...
    kms_draw_text(state, x, y, "A90 NATIVE INIT", 0xffffff, title_scale);
    y += title_scale * 10;
    kms_draw_text(state, x, y, "BOOTING CUSTOM USERSPACE", 0x88ee88, scale);
    y += line_h;
    kms_draw_text(state, x, y, INIT_BANNER, 0xffcc33, scale);
    ...
    row_y = card_y + scale;
    kms_draw_text(state, x + scale * 4, row_y, "[ KERNEL  ] STOCK ANDROID LINUX 4.14", 0xffffff, scale);
    row_y += row_gap;
    kms_draw_text(state, x + scale * 4, row_y, "[ DISPLAY ] KMS FRAMEBUFFER READY", 0xffffff, scale);
    row_y += row_gap;
    kms_draw_text(state, x + scale * 4, row_y, "[ SERIAL  ] USB ACM CONSOLE READY", 0xffffff, scale);
    row_y += row_gap;
    kms_draw_text(state, x + scale * 4, row_y, "[ RUNTIME ] HUD + BUTTON MENU LOADING", 0xffffff, scale);

    kms_draw_text(state,
                  x,
                  height - footer_scale * 14,
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v64: boot splash applied\n");
        ...
        klogf("<6>A90v64: boot splash skipped (%d)\n", saved_errno);
...
static int cmd_screenmenu(void) {
    ...
    while (1) {
        ...
        if (code == KEY_VOLUMEUP) {
            ...
            selected = (selected + page->count - 1) % page->count;
            print_screen_menu_selection(page, selected);
            draw_rc = draw_screen_menu(page, selected);
            if (draw_rc < 0) {
                close_key_wait_context(&ctx);
        ...
        if (code == KEY_VOLUMEDOWN) {
            page = screen_menu_get_page(current_page);
            selected = (selected + 1) % page->count;
            print_screen_menu_selection(page, selected);
            draw_rc = draw_screen_menu(page, selected);
            if (draw_rc < 0) {
                close_key_wait_context(&ctx);
...
static int reattach_console(const char *reason, bool announce) {
    ...
    native_logf("console", "reattach requested reason=%s old_fd=%d",
                reason, old_fd);
    klogf("<6>A90v64: console reattach requested reason=%s old_fd=%d\n",
          reason, old_fd);
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v64: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v64: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    klogf("<6>A90v64: console reattached reason=%s fd=%d\n", reason, console_fd);
    if (announce) {
...
int main(void) {
    ...
    klogf("<6>A90v64: base mounts ready\n");
    ...
    klogf("<6>A90v64: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v64\n");
        klogf("<6>A90v64: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v64: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v64: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v64\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                mark_step("5_netservice_ok_v64\n");
                cprintf("# Netservice: NCM %s %s, tcpctl port %s.\r\n",
                        NETSERVICE_IFNAME,
                        NETSERVICE_DEVICE_IP,
                        NETSERVICE_TCP_PORT);
                klogf("<6>A90v64: netservice started\n");
            } else {
                ...
                timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v64: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v64: console attach failed (%d)\n", saved_errno);
    }
