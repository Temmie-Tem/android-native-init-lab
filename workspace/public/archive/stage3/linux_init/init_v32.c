...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v32: automatic giant TEST probe applied\n");
        }
    } else {
        klogf("<6>A90v32: automatic giant TEST probe skipped (%d)\n", errno);
    }
}
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v32: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v32: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v32\n");
        klogf("<6>A90v32: cache mounted\n");
    } else {
        klogf("<6>A90v32: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v32\n");
        klogf("<6>A90v32: ACM gadget configured\n");
    } else {
        klogf("<6>A90v32: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v32\n");
        klogf("<6>A90v32: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v32: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v32\n");
        cprintf("\r\n%s\r\n", INIT_BANNER);
