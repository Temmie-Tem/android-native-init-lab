...
static int kms_prepare_buffer(uint32_t color) {
    ...
    if (kms_state.fd >= 0 &&
        ...
        page_flip.crtc_id = kms_state.crtc_id;
        page_flip.fb_id = kms_state.fb_id[next_buffer];
        if (drm_ioctl_retry(kms_state.fd, DRM_IOCTL_MODE_PAGE_FLIP, &page_flip) < 0) {
            cprintf("kmssolid: PAGE_FLIP failed: %s\r\n", strerror(errno));
            memset(&setcrtc, 0, sizeof(setcrtc));
            connector_list[0] = kms_state.connector_id;
            setcrtc.crtc_id = kms_state.crtc_id;
            setcrtc.fb_id = kms_state.fb_id[next_buffer];
            ...
            drm_ioctl_retry(kms_state.fd, DRM_IOCTL_MODE_SETCRTC, &setcrtc);
        }
        cprintf("kmssolid: flipped framebuffer %ux%u on crtc=%u\r\n",
    ...
    setcrtc.fb_id = kms_state.fb_id[1];
    setcrtc.set_connectors_ptr = (uintptr_t)connector_list;
    setcrtc.count_connectors = 1;
    setcrtc.mode = mode;
    setcrtc.mode_valid = 1;
...
static void cmd_statusscreen(void) {
    if (kms_prepare_buffer(0x080808) == 0) {
        kms_draw_visibility_probe(&kms_state);
        cprintf("statusscreen: rendered %ux%u visibility probe on crtc=%u\r\n",
                kms_state.width, kms_state.height, kms_state.crtc_id);
    }
}
...
static void boot_auto_frame(void) {
    if (kms_prepare_buffer(0x000000) == 0) {
        kms_draw_visibility_probe(&kms_state);
        klogf("<6>A90v26: automatic visibility probe applied\n");
    } else {
        klogf("<6>A90v26: automatic visibility probe skipped (%d)\n", errno);
    }
}
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v26: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v26: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v26\n");
        klogf("<6>A90v26: cache mounted\n");
    } else {
        klogf("<6>A90v26: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v26\n");
        klogf("<6>A90v26: ACM gadget configured\n");
    } else {
        klogf("<6>A90v26: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v26\n");
        klogf("<6>A90v26: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v26: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v26\n");
        cprintf("\r\nA90 Linux init v26\r\n");
