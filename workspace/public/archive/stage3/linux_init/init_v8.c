...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v8: base mounts ready\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v8\n");
        klogf("<6>A90v8: cache mounted\n");
    } else {
        klogf("<6>A90v8: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v8\n");
        klogf("<6>A90v8: ACM gadget configured\n");
    } else {
        klogf("<6>A90v8: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v8\n");
        klogf("<6>A90v8: ttyGS0 ready\n");
    } else {
        klogf("<6>A90v8: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v8\n");
        cprintf("\r\nA90 Linux init v8\r\n");
