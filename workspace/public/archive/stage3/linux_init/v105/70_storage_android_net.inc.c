...
static int cmd_rshell(char **argv, int argc) {
    ...
    if (strcmp(subcommand, "status") == 0) {
        ...
        a90_console_printf("rshell: token=%s token_path=%s flag_path=%s\r\n",
    ...
    a90_console_printf("usage: rshell [status|start|stop|enable|disable|token [show]|rotate-token [value]]\r\n");
    return -EINVAL;
}
