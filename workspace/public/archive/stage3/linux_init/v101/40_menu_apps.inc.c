/* Included by stage3/linux_init/init_v101.c. Do not compile standalone. */
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v101: boot splash applied\n");
        ...
        klogf("<6>A90v101: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_changelog_v0830(void) {
    const char *lines[] = {
        "0.9.1 v101 SERVICE MANAGER",
        "Adds service list/status command",
        "Adds service start/stop dispatch",
        "Adds service enable/disable policy",
        "Keeps service owners unchanged",
        "Preserves serial rescue path",
    };

    return draw_screen_about_lines("CHANGELOG / 0.9.1", lines, SCREEN_MENU_COUNT(lines));
}
