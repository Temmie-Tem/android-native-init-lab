...
static int reattach_console(const char *reason, bool announce) {
    ...
    if (!quiet_success) {
        native_logf("console", "reattach requested reason=%s old_fd=%d",
                    reason, old_fd);
        klogf("<6>A90v81: console reattach requested reason=%s old_fd=%d\n",
              reason, old_fd);
    }
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v81: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v81: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (!quiet_success) {
        ...
        klogf("<6>A90v81: console reattached reason=%s fd=%d\n", reason, console_fd);
    }
    if (announce) {
