...
#define INIT_VERSION "0.7.3"
#define INIT_BUILD "v66"
...
static const struct screen_menu_item screen_menu_about_items[] = {
    ...
    { "CHANGELOG", "RECENT UPDATES", SCREEN_MENU_ABOUT_CHANGELOG, SCREEN_MENU_PAGE_ABOUT },
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v66: boot splash applied\n");
        ...
        klogf("<6>A90v66: boot splash skipped (%d)\n", saved_errno);
...
static int draw_screen_about_changelog(void) {
    const char *lines[] = {
        ...
        "0.4.0 v55 TCP CONTROL OVER NCM",
    };
...
static int reattach_console(const char *reason, bool announce) {
    ...
    native_logf("console", "reattach requested reason=%s old_fd=%d",
                reason, old_fd);
    klogf("<6>A90v66: console reattach requested reason=%s old_fd=%d\n",
          reason, old_fd);
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v66: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v66: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    klogf("<6>A90v66: console reattached reason=%s fd=%d\n", reason, console_fd);
    if (announce) {
...
int main(void) {
    ...
    klogf("<6>A90v66: base mounts ready\n");
    ...
    klogf("<6>A90v66: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v66\n");
        klogf("<6>A90v66: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v66: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v66: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v66\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                mark_step("5_netservice_ok_v66\n");
                cprintf("# Netservice: NCM %s %s, tcpctl port %s.\r\n",
                        NETSERVICE_IFNAME,
                        NETSERVICE_DEVICE_IP,
                        NETSERVICE_TCP_PORT);
                klogf("<6>A90v66: netservice started\n");
            } else {
                ...
                timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v66: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v66: console attach failed (%d)\n", saved_errno);
    }
