...
#define INIT_VERSION "0.8.0"
#define INIT_BUILD "v69"
...
static void auto_hud_loop(unsigned int refresh_sec) {
    ...
    while (1) {
        ...
        set_auto_menu_active(menu_active || active_app != SCREEN_APP_NONE);
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v69: boot splash applied\n");
        ...
        klogf("<6>A90v69: boot splash skipped (%d)\n", saved_errno);
...
static int wait_for_input_gesture(struct key_wait_context *ctx,
    ...
    unsigned int primary_code = 0;
    unsigned int primary_mask = 0;
    unsigned int pending_code = 0;
    unsigned int pending_mask = 0;
    long first_down_ms = 0;
    long pending_up_ms = 0;
    ...
    while (1) {
        ...
        if (waiting_second) {
            long now_ms = monotonic_millis();
            long elapsed_ms = now_ms - pending_up_ms;

            if (elapsed_ms >= INPUT_DOUBLE_CLICK_MS) {
                input_gesture_set(gesture,
                                  input_single_gesture(pending_code, 1,
                                                       pending_duration_ms),
                                  pending_code,
                                  pending_mask,
                                  1,
            ...
            timeout_ms = (int)(INPUT_DOUBLE_CLICK_MS - elapsed_ms);
        }
        ...
        for (index = 0; index < 2; ++index) {
            if (fds[index].revents & POLLIN) {
                ...
                while ((rd = read(fds[index].fd, &event, sizeof(event))) ==
                    ...
                    if (event.value == 1) {
                        if (waiting_second) {
                            if (mask == pending_mask &&
                                down_mask == 0 &&
                                now_ms - pending_up_ms <= INPUT_DOUBLE_CLICK_MS) {
                                waiting_second = false;
                                second_click = true;
                                down_mask = mask;
                                pressed_mask = mask;
                                primary_code = event.code;
                                primary_mask = mask;
                                first_down_ms = now_ms;
                            } else {
                                input_gesture_set(gesture,
                                                  input_single_gesture(pending_code,
                                                                       1,
                                                                       pending_duration_ms),
                                                  pending_code,
                                                  pending_mask,
                    ...
                    } else if (event.value == 0 && (down_mask & mask) != 0) {
                        down_mask &= ~mask;
                        if (down_mask == 0) {
                            long duration_ms = now_ms - first_down_ms;
                            unsigned int count = input_button_count(pressed_mask);

                            if (count > 1) {
                                input_gesture_set(gesture,
                                                  input_combo_gesture(pressed_mask),
                                                  primary_code,
                                                  pressed_mask,
                                                  1,
                                                  duration_ms);
                            ...
                            if (second_click) {
                                input_gesture_set(gesture,
                                                  input_single_gesture(primary_code,
                                                                       2,
                                                                       duration_ms),
                                                  primary_code,
                                                  primary_mask,
                            ...
                            waiting_second = true;
                            pending_code = primary_code;
                            pending_mask = primary_mask;
                            pending_up_ms = now_ms;
...
/* Draw last N log lines into the HUD overlay area below the status cards. */
static void kms_draw_hud_log_tail(struct kms_display_state *state) {
#define HUD_LOG_TAIL  14
#define HUD_LOG_LMAX  80
    static char buf[HUD_LOG_TAIL][HUD_LOG_LMAX];
    static int  nstored;
    static int  ring_start;
    uint32_t scale = 4;
...
static int reattach_console(const char *reason, bool announce) {
    ...
    native_logf("console", "reattach requested reason=%s old_fd=%d",
                reason, old_fd);
    klogf("<6>A90v69: console reattach requested reason=%s old_fd=%d\n",
          reason, old_fd);
    ...
    if (wait_for_tty_gs0() < 0) {
        ...
        klogf("<6>A90v69: console reattach wait failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    if (attach_console() < 0) {
        ...
        native_logf("console", "reattach open failed reason=%s errno=%d error=%s",
                    reason, saved_errno, strerror(saved_errno));
        klogf("<6>A90v69: console reattach open failed (%d)\n", saved_errno);
        errno = saved_errno;
        return -1;
    }
    ...
    klogf("<6>A90v69: console reattached reason=%s fd=%d\n", reason, console_fd);
    if (announce) {
...
int main(void) {
    ...
    klogf("<6>A90v69: base mounts ready\n");
    ...
    klogf("<6>A90v69: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v69\n");
        klogf("<6>A90v69: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v69: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v69: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v69\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                mark_step("5_netservice_ok_v69\n");
                cprintf("# Netservice: NCM %s %s, tcpctl port %s.\r\n",
                        NETSERVICE_IFNAME,
                        NETSERVICE_DEVICE_IP,
                        NETSERVICE_TCP_PORT);
                klogf("<6>A90v69: netservice started\n");
            } else {
                ...
                timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v69: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v69: console attach failed (%d)\n", saved_errno);
    }
