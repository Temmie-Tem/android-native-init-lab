...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v99: boot splash applied\n");
        ...
        klogf("<6>A90v99: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_display_test_page(unsigned int page_index) {
    ...
    if (page_index == 0) {
        ...
        for (index = 1; index <= 8; ++index) {
            ...
            snprintf(line, sizeof(line), "SCALE %u ABC123 0.8.30", row_scale);
...
static int draw_screen_changelog_v0830(void) {
    const char *lines[] = {
        "0.8.30 v99 BUSYBOX USERLAND",
        "Adds a90_userland.c/h",
        "Adds userland shell command",
        "Adds busybox/toybox wrappers",
        "Keeps native shell as PID1",
        "Treats BusyBox as optional",
    };

    return draw_screen_about_lines("CHANGELOG / 0.8.30", lines, SCREEN_MENU_COUNT(lines));
}
