...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v102: boot splash applied\n");
        ...
        klogf("<6>A90v102: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_changelog_v0830(void) {
    const char *lines[] = {
        "0.9.2 v102 DIAGNOSTICS",
        "Adds diag summary/full command",
        "Adds diag bundle writer",
        "Adds serial-first host collector",
        "Keeps diagnostics read-only",
        "Preserves service/USB state",
    };

    return draw_screen_about_lines("CHANGELOG / 0.9.2", lines, SCREEN_MENU_COUNT(lines));
}
