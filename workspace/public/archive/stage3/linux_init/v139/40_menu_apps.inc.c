/* Included by stage3/linux_init/init_v139.c. Do not compile standalone. */
...
struct auto_hud_state {
    ...
    pid_t app_stress_pid;
    unsigned int app_stress_workers;
    long app_stress_deadline_ms;
    long app_stress_duration_ms;
    bool app_stress_done;
    bool app_stress_failed;
    ...
    unsigned int cutout_down_mask;
    long cutout_power_down_ms;
    long cutout_last_power_up_ms;
...
static void auto_hud_reset_cutout_state(struct auto_hud_state *state) {
    cutout_calibration_init(&state->cutout_cal);
    state->cutout_down_mask = 0;
    state->cutout_power_down_ms = 0;
    state->cutout_last_power_up_ms = 0;
}
...
static void auto_hud_stop_active_app(struct auto_hud_state *state) {
    if (state->active_app == SCREEN_APP_CPU_STRESS &&
        state->app_stress_pid > 0) {
        screen_app_stop_cpu_stress(&state->app_stress_pid);
    }
}
...
static void auto_hud_state_init(struct auto_hud_state *state) {
    ...
    state->active_app = SCREEN_APP_NONE;
    state->app_stress_pid = -1;
    state->app_stress_workers = 8;
    state->app_stress_duration_ms = 10000L;
...
static void auto_hud_draw_current_screen(struct auto_hud_state *state) {
    if (state->active_app == SCREEN_APP_LOG) {
        ...
        draw_screen_cutout_calibration(&state->cutout_cal, true);
        ...
        long now_ms = monotonic_millis();
        long remaining_ms = state->app_stress_deadline_ms - now_ms;

        if (screen_app_reap_cpu_stress(&state->app_stress_pid,
                                       &state->app_stress_failed) &&
            !state->app_stress_failed) {
            state->app_stress_done = true;
        } else if (state->app_stress_pid > 0 &&
                   now_ms > state->app_stress_deadline_ms + 2000L) {
            screen_app_stop_cpu_stress(&state->app_stress_pid);
            state->app_stress_done = true;
            state->app_stress_failed = true;
        }
        if (remaining_ms < 0) {
            remaining_ms = 0;
        }
        draw_screen_cpu_stress_app(state->app_stress_pid > 0,
                                   state->app_stress_done,
                                   state->app_stress_failed,
                                   remaining_ms,
                                   state->app_stress_duration_ms);
...
static bool auto_hud_handle_special_app_event(struct auto_hud_state *state,
    ...
    if (state->active_app == SCREEN_APP_CUTOUT_CAL &&
        event->type == EV_KEY) {
        if (cutout_calibration_feed_key(&state->cutout_cal,
                                        event,
                                        &state->cutout_down_mask,
                                        &state->cutout_power_down_ms,
                                        &state->cutout_last_power_up_ms)) {
            auto_hud_show_menu(state, false);
            auto_hud_reset_cutout_state(state);
        }
        return true;
...
static void auto_hud_start_cpu_stress_app(struct auto_hud_state *state,
    ...
    state->app_stress_done = false;
    state->app_stress_failed = false;
    state->app_stress_duration_ms = stress_seconds * 1000L;
    state->app_stress_deadline_ms = monotonic_millis() +
                                    state->app_stress_duration_ms;
    if (screen_app_start_cpu_stress(&state->app_stress_pid,
                                    stress_seconds,
                                    state->app_stress_workers) < 0) {
        state->app_stress_failed = true;
        state->app_stress_done = true;
        state->app_stress_pid = -1;
    }
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v139: boot splash applied\n");
        ...
        klogf("<6>A90v139: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
