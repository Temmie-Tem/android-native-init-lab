...
static void kms_draw_status_overlay(struct kms_display_state *state) {
    ...
    long mem_used_mb = -1;
    long mem_total_mb = -1;
    uint32_t scale = (state->width >= 1080) ? 8 : 6;
    uint32_t x = state->width / 22;
    uint32_t y = state->height / 9;
    ...
    kms_draw_text(state, x, footer_y, line6, 0xffffff, scale);
}
...
static void boot_auto_frame(void) {
    if (kms_prepare_buffer(0x000000) == 0) {
        kms_draw_boot_frame(&kms_state);
        kms_draw_status_overlay(&kms_state);
        klogf("<6>A90v24: automatic display frame applied\n");
    } else {
        klogf("<6>A90v24: automatic display frame skipped (%d)\n", errno);
    }
}
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v24: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v24: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v24\n");
        klogf("<6>A90v24: cache mounted\n");
    } else {
        klogf("<6>A90v24: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v24\n");
        klogf("<6>A90v24: ACM gadget configured\n");
    } else {
        klogf("<6>A90v24: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v24\n");
        klogf("<6>A90v24: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v24: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v24\n");
        cprintf("\r\nA90 Linux init v24\r\n");
