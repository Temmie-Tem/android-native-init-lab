...
int main(void) {
    ...
    klogf("<6>A90v133: base mounts ready\n");
    ...
    klogf("<6>A90v133: early display/input nodes prepared\n");
    ...
    if (a90_storage_mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v133\n");
        klogf("<6>A90v133: cache mounted\n");
    } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        a90_log_path());
        klogf("<6>A90v133: cache mount failed (%d)\n", saved_errno);
    }
    boot_auto_frame();
    a90_storage_probe_boot(&storage_hooks, NULL);
    {
        ...
        if (runtime_rc == 0) {
            ...
            klogf("<6>A90v133: runtime root ready %s\n", a90_runtime_root());
        } else {
            ...
            klogf("<6>A90v133: runtime root warning (%d)\n", runtime_errno);
        }
        boot_auto_frame();
    }
    ...
    if (a90_helper_scan() == 0) {
        ...
        klogf("<6>A90v133: helper inventory ready %s\n", helper_summary);
    } else {
        char helper_summary[96];
        ...
        klogf("<6>A90v133: helper inventory warning %s\n", helper_summary);
    }
    boot_auto_frame();
    ...
    if (a90_userland_scan() == 0) {
        ...
        klogf("<6>A90v133: userland inventory ready %s\n", userland_summary);
    } else {
        char userland_summary[96];
        ...
        klogf("<6>A90v133: userland inventory warning %s\n", userland_summary);
    }
    boot_auto_frame();

    if (a90_usb_gadget_setup_acm() == 0) {
        mark_step("2_gadget_ok_v133\n");
        ...
        klogf("<6>A90v133: ACM gadget configured\n");
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v133: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_wait_tty() == 0) {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v133: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_attach() == 0) {
        mark_step("4_console_attached_v133\n");
        ...
        if (a90_netservice_enabled()) {
            ...
            if (net_rc == 0) {
                ...
                a90_timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v133: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        if (rshell_enabled()) {
            ...
            if (rshell_rc == 0) {
                ...
                a90_timeline_record(rshell_rc,
                                ...
                                "start failed: %s",
                                strerror(rshell_errno));
                klogf("<6>A90v133: rshell failed (%d)\n", rshell_errno);
            }
        } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v133: console attach failed (%d)\n", saved_errno);
    }
