            ...
            while (read(fds[fi].fd, &ev, sizeof(ev)) == (ssize_t)sizeof(ev)) {
                ...
                if (active_app != SCREEN_APP_NONE) {
                    if (a90_menu_app_is_about(active_app)) {
                        size_t page_count = a90_app_about_page_count(active_app, about_changelog_index);

                        if (page_count > 1 && ev.code == KEY_VOLUMEUP) {
                            about_page_index = (about_page_index + page_count - 1) % page_count;
                            continue;
                        }
                        if (page_count > 1 && ev.code == KEY_VOLUMEDOWN) {
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v130: boot splash applied\n");
        ...
        klogf("<6>A90v130: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
