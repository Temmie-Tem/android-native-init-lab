/* Included by stage3/linux_init/init_v87.c. Do not compile standalone. */
...
static void kms_draw_menu_section(struct a90_fb *fb,
                                  ...
                                  size_t selected);
static void kms_draw_log_tail_panel(struct a90_fb *fb,
                                    uint32_t x,
                                    uint32_t y,
                                    uint32_t width,
                                    uint32_t bottom,
                                    ...
                                    const char *title,
                                    uint32_t scale);
static void kms_draw_hud_log_tail(struct a90_fb *fb);
    ...
    while (1) {
        ...
        if (active_app == SCREEN_APP_LOG) {
            ...
            draw_screen_cutout_calibration(&cutout_cal, true);
                   ...
                   active_app == SCREEN_APP_CHANGELOG_0818 ||
                   active_app == SCREEN_APP_CHANGELOG_0817 ||
                   active_app == SCREEN_APP_CHANGELOG_0816 ||
            ...
            kms_draw_status_overlay(a90_kms_framebuffer(), 0, 0);
            if (menu_active)
                kms_draw_menu_section(a90_kms_framebuffer(), page, menu_sel);
            else
                kms_draw_hud_log_tail(a90_kms_framebuffer());
...
static void kms_draw_boot_splash(struct a90_fb *fb) {
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        kms_draw_boot_splash(a90_kms_framebuffer());
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v87: boot splash applied\n");
        ...
        klogf("<6>A90v87: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static void kms_draw_log_tail_panel(struct a90_fb *fb,
    ...
    a90_draw_text(fb, x, y + scale * 2, title, 0xffcc33,
...
static int draw_screen_menu(const struct screen_menu_page *page,
    ...
    a90_draw_text(a90_kms_framebuffer(), x, title_y, page->title, 0xffcc33,
                  shrink_text_scale(page->title, title_scale, card_w));
    a90_draw_text(a90_kms_framebuffer(), x, title_y + title_scale * 9,
    ...
    for (index = 0; index < page->count; ++index) {
        ...
        a90_draw_rect(a90_kms_framebuffer(), x - scale, item_y - scale, card_w, item_h, fill);
        if (index == selected) {
            a90_draw_rect(a90_kms_framebuffer(), x - scale, item_y - scale, scale * 2, item_h, 0xffcc33);
        }
        a90_draw_text(a90_kms_framebuffer(), x + scale * 3, item_y + scale,
                      page->items[index].name, name_color, scale);
        a90_draw_text(a90_kms_framebuffer(), x + scale * 3, item_y + scale * 9,
                      page->items[index].summary, summary_color, scale > 1 ? scale - 1 : scale);
    }

    footer_scale = shrink_text_scale(footer, scale, card_w);
    kms_draw_log_tail_panel(a90_kms_framebuffer(),
                            x,
                            list_y + (uint32_t)page->count * (item_h + gap) + scale * 4,
                            card_w,
                            a90_kms_framebuffer()->height - footer_scale * 16,
                            12,
                            "LIVE LOG TAIL",
                            scale > 3 ? scale - 3 : (scale > 2 ? scale - 1 : scale));
    a90_draw_text(a90_kms_framebuffer(), x, a90_kms_framebuffer()->height - footer_scale * 12,
                  footer, 0xffffff, footer_scale);

    if (a90_kms_present("screenmenu", true) < 0) {
        return negative_errno_or(EIO);
    }
    return 0;
}
...
static int draw_screen_display_test_page(unsigned int page_index) {
    ...
    if (page_index == 0) {
        ...
        for (index = 1; index <= 8; ++index) {
            ...
            snprintf(line, sizeof(line), "SCALE %u ABC123 0.8.18", row_scale);
        ...
        kms_draw_status_overlay(a90_kms_framebuffer(), left, card_y);
...
static int draw_screen_changelog_v0818(void) {
    ...
    return draw_screen_about_lines("CHANGELOG / 0.8.18", lines, SCREEN_MENU_COUNT(lines));
}
...
static int draw_screen_changelog_v0817(void) {
    ...
    return draw_screen_about_lines("CHANGELOG / 0.8.17", lines, SCREEN_MENU_COUNT(lines));
}
...
static int draw_screen_changelog_v0816(void) {
    ...
    return draw_screen_about_lines("CHANGELOG / 0.8.16", lines, SCREEN_MENU_COUNT(lines));
}
...
static int draw_screen_network_summary(void) {
    ...
    char line3[96];
    char line4[96];
    pid_t tcpctl_pid = a90_service_pid(A90_SERVICE_TCPCTL);
...
static int wait_for_menu_return(struct a90_input_context *ctx, const char *tag) {
    struct a90_input_gesture gesture;
    int wait_rc = a90_input_wait_gesture(ctx, tag, &gesture);
...
static int cmd_screenmenu(void) {
    ...
    reap_hud_child();
    restore_hud = a90_service_pid(A90_SERVICE_HUD) > 0;
    stop_auto_hud(false);

    if (a90_input_open(&ctx, "screenmenu") < 0) {
    ...
    while (1) {
        struct a90_input_gesture gesture;
        enum a90_input_menu_action menu_action;
        int wait_rc = a90_input_wait_gesture(&ctx, "screenmenu", &gesture);
        ...
        if (menu_action == A90_INPUT_MENU_ACTION_HIDE) {
            a90_input_close(&ctx);
            restore_auto_hud_if_needed(restore_hud);
            return 0;
        }

        if (menu_action == A90_INPUT_MENU_ACTION_STATUS) {
        ...
        if (menu_action == A90_INPUT_MENU_ACTION_LOG) {
