...
static void cmd_waitkey(char **argv, int argc) {
    ...
    if (get_input_event_path("event0", event0_path, sizeof(event0_path)) < 0 ||
        ...
        cprintf("waitkey: input node setup failed: %s\r\n", strerror(errno));
        return;
    }

    fd0 = open(event0_path, O_RDONLY | O_NONBLOCK);
    if (fd0 < 0) {
        cprintf("waitkey: open %s: %s\r\n", event0_path, strerror(errno));
        return;
    }

    fd3 = open(event3_path, O_RDONLY | O_NONBLOCK);
    if (fd3 < 0) {
        cprintf("waitkey: open %s: %s\r\n", event3_path, strerror(errno));
        close(fd0);
        return;
    }
    ...
    while (seen < target) {
        ...
        if (poll_rc < 0) {
            ...
            cprintf("waitkey: poll: %s\r\n", strerror(errno));
            break;
        }

        for (index = 0; index < 2 && seen < target; ++index) {
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v14: base mounts ready\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v14\n");
        klogf("<6>A90v14: cache mounted\n");
    } else {
        klogf("<6>A90v14: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v14\n");
        klogf("<6>A90v14: ACM gadget configured\n");
    } else {
        klogf("<6>A90v14: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v14\n");
        klogf("<6>A90v14: ttyGS0 ready\n");
    } else {
        klogf("<6>A90v14: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v14\n");
        cprintf("\r\nA90 Linux init v14\r\n");
