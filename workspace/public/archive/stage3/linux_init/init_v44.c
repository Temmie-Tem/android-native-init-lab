...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v44: automatic giant TEST probe applied\n");
        ...
        klogf("<6>A90v44: automatic giant TEST probe skipped (%d)\n", saved_errno);
...
int main(void) {
    ...
    klogf("<6>A90v44: base mounts ready\n");
    ...
    klogf("<6>A90v44: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v44\n");
        klogf("<6>A90v44: cache mounted\n");
    } else {
        int saved_errno = errno;
    ...
    if (setup_acm_gadget() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v44: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        ...
        timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v44: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v44\n");
        ...
        timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v44: console attach failed (%d)\n", saved_errno);
    }
