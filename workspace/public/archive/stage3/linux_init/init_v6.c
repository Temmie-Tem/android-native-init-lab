...
static void cmd_startadbd(void) {
    ...
    if (needs_rebind) {
        ...
        if (symlink("/config/usb_gadget/g1/functions/ffs.adb",
                    "/config/usb_gadget/g1/configs/b.1/f2") < 0 &&
            errno != EEXIST) {
            cprintf("startadbd: ffs link failed: %s\r\n", strerror(errno));
        }
        wf("/config/usb_gadget/g1/UDC", "a600000.dwc3");
    }
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v6: base mounts ready\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v6\n");
        klogf("<6>A90v6: cache mounted\n");
    } else {
        klogf("<6>A90v6: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v6\n");
        klogf("<6>A90v6: ACM gadget configured\n");
    } else {
        klogf("<6>A90v6: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v6\n");
        klogf("<6>A90v6: ttyGS0 ready\n");
    } else {
        klogf("<6>A90v6: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v6\n");
        cprintf("\r\nA90 Linux init v6\r\n");
