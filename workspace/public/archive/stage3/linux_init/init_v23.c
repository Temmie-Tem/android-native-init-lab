...
static void kms_fill_color(struct kms_display_state *state, uint32_t color) {
    ...
    for (y = 0; y < state->height; ++y) {
        uint32_t *row = (uint32_t *)((char *)state->map + (y * state->stride));
        size_t x;
...
static void kms_draw_status_overlay(struct kms_display_state *state) {
    ...
    long mem_used_mb = -1;
    long mem_total_mb = -1;
    uint32_t scale = (state->width >= 1080) ? 4 : 3;
    uint32_t x = state->width / 24;
    uint32_t y = state->height / 10;
    uint32_t line_h = scale * 9;

    if (read_long_value("/sys/class/power_supply/battery/capacity", &battery_pct) < 0) {
        battery_pct = -1;
    }
    if (read_long_value("/sys/class/power_supply/battery/temp", &battery_temp_raw) == 0) {
        format_temp_tenths(battery_temp, sizeof(battery_temp), battery_temp_raw * 100);
    }
    if (read_average_thermal_temp("cpu-", "cpuss", &cpu_temp_raw) == 0) {
        format_temp_tenths(cpu_temp, sizeof(cpu_temp), cpu_temp_raw);
    }
    if (read_average_thermal_temp("gpuss", NULL, &gpu_temp_raw) == 0) {
        format_temp_tenths(gpu_temp, sizeof(gpu_temp), gpu_temp_raw);
    }
    ...
    read_first_token("/proc/loadavg", loadavg, sizeof(loadavg));

    snprintf(line1, sizeof(line1), "A90 NATIVE INIT");
    if (battery_pct >= 0) {
        snprintf(line2, sizeof(line2), "BAT %ld%% %s", battery_pct, battery_temp);
    } else {
        snprintf(line2, sizeof(line2), "BAT ?");
    }
    snprintf(line3, sizeof(line3), "CPU %s GPU %s", cpu_temp, gpu_temp);
    if (mem_total_mb >= 0 && mem_used_mb >= 0) {
        snprintf(line4, sizeof(line4), "MEM %ld/%ldMB", mem_used_mb, mem_total_mb);
    } else {
        snprintf(line4, sizeof(line4), "MEM ?");
    }
    snprintf(line5, sizeof(line5), "LOAD %s", loadavg);
    snprintf(line6, sizeof(line6), "VOL-/VOL+ MOVE  PWR SEL");

    kms_draw_text(state, x, y, line1, 0xffffff, scale);
    kms_draw_text(state, x, y + line_h * 2, line2, 0xffffff, scale);
    kms_draw_text(state, x, y + line_h * 4, line3, 0xffffff, scale);
    kms_draw_text(state, x, y + line_h * 6, line4, 0xffffff, scale);
    kms_draw_text(state, x, y + line_h * 8, line5, 0xffffff, scale);
    kms_draw_text(state, x, state->height - (line_h * 3), line6, 0xffffff, scale);
}

static int kms_prepare_buffer(uint32_t color) {
    ...
    if (kms_state.fd >= 0 && kms_state.map != MAP_FAILED) {
        kms_fill_color(&kms_state, color);
        cprintf("kmssolid: updated existing framebuffer %ux%u on crtc=%u\r\n",
                kms_state.width, kms_state.height, kms_state.crtc_id);
        return 0;
    }
    ...
    if (drm_ioctl_retry(fd, DRM_IOCTL_MODE_CREATE_DUMB, &create) < 0) {
        cprintf("kmssolid: CREATE_DUMB failed: %s\r\n", strerror(errno));
        close(fd);
        return -1;
    }
    ...
    addfb.bpp = 32;
    addfb.depth = (preferred_depth > 0 && preferred_depth <= 32) ? (uint32_t)preferred_depth : 24;
    addfb.handle = create.handle;

    if (drm_ioctl_retry(fd, DRM_IOCTL_MODE_ADDFB, &addfb) < 0) {
        cprintf("kmssolid: ADDFB failed: %s\r\n", strerror(errno));
        close(fd);
        return -1;
    }
    ...
    if (drm_ioctl_retry(fd, DRM_IOCTL_MODE_MAP_DUMB, &map_dumb) < 0) {
        cprintf("kmssolid: MAP_DUMB failed: %s\r\n", strerror(errno));
        close(fd);
        return -1;
    }
    ...
    if (map == MAP_FAILED) {
        cprintf("kmssolid: mmap failed: %s\r\n", strerror(errno));
        close(fd);
        return -1;
    }

    connector_list[0] = connector_id;
    memset(&setcrtc, 0, sizeof(setcrtc));
    setcrtc.crtc_id = crtc_id;
    setcrtc.fb_id = addfb.fb_id;
    ...
    kms_state.fb_id = addfb.fb_id;
    kms_state.handle = create.handle;
    ...
    if (drm_ioctl_retry(fd, DRM_IOCTL_MODE_SETCRTC, &setcrtc) < 0) {
        cprintf("kmssolid: SETCRTC failed: %s\r\n", strerror(errno));
        return -1;
    }

    cprintf("kmssolid: %s -> %ux%u on connector=%u crtc=%u\r\n",
...
static void cmd_kmssolid(char **argv, int argc) {
    ...
    kms_prepare_buffer(color);
}

static void cmd_kmsframe(void) {
    if (kms_prepare_buffer(0x080808) == 0) {
        kms_draw_boot_frame(&kms_state);
        cprintf("kmsframe: rendered %ux%u boot frame on crtc=%u\r\n",
                kms_state.width, kms_state.height, kms_state.crtc_id);
    }
}
...
static void cmd_statusscreen(void) {
    if (kms_prepare_buffer(0x080808) == 0) {
        kms_draw_boot_frame(&kms_state);
        kms_draw_status_overlay(&kms_state);
        cprintf("statusscreen: rendered %ux%u status overlay on crtc=%u\r\n",
                kms_state.width, kms_state.height, kms_state.crtc_id);
    }
}

static void boot_auto_frame(void) {
    if (kms_prepare_buffer(0x000000) == 0) {
        kms_draw_boot_frame(&kms_state);
        kms_draw_status_overlay(&kms_state);
        klogf("<6>A90v23: automatic display frame applied\n");
    } else {
        klogf("<6>A90v23: automatic display frame skipped (%d)\n", errno);
    }
}
...
int main(void) {
    setup_base_mounts();
    klogf("<6>A90v23: base mounts ready\n");
    prepare_early_display_environment();
    klogf("<6>A90v23: early display/input nodes prepared\n");

    if (mount_cache() == 0) {
        mark_step("1_cache_ok_v23\n");
        klogf("<6>A90v23: cache mounted\n");
    } else {
        klogf("<6>A90v23: cache mount failed (%d)\n", errno);
    }

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v23\n");
        klogf("<6>A90v23: ACM gadget configured\n");
    } else {
        klogf("<6>A90v23: ACM gadget failed (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v23\n");
        klogf("<6>A90v23: ttyGS0 ready\n");
        boot_auto_frame();
    } else {
        klogf("<6>A90v23: ttyGS0 missing (%d)\n", errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v23\n");
        cprintf("\r\nA90 Linux init v23\r\n");
