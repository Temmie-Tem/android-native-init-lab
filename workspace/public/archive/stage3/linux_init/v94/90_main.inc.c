...
int main(void) {
    ...
    klogf("<6>A90v94: base mounts ready\n");
    ...
    klogf("<6>A90v94: early display/input nodes prepared\n");
    ...
    if (a90_storage_mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v94\n");
        klogf("<6>A90v94: cache mounted\n");
    } else {
    ...
    if (setup_acm_gadget() == 0) {
        ...
        klogf("<6>A90v94: ACM gadget configured\n");
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v94: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_wait_tty() == 0) {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v94: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_attach() == 0) {
        mark_step("4_console_attached_v94\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                ...
                a90_timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v94: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v94: console attach failed (%d)\n", saved_errno);
    }
