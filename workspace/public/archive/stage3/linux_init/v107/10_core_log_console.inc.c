...
enum auto_menu_request {
    AUTO_MENU_REQUEST_NONE = 0,
    AUTO_MENU_REQUEST_HIDE,
    AUTO_MENU_REQUEST_SHOW,
};

static void request_auto_menu_show(void) {
    wf(AUTO_MENU_REQUEST_PATH, "show\n");
}
...
static enum auto_menu_request consume_auto_menu_request(void) {
    char request[32];

    if (read_text_file(AUTO_MENU_REQUEST_PATH, request, sizeof(request)) < 0) {
        return AUTO_MENU_REQUEST_NONE;
    }
    unlink(AUTO_MENU_REQUEST_PATH);
