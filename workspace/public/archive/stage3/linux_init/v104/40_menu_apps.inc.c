/* Included by stage3/linux_init/init_v104.c. Do not compile standalone. */
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v104: boot splash applied\n");
        ...
        klogf("<6>A90v104: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
...
static int draw_screen_changelog_v0830(void) {
    const char *lines[] = {
        "0.9.4 v104 WIFI FEASIBILITY",
        "Adds wififeas gate command",
        "Reports go/no-go/baseline-required",
        "Blocks Wi-Fi bring-up without gates",
        "Keeps feasibility read-only",
        "Preserves service/USB state",
    };

    return draw_screen_about_lines("CHANGELOG / 0.9.4", lines, SCREEN_MENU_COUNT(lines));
}
