...
int main(void) {
    ...
    klogf("<6>A90v86: base mounts ready\n");
    ...
    klogf("<6>A90v86: early display/input nodes prepared\n");
    ...
    if (mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v86\n");
        klogf("<6>A90v86: cache mounted\n");
    } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        a90_log_path());
        klogf("<6>A90v86: cache mount failed (%d)\n", saved_errno);
    }
    boot_auto_frame();
    boot_storage_probe();

    if (setup_acm_gadget() == 0) {
        mark_step("2_gadget_ok_v86\n");
        ...
        klogf("<6>A90v86: ACM gadget configured\n");
    } else {
        int saved_errno = errno;
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v86: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_wait_tty() == 0) {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v86: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_attach() == 0) {
        mark_step("4_console_attached_v86\n");
        ...
        if (netservice_enabled_flag()) {
            ...
            if (net_rc == 0) {
                ...
                a90_timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v86: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v86: console attach failed (%d)\n", saved_errno);
    }
