/* Included by stage3/linux_init/init_v630.c. Do not compile standalone. */
...
#define A90_V630_SIBLING_SSCTL_FLAG "/cache/native-init-sibling-ssctl-v630"
#define A90_V630_SIBLING_SSCTL_LOG "/cache/native-init-sibling-ssctl-v630.log"
#define A90_V630_SIBLING_SSCTL_TIMEOUT_MS 5000

static int v630_append_ssctl_log(const char *fmt, ...) {
    int fd;
    int rc;
    va_list ap;

    fd = open(A90_V630_SIBLING_SSCTL_LOG,
...
static bool v630_sibling_ssctl_flag_armed(void) {
    char state[32];

    if (read_trimmed_text_file(A90_V630_SIBLING_SSCTL_FLAG,
                               state,
                               sizeof(state)) < 0) {
        a90_logf("wifi-v630", "sibling ssctl disabled flag=%s errno=%d error=%s",
                 A90_V630_SIBLING_SSCTL_FLAG,
                 errno,
                 strerror(errno));
        klogf("<6>A90v630: sibling ssctl disabled flag=%s\n",
              A90_V630_SIBLING_SSCTL_FLAG);
        return false;
    }

    if (strcmp(state, "run") != 0) {
        a90_logf("wifi-v630", "sibling ssctl ignored flag=%s state=%.16s",
                 A90_V630_SIBLING_SSCTL_FLAG,
                 state);
        klogf("<6>A90v630: sibling ssctl ignored state=%.16s\n", state);
        return false;
    }

    if (unlink(A90_V630_SIBLING_SSCTL_FLAG) < 0 && errno != ENOENT) {
        a90_logf("wifi-v630", "sibling ssctl flag unlink warning errno=%d error=%s",
                 errno,
                 strerror(errno));
    }
    sync();
...
static int v630_write_sysfs_once(const char *path) {
    int fd;
    int saved_errno;
...
static void v630_sibling_ssctl_child(void) {
    ...
    (void)v630_append_ssctl_log("v630 sibling ssctl child start\n");
    for (index = 0; index < sizeof(nodes) / sizeof(nodes[0]); ++index) {
        int rc = v630_write_sysfs_once(nodes[index]);

        if (rc < 0) {
            ++failures;
            (void)v630_append_ssctl_log("write %s rc=%d errno=%d error=%s\n",
                                       nodes[index],
                                       rc,
                                       -rc,
                                       strerror(-rc));
            klogf("<6>A90v630: sibling ssctl write failed path=%s rc=%d\n",
                  nodes[index],
                  rc);
        } else {
            (void)v630_append_ssctl_log("write %s rc=0\n", nodes[index]);
            klogf("<6>A90v630: sibling ssctl write ok path=%s\n", nodes[index]);
        }
    }
    (void)v630_append_ssctl_log("v630 sibling ssctl child end failures=%d\n", failures);
    _exit(failures == 0 ? 0 : 1);
}

static int v630_wait_child_timeout(pid_t pid, int timeout_ms, int *status_out) {
...
static void v630_run_sibling_ssctl_once(void) {
    pid_t pid;
    int status = 0;
    int rc;

    if (!v630_sibling_ssctl_flag_armed()) {
        return;
    }

    boot_splash_set_line(5, "[ WIFI   ] V630 SSCTL PROOF");
    boot_auto_frame();
    a90_console_printf("# V630 sibling SSCTL proof: armed one-shot.\r\n");
    a90_logf("wifi-v630", "sibling ssctl proof armed timeout_ms=%d",
             A90_V630_SIBLING_SSCTL_TIMEOUT_MS);
    a90_timeline_record(0, 0, "wifi-v630-ssctl", "armed one-shot");
    klogf("<6>A90v630: sibling ssctl proof armed\n");

    pid = fork();
    if (pid < 0) {
        int saved_errno = errno;

        a90_logf("wifi-v630", "sibling ssctl fork failed errno=%d error=%s",
                 saved_errno,
    ...
    if (pid == 0) {
        v630_sibling_ssctl_child();
    }

    rc = v630_wait_child_timeout(pid, A90_V630_SIBLING_SSCTL_TIMEOUT_MS, &status);
    if (rc == 0) {
        a90_console_printf("# V630 sibling SSCTL proof: child status=0x%x.\r\n", status);
        a90_logf("wifi-v630", "sibling ssctl child status=0x%x", status);
        a90_timeline_record(WIFEXITED(status) && WEXITSTATUS(status) == 0 ? 0 : -EIO,
                            WIFEXITED(status) ? WEXITSTATUS(status) : EIO,
                            "wifi-v630-ssctl",
                            "child status=0x%x",
                            status);
        klogf("<6>A90v630: sibling ssctl child status=0x%x\n", status);
    } else {
        int saved_errno = -rc;
        ...
        a90_console_printf("# V630 sibling SSCTL proof: timeout/error rc=%d.\r\n", rc);
        a90_logf("wifi-v630", "sibling ssctl wait failed rc=%d errno=%d error=%s",
                 rc,
                 saved_errno,
                 strerror(saved_errno));
        a90_timeline_record(rc,
                            ...
                            "wifi-v630-ssctl",
                            "wait failed");
        klogf("<6>A90v630: sibling ssctl wait failed rc=%d\n", rc);
    }
}
...
int main(void) {
    ...
    klogf("<6>A90v630: base mounts ready\n");
    ...
    klogf("<6>A90v630: early display/input nodes prepared\n");
    ...
    if (a90_storage_mount_cache() == 0) {
        ...
        mark_step("1_cache_ok_v630\n");
        klogf("<6>A90v630: cache mounted\n");
    } else {
        int saved_errno = errno;
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno),
                        a90_log_path());
        klogf("<6>A90v630: cache mount failed (%d)\n", saved_errno);
    }
    boot_auto_frame();
    a90_storage_probe_boot(&storage_hooks, NULL);
    {
        ...
        if (runtime_rc == 0) {
            ...
            klogf("<6>A90v630: runtime root ready %s\n", a90_runtime_root());
        } else {
            ...
            klogf("<6>A90v630: runtime root warning (%d)\n", runtime_errno);
        }
        boot_auto_frame();
    }
    ...
    if (a90_helper_scan() == 0) {
        ...
        klogf("<6>A90v630: helper inventory ready %s\n", helper_summary);
    } else {
        char helper_summary[96];
        ...
        klogf("<6>A90v630: helper inventory warning %s\n", helper_summary);
    }
    boot_auto_frame();
    ...
    if (a90_userland_scan() == 0) {
        ...
        klogf("<6>A90v630: userland inventory ready %s\n", userland_summary);
    } else {
        char userland_summary[96];
        ...
        klogf("<6>A90v630: userland inventory warning %s\n", userland_summary);
    }
    boot_auto_frame();

    if (a90_usb_gadget_setup_acm() == 0) {
        mark_step("2_gadget_ok_v630\n");
        ...
        klogf("<6>A90v630: ACM gadget configured\n");
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "ACM gadget failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v630: ACM gadget failed (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_wait_tty() == 0) {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        strerror(saved_errno));
        klogf("<6>A90v630: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (a90_console_attach() == 0) {
        mark_step("4_console_attached_v630\n");
        ...
        if (a90_netservice_enabled()) {
            ...
            if (net_rc == 0) {
                ...
                a90_timeline_record(net_rc,
                                ...
                                "start failed: %s",
                                strerror(net_errno));
                klogf("<6>A90v630: netservice failed (%d)\n", net_errno);
            }
        } else {
        ...
        if (rshell_enabled()) {
            ...
            if (rshell_rc == 0) {
                ...
                a90_timeline_record(rshell_rc,
                                ...
                                "start failed: %s",
                                strerror(rshell_errno));
                klogf("<6>A90v630: rshell failed (%d)\n", rshell_errno);
            }
        } else {
        ...
        a90_timeline_record(-saved_errno,
                        ...
                        "attach failed: %s",
                        strerror(saved_errno));
        klogf("<6>A90v630: console attach failed (%d)\n", saved_errno);
    }
