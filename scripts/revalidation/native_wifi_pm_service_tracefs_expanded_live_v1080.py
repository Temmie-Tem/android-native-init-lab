#!/usr/bin/env python3
"""V1080 expanded tracefs-only PM-service PLT collector."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1080-pm-service-tracefs-expanded-live")
DEFAULT_V1079_MANIFEST = Path("tmp/wifi/v1079-pm-service-tracefs-uprobe-live/manifest.json")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1080"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1080/pm-observer.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1080/tracefs-uprobe-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1080/pm-observer-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1080-pm-service-tracefs-expanded-live.txt")
...
EVENT_SPECS = (
    ("elf_entry", "6000"),
    ("libc_init_main_candidate", "7650"),
    ("android_log", "9e60"),
    ("binder_driver", "a0a0"),
    ("binder_service_manager", "a0b0"),
    ("mdmdetect_system_info", "9f40"),
    ("qmi_csi_register", "9fb0"),
    ("qmi_csi_event_loop", "9ff0"),
    ("property_set", "9ec0"),
    ("pipe", "a040"),
    ("access", "a310"),
    ("open", "a2c0"),
...
ALLOWED_V1079_DECISIONS = {
    "v1079-pm-service-tracefs-uprobe-boundary-captured",
}
...
echo tracefs_uprobe_collector=v1080
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1080-pm-service-tracefs-uprobe-plan-ready",
            True,
            "plan-only; no tracefs write, PM actor, BPF attach, or Wi-Fi action executed",
            "run V1080 with explicit allow flags",
        )
    v1079_decision = (manifest.get("v1079") or {}).get("decision")
    if v1079_decision not in ALLOWED_V1079_DECISIONS:
        return (
            "v1080-v1079-predecessor-missing",
            False,
            f"unexpected V1079 decision={v1079_decision!r}",
            "rerun or inspect V1079 before tracefs-only collector",
        )
    missing = required_flags(args)
    if missing:
        return (
            "v1080-pm-service-tracefs-uprobe-approval-required",
            False,
            "missing explicit flags: " + ", ".join(missing),
            "rerun with all V1080 allow flags",
        )
    analysis = manifest.get("analysis") or {}
    tracefs = analysis.get("tracefs_uprobe") or {}
    post = analysis.get("post_surface") or {}
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1080-execns-helper-sha-mismatch", False, "remote execns helper is not v196", "redeploy V1074 helper v196")
    if not analysis.get("pm_binary_visible"):
        return ("v1080-pm-binary-not-visible", False, "read-only vendor mount did not expose pm-service", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1080-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1080-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1080-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1080-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1080-pm-service-tracefs-uprobe-no-hit", False, "no pm-service uprobe trace lines were captured", "verify uprobe offset/file mapping or child runtime")
    return (
        "v1080-pm-service-tracefs-uprobe-boundary-captured",
        True,
        f"entry_hit={tracefs.get('entry_hit')} main_hit={tracefs.get('main_hit')} hit_count={tracefs.get('hit_count')}",
        "classify PM-service callsite hits and choose the next exit-255 root-cause probe",
    )


def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    v1079 = load_json(args.v1079_manifest)
    manifest: dict[str, Any] = {
        "cycle": "v1080",
        "generated_at": now_iso(),
        "command": args.command,
        "v1079": {
            "manifest": str(repo_path(args.v1079_manifest)),
            "decision": v1079.get("decision", ""),
            "pass": bool(v1079.get("pass")),
        },
    ...
    if (
        ...
        and v1079.get("decision") in ALLOWED_V1079_DECISIONS
    ):
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1080 PM Service Tracefs Uprobe Live",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1079-manifest", type=Path, default=DEFAULT_V1079_MANIFEST)
