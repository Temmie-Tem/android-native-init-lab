#!/usr/bin/env python3
"""V1391 bounded Android participant parity + early-observer corrected RC1 enumerate live gate."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1391-android-participant-early-powerup-corrected-rc1-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1391-android-participant-early-powerup-corrected-rc1-live.txt")
PLAN_OUT_DIR = Path("tmp/wifi/v1391-android-participant-early-powerup-corrected-rc1-plan")
PLAN_LATEST_POINTER = Path("tmp/wifi/latest-v1391-android-participant-early-powerup-corrected-rc1-plan.txt")
REPORT_PATH = Path("docs/reports/NATIVE_INIT_V1391_ANDROID_PARTICIPANT_EARLY_POWERUP_CORRECTED_RC1_LIVE_2026-06-01.md")
...
CORRECTED_RC1_FLAG = "--pm-observer-early-powerup-corrected-rc1-enumerate"
CORRECTED_PREFIX = "pm_service_trigger_observer.corrected_rc1_enumerate."
EARLY_PREFIX = "pm_service_trigger_observer.early_powerup_corrected_rc1."
...
def _force_v1391_child_command(original):
...
def configure() -> None:
    ...
    base.CYCLE_LABEL = "v1391"
    base.CYCLE_NAME = "V1391"
    base.SUMMARY_HEADING = "V1391 Android Participant Early-Observer Corrected RC1 Live"
    base.EVIDENCE_FILE_PREFIX = "v1391"
    base._force_response_sampler_child_command = _force_v1391_child_command
...
def _early_summary(raw: dict[str, str]) -> dict[str, Any]:
    return {
        "emitted": bool(raw),
        "begin": _int_value(raw.get("begin"), 0) == 1,
        "end": _int_value(raw.get("end"), 0) == 1,
...
def decide_v1391(manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    ...
    if not corrected.get("emitted") or not corrected.get("end"):
        return (
            "v1391-corrected-rc1-summary-missing",
            False,
            "corrected RC1 enumerate summary did not emit a complete block",
            "inspect helper output and debugfs availability before retry",
        )
    if not early.get("emitted") or not early.get("end"):
        return (
            "v1391-early-powerup-summary-missing",
            False,
            "early-observer corrected RC1 gate summary did not emit a complete block",
            "inspect helper v286 output before retry",
        )
    if not corrected.get("triggered"):
        return (
            "v1391-corrected-rc1-not-triggered",
            False,
    ...
    if not early.get("triggered") or corrected.get("phase") != "early_powerup_observer":
        return (
            "v1391-corrected-rc1-not-early-triggered",
            False,
            f"corrected RC1 enumerate did not fire from the v286 early observer gate: phase={corrected.get('phase')} early_triggered={early.get('triggered')}",
            "repair V1391 early observer ordering before another live retry",
        )
    if _int_value(corrected.get("gate_pm_service_powerup_thread_count"), -1) <= 0:
        return (
            "v1391-corrected-rc1-triggered-without-powerup-thread",
            False,
            f"corrected RC1 enumerate triggered without the v286 powerup-thread gate: gate_pm_service_powerup_thread_count={corrected.get('gate_pm_service_powerup_thread_count')}",
            "stop and inspect corrected RC1 gate evidence before retry",
        )
    if corrected.get("rc_sel_rc") != 0 or corrected.get("case_rc") != 0:
        return (
            "v1391-corrected-rc1-write-failed",
            False,
    ...
    if _timing_downstream_progress(sampler):
        return (
            "v1391-corrected-rc1-downstream-progress",
            True,
            "early-observer corrected RC1 enumerate ran in the Android participant powerup-thread window and downstream GPIO/PCI/MHI/WLFW/wlan0 progress appeared",
    ...
    if bool(sampler.get("timing_pcie_rc1_transition_seen")) or bool(dmesg_rc1.get("transition_seen")):
        ...
        return (
            "v1391-corrected-rc1-ltssm-no-downstream-clean",
            True,
            "early-observer corrected RC1 enumerate fired from the v286 early observer gate"
    ...
    return (
        "v1391-corrected-rc1-no-rc1-transition",
        True,
        "early-observer corrected RC1 enumerate write returned inside the Android participant powerup-thread window, but timing summary saw no RC1/downstream transition",
...
def augment_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    early = _early_summary(_parse_prefixed_lines(run_text, EARLY_PREFIX))
    route = manifest.get("current_route") or {}
    route.update({
    ...
    manifest["early_powerup_corrected_rc1"] = early
    ...
    decision, passed, reason, next_step = decide_v1391(manifest)
...
def _key_rows(manifest: dict[str, Any]) -> list[list[Any]]:
    ...
    return [
        ...
        ["early_triggered", early.get("triggered")],
        ["early_elapsed_ms", early.get("elapsed_ms")],
        ["early_gate_per_mgr_subsys_esoc0_count", early.get("gate_per_mgr_subsys_esoc0_count")],
        ["early_gate_pm_service_powerup_thread_count", early.get("gate_pm_service_powerup_thread_count")],
...
def render_summary(manifest: dict[str, Any]) -> str:
    return "\n".join([
        "# V1391 Android Participant Early-Observer Corrected RC1 Live",
        "",
...
def print_result(manifest: dict[str, Any]) -> None:
    print(f"v1391_decision: {manifest.get('decision')}")
    print(f"v1391_pass: {manifest.get('pass')}")
    print(f"v1391_reason: {manifest.get('reason')}")
    print(f"v1391_next: {manifest.get('next_step')}")
    print(f"v1391_manifest: {_run_dir() / 'manifest.json'}")
