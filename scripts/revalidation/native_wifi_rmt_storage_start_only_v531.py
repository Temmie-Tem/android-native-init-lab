#!/usr/bin/env python3
"""V531 bounded rmt_storage start-only proof.
...
external ping.  The helper v64 namespace supplies the V529-classified
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v531-rmt-storage-start-only")
DEFAULT_HOST = "127.0.0.1"
DEFAULT_PORT = 54321
DEFAULT_TIMEOUT = 45.0
DEFAULT_EXPECT_VERSION = "A90 Linux init 0.9.61 (v319)"
DEFAULT_HELPER = "/cache/bin/a90_android_execns_probe"
DEFAULT_HELPER_SHA256 = "03bf29944d798c285b3557b6be3dee98fa9ae8d3cc30977766a74ae0ad076cd9"
DEFAULT_HELPER_MARKER = "a90_android_execns_probe v64"
...
APPROVAL_PHRASE = (
    "approve v531 rmt-storage start-only proof only; "
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    live = run_step(args, store, "v531-helper-run", helper_command(args), args.max_runtime_sec + 45.0)
    ...
    keys = parse_keys(step_payload([live], "v531-helper-run"))
...
def build_checks(args: argparse.Namespace,
    ...
    add_check(checks, "native-clean", "pass" if args.expect_version in version and "fail=0" in status and "fail=0" in selftest else "blocked", "blocker",
              ...
              "restore native baseline before V531")
    add_check(checks, "helper-v64-ready", "pass" if helper_ready else "blocked", "blocker",
    ...
    add_check(checks, "v490-current-policy-load", "pass" if v490.get("decision") == "v490-selinux-policy-load-proof-pass" and v490.get("policy_load_executed") is True and v490_fresh else "blocked", "blocker",
              ...
              [str(v490.get("path"))],
              "run approved V490 after current boot before V531")
    ...
    add_check(checks, "property-root-present", "pass" if args.property_root in property_root and "No such file" not in property_root else "blocked", "blocker",
              ...
              "refresh private property snapshot from Android before V531")
...
def classify(args: argparse.Namespace,
    ...
    if args.command == "plan":
        return "v531-rmt-storage-start-only-plan-ready", True, "plan-only; no device command executed", "deploy helper v64 and run preflight", False
    blocked = blockers(checks)
    if blocked:
        return "v531-rmt-storage-start-only-blocked", False, "blocked before live run by " + ", ".join(blocked), "resolve blockers before V531 live", False
    if args.command == "preflight":
        return "v531-rmt-storage-start-only-preflight-ready", True, "read-only preflight ready; live run still needs exact approval", "run approved V531 rmt-only proof", False
    if not approved(args):
        return "v531-rmt-storage-start-only-approval-required", True, "exact approval phrase required; no live command executed", "rerun with exact V531 approval", False
    if not live_result:
        return "v531-rmt-storage-start-only-review-required", False, "missing live result", "inspect runner failure", True
    if not live_result["all_postflight_safe"]:
        return "v531-rmt-storage-start-only-cleanup-review", False, "rmt_storage was not proven cleaned", "inspect evidence and consider recovery reboot", True
    helper_result = live_result["helper_result"]
    if helper_result == "rmt-window-pass":
        return "v531-rmt-storage-window-pass", True, "rmt_storage stayed observable through the bounded window and cleaned up", "advance to companion replay with v64 surfaces", True
    if helper_result == "start-only-runtime-gap":
        return "v531-rmt-storage-runtime-gap", True, "rmt_storage still exited before the observe window", "inspect stderr/context keys; add only the next missing surface", True
    return "v531-rmt-storage-review", True, f"helper_result={helper_result} dmesg_counts={dmesg.get('counts')}", "inspect V531 transcript", True
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V531 rmt_storage Start-Only Proof",
        "",
