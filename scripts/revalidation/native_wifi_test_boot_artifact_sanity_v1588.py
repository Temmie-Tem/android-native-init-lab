#!/usr/bin/env python3
"""V1588 local-only sanity verifier for the service-window lower-marker test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1588-service-window-lower-marker-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1588_SERVICE_WINDOW_LOWER_MARKER_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1588-service-window-lower-marker-test-boot-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v293"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.101 (v1588-service-window-lower-marker)",
    "a90_android_execns_probe v293",
    "A90v1588",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "helper_result": "/cache/native-init-wifi-test-boot-v1588-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            ...
            True,
            "V1588 lower-marker service-window test boot artifact passed local sanity",
        )
    return (
        "v1588-service-window-lower-marker-artifact-sanity-blocked",
        False,
        "V1588 artifact sanity failed; fix local artifact before any live handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1588 manifest: `{manifest['_path']}`",
        f"- V1588 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
