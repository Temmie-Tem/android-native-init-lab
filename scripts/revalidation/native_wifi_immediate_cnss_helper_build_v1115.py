#!/usr/bin/env python3
"""V1115 source/build-only gate for immediate CNSS after per_mgr start."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1115-execns-helper-v210-build")
...
DEFAULT_V1114_MANIFEST = Path("tmp/wifi/v1114-pm-service-lifetime-classifier/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1115-execns-helper-v210-build.txt")
HELPER_MARKER = "a90_android_execns_probe v210"
EXPECTED_V1114_DECISION = "v1114-select-immediate-cnss-after-per-mgr-start-gate"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v210"',
    "pm_observer_start_cnss_immediate_after_per_mgr",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1114 = load_json(args.v1114_manifest)
    analysis: dict[str, Any] = {
        "v1114": {
            "decision": v1114.get("decision", ""),
            "pass": bool(v1114.get("pass")),
            "path": str(repo_path(args.v1114_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1114-input",
            "status": "pass" if analysis["v1114"]["decision"] == EXPECTED_V1114_DECISION and analysis["v1114"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1114']['decision']} pass={analysis['v1114']['pass']}",
            "next_step": "refresh V1114 before building helper v210",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source immediate-CNSS support",
        },
    ]
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1115 build-only gate"})
        return rows
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1115-immediate-cnss-helper-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1115 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1115-immediate-cnss-helper-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1115-immediate-cnss-helper-build-pass",
        True,
        f"helper v210 built sha256={analysis['build']['output_sha256']}",
        "V1116 should deploy helper v210 and run the combined global-holder immediate-CNSS live gate",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1115 Immediate CNSS Helper Build",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1114-manifest", type=Path, default=DEFAULT_V1114_MANIFEST)
