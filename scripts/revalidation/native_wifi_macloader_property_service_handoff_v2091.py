#!/usr/bin/env python3
"""V2091 rollbackable handoff for the bounded macloader property-service ACK discriminator."""
...
OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v2091-macloader-property-service-handoff"
HANDOFF_DIR = OUT_DIR / "v2090-handoff"
HANDOFF_REPORT = OUT_DIR / "v2090-handoff-report.md"
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2091_MACLOADER_PROPERTY_SERVICE_HANDOFF_2026-06-05.md"
)
V2090_OUT = REPO_ROOT / "tmp" / "wifi" / "v2090-macloader-property-service-test-boot"
V2090_INIT = V2090_OUT / "init_v2090_macloader_property_service"
V2090_BOOT = V2090_OUT / "boot_linux_v2090_macloader_property_service.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2090/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.224 (v2090-macloader-property-service)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2090.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2090.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2090-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v408"
...
def collect_property_service_shim(fields: dict[str, str]) -> dict[str, Any]:
    prefix = "wifi_hal_composite_start.property_service_shim"

    def field_int(name: str) -> int | str:
        key = f"{prefix}.{name}"
    ...
    for index in range(1, 17):
        request_prefix = f"{prefix}.request.{index}"
        if f"{request_prefix}.name" not in fields:
            continue
        requests.append({
            "index": index,
            "cmd": fields.get(f"{request_prefix}.cmd", ""),
            "name": fields.get(f"{request_prefix}.name", ""),
            "value": fields.get(f"{request_prefix}.value", ""),
            "allowed": intish(fields.get(f"{request_prefix}.allowed")),
            "result": fields.get(f"{request_prefix}.result", ""),
        })

    macloader_ack_requests = [
        request for request in requests
        if request["name"] in {
            "vendor.wifi.dualconcurrent.interface",
            "ro.vendor.wifi.sap.interface",
        }
        and request["value"] == "swlan0"
        and intish(request["allowed"]) == 1
        and request["result"] == "0x00000000"
    ]
    return {
        "started": field_int("started"),
        "pid": field_int("pid"),
        "child_started": field_int("child_started"),
        "macloader_interface_props_allowed": field_int("macloader_interface_props_allowed"),
        "request_count": field_int("request_count"),
        "term_sent": field_int("term_sent"),
        "kill_sent": field_int("kill_sent"),
        "reaped": field_int("reaped"),
        "postflight_safe": field_int("postflight_safe"),
        "requests": requests,
        "macloader_ack_count": len(macloader_ack_requests),
    }
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    boot_required = (
        ...
        "wifi_hal_composite_start.property_service_shim.macloader_interface_props_allowed=%d",
        "vendor.wifi.dualconcurrent.interface",
        "ro.vendor.wifi.sap.interface",
        "swlan0",
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2090_INIT, init_required), (V2090_BOOT, boot_required)):
        key = rel(path)
        forbidden_tokens = init_forbidden if path == V2090_INIT else boot_forbidden
...
def collect_details(handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    details["property_service_shim"] = collect_property_service_shim(fields)
    return details
...
def classify(handoff: dict[str, Any],
    ...
    v2091_rollback_ok = step_ok.get("post-selftest", False) and step_ok.get("post-status", False)

    if not hook_ok:
        label = "macloader-trace-artifact-hook-regression"
        passed = False
        reason = "V2090 artifact does not contain the bounded macloader property-service ACK discriminator contract"
    ...
    return {
        **base,
        "decision": f"v2091-{label}-rollback-{'pass' if passed else 'blocked'}",
        "label": label,
        "pass": passed,
        "reason": reason,
        "hook_ok": hook_ok,
        ...
        "v2091_rollback_ok": v2091_rollback_ok,
    }
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    mac_source = details.get("mac_source_bridge", {}) if isinstance(details.get("mac_source_bridge"), dict) else {}
    trace = details.get("macloader_syscall_trace", {}) if isinstance(details.get("macloader_syscall_trace"), dict) else {}
    property_shim = details.get("property_service_shim", {}) if isinstance(details.get("property_service_shim"), dict) else {}
    ...
    property_requests = property_shim.get("requests", []) if isinstance(property_shim.get("requests"), list) else []
    property_request_lines = [
        f"- `{item.get('index')}` `{item.get('name')}`=`{item.get('value')}` allowed `{item.get('allowed')}` result `{item.get('result')}`"
    ...
    return "\n".join([
        "# Native Init V2091 Macloader Property Service Handoff",
        "",
        "## Summary",
        "",
        "- Cycle: `V2091`",
        ...
        markdown_table(
            ...
            [
                ...
                ["property_shim", property_shim.get("started"), f"child={property_shim.get('child_started')} macloader_ack_enabled={property_shim.get('macloader_interface_props_allowed')} requests={property_shim.get('request_count')} macloader_ack={property_shim.get('macloader_ack_count')}"],
                ...
                ["rollback", classification.get("v2091_rollback_ok"), "post-selftest and post-status succeeded after rollback"],
            ],
        ),
        "",
        "## Namespace Proof",
        ...
        *([] if error_lines else ["- `none`"]),
        ...
        markdown_table(
            ["field", "value"],
            [
                ["started", property_shim.get("started", "")],
                ["pid", property_shim.get("pid", "")],
                ["child_started", property_shim.get("child_started", "")],
                ["macloader_interface_props_allowed", property_shim.get("macloader_interface_props_allowed", "")],
                ["request_count", property_shim.get("request_count", "")],
                ["macloader_ack_count", property_shim.get("macloader_ack_count", "")],
                ["postflight_safe", property_shim.get("postflight_safe", "")],
            ],
        ),
        "",
        *property_request_lines,
        *([] if property_request_lines else ["- `none`"]),
        "",
        "## Interpretation",
        "",
        ...
        *([] if step_lines else ["- `none`"]),
        ...
        "- Mutation scope: `/cache` one-shot clean-DSP flag, V2090 test-boot flash-handoff, namespace-local RFS bridges/tmpfs mirrors, read-only EFS/persist mounts for `macloader`, `/sys/wifi` and `/sys/kernel/boot_wlan` exposure, private tmp-root `/dev/socket/logdw`, tracefs uprobes, Android-parity `macloader` driver-start action, compile-gated private property-service ACK for two macloader interface properties, single-child redacted `macloader` ptrace, and rollback to `stage3/boot_linux_v724.img` with selftest fail=0.",
        "",
    ])
...
def configure_prev2083() -> None:
    ...
    prev2083.V2082_OUT = V2090_OUT
    prev2083.V2082_INIT = V2090_INIT
    prev2083.V2082_BOOT = V2090_BOOT
