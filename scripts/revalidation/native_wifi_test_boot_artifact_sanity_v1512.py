#!/usr/bin/env python3
"""V1512 local-only sanity verifier for the V1511 source-timestamped Wi-Fi test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1512-wifi-source-timestamped-pre-l0-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1512_WIFI_SOURCE_TIMESTAMPED_PRE_L0_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1511-wifi-source-timestamped-pre-l0-test-boot-source-build-pass"
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1511-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1511-source-timestamped-pre-l0.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1512-wifi-source-timestamped-pre-l0-artifact-sanity-pass",
            True,
            "V1511 source-timestamped pre-L0 test boot artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1512-wifi-source-timestamped-pre-l0-artifact-sanity-blocked",
        False,
        "V1511 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1511 manifest: `{manifest['_path']}`",
        f"- V1511 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- source-timestamped pre-L0 contract: `{checks['wifi_test_contract']['ok']}`",
