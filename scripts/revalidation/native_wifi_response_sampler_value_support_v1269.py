#!/usr/bin/env python3
"""V1269 source/build-only gate for AP2MDM value/power sampler support."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1269-execns-helper-v265-build")
DEFAULT_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v265")
DEFAULT_BUILD_SCRIPT = Path("scripts/revalidation/build_android_execns_probe_helper.sh")
DEFAULT_V1268 = Path("tmp/wifi/v1268-ap2mdm-value-observer-classifier/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1269-execns-helper-v265-build.txt")
HELPER_MARKER = "a90_android_execns_probe v265"
EXPECTED_V1268_DECISION = "v1268-ap2mdm-value-observer-selected"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v265"',
    "read_debugfs_gpio_line",
    "read_tlmm_pinconf_line",
    "read_pmic_pinconf_line",
    "pmic_gpio1270_debugfs_seen=%d",
    "tlmm_gpio135_debugfs_seen=%d",
    "tlmm_gpio142_debugfs_seen=%d",
    "pmic9_pinconf_seen=%d",
    "pin135_pinconf_seen=%d",
    "pin142_pinconf_seen=%d",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1268-manifest", type=Path, default=DEFAULT_V1268)
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1268 = load_json(args.v1268_manifest)
    build: dict[str, Any] = {}
    if args.command == "run":
        logs = store.mkdir("logs")
    ...
    return {
        "v1268": {
            "decision": v1268.get("decision", ""),
            "pass": bool(v1268.get("pass")),
            "path": str(repo_path(args.v1268_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(command: str, analysis: dict[str, Any]) -> list[dict[str, str]]:
    rows = [
        {
            "name": "v1268-input",
            "status": "pass" if analysis["v1268"]["decision"] == EXPECTED_V1268_DECISION and analysis["v1268"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1268']['decision']} pass={analysis['v1268']['pass']}",
            "next_step": "rerun V1268 before helper build",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source value/power sampler support",
        },
    ]
    if command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1269 build-only gate"})
        return rows
...
def decide(command: str, rows: list[dict[str, str]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if command == "plan":
        return ("v1269-value-power-sampler-build-plan-ready", True, "plan-only", "run V1269 source/build-only gate")
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1269-value-power-sampler-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1269-value-power-sampler-build-pass",
        True,
        f"helper v265 built sha256={analysis['build']['stage3']['sha256']}",
        "V1270 should deploy helper v265 only; V1271 should run the bounded value/power observer",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1269 AP2MDM Value/Power Sampler Build",
        "",
