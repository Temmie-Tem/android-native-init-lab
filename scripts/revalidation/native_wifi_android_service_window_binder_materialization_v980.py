#!/usr/bin/env python3
"""V980 source/build verifier for Android service-window binder device materialization."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v980-android-service-window-binder-materialization")
LATEST_POINTER = Path("tmp/wifi/latest-v980-android-service-window-binder-materialization.txt")
DEFAULT_HELPER_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_BUILD_ARTIFACT = Path("tmp/wifi/v980-execns-helper-v165-build/a90_android_execns_probe")
DEFAULT_BUILD_LOG = Path("tmp/wifi/v980-execns-helper-v165-build/build.log")
...
def function_body(source: str, name: str) -> str:
    match = re.search(rf"\nstatic int {re.escape(name)}\([^)]*\)[\s\S]*? \{{", source)
    if not match:
        return ""
    start = match.start() + 1
...
def classify(source: str, build_rc: int, build_log: str, artifact: Path) -> dict[str, Any]:
    materialize_fn = function_body(source, "materialize_service_manager_binder_devices")
    strings_output = artifact_strings(artifact)
    checks = {
        "execns_version_v165": 'EXECNS_VERSION "a90_android_execns_probe v165"' in source,
        "binder_materialization_has_android_service_window_condition": (
        ...
        "binder_nodes_still_materialized": all(
            token in materialize_fn
            for token in (
        ...
        "strings_confirm_version": "a90_android_execns_probe v165" in strings_output,
    ...
    return {
        ...
        if passed
        else "v980-android-service-window-binder-materialization-incomplete",
        "pass": passed,
        "reason": (
            "helper v165 materializes private binder, hwbinder, and vndbinder devices for the dedicated Android service-window mode"
            if passed
        ...
        "next_step": (
            ...
            if passed
            else "repair binder materialization coverage before deploy"
        ),
        "checks": checks,
    }
