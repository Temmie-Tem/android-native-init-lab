#!/usr/bin/env python3
"""V438 Android Wi-Fi re-enable observation handoff.

This wrapper temporarily boots Android, waits for `sys.boot_completed=1`, runs
the bounded V438 Wi-Fi re-enable observation, then restores the native init boot
image.  The only Wi-Fi mutation allowed by the nested collector is
`cmd wifi set-wifi-enabled enabled`; scan/connect, credentials, server exposure,
external probes, routing mutation, rfkill/sysfs writes, and daemon starts remain
blocked.
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v438-android-wifi-reenable-observation-handoff")
...
def v438_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v438-android-wifi-reenable-observation-run"


def enable_approval_ok(args: argparse.Namespace) -> tuple[bool, list[str]]:
    missing: list[str] = []
...
def v438_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    base = build_v424_step_plan(args, store, android_image, native_image)
    plan: list[tuple[str, list[str] | str, int]] = []
    collector_timeout = max(args.timeout, int(args.enable_settle_sleep + 300))
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            if args.serial:
                v438_command.extend(["--serial", args.serial])
            ...
            v438_command.append("run")
            plan.append(("v438-android-wifi-reenable-observation", v438_command, collector_timeout))
        else:
...
def load_v438_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v438_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v438 = load_v438_result(store)
    classification = v438.get("classification") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v438-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass"
    elif not v438.get("pass"):
        decision = str(v438.get("decision") or "v438-handoff-reenable-failed-rollback-complete")
        pass_ok = False
        reason = str(v438.get("reason") or "V438 re-enable observation did not pass")
    else:
        decision = str(v438.get("decision") or "v438-handoff-reenable-pass")
        pass_ok = True
        reason = str(v438.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "state": {
            ...
            "pre_contained": classification.get("pre_contained"),
            "post_exposure": classification.get("post_exposure"),
            "listener_safe": classification.get("listener_safe"),
            "next_gate": classification.get("next_gate"),
        },
        "v438": {
            "path": v438.get("_path"),
            "decision": v438.get("decision"),
            "pass": v438.get("pass"),
            "reason": v438.get("reason"),
        },
    }
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    enable_ok, enable_missing = enable_approval_ok(args)
    context: dict[str, Any] = {
        ...
        "enable_approval_ok": enable_ok,
        "missing_enable_approval_flags": enable_missing,
        "v438_out_dir": str(v438_out_dir(store)),
    ...
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v438-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v438-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v438-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v438-handoff-approval-required", False
    if args.command == "run" and not enable_ok:
        return [], context, "v438-handoff-enable-approval-required", False

    plan = v438_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_plan_command(plan)
    if offenders:
        context["forbidden_plan_commands"] = offenders
        return [], context, "v438-handoff-forbidden-command-blocked", False

    steps: list[StepResult] = []
    if args.command == "plan":
        for name, command, _ in plan:
            steps.append(write_step(store, name, command, "[plan] not executed\n", "", 0, 0.0, skipped=True, ok_override=True))
        return steps, context, "v438-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v438-android-wifi-reenable-observation"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v438-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v438-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v438-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v438-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v438-android-wifi-reenable-observation":
            v438_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v438-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v438_step_ok:
            return steps, context, "v438-handoff-reenable-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v438-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v438-handoff-plan-ready": "execution plan generated without device mutation",
        "v438-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v438-handoff-approval-required": "live run refused because boot/rollback approval flags are missing",
        "v438-handoff-enable-approval-required": "live run refused because Wi-Fi enable approval flags are missing",
        "v438-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v438-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v438-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v438-handoff-forbidden-command-blocked": "handoff plan contains a forbidden Wi-Fi/server/probe command",
        "v438-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v438-handoff-reenable-failed-rollback-complete": "V438 re-enable observation failed, and native rollback completed",
        "v438-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v438-handoff-readback-failed-rollback-attempted": "Android boot readback failed and native rollback was attempted from recovery",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    for key in ("pre_contained", "post_exposure", "listener_safe", "next_gate"):
        flat_rows.append([key, str(state.get(key, "-"))])
    return "\n".join(
        [
            "# V438 Boot-complete Android Wi-Fi Re-enable Observation Handoff",
            "",
            ...
            f"- enable_approval_ok: `{manifest['context']['enable_approval_ok']}`",
            f"- missing_approval_flags: `{', '.join(manifest['context']['missing_approval_flags']) or '-'}`",
            f"- missing_enable_approval_flags: `{', '.join(manifest['context']['missing_enable_approval_flags']) or '-'}`",
            f"- v438_out_dir: `{manifest['context']['v438_out_dir']}`",
            ...
            f"- v438_decision: `{(comparison.get('v438') or {}).get('decision', '-')}`",
            "",
            ...
            "## Guardrails",
            "",
            "- `run` requires boot/rollback approval flags and Wi-Fi enable approval flags.",
            "- The only allowed Wi-Fi mutation is `cmd wifi set-wifi-enabled enabled`.",
...
def main() -> int:
    ...
    wifi_enable_executed = any(
        step.name == "v438-android-wifi-reenable-observation" and not step.skipped
        for step in steps
    )
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v438-handoff-approval-required",
            "v438-handoff-enable-approval-required",
            "v438-handoff-missing-native-rollback",
            "v438-handoff-missing-android-boot",
            "v438-handoff-image-collision",
            "v438-handoff-forbidden-command-blocked",
        },
        "wifi_enable_executed": wifi_enable_executed,
        "wifi_bringup_executed": wifi_enable_executed,
    }
