#!/usr/bin/env python3
"""V532 bounded rmt_storage start-only proof.
...
external ping.  The helper v65 namespace supplies the V529-classified
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v532-rmt-storage-start-only")
DEFAULT_HOST = "127.0.0.1"
DEFAULT_PORT = 54321
DEFAULT_TIMEOUT = 45.0
DEFAULT_EXPECT_VERSION = "A90 Linux init 0.9.61 (v319)"
DEFAULT_HELPER = "/cache/bin/a90_android_execns_probe"
DEFAULT_HELPER_SHA256 = "5c6769cf65757d38622f4387ebdb53eaca41a1c3234754c8a4d6c2f2f59dee2e"
DEFAULT_HELPER_MARKER = "a90_android_execns_probe v65"
...
APPROVAL_PHRASE = (
    "approve v532 rmt-storage start-only proof only; "
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    live = run_step(args, store, "v532-helper-run", helper_command(args), args.max_runtime_sec + 45.0)
    ...
    keys = parse_keys(step_payload([live], "v532-helper-run"))
...
def build_checks(args: argparse.Namespace,
    ...
    add_check(checks, "native-clean", "pass" if args.expect_version in version and "fail=0" in status and "fail=0" in selftest else "blocked", "blocker",
              ...
              "restore native baseline before V532")
    add_check(checks, "helper-v65-ready", "pass" if helper_ready else "blocked", "blocker",
    ...
    add_check(checks, "v490-current-policy-load", "pass" if v490.get("decision") == "v490-selinux-policy-load-proof-pass" and v490.get("policy_load_executed") is True and v490_fresh else "blocked", "blocker",
              ...
              [str(v490.get("path"))],
              "run approved V490 after current boot before V532")
    ...
    add_check(checks, "property-root-present", "pass" if args.property_root in property_root and "No such file" not in property_root else "blocked", "blocker",
              ...
              "refresh private property snapshot from Android before V532")
...
def classify(args: argparse.Namespace,
    ...
    if args.command == "plan":
        return "v532-rmt-storage-start-only-plan-ready", True, "plan-only; no device command executed", "deploy helper v65 and run preflight", False
    blocked = blockers(checks)
    if blocked:
        return "v532-rmt-storage-start-only-blocked", False, "blocked before live run by " + ", ".join(blocked), "resolve blockers before V532 live", False
    if args.command == "preflight":
        return "v532-rmt-storage-start-only-preflight-ready", True, "read-only preflight ready; live run still needs exact approval", "run approved V532 rmt-only proof", False
    if not approved(args):
        return "v532-rmt-storage-start-only-approval-required", True, "exact approval phrase required; no live command executed", "rerun with exact V532 approval", False
    if not live_result:
        return "v532-rmt-storage-start-only-review-required", False, "missing live result", "inspect runner failure", True
    if not live_result["all_postflight_safe"]:
        return "v532-rmt-storage-start-only-cleanup-review", False, "rmt_storage was not proven cleaned", "inspect evidence and consider recovery reboot", True
    helper_result = live_result["helper_result"]
    if helper_result == "rmt-window-pass":
        return "v532-rmt-storage-window-pass", True, "rmt_storage stayed observable through the bounded window and cleaned up", "advance to companion replay with v65 surfaces", True
    if helper_result == "start-only-runtime-gap":
        return "v532-rmt-storage-runtime-gap", True, "rmt_storage still exited before the observe window", "inspect stderr/context keys; add only the next missing surface", True
    return "v532-rmt-storage-review", True, f"helper_result={helper_result} dmesg_counts={dmesg.get('counts')}", "inspect V532 transcript", True
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V532 rmt_storage Start-Only Proof",
        "",
