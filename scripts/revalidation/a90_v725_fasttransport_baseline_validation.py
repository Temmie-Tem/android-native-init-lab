...
def a90ctl(command: list[str], *, timeout: float = 20.0, bridge_timeout: float = 10.0) -> list[object]:
    return ["python3", "scripts/revalidation/a90ctl.py", "--timeout", str(bridge_timeout), *command]
...
def run_transport_smoke(store: EvidenceStore,
    ...
    command: list[object] = [
        "python3",
        "scripts/revalidation/a90_ncm_transport_smoke.py",
        "--label",
        label,
        "--sizes-mib",
        sizes_mib,
...
def main() -> int:
    ...
    safe_label = "".join(ch if ch.isalnum() or ch in "._-" else "-" for ch in args.label).strip(".-") or "baseline"
    out_dir = DEFAULT_OUT_BASE / f"v725-fasttransport-baseline-validation-{safe_label}-{now_label()}"
