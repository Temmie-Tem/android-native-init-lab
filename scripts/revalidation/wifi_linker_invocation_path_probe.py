#!/usr/bin/env python3
"""Run v235 Android linker invocation-path comparison.
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v235-linker-invocation-path")
...
TARGET_PROFILES = ("system-toybox", "system-sh", "linker64-self", "apex-linker64-self", "cnss-daemon")
...
def decide(results: list[MatrixResult]) -> tuple[str, str, bool]:
    ...
    by_linker = {linker: [item for item in clean if item.linker_profile == linker] for linker in LINKER_PROFILES}
    system_clean = by_linker.get("system-linker", [])
    apex_clean = by_linker.get("apex-linker", [])
    system_crashes = bool(system_clean) and all(item.child_signal != 0 for item in system_clean)
    apex_crashes = bool(apex_clean) and all(item.child_signal != 0 for item in apex_clean)
    system_pass = any(item.child_signal == 0 and item.child_exit_code == 0 for item in system_clean)
    apex_pass = any(item.child_signal == 0 and item.child_exit_code == 0 for item in apex_clean)

    if apex_pass and system_crashes:
        return "android-linker-apex-direct-pass", "direct APEX linker passed while /system/bin/linker64 path crashed", True
    if system_pass and apex_crashes:
        return "android-linker-system-symlink-pass", "/system/bin/linker64 passed while direct APEX linker path crashed", True
    if system_crashes and apex_crashes:
        signals = sorted({item.child_signal for item in clean})
        return "android-linker-crash-path-independent", f"both linker invocation paths crashed signals={signals}", True
    if system_pass or apex_pass:
        return "android-linker-list-baseline-pass", "at least one clean linker invocation path exited 0", True
    return "android-linker-path-manual-review-required", "matrix completed but linker path comparison is inconclusive", True


def build_summary(manifest: dict[str, Any]) -> str:
    lines = [
        "# v235 Linker Invocation Path Probe",
        "",
        ...
        "## Matrix",
        "",
        "| env | linker | profile | decision | signal | exit | stdout | stderr |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: |",
    ]
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.subcommand == "plan":
        ...
        decision, reason, pass_ok = "android-linker-path-plan-ready", "plan generated; no helper matrix executed", True
    else:
