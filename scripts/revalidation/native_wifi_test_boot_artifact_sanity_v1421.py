#!/usr/bin/env python3
"""V1421 local-only sanity verifier for the V1420 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1420-wifi-test-boot-rc1-window-sampler" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1421-wifi-test-boot-rc1-window-sampler-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1421_WIFI_TEST_BOOT_RC1_WINDOW_SAMPLER_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1420-wifi-test-boot-rc1-window-sampler-source-build-pass"
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1421-wifi-test-boot-rc1-window-sampler-artifact-sanity-pass",
            True,
            "V1420 RC1-window sampler test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1421-wifi-test-boot-rc1-window-sampler-artifact-sanity-blocked",
        False,
        "V1420 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1420 manifest: `{manifest['_path']}`",
        f"- V1420 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- Wi-Fi test RC1-window contract: `{checks['wifi_test_contract']['ok']}`",
        f"- RC1 watcher delay ms: `{checks['wifi_test_contract']['rc1_watcher_delay_ms']}`",
        f"- RC1 window sampler: `{checks['wifi_test_contract']['rc1_window_sampler']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1420-rc1-watcher.result"
                and wifi_test.get("rc1_window_sampler") is True
                and wifi_test.get("rc1_window_result") == "/cache/native-init-wifi-test-boot-v1420-rc1-window.result"
            ),
        },
    }
