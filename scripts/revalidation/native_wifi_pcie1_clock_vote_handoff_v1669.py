#!/usr/bin/env python3
"""V1669 rollbackable live retry for the V1668 pcie1 clock vote wait-window repair."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1669-pcie1-clock-vote-wait-retry-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1669_PCIE1_CLOCK_VOTE_WAIT_RETRY_HANDOFF_2026-06-02.md"
)
DEFAULT_V1668_MANIFEST = (
    REPO_ROOT
    / "tmp"
    / "wifi"
    / "v1668-pcie1-clock-vote-wait-repair-test-boot"
    / "manifest.json"
)
DEFAULT_TEST_IMAGE = (
    REPO_ROOT
    ...
    / "wifi"
    / "v1668-pcie1-clock-vote-wait-repair-test-boot"
    / "boot_linux_v1668_pcie1_clock_vote_wait.img"
)
TEST_EXPECT_VERSION = "A90 Linux init 0.9.118 (v1668-pcie1-clock-vote-wait)"
DEFAULT_TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v1668.log"
DEFAULT_TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v1668.summary"
DEFAULT_TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1668-helper.result"
DEFAULT_TEST_WATCHER_PATH = "/cache/native-init-wifi-test-boot-v1668-natural-watcher.result"
DEFAULT_TEST_WINDOW_PATH = "/cache/native-init-wifi-test-boot-v1668-clock-vote-window.result"
DEFAULT_TEST_CLOCK_VOTE_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1668-pcie1-clock-vote-wait.result"
DEFAULT_DMESG_PATTERN = (
    "A90v1668|pcie1 clock vote|subsystem_get|mdm_subsys_powerup|"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1668-manifest", type=Path, default=DEFAULT_V1668_MANIFEST)
...
def main() -> int:
    ...
    if not handoff:
        print(json.dumps({
            "decision": "v1669-pcie1-clock-vote-wait-retry-handoff-manifest-missing",
            "pass": False,
            "base_rc": base_rc,
        }, indent=2))
        return 1
