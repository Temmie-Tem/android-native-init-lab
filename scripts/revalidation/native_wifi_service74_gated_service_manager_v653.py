#!/usr/bin/env python3
"""V653 service-74 gated service-manager proof.

This proof reuses the V644 clean-DSP preflight and helper v105's
service-74 gated service-manager mode. It does not write DSP boot nodes,
...
base.DEFAULT_OUT_DIR = base.Path("tmp/wifi/v653-service74-gated-service-manager")
base.DEFAULT_EXPECT_VERSION = "A90 Linux init 0.9.67 (v641)"
base.DEFAULT_HELPER_SHA256 = "8e712fab67de9e4e330e2c9ac2ab2f3328fe4b08fbad02b7279977ba6db76117"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v105"
base.DEFAULT_V490_MANIFEST = base.Path("tmp/wifi/v653-v490-current-run/manifest.json")
base.APPROVAL_PHRASE = (
    "approve v653 service74 gated service-manager proof only; "
...
SERVICE74_GATED_MODE = "wifi-companion-service74-gated-vnd-service-manager-start-only"
EXPECTED_ORDER = (
...
V653_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
...
def v653_counts(text: str) -> dict[str, int]:
    return {name: _count(pattern, text) for name, pattern in V653_PATTERNS}
...
def build_checks(args: base.argparse.Namespace,
    ...
    base.add_check(
        ...
        "restore Android-captured ld.config.txt before V653",
    )
    base.add_check(
        checks,
        ...
        "restore Android-captured apex.libraries.config.txt before V653",
    )
    return checks
...
def companion_command(args: base.argparse.Namespace) -> list[str]:
    ...
    _set_option(command, "--mode", SERVICE74_GATED_MODE)
    ...
    if len(command) > MAX_CMDV1_COMMAND_ARGS:
        raise RuntimeError(f"V653 helper command has {len(command)} args; max safe args={MAX_CMDV1_COMMAND_ARGS}")
    return command
...
def run_live(args: base.argparse.Namespace,
    ...
    result["v653_counts"] = v653_counts(str(result.get("dmesg_delta") or ""))
    return result
...
def decide(args: base.argparse.Namespace,
    ...
    if args.command == "plan":
        return (
            "v653-service74-gated-service-manager-plan-ready",
            True,
            "plan-only; no device command executed",
            "refresh current-boot V490/runtime prerequisites, then run V653 preflight",
            False,
        )
    blocked = base.blockers(checks)
    if blocked:
        return "v653-service74-gated-service-manager-blocked", False, "blocked by " + ", ".join(blocked), "resolve blockers before V653", False
    if args.command == "preflight":
        return (
            "v653-service74-gated-service-manager-preflight-ready",
            True,
    ...
    if not base.approved(args):
        return "v653-service74-gated-service-manager-approval-required", True, "exact approval phrase required; no live command executed", "rerun with exact V653 approval", False
    if not live:
        return "v653-service74-gated-service-manager-review-required", False, "missing live result", "inspect runner failure", True

    reboot = live.get("reboot_cleanup") or {}
    if not reboot.get("version_seen") or not reboot.get("status_healthy"):
        return "v653-cleanup-review", False, f"reboot_cleanup={reboot}", "verify device manually before continuing", True
    if not live.get("holder_started") or not (live.get("qrtr_rx_wait") or {}).get("seen"):
        return "v653-lower-modem-blocked", False, "subsys_modem holder did not reproduce QRTR RX", "restore lower modem readiness before V653 retry", True
    if not live.get("companion_executed") or not live.get("all_postflight_safe"):
        return "v653-cleanup-review", False, f"helper_result={live.get('helper_result')} all_postflight_safe={live.get('all_postflight_safe')}", "inspect companion transcript before retry", True

    service_manager = live.get("service_manager") or {}
    service74_gate = service_manager.get("service74_gate") or {}
    counts = live.get("v653_counts") or {}
    service_manager_executed = (
        ...
        and service_manager.get("service_manager_started") == "1"
    )
    if service74_gate.get("seen") != "1" and service_manager.get("service_manager_started") != "1":
        return (
            "v653-service74-gate-timeout",
            True,
            (
            ...
            "service-manager was correctly withheld; classify why lower CNSS path no longer publishes service74 before retry",
            True,
        )
    if not service_manager_executed:
        return "v653-service-manager-not-executed", False, f"service_manager={service_manager}", "inspect helper mode and approval propagation", True
    if _int_count(counts.get("service_notifier_74")) == 0:
        return (
            "v653-service74-still-missing-after-gate-open",
            True,
            f"service_notifier_180={counts.get('service_notifier_180', 0)} service_notifier_74=0",
            "inspect gate/klog mismatch before any wider service-manager or Wi-Fi HAL retry",
            True,
        )
    if _int_count(counts.get("wlfw_start")) > 0 or _int_count(counts.get("wlfw_service_request")) > 0:
        return (
            "v653-service74-gated-wlfw-advanced",
            True,
            f"counts={counts}",
    ...
    if _int_count(counts.get("binder_transaction_failed")) == 0:
        return (
            "v653-service74-binder-clean-wlfw-missing",
            True,
    ...
    return (
        "v653-service74-gated-service-manager-review-required",
        False,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        text,
        "",
        "## V653 Gate And Service-Manager Surface",
        "",
        ...
        f"- service_manager_started: `{service_manager.get('service_manager_started', '')}`",
        ...
        base.markdown_table(
            ["key", "value"],
            [[key, str(value)] for key, value in sorted((service_manager.get("service74_gate") or {}).items())],
        ) if service_manager.get("service74_gate") else "- none",
        "",
        base.markdown_table(
...
def build_manifest(args: base.argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    ...
    manifest["explicitly_approved"] = [
        ...
        "service74-gated service-manager companion ordering under V641 clean-DSP state",
