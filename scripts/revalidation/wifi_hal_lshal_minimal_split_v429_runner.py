#!/usr/bin/env python3
"""V429 minimal lshal status split Wi-Fi HAL query runner.

V429 is the next gate after V428. It collects two bounded service-surface
views with helper v30:

* a no-daemon-start VINTF-only lshal control;
* a bounded composite servicemanager + hwservicemanager + Wi-Fi HAL start-only
  query using `/system/bin/lshal list --types=binderized --neat -S`.
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v429-lshal-minimal-split-runner")
base.DEFAULT_HELPER_SHA256 = "65b279db9f5a66979140b71688cd3998ddc5832c1ca374e2187db981d5c17757"
base.DEFAULT_V404 = Path("tmp/wifi/v428-lshal-status-query-live-after-selinux-20260520-142354/manifest.json")
base.HELPER_LABEL = "v30"
base.APPROVAL_PHRASE = (
    "approve v429 minimal lshal status split only; "
...
def build_plan(args: base.argparse.Namespace) -> dict[str, Any]:
    ...
    plan["query_argv"] = "/system/bin/lshal list --types=binderized --neat -S"
    plan["vintf_control_argv"] = "/system/bin/lshal list --types=vintf --neat -V -S -i"
    plan["arg_budget_note"] = (
        "helper v30 keeps data-wifi private-empty implicit for status-query modes "
...
def build_checks(args: base.argparse.Namespace, store: base.EvidenceStore, steps: list[base.Step],
                 v428: dict[str, Any]) -> list[base.Check]:
    checks: list[base.Check] = []
    if args.command == "plan":
        base.add_check(checks, "plan-only", "pass", "info", "no device command executed", [], "run preflight after helper v30 deploy")
        return checks
    ...
    helper_ready = (
        ...
        and "a90_android_execns_probe v30" in helper_usage
        and "wifi-hal-composite-lshal-binderized-status-list" in helper_usage
        and "wifi-hal-lshal-vintf-status-list" in helper_usage
        and "--allow-hal-service-query" in helper_usage
        and "--types=binderized" in helper_usage
    ...
    base.add_check(
        checks,
        "v428-status-column-result-present",
        "pass" if v428.get("decision") == "v428-lshal-status-query-runtime-gap" and v428.get("pass") else "blocked",
        "blocker",
        f"decision={v428.get('decision')} pass={v428.get('pass')}",
        [str(v428.get("path", ""))],
        "V428 status-column result must exist before V429 minimal split",
    )
    ...
    base.add_check(checks, "helper-v30", "pass" if helper_ready else "blocked", "blocker", "remote helper must be v30 with minimal split and VINTF-control lshal modes", [line for line in helper_sha.splitlines() if args.helper in line][:2], "deploy helper v30 before V429 live query")
    base.add_check(checks, "selinuxfs-runtime-surface", "pass" if base.step_ok(steps, "stat-selinux-status") and selinuxfs_mounted else "blocked", "blocker", f"mounted={selinuxfs_mounted} status={base.step_ok(steps, 'stat-selinux-status')}", [line for line in mounts.splitlines() if "/sys/fs/selinux" in line][:3], "run the bounded V401 toybox selinuxfs mount before V429 minimal split query")
    base.add_check(checks, "lshal-binary", "pass" if base.step_ok(steps, "stat-lshal") else "blocked", "blocker", "/mnt/system/system/bin/lshal must exist for minimal split listing", [], "if absent, route to Android-side lshal extraction")
    ...
    base.add_check(checks, "approval-gate", "pass" if base.approved(args) else "needs-operator", "approval", f"phrase_match={args.approval_phrase == base.APPROVAL_PHRASE} apply={args.apply} assume_yes={args.assume_yes}", [base.APPROVAL_PHRASE], "exact phrase and flags required before V429 minimal split live query")
    return checks
...
def run_live(args: base.argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    ...
    status_record = base.run_capture(args, "run-binderized-status-query", status_command, timeout=args.timeout + args.max_runtime_sec + 45.0)
    status_rel = "native/run-binderized-status-query.txt"
...
def decide(args: base.argparse.Namespace, checks: list[base.Check], live_result: dict[str, Any] | None,
    ...
    if args.command == "plan":
        return "v429-lshal-minimal-split-plan-ready", True, "plan-only; no device command executed", "run preflight after helper v30 deploy", False
    blocked = base.blockers(checks)
    if blocked:
        return "v429-lshal-minimal-split-blocked", False, "blocked before live run by " + ", ".join(blocked), "resolve blockers before approval", False
    if args.command == "preflight":
        return "v429-lshal-minimal-split-preflight-ready", True, "read-only preflight is ready; live query still needs approval", "operator may approve exact V429 phrase", False
    if not base.approved(args):
        return "v429-lshal-minimal-split-approval-required", True, "exact approval phrase required; no device command executed", "rerun with exact approval if intended", False
    if not live_result or not post or not post["clean"]:
        return "v429-lshal-minimal-split-review-required", False, "live result or postflight cleanliness missing", "inspect evidence and consider recovery reboot", True
    ...
    if control_result == "service-query-pass" and helper_result == "service-query-pass" and query_result == "service-query-pass" and live_result.get("all_postflight_safe") and post.get("clean"):
        return "v429-lshal-minimal-split-pass", True, "VINTF control and explicit minimal split query completed while composite HAL trio stayed bounded and cleaned", "classify status rows before any Wi-Fi bring-up", True
    if helper_result == "service-query-tool-missing":
        return "v429-lshal-minimal-split-tool-missing", True, "lshal is unavailable in this system image", "route to Android-side lshal extraction", True
    if control_result != "service-query-pass" and not control_has_rows:
        return "v429-lshal-minimal-split-control-gap", True, f"VINTF control result={control_result}", "inspect VINTF-only command evidence before retrying composite query", True
    if helper_result == "service-query-runtime-gap":
        return "v429-lshal-minimal-split-runtime-gap", True, f"status query failed: {live_result.get('service_query_reason')}; VINTF rows present={control_has_rows}", "compare target rows and decide Android-managed pivot versus native retry", True
    return "v429-lshal-minimal-split-review-required", False, f"helper_result={helper_result} query_result={query_result}", "inspect helper output before widening scope", True


def refusal_manifest(args: base.argparse.Namespace, v428: dict[str, Any]) -> dict[str, Any]:
    return {
        "generated_at": base.now_iso(),
        "command": args.command,
        "decision": "v429-lshal-minimal-split-approval-required",
        "pass": True,
        "reason": "exact approval phrase required; no device command executed",
        "next_step": "rerun with exact approval only after helper v30 deploy and preflight",
        "host": base.collect_host_metadata(),
        "v428": {"path": v428.get("path"), "decision": v428.get("decision"), "pass": v428.get("pass")},
        "plan": base.build_plan(args),
        "steps": [],
        "checks": [asdict(base.Check("approval-gate", "needs-operator", "approval", base.APPROVAL_PHRASE, [base.APPROVAL_PHRASE], "approve before live minimal split query"))],
        "live_result": None,
        "postflight": None,
...
def build_manifest(args: base.argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    v428 = base.load_json(args.v404_manifest)
    if args.command == "run" and not base.approved(args):
        return refusal_manifest(args, v428)
    steps: list[base.Step] = []
    live_result: dict[str, Any] | None = None
    post: dict[str, Any] | None = None
    if args.command != "plan":
        steps = base.run_preflight(args, store)
    checks = build_checks(args, store, steps, v428)
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V429 Minimal lshal Status Split Runner",
        "",
