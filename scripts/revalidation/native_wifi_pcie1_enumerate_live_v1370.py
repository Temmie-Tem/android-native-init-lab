#!/usr/bin/env python3
"""V1370 bounded live corrected-RC1 pci-msm enumerate proof."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1370-pci-msm-corrected-rc1-enumerate-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1370-pci-msm-corrected-rc1-enumerate-live.txt")
REPORT_PATH = Path("docs/reports/NATIVE_INIT_V1370_PCI_MSM_CORRECTED_RC1_ENUMERATE_LIVE_2026-06-01.md")
...
def plan_manifest() -> dict[str, Any]:
    return {
        "cycle": "V1370",
        "type": "bounded live corrected-RC1 enumerate plan",
        "generated_at": now_iso(),
        "decision": "v1370-corrected-rc1-enumerate-plan-ready",
        "pass": True,
        "reason": "plan-only; no device command executed",
        "next_step": "run V1370 bounded corrected-RC1 enumerate proof",
        "candidate_writes": [
            "printf '2\\n' > /sys/kernel/debug/pci-msm/rc_sel",
            "printf '11\\n' > /sys/kernel/debug/pci-msm/case",
        ],
        "forbidden": [
            "any pci-msm case other than case=11",
...
def collect_run(args: argparse.Namespace, store: EvidenceStore) -> list[dict[str, Any]]:
    ...
    run_shell(
        ...
        store,
        steps,
        "write-rc1-bitmask2-case11-enumerate",
        f"printf '2\\n' > {PCI_MSM_DEBUGFS}/rc_sel && printf '11\\n' > {PCI_MSM_DEBUGFS}/case",
        timeout=45.0,
    )
    run_shell(args, store, steps, "settle", f"{args.busybox} sleep 3; true", timeout=8.0)
...
def analyze(store: EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    mounted_by_v1370 = not mounted_before and command_ok(steps, "debugfs-mount")
    cleanup_ok = (mounted_after == mounted_before) if mounted_by_v1370 and mounts_after_ok else (True if not mounted_by_v1370 else None)
    write_step = base.step_by_name(steps, "write-rc1-bitmask2-case11-enumerate")
    ...
    pcie_enable_attempt_seen = "msm_pcie_enable: PCIe: Assert the reset of endpoint of RC1." in after_dmesg
    pcie_release_reset_seen = "msm_pcie_enable: PCIe: Release the reset of endpoint of RC1." in after_dmesg
    pcie_phy_ready_seen = "PCIe RC1 PHY is ready" in after_dmesg
    ltssm_poll_active_seen = "PCIe RC1: LTSSM_STATE: LTSSM_POLL_ACTIVE" in after_dmesg
    ltssm_poll_compliance_seen = "PCIe RC1: LTSSM_STATE: LTSSM_POLL_COMPLIANCE" in after_dmesg
    pcie_link_failed_seen = "PCIe RC1 link initialization failed" in after_dmesg
    transient_link_training_seen = any(
        (
            pcie_enable_attempt_seen,
            pcie_release_reset_seen,
            pcie_phy_ready_seen,
            ltssm_poll_active_seen,
            ltssm_poll_compliance_seen,
            pcie_link_failed_seen,
        )
    )
    rc1_enumerate_patterns = (
        "PCIe: attempting to enumerate RC1",
        "Enumerate RC1",
        "PCIe: RC1 is successfully enumerated",
        "PCIe: RC1 enumeration failed",
        "PCIe: RC1 has already been enumerated",
        "PCIe: failed to enable RC1",
        "PCIe: RC1 has not been successfully probed yet",
    )
    rc1_enumerate_seen = any(pattern in after_dmesg for pattern in rc1_enumerate_patterns)
    enumerate_seen = rc1_enumerate_seen or "enumerate" in after_dmesg.lower() or "enumeration" in after_dmesg.lower()
    pci_or_mhi_changed = after_pci_count != before_pci_count or after_mhi_present != before_mhi_present
    steady_state_change_seen = bool(pci_or_mhi_changed or pcie_link_seen or gdsc_changed or clk_changed)
    post_selftest_fail0 = "fail=0" in post_selftest

    if not mounted_during:
        decision = "v1370-debugfs-mount-unavailable"
        pass_condition = False
        reason = "debugfs was not mounted during the corrected-RC1 proof window"
        next_step = "repair debugfs access before corrected-RC1 enumerate proof"
    elif reset_after_write:
        decision = "v1370-corrected-rc1-enumerate-transport-reset"
        pass_condition = False
        reason = "corrected rc_sel=2 case=11 enumerate caused command transport loss before after-captures completed"
        next_step = "stop pci-msm debugfs case writes; use kernel-side shim or Android-reference route"
    elif not write_ok:
        decision = "v1370-corrected-rc1-enumerate-write-failed"
        pass_condition = False
        reason = "rc_sel=2 or case=11 enumerate write failed"
        next_step = "inspect write error; do not attempt enumerate"
    elif not after_captures_ok:
        decision = "v1370-corrected-rc1-enumerate-after-capture-incomplete"
        pass_condition = False
        reason = "corrected-RC1 enumerate returned but after-captures were incomplete"
        next_step = "inspect after-capture failure before further pci-msm debugfs use"
    elif not cleanup_ok or not post_selftest_fail0:
        decision = "v1370-corrected-rc1-enumerate-health-or-cleanup-issue"
        pass_condition = False
        reason = "corrected-RC1 enumerate completed but cleanup or health check failed"
        next_step = "inspect cleanup/health issue before further pci-msm debugfs use"
    elif rc1_enumerate_seen and (pcie_link_seen or pci_or_mhi_changed):
        decision = "v1370-corrected-rc1-enumerate-link-up-clean"
        pass_condition = True
        reason = "rc_sel=2 and case=11 reached RC1 enumerate and produced a PCI/MHI/link transition while device health remained clean"
        next_step = "classify the new PCI/MHI state before any Wi-Fi HAL or network bring-up"
    elif rc1_enumerate_seen and transient_link_training_seen:
        decision = "v1370-corrected-rc1-link-training-no-l0-clean"
        pass_condition = True
        reason = "rc_sel=2 and case=11 reached RC1 enumerate and transient RC1 PHY/LTSSM link training, but did not reach L0 or create PCI/MHI devices; device health remained clean"
        next_step = "classify why RC1 stops at LTSSM poll/compliance before any Wi-Fi HAL or network bring-up"
    elif rc1_enumerate_seen:
        decision = "v1370-corrected-rc1-enumerate-attempt-no-link-clean"
        pass_condition = True
        reason = "rc_sel=2 and case=11 reached RC1 enumerate but did not produce PCI/MHI/link transition; device health remained clean"
        next_step = "inspect dmesg/power deltas to decide whether pcie1 needs a narrower RC power or PERST/refclk precondition"
    elif enumerate_seen:
        decision = "v1370-corrected-rc1-enumerate-non-rc1-output"
        pass_condition = False
        reason = "enumerate output was observed but not clearly attributed to RC1"
        next_step = "inspect dmesg output before any further pci-msm debugfs use"
    else:
        decision = "v1370-corrected-rc1-enumerate-output-missing"
        pass_condition = False
        reason = "corrected-RC1 enumerate returned cleanly but RC1 enumerate output was not observed"
    ...
    return {
        ...
        "mounted_by_v1370": mounted_by_v1370,
        ...
        "steady_state_change_seen": steady_state_change_seen,
        "pcie_link_seen": pcie_link_seen,
        "pcie_enable_attempt_seen": pcie_enable_attempt_seen,
        "pcie_release_reset_seen": pcie_release_reset_seen,
        "pcie_phy_ready_seen": pcie_phy_ready_seen,
        "ltssm_poll_active_seen": ltssm_poll_active_seen,
        "ltssm_poll_compliance_seen": ltssm_poll_compliance_seen,
        "pcie_link_failed_seen": pcie_link_failed_seen,
        "transient_link_training_seen": transient_link_training_seen,
        "enumerate_seen": enumerate_seen,
        "rc1_enumerate_seen": rc1_enumerate_seen,
        "post_selftest_fail0": post_selftest_fail0,
        "safety": {
            "debugfs_write_executed": True,
            "rc_sel_2_write_executed": True,
            "case11_enumerate_executed": True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1370 Corrected-RC1 pci-msm Enumerate Live",
        "",
