#!/usr/bin/env python3
"""V1235: bounded mdm_helper post-WAIT_FOR_REQ branch snapshot live gate.

This is a V1232 derivative.  It preserves the non-ptrace PM/CNSS path, uses
helper v257, and adds the post-WAIT_FOR_REQ branch snapshot flag:
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1235-mdm-helper-post-wait-req-branch-snapshot-live")
LATEST_POINTER = Path("tmp/wifi/latest-v1235-mdm-helper-post-wait-req-branch-snapshot-live.txt")
...
def decide_v1235(manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if manifest.get("command") == "plan":
        return (
            "v1235-post-wait-req-observer-plan-ready",
            True,
            "plan-only; no device mutation or live actor executed",
            "run V1235 bounded post-WAIT_FOR_REQ observer",
        )
    ...
    if not post_wait.get("emitted"):
        return (
            "v1235-post-wait-req-not-emitted",
            False,
    ...
    if post_wait.get("forbidden"):
        return (
            "v1235-post-wait-req-forbidden-action",
            False,
    ...
    if not branch.get("emitted"):
        return (
            "v1235-post-wait-branch-not-emitted",
            False,
    ...
    if branch.get("forbidden"):
        return (
            "v1235-post-wait-branch-forbidden-action",
            False,
    ...
    if _post_wait_has_ks_or_mhi(post_wait):
        return (
            "v1235-post-wait-req-ks-mhi-progress",
            True,
    ...
    if transition > 0:
        if _int_value(branch.get("execve_count"), 0) > 0:
            return (
                "v1235-branch-snapshot-exec-attempt-no-ks-mhi",
                True,
        ...
        if _int_value(branch.get("phase_count"), 0) > 0:
            return (
                "v1235-branch-snapshot-no-exec-no-ks-mhi",
                True,
        ...
        return (
            "v1235-wait-req-returned-no-ks-mhi",
            True,
    ...
    if early.get("wait_for_req_thread_count", 0) > 0 or post_wait.get("initial_wait_for_req_thread_count", 0) > 0:
        return (
            "v1235-still-waiting-for-req-no-transition",
            True,
    ...
    if parity.get("pm_service_subsys_esoc0_attempt"):
        return (
            "v1235-post-wait-req-no-wait-thread-with-pm-trigger",
            True,
    ...
    return (
        "v1235-post-wait-req-inconclusive",
        False,
        f"post_wait={post_wait} early={early}",
        "inspect full child output and repair V1235 parser or live path",
    )
...
def main() -> int:
    ...
    decision, passed, reason, next_step = decide_v1235(manifest)
