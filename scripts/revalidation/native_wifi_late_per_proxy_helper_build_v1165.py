#!/usr/bin/env python3
"""V1165 source/build-only gate for late pm-proxy actionability instrumentation."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1165-execns-helper-v217-build")
...
DEFAULT_V1164_MANIFEST = Path("tmp/wifi/v1164-pm-proxy-binder-delta-classifier/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1165-execns-helper-v217-build.txt")
HELPER_MARKER = "a90_android_execns_probe v217"
EXPECTED_V1164_DECISION = "v1164-pm-proxy-binder-actionability-gap"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v217"',
    ...
    "late_per_proxy.poll_max",
    "late_per_proxy.instrumentation=v1165",
    "per_proxy_alive",
    "per_proxy_exit_code",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1164 = load_json(args.v1164_manifest)
    analysis: dict[str, Any] = {
        "v1164": {
            "decision": v1164.get("decision", ""),
            "pass": bool(v1164.get("pass")),
            "path": str(repo_path(args.v1164_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1164-input",
            "status": "pass" if analysis["v1164"]["decision"] == EXPECTED_V1164_DECISION and analysis["v1164"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1164']['decision']} pass={analysis['v1164']['pass']}",
            "next_step": "refresh V1164 classifier before building helper v217",
        },
        {
            "name": "source-strings",
    ...
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1165 build-only gate"})
        return rows
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1165-late-per-proxy-actionability-helper-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, PM actor, mdm_helper, CNSS actor, or Wi-Fi action executed",
            "run V1165 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1165-late-per-proxy-actionability-helper-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1165-late-per-proxy-actionability-helper-build-pass",
        True,
        f"helper v217 built sha256={analysis['build']['output_sha256']}",
        "V1165 should deploy helper v217 and run the longer bounded late pm-proxy actionability gate without Wi-Fi HAL or bring-up",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1165 Late pm-proxy Actionability Helper Build",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1164-manifest", type=Path, default=DEFAULT_V1164_MANIFEST)
