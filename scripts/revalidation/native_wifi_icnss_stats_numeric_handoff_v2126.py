#!/usr/bin/env python3
"""V2126 rollbackable handoff for numeric ICNSS stats."""
...
OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v2126-icnss-stats-numeric-handoff"
HANDOFF_DIR = OUT_DIR / "v2125-handoff"
HANDOFF_REPORT = OUT_DIR / "v2125-handoff-report.md"
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2126_ICNSS_STATS_NUMERIC_HANDOFF_2026-06-05.md"
)
V2125_OUT = REPO_ROOT / "tmp" / "wifi" / "v2125-icnss-stats-numeric-test-boot"
V2125_INIT = V2125_OUT / "init_v2125_icnss_stats_numeric"
V2125_BOOT = V2125_OUT / "boot_linux_v2125_icnss_stats_numeric.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2125/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.237 (v2125-icnss-stats-numeric)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2125.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2125.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2125-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v421"
V421_IND_EVENTS = (
...
def stats_rows(stats: dict[str, Any]) -> list[list[object]]:
    ...
    for phase in ICNSS_STATS_PHASES:
        ...
        rows.append([
            ...
            f"ind_reg={values.get('ind_register_resp')} msa_ready={values.get('msa_ready_resp')} msa_ind={values.get('msa_ready_ind')} cap={values.get('cap_resp')}",
        ])
    return rows
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2125_INIT, init_required), (V2125_BOOT, boot_required)):
        key = rel(path)
        forbidden_tokens = init_forbidden if path == V2125_INIT else boot_forbidden
...
def classify(handoff: dict[str, Any],
    ...
    if not route_ok:
        ...
        passed = False
        reason = "V2126 did not preserve shared-server-info, wlan_pd UP, ICNSS QMI, cap/BDF/cal, or rollback prerequisites"
    elif stats_open <= 0 or stats_numeric <= 0:
        label = "icnss-stats-numeric-missing"
        passed = False
        reason = "helper v421 ran but did not return readable numeric `/sys/kernel/debug/icnss/stats` counters"
    elif wlan0:
        label = "icnss-stats-wlan0-progress"
        passed = True
        reason = "native reached wlan0; stop before scan/connect and run the dedicated connectivity gate"
    elif fw_ready:
        label = "icnss-stats-fw-ready-progress"
        passed = True
    ...
    elif msa_ready_ind > 0 and saw_msg21 and not fw_ready:
        ...
        label = "icnss-stats-none-from-wlfw"
        passed = True
        ...
        label = "icnss-stats-status-version-no-fw-ready"
        passed = True
        ...
        label = "icnss-stats-msa-ready-no-fw-mem-ready"
        passed = True
        reason = "MSA-ready msg 0x2b reached cnss-daemon, but FW-memory-ready msg 0x37 did not appear"
    elif queue_hits == 0:
        label = "icnss-stats-indication-callback-not-queued"
        passed = True
        reason = "WLFW QMI callback ran, but no decoded indication was queued for the worker"
    elif handle_hits == 0:
        label = "icnss-stats-indication-queued-not-drained"
        passed = True
        reason = "WLFW indication was queued, but the worker did not drain it"
    else:
        label = "icnss-stats-unresolved-no-fw-ready"
        passed = True
        reason = "numeric ICNSS counters and WLFW indications advanced, but kernel FW_READY/wlan0 still did not appear"

    return {
        **base,
        "decision": f"v2126-{label}-rollback-{'pass' if passed else 'blocked'}",
        "label": label,
        "pass": passed,
        "reason": reason,
        "route_ok": route_ok,
...
def configure_modules() -> None:
    ...
    prev2121.V2120_OUT = V2125_OUT
    prev2121.V2120_INIT = V2125_INIT
    prev2121.V2120_BOOT = V2125_BOOT
    ...
    prev2113.V2112_OUT = V2125_OUT
    prev2113.V2112_INIT = V2125_INIT
    prev2113.V2112_BOOT = V2125_BOOT
    ...
    prev2113.BRIDGE_CAPTURE = OUT_DIR / "host" / "v2126-autostart-bridge.log"
    prev2113.BRIDGE_STDOUT = OUT_DIR / "host" / "v2126-autostart-bridge.stdout.txt"
    prev2113.BRIDGE_STDERR = OUT_DIR / "host" / "v2126-autostart-bridge.stderr.txt"
    prev2113.BRIDGE_PID = OUT_DIR / "host" / "v2126-autostart-bridge.pid"
