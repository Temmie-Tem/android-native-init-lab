#!/usr/bin/env python3
"""V1595 local-only sanity verifier for the V1594 PM-first lower-marker test boot."""
...
base.DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1595-pm-first-lower-marker-artifact-sanity"
base.DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1595_PM_FIRST_LOWER_MARKER_ARTIFACT_SANITY_2026-06-02.md"
)
base.EXPECTED_DECISION = "v1594-pm-first-lower-marker-test-boot-source-build-pass"
base.EXPECTED_HELPER = "a90_android_execns_probe_v295"
base.EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.103 (v1594-pm-first-lower-marker)",
    "a90_android_execns_probe v295",
    "A90v1594",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "watch_sec": 120,
        "helper_mode": "android-service-window-pm-proxy-contract-pm-first-lower-marker",
        "helper_runtime_mode": "wifi-companion-android-wifi-service-window-subsys-trigger-capture",
        "helper_result": "/cache/native-init-wifi-test-boot-v1594-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if pass_ok:
        return (
            "v1595-pm-first-lower-marker-artifact-sanity-pass",
            True,
            "V1594 PM-first lower-marker test boot artifact passed local sanity",
        )
    return (
        "v1595-pm-first-lower-marker-artifact-sanity-blocked",
        False,
        f"V1594 artifact sanity failed; base decision was {decision}",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1594 manifest: `{manifest['_path']}`",
        f"- V1594 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- boot PM-first markers: `{checks['boot_markers']['ok']}`",
        f"- helper PM-first markers: `{checks['helper_markers']['ok']}`",
