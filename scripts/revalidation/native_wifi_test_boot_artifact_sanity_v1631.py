#!/usr/bin/env python3
"""V1631 local-only sanity verifier for the V1630 natural-path MDM2AP test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1631-natural-path-mdm2ap-observation-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1631_NATURAL_PATH_MDM2AP_OBSERVATION_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1630-natural-path-mdm2ap-observation-source-build-pass"
EXPECTED_INIT_MARKERS = (
    "A90 Linux init 0.9.112 (v1630-natural-mdm2ap)",
    "A90v1630",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_timeout_sec": 60,
        "rc1_watcher_delay_ms": 0,
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1630-natural-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1630-natural-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            ...
            True,
            "V1630 natural-path MDM2AP observation artifact passed local sanity; one rollbackable live handoff may proceed",
        )
    return (
        "v1631-natural-path-mdm2ap-observation-artifact-sanity-blocked",
        False,
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1630 manifest: `{manifest['_path']}`",
        f"- V1630 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- mdm2ap timing helper markers: `{checks['boot_markers']['ok']}`",
