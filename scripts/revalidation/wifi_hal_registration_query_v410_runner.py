#!/usr/bin/env python3
"""V410 bounded Wi-Fi HAL registration query runner.

V410 is the next gate after the V408 service-surface packet.  It requires
helper v26 and uses the helper-owned private namespace to start only:
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v410-hal-registration-query-runner")
base.DEFAULT_HELPER_SHA256 = "daf1b59e2475c0db28fb99eb83f8be02a46f695d8c4e435c47e68f45370a7caa"
base.DEFAULT_V404 = Path("tmp/wifi/v408-hal-registration-surface-packet-20260520-102249/manifest.json")
base.HELPER_LABEL = "v26"
...
def build_plan(args: base.argparse.Namespace) -> dict[str, Any]:
    ...
    plan["arg_budget_note"] = (
        "helper v26 defaults wifi-hal-composite-lshal-list to private-empty "
...
def build_checks(args: base.argparse.Namespace, store: base.EvidenceStore, steps: list[base.Step],
    ...
    if args.command == "plan":
        base.add_check(checks, "plan-only", "pass", "info", "no device command executed", [], "run preflight after helper v26 deploy")
        return checks
    ...
    helper_ready = (
        ...
        and "a90_android_execns_probe v26" in helper_usage
    ...
    base.add_check(
        ...
        [str(v408.get("path", ""))],
        "V408 evidence packet must pass before V410 registration query",
    )
    ...
    base.add_check(checks, "helper-v26", "pass" if helper_ready else "blocked", "blocker", "remote helper must be v26 with lshal query mode", [line for line in helper_sha.splitlines() if args.helper in line][:2], "deploy helper v26 before V410 live query")
    ...
    base.add_check(checks, "approval-gate", "pass" if base.approved(args) else "needs-operator", "approval", f"phrase_match={args.approval_phrase == base.APPROVAL_PHRASE} apply={args.apply} assume_yes={args.assume_yes}", [base.APPROVAL_PHRASE], "exact phrase and flags required before V410 live query")
    return checks
...
def decide(args: base.argparse.Namespace, checks: list[base.Check], live_result: dict[str, Any] | None,
    ...
    if args.command == "plan":
        return "v410-hal-registration-query-plan-ready", True, "plan-only; no device command executed", "run preflight after helper v26 deploy", False
    blocked = base.blockers(checks)
    if blocked:
        return "v410-hal-registration-query-blocked", False, "blocked before live run by " + ", ".join(blocked), "resolve blockers before approval", False
    if args.command == "preflight":
        return "v410-hal-registration-query-preflight-ready", True, "read-only preflight is ready; live query still needs approval", "operator may approve exact V410 phrase", False
    if not base.approved(args):
        return "v410-hal-registration-query-approval-required", True, "exact approval phrase required; no device command executed", "rerun with exact approval if intended", False
    if not live_result or not post or not post["clean"]:
        return "v410-hal-registration-query-review-required", False, "live result or postflight cleanliness missing", "inspect evidence and consider recovery reboot", True
    ...
    if helper_result == "service-query-pass" and query_result == "service-query-pass" and live_result.get("all_postflight_safe") and post.get("clean"):
        return "v410-hal-registration-query-pass", True, "lshal query completed while composite HAL trio stayed bounded and cleaned", "classify listed Wi-Fi HAL service and plan next gate", True
    if helper_result == "service-query-tool-missing":
        return "v410-hal-registration-query-tool-missing", True, "lshal is unavailable in this system image", "route to V410 micro HIDL client or Android-side lshal extraction", True
    if helper_result == "service-query-runtime-gap":
        return "v410-hal-registration-query-runtime-gap", True, f"service query failed: {live_result.get('service_query_reason')}", "classify lshal/HIDL query failure before wider Wi-Fi work", True
    return "v410-hal-registration-query-review-required", False, f"helper_result={helper_result} query_result={query_result}", "inspect helper output before widening scope", True
...
def refusal_manifest(args: base.argparse.Namespace, v408: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "next_step": "rerun with exact approval only after helper v26 deploy and preflight",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V410 Wi-Fi HAL Registration Query Runner",
        "",
