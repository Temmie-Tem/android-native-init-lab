#!/usr/bin/env python3
"""Build the V1454 exact-line provider-trigger long-window Wi-Fi test boot artifact."""
...
OUT_DIR = base.REPO_ROOT / "tmp" / "wifi" / "v1454-wifi-test-boot-exact-provider-long-endpoint-sampler"
REPORT_PATH = (
    base.REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1454_WIFI_TEST_BOOT_EXACT_PROVIDER_LONG_ENDPOINT_SOURCE_BUILD_2026-06-01.md"
)
...
DEFAULT_ARGS = [
    ...
    "V1454",
    "--decision",
    "v1454-wifi-test-boot-exact-provider-long-endpoint-source-build-pass",
    "--cycle-label",
    "v1454",
    "--init-version",
    "0.9.84",
    ...
    str(OUT_DIR),
    "--init-binary",
    str(OUT_DIR / "init_v1454_wifi_test"),
    "--ramdisk-cpio",
    str(OUT_DIR / "ramdisk_v1454_wifi_test.cpio"),
    "--boot-image",
    str(OUT_DIR / "boot_linux_v1454_wifi_test.img"),
    "--wifi-test-klog-prefix",
    "A90v1454",
    "--wifi-test-disable",
    "/cache/native-init-wifi-test-boot-v1454.disable",
    "--wifi-test-log",
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Sampler marker: `read-only-v1454-exact-provider-long-endpoint`.",
        f"- Log path: `{wifi['log']}`",
