#!/usr/bin/env python3
"""V2152 native wlan0 connect, DHCP, and google.com ping handoff."""
...
OUT_DIR = REPO_ROOT / "tmp" / "wifi" / f"v2152-connect-dhcp-google-ping-handoff{RUN_SUFFIX}"
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / f"NATIVE_INIT_V2152_CONNECT_DHCP_GOOGLE_PING_HANDOFF{REPORT_SUFFIX}_2026-06-05.md"
)
HELPER_BUILD_DIR = OUT_DIR / "host-build"
HELPER_LOCAL = HELPER_BUILD_DIR / "a90_android_execns_probe_v2152"
HELPER_REMOTE = "/cache/bin/a90_android_execns_probe_v2152"
HELPER_REMOTE_B64 = "/cache/a90-execns-v2152.gz.b64"
HELPER_REMOTE_GZ = "/cache/a90-execns-v2152.gz"
CONNECT_DIR = "/cache/a90-wifi"
CONNECT_CONFIG = f"{CONNECT_DIR}/v2152.conf"
CONNECT_CONFIG_B64 = f"{CONNECT_CONFIG}.b64"
CONNECT_SCRIPT = "/cache/a90-v2152-connect-ping.sh"
CONNECT_RESULT = "/cache/a90-v2152-connect-ping.result"
...
def build_helper(store: base.EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    (HELPER_BUILD_DIR / "a90_android_execns_probe_v2152.gz").write_bytes(gzip_bytes)
    return {
        "ok": True,
        "sha256": helper_sha,
...
def stage_connect_script(store: base.EvidenceStore,
    ...
    script_lines = [
        ...
        "echo v2152.begin=1 > \"$out\"",
        "echo v2152.raw_values_logged=0 >> \"$out\"",
        "echo v2152.credentials_read=1 >> \"$out\"",
        "echo v2152.connect_attempted=1 >> \"$out\"",
        "echo v2152.dhcp_route_attempted=1 >> \"$out\"",
        "echo v2152.external_ping_attempted=1 >> \"$out\"",
        "loop=0",
        "while [ \"$loop\" -lt 1200 ]; do",
        "if [ -e /sys/class/net/wlan0 ]; then echo v2152.wlan0_seen=1 >> \"$out\"; break; fi",
        "loop=$((loop+1))",
        "sleep 0.2",
        "done",
        "if [ ! -e /sys/class/net/wlan0 ]; then echo v2152.wlan0_seen=0 >> \"$out\"; echo v2152.result=wlan0-missing >> \"$out\"; echo v2152.end=1 >> \"$out\"; exit 20; fi",
        "printf 'v2152.pre_operstate=' >> \"$out\"; cat /sys/class/net/wlan0/operstate >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "printf 'v2152.pre_carrier=' >> \"$out\"; cat /sys/class/net/wlan0/carrier >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "printf 'v2152.pre_flags=' >> \"$out\"; cat /sys/class/net/wlan0/flags >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "echo v2152.helper_invoked=1 >> \"$out\"",
        f"{helper_command} >> \"$out\" 2>&1",
        "echo v2152.helper_rc=$? >> \"$out\"",
        "printf 'v2152.post_operstate=' >> \"$out\"; cat /sys/class/net/wlan0/operstate >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "printf 'v2152.post_carrier=' >> \"$out\"; cat /sys/class/net/wlan0/carrier >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "printf 'v2152.post_flags=' >> \"$out\"; cat /sys/class/net/wlan0/flags >> \"$out\" 2>/dev/null || echo unreadable >> \"$out\"",
        "echo v2152.end=1 >> \"$out\"",
    ]
    line_ok = True
...
def wait_for_connect_result(store: base.EvidenceStore,
    ...
    while time.monotonic() <= deadline:
        command = (
            ...
            f"/cache/bin/busybox grep -q '^v2152.end=1' {CONNECT_RESULT} 2>/dev/null && complete=1; "
...
def post_flash_connect(store: base.EvidenceStore,
    ...
    ok = (
        helper_fields.get("helper_stage.ok") == "1"
    ...
    return {
        "ok": ok,
        "fields": {**helper_fields, **config_fields, **script_fields},
        "wait": wait_fields,
    }
...
def collect_gate(manifest: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    wlan0_seen = fields.get("v2152.wlan0_seen") == "1"
    helper_invoked = fields.get("v2152.helper_invoked") == "1"
    helper_rc = intish(fields.get("v2152.helper_rc"))
    ...
    no_raw = fields.get("v2152.raw_values_logged") == "0" and fields.get("wifi_connect_ping.secret_values_logged", "0") == "0"
    ...
    elif not helper_stage_ok or not config_ok or not script_ok or not wait_complete:
        label = "connect-dhcp-ping-stage-or-wait-failed"
        passed = False
        reason = (
            f"stage/wait failed helper={helper_stage_ok} config={config_ok} "
            f"script={script_ok} wait={wait_complete}"
        )
    elif not no_raw:
    ...
    return {
        "label": label,
        "decision": f"v2152-{label}-rollback-{'pass' if passed else 'blocked'}",
        "pass": passed,
        "reason": reason,
        ...
        "pre_operstate": fields.get("v2152.pre_operstate", ""),
        "pre_carrier": fields.get("v2152.pre_carrier", ""),
        "pre_flags": fields.get("v2152.pre_flags", ""),
        "post_operstate": fields.get("v2152.post_operstate", ""),
        "post_carrier": fields.get("v2152.post_carrier", ""),
        "post_flags": fields.get("v2152.post_flags", ""),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- `wpa_ctrl`: ready `{gate['wpa_ctrl_ready']}` surface `{gate['wpa_ctrl_surface']}` ping `{gate['wpa_ctrl_ping_reply']}` country_rc `{gate['driver_country_rc']}` reply `{gate['driver_country_reply']}` enable `{gate['enable_network_reply']}` reassociate `{gate['reassociate_reply']}`",
        ...
        f"- `helper_stage_ok`: `{gate['helper_stage_ok']}` config_ok `{gate['config_ok']}` script_ok `{gate['script_ok']}` wait_complete `{gate['wait_complete']}`",
...
def main() -> int:
    ...
    if helper_stage.get("helper_stage.ok") != "1" or config_stage.get("connect_config.ok") != "1":
        write_preflight_manifest(
            ...
            (
                f"prestage failed helper={helper_stage.get('helper_stage.reason')} "
    ...
    def hook(hook_store: base.EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return post_flash_connect(hook_store, steps, helper_stage, config_stage)
    ...
    if hits:
        manifest["decision"] = "v2152-forbidden-output-hit"
