#!/usr/bin/env python3
"""V694 current-boot orchestrator for PeripheralManager vndservice query proof."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v694-peripheral-vndservice-query-orchestrated")
V694_SCRIPT = "scripts/revalidation/native_wifi_peripheral_vndservice_query_v694.py"
V694_APPROVAL = (
    "approve v694 bounded vndservice query proof only; "
    "no Wi-Fi HAL start, no scan/connect/link-up, no DHCP and no external ping"
)
HELPER_SHA256 = "4739699b794a4129f0bb84b61ecbbaf726e53eeb51ea93ad0a0b0497b60eeb83"
HELPER_MARKER = "a90_android_execns_probe v117"

ALLOWED_LIVE_ACTIONS = (
    "V641 one-shot clean-DSP reboot",
    "V401 SELinuxfs mount surface",
    "V490 Android SELinux policy-load proof",
    "bounded helper v117 PeripheralManager vndservice query proof",
...
def peripheral_for(arm: dict[str, Any] | None) -> dict[str, Any]:
    surface = arm_live(arm).get("v694_peripheral_manager_surface")
    return surface if isinstance(surface, dict) else {}


def query_for(arm: dict[str, Any] | None) -> dict[str, Any]:
    surface = arm_live(arm).get("v694_vndservice_query_surface")
    return surface if isinstance(surface, dict) else {}
...
def checks_for(prep: dict[str, Any] | None, arm: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "next_step": "do not evaluate provider query until service74 is positive",
        },
        {
            "name": "vndservice-query-ran",
            "status": "pass" if query_ran(query) else "blocked",
            "detail": query,
            "next_step": "fix helper query placement if absent",
        },
        {
            "name": "peripheral-registration-seen",
            "status": "pass" if query_exact(query) else "finding",
            "detail": query,
            "next_step": "if absent, repair provider registration/runtime before another CNSS retry",
        },
        {
...
def decide(command: str, prep: dict[str, Any] | None, arm: dict[str, Any] | None) -> tuple[str, bool, str, str]:
    if command == "plan":
        return (
            "v694-peripheral-vndservice-query-orchestrator-plan-ready",
            True,
            "plan-only; no device command executed",
            "deploy helper v117, then run V694 orchestrated vndservice query proof",
        )
    if not prep or not prep.get("ready"):
        return "v694-current-boot-prep-blocked", False, f"prep={prep}", "restore current-boot prerequisites before V694"
    if not arm or not arm.get("decision"):
        return "v694-arm-missing", False, "V694 arm did not produce a manifest", "inspect arm evidence"
    if "blocked" in str(arm.get("decision")):
        return "v694-arm-blocked", False, f"arm={summarize_arm(arm)}", "resolve V694 arm blocker before retry"
    query = query_for(arm)
    counts = counts_for(arm)
    markers = markers_for(arm)
    ...
    if query_exact(query):
        return (
            "v694-peripheral-vndservice-registration-confirmed",
            True,
            f"vndservice query confirmed PeripheralManager registration; query={query}",
    ...
    if query_ran(query):
        return (
            ...
            True,
            f"vndservice query ran without exact PeripheralManager registration; query={query}",
            "repair provider registration/runtime before CNSS retry or Wi-Fi HAL start",
        )
    return "v694-vndservice-query-not-executed", False, f"arm={summarize_arm(arm)}", "fix helper query placement"
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V694 PeripheralManager vndservice Query Orchestrator",
        "",
...
def main() -> int:
    ...
    if args.command == "run":
        root = store.run_dir
        arm_root = root / "arm-v694-v117-vndservice-query"
        prep = v673.prep_current_boot(args, store, "v694", arm_root)
        if prep.get("ready"):
            arm = v673.run_arm(
                args,
                store,
