#!/usr/bin/env python3
"""V853 bounded Android handoff for eSoC actor/device-node sampling."""
...
def v853_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v853-android-esoc-actor-run"


def v853_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    ...
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            if args.serial:
                v853_command[4:4] = ["--serial", args.serial]
            plan.append(("v853-android-esoc-actor-sample", v853_command, collector_timeout))
        else:
            plan.append((name, command, timeout))
    return plan


def load_v853_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v853_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v853 = load_v853_result(store)
    summary = v853.get("android_summary") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v853-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass before V853"
    elif not v853.get("pass"):
        decision = "v853-handoff-collector-failed-rollback-complete"
        pass_ok = False
        reason = "V853 Android actor sampler failed after rollback completed"
    else:
        decision = str(v853.get("decision") or "v853-handoff-review-required")
        pass_ok = bool(v853.get("pass"))
        reason = str(v853.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "state": {
            ...
            "holder_count": summary.get("holder_count"),
            "holder_lines": summary.get("holder_lines"),
            "process_lines": summary.get("process_lines"),
            "ueventd_lines": summary.get("ueventd_lines"),
            "selinux_lines": summary.get("selinux_lines"),
            "dmesg_lines": summary.get("dmesg_lines"),
        },
        "v853": {
            "path": v853.get("_path"),
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v853_out_dir": str(v853_out_dir(store)),
    }
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v853-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v853-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v853-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v853-handoff-approval-required", False

    plan = v853_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_active_wifi(plan)
    if offenders:
        context["forbidden_active_wifi_steps"] = offenders
        return [], context, "v853-handoff-active-wifi-command-blocked", False
    ...
    if args.command == "plan":
        ...
        return steps, context, "v853-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v853-android-esoc-actor-sample"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v853-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v853-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v853-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v853-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v853-android-esoc-actor-sample":
            v853_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v853-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v853_step_ok:
            return steps, context, "v853-handoff-collector-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v853-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v853-handoff-plan-ready": "execution plan generated without device mutation",
        "v853-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v853-handoff-approval-required": "live run refused because approval flags are missing",
        "v853-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v853-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v853-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v853-handoff-active-wifi-command-blocked": "handoff plan contains a forbidden active Wi-Fi command pattern",
        "v853-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v853-handoff-collector-failed-rollback-complete": "V853 Android actor sampler failed, and native rollback completed",
        "v853-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v853-handoff-readback-failed-rollback-attempted": "Android boot readback failed and native rollback was attempted from recovery",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- v853_out_dir: `{manifest['context']['v853_out_dir']}`",
        "",
        "## Comparison",
        "",
        f"- boot_complete: `{comparison.get('boot_complete', '-')}`",
        f"- v853_decision: `{(comparison.get('v853') or {}).get('decision', '-')}`",
        f"- v853_next_step: `{(comparison.get('v853') or {}).get('next_step', '-')}`",
        "",
        ...
        "- Live mode temporarily flashes Android boot, runs V853 read-only collection, then restores native init.",
        "- No direct eSoC/subsys node open/ioctl; only `/proc/<pid>/fd` symlink inspection.",
...
def main() -> int:
    ...
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v853-handoff-approval-required",
            "v853-handoff-missing-native-rollback",
            "v853-handoff-missing-android-boot",
            "v853-handoff-image-collision",
            "v853-handoff-active-wifi-command-blocked",
        },
