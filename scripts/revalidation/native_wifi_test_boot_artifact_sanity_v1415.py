#!/usr/bin/env python3
"""V1415 local-only sanity verifier for the V1414 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1414-wifi-test-boot-delayed-rc1" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1415-wifi-test-boot-delayed-rc1-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1415_WIFI_TEST_BOOT_DELAYED_RC1_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1414-wifi-test-boot-delayed-rc1-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v286"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.75 (v1414-wifitest)",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1415-wifi-test-boot-delayed-rc1-artifact-sanity-pass",
            True,
            "V1414 delayed-rc1 test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1415-wifi-test-boot-delayed-rc1-artifact-sanity-blocked",
        False,
        "V1414 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1414 manifest: `{manifest['_path']}`",
        f"- V1414 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- Wi-Fi test delayed RC1 contract: `{checks['wifi_test_contract']['ok']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1414-rc1-watcher.result"
            ),
        },
    }
