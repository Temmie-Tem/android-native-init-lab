#!/usr/bin/env python3
"""V2055 rollbackable handoff for TFTP-ready pre-vote ordering."""
...
OUT_DIR = prev2046.prev2035.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2055-tftp-ready-pre-vote-handoff"
)
HANDOFF_DIR = OUT_DIR / "v2054-handoff"
HANDOFF_REPORT = OUT_DIR / "v2054-handoff-report.md"
REPORT_PATH = prev2046.prev2035.prev1998.prev1992.prev.repo_path(
    "docs/reports/NATIVE_INIT_V2055_TFTP_READY_PRE_VOTE_HANDOFF_2026-06-04.md"
)
V2054_OUT = prev2046.prev2035.prev1998.prev1992.prev.repo_path(
    "tmp/wifi/v2054-tftp-ready-pre-vote-pre-wlanmdsp-trigger-test-boot"
)
V2054_INIT = V2054_OUT / "init_v2054_tftp_ready_pre_vote_pre_wlanmdsp_trigger"
V2054_BOOT = V2054_OUT / "boot_linux_v2054_tftp_ready_pre_vote_pre_wlanmdsp_trigger.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2054/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.206 (v2054-tftp-ready-pre-vote-pre-wlanmdsp-trigger)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2054.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2054.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2054-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v391"
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2054_INIT, init_required), (V2054_BOOT, boot_required)):
        key = rel(path)
        forbidden_tokens = init_forbidden if path == V2054_INIT else boot_forbidden
...
def classify(handoff: dict[str, Any],
    ...
    if not route_ok:
        ...
        reason = "V2054 did not preserve rollback, route prerequisites, bridges, passive order timestamps, or long lower window"
        passed = False
    elif wlan0:
    ...
    return {
        **base,
        "label": label,
        "decision": f"v2055-{label}-rollback-{'pass' if passed else 'blocked'}",
        "pass": passed,
        "reason": reason,
        "route_ok": route_ok,
        "hook_ok": hook_ok,
...
def configure_prev2046() -> None:
    ...
    prev2046.V2045_OUT = V2054_OUT
    prev2046.V2045_INIT = V2054_INIT
    prev2046.V2045_BOOT = V2054_BOOT
