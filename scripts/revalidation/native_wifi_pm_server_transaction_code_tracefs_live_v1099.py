#!/usr/bin/env python3
"""V1099 tracefs-only PM server transaction-code classifier.
...
question: which `BnPeripheralManager::onTransact` transaction codes are
executed after the CNSS PM request, before mdm3 stays OFFLINING?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1099-pm-server-transaction-code-tracefs-live")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1099"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1099/pm-cnss-voter-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1099/server-transaction-code-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1099/pm-cnss-voter-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1099-pm-server-transaction-code-tracefs-live.txt")

EVENT_SPECS = (
    ("pm_client_register", "6ec8", ""),
    ("pm_register_connect", "612c", ""),
    ("pm_client_connect", "7544", ""),
    ("pm_client_ack", "76f0", ""),
...
def write_child_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    script = "\n".join([
        ...
        f"echo v1099.child_full_output={shlex.quote(args.child_output)}",
        "echo v1099.child_rc=$rc",
        "exit $rc",
        "",
    ])
...
echo tracefs_uprobe_collector=v1099
...
register_event() {{
  ...
  if $BB test -n "$fetch"; then
    ...
    event_line="p:$GROUP/$label $BIN:0x$offset"
  fi
...
$BB grep -E '^(A90_EXECNS_(END|STDOUT_END)|pm_service_trigger_observer\\.|wifi_vndservice_query\\.|wifi_companion_qrtr_readback\\.|v1099\\.)' "$CHILD_LOG" 2>/dev/null || true
...
def write_collector_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    store.write_text("host/server-transaction-code-tracefs-collector-script.txt", script)
...
def parse_tracefs_output(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "child_rc": keys.get("child.rc", keys.get("v1099.child_rc", "")),
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    collector_step = base.run_a90ctl(
        ...
        store,
        steps,
        "pm-server-transaction-code-tracefs-observer",
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1099-pm-server-transaction-code-tracefs-plan-ready",
            True,
            "plan-only; no tracefs write, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1099 with explicit allow flags",
        )
    v1095_decision = (manifest.get("v1095") or {}).get("decision")
    if v1095_decision not in ALLOWED_V1095_DECISIONS:
        return (
            "v1099-v1095-predecessor-missing",
            False,
            f"unexpected V1095 decision={v1095_decision!r}",
            "rerun or inspect V1095 before PM transaction-code tracefs classifier",
        )
    missing = required_flags(args)
    if missing:
        return (
            "v1099-pm-server-transaction-code-tracefs-approval-required",
            False,
            "missing explicit flags: " + ", ".join(missing),
            "rerun with all V1099 allow flags",
        )
    ...
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1099-execns-helper-sha-mismatch", False, "remote execns helper is not v206", "redeploy helper v206")
    usage = analysis.get("execns_usage") or {}
    if not (usage.get("marker_ok") and usage.get("mode_ok") and usage.get("start_cnss_flag_ok")):
        return ("v1099-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v206")
    if not analysis.get("peripheral_client_visible"):
        return ("v1099-peripheral-client-not-visible", False, "read-only vendor mount did not expose libperipheral_client.so", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1099-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1099-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1099-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1099-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if contract.get("cnss_daemon_start_executed") != "1":
        return ("v1099-cnss-daemon-not-started", False, "child did not execute V1095 CNSS phase", "repair child command before retry")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1099-pm-server-uprobe-no-hit", False, "no libperipheral_client trace lines were captured", "verify shared-library inode mapping or offsets")
    ...
    if per_mgr_server_hits > 0:
        if code_counts:
            return (
                ...
                "map transaction codes to register/connect/vote cases before lower eSoC action",
            )
        return (
            "v1099-pm-server-ontransact-hit-no-code-fields",
            True,
            (
                f"per_mgr_server_hits={per_mgr_server_hits} cnss_hits={cnss_hits} "
                f"mdm3_state={mdm3_state} server_hits={tracefs.get('pm_server_hits_by_comm')}"
            ),
            "repair fetcharg capture or fall back to callsite classifier",
        )
    if server_hits > 0:
        return (
            "v1099-pm-server-ontransact-hit-unmapped-binder-thread",
            True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1099 PM Server Transaction Code Tracefs Live",
        "",
