#!/usr/bin/env python3
"""V1430 local-only sanity verifier for the V1429 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1430-wifi-test-boot-endpoint-prereq-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1430_WIFI_TEST_BOOT_ENDPOINT_PREREQ_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1429-wifi-test-boot-endpoint-prereq-source-build-pass"
...
EXPECTED_BOOT_MARKERS = (
    ...
    "/cache/native-init-wifi-test-boot-v1429.pid",
    "/cache/native-init-wifi-test-boot-v1429-supervisor.pid",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1430-wifi-test-boot-endpoint-prereq-artifact-sanity-pass",
            True,
            "V1429 endpoint-prerequisite test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1430-wifi-test-boot-endpoint-prereq-artifact-sanity-blocked",
        False,
        "V1429 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1429 manifest: `{manifest['_path']}`",
            f"- V1429 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- endpoint sampler contract: `{checks['wifi_test_contract']['ok']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1429-rc1-watcher.result"
                and wifi_test.get("rc1_window_sampler") is True
                and wifi_test.get("rc1_window_result") == "/cache/native-init-wifi-test-boot-v1429-rc1-window.result"
