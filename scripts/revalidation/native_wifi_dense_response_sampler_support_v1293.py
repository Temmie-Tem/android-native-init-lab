#!/usr/bin/env python3
"""V1293 source/build-only gate for the dense dynamic response sampler."""
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v1293-execns-helper-v271-build")
base.DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v271")
base.DEFAULT_V1268 = Path("tmp/wifi/v1292-dynamic-sequence-plan/manifest.json")
base.LATEST_POINTER = Path("tmp/wifi/latest-v1293-execns-helper-v271-build.txt")
base.HELPER_MARKER = "a90_android_execns_probe v271"
base.EXPECTED_V1268_DECISION = "v1292-dense-dynamic-response-sampler-selected"
base.REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v271"',
    "pm_observer_late_per_proxy_dense_response_sampler",
    "--pm-observer-late-per-proxy-dense-response-sampler",
    "late-per-proxy-dense-pinctrl-irq-pcie",
    "response_sampler.dense_enabled=%d",
    "response_sampler.dense_sample_interval_ms=%d",
    "response_sampler.dense_sample_count=%d",
    "response_sampler.dense_window_ms=%d",
    "late_per_proxy_dense_response_sampler ? 40 : 12",
    "late_per_proxy_dense_response_sampler ? 50 : 1000",
    "append_pm_esoc_response_sample",
)
base.REQUIRED_BINARY_STRINGS = (
    ...
    "response_sampler.dense_sample_count=%d",
    "response_sampler.dense_window_ms=%d",
    "gpio_sysfs_write_executed=0",
    "debugfs_control_write_executed=0",
    "subsys_esoc0_direct_open_executed=0",
...
def decide(command: str, rows: list[dict[str, str]], analysis: dict) -> tuple[str, bool, str, str]:
    if command == "plan":
        return ("v1293-dense-response-sampler-build-plan-ready", True, "plan-only", "run V1293 source/build-only gate")
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1293-dense-response-sampler-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1293-dense-response-sampler-build-pass",
        True,
        f"helper v271 built sha256={analysis['build']['stage3']['sha256']}",
        "V1294 should deploy helper v271 only; V1295 should run the bounded dense no-write response sampler",
    )
...
def checks(command: str, analysis: dict) -> list[dict[str, str]]:
    ...
    for row in rows:
        if row.get("name") == "v1268-input":
            row["name"] = "v1292-input"
            row["next_step"] = "rerun V1292 before helper build"
        elif row.get("name") == "source-strings":
            row["next_step"] = "repair helper source dense response sampler support"
        elif row.get("name") == "binary-strings":
            row["next_step"] = "repair built helper dense response sampler strings"
    return rows
...
def render_summary(manifest: dict) -> str:
    ...
    return "\n".join([
        ...
        "- dense cadence: `50 ms`",
        "- dense samples: `40`",
        "- dense window: `2000 ms`",
        "- uses existing `append_pm_esoc_response_sample()` no-write reader",
        "",
        "## Safety",
        "",
