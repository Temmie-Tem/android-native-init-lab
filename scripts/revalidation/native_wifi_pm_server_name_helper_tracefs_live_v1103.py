#!/usr/bin/env python3
"""V1103 tracefs-only PM server name-helper classifier.
...
answers one narrow question: whether helper `0x9538` blocks inside
`pthread_mutex_lock(entry + 0x18)` for the CNSS modem peripheral entry.
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1103-pm-server-name-helper-tracefs-live")
DEFAULT_V1102_MANIFEST = Path("tmp/wifi/v1102-pm-server-early-register-tracefs-live/manifest.json")
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1103"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1103/pm-cnss-voter-child.sh"
DEFAULT_COLLECTOR_SCRIPT = "/cache/a90-runtime/v1103/pm-server-name-helper-tracefs-collector.sh"
DEFAULT_CHILD_OUTPUT = "/cache/a90-runtime/v1103/pm-cnss-voter-output.txt"
LATEST_POINTER = Path("tmp/wifi/latest-v1103-pm-server-name-helper-tracefs-live.txt")
...
def write_child_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    script = "\n".join([
        ...
        f"echo v1103.child_full_output={shlex.quote(args.child_output)}",
        "echo v1103.child_rc=$rc",
        "exit $rc",
        "",
    ])
...
echo tracefs_uprobe_collector=v1103
...
$BB grep -E '^(A90_EXECNS_(END|STDOUT_END)|pm_service_trigger_observer\\.|wifi_vndservice_query\\.|wifi_companion_qrtr_readback\\.|v1103\\.)' "$CHILD_LOG" 2>/dev/null || true
...
def write_collector_script(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> None:
    ...
    store.write_text("host/pm-server-name-helper-tracefs-collector-script.txt", script)
...
def parse_tracefs_output(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "child_rc": keys.get("child.rc", keys.get("v1103.child_rc", "")),
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    collector_step = base.run_a90ctl(
        ...
        store,
        steps,
        "pm-server-name-helper-tracefs-observer",
...
def decide(args: argparse.Namespace, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1103-pm-server-name-helper-tracefs-plan-ready",
            True,
            "plan-only; no tracefs write, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1103 with explicit allow flags",
        )
    v1102_decision = (manifest.get("v1102") or {}).get("decision")
    if v1102_decision not in ALLOWED_V1102_DECISIONS:
        return (
            "v1103-v1102-predecessor-missing",
            False,
            f"unexpected V1102 decision={v1102_decision!r}",
            "rerun or inspect V1102 before PM server name-helper tracefs classifier",
        )
    missing = required_flags(args)
    if missing:
        return (
            "v1103-pm-server-name-helper-tracefs-approval-required",
            False,
            "missing explicit flags: " + ", ".join(missing),
            "rerun with all V1103 allow flags",
        )
    ...
    if not analysis.get("execns_helper", {}).get("ok"):
        return ("v1103-execns-helper-sha-mismatch", False, "remote execns helper is not v206", "redeploy helper v206")
    usage = analysis.get("execns_usage") or {}
    if not (usage.get("marker_ok") and usage.get("mode_ok") and usage.get("start_cnss_flag_ok")):
        return ("v1103-execns-helper-usage-mismatch", False, f"usage={usage}", "redeploy or rebuild helper v206")
    if not analysis.get("peripheral_client_visible"):
        return ("v1103-peripheral-client-not-visible", False, "read-only vendor mount did not expose libperipheral_client.so", "repair synthetic sda29 mount before retry")
    if not analysis.get("pm_service_visible"):
        return ("v1103-pm-service-not-visible", False, "read-only vendor mount did not expose pm-service", "repair synthetic sda29 mount before retry")
    if tracefs.get("result") != "tracefs-uprobe-pass":
        return ("v1103-tracefs-uprobe-failed", False, f"tracefs result={tracefs.get('result')}", "inspect tracefs collector transcript")
    if tracefs.get("register_failures") or tracefs.get("enable_failures") or tracefs.get("cleanup_failures"):
        return ("v1103-tracefs-uprobe-cleanup-review", False, "register/enable/cleanup failures present", "inspect tracefs cleanup before retry")
    if tracefs.get("forbidden_true"):
        return ("v1103-forbidden-action-observed", False, f"forbidden={tracefs.get('forbidden_true')}", "stop and audit helper contract")
    if post.get("forbidden_actor_hits") or post.get("wifi_link_hits"):
        return ("v1103-postflight-safety-review", False, "forbidden actors or Wi-Fi link appeared", "cleanup device before continuing")
    if contract.get("cnss_daemon_start_executed") != "1":
        return ("v1103-cnss-daemon-not-started", False, "child did not execute CNSS phase", "repair child command before retry")
    if tracefs.get("hit_count", 0) <= 0:
        return ("v1103-pm-server-uprobe-no-hit", False, "no tracefs lines were captured", "verify binary inode mappings or offsets")
    ...
    if cnss_register_entries <= 0:
        return ("v1103-cnss-register-entry-missing", False, "cnss-daemon did not enter pm_client_register", "return to V1096 CNSS PM client path gate")
    if cnss_server_register_entries <= 0:
        return (
            "v1103-cnss-server-register-entry-missing",
            True,
            (
    ...
    if cnss_server_label_counts.get("pm_server_register_no_peripheral", 0) > 0:
        return (
            "v1103-cnss-register-no-supported-peripheral",
            True,
    ...
    if cnss_server_label_counts.get("pm_server_register_permission_denied", 0) > 0:
        return (
            "v1103-cnss-register-permission-denied",
            True,
            f"server_labels={cnss_server_label_counts} mdm3_state={mdm3_state}",
    ...
    if not cnss_server_register_returns:
        return (
            f"v1103-cnss-server-register-no-return-at-{last_server_checkpoint}",
            True,
            (
    ...
    if cnss_server_register_returns and not cnss_register_returns:
        return (
            "v1103-cnss-server-returned-client-still-blocked",
            True,
            (
    ...
    if not cnss_register_returns:
        return (
            "v1103-cnss-pm-register-blocks-after-code1-mdm3-still-offline",
            True,
            (
    ...
    if nonzero_cnss_register_returns and cnss_connect_entries <= 0:
        return (
            "v1103-cnss-pm-register-fails-before-connect-mdm3-still-offline",
            True,
            (
    ...
    if zero_cnss_register_returns and cnss_connect_entries <= 0:
        return (
            "v1103-cnss-pm-register-succeeds-but-connect-not-called",
            True,
            (
                f"cnss_register_ret={cnss_register_returns} cnss_connect_entries={cnss_connect_entries} "
                f"server_register_ret={cnss_server_register_returns} mdm3_state={mdm3_state}"
    ...
    if cnss_connect_entries > 0:
        if any(value not in ("0", "0x0") for value in cnss_connect_returns):
            return (
                "v1103-cnss-pm-connect-fails-mdm3-still-offline",
                True,
                (
        ...
        return (
            "v1103-cnss-pm-connect-succeeds-mdm3-still-offline",
            True,
            (
                f"cnss_register_ret={cnss_register_returns} cnss_connect_ret={cnss_connect_returns} "
    ...
    if server_hits > 0:
        return (
            "v1103-pm-server-event-hit-unmapped-binder-thread",
            True,
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    v1102_manifest = load_json(args.v1102_manifest)
    manifest: dict[str, Any] = {
        "cycle": "v1103",
        "generated_at": now_iso(),
        "command": args.command,
        "v1102": {
            "manifest": str(repo_path(args.v1102_manifest)),
            "decision": v1102_manifest.get("decision", ""),
            "pass": bool(v1102_manifest.get("pass")),
        },
        "event_specs": [
    ...
    if (
        ...
        and v1102_manifest.get("decision") in ALLOWED_V1102_DECISIONS
    ):
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1103 PM Server Name Helper Tracefs Live",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1102-manifest", type=Path, default=DEFAULT_V1102_MANIFEST)
