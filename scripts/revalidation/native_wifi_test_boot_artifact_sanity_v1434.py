#!/usr/bin/env python3
"""V1434 local-only sanity verifier for the V1433 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1434-wifi-test-boot-focused-endpoint-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1434_WIFI_TEST_BOOT_FOCUSED_ENDPOINT_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1433-wifi-test-boot-focused-endpoint-source-build-pass"
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1434-wifi-test-boot-focused-endpoint-artifact-sanity-pass",
            True,
            "V1433 focused endpoint test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1434-wifi-test-boot-focused-endpoint-artifact-sanity-blocked",
        False,
        "V1433 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1433 manifest: `{manifest['_path']}`",
            f"- V1433 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- focused endpoint sampler contract: `{checks['wifi_test_contract']['ok']}`",
...
def main() -> int:
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "pid1_rc1_watcher": wifi_test.get("pid1_rc1_watcher"),
            "rc1_watcher_timeout_sec": wifi_test.get("rc1_watcher_timeout_sec"),
            "rc1_watcher_delay_ms": wifi_test.get("rc1_watcher_delay_ms"),
            "rc1_watcher_result": wifi_test.get("rc1_watcher_result"),
            "rc1_window_sampler": wifi_test.get("rc1_window_sampler"),
            "rc1_window_result": wifi_test.get("rc1_window_result"),
            "rc1_endpoint_sampler": wifi_test.get("rc1_endpoint_sampler"),
            "rc1_focused_endpoint_sampler": wifi_test.get("rc1_focused_endpoint_sampler"),
            "rc1_retry_count": wifi_test.get("rc1_retry_count"),
            "rc1_retry_delay_ms": wifi_test.get("rc1_retry_delay_ms"),
            "ok": (
                wifi_test.get("label") == "v1433"
                ...
                and wifi_test.get("pid1_rc1_watcher") is True
                and wifi_test.get("rc1_watcher_timeout_sec") == 45
                and wifi_test.get("rc1_watcher_delay_ms") == 250
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1433-rc1-watcher.result"
                and wifi_test.get("rc1_window_sampler") is True
                and wifi_test.get("rc1_window_result") == "/cache/native-init-wifi-test-boot-v1433-rc1-window.result"
                and wifi_test.get("rc1_endpoint_sampler") is True
                and wifi_test.get("rc1_focused_endpoint_sampler") is True
                and wifi_test.get("rc1_retry_count") == 0
                and wifi_test.get("rc1_retry_delay_ms") == 0
            ),
        },
    }
