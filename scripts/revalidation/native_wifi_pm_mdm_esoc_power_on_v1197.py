#!/usr/bin/env python3
"""V1197 PM observer: subsys_esoc0 open + RELATED restart_level + mdm_helper fd/wchan listing.

V1196 result: GPIO 142 silent, mhi_dev_count=0, PCIe RC1 POLL_COMPLIANCE,
MDM PMIC GPIO not supported. mdm_helper's thread 2 went
esoc_dev_ioctl→SyS_nanosleep at t<10s (ESOC_REQ_IMG received).

v237 adds mdm_helper fd listing and wchan listing to every 10s status:
  pm_observer_mdm_power_on.status.mdm_helper_fds=<interesting-fds>
  pm_observer_mdm_power_on.status.mdm_helper_wchans=<tid:wchan,...>
  pm_observer_mdm_power_on.status.mhi_dev_count=N

This lets us observe:
- Does mdm_helper open /dev/mhi* (MHI path) or /dev/esoc* during retry loop?
- What are all thread wchans after ESOC_REQ_IMG arrives?
- Does /dev/mhi_0305_01.01.00_pipe_10 ever appear?
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1197-pm-mdm-esoc-power-on-mdm-helper-fds")
LATEST_POINTER = Path(
    "tmp/wifi/latest-v1197-pm-mdm-esoc-power-on-mdm-helper-fds.txt"
)
...
DEFAULT_WORK_DIR = "/cache/a90-runtime/v1197"
DEFAULT_CHILD_SCRIPT = "/cache/a90-runtime/v1197/pm-mdm-esoc-power-on-child.sh"
DEFAULT_COLLECTOR_SCRIPT = (
    "/cache/a90-runtime/v1197/pm-mdm-esoc-power-on-collector.sh"
)
DEFAULT_CHILD_OUTPUT = (
    "/cache/a90-runtime/v1197/pm-mdm-esoc-power-on-output.txt"
)
PROOF_PREFIX = "/tmp/a90-v1197-"
...
def pm_mdm_esoc_power_on_v1197_child_command(args: Any) -> list[str]:
...
def decide_v1197(args: Any, manifest: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1197-pm-mdm-esoc-power-on-mdm-helper-fds-plan-ready",
            True,
            "plan-only; no device mutation",
            "deploy helper v237, then run V1197 live gate",
        )
    ...
    if gate.get("begin") != "1":
        return (
            "v1197-vndservice-gate-not-activated",
            False,
    ...
    if not esoc0_found:
        return (
            "v1197-mdm-helper-esoc0-not-found",
            False,
    ...
    if not power_begin:
        return (
            "v1197-mdm-power-on-not-triggered",
            False,
    ...
    if not rl_ok:
        return (
            "v1197-restart-level-set-failed",
            False,
    ...
    # v237: check if mdm_helper opened any MHI fds
    ...
    if not status_entries and not power_on.get("end", ""):
        return (
            "v1197-device-rebooted-before-polling",
            False,
    ...
    if max_crash > 0:
        return (
            "v1197-mdm3-crash-count-nonzero",
            False,
            (
    ...
    if mhi_fds_seen:
        return (
            "v1197-mhi-fd-opened-by-mdm-helper",
            False,
            (
    ...
    # GPIO silent, no crash, no MHI fd
    wchans_sample = [
        e.get("mdm_helper_wchans", "") for e in status_entries[:3]
    ]
    return (
        "v1197-subsys-esoc0-blocked-gpio142-silent",
        False,
        (
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# V1197 PM Observer: subsys_esoc0 + restart_level=RELATED + mdm_helper fd/wchan",
        "",
...
def patch_defaults() -> None:
    ...
    v1183.pm_per_proxy_vndservice_gate_child_command = pm_mdm_esoc_power_on_v1197_child_command
    v1106.pm_cnss_child_command = pm_mdm_esoc_power_on_v1197_child_command
...
def main() -> int:
    ...
    decision, passed, reason, next_step = decide_v1197(args, manifest)
    manifest.update(
