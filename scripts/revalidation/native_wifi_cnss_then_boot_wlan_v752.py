#!/usr/bin/env python3
"""V752 bounded CNSS-daemon then boot_wlan ordering proof.

This runner tests the next gate selected by V751:

    firmware ro mounts -> subsys_modem holder -> QRTR RX wait
      -> lower companions + cnss_diag + cnss-daemon start-only
...
driver_override, module load/unload, esoc0, or qcwlanstate writes.
"""
...
import native_wifi_holder_lower_companion_v733 as lower
import native_wifi_firmware_mounted_modem_holder_v731 as holder
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v752-cnss-then-boot-wlan")
...
DEFAULT_V490_MANIFEST = Path("tmp/wifi/v752-v490-current-run/manifest.json")
DEFAULT_V751_MANIFEST = Path("tmp/wifi/v751-icnss-module-init-classifier/manifest.json")
PROOF_PREFIX = "/tmp/a90-v752-"
...
MARKERS: dict[str, re.Pattern[str]] = {
    **holder.FOCUS_PATTERNS,
    "boot_wlan": re.compile(r"boot_wlan|Wifi Turning On", re.I),
    "qcwlanstate": re.compile(r"qcwlanstate|Modules not initialized", re.I),
    "wlan_loading": re.compile(r"wlan: Loading driver", re.I),
    "wlan_driver_loaded": re.compile(r"wlan: driver loaded", re.I),
    "wlan_driver_failure": re.compile(r"driver load failure|hdd_init failed|wlan driver initialization failed", re.I),
    "hdd_state_major": re.compile(r"wlan_hdd_state wlan major", re.I),
    "icnss_qmi_connected": re.compile(r"icnss_qmi: QMI Server Connected", re.I),
    "fw_ready": re.compile(r"WLAN FW is ready", re.I),
    "wiphy": re.compile(r"ieee80211|wiphy|cfg80211|phy[0-9]+", re.I),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cnss-runtime-sec", type=int, default=DEFAULT_CNSS_RUNTIME_SEC)
    ...
    parser.add_argument("--qrtr-rx-timeout-sec", type=float, default=lower.DEFAULT_QRTR_RX_TIMEOUT_SEC)
    parser.add_argument("--qrtr-rx-poll-sec", type=float, default=lower.DEFAULT_QRTR_RX_POLL_SEC)
    ...
    parser.add_argument("--allow-cnss-then-boot-wlan", action="store_true")
...
def step_payload(steps: list[dict[str, Any]], name: str) -> str:
    return lower.step_payload(steps, name)


def load_json_if_exists(path: Path) -> dict[str, Any]:
    return lower.load_json_if_exists(path)


def int_key(keys: dict[str, str], key: str) -> int:
    return lower.int_key(keys, key)
...
def validate_device_command(args: argparse.Namespace, command: list[str], proof_base: str | None = None) -> None:
    ...
    for term in FORBIDDEN_TERMS:
        if term.lower() in lowered:
            raise RuntimeError(f"forbidden V752 command term {term!r}: {joined}")
    ...
    if command[:2] == ["run", args.wlanbootctl] and len(command) >= 3:
        ...
        if command[2] == "boot-observe" and len(command) == 4:
            ...
            if 5 <= seconds <= 60:
                return
    try:
        lower.validate_device_command(args, command, proof_base)
    except RuntimeError as exc:
        raise RuntimeError(f"unexpected V752 command: {joined}") from exc
...
def marker_summary(text: str) -> dict[str, Any]:
    focus: list[str] = []
    events: dict[str, list[str]] = {name: [] for name in MARKERS}
    for raw_line in text.splitlines():
        line = holder.ANSI_RE.sub("", raw_line).strip()
        if not line:
            continue
        matched = False
    ...
    return {
        ...
        "focus_tail": focus[-260:],
    }


def parse_helper_payload(text: str) -> dict[str, Any]:
    keys = lower.parse_keys(text)
    result = lower.helper_surface(text)
    cnss_diag_started = max(
        int_key(keys, "wifi_hal_composite_start.child.cnss_diag.child_started"),
        int_key(keys, "wifi_companion_start.child.cnss_diag.observable"),
    )
    cnss_daemon_started = max(
        int_key(keys, "wifi_hal_composite_start.child.cnss_daemon.child_started"),
        int_key(keys, "wifi_companion_start.child.cnss_daemon.observable"),
    )
    result["keys"] = keys
    result["cnss_diag"] = max(int(result.get("cnss_diag") or 0), cnss_diag_started)
    result["cnss_daemon"] = max(int(result.get("cnss_daemon") or 0), cnss_daemon_started)
    result["cnss_diag_pid"] = keys.get("wifi_hal_composite_start.child.cnss_diag.pid", "")
    result["cnss_daemon_pid"] = keys.get("wifi_hal_composite_start.child.cnss_daemon.pid", "")
    return result
...
def capture_preflight(args: argparse.Namespace, store: EvidenceStore, steps: list[dict[str, Any]]) -> dict[str, Any]:
    preflight = lower.capture_preflight(args, store, steps)
    ...
    run_step(args, store, steps, "icnss-device-ls-before", ["run", args.toybox, "ls", "-la", "/sys/devices/platform/soc/18800000.qcom,icnss"], 15.0)
    run_step(args, store, steps, "icnss-net-ls-before", ["run", args.toybox, "ls", "-la", "/sys/devices/platform/soc/18800000.qcom,icnss/net"], 15.0)
    run_step(args, store, steps, "icnss-ieee80211-ls-before", ["run", args.toybox, "ls", "-la", "/sys/devices/platform/soc/18800000.qcom,icnss/ieee80211"], 15.0)
    return preflight
...
def preflight_blockers(args: argparse.Namespace,
    ...
    blockers = lower.preflight_blockers(args, steps, preflight, v731, v732, v490)
    ...
    if v751.get("decision") != "v751-boot-wlan-hdd-init-stalls-before-driver-loaded":
        blockers.append("v751-reference-missing")
    if active_process_hits(step_payload(steps, "ps-before")):
        blockers.append("residual-target-process")
    if args.command == "run" and not (args.allow_cnss_then_boot_wlan and args.assume_yes):
        blockers.append("live-cnss-then-boot-wlan-not-approved")
    if not (10 <= args.cnss_runtime_sec <= 30):
        blockers.append("cnss-runtime-out-of-range")
...
def run_live(args: argparse.Namespace,
    ...
    try:
        for name, command, timeout in mountv.build_mount_commands(preflight, proof_base):
            item = run_step(args, store, steps, f"v752-{name}", command, timeout, proof_base)
        ...
        run_step(args, store, steps, "mounted-firmware-class-path", ["cat", lower.FIRMWARE_CLASS_PATH], 10.0, proof_base)
        for path in lower.GLOBAL_MODEM_BLOB_PATHS + lower.WLAN_FIRMWARE_PATHS:
            run_step(args, store, steps, f"mounted-stat-{safe_name(path)}", ["stat", path], 10.0, proof_base)

        dev = lower.parse_dev(step_payload(steps, "subsys-modem-dev"))
        if not dev:
            raise RuntimeError("subsys_modem dev missing after preflight")
        holder = lower.holder_script(args, node, status_file, pid_file, dev[0], dev[1])
        ...
        run_step(args, store, steps, "mss-state-after-holder", ["cat", lower.MSS_STATE], 10.0, proof_base)
        run_step(args, store, steps, "mss-crash-after-holder", ["cat", lower.MSS_CRASH_COUNT], 10.0, proof_base)
        run_step(args, store, steps, "mdm3-state-after-holder", ["cat", lower.MDM3_STATE], 10.0, proof_base)
        run_step(args, store, steps, "mdm3-crash-after-holder", ["cat", lower.MDM3_CRASH_COUNT], 10.0, proof_base)
        qrtr_wait = lower.wait_for_qrtr_rx(args, store, steps, str(before.get("payload") or ""), proof_base)
        if qrtr_wait.get("seen"):
            helper_item = run_step(args, store, steps, "cnss-companion-start-only", helper_command(args), args.cnss_runtime_sec + 75.0, proof_base)
        else:
            helper_item = {
                "name": "cnss-companion-start-only",
                "ok": True,
                "rc": 0,
                "status": "skipped",
                ...
                "file": write_capture(store, "cnss-companion-start-only-skipped", "skipped: QRTR RX marker was not observed\n"),
            }
        ...
        run_step(args, store, steps, "mss-state-after-boot", ["cat", lower.MSS_STATE], 10.0, proof_base)
        run_step(args, store, steps, "mdm3-state-after-boot", ["cat", lower.MDM3_STATE], 10.0, proof_base)
        ...
        delta = lower.dmesg_delta(str(before.get("payload") or ""), str(after.get("payload") or ""))
        write_capture(store, "dmesg-delta", delta)
        markers = marker_summary(delta)
    finally:
        reboot = lower.reboot_and_wait(args, store)

    mounted = mountv.parse_mounts(step_payload(steps, "mounted-proc-mounts"))
    helper = parse_helper_payload(str((helper_item or {}).get("payload") or ""))
    ...
    sys_class_ieee80211 = step_payload(steps, "post-boot-ls-sys-class-ieee80211")
    icnss_net = step_payload(steps, "post-boot-ls-sys-devices-platform-soc-18800000.qcom-icnss-net")
    icnss_ieee = step_payload(steps, "post-boot-ls-sys-devices-platform-soc-18800000.qcom-icnss-ieee80211")
    return {
        "base": proof_base,
        "node": node,
        ...
        "modem_blob_visible": {
            path: lower.path_exists(step_payload(steps, f"mounted-stat-{safe_name(path)}"))
            for path in lower.GLOBAL_MODEM_BLOB_PATHS
        },
        "wlan_firmware_visible": {
            path: lower.path_exists(step_payload(steps, f"mounted-stat-{safe_name(path)}"))
            for path in lower.WLAN_FIRMWARE_PATHS
        },
        ...
        "cnss_companion_executed": bool(qrtr_wait.get("seen")),
        ...
        "qcwlanstate_after_on": "wlanboot.status.qcwlanstate.value=ON" in after_status,
        "qcwlanstate_after_off": "wlanboot.status.qcwlanstate.value=OFF" in after_status,
        "dev_wlan_after": "wlanboot.status.dev_wlan.exists=1" in after_status or "No such file" not in step_payload(steps, "post-boot-ls-dev-wlan"),
        "wlan0_after": "wlan0" in proc_net_dev or "wlan0" in sys_class_net or "wlan0" in icnss_net,
        "wiphy_after": "phy" in sys_class_ieee80211 or "phy" in icnss_ieee,
        "icnss_net_after": "No such file" not in icnss_net and bool(icnss_net.strip()),
        "icnss_ieee80211_after": "No such file" not in icnss_ieee and bool(icnss_ieee.strip()),
        "qrtr_services_after_boot": lower.qrtr_service_counts(step_payload(steps, "post-boot-cat-proc-net-qrtr")),
        "post_boot_files": {
            ...
            "icnss_net": "native/post-boot-ls-sys-devices-platform-soc-18800000.qcom-icnss-net.txt",
            "icnss_ieee80211": "native/post-boot-ls-sys-devices-platform-soc-18800000.qcom-icnss-ieee80211.txt",
        },
...
def build_checks(args: argparse.Namespace,
    ...
    if args.command == "plan":
        return [{
            ...
            "next_step": "run preflight; live run requires --allow-cnss-then-boot-wlan --assume-yes",
        }]
    checks = lower.build_checks(args, steps, preflight, v731, v732, v490, None, blockers)
    for check in checks:
        next_step = check.get("next_step")
        if isinstance(next_step, str):
            check["next_step"] = next_step.replace("V733", "V752")
    checks.append({
        "name": "v751-reference",
        "status": "pass" if v751.get("decision") == "v751-boot-wlan-hdd-init-stalls-before-driver-loaded" else "blocked",
        "detail": {"decision": v751.get("decision"), "pass": v751.get("pass")},
        "next_step": "complete V751 before V752",
    })
    checks.append({
        ...
        "next_step": "deploy expected a90_wlanbootctl before V752 live",
    })
    if args.command == "run":
        checks.append({
            "name": "live-approval",
            "status": "pass" if args.allow_cnss_then_boot_wlan and args.assume_yes else "blocked",
            "detail": {"allow_cnss_then_boot_wlan": args.allow_cnss_then_boot_wlan, "assume_yes": args.assume_yes},
            "next_step": "rerun with explicit V752 gate flags",
        })
    if not live:
        return checks
    ...
    forbidden_helper_values = {
        key: helper.get(key)
        for key in ("service_manager", "wifi_hal", "wificond", "scan_connect_linkup", "external_ping", "qmi_attempted")
    }
    checks.extend([
        {
        ...
        },
        {
            ...
            "next_step": "if missing, compare V731/V733 current deltas before retry",
        },
        {
            "name": "cnss-companion-contract",
            ...
            "detail": {"mode": helper.get("mode"), "order": helper.get("order"), "child_started": helper.get("child_started"), "all_observable": helper.get("all_observable"), "all_postflight_safe": helper.get("all_postflight_safe"), "cnss_diag": helper.get("cnss_diag"), "cnss_daemon": helper.get("cnss_daemon"), "result": helper.get("result")},
        ...
        },
        {
            ...
            "next_step": "stop if helper crossed into service-manager/HAL/connect or QMI payload",
        },
        {
            "name": "boot-wlan-observe-after-cnss",
        ...
        {
            "name": "driver-readiness-progression",
            "status": "pass" if live.get("wlan0_after") or live.get("wiphy_after") or live.get("icnss_net_after") or counts.get("wlan_driver_loaded", 0) or counts.get("icnss_qmi_connected", 0) or counts.get("fw_ready", 0) or counts.get("wlfw", 0) or counts.get("bdf", 0) or (live.get("qrtr_services_after_boot") or {}).get("69", 0) else "finding",
            "detail": {"markers": {key: counts.get(key, 0) for key in ("wlan_loading", "hdd_state_major", "wlan_driver_loaded", "wlan_driver_failure", "icnss_qmi_connected", "fw_ready", "wlfw", "bdf", "wlan0", "wiphy", "qcwlanstate", "kernel_warning")}, "qrtr_services": live.get("qrtr_services_after_boot"), "wlan0_after": live.get("wlan0_after"), "wiphy_after": live.get("wiphy_after"), "icnss_net_after": live.get("icnss_net_after")},
            "next_step": "if no progression, choose deeper HDD/PLD instrumentation rather than repeating this ordering",
        },
        {
...
def decide(args: argparse.Namespace, checks: list[dict[str, Any]], live: dict[str, Any] | None) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v752-cnss-then-boot-wlan-plan-ready",
            True,
    ...
    if blocked:
        return (
            "v752-cnss-then-boot-wlan-blocked",
            False,
    ...
    if not live:
        return (
            "v752-cnss-then-boot-wlan-preflight-ready",
            True,
            "preflight ready; live proof remains below service-manager/HAL/connect",
    ...
    if live.get("wlan0_after") or live.get("wiphy_after"):
        return (
            "v752-cnss-then-boot-wlan-netdev-appeared",
            True,
            "CNSS then boot_wlan ordering produced wlan0/wiphy evidence without service-manager, HAL, scan/connect, credentials, DHCP, routes, or external ping",
    ...
    if counts.get("wlan_driver_loaded", 0) or counts.get("icnss_qmi_connected", 0) or counts.get("fw_ready", 0) or counts.get("wlfw", 0) or counts.get("bdf", 0) or services.get("69", 0):
        return (
            "v752-cnss-then-boot-wlan-driver-advanced",
            True,
            "CNSS then boot_wlan ordering advanced the WLAN driver/WLFW path but no wlan0 appeared",
    ...
    if counts.get("wlan_loading", 0) or counts.get("hdd_state_major", 0) or counts.get("qcwlanstate", 0):
        return (
            "v752-cnss-then-boot-wlan-hdd-init-still-stalls",
            True,
            "CNSS then boot_wlan ordering still reaches only HDD init/qcwlanstate; driver-loaded/QMI/FW-ready/netdev remain absent",
            "next gate should instrument HDD/PLD init prerequisites; do not repeat ordering blindly",
        )
    return (
        "v752-cnss-then-boot-wlan-no-trigger-progress",
        True,
        "CNSS then boot_wlan ordering executed but produced no WLAN trigger markers",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    state_rows = [
        ...
        ["helper", json.dumps({key: helper.get(key) for key in ("mode", "order", "child_started", "all_observable", "all_postflight_safe", "cnss_diag", "cnss_daemon", "service_manager", "wifi_hal", "result", "qmi_attempted")}, sort_keys=True)],
        ["boot_wlan", json.dumps({key: live.get(key) for key in ("boot_wlan_write_executed", "boot_wlan_ok", "boot_wlan_status", "qcwlanstate_after_on", "qcwlanstate_after_off", "dev_wlan_after", "icnss_net_after", "icnss_ieee80211_after", "wlan0_after", "wiphy_after")}, sort_keys=True)],
    ...
    marker_names = ("qrtr_rx", "qrtr_tx", "sysmon_qmi", "service_notifier", "wlan_pd", "mhi", "qca6390", "wlfw", "bdf", "wlan0", "boot_wlan", "qcwlanstate", "wlan_loading", "hdd_state_major", "wlan_driver_loaded", "wlan_driver_failure", "icnss_qmi_connected", "fw_ready", "wiphy", "kernel_warning")
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v751 = load_json_if_exists(args.v751_manifest)
    preflight: dict[str, Any] = {}
    live: dict[str, Any] | None = None
    blockers: list[str] = []
    if args.command != "plan":
        preflight = capture_preflight(args, store, steps)
        blockers = preflight_blockers(args, steps, preflight, v731, v732, v490, v751)
        if args.command == "run" and not blockers:
            live = run_live(args, store, steps, preflight)
    checks = build_checks(args, steps, preflight, v731, v732, v490, v751, live, blockers)
    ...
    manifest: dict[str, Any] = {
        ...
        "v751": {"decision": v751.get("decision"), "pass": v751.get("pass"), "path": v751.get("path", str(repo_path(args.v751_manifest)))},
        "preflight": preflight,
        "steps": steps,
        "checks": checks,
        "live": live or {},
        ...
        "lower_companion_start_executed": bool((live or {}).get("cnss_companion_executed")),
        "cnss_diag_start_executed": bool(int(helper.get("cnss_diag") or 0)),
        "cnss_daemon_start_executed": bool(int(helper.get("cnss_daemon") or 0)),
        "boot_wlan_write_executed": bool((live or {}).get("boot_wlan_write_executed")),
        "daemon_or_hal_start_executed": bool(int(helper.get("cnss_daemon") or 0)),
...
def main() -> int:
    ...
    latest = repo_path("tmp/wifi/latest-v752-cnss-then-boot-wlan.txt")
