#!/usr/bin/env python3
"""V1337 source/build-only gate for Android-order pre-CNSS provider observer support."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1337-android-pre-cnss-provider-support")
DEFAULT_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_STAGE3_BINARY = Path("stage3/linux_init/helpers/a90_android_execns_probe_v278")
DEFAULT_BUILD_SCRIPT = Path("scripts/revalidation/build_android_execns_probe_helper.sh")
LATEST_POINTER = Path("tmp/wifi/latest-v1337-android-pre-cnss-provider-support.txt")
HELPER_MARKER = "a90_android_execns_probe v278"
MODE = "wifi-companion-android-order-pre-cnss-provider-observe-only"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v278"',
    MODE,
    "is_wifi_companion_android_order_pre_cnss_provider_observe_only_mode",
    "android_order_pre_cnss_provider_observer",
    "servicemanager,hwservicemanager,vndservicemanager,per_proxy_helper,qrtr_ns,rmt_storage,tftp_server,pd_mapper,per_mgr,per_proxy,mdm_helper,cnss_diag,cnss_daemon",
    ...
    rows = [
        {
            ...
            "next_step": "repair V1337 helper source support",
        },
        {
...
def decide(command: str, rows: list[dict[str, str]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if command == "plan":
        return (
            "v1337-android-pre-cnss-provider-build-plan-ready",
            True,
            "plan-only",
            "run V1337 source/build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return (
            "v1337-android-pre-cnss-provider-build-blocked",
            False,
    ...
    return (
        "v1337-android-pre-cnss-provider-build-pass",
        True,
        f"helper v278 built sha256={stage3['sha256']}",
        "V1338 should deploy helper v278; V1339 should run bounded Android-order pre-CNSS provider observe-only live",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1337 Android-order Pre-CNSS Provider Observer Build",
        "",
        ...
        "",
        "- starts the Android-order pre-CNSS provider chain: service-manager trio, `pm_proxy_helper`, QRTR/RFS/pd-mapper companions, `pm-service`, `pm-proxy`, `mdm_helper`, `cnss_diag`, and `cnss-daemon`",
        "- emits `wifi_companion_start.android_pre_cnss_provider_observe_only=1` and `wifi_companion_start.subsys_esoc0_manual_open_attempted=0`",
        "- captures `mdm_helper_queue_timing.android_pre_cnss_provider_window.*` for PM/MDM helper fd and process evidence",
        "",
        "## Safety",
        "",
        "- source/build-only; no deploy or device command",
        "- no manual eSoC open/ioctl/notify/BOOT_DONE, PMIC write, GPIO request/hold, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write",
        "",
    ])
