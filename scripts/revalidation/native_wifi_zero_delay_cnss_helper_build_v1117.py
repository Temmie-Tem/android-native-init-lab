#!/usr/bin/env python3
"""V1117 source/build-only gate for zero-delay CNSS after per_mgr start."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v1117-execns-helper-v211-build")
...
DEFAULT_V1116_MANIFEST = Path("tmp/wifi/v1116-global-holder-immediate-cnss-live/manifest.json")
LATEST_POINTER = Path("tmp/wifi/latest-v1117-execns-helper-v211-build.txt")
HELPER_MARKER = "a90_android_execns_probe v211"
EXPECTED_V1116_DECISION = "v1116-immediate-cnss-pm-register-path-reached"
REQUIRED_SOURCE_STRINGS = (
    'EXECNS_VERSION "a90_android_execns_probe v211"',
    "pm_observer_start_cnss_zero_delay_after_per_mgr",
...
def analyze(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    v1116 = load_json(args.v1116_manifest)
    analysis: dict[str, Any] = {
        "v1116": {
            "decision": v1116.get("decision", ""),
            "pass": bool(v1116.get("pass")),
            "path": str(repo_path(args.v1116_manifest)),
        },
        "source": {
            "path": str(source),
...
def checks(args: argparse.Namespace, analysis: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "name": "v1116-input",
            "status": "pass" if analysis["v1116"]["decision"] == EXPECTED_V1116_DECISION and analysis["v1116"]["pass"] else "blocked",
            "detail": f"decision={analysis['v1116']['decision']} pass={analysis['v1116']['pass']}",
            "next_step": "refresh V1116 before building helper v211",
        },
        {
            "name": "source-strings",
            ...
            "next_step": "repair helper source zero-delay-CNSS support",
        },
    ]
    if args.command == "plan":
        rows.append({"name": "plan-only", "status": "pass", "detail": "no build or device command executed", "next_step": "run V1117 build-only gate"})
        return rows
...
def decide(args: argparse.Namespace, rows: list[dict[str, Any]], analysis: dict[str, Any]) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v1117-zero-delay-cnss-helper-build-plan-ready",
            True,
            "plan-only; no build, deploy, device command, PM actor, CNSS actor, or Wi-Fi action executed",
            "run V1117 build-only gate",
        )
    blockers = [row["name"] for row in rows if row["status"] != "pass"]
    if blockers:
        return ("v1117-zero-delay-cnss-helper-build-blocked", False, "blocked by " + ", ".join(blockers), "fix blockers before deploy")
    return (
        "v1117-zero-delay-cnss-helper-build-pass",
        True,
        f"helper v211 built sha256={analysis['build']['output_sha256']}",
        "V1118 should deploy helper v211 and run the combined global-holder zero-delay-CNSS live gate",
    )
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V1117 Zero-delay CNSS Helper Build",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1116-manifest", type=Path, default=DEFAULT_V1116_MANIFEST)
