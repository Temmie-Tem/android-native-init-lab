#!/usr/bin/env python3
"""V690 service74-gated PeripheralManager provider + CNSS retry proof.

This proof replays the current service74-positive CNSS retry path with helper
v115. It inserts the Android PeripheralManager provider pair (`pm-service` and
...
base.DEFAULT_OUT_DIR = base.Path("tmp/wifi/v690-peripheral-manager-cnss-retry")
base.DEFAULT_HELPER_SHA256 = "60d8ca3c5e652b4f68c519613f10fb91c582a49cb3187ba301f29d5c7027c2fb"
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v115"
base.DEFAULT_V490_MANIFEST = base.Path("tmp/wifi/v690-v490-current-run/manifest.json")
...
V690_MODE = "wifi-companion-service74-gated-peripheral-manager-cnss-retry-start-only"
EXPECTED_ORDER = (
...
def build_checks(args: base.argparse.Namespace,
    ...
    helper_ready = (
        ...
        and all(token in usage for token in V690_USAGE_TOKENS)
    )
    base.add_check(
        checks,
        "helper-v115-peripheral-manager-cnss-retry-contract",
        "pass" if helper_ready else "blocked",
        "blocker",
        "remote helper must expose v115 service74-gated PeripheralManager/CNSS retry mode",
        [
            line
            for line in (sha_text + "\n" + usage).splitlines()
            if args.helper_sha256 in line
            or args.helper_marker in line
            or V690_MODE in line
        ...
        "deploy helper v115 before V690 live proof",
    )
    return checks
...
def companion_command(args: base.argparse.Namespace) -> list[str]:
    ...
    _set_option(command, "--mode", V690_MODE)
    _set_option(command, "--property-root", V535_PROPERTY_ROOT)
    if len(command) > MAX_CMDV1_COMMAND_ARGS:
        raise RuntimeError(f"V690 helper command has {len(command)} args; max safe args={MAX_CMDV1_COMMAND_ARGS}")
    return command
...
def run_live(args: base.argparse.Namespace,
    ...
    live["v690_peripheral_manager_surface"] = surface
    live["v690_peripheral_manager_ready"] = _provider_ready(surface)
    live["v690_property_shim_surface"] = _property_shim_surface(keys)
    return live
...
def decide(args: base.argparse.Namespace,
    ...
    if args.command == "plan":
        return (
            "v690-peripheral-manager-cnss-retry-plan-ready",
            True,
            "plan-only; no device command executed",
            "deploy helper v115, refresh current-boot prerequisites, then run V690 preflight/live",
            False,
        )
    blocked = base.blockers(checks)
    if blocked:
        return "v690-peripheral-manager-cnss-retry-blocked", False, "blocked by " + ", ".join(blocked), "resolve blockers before V690", False
    ...
    if not base.approved(args):
        return "v690-peripheral-manager-cnss-retry-approval-required", True, "exact approval phrase required; no live command executed", "rerun with exact V690 approval", False
    ...
    if not reboot.get("version_seen") or not reboot.get("status_healthy"):
        return "v690-cleanup-review", False, f"reboot_cleanup={reboot}", "verify device manually before continuing", live_executed
    if not live.get("holder_started") or not (live.get("qrtr_rx_wait") or {}).get("seen"):
        return "v690-lower-modem-blocked", False, "subsys_modem holder did not reproduce QRTR RX", "restore lower modem readiness before V690", live_executed
    if not live.get("companion_executed"):
        return "v690-companion-skipped", False, "companion was skipped by QRTR gate", "inspect QRTR wait evidence", live_executed

    surface = live.get("v690_peripheral_manager_surface") or {}
    ...
    property_surface = live.get("v690_property_shim_surface") or {}

    if service74_gate.get("seen") != "1" or service74_gate.get("open") != "1":
        return (
            "v690-service74-gate-timeout",
            True,
    ...
    if surface.get("order") != EXPECTED_ORDER or surface.get("peripheral_manager_enabled") != "1":
        return (
            "v690-helper-order-contract-gap",
            False,
            f"surface={surface}",
            "fix helper v115 provider order contract before another live attempt",
            live_executed,
        )
    if vnd_ready.get("ready") != "1" or cnss_retry.get("initial_cleanup_safe") != "1":
        return (
            "v690-vnd-or-initial-cnss-cleanup-gap",
            True,
    ...
    if not _provider_ready(surface):
        return (
            ...
            True,
            f"surface={surface}",
            "property ack held; inspect pm-service/pm-proxy post-property runtime output before Wi-Fi HAL start",
            live_executed,
        )
    if not cnss_retry.get("retry_start_order"):
        return (
            "v690-provider-ready-cnss-retry-withheld",
            True,
            f"surface={surface}",
    ...
    return (
        "v690-peripheral-manager-cnss-retry-gap-persists",
        True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    text = _v668_render_summary(manifest).replace(
        ...
        "# V690 PeripheralManager Provider CNSS Retry Proof",
        1,
    )
    ...
    return "\n".join([
        text,
        "",
        "## V690 PeripheralManager Contract",
        "",
        ...
        f"- peripheral_manager_ready: `{live.get('v690_peripheral_manager_ready', '')}`",
...
def build_manifest(args: base.argparse.Namespace, store: base.EvidenceStore) -> dict[str, Any]:
    ...
    manifest["cycle"] = "v690"
    manifest["helper_version"] = "v115"
    manifest["property_root"] = V535_PROPERTY_ROOT
    manifest["peripheral_manager_mode"] = V690_MODE
    manifest["expected_order"] = EXPECTED_ORDER
    manifest["peripheral_manager_surface"] = live.get("v690_peripheral_manager_surface") if live else {}
    manifest["peripheral_manager_ready"] = bool(live.get("v690_peripheral_manager_ready")) if live else False
    manifest["context_repair_regressed"] = _context_repair_regressed(live.get("v690_peripheral_manager_surface") or {}) if live else False
    manifest["property_shim_surface"] = live.get("v690_property_shim_surface") if live else {}
    manifest["property_ack_regressed"] = _property_ack_regressed(live.get("v690_property_shim_surface") or {}) if live else False
    ...
    manifest["explicitly_approved"] = [
        "helper v115 service74-gated PeripheralManager provider start-only mode",
