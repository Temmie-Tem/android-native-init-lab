#!/usr/bin/env python3
"""V831 bounded early-window service-notifier REGISTER_LISTENER probe.

V829 proved that service-locator maps `wlan/fw` to `msm/modem/wlan_pd`
instance 180.  V831 sends only the corresponding service-notifier
REGISTER_LISTENER request to service 66 / encoded instance 46081, then keeps the listener open in the lower companion window to parse the response and any STATE_UPDATED indication.  It does not start
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v831-service-notifier-early-listener-probe")
LATEST_POINTER = Path("tmp/wifi/latest-v831-service-notifier-early-listener-probe.txt")
...
DEFAULT_HELPER_SHA256 = "30a509d500a8c887c1fb43c506c86aa2bf3b450bb770043d91a38a9d11dddfb8"
DEFAULT_HELPER_MARKER = "a90_android_execns_probe v128"
...
DEPLOY_APPROVAL = "approve v831 deploy execns helper v128 only; no daemon start and no Wi-Fi bring-up"
PROOF_PREFIX = "/tmp/a90-v831-"
...
def deploy_helper(args: argparse.Namespace, store: EvidenceStore, command: str) -> dict[str, Any]:
    ...
    if command == "run":
        ...
        timeout = 240.0
    result = run_host(store, f"v831-deploy-{command}", cmd, timeout)
...
def configure_v831(args: argparse.Namespace) -> tuple[Any, Any]:
    v817.configure_base()
...
def run_live(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    live_store = EvidenceStore(store.path("live-v817-v128-service-notifier-listener"))
    live_store.mkdir("native")
    live_store.mkdir("host")
    original_v735_helper, original_base_helper = configure_v831(args)
    try:
    ...
    for step in live_manifest.get("steps", []):
        if step.get("name") == "lower-companion-start-only":
            ...
            break
    live_manifest["v831_matrix"] = v821.summarize_matrix(helper_payload)
    live_manifest["v831_service_notifier"] = summarize_service_notifier(helper_payload)
    live_store.write_json("manifest-v831-annotated.json", live_manifest)
    live_store.write_text("summary-v831.md", v817.render_summary(live_manifest))
    return {
        "manifest": str(live_store.run_dir / "manifest-v831-annotated.json"),
        "decision": live_manifest.get("decision"),
        "pass": live_manifest.get("pass"),
        "live": live_manifest.get("live", {}),
        "matrix": live_manifest["v831_matrix"],
        "service_notifier": live_manifest["v831_service_notifier"],
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "detail": v829,
            "next_step": "complete V829 domain-list proof before V831",
        },
        {
            "name": "local-helper-v128",
            "status": "pass" if (
                local["exists"]
            ...
            "detail": local,
            "next_step": "rebuild helper v128 before live service-notifier probe",
        },
        {
            ...
            "next_step": "continue only inside V831 guardrails",
        },
    ]
    if args.command == "plan":
        return checks
    checks.append({
        "name": "helper-v128-ready" if args.command == "preflight" else "helper-v128-deploy-run",
        "status": "pass" if deploy and deploy.get("pass") else "blocked",
        "detail": deploy or {},
        "next_step": "fix helper v128 readiness before service-notifier probe",
    })
...
def decide(args: argparse.Namespace, checks: list[dict[str, Any]], live: dict[str, Any] | None) -> tuple[str, bool, str, str]:
    if args.command == "plan":
        return (
            "v831-service-notifier-early-listener-plan-ready",
            True,
            "plan-only; helper v128 bounded service-notifier listener gate defined without live action",
            "run V831 preflight, then live listener probe if clean",
        )
    blocked = [check["name"] for check in checks if check["status"] == "blocked"]
    if blocked:
        return (
            "v831-service-notifier-early-listener-blocked",
            False,
    ...
    if args.command == "preflight":
        return (
            "v831-service-notifier-early-listener-preflight-ready",
            True,
            "preflight passed; helper v128 deploy/live listener probe remains gated",
            "run V831 live bounded service-notifier listener probe",
        )
    servnotif = (live or {}).get("service_notifier", {})
    up_seen = (
    ...
    if servnotif.get("response_success") == 1 and up_seen:
        return (
            "v831-service-notifier-early-listener-state-up",
            True,
    ...
    if servnotif.get("response_success") == 1:
        return (
            "v831-service-notifier-early-listener-state-not-up",
            True,
    ...
    if servnotif.get("response_seen") == 1:
        return (
            "v831-service-notifier-early-listener-response-error",
            True,
    ...
    return (
        "v831-service-notifier-early-listener-no-response",
        True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V831 Service-notifier Listener Probe",
        "",
