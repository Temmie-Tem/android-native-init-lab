#!/usr/bin/env python3
"""V436 Android Wi-Fi disabled persistence handoff.

This wrapper temporarily boots Android, waits for `sys.boot_completed=1`, runs
the read-only V436 disabled persistence check, then restores the native init
boot image.  It does not enable/disable Wi-Fi, scan, connect, mutate routing,
send probes, expose servers, start daemons, write sysfs/rfkill, or change
credentials.
"""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v436-android-wifi-disabled-persistence-handoff")
...
def v436_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v436-android-wifi-disabled-persistence-run"


def v436_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    ...
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            v436_command = [
                ...
                "--out-dir",
                str(v436_out_dir(store)),
                "--adb",
                args.adb,
            ]
            if args.serial:
                v436_command.extend(["--serial", args.serial])
            v436_command.append("run")
            plan.append(("v436-android-wifi-disabled-persistence", v436_command, args.timeout * 10))
        else:
...
def load_v436_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v436_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v436 = load_v436_result(store)
    classification = v436.get("classification") or {}
    sample = classification.get("sample") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v436-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass"
    elif not v436.get("pass"):
        decision = str(v436.get("decision") or "v436-handoff-persistence-failed-rollback-complete")
        pass_ok = False
        reason = str(v436.get("reason") or "V436 persistence check did not pass")
    else:
        decision = str(v436.get("decision") or "v436-handoff-persistence-pass")
        pass_ok = True
        reason = str(v436.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "state": {
            ...
            "disabled": classification.get("disabled"),
            "no_wlan_ip": classification.get("no_wlan_ip"),
        ...
        "v436": {
            "path": v436.get("_path"),
            "decision": v436.get("decision"),
            "pass": v436.get("pass"),
            "reason": v436.get("reason"),
        },
    }
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v436_out_dir": str(v436_out_dir(store)),
    }
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v436-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v436-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v436-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v436-handoff-approval-required", False

    plan = v436_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_plan_command(plan)
    if offenders:
        context["forbidden_plan_commands"] = offenders
        return [], context, "v436-handoff-forbidden-command-blocked", False

    steps: list[StepResult] = []
    if args.command == "plan":
        for name, command, _ in plan:
            steps.append(write_step(store, name, command, "[plan] not executed\n", "", 0, 0.0, skipped=True, ok_override=True))
        return steps, context, "v436-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v436-android-wifi-disabled-persistence"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v436-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v436-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v436-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v436-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v436-android-wifi-disabled-persistence":
            v436_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v436-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v436_step_ok:
            return steps, context, "v436-handoff-persistence-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v436-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v436-handoff-plan-ready": "execution plan generated without device mutation",
        "v436-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v436-handoff-approval-required": "live run refused because boot/rollback approval flags are missing",
        "v436-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v436-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v436-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v436-handoff-forbidden-command-blocked": "handoff plan contains a forbidden Wi-Fi/server/probe command",
        "v436-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v436-handoff-persistence-failed-rollback-complete": "V436 persistence check failed, and native rollback completed",
        "v436-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v436-handoff-readback-failed-rollback-attempted": "Android boot readback failed and native rollback was attempted from recovery",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    state_rows = [[key, str(value)] for key, value in sample.items()]
    for key in ("disabled", "no_wlan_ip", "route_absent", "connectivity_absent", "listener_safe", "next_gate"):
        state_rows.append([key, str(state.get(key, "-"))])
    return "\n".join(
        [
            "# V436 Boot-complete Android Wi-Fi Disabled Persistence Handoff",
            "",
            ...
            f"- v436_out_dir: `{manifest['context']['v436_out_dir']}`",
            ...
            f"- v436_decision: `{(comparison.get('v436') or {}).get('decision', '-')}`",
            "",
            ...
            "- V436 is read-only after Android boot-complete; it does not issue another Wi-Fi disable command.",
            "- No Wi-Fi enable/disable, scan/connect, credentials, server exposure, external packet probes, DHCP/routing mutation, rfkill/sysfs write, module load/unload, or property mutation.",
            "",
        ]
    )
...
def main() -> int:
    ...
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v436-handoff-approval-required",
            "v436-handoff-missing-native-rollback",
            "v436-handoff-missing-android-boot",
            "v436-handoff-image-collision",
