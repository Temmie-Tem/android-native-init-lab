#!/usr/bin/env python3
"""V411 bounded binderized Wi-Fi HAL registration query runner.

V411 is the next gate after the V408 service-surface packet.  It requires
helper v27 and uses the helper-owned private namespace to start only:

* servicemanager
* hwservicemanager
* first Wi-Fi HAL candidate
* a bounded `/system/bin/lshal list --types=binderized --neat` query child
...
base.DEFAULT_OUT_DIR = Path("tmp/wifi/v411-binderized-hal-registration-query-runner")
base.DEFAULT_HELPER_SHA256 = "0519b557482f347d47962e9da76ee7afcce270bf12df860d37678e9a26bf2c74"
base.DEFAULT_V404 = Path("tmp/wifi/v408-hal-registration-surface-packet-20260520-102249/manifest.json")
base.HELPER_LABEL = "v27"
...
def build_plan(args: base.argparse.Namespace) -> dict[str, Any]:
    ...
    plan["arg_budget_note"] = (
        "helper v27 defaults wifi-hal-composite-lshal-binderized-list to private-empty "
        "so the approved command keeps the 30-arg native shell limit"
    )
    return plan
...
def build_checks(args: base.argparse.Namespace, store: base.EvidenceStore, steps: list[base.Step],
    ...
    if args.command == "plan":
        base.add_check(checks, "plan-only", "pass", "info", "no device command executed", [], "run preflight after helper v27 deploy")
        return checks
    ...
    helper_ready = (
        ...
        and "a90_android_execns_probe v27" in helper_usage
        and "wifi-hal-composite-lshal-binderized-list" in helper_usage
        and "--allow-hal-service-query" in helper_usage
        and "--types=binderized" in helper_usage
        and "--neat" in helper_usage
    )
    ...
    base.add_check(
        ...
        [str(v408.get("path", ""))],
        "V408 evidence packet must pass before V411 binderized registration query",
    )
    ...
    base.add_check(checks, "helper-v27", "pass" if helper_ready else "blocked", "blocker", "remote helper must be v27 with binderized lshal query mode", [line for line in helper_sha.splitlines() if args.helper in line][:2], "deploy helper v27 before V411 live query")
    base.add_check(checks, "lshal-binary", "pass" if base.step_ok(steps, "stat-lshal") else "blocked", "blocker", "/mnt/system/system/bin/lshal must exist for binderized registration listing", [], "if absent, route to V412 micro HIDL client or Android-side lshal extraction")
    ...
    base.add_check(checks, "approval-gate", "pass" if base.approved(args) else "needs-operator", "approval", f"phrase_match={args.approval_phrase == base.APPROVAL_PHRASE} apply={args.apply} assume_yes={args.assume_yes}", [base.APPROVAL_PHRASE], "exact phrase and flags required before V411 binderized live query")
    return checks
...
def decide(args: base.argparse.Namespace, checks: list[base.Check], live_result: dict[str, Any] | None,
    ...
    if args.command == "plan":
        return "v411-hal-registration-query-plan-ready", True, "plan-only; no device command executed", "run preflight after helper v27 deploy", False
    blocked = base.blockers(checks)
    if blocked:
        return "v411-hal-registration-query-blocked", False, "blocked before live run by " + ", ".join(blocked), "resolve blockers before approval", False
    if args.command == "preflight":
        return "v411-hal-registration-query-preflight-ready", True, "read-only preflight is ready; live query still needs approval", "operator may approve exact V411 phrase", False
    if not base.approved(args):
        return "v411-hal-registration-query-approval-required", True, "exact approval phrase required; no device command executed", "rerun with exact approval if intended", False
    if not live_result or not post or not post["clean"]:
        return "v411-hal-registration-query-review-required", False, "live result or postflight cleanliness missing", "inspect evidence and consider recovery reboot", True
    ...
    if helper_result == "service-query-pass" and query_result == "service-query-pass" and live_result.get("all_postflight_safe") and post.get("clean"):
        return "v411-hal-registration-query-pass", True, "binderized lshal query completed while composite HAL trio stayed bounded and cleaned", "classify binderized Wi-Fi HAL service and plan next gate", True
    if helper_result == "service-query-tool-missing":
        return "v411-hal-registration-query-tool-missing", True, "lshal is unavailable in this system image", "route to V412 micro HIDL client or Android-side lshal extraction", True
    if helper_result == "service-query-runtime-gap":
        return "v411-hal-registration-query-runtime-gap", True, f"service query failed: {live_result.get('service_query_reason')}", "classify binderized lshal/HIDL query failure before wider Wi-Fi work", True
    return "v411-hal-registration-query-review-required", False, f"helper_result={helper_result} query_result={query_result}", "inspect helper output before widening scope", True
...
def refusal_manifest(args: base.argparse.Namespace, v408: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "next_step": "rerun with exact approval only after helper v27 deploy and preflight",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# V411 Binderized Wi-Fi HAL Registration Query Runner",
        "",
