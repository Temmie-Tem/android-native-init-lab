#!/usr/bin/env python3
"""V1409 local-only sanity verifier for the V1408 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1408-wifi-test-boot-pid1-rc1-watcher" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1409-wifi-test-boot-pid1-rc1-watcher-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1409_WIFI_TEST_BOOT_PID1_RC1_WATCHER_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1408-wifi-test-boot-pid1-rc1-watcher-source-build-pass"
EXPECTED_HELPER = "a90_android_execns_probe_v286"
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.73 (v1408-wifitest)",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1409-wifi-test-boot-pid1-rc1-watcher-artifact-sanity-pass",
            True,
            "V1408 PID1 RC1 watcher test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1409-wifi-test-boot-pid1-rc1-watcher-artifact-sanity-blocked",
        False,
        "V1408 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1408 manifest: `{manifest['_path']}`",
        f"- V1408 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- Wi-Fi test PID1 RC1 watcher contract: `{checks['wifi_test_contract']['ok']}`",
    ...
    checks: dict[str, Any] = {
        ...
        "wifi_test_contract": {
            ...
            "ok": (
                ...
                and wifi_test.get("rc1_watcher_result") == "/cache/native-init-wifi-test-boot-v1408-rc1-watcher.result"
            ),
        },
    }
