#!/usr/bin/env python3
"""V1049 bounded PM full-contract live proof with helper v178 modem pre-holder."""
...
HELPER_SHA256_V178 = "7df75c618f58d599ece1a6017f66040aff57badb8955a70e07de2a77a3561c75"
SERVICE_MANAGER_ORDER = "after-mdm-helper-esoc-fd-with-pm-full-contract-with-modem-holder"

base.DEFAULT_OUT_DIR = Path("tmp/wifi/v1049-pm-full-contract-with-modem-holder-live")
base.LATEST_POINTER = Path("tmp/wifi/latest-v1049-pm-full-contract-with-modem-holder-live.txt")
base.DEFAULT_LOCAL_HELPER = Path("tmp/wifi/v1047-execns-helper-v178-build/a90_android_execns_probe")
base.DEFAULT_HELPER_SHA256 = HELPER_SHA256_V178
base.DEFAULT_HELPER_MARKER = "a90_android_execns_probe v178"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeout", type=float, default=v1027.v857.DEFAULT_TIMEOUT)
...
def _map_v1049(value: str) -> str:
    return (
        value.replace("v1032", "v1049")
        .replace("V1032", "V1049")
        .replace("v175", "v178")
        .replace("v177", "v178")
        .replace("V1041", "V1049")
...
def decide(args, local: dict[str, Any], steps: list[dict[str, Any]], analysis: dict[str, Any]):
    ...
    if args.command == "plan":
        return "v1049-pm-full-contract-with-modem-holder-plan-ready", pass_ok, _map_v1049(reason), "refresh V401/V490, then run bounded V1049 live gate"
    ...
    if decision in {
        ...
        return _map_v1049(decision), pass_ok, _map_v1049(reason), _map_v1049(next_step)

    if contract.get("pm_full_contract_with_modem_holder_matrix") != "1":
        return (
            "v1049-modem-holder-matrix-missing",
            False,
            "pm_full_contract_with_modem_holder_matrix!=1",
            "fix helper command or v178 matrix selection before retry",
        )
    if contract.get("modem_pre_holder_confirmed") != "1":
        return (
            "v1049-modem-pre-holder-not-confirmed-clean",
            True,
            (
    ...
    if contract.get("pm_full_contract_seen") == "1":
        return (
            "v1049-modem-holder-pm-full-contract-seen",
            True,
            (
    ...
    return (
        "v1049-modem-holder-confirmed-pm-full-contract-missing",
        True,
        (
        ...
        "inspect v1049 PM fd snapshots before deciding whether holder timing or actor order needs repair",
    )
...
def render_summary(manifest):
    return (
        ...
        .replace("# V1032 PM Runtime-Domain Guard Live", "# V1049 PM Full-Contract with Modem Holder Live")
        .replace("V1032", "V1049")
        .replace("helper `v175`", "helper `v178`")
        .replace("helper v175", "helper v178")
        .replace("v175", "v178")
    )
...
def build_manifest(args, store):
    ...
    manifest["rerun_after_v1048_v178_deploy"] = True
