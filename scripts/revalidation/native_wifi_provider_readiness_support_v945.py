#!/usr/bin/env python3
"""V945 source/build verifier for mdm_helper provider-readiness diagnostics."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v945-provider-readiness-support")
LATEST_POINTER = Path("tmp/wifi/latest-v945-provider-readiness-support.txt")
DEFAULT_HELPER_SOURCE = Path("stage3/linux_init/helpers/a90_android_execns_probe.c")
DEFAULT_BUILD_ARTIFACT = Path("tmp/wifi/v945-execns-helper-v157-build/a90_android_execns_probe")
DEFAULT_BUILD_LOG = Path("tmp/wifi/v945-execns-helper-v157-build/build.log")
...
def classify(source: str, build_rc: int, build_log: str, artifact: Path) -> dict[str, Any]:
    ...
    provider_fd_fn = extract_static_function(
        source,
        "int",
        "append_mdm_helper_provider_readiness_fd_matches",
    )
    path_fn = extract_static_function(
        source,
        "int",
        "append_mdm_helper_provider_path_status",
    )
    strings_output = artifact_strings(artifact)
    required_phase_calls = (
    ...
    checks = {
        ...
        "provider_path_statuses": all(
            token in provider_fn
            for token in (
                "paths->dev_binder",
                "paths->dev_hwbinder",
                "paths->dev_vndbinder",
                "paths->property_service_socket",
            )
        )
        and all(token in path_fn for token in ("char_device=%d", "socket=%d", "mode=%04o")),
        "provider_fd_matches": all(
            token in provider_fd_fn
            for token in (
                "/dev/binder",
                "/dev/hwbinder",
                "/dev/vndbinder",
        ...
        "runtime_phase_markers": all(token in runtime_fn for token in required_phase_calls),
        ...
        "strings_confirm_marker": "a90_android_execns_probe v157" in strings_output,
    ...
    return {
        ...
        if passed
        else "v945-provider-readiness-support-incomplete",
        "pass": passed,
        "reason": (
            "helper v157 adds provider-readiness diagnostics to the existing mdm_helper runtime-contract mode without live trigger expansion"
            if passed
            else "missing checks: " + ", ".join(name for name, ok in checks.items() if not ok)
        ),
        "next_step": (
            "deploy helper v157 only, then run bounded provider-readiness capture"
            if passed
            else "repair helper v157 provider-readiness diagnostics before deploy"
        ),
        "checks": checks,
    }
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V945 Provider-Readiness Support",
            "",
