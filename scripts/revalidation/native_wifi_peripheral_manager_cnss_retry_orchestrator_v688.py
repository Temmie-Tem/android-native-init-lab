#!/usr/bin/env python3
"""V688 current-boot orchestrator for PeripheralManager/CNSS retry proof."""
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v688-peripheral-manager-cnss-retry-orchestrated")
V688_SCRIPT = "scripts/revalidation/native_wifi_peripheral_manager_cnss_retry_v688.py"
V688_APPROVAL = (
...
HELPER_SHA256 = "6de2ee5efc65441f18ee587f54b525c4006553cae54363c2e036aa21f976a5f4"
HELPER_MARKER = "a90_android_execns_probe v114"

ALLOWED_LIVE_ACTIONS = (
    "V641 one-shot clean-DSP reboot",
    "V401 SELinuxfs mount surface",
    "V490 Android SELinux policy-load proof",
    "bounded helper v114 PeripheralManager provider/CNSS retry proof",
...
def peripheral_for(arm: dict[str, Any] | None) -> dict[str, Any]:
    live = (arm or {}).get("live") or {}
    surface = live.get("v688_peripheral_manager_surface")
...
def decide(command: str, prep: dict[str, Any] | None, arm: dict[str, Any] | None) -> tuple[str, bool, str, str]:
    if command == "plan":
        return (
            "v688-peripheral-manager-orchestrator-plan-ready",
            True,
            "plan-only; no device command executed",
            "deploy helper v114, then run V688 orchestrated live proof",
        )
    if not prep or not prep.get("ready"):
        return "v688-current-boot-prep-blocked", False, f"prep={prep}", "restore current-boot prerequisites before V688"
    if not arm or not arm.get("decision"):
        return "v688-arm-missing", False, "V688 arm did not produce a manifest", "inspect arm evidence"
    if "blocked" in str(arm.get("decision")):
        return "v688-arm-blocked", False, f"arm={summarize_arm(arm)}", "resolve V688 arm blocker before retry"
    counts = counts_for(arm)
    markers = markers_for(arm)
    surface = peripheral_for(arm)
    ...
    if provider_ready(surface):
        return (
            "v688-provider-ready-no-wlfw-advance",
            True,
...
def render_summary(manifest: dict[str, Any]) -> str:
    arm = manifest["arm_v688"]
    rows: list[list[str]] = []
    for surface_name in ("counts", "markers", "peripheral", "reboot_cleanup"):
    ...
    return "\n".join([
        "# V688 PeripheralManager CNSS Retry Orchestrator",
        "",
...
def main() -> int:
    ...
    if args.command == "run":
        root = store.run_dir
        arm_root = root / "arm-v688-v114-peripheral-manager"
        prep = v673.prep_current_boot(args, store, "v688", arm_root)
        if prep.get("ready"):
            arm = v673.run_arm(
                args,
                store,
