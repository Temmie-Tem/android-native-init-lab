#!/usr/bin/env python3
"""V591 bounded Android handoff for the V590 subsystem-state sampler.

This wrapper temporarily boots a known Android boot image, waits for
`sys.boot_completed=1`, runs the V590 read-only Android subsystem-state
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v591-android-subsys-state-sample-handoff")
...
def v590_out_dir(store: EvidenceStore) -> Path:
    return store.run_dir / "v590-android-subsys-state-sample-run"


def v591_step_plan(args: argparse.Namespace, store: EvidenceStore, android_image: Any, native_image: Any) -> list[tuple[str, list[str] | str, int]]:
    base = build_v424_step_plan(args, store, android_image, native_image)
    plan: list[tuple[str, list[str] | str, int]] = []
    collector_timeout = max(args.timeout * 8, int(args.sample_delay + 240))
    for name, command, timeout in base:
        if name == "v423-android-hwservice-inventory":
            ...
            v590_command = [
                "python3",
                "scripts/revalidation/native_wifi_android_subsys_state_sample_v590.py",
                "--out-dir",
                str(v590_out_dir(store)),
                "--adb",
                args.adb,
                "--timeout",
            ...
            if args.serial:
                v590_command.extend(["--serial", args.serial])
            v590_command.append("run")
            plan.append(("v590-android-subsys-state-sample", v590_command, collector_timeout))
        else:
            plan.append((name, command, timeout))
    return plan


def load_v590_result(store: EvidenceStore) -> dict[str, Any]:
    manifest = v590_out_dir(store) / "manifest.json"
...
def build_comparison(store: EvidenceStore, boot_state: dict[str, Any]) -> dict[str, Any]:
    v590 = load_v590_result(store)
    android_summary = v590.get("android_summary") or {}
    boot_complete = bool(boot_state.get("boot_completed"))
    if not boot_complete:
        decision = "v591-handoff-bootcomplete-missing"
        pass_ok = False
        reason = "Android boot-complete gate did not pass before V590"
    elif not v590.get("pass"):
        decision = "v591-handoff-v590-failed-rollback-complete"
        pass_ok = False
        reason = "V590 Android subsystem-state sampler failed after rollback completed"
    else:
        decision = str(v590.get("decision") or "v591-handoff-v590-review-required")
        pass_ok = bool(v590.get("pass"))
        reason = str(v590.get("reason") or decision)
    return {
        "decision": decision,
        "pass": pass_ok,
        "reason": reason,
        ...
        "state": {
            ...
            "non_offline_state": android_summary.get("non_offline_state"),
            "has_qrtr_readiness": android_summary.get("has_qrtr_readiness"),
            "has_sysmon_qmi": android_summary.get("has_sysmon_qmi"),
            "has_service_notifier": android_summary.get("has_service_notifier"),
            "has_wlan_pd": android_summary.get("has_wlan_pd"),
        },
        "v590": {
            "path": v590.get("_path"),
            "decision": v590.get("decision"),
            "pass": v590.get("pass"),
            "reason": v590.get("reason"),
            "next_step": v590.get("next_step"),
        },
    }
...
def execute_plan(args: argparse.Namespace, store: EvidenceStore, execute: bool) -> tuple[list[StepResult], dict[str, Any], str, bool]:
    ...
    context: dict[str, Any] = {
        ...
        "v590_out_dir": str(v590_out_dir(store)),
    ...
    if not native_image.present or not native_image.aligned_4k or not native_image.android_magic or not native_image.native_marker:
        return [], context, "v591-handoff-missing-native-rollback", False
    if android_image is None:
        return [], context, "v591-handoff-missing-android-boot", False
    if android_image.sha256 == native_image.sha256:
        return [], context, "v591-handoff-image-collision", False
    if args.command == "run" and not approval_ok:
        return [], context, "v591-handoff-approval-required", False

    plan = v591_step_plan(args, store, android_image, native_image)
    offenders = contains_forbidden_active_wifi(plan)
    if offenders:
        context["forbidden_active_wifi_steps"] = offenders
        return [], context, "v591-handoff-active-wifi-command-blocked", False
    ...
    if args.command == "plan":
        ...
        return steps, context, "v591-handoff-plan-ready", True
    ...
    for name, command, timeout in plan:
        if skip_collector and name in {"settle-after-boot-complete", "v590-android-subsys-state-sample"}:
        ...
        if name == "remote-android-sha" and execute and step.ok and android_image.sha256 not in step_text(store, step):
            return steps, context, "v591-handoff-remote-sha-mismatch", False
        if name == "flash-android-boot" and execute and not step.ok:
            ...
            return steps, context, "v591-handoff-flash-failed-rollback-attempted", False
        if name == "readback-android-boot" and execute:
            ...
            if not step.ok or android_image.sha256 not in readback_text:
                ...
                steps.append(rollback_step)
                context["emergency_rollback_ok"] = rollback_step.ok
                return steps, context, "v591-handoff-readback-failed-rollback-attempted", False
        if name == "wait-boot-complete" and execute and not step.ok:
            failure_after_rollback = "v591-handoff-failed-wait-boot-complete-rollback-complete"
            skip_collector = True
            continue
        if name == "v590-android-subsys-state-sample":
            v590_step_ok = step.ok
            if execute:
                continue
        if execute and not step.ok:
            return steps, context, f"v591-handoff-failed-{name}", False
    ...
    if execute:
        ...
        if not v590_step_ok:
            return steps, context, "v591-handoff-v590-failed-rollback-complete", False
        return steps, context, comparison["decision"], bool(comparison["pass"])
    return steps, context, "v591-handoff-dryrun-ready", True
...
def reason_for(decision: str, context: dict[str, Any]) -> str:
    ...
    return {
        "v591-handoff-plan-ready": "execution plan generated without device mutation",
        "v591-handoff-dryrun-ready": "dry-run recorded all steps without device mutation",
        "v591-handoff-approval-required": "live run refused because approval flags are missing",
        "v591-handoff-missing-native-rollback": "native rollback image is missing or does not contain the expected version marker",
        "v591-handoff-missing-android-boot": "no Android boot candidate passed local safety checks",
        "v591-handoff-image-collision": "Android and native rollback images unexpectedly have the same hash",
        "v591-handoff-active-wifi-command-blocked": "handoff plan contains a forbidden active Wi-Fi command pattern",
        "v591-handoff-failed-wait-boot-complete-rollback-complete": "Android boot-complete gate failed, and native rollback completed",
        "v591-handoff-v590-failed-rollback-complete": "V590 Android subsystem-state sampler failed, and native rollback completed",
        "v591-handoff-flash-failed-rollback-attempted": "Android boot flash failed and native rollback was attempted from recovery",
        "v591-handoff-readback-failed-rollback-attempted": "Android boot readback failed and native rollback was attempted from recovery",
...
def render_summary(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V591 Android Subsystem State Sample Handoff",
            "",
            ...
            f"- v590_out_dir: `{manifest['context']['v590_out_dir']}`",
            f"- sample_delay_sec: `{manifest['context']['sample_delay_sec']}`",
            "",
            "## Comparison",
            "",
            f"- boot_complete: `{comparison.get('boot_complete', '-')}`",
            f"- v590_decision: `{(comparison.get('v590') or {}).get('decision', '-')}`",
            f"- v590_next_step: `{(comparison.get('v590') or {}).get('next_step', '-')}`",
            "",
            ...
            "- Live mode temporarily flashes Android boot, runs V590 read-only collection, then restores native init.",
...
def main() -> int:
    ...
    manifest = {
        ...
        "device_mutations": args.command == "run" and decision not in {
            "v591-handoff-approval-required",
            "v591-handoff-missing-native-rollback",
            "v591-handoff-missing-android-boot",
            "v591-handoff-image-collision",
            "v591-handoff-active-wifi-command-blocked",
        },
