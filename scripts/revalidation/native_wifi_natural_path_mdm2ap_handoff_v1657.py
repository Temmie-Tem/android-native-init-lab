#!/usr/bin/env python3
"""V1657 one-run natural-path MDM2AP observation handoff.

This is a V1657-labeled wrapper around the existing V1636 natural-path test boot
artifact.  It keeps the ESOC_NATURAL_PATH_MDM2AP_OBSERVATION_CONTRACT scope:
natural provider path only, no forced RC1, no fake ONLINE, no PMIC/GPIO/GDSC
write, no Wi-Fi HAL, no scan/connect, no DHCP/routes, and no external ping.
"""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1657-natural-path-mdm2ap-observation-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1657_NATURAL_PATH_MDM2AP_OBSERVATION_HANDOFF_2026-06-02.md"
)
...
def render_report(result: dict) -> str:
    ...
    if observation["label"] == "mdm2ap-silent-natural-path":
        ...
        lines.append(
            "Treat this as lower-layer progress and design the next bounded gate "
            "around RC1/MHI/WLFW after the confirmed MDM2AP response."
        )
    elif observation["label"] == "provider-did-not-trigger":
        lines.append(
            "Treat this as route regression; inspect why the natural path did not "
            "reach `subsys_esoc0` before changing hardware assumptions."
        )
    else:
...
def main() -> int:
    ...
    if not handoff:
        print(json.dumps({
            "decision": "v1657-natural-path-handoff-manifest-missing",
            "pass": False,
            "base_rc": base_rc,
        }, indent=2))
        return 1
    ...
    result = dict(handoff)
    result.update({
        "decision": f"v1657-{observation['label']}",
