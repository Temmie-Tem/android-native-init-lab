#!/usr/bin/env python3
"""V1563 local-only sanity verifier for the V1562 Android Wi-Fi service-window test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1563-android-wifi-service-window-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1563_ANDROID_WIFI_SERVICE_WINDOW_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1562-android-wifi-service-window-test-boot-source-build-pass"
EXPECTED_BOOT_MARKERS = (
    ...
    "android_wifi_service_window.begin=1",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "watch_sec": 45,
        "helper_mode": "android-service-window-start-only",
        "helper_runtime_mode": "wifi-companion-android-wifi-service-window-start-only",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1563-android-wifi-service-window-artifact-sanity-pass",
            True,
            "V1562 Android Wi-Fi service-window test boot artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1563-android-wifi-service-window-artifact-sanity-blocked",
        False,
        "V1562 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1562 manifest: `{manifest['_path']}`",
        f"- V1562 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- init service-window route: `{checks['init_route']['ok']}`",
        f"- service-window contract: `{checks['wifi_test_contract']['ok']}`",
