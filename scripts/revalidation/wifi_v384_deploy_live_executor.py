#!/usr/bin/env python3
"""Fail-closed executor for V384 helper v15 deploy and ptrace-lite crash capture.

This sequences the V384 tools:

* helper v15 deploy wrapper
* service-manager ptrace-lite crash-capture live runner
* runtime-gap classifier

Approved actions remain split. Without exact V384 approval phrases and explicit
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v384-deploy-live-executor")
...
DEPLOY_APPROVAL_BLOCKER = "exact-v384-deploy-approval-phrase"
LIVE_APPROVAL_BLOCKER = "exact-v384-ptrace-live-approval-phrase"
...
def deploy_ok(store: EvidenceStore) -> bool:
    ...
    return manifest.get("decision") == "execns-helper-v15-deploy-pass" and bool(manifest.get("pass"))
...
def refusal_manifest(args: argparse.Namespace, store: EvidenceStore, blockers: list[str]) -> dict[str, Any]:
    return base_result(
        ...
        store,
        [],
        "v384-deploy-live-executor-approval-required",
        True,
        "exact V384 approval phrase and explicit flags are required for requested live action",
        "provide exact V384 approval phrase only if accepting the scoped action",
        blockers,
        False,
        False,
        False,
...
def classify_if_needed(args: argparse.Namespace, store: EvidenceStore, steps: list[StepResult]) -> tuple[str, bool, str, str, list[str]]:
    ...
    if live.get("decision") == "service-manager-start-only-live-runtime-gap":
        ...
        return (
            str(classified.get("decision") or "v384-classify-missing"),
    ...
    return (
        str(live.get("decision") or "v384-live-missing"),
        bool(live.get("pass")),
        f"live decision={live.get('decision')} pass={live.get('pass')}",
        str(live.get("next_step") or "inspect V384 live evidence"),
        [],
    )
...
def build_manifest(args: argparse.Namespace, store: EvidenceStore) -> dict[str, Any]:
    ...
    if args.command == "plan":
        ...
        return base_result(
            args, store, steps,
            "v384-deploy-live-executor-plan-ready",
            True,
            "plan-only; deploy/live remain approval-gated",
            "run V384 deploy with exact approval before ptrace-lite live capture",
    ...
    if args.command in {"deploy", "full"}:
        ...
        if not deploy_ok(store):
            return base_result(
                args, store, steps,
                "v384-deploy-live-executor-deploy-review",
                False,
        ...
        if args.command == "deploy":
            return base_result(
                args, store, steps,
                "v384-deploy-live-executor-deploy-pass",
                True,
                f"deploy decision={load_json(deploy_dir(store) / 'manifest.json').get('decision')}",
                "run V384 ptrace-lite live capture after exact live approval phrase",
    ...
    if not live_preflight_ok(store):
        return base_result(
            args, store, steps,
            "v384-deploy-live-executor-live-preflight-blocked",
            False,
    ...
    return base_result(
        args, store, steps,
        f"v384-deploy-live-executor-{decision_prefix}-{decision}",
        pass_ok,
        reason,
        next_step,
        remaining,
