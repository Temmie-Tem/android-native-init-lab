#!/usr/bin/env python3
"""V1455 local-only sanity verifier for the V1454 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1455-wifi-test-boot-exact-provider-long-endpoint-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1455_WIFI_TEST_BOOT_EXACT_PROVIDER_LONG_ENDPOINT_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1454-wifi-test-boot-exact-provider-long-endpoint-source-build-pass"
...
EXPECTED_BOOT_MARKERS = (
    ...
    "/cache/native-init-wifi-test-boot-v1454.pid",
    "/cache/native-init-wifi-test-boot-v1454-supervisor.pid",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1454-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1454-rc1-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1455-wifi-test-boot-exact-provider-long-endpoint-artifact-sanity-pass",
            True,
            "V1454 exact-line provider-trigger long-window test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1455-wifi-test-boot-exact-provider-long-endpoint-artifact-sanity-blocked",
        False,
        "V1454 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1454 manifest: `{manifest['_path']}`",
            f"- V1454 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- exact provider long-window contract: `{checks['wifi_test_contract']['ok']}`",
...
def main() -> int:
    ...
    result = {
        ...
        "next_gate": "V1456 rollbackable live handoff for only the V1454 image",
    }
