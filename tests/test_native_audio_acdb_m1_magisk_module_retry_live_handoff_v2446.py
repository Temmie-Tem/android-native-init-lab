"""Host-only tests for the V2446 ACDB M1 post-module wait-budget runner."""
...
v2446 = load_revalidation("native_audio_acdb_m1_magisk_module_retry_live_handoff_v2446")
...
def args(**overrides: object) -> argparse.Namespace:
    values: dict[str, object] = {
        ...
        "module_out_dir": v2446.v2429.DEFAULT_MODULE_OUT_DIR,
        "cc": v2446.v2429.DEFAULT_CC,
        "stimulus_apk": v2446.v2396.DEFAULT_STIMULUS_APK,
        "adb": "adb",
        "serial": None,
        "android_timeout": 420.0,
        "adb_command_timeout": 120.0,
        "flash_timeout": 900.0,
        "duration_ms": v2446.v2396.DEFAULT_DURATION_MS,
        "sample_rate": v2446.v2396.DEFAULT_SAMPLE_RATE,
        "amplitude": v2446.v2396.DEFAULT_AMPLITUDE,
        "active_delay_sec": 0.75,
        "post_delay_sec": 1.0,
        "capture_duration_sec": v2446.DEFAULT_CAPTURE_DURATION_SEC,
        "capture_observe_sec": 6.0,
        "post_module_root_retry_attempts": v2446.DEFAULT_POST_MODULE_ROOT_RETRY_ATTEMPTS,
        "post_module_root_retry_sleep_sec": v2446.DEFAULT_POST_MODULE_ROOT_RETRY_SLEEP_SEC,
        "post_module_adb_wait_timeout": v2446.DEFAULT_POST_MODULE_ADB_WAIT_TIMEOUT_SEC,
        "max_bytes": v2446.v2429.DEFAULT_MAX_BYTES,
        "process_poll_sec": v2446.v2429.DEFAULT_PROCESS_POLL_SEC,
        "from_native": True,
        "approval": None,
        "out_dir": None,
    }
...
class AcdbM1MagiskModuleRetryLiveHandoffV2446(unittest.TestCase):
    def test_dry_run_declares_v2446_retry_boundary(self) -> None:
        payload = v2446.dry_run(args())

        self.assertEqual(payload["run_id"], "V2446")
        self.assertEqual(payload["decision"], "v2446-acdb-m1-magisk-module-retry-live-dry-run")
        self.assertTrue(payload["host_only"])
        self.assertEqual(payload["device_action"], "none")
        self.assertEqual(payload["approval_phrase_required_for_live"], v2446.APPROVAL_PHRASE)
        self.assertEqual(payload["module_lifecycle"]["remote_module_dir"], f"/data/adb/modules/{v2446.v2429.MODULE_ID}")
        ...
        self.assertTrue(payload["module_lifecycle"]["v2446_post_module_adb_wait_budget"])
        self.assertEqual(
            payload["commands"]["android_post_module_reboot_settle"]["root_retry_attempts"],
            v2446.DEFAULT_POST_MODULE_ROOT_RETRY_ATTEMPTS,
        )
        self.assertEqual(
            payload["commands"]["android_post_module_reboot_settle"]["adb_wait_timeout_sec"],
            v2446.DEFAULT_POST_MODULE_ADB_WAIT_TIMEOUT_SEC,
        )
        self.assertGreater(payload["commands"]["android_post_module_reboot_settle"]["adb_wait_timeout_sec"], 206.359)
        self.assertGreater(
            payload["commands"]["android_post_module_reboot_settle"]["adb_wait_timeout_sec"],
            120.0,
        )
    ...
    def test_pushes_target_shell_writable_incoming_only(self) -> None:
        ...
        push_targets = [
            ...
            if v2446.adb_subcommand(command) == "push"
        ]
    ...
    def test_post_module_reboot_settle_retries_root_until_uid0(self) -> None:
        ...
        original_run_step = v2446.route.run_step
        original_sleep = v2446.time.sleep
        ...
        try:
            v2446.route.run_step = fake_run_step
            v2446.time.sleep = lambda _seconds: None
            with tempfile.TemporaryDirectory() as temp_dir:
                out_dir = Path(temp_dir)
                steps: list[dict[str, object]] = []
                v2446.run_post_module_reboot_settle(
                    args(post_module_root_retry_attempts=3, post_module_root_retry_sleep_sec=0.01),
                    out_dir,
                    steps,
                )
        finally:
            v2446.route.run_step = original_run_step
            v2446.time.sleep = original_sleep
        ...
        wait_records = [step for step in steps if step["name"].endswith("wait-for-device") or "root-wait" in step["name"]]
        self.assertTrue(wait_records)
        self.assertTrue(all(step["timeout_sec"] == v2446.DEFAULT_POST_MODULE_ADB_WAIT_TIMEOUT_SEC for step in wait_records))
    ...
    def test_run_live_wires_retry_after_module_activation_reboot(self) -> None:
        source = Path(v2446.__file__).read_text()
    ...
    def test_wrong_live_approval_exits_before_device_action(self) -> None:
        script = Path("workspace/public/src/scripts/revalidation/native_audio_acdb_m1_magisk_module_retry_live_handoff_v2446.py")
        ...
        self.assertEqual(payload["decision"], "v2446-acdb-m1-magisk-module-retry-live-refused")
