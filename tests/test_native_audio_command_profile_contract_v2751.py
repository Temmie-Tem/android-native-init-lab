...
class NativeAudioCommandProfileContractV2751(unittest.TestCase):
    ...
    def test_native_audio_exports_read_only_profile_subcommands(self) -> None:
        ...
        self.assertIn('usage: audio [adsp-status|status|profiles|profile [id]|app-type [profile] [--dry-run|--write]|route [profile] [--dry-run|--apply|--reset] [--layer all|core|feedback|endpoint|blocked]|snd-status|adsp-boot-once|snd-materialize-once]', text)
