...
class NativeAudioCommandProfileContractV2751(unittest.TestCase):
    ...
    def test_native_audio_exports_read_only_profile_subcommands(self) -> None:
        ...
        self.assertIn('usage: audio [adsp-status|status|profiles|profile [id]|stages [id]|app-type [profile] [--dry-run|--write]|setcal [profile] [--dry-run|--execute]|route [profile] [--dry-run|--apply|--reset] [--layer all|core|feedback|endpoint|blocked]|snd-status|adsp-boot-once|snd-materialize-once]', text)
