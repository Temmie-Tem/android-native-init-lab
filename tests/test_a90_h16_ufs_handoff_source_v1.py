"""Static closure tests for the A90 H16 read-only UFS handoff."""
...
H16_MANIFEST = VERSIONS / "phase3-minimal-h16/manifest.toml"
H13_MANIFEST = VERSIONS / "phase3-minimal-h13/manifest.toml"
H16_CONTENT = VERSIONS / "phase3-minimal-h14/userdata-content-manifest.json"
H16_F1_RUNNER = (
    REPO_ROOT
...
class A90UfsHandoffSourceV2Tests(unittest.TestCase):
    ...
    def _run_reconcile_fault(
        ...
        *,
        ...
        manifest = {"run_id": "a90-h16-ufs-f1-20260810-99"}
        appended: list[str] = []
        ...
        with tempfile.TemporaryDirectory() as raw, mock.patch.multiple(
            ...
            try:
                H16_F1.reconcile_health(
                    Path(raw) / "manifest.json",
                    "a" * 64,
                    self._f1_live_args(),
                )
            except H16_F1.ContractError as exc:
                return appended, exc
        return appended, None

    def test_h16_compiled_binding_is_dynamic_devt_v4_without_sd_image(self) -> None:
        h16 = self._manifest(H16_MANIFEST)
        binding = FLAT_BUILDER.normalized_auto_handoff_binding(h16)
        ...
        self.assertEqual(
            binding,
            {
                ...
                "enable_path": (
                    "/cache/a90-auto-handoff-phase3-minimal-h16.enable"
                ),
                ...
                "userdata_devt_policy": "runtime-resolved-same-session",
        ...
        flags = h16["init"]["cflags"]
        self.assertIn("-DA90_AUTO_HANDOFF_USERDATA_DYNAMIC_DEVT=1", flags)
        self.assertIn(
            '-DA90_AUTO_HANDOFF_USERDATA_DEVT_POLICY="runtime-resolved-same-session"',
            flags,
        )
        self.assertFalse(any("AUTO_HANDOFF_USERDATA_DEV=" in flag for flag in flags))
    ...
    def test_h16_retains_h13_minimal_and_wifi_components(self) -> None:
        ...
        self.assertEqual(h16["init"]["sources"], h13["init"]["sources"])
        self.assertEqual(h16["helper"], h13["helper"])
        self.assertEqual(h16["ramdisk"], h13["ramdisk"])
        self.assertFalse(h16["candidate_authority"])
        init_root = REPO_ROOT / h16["init"]["source_root"]
    ...
    def test_h16_unarmed_boot_skips_wifi_and_armed_wifi_is_asynchronous(self) -> None:
    ...
    def test_h16_replacement_identity_and_h15_resident_predecessor_are_exact(self) -> None:
        h16 = self._manifest(H16_MANIFEST)
        flags = h16["init"]["cflags"]
        for required in (
            '-DINIT_VERSION="0.11.184"',
            '-DINIT_BUILD="phase3-minimal-h16-direct-ufs-ro-async-wifi-auto-benchmark"',
            '-DA90_AUTO_HANDOFF_ENABLE_PATH="/cache/a90-auto-handoff-phase3-minimal-h16.enable"',
            '-DA90_AUTO_HANDOFF_LATCH_PATH="/cache/a90-auto-handoff-phase3-minimal-h16.done"',
            '-DA90_WIFI_TEST_BOOT_LABEL="v2811"',
        ):
        ...
        for required in (
            ...
            'result.get("schema") != "a90-h15-ufs-f1-result-v1"',
            'result.get("status") != "PASS_A90_H15_UFS_RESIDENT_INSTALLED"',
            'result.get("rollback_transfer_count") != 0',
    ...
    def test_userdata_binding_rejects_sd_fields_and_identity_drift(self) -> None:
        ...
        drifted = copy.deepcopy(h16)
        flags = drifted["init"]["cflags"]
        index = flags.index(
            '-DA90_AUTO_HANDOFF_USERDATA_DEVT_POLICY="runtime-resolved-same-session"'
        )
        flags[index] = '-DA90_AUTO_HANDOFF_USERDATA_DEVT_POLICY="fixed-cross-boot"'
        with self.assertRaisesRegex(RuntimeError, "userdata tuple"):
        ...
        for conflicting in (
            ...
            "-DA90_AUTO_HANDOFF_USERDATA_DYNAMIC_DEVT=0",
            "-UA90_AUTO_HANDOFF_USERDATA_DYNAMIC_DEVT",
            '-DA90_AUTO_HANDOFF_USERDATA_DEV="259:18"',
            '-DA90_AUTO_HANDOFF_USERDATA_DEVT_POLICY="fixed-cross-boot"',
    ...
    def test_auto_handoff_qualifies_before_arm_and_boot_latch(self) -> None:
        ...
        self.assertIn("a90-auto-handoff-userdata-ro-v2", source)
    ...
    def test_dynamic_devt_is_same_session_only_and_rechecked_before_mount(self) -> None:
        source = (NATIVE / "a90_server_distro.c").read_text(encoding="utf-8")
        compare = source[
            source.index("static int d4_compare_ro_expected("):
            source.index("static int d4_ensure_userdata_node(")
        ]
        handoff = source[
            source.index("int a90_server_distro_switch_root_userdata_ro("):
        ...
        self.assertIn('"runtime-resolved-same-session"', compare)
        self.assertIn("strcmp(target->devname, expected_devname)", compare)
        self.assertIn("target->sectors != sectors", compare)
        self.assertGreaterEqual(
            handoff.count("d4_userdata_ro_static_preflight("),
            3,
        )
        for token in (
            ...
            "stop=userdata-changed-during-display-cleanup",
        ):
            self.assertIn(token, handoff)
        self.assertLess(
            handoff.index("stop=userdata-changed-during-display-cleanup"),
            handoff.index("d4_mount_userdata_readonly_no_replay()"),
        )

    def test_h16_inventory_records_but_does_not_cross_boot_pin_devt(self) -> None:
        inventory = {
            "schema": H16_F1.INVENTORY_SCHEMA,
            "status": "PASS",
            "target": "Samsung Galaxy A90 5G",
            "identity": dict(H16_F1.UFS_IDENTITY),
            "observed_devt": "259:36",
        ...
        H16_F1.validate_ufs_inventory(inventory)

        for field, value in (
            ("observed_devt", "not-a-devt"),
            ("devt_stability", "cross-boot-fixed"),
        ):
            ...
            with self.subTest(field=field):
                with self.assertRaisesRegex(
                    ...
                    "read-only UFS inventory is not exact",
                ):
                    H16_F1.validate_ufs_inventory(changed)

        self.assertNotIn("dev", inventory["identity"])
        self.assertEqual(
            inventory["identity"]["devt_policy"],
            "runtime-resolved-same-session",
        )

    def test_marker_and_content_checks_are_exact_and_secret_safe(self) -> None:
        source = (NATIVE / "a90_server_distro.c").read_text(encoding="utf-8")
        content = json.loads(H16_CONTENT.read_text(encoding="utf-8"))
        encoded = json.dumps(
            content,
            sort_keys=True,
            separators=(",", ":"),
        ...
        self.assertEqual(
            hashlib.sha256(H16_CONTENT.read_bytes()).hexdigest(),
            "a878f6dec82bf799c3d2cd43beeda3c5494a8882ce116327f497d822b707d5ce",
        )
        self.assertEqual(
    ...
    def test_h16_f1_runner_has_no_sd_rootfs_effect_lane(self) -> None:
        source = H16_F1_RUNNER.read_text(encoding="utf-8")
        execute = source[
    ...
    def test_h16_f1_runner_binds_reusable_capability_review(self) -> None:
        source = H16_F1_RUNNER.read_text(encoding="utf-8")
    ...
    def test_h16_f1_closure_covers_transitive_local_imports(self) -> None:
        closure = set(H16_F1.EXECUTION_SOURCE_RELS)
        local_roots = (
    ...
    def test_h16_flash_timeout_quiesces_descendant_process_group(self) -> None:
        manifest = {
            "run_id": "a90-h16-ufs-f1-20260810-99",
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch.object(
                ...
                record = H16_F1._flash_record(  # noqa: SLF001
                    manifest,
                    manifest_sha,
                    None,
                    args,
            ...
            self.assertEqual(H16_F1._process_group_members(pgid), [])  # noqa: SLF001
            records = H16_F1.read_journal(journal, manifest, manifest_sha)
    ...
    def test_h16_f1_load_binds_exact_target_and_recovery_predecessor(self) -> None:
        source = H16_F1_RUNNER.read_text(encoding="utf-8")
        loader = source[
    ...
    def test_h16_d1_runner_is_one_arm_one_reboot_no_payload(self) -> None:
        source = H16_D1_RUNNER.read_text(encoding="utf-8")
    ...
    def test_h16_d1_finalize_paths_never_repeat_effects(self) -> None:
        source = H16_D1_RUNNER.read_text(encoding="utf-8")
        finalize = source[
    ...
    def test_h16_native_combined_arm_reboot_cancels_every_syscall_return(self) -> None:
        source = (NATIVE / "a90_auto_handoff.c").read_text(encoding="utf-8")
        command = source[
    ...
    def test_h16_authority_binds_agents_and_requires_fresh_approval(self) -> None:
        self.assertIn("AGENTS.md", H16_F1.EXECUTION_SOURCE_RELS)
        f1_source = H16_F1_RUNNER.read_text(encoding="utf-8")
        d1_source = H16_D1_RUNNER.read_text(encoding="utf-8")
        for source, workflow, prefix in (
            (f1_source, "A90_F1_RESIDENT_INSTALL_V1", "A90-H16-F1-APPROVE:"),
            (d1_source, "A90_D1_ATTENDED_SESSION_V1", "A90-H16-D1-APPROVE:"),
        ):
        ...
        self.assertIn("H16 D1 execute requires fresh exact approval", d1_source)

    def test_h16_f1_and_d1_approval_tokens_round_trip_exact_bindings(self) -> None:
        created = H16_F1.dt.datetime.now(H16_F1.dt.UTC).replace(microsecond=0)
        expires = created + H16_F1.dt.timedelta(seconds=H16_F1.APPROVAL_TTL_SEC)
        ...
        manifest = {
            ...
            "target": {
                ...
                "bridge_device": H16_F1.EXACT_BRIDGE_DEVICE,
        ...
        manifest_sha = "4" * 64
        f1_binding = H16_F1.approval_binding(
            manifest,
            manifest_sha,
            created_utc=created_text,
            expires_utc=expires_text,
        )
        f1_binding_sha = H16_F1.json_sha256(f1_binding)
        f1_token = H16_F1.APPROVAL_PREFIX + f1_binding_sha
        f1_value = {
            "schema": H16_F1.APPROVAL_SCHEMA,
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            H16_F1.write_json_exclusive(f1_path, f1_value)
            with mock.patch.object(H16_F1, "_approval_path", return_value=f1_path):
                approved = H16_F1.validate_approval(
                    manifest,
                    manifest_sha,
                    f1_token,
                )
            ...
            self.assertEqual(
                H16_F1.require_consumed_approval(
                    [consumed],
                    manifest,
                    manifest_sha,
                ),
            ...
            d1_binding = H16_D1.approval_binding(
                manifest,
                d1_args,
                transaction,
                created_utc=created_text,
            ...
            d1_binding_sha = H16_F1.json_sha256(d1_binding)
            d1_token = H16_D1.APPROVAL_PREFIX + d1_binding_sha
            d1_args.approval = d1_token
            d1_value = {
                "schema": H16_D1.APPROVAL_SCHEMA,
            ...
            d1_path = H16_D1._approval_path(transaction)  # noqa: SLF001
            H16_F1.write_json_exclusive(d1_path, d1_value)
            self.assertEqual(
                H16_D1.validate_approval(manifest, d1_args, transaction),
                d1_value,
            )
    ...
    def test_candidate_launch_result_gap_with_write_marker_parks_rollback(self) -> None:
        ...
        appended, error = self._run_reconcile_fault(
            records,
            candidate_health=H16_F1.ContractError("candidate unavailable"),
        ...
        self.assertIsInstance(error, H16_F1.ContractError)
    ...
    def test_candidate_success_result_health_gap_parks_rollback(self) -> None:
        ...
        appended, error = self._run_reconcile_fault(
            records,
            candidate_health=H16_F1.ContractError("health unavailable"),
        )
        self.assertIsInstance(error, H16_F1.ContractError)
        self.assertEqual(appended, ["recovery-required"])
        self.assertNotIn("closed", appended)
    ...
    def test_d1_cancellation_requires_same_intent_post_fsync_log(self) -> None:
        ...
        with mock.patch.object(
            ...
            proof = H16_D1.require_cancellation_fsync_proof(
                opening,
                final,
                intent,
            )
            ...
            with self.assertRaisesRegex(
                H16_D1.ContractError,
                "cancellation fsync proof",
            ):
                H16_D1.require_cancellation_fsync_proof(
                    opening,
    ...
    def test_d1_native_failure_requires_clean_restore_and_root_unmount(self) -> None:
        ...
        with mock.patch.object(
            ...
            proof = H16_D1.native_failure_cleanup_disposition(
                opening,
                safe,
            ...
            with self.assertRaisesRegex(
                ...
                H16_D1.native_failure_cleanup_disposition(
                    opening,
                    unsafe,
                    native_handoff_failed=True,
                )
    ...
    def test_d1_finalize_return_proves_latched_state_before_publication(self) -> None:
        ...
        with mock.patch.multiple(
            ...
            patched["require_status"].side_effect = H16_D1.ContractError(
                "not latched"
            )
            with self.assertRaisesRegex(H16_D1.ContractError, "not latched"):
                H16_D1.finalize_return(args)
    ...
    def test_d1_durable_opening_requires_exact_log_receipt(self) -> None:
        ...
        opening = {
            ...
            "approval_binding_sha256": H16_F1.json_sha256(binding),
        ...
        with self.assertRaisesRegex(
            ...
            H16_D1._validate_records(  # noqa: SLF001
                [opening],
    ...
    def test_d1_reconcile_zero_zero_without_cancel_proof_stays_unknown(self) -> None:
        ...
        with mock.patch.multiple(
            ...
            patched["parse_status"].return_value = {
                ...
                "enable": 0,
                "latch": 0,
                "build": H16_F1.CANDIDATE_BUILD,
            }
            patched["require_cancellation_fsync_proof"].side_effect = (
                H16_D1.ContractError("proof absent")
            )
            result = H16_D1.reconcile(args)
        self.assertEqual(
            result["terminal"],
    ...
    def test_d1_reconcile_missing_cleanup_evidence_parks_recovery(self) -> None:
        ...
        with mock.patch.multiple(
            ...
            side_effect=H16_D1.ContractError("cleanup marker absent"),
        ):
            patched["load_inputs"].return_value = ({"run_id": "r"}, object(), {})
            patched["_require_transaction_dir"].return_value = Path("/private/ordinal")
            patched["_read_records"].return_value = records
