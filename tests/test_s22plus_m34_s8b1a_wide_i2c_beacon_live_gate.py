...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.py")
ANALYZER_SCRIPT = Path("workspace/public/src/scripts/revalidation/analyze_s22plus_m34_s8b1a_result.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m34_s8b1a_wide_i2c_beacon_live_gate", SCRIPT)
...
def load_analyzer():
    ...
    if script_dir not in sys.path:
        sys.path.insert(0, script_dir)
    spec = importlib.util.spec_from_file_location("analyze_s22plus_m34_s8b1a_result", ANALYZER_SCRIPT)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
...
class S22PlusM34S8B1ABeaconProbeLiveGateTest(unittest.TestCase):
    def setUp(self):
        self.module = load_module()

    def test_policy_markers_include_hashes_tokens_and_s8b1a_semantics(self):
        ...
        self.assertIn("S8B1A keeps the S7A2 module recipe fixed", markers)
    ...
    def test_agents_exception_draft_satisfies_same_policy_markers(self):
        ...
        self.assertIn("does not authorize S1/S2/S3/S4/S5/S6/S7A/S7A2/S8B1 repeat", " ".join(draft.split()))
    ...
    def test_verify_agents_exception_rejects_draft_only_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(SystemExit) as caught:
                self.module.verify_agents_exception(root, log_path)
            self.assertIn("draft-only M34 S8B1A", str(caught.exception))
    ...
    def test_verify_agents_exception_rejects_marker_complete_modified_active_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            modified = self.module.agents_exception_active_template().replace(
                "one bounded attended boot-partition-only M34 S8B1A live gate",
                "one unbounded attended boot-partition-only M34 S8B1A live gate",
                1,
            )
            ...
            with self.assertRaises(SystemExit) as caught:
                self.module.verify_agents_exception(root, log_path)
            self.assertIn("exact M34 S8B1A active authorization template is absent", str(caught.exception))
            text = log_path.read_text(encoding="utf-8")
            self.assertIn("agents_exception_missing=[]", text)
            self.assertIn("agents_exception_exact_active_template_present=0", text)

    @unittest.skipUnless(MANIFEST.exists(), "private M34 v0.9 manifest missing")
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            log_path = Path(tmp) / "m34_s8b1a_manifest_check.log"
            self.module.verify_m34_manifest(MANIFEST, log_path)
            text = log_path.read_text(encoding="utf-8")
            self.assertIn("m34_s8b1a_manifest_hashes=", text)
            self.assertIn("m34_manifest_safety=", text)
            self.assertIn("m34_s8b1a_manifest_runtime_steps=", text)
            self.assertIn("m34_s8b1a_manifest_closure=", text)
    ...
    def test_run_android_readonly_preflight_checks_android_boot_and_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                mock.patch.object(self.module, "collect_android_s8b1a_predicate_baseline") as collect_baseline,
    ...
    def test_collect_android_s8b1a_predicate_baseline_writes_true_baseline(self):
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp)
            ...
            with mock.patch.object(self.module, "run", side_effect=[proc, *root_procs]) as run:
                payload = self.module.collect_android_s8b1a_predicate_baseline(
                    run_dir=run_dir,
                    log_path=log_path,
            ...
            self.assertIn("s8b1a_android_predicate_baseline_json=", log_path.read_text(encoding="utf-8"))
    ...
    def test_prelive_packet_skips_agents_gate_and_writes_exact_run_material(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            packet = json.loads((run_dir / "s22plus_m34_s8b1a_prelive_packet.json").read_text(encoding="utf-8"))
            self.assertEqual(packet["schema"], "s22plus_m34_s8b1a_prelive_packet_v1")
            ...
            self.assertEqual(packet["android_s8b1a_predicate_baseline"]["schema"], "s22plus_m34_s8b1a_android_predicate_baseline_v1")
            self.assertTrue(packet["android_s8b1a_predicate_baseline"]["predicate_true"])
            self.assertEqual(packet["android_s8b1a_predicate_baseline"]["serial"], "<S22_SERIAL_REDACTED>")
            hints = packet["android_s8b1a_predicate_baseline"]["future_b2_hints"]
            ...
            self.assertTrue(hints["paths"][self.module.ANDROID_MAX77705_USBC_TYPEC_PORT0]["exists"])
            self.assertEqual(
                packet["android_s8b1a_predicate_baseline_json"],
                str(run_dir / "s22plus_m34_s8b1a_android_predicate_baseline.json"),
            )
            self.assertEqual(
                packet["android_reset_context_baseline"]["schema"],
                "s22plus_m34_s8b1a_android_reset_context_baseline_v1",
            )
            ...
            self.assertEqual(
                ...
                str(run_dir / "s22plus_m34_s8b1a_android_reset_context_baseline.json"),
            )
            self.assertEqual(
                set(packet["material_sha256"]),
                {
                    ...
                    "android_s8b1a_predicate_baseline_json",
                    "android_reset_context_baseline_json",
                },
            )
            self.assertEqual(packet["material_sha256"]["runbook"], self.module.sha256_file(run_dir / "s22plus_m34_s8b1a_live_runbook.txt"))
            self.assertEqual(
                packet["material_sha256"]["active_exception_template"],
                self.module.sha256_file(run_dir / "s22plus_m34_s8b1a_active_exception_template.txt"),
            )
            self.assertEqual(
                packet["material_sha256"]["android_s8b1a_predicate_baseline_json"],
                self.module.sha256_file(run_dir / "s22plus_m34_s8b1a_android_predicate_baseline.json"),
            )
            self.assertEqual(
                packet["material_sha256"]["android_reset_context_baseline_json"],
                self.module.sha256_file(run_dir / "s22plus_m34_s8b1a_android_reset_context_baseline.json"),
            )
            runbook = (run_dir / "s22plus_m34_s8b1a_live_runbook.txt").read_text(encoding="utf-8")
            active_template = (run_dir / "s22plus_m34_s8b1a_active_exception_template.txt").read_text(encoding="utf-8")
            ...
            log_text = (run_dir / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt").read_text(encoding="utf-8")
    ...
    def test_verify_prelive_packet_skips_device_and_agents_gates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log_text = (verify_dir / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt").read_text(encoding="utf-8")
    ...
    def test_verify_prelive_packet_after_dryrun_allows_only_dryrun_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dryrun_dir.mkdir()
            (dryrun_dir / "s22plus_m34_s8b1a_wide_i2c_beacon_live_gate.txt").write_text(
                "\n".join(
                    [
    ...
    def test_verify_prelive_packet_rejects_moved_packet_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            moved_dir.mkdir()
            moved_packet = moved_dir / "s22plus_m34_s8b1a_prelive_packet.json"
            moved_packet.write_text(
                (run_dir / "s22plus_m34_s8b1a_prelive_packet.json").read_text(encoding="utf-8"),
                encoding="utf-8",
            )
            with mock.patch.object(self.module, "verify_m34_artifacts"):
                with self.assertRaises(SystemExit) as caught:
    ...
    def test_verify_prelive_packet_rejects_missing_future_b2_hints(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            packet_path = run_dir / "s22plus_m34_s8b1a_prelive_packet.json"
            packet = json.loads(packet_path.read_text(encoding="utf-8"))
            packet["android_s8b1a_predicate_baseline"].pop("future_b2_hints")
    ...
    def test_runbook_phase_run_dir_keeps_live_result_dir_unique(self):
        base = Path("/tmp/s8b1a-live")
        self.assertEqual(self.module.runbook_phase_run_dir(base, "live"), base)
        self.assertEqual(self.module.runbook_phase_run_dir(base, "preflight"), Path("/tmp/s8b1a-live_preflight"))
        self.assertEqual(self.module.runbook_phase_run_dir(base, "template"), Path("/tmp/s8b1a-live_template"))
        self.assertIsNone(self.module.runbook_phase_run_dir(None, "live"))
        self.assertEqual(
            self.module.runbook_phase_run_dirs(base),
            {
                "preflight": "/tmp/s8b1a-live_preflight",
                "template": "/tmp/s8b1a-live_template",
                "dryrun": "/tmp/s8b1a-live_dryrun",
                "live": "/tmp/s8b1a-live",
                "rollback": "/tmp/s8b1a-live_rollback",
            },
        )
    ...
    def test_write_result_summary_creates_machine_readable_result_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["schema"], "s22plus_m34_s8b1a_result_v1")
            self.assertEqual(payload["target"], self.module.EXPECTED_TARGET)
            self.assertEqual(payload["stage"], "S8B1A")
            ...
            analysis = json.loads((run_dir / "s22plus_m34_s8b1a_result_analysis.json").read_text(encoding="utf-8"))
            self.assertEqual(analysis["decision"], "s22plus-m34-s8b1a-no-b1-proof")
    ...
    def test_helper_result_and_timeline_are_accepted_by_s8b1a_analyzer_for_hit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            written_analysis = json.loads(
                (run_dir / "s22plus_m34_s8b1a_result_analysis.json").read_text(encoding="utf-8")
            )
    ...
    def test_helper_result_and_timeline_are_accepted_by_s8b1a_analyzer_for_miss(self):
        analyzer = load_analyzer()
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp)
            log_path = run_dir / "result.log"
    ...
    def test_observe_download_beacon_classifies_new_odin_endpoint_as_hit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m34_s8b1a_result=download-beacon-hit", log_path.read_text(encoding="utf-8"))
    ...
    def test_observe_download_beacon_miss_requires_manual_download(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m34_s8b1a_result=download-beacon-miss-parked-manual-download-required", log_path.read_text(encoding="utf-8"))
    ...
    def test_observe_download_beacon_classifies_adb_return_before_rollback_as_unexpected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIsNone(device)
            self.assertIn("m34_s8b1a_result=unexpected-adb-before-rollback", log_path.read_text(encoding="utf-8"))
