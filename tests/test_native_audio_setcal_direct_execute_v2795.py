"""Tests for the V2795 audio SET-cal direct execute fix."""
...
BUILDER = REPO / "workspace/public/src/scripts/revalidation/build_native_init_boot_v2795_audio_setcal_direct_execute.py"
LIVE = REPO / "workspace/public/src/scripts/revalidation/native_audio_setcal_direct_execute_live_handoff_v2795.py"


def load_live_module():
    sys.path.insert(0, str(LIVE.parent))
    spec = importlib.util.spec_from_file_location("v2795_live", LIVE)
...
class NativeAudioSetcalDirectExecuteV2795(unittest.TestCase):
    @classmethod
    ...
    def test_builder_uses_v2795_patch_artifact_identity(self) -> None:
        text = BUILDER.read_text(encoding="utf-8")

        self.assertIn('CYCLE = "V2795"', text)
        self.assertIn('INIT_VERSION = "0.9.308"', text)
        self.assertIn('INIT_BUILD = "v2795-audio-setcal-direct-execute"', text)
        self.assertIn('boot_linux_v2795_audio_setcal_direct_execute.img', text)
        self.assertIn('NATIVE_INIT_V2795_AUDIO_SETCAL_DIRECT_EXECUTE_SOURCE_BUILD_2026-06-19.md', text)
        self.assertIn('"sound_control_wait_timeout_ms": 70000', text)
        self.assertIn('"play_worker_executor_compiled": True', text)

    def test_live_runner_targets_v2795_and_allowed_manifest_path(self) -> None:
        module = self.live
        args = module.parse_args(["--dry-run"])
        state = module.preflight_state()
        payload = module.dry_run_payload(args, state)

        self.assertEqual(module.CANDIDATE_VERSION, "0.9.308")
        self.assertIn("boot_linux_v2795_audio_setcal_direct_execute.img", str(module.CANDIDATE_IMAGE))
        self.assertEqual(
