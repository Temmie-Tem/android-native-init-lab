...
import s22plus_fyg8_p357_research_shell_runtime as runtime
import s22plus_fyg8_p357_research_shell_observer as observer
...
static void mark(const char *s){int f=open(getenv("P357_H0_MARK"),O_WRONLY|O_CREAT|O_APPEND,0600);if(f<0)_Exit(88);if(write(f,s,strlen(s))!=(ssize_t)strlen(s))_Exit(87);close(f);}
...
static long neg(long n)''')
 ...
 if(!strcmp(getenv("P357_H0_CASE"),"timeout"))for(;;)usleep(10000);
 _Exit(!strcmp(getenv("P357_H0_CASE"),"error")?7:0);
    ...
    text=text.replace('out->tv_sec=t.tv_sec;out->tv_nsec=t.tv_nsec;return 0;', '''int factor=h0_dispatched&&!strcmp(getenv("P357_H0_CASE"),"timeout")?60:1;
 out->tv_sec=t.tv_sec*factor+(t.tv_nsec*factor)/1000000000;
 out->tv_nsec=(t.tv_nsec*factor)%1000000000;return 0;''')
    text=text.replace('if (p357_display) {\n        if (p357_display_consumed)', 'if (p357_display) {\n        h0_dispatched=1;\n        if (p357_display_consumed)')
...
class CJoinTests(unittest.TestCase):
    ...
    def run_case(self,case):
        ...
        env=dict(os.environ,P357_H0_MARK=str(mark),P357_H0_CASE=case)
        process=subprocess.Popen([str(self.binary),str(peer.fileno()),'1'],env=env,pass_fds=(peer.fileno(),),stderr=subprocess.PIPE)
        peer.close()
        try:
            codec=live._open_header_initial_observer_module(runtime,observer,'p357-real-c-'+case)
