...
import s22plus_fyg8_p343_stock_candidate_build as builder  # noqa: E402


class P343StockCandidateBuildTests(unittest.TestCase):
    def test_existing_build_reopens_real_ab_join_and_catalog_runtime(self) -> None:
        result = builder.audit_existing(builder.DEFAULT_OUTPUT_ROOT)
        self.assertEqual(result["run_id_hex"], builder.P343_RUN_ID_HEX)
        ...
        self.assertEqual(candidate["run_id_join"]["run_id_hex"], builder.P343_RUN_ID_HEX)
        self.assertEqual(candidate["a"]["ap_tar_md5"], candidate["b"]["ap_tar_md5"])
        self.assertEqual(candidate["a"]["ap_tar_md5"], builder.artifact.P343_AP_IDENTITY)
        self.assertEqual(candidate["run_id_join"]["image_run_id_hex"], builder.P343_RUN_ID_HEX)
        self.assertEqual(candidate["run_id_join"]["init_run_id_hex"], builder.P343_RUN_ID_HEX)
        self.assertTrue(result["catalog_allowlist_expanded"])
        self.assertEqual(
            result["catalog_allowlist_actions"],
            list(builder.runtime.CATALOG_ACTIONS),
        )
        self.assertFalse(result["runtime_behavior_unchanged"])
        self.assertTrue(result["default_runtime_behavior_unchanged"])
