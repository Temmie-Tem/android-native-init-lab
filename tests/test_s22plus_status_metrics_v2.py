...
#include <time.h>
#include <unistd.h>
#include "s22plus_status_metrics_v2.inc.c"
int main(int argc,char **argv) {
 if(argc==2&&!strcmp(argv[1],"fd3")) {
  ...
  for(unsigned i=0;i<1000;i++){ssize_t n=recv(3,&wire,sizeof(wire),MSG_TRUNC);if(n==96){assert(wire.magic==0x32545353U);return 0;}assert(n<0&&errno==EAGAIN);usleep(1000);}return 99;
 }
 assert(argc==2);
 ...
 struct status_metrics packet={.magic=0x32545353U};assert(send(inherited[0],&packet,96,MSG_NOSIGNAL)==96);
 ...
 if(!child){close(pair[0]);assert(dup2(pair[1],1)==1);close(pair[1]);_exit(status_collect("c378f1e0a90b5e6d7c8a9b0c1d2e3f0b"));}
 ...
 for(unsigned attempt=0;attempt<400&&samples<2;attempt++) {
  ...
  assert(got==96&&wire.magic==0x32545353U&&wire.sequence==samples+1&&wire.reserved==0);
...
class StatusMetricsV2(unittest.TestCase):
