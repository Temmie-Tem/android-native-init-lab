...
class ModelWrapper(base.Wrapper):
    @classmethod
    def setUpClass(cls):
        stub=base.STUB.replace("Qualcomm Technologies, Inc. Waipio v2 SoC", "Samsung G0Q PROJECT (board-id,12)").replace('static unsigned resistor=5,calls,fault;',
        ...
        main=base.MAIN.replace(' if(!strcmp(test,\"wrong-model\"))', ' if(!strcmp(test,\"wrong-old-soc\"))model=\"Qualcomm Technologies, Inc. Waipio v2 SoC\";\n if(!strcmp(test,\"wrong-board\"))model=\"Samsung G0Q PROJECT (board-id,11)\";\n if(!strcmp(test,\"wrong-model\"))').replace(' struct platform_device child=', ' char diag[PAGE_SIZE];\n assert(diagnostic_get(diag,NULL)>0&&!calls&&strstr(diag,"probe=0 probe_error=0 bound=0"));\n struct platform_device child=')
        main=main.replace('assert(telemetry_probe(&child)==-ENODEV&&!calls&&!bound_parent);return 0;', '''assert(telemetry_probe(&child)==-ENODEV&&!calls&&!bound_parent);
          unsigned expected=!strcmp(test,"wrong-parent")?3:!strcmp(test,"wrong-fg")||!strcmp(test,"wrong-abi")?10:(!strcmp(test,"wrong-model")||!strcmp(test,"wrong-old-soc")||!strcmp(test,"wrong-board"))?7:!strcmp(test,"wrong-resistor")?9:5;
        ...
        with mock.patch.object(base,'STUB',stub),mock.patch.object(base,'MODULE' ,base.MODULE.with_name('s22plus_max77705_telemetry_v3')),mock.patch.object(base,'MAIN',main):
            base.Wrapper.setUpClass.__func__(cls)

    def test_identity_rejection_keeps_raw_return(self):
        for case in ('bad-id','bad-revision','wrong-old-soc','wrong-board'):
