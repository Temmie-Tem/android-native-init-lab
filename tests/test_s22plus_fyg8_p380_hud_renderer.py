...
import s22plus_fyg8_p380_display_renderer as renderer
ROOT=base.ROOT


def fixture():
    source,headers=base.fixture()
    source=source.replace('0x76','0x80').replace('c376','c380')
 ...
 if(f==3){
  ...
  const unsigned char id[16]={0xc3,0x80,0xf1,0xe0,0xa9,0x0b,0x5e,0x6d,0x7c,0x8a,0x9b,0x0c,0x1d,0x2e,0x3f,0x0b};memcpy(value.run,id,16);
 ...
 static unsigned seq,empty;assert''')
    source=source.replace('paint(&b,0xffffffffU);','run_id="c380f1e0a90b5e6d7c8a9b0c1d2e3f0b";paint(&b,0xffffffffU);')
