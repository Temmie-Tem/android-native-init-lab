...
class P333CommonIntegrationTests(unittest.TestCase):
    def setUp(self) -> None:
        self.acceptance = evidence.p333_stock_adapter.acceptance_fixture()
        self.acceptance["auth_key"] = dict(
            evidence.P333_AUTH_EXEC_AUTH_KEY_IDENTITY
        )
        self.observer = evidence.p333_authenticated_logical_resident_observer_spec()

    def test_exact_role_sources_and_consumed_p332_baseline(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(
            evidence.validate_candidate_arrival_proof_role(
                ...
                self.observer,
                expected_run_id=evidence.P333_RUN_ID,
            ),
            evidence.CANDIDATE_AUTHENTICATED_LOGICAL_RESIDENT_EXEC_ROLE,
        )
        ...
        path = ROOT / (
            ...
            "p332-ready1-prepared-20260904-2/rollback-observer-2.bin"
        )
        raw = path.read_bytes()
    ...
    def test_entry_diagnostic_contract_is_exact_and_predecessors_fail(self) -> None:
        ...
        for run_id in (evidence.P332_RUN_ID, evidence.P331_RUN_ID):
    ...
    def test_backend_selects_p333_before_authenticated_fallback(self) -> None:
        manifest = {
            "observation": {
        ...
        with (
            ...
            mock.patch.object(
                live, "_p333_candidate_observer_session", return_value=sentinel
            ) as selected,
    ...
    def test_live_session_uses_p333_observer_on_one_tty(self) -> None:
        base = SimpleNamespace(dev_root=Path("/dev"), _raw_tty=lambda _fd: None)
        session = live._P333ObserverSession(
