"""Tests for the V2669 direct-real-common ACDB SET capture live wrapper."""
...
v2669 = load_revalidation("native_audio_acdb_direct_real_common_setcal_capture_live_handoff_v2669")


def args(**overrides: object) -> argparse.Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2669-test-"))
    defaults: dict[str, object] = {
        "dry_run": True,
        "run_live": False,
        "write_report": False,
        "report_path": root / "report.md",
        "build_v2668_artifacts": False,
        "v2668_build_root": v2669.v2668.DEFAULT_BUILD_ROOT,
        "v2668_manifest_path": v2669.v2668.DEFAULT_MANIFEST,
        "helper_path": None,
        "helper_sha256": None,
        "preload_path": None,
        "preload_sha256": None,
        ...
        "helper_timeout": 90.0,
        "android_root_recheck_attempts": v2669.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_ATTEMPTS,
        "android_root_recheck_sleep_sec": v2669.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_SLEEP_SEC,
        "android_settle_adb_retry_attempts": v2669.v2490.DEFAULT_SETTLE_ADB_RETRY_ATTEMPTS,
        "android_settle_adb_retry_sleep_sec": v2669.v2490.DEFAULT_SETTLE_ADB_RETRY_SLEEP_SEC,
        "readelf": "readelf",
        "file": "file",
    }
...
def setcal_event(seq: int, cal_type: int, cal_size: int, mem_handle: int, dmabuf_status: str) -> dict[str, object]:
    """Build a synthetic setcal_capture event matching the V2668 preload schema."""
    return {
...
class NativeAudioAcdbCustomTopologySetcalCaptureLiveHandoffV2669(unittest.TestCase):
    def test_read_v2668_manifest_checks_setcal_contract(self) -> None:
        manifest = v2669.read_v2668_manifest(v2669.v2668.DEFAULT_MANIFEST)
    ...
    def test_summarize_records_phase_common_stages(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-phase-common-"))
        write_jsonl(
            root / "acdb-v2668-direct-real-common-events.jsonl",
            [
                {"event": "v2668_direct_real_common", "stage": "init_common_enter"},
                {"event": "v2668_direct_real_common", "stage": "init_patch_initialized_flag_return"},
                {"event": "v2668_direct_real_common", "stage": "init_before_real_common"},
                {"event": "v2668_direct_real_common", "stage": "init_real_common_return"},
                {"event": "v2668_direct_real_common", "stage": "init_exit_after_real_common"},
            ],
        )
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))

        summary = v2669.summarize_setcal_capture(root)

        self.assertTrue(summary["direct_real_common_called"])
        self.assertFalse(summary["postinit_real_common_called"])
        self.assertTrue(summary["init_short_success"])
        self.assertEqual(summary["classification"], "v2669-custom-topology-setcal-captured")

    def test_direct_real_common_return_before_setcal_is_partial_frontier_signal(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-phase-common-no-set-"))
        write_jsonl(
            root / "acdb-v2668-direct-real-common-events.jsonl",
            [
                {"event": "v2668_direct_real_common", "stage": "init_common_enter", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "init_patch_initialized_flag_return", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "init_before_real_common", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "init_real_common_return", "code": -92},
            ],
        )

        summary = v2669.summarize_setcal_capture(root)

        self.assertEqual(summary["classification"], "v2669-direct-real-common-returned-before-setcal-no-setcal")
    ...
    def test_init_short_success_before_sigsegv_is_partial_frontier_signal(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-init-short-"))
        write_jsonl(
            root / "acdb-v2668-direct-real-common-events.jsonl",
            [
                {"event": "v2668_direct_real_common", "stage": "init_common_enter", "code": 0},
        ...
        self.assertEqual(summary["classification"], "v2669-init-hook-entered-no-setcal")
        self.assertFalse(summary["success"])
        self.assertTrue(summary["partial_success"])
        self.assertTrue(summary["operator_valuable"])
        self.assertFalse(summary["counts_toward_fails_twice"])
        self.assertTrue(summary["init_short_success"])
        self.assertFalse(summary["direct_real_common_called"])

    def test_summarize_custom_topology_captured(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-full-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))
    ...
    def test_summarize_partial_when_custom_dmabuf_fails(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-partial-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=False))

        summary = v2669.summarize_setcal_capture(root)

        self.assertEqual(summary["classification"], "v2669-custom-topology-setcal-partial")
        self.assertFalse(summary["success"])
        self.assertTrue(summary["partial_success"])
        self.assertTrue(summary["custom_topology_complete"])
    ...
    def test_summarize_records_without_custom_topology(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-nocustom-"))
        write_jsonl(
        ...
        self.assertEqual(summary["classification"], "v2669-setcal-records-no-custom-topology")
        self.assertTrue(summary["partial_success"])
    ...
    def test_summarize_reentry_neutralized_without_setcal_is_partial_evidence(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-reentry-"))
        write_jsonl(
            root / "acdb-v2668-direct-real-common-events.jsonl",
            [
                {"event": "v2668_direct_real_common", "stage": "init_common_enter", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "init_patch_initialized_flag_return", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "init_before_real_common", "code": 0},
                {"event": "v2668_direct_real_common", "stage": "common_reentry_neutralized", "code": 0},
            ],
        )

        summary = v2669.summarize_setcal_capture(root)

        self.assertEqual(summary["classification"], "v2669-phase-common-reentry-neutralized-no-setcal")
        self.assertFalse(summary["success"])
        self.assertTrue(summary["partial_success"])
        self.assertTrue(summary["operator_valuable"])
        self.assertFalse(summary["counts_toward_fails_twice"])
    ...
    def test_summarize_missing_one_custom_type_is_partial(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-missing-"))
        write_jsonl(
            root / "setcal-events.jsonl",
            [
    ...
    def test_summarize_flags_real_set_pass_through(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2669-boundary-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))
        write_jsonl(
        ...
        self.assertEqual(summary["classification"], "v2669-boundary-violation-real-audio-set-passthrough")
        self.assertFalse(summary["success"])
    ...
    def test_summarize_no_pulled_artifacts_detects_preflash_bridge_timeout(self) -> None:
        run_dir = Path(tempfile.mkdtemp(prefix="a90-v2669-preflash-"))
        ...
        summary = v2669.summarize_no_pulled_artifacts(
            {
                "out_dir": str(run_dir.relative_to(v2669.ROOT)) if run_dir.is_relative_to(v2669.ROOT) else str(run_dir),
                "error": "flash-android failed rc=1",
                "counts_toward_fails_twice": None,
            }
        )

        self.assertEqual(summary["classification"], "v2669-preflash-native-bridge-unavailable")
        self.assertEqual(summary["failure_phase"], "native_to_recovery_before_android_flash")
        self.assertFalse(summary["counts_toward_fails_twice"])
        self.assertFalse(summary["operator_valuable"])

    def test_dry_run_uses_v2490_engine_with_v2668_artifacts(self) -> None:
        payload = v2669.dry_run_payload(args())
        ...
        self.assertTrue(payload["v2668_artifacts"]["ok"])
