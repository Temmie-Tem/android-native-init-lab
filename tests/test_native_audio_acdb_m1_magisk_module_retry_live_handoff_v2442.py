"""Host-only tests for the V2442 ACDB M1 Magisk-module retry runner."""
...
v2442 = load_revalidation("native_audio_acdb_m1_magisk_module_retry_live_handoff_v2442")
...
def args(**overrides: object) -> argparse.Namespace:
    values: dict[str, object] = {
        ...
        "module_out_dir": v2442.v2429.DEFAULT_MODULE_OUT_DIR,
        "cc": v2442.v2429.DEFAULT_CC,
        "stimulus_apk": v2442.v2396.DEFAULT_STIMULUS_APK,
        "adb": "adb",
        "serial": None,
        "android_timeout": 420.0,
        "adb_command_timeout": 120.0,
        "flash_timeout": 900.0,
        "duration_ms": v2442.v2396.DEFAULT_DURATION_MS,
        "sample_rate": v2442.v2396.DEFAULT_SAMPLE_RATE,
        "amplitude": v2442.v2396.DEFAULT_AMPLITUDE,
        "active_delay_sec": 0.75,
        "post_delay_sec": 1.0,
        "capture_duration_sec": v2442.DEFAULT_CAPTURE_DURATION_SEC,
        "capture_observe_sec": 6.0,
        "post_module_root_retry_attempts": v2442.DEFAULT_POST_MODULE_ROOT_RETRY_ATTEMPTS,
        "post_module_root_retry_sleep_sec": v2442.DEFAULT_POST_MODULE_ROOT_RETRY_SLEEP_SEC,
        "max_bytes": v2442.v2429.DEFAULT_MAX_BYTES,
        "process_poll_sec": v2442.v2429.DEFAULT_PROCESS_POLL_SEC,
        "from_native": True,
        "approval": None,
        "out_dir": None,
    }
...
class AcdbM1MagiskModuleRetryLiveHandoffV2442(unittest.TestCase):
    def test_dry_run_declares_v2442_retry_boundary(self) -> None:
        payload = v2442.dry_run(args())

        self.assertEqual(payload["run_id"], "V2442")
        self.assertEqual(payload["decision"], "v2442-acdb-m1-magisk-module-retry-live-dry-run")
        self.assertTrue(payload["host_only"])
        self.assertEqual(payload["device_action"], "none")
        self.assertEqual(payload["approval_phrase_required_for_live"], v2442.APPROVAL_PHRASE)
        self.assertEqual(payload["module_lifecycle"]["remote_module_dir"], f"/data/adb/modules/{v2442.v2429.MODULE_ID}")
        ...
        self.assertTrue(payload["module_lifecycle"]["v2442_post_module_reboot_retry"])
        self.assertEqual(
            payload["commands"]["android_post_module_reboot_settle"]["root_retry_attempts"],
            v2442.DEFAULT_POST_MODULE_ROOT_RETRY_ATTEMPTS,
        )
    ...
    def test_pushes_target_shell_writable_incoming_only(self) -> None:
        ...
        push_targets = [
            ...
            if v2442.adb_subcommand(command) == "push"
        ]
    ...
    def test_post_module_reboot_settle_retries_root_until_uid0(self) -> None:
        ...
        original_run_step = v2442.route.run_step
        original_sleep = v2442.time.sleep
        ...
        try:
            v2442.route.run_step = fake_run_step
            v2442.time.sleep = lambda _seconds: None
            with tempfile.TemporaryDirectory() as temp_dir:
                out_dir = Path(temp_dir)
                steps: list[dict[str, object]] = []
                v2442.run_post_module_reboot_settle(
                    args(post_module_root_retry_attempts=3, post_module_root_retry_sleep_sec=0.01),
                    out_dir,
                    steps,
                )
        finally:
            v2442.route.run_step = original_run_step
            v2442.time.sleep = original_sleep
    ...
    def test_run_live_wires_retry_after_module_activation_reboot(self) -> None:
        source = Path(v2442.__file__).read_text()
    ...
    def test_wrong_live_approval_exits_before_device_action(self) -> None:
        script = Path("workspace/public/src/scripts/revalidation/native_audio_acdb_m1_magisk_module_retry_live_handoff_v2442.py")
        ...
        self.assertEqual(payload["decision"], "v2442-acdb-m1-magisk-module-retry-live-refused")
