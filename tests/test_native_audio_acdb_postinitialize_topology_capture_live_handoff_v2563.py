"""Tests for the V2563 post-initialize target-only ACDB live handoff wrapper."""
...
v2563_live = load_revalidation("native_audio_acdb_postinitialize_topology_capture_live_handoff_v2563")
v2563_build = load_revalidation("build_android_acdb_postinitialize_topology_capture_v2563")
...
def args(**overrides: object) -> Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2563-live-test-"))
    defaults: dict[str, object] = {
        "dry_run": False,
        "run_live": False,
        "write_report": False,
        "report_path": root / "report.md",
        "build_v2563_artifacts": False,
        "v2563_build_root": root / "build",
        "v2563_manifest_path": root / "build/manifest.json",
        "helper_path": None,
        "helper_sha256": None,
        "preload_path": None,
        "preload_sha256": None,
...
class NativeAudioAcdbPostinitializeTopologyCaptureLiveHandoffV2563(unittest.TestCase):
    def test_to_v2490_args_forces_combined_preload_and_fake_allocate(self) -> None:
        ...
        base = v2563_live.to_v2490_args(local, artifacts)
    ...
    def test_summary_accepts_only_postinit_nonzero_4916_capture(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2563-summary-ok-"))
        ...
        summary = v2563_live.summarize_postinit_capture(root)

        self.assertEqual(summary["classification"], "v2563-postinitialize-topology-captured")
    ...
    def test_summary_rejects_zero_4916_capture(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2563-summary-zero-"))
        acdbtap = root / "acdbtap"
        raw = acdbtap / "acdbtap-00000002-cmd-00013296-len-00001334.bin"
        raw.parent.mkdir(parents=True, exist_ok=True)
        ...
        self.assertNotEqual(summary["classification"], "v2563-postinitialize-topology-captured")
        self.assertFalse(summary["success"])

    def test_summary_identifies_topology_inside_init_before_target_capture(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2563-summary-init-topology-"))
        ...
        self.assertEqual(summary["classification"], "v2563-init-internal-topology-before-target-only-sigsegv")
    ...
    def test_summary_flags_real_set_passthrough(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2563-summary-set-"))
        ...
        self.assertEqual(summary["classification"], "v2563-boundary-violation-real-audio-set-passthrough")
        self.assertFalse(summary["success"])

    def test_dry_run_reports_not_ready_without_artifacts(self) -> None:
        payload = v2563_live.dry_run_payload(args())

        self.assertFalse(payload["ok"])
        self.assertIn("V2563 post-initialize target-only helper/preload artifacts are not ready", payload["live_blockers"])

    def test_builder_manifest_can_be_selected_when_artifacts_exist(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2563-select-test-"))
        helper = root / "helper"
        ...
        manifest = {
            "ok": True,
            "build": {
                "helper": {"ok": True, "path": v2563_build.rel(helper), "sha256": hashlib.sha256(b"helper").hexdigest()},
                "preload": {"ok": True, "path": v2563_build.rel(preload), "sha256": hashlib.sha256(b"preload").hexdigest()},
            },
            "source_state": {"required": {"helper_has_postinit_fallback_arm": True}},
            "toolchain": {
                "preload_cflags": [
                    "-DA90_ACDBTAP_AUTO_ARM_ON_INITIALIZE=1",
                    "-DA90_ACDBTAP_CUSTOM_TOPOLOGY_ONLY=1",
                ]
            },
        }
        ...
        selected = v2563_live.selected_artifacts(args(v2563_manifest_path=manifest_path))

        self.assertTrue(selected["ok"], selected)
        self.assertTrue(selected["manifest"]["target_only_after_initialize"])
