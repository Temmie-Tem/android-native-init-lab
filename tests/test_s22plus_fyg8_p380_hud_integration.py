...
import test_s22plus_fyg8_p380_hud_renderer as renderer_test
import s22plus_fyg8_p380_research_shell_runtime as runtime
import s22plus_fyg8_p380_research_shell_observer as observer


def source():
    with mock.patch.object(base,'runtime',runtime):value=base.source().replace('p376','p380')
...
'''
    ...
    def test_combined_console_collector_renderer_and_replay(self):
        ...
        self.assertGreater(result.receipt['commands'][5]['stderr']['size'],0)
        self.assertIn(b'MEM_SNAPSHOT early',bytes(audit.rx))
        self.assertIn(b'MEM_SNAPSHOT late',bytes(audit.rx))
        codec=base.live._open_header_initial_observer_module(runtime,observer,'p380-replay')
