"""Host-only tests for the V2563 post-initialize target-only ACDB capture build."""
...
v2563 = load_revalidation("build_android_acdb_postinitialize_topology_capture_v2563")


def args(**overrides: object) -> Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2563-build-test-"))
...
class BuildAndroidAcdbPostinitializeTopologyCaptureV2563(unittest.TestCase):
    def test_source_state_requires_target_only_auto_arm(self) -> None:
        state = v2563.source_state()
    ...
    def test_manifest_declares_target_only_after_initialize_compile_flags(self) -> None:
        payload = v2563.manifest(args())
        ...
        self.assertIn("-DA90_ACDBTAP_AUTO_ARM_ON_INITIALIZE=1", cflags)
        self.assertIn("-DA90_ACDBTAP_CUSTOM_TOPOLOGY_ONLY=1", cflags)
        self.assertIn("-DA90_ACDBTAP_EXIT_ON_TARGET=1", cflags)
        self.assertTrue(payload["capture_contract"]["arm_point"].startswith("auto-arm"))

    @unittest.skipUnless(
        (v2563.TOOLCHAIN_ROOT / "bin/clang").exists() and (v2563.TOOLCHAIN_ROOT / "bin/ld.lld").exists(),
        "private Android clang/lld unavailable",
    )
    def test_build_outputs_helper_and_single_combined_preload(self) -> None:
        payload = v2563.manifest(args(build=True))
        build = payload["build"]
    ...
    def test_cli_writes_manifest(self) -> None:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "workspace/public/src/scripts/revalidation/build_android_acdb_postinitialize_topology_capture_v2563.py",
                "--build-root",
