...
import s22plus_fyg8_p353_display_renderer as renderer
...
class RendererTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        source=source.replace('"type"};','"type", "zpos", "alpha", "fb_translation_mode"};')
...
'''+source[b:]
        ...
        source=source.replace('uint32_t ids[13],n=0;','uint32_t ids[17],n=0;')
        ...
        source=source.replace('q->prop_id<14','q->prop_id<17')
 ...
 assert(o[0]==20&&o[1]==10&&o[2]==30&&c[0]==2&&c[1]==1&&c[2]==10);
 const uint32_t ids[]={2,3,1,4,1,5,6,7,8,9,10,11,12};assert(!memcmp(p,ids,sizeof(ids)));
 assert(v[0]==50&&v[1]==1&&v[2]==20&&v[3]==101&&v[4]==20&&v[5]==0&&v[6]==0&&v[7]==((uint64_t)WIDTH<<16)&&v[8]==((uint64_t)HEIGHT<<16)&&v[9]==0&&v[10]==0&&v[11]==WIDTH&&v[12]==HEIGHT);
 if(q->count_objs==4){assert(o[3]==31&&c[3]==2&&p[13]==4&&p[14]==1&&!v[13]&&!v[14]);}
 commits++;fprintf(stderr,"H0_ATOMIC_COUNT=%u\\n",commits);
 if(!strcmp(scenario,"commit-error")){errno=EIO;return -1;}return 0;}
'''+source[b:]
        source=source[:source.index('int main(int argc')]+'''int main(int argc,char **argv){assert(argc==2);scenario=argv[1];char *args[]={"renderer","--supervised-drm","c353f1e0a90b5e6d7c8a9b0c1d2e3f0b",NULL};return renderer_main(3,args);}
'''
    ...
    def test_negative_inputs_stop_before_commit(self):
        for name in ('foreign-crtc','inherited-fb','duplicate-plane','bad-alpha','missing-property','bad-map','bad-mode'):
