...
class P333OpenEntryDiagnosticObserverTests(unittest.TestCase):
    @staticmethod
    ...
    def _serve_session(
        ...
        *,
        ...
        try:
            ...
            self.assertEqual(
                opened, observer.Frame(runtime.FRAME_OPEN, 0, runtime.P333_RUN_ID)
            )
            self._send(
                peer,
            ...
            self.assertEqual(
                auth.payload,
                observer.compute_open_tag(TEST_KEY, runtime.P333_RUN_ID, nonce),
            )
            self._send(
                peer,
                runtime.FRAME_READY,
                1,
                observer.compute_ready_tag(TEST_KEY, runtime.P333_RUN_ID, nonce),
            )
            for sequence, command in enumerate(runtime.DEFAULT_COMMANDS, start=2):
                ...
                if sequence == 2:
                    ...
                    output = b"Linux p333 5.10.198 aarch64 GNU/Linux\n"
                else:
                    output = f"P328-NONCE {runtime.P333_RUN_ID_HEX}\n".encode()
    ...
    def test_two_sessions_require_entry_then_existing_diagnostics(self) -> None:
        ...
        try:
            with mock.patch.object(
                observer._P332, "_descriptor", wraps=observer._P332._descriptor
            ) as descriptor:
    ...
    def test_binding_retains_same_fd_and_fixed_commands(self) -> None:
        ...
        self.assertEqual(observer.P333_RUN_ID, runtime.P333_RUN_ID)
