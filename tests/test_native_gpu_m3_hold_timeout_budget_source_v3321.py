...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3321_gpu_m3_hold_timeout_budget.py"
)


class NativeGpuM3HoldTimeoutBudgetSourceV3321Tests(unittest.TestCase):
    def test_v3321_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3321")
        self.assertEqual(runner.INIT_VERSION, "0.11.92")
        self.assertEqual(runner.INIT_BUILD, "v3321-gpu-m3-hold-timeout-budget")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.92", required)
        self.assertIn(b"v3321-gpu-m3-hold-timeout-budget", required)
        ...
        self.assertIn(b"gpu.m2.graph.parent_timeout_ms=%d", required)
        self.assertIn(b"gpu.m2.graph.timeout_split=render-plus-visual-hold", required)
    ...
    def test_dispatch_routes_m3_extraction_through_shared_2d_present(self) -> None:
        ...
        self.assertIn("GPU_M2_GRAPH_PARENT_TIMEOUT_MARGIN_MS", source)
        ...
        self.assertIn("gpu.m2.graph.parent_timeout_ms=%d", source)
        self.assertIn("gpu.m2.graph.timeout_split=render-plus-visual-hold", source)
        self.assertIn("parent_timeout_ms += hold_ms + GPU_M2_GRAPH_PARENT_TIMEOUT_MARGIN_MS", source)
        self.assertIn("deadline_ms = monotonic_millis() + parent_timeout_ms", source)
    ...
    def test_builder_manifest_records_m3_extraction_validation(self) -> None:
        ...
        self.assertEqual(manifest["default_hold_ms"], 60000)
        ...
        self.assertIn("require-render-plus-visual-hold-timeout-split", manifest["next_live_validation"])
        self.assertIn("require-parent-timeout-125000", manifest["next_live_validation"])
        self.assertIn("require-hold-ms-60000", manifest["next_live_validation"])
        self.assertIn("require-not-timed-out", manifest["next_live_validation"])
        ...
        self.assertIn("gpu_2d_present_v1", report)
        self.assertIn("render timeout plus visual hold budget", report)
        self.assertIn("parent_timeout_ms=125000", report)
