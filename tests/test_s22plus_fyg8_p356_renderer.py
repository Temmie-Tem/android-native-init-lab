...
import s22plus_fyg8_p356_display_renderer as renderer
...
int fake_nanosleep(const struct timespec *d,struct timespec *r){
 ...
 for(unsigned y=0;y<HEIGHT;y++)for(unsigned x=0;x<PITCH/4;x++){
  uint32_t want=x<WIDTH?0x00ff00ff:0x00ffffff;
...
'''+source[b:]
        source=source[:source.index('int main(int argc')]+'''int main(int argc,char **argv){assert(argc==2);scenario=argv[1];char *args[]={"renderer","--supervised-drm","c356f1e0a90b5e6d7c8a9b0c1d2e3f0b",NULL};return renderer_main(3,args);}
'''
