...
class DeviceActionProcessV2DocsTest(unittest.TestCase):
    ...
    def test_frontier_records_closed_p313_without_reuse(self):
        normalized_goal = " ".join(self.goal.split())
        self.assertIn("P3.13 is the latest closed live unit", normalized_goal)
        ...
        self.assertIn("exactly one restorative restart", normalized_goal)
        self.assertIn("revoke every cycle-causal claim", normalized_goal)
        self.assertIn("6,741 failure round trips pass", normalized_goal)
        self.assertIn("all 1,200 B outputs", normalized_goal)
