...
class CampaignLedgerTaxonomyTest(unittest.TestCase):
    ...
    def test_review_label_counts_are_not_unresolved_review_debt(self):
        ...
        # V2 successor and the current-authority repin are independently
        # reviewed; the candidate/baseline cross-binding is review-pending;
        # older pending topics remain explicit rather than inferred from prose.
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                ...
                "h0-process-v2-candidate-baseline-crossbind-47",
        ...
        self.assertEqual(current["total"], 70)
        self.assertEqual(current["resolved_count"], 52)
        self.assertEqual(current["unresolved_count"], 18)
        self.assertEqual(
            [item["review_topic"] for item in current["resolved"][-4:]],
            [
                "pref1-normal-reboot-live",
        ...
        self.assertEqual(
            current["resolved"][-1],
            {
                ...
                "review_topic": "process-v2-current-authority-repin",
                "pending_ordinal": "h0-process-v2-current-authority-repin-46",
                "pending_action": (
                    "P319_PROCESS_V2_CURRENT_AUTHORITY_REPIN_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-process-v2-current-authority-repin-review-46"
                ),
                "resolution_action": (
                    "PASS_GO_P319_PROCESS_V2_CURRENT_AUTHORITY_REPIN_"
                    "H0_CAPABILITY_V1"
                ),
            },
        )
    ...
    def _assert_topic_29_resolution_is_scoped(self):
        ...
        self.assertEqual(len(all_rows), 409)
