...
class CampaignLedgerTaxonomyTest(unittest.TestCase):
    ...
    def test_review_label_counts_are_not_unresolved_review_debt(self):
        ...
        # candidate/baseline cross-binding and the repaired stock-adapter
        # registration are independently reviewed; the post-registration
        # prerequisite/integration repin remains pending; older pending topics
        # remain explicit rather than inferred from prose.
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                ...
                "log-harvest-runner",
                "mux-module-chain",
                "process-v2-post-registration-repin",
                "raw-first-observer",
        ...
        self.assertEqual(
            ...
            [
                ...
                "h0-process-v2-post-registration-repin-49",
        ...
        self.assertEqual(current["total"], 72)
        self.assertEqual(current["resolved_count"], 54)
        self.assertEqual(current["unresolved_count"], 18)
        self.assertEqual(
            [item["review_topic"] for item in current["resolved"][-4:]],
            [
                "process-v2-integration-v2",
                "process-v2-current-authority-repin",
                "process-v2-candidate-baseline-crossbind",
                "process-v2-stock-adapter-registration",
            ],
        )
        self.assertEqual(
            current["resolved"][-1],
            {
                ...
                "review_topic": "process-v2-stock-adapter-registration",
                "pending_ordinal": "h0-process-v2-stock-adapter-registration-48",
                "pending_action": (
                    "P319_PROCESS_V2_STOCK_ADAPTER_REGISTRATION_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-process-v2-stock-adapter-registration-review-48"
                ),
                "resolution_action": (
                    "PASS_GO_P319_PROCESS_V2_STOCK_ADAPTER_REGISTRATION_"
                    "H0_CAPABILITY_V1"
                ),
            },
        )
    ...
    def _assert_topic_29_resolution_is_scoped(self):
        ...
        self.assertEqual(len(all_rows), 416)
