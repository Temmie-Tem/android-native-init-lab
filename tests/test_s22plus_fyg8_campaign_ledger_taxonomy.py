...
class CampaignLedgerTaxonomyTest(unittest.TestCase):
    ...
    def test_review_label_counts_are_not_unresolved_review_debt(self):
        ...
        # 32, 34, and 37 through 43 are independently reviewed. Current recent
        # pending topics are 33, 35, and 36; the pre-F1 policy, coordinator,
        # journal, live integration, D0 V3 repin, and the Process-v2 integration
        # V2 successor, the current-authority repin, and the repaired
        ...
        # P319 candidate-static, offline-ready, census-decoupled, and prepared/
        # runtime-bound successors are independently reviewed. Older pending
        ...
        self.assertEqual(current["total"], 78)
        self.assertEqual(current["resolved_count"], 60)
        self.assertEqual(current["unresolved_count"], 18)
        self.assertEqual(
            [item["review_topic"] for item in current["resolved"][-5:]],
            [
                "process-v2-candidate-static",
                "process-v2-offline-ready",
                "process-v2-offline-ready-census-decouple",
        ...
        self.assertEqual(
            current["resolved"][-1],
            {
                "campaign": "s22plus-fyg8-p326",
                "review_topic": "bidirectional-acm-busybox-console",
                "pending_ordinal": "h0-bidirectional-acm-busybox-console-1",
                "pending_action": (
                    "P326_BIDIRECTIONAL_ACM_BUSYBOX_CONSOLE_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-bidirectional-acm-busybox-console-review-1"
                ),
                "resolution_action": "PASS_GO_P326_H0",
            },
        )
        self.assertEqual(
            [
        ...
        self.assertEqual(
            current["resolved"][-32],
            {
                ...
                "pending_ordinal": "h0-stock-candidate-qualification-24",
                "pending_action": (
                    "P319_STOCK_CANDIDATE_QUALIFICATION_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
        ...
        self.assertEqual(
            current["resolved"][-30],
            {
                ...
                "pending_ordinal": "h0-raw-first-cross-target-membership-27",
                "pending_action": (
                    "P319_RAW_FIRST_CROSS_TARGET_MEMBERSHIP_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-raw-first-cross-target-membership-review-27"
                ),
                "resolution_action": (
        ...
        self.assertEqual(
            current["resolved"][-28],
            {
                ...
                "resolution_ordinal": (
                    "h0-process-v2-integration-prerequisites-review-29"
                ),
                "resolution_action": (
        ...
        self.assertEqual(
            ...
            {
                "campaign": "s22plus-fyg8-p319",
                "review_topic": "process-v2-contract-repin",
                "pending_ordinal": "h0-process-v2-contract-repin-31",
                "pending_action": (
                    "P319_PROCESS_V2_CONTRACT_REPIN_AND_SUITE_CARDINALITY_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": "h0-process-v2-contract-repin-review-31",
                "resolution_action": (
                    "PASS_GO_P319_PROCESS_V2_CONTRACT_REPIN_AND_SUITE_"
                    "CARDINALITY_H0_CAPABILITY_V1"
                ),
            },
        )
        self.assertEqual(
            ...
            {
                "campaign": "s22plus-fyg8-p319",
                "review_topic": "download-request-cut-recovery",
                "pending_ordinal": "h0-download-request-cut-recovery-32",
                "pending_action": (
                    "P319_DOWNLOAD_REQUEST_CUT_RECOVERY_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-download-request-cut-recovery-review-32"
                ),
                "resolution_action": (
                    "PASS_GO_P319_DOWNLOAD_REQUEST_CUT_RECOVERY_"
                    "H0_CAPABILITY_V1"
                ),
            },
        )
        ...
        self.assertTrue(
            {
                ...
                for item in scoped["pass_go_resolving_no_obligation"]
            }.issubset(
                {
                    tuple(sorted(item.items()))
                    for item in current["pass_go_resolving_no_obligation"]
                }
            )
        )
        ...
        self._assert_topic_29_resolution_is_scoped()

    def _assert_topic_29_resolution_is_scoped(self):
        marker = self.auditor.MARKER
        all_lines = self.ledger_data.split(marker, 1)[1].splitlines(keepends=True)
        all_rows, _, _ = self.auditor.parse_log_rows(all_lines)
        followups = [
        ...
        # The append-only ledger legitimately grows after this review pair.
        self.assertGreaterEqual(len(all_rows), 467)
        ...
        review = reviews[0]
        self.assertEqual(
            self.auditor.resolution_review_topic(review),
            "process-v2-integration-prerequisites",
        )
        self.assertEqual(
            self.auditor.capability_review_state(
                review["tier"], review["action"]
            ),
            "PASS_GO",
        )
        ...
        self.assertEqual(
            [
                item
                for item in obligations["resolved"]
                if item["review_topic"] == "process-v2-contract-repin"
            ],
            [
                {
                    "campaign": "s22plus-fyg8-p319",
    ...
    def test_unrelated_pass_go_cannot_resolve_open_topic(self):
        ...
        self.assertEqual(
            obligations["pass_go_resolving_no_obligation_count"],
            live["pass_go_resolving_no_obligation_count"] + 1,
        )
