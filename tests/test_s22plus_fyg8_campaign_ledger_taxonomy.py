...
class CampaignLedgerTaxonomyTest(unittest.TestCase):
    ...
    def test_review_label_counts_are_not_unresolved_review_debt(self):
        ...
        # candidate/baseline cross-binding are independently reviewed; the
        # stock-adapter registration remains review-pending; older pending
        ...
        self.assertEqual(
            ...
            [
                ...
                "h0-process-v2-stock-adapter-registration-48",
        ...
        self.assertEqual(current["total"], 71)
        self.assertEqual(current["resolved_count"], 53)
        self.assertEqual(current["unresolved_count"], 18)
        self.assertEqual(
            [item["review_topic"] for item in current["resolved"][-4:]],
            [
                "d0-fresh-baseline-v3-repin",
        ...
        self.assertEqual(
            current["resolved"][-1],
            {
                ...
                "review_topic": "process-v2-candidate-baseline-crossbind",
                "pending_ordinal": "h0-process-v2-candidate-baseline-crossbind-47",
                "pending_action": (
                    "P319_PROCESS_V2_CANDIDATE_BASELINE_CROSSBIND_"
                    "IMPLEMENTED_REVIEW_PENDING"
                ),
                "resolution_ordinal": (
                    "h0-process-v2-candidate-baseline-crossbind-review-47"
                ),
                "resolution_action": (
                    "PASS_GO_P319_PROCESS_V2_CANDIDATE_BASELINE_CROSSBIND_"
                    "H0_CAPABILITY_V1"
                ),
            },
        )
    ...
    def _assert_topic_29_resolution_is_scoped(self):
        ...
        self.assertEqual(len(all_rows), 412)
