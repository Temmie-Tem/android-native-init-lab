...
SOURCE = ROOT / (
    ...
    "s22plus_fyg8_p332_closed_result_finalizer.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("p332_closed_finalizer_tested", SOURCE)
...
class P332ClosedResultFinalizerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_scope_is_one_closed_run_and_no_device_backend(self):
        ...
        self.assertEqual(module.EXPECTED_PRE_STATE[0], 31_209)
        self.assertEqual(module.EXPECTED_FINAL_STATE[0], 32_779)
        self.assertEqual(module.EXPECTED_RESULT[0], 36_558)
    ...
    def test_final_state_uses_terminal_bound_without_widening_core(self):
        module = self.module
        pre = b"pre"
        value = {"schema": "device_action_f1_live_state_v2", "closed": True}
        payload = json.dumps(value, indent=2, sort_keys=True).encode() + b"\n"
        with tempfile.TemporaryDirectory() as name:
            path = Path(name) / "live-state.json"
            path.write_bytes(pre)
            path.chmod(0o400)
            core = mock.Mock(MAX_RECORD=module.CORE_MAX_RECORD, MAX_RESULT_RECORD=module.MAX_FINAL_RECORD)

            def write(target, written, limit):
            ...
            core._write_atomic_bounded.side_effect = write
            with (
                ...
                mock.patch.object(
                    ...
                    (len(payload), module._sha256(payload)),
                ),
            ):
                self.assertTrue(module._publish_state(core, value, payload))
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            class Core:
                ...
                def _write_exclusive_bounded(target, written, limit):
                    self.assertEqual(limit, module.MAX_FINAL_RECORD)
                    target.write_bytes(json.dumps(written, indent=2, sort_keys=True).encode() + b"\n")
                    target.chmod(0o400)

            with (
                mock.patch.object(module, "RESULT_PATH", path),
                mock.patch.object(
                    module, "EXPECTED_RESULT", (len(payload), module._sha256(payload))
                ),
            ):
    ...
    def test_audit_reports_zero_device_effects(self):
        module = self.module
        value = {
        ...
        with mock.patch.object(
            module,
            "reconstruct",
            return_value=(value, payload, state, state_payload, "pre", object(), object()),
        ):
        ...
        self.assertEqual(
            hashlib.sha256(payload).hexdigest(), result["result"]["sha256"]
        )
