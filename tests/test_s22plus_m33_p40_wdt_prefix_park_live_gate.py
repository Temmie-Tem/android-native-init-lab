...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m33_p40_wdt_prefix_park_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m33_p40_wdt_prefix_park_live_gate", SCRIPT)
...
class S22PlusM33P40WdtPrefixParkLiveGateTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_policy_markers_include_hashes_tokens_and_prefix_semantics(self):
        ...
        self.assertIn("prefix_targets=40", markers)
        self.assertIn("module_load_only=1", markers)
        self.assertIn("full-M32-module-closure-without-configfs prefix", markers)
        self.assertIn("ACM function module included", markers)
        self.assertIn("no runtime ACM/configfs binding", markers)
    ...
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m33_p40_manifest_hashes=", text)
            self.assertIn("m33_manifest_safety=", text)
            self.assertIn("m33_p40_manifest_closure=", text)
