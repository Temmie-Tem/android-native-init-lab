...
class StatusEvidence(unittest.TestCase):
    ...
    def test_optional_memory_records_preserve_raw_identity_and_other_guards(self):
        for record in (b'HUD_MEM seq=3 ms=3000 alloc=3 retired=2 live=1 peak=2 bytes=10183680\n',
                       b'HUD_MEM malformed-optional-observation\n'):
            raw=log()+record;proof=observer.hud_log_proof(raw)
            self.assertTrue(proof['proved']);self.assertEqual(proof['log'],observer.identity(raw))
            self.assertFalse(observer.hud_log_proof(raw[:-1])['proved'])
            self.assertFalse(observer.hud_log_proof(raw+b'HUD_UNKNOWN 1\n')['proved'])
        self.assertFalse(observer.hud_log_proof(log()+b'HUD_MEM '+b'x'*262144+b'\n')['proved'])
