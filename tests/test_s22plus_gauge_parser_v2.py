...
import tempfile
import unittest
import test_s22plus_max77705_telemetry_v2 as wrapper
from test_s22plus_status_metrics_v2 import SOURCE,ROOT

class GaugeParser(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        wrapper.DiagnosticWrapper.setUpClass()
        cls.addClassCleanup(wrapper.DiagnosticWrapper.doClassCleanups)
        cls.temp=tempfile.TemporaryDirectory();cls.addClassCleanup(cls.temp.cleanup)
        cls.binary=Path(cls.temp.name)/'parser'
        text=SOURCE[:SOURCE.index('int main(')].replace("s22plus_status_metrics_v2.inc.c","s22plus_status_metrics_v3.inc.c")+r'''
int main(void){
 assert(status_collect("invalid")==126);
 char text[1024];assert(fgets(text,sizeof(text),stdin));struct gauge_sample sample;
 if(gauge_parse_reason(text,&sample)!=GAUGE_PARSE_OK)return 7;
...
'''
        ...
        cls.record=subprocess.check_output([wrapper.DiagnosticWrapper.binary,'emit'],text=True)
