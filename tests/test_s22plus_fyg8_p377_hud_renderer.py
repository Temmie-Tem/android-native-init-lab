...
import s22plus_fyg8_p377_display_renderer as renderer
ROOT=base.ROOT


def fixture():
    source,headers=base.fixture()
    source=source.replace('0x76','0x77').replace('c376','c377')
 ...
 if(f==3){
  ...
  assert(size==72&&flags==(MSG_DONTWAIT|MSG_TRUNC));
  ...
  struct status_metrics value={.magic=0x31545353U,.sequence=++sequence,.valid=31,.collected_ms=1,
    .mem_total_kib=8*1024*1024,.mem_available_kib=3*1024*1024,.cpu_permille=357,.battery_pct=100,.charge=4,.battery_temp_deci=262};
  const unsigned char id[16]={0xc3,0x77,0xf1,0xe0,0xa9,0x0b,0x5e,0x6d,0x7c,0x8a,0x9b,0x0c,0x1d,0x2e,0x3f,0x0b};memcpy(value.run,id,16);
  if(!strcmp(scenario,"metrics-malformed"))value.valid=32;
  ...
  memcpy(out,&value,72);empty=1;return 72;
 }
 static unsigned seq,empty;assert''')
    source=source.replace('paint(&b,0xffffffffU);','run_id="c377f1e0a90b5e6d7c8a9b0c1d2e3f0b";paint(&b,0xffffffffU);')
