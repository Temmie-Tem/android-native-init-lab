...
class NativeGpuH5LinearTriangleKmsSourceV3293Tests(unittest.TestCase):
    ...
    def test_h5_uses_linear_snapshot_and_stronger_success_gate(self) -> None:
        ...
        self.assertIn("gpu.h5.kms.h3_linear_readback_changed_count=%u", source)
        self.assertIn("h3->linear_readback_changed_count == 0U", source)
        self.assertIn("gpu.h5.kms.result=h3-linear-readback-failed", source)
        self.assertIn("h3-linear-readback-kms-presented", source)
        self.assertIn("A2D LINEARIZED H3 COLOR", source)
