"""Host-only tests for the V2562 post-init armed ACDB capture build."""
...
v2562 = load_revalidation("build_android_acdb_postinit_armed_capture_v2562")


def args(**overrides: object) -> Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2562-build-test-"))
...
class BuildAndroidAcdbPostinitArmedCaptureV2562(unittest.TestCase):
    def test_source_state_requires_manual_post_init_arm(self) -> None:
        state = v2562.source_state()
        ...
        self.assertTrue(state["required"]["helper_arms_after_init"])
        self.assertTrue(state["required"]["helper_calls_common_topology_after_arm"])

    def test_manifest_declares_manual_arm_only_compile_flags(self) -> None:
        payload = v2562.manifest(args())
        ...
        self.assertEqual(payload["capture_contract"]["arm_point"].split()[0], "helper")

    @unittest.skipUnless(
        (v2562.TOOLCHAIN_ROOT / "bin/clang").exists() and (v2562.TOOLCHAIN_ROOT / "bin/ld.lld").exists(),
        "private Android clang/lld unavailable",
    )
    def test_build_outputs_helper_and_single_combined_preload(self) -> None:
        payload = v2562.manifest(args(build=True))
        build = payload["build"]
    ...
    def test_cli_writes_manifest(self) -> None:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "workspace/public/src/scripts/revalidation/build_android_acdb_postinit_armed_capture_v2562.py",
                "--build-root",
