...
class P344AdapterTests(unittest.TestCase):
    def test_raw_parser_binds_current_id_without_relabeling_predecessor(self) -> None:
        ...
        current = raw_for(adapter.P344_RUN_ID)
        ...
        self.assertEqual(value["run_id"], adapter.P344_RUN_ID_HEX)
        self.assertFalse(value["predecessor_raw_relabelled"])
        previous = raw_for(adapter.P343_PREDECESSOR_RUN_ID)
        ...
        self.assertEqual(rejected["raw_parser_bound_run_id"], adapter.P344_RUN_ID_HEX)

    def test_audit_is_fresh_identity_only_and_no_live_authority(self) -> None:
        value = adapter.audit()
        self.assertEqual(value["schema"], adapter.SCHEMA)
        self.assertEqual(value["run_id"], adapter.P344_RUN_ID_HEX)
        self.assertEqual(value["predecessor_run_id"], adapter.P343_PREDECESSOR_RUN_ID_HEX)
    ...
    def test_acceptance_round_trip_and_exact_lease_schema(self) -> None:
        ...
        self.assertEqual(
            value["observer_contract"]["resident_lease_schema"], adapter.LEASE_SCHEMA
        )
    ...
    def test_contract_requires_identity_only_projection(self) -> None:
        contract = {
            ...
            "payload_abi": adapter.P320_PAYLOAD_ABI,
            "observer_receipt_size": adapter.OBSERVER_RECEIPT_SIZE,
        ...
        stale = dict(contract)
        stale["userspace_overlay_contract_id"] = "s22plus-fyg8-p343-readonly-exploration-v1"
        with self.assertRaises(adapter.ContractError):
