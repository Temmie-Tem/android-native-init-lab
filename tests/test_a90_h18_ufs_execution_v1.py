...
class A90H18UfsExecutionV1Tests(unittest.TestCase):
    @classmethod
    ...
    def _baseline(self) -> tuple[dict, dict]:
        ...
        result = {
            "schema": "a90-h17-ufs-f1-result-v1",
    ...
    def _candidate_native_health(self, selftest_text: str) -> tuple[dict, dict]:
        ...
        native = {
            ...
            "version": self._framed_receipt(
                "version",
                "version: 0.11.186 "
                "build=phase3-minimal-h18-post-root-failure-attribution\n",
            ),
    ...
    def test_f1_identity_is_h18_only(self) -> None:
        self.assertEqual(
            self.f1.CAPABILITY,
            "A90_H18_POST_ROOT_FAILURE_ATTRIBUTION_V1",
        )
        self.assertEqual(self.f1.CURRENT_VERSION, "0.11.185")
        self.assertEqual(self.f1.CANDIDATE_VERSION, "0.11.186")
        self.assertEqual(
            self.f1.CANDIDATE_BUILD,
            "phase3-minimal-h18-post-root-failure-attribution",
        )
        self.assertEqual(self.f1.CANDIDATE_BOOT_SIZE, 58384384)
        self.assertEqual(
        ...
        self.assertEqual(self.f1.CANDIDATE_AB_RECEIPT_SIZE, 4927)
    ...
    def test_auto_status_rejects_contradictory_duplicate_fact(self) -> None:
        ...
        with self.assertRaisesRegex(self.f1.ContractError, "not unique"):
            self.f1.validate_h18_auto_status_record(record, enable=0, latch=0)
    ...
    def test_h17_predecessor_requires_exact_installed_boot(self) -> None:
        manifest, result = self._baseline()
        self.f1._baseline_inputs(manifest, result)
        manifest["candidate_boot"]["sha256"] = "0" * 64
        with self.assertRaisesRegex(self.f1.ContractError, "H17 resident"):
            self.f1._baseline_inputs(manifest, result)

    def test_h17_d1_predecessor_requires_exact_seven_record_terminal(self) -> None:
        self.assertEqual(len(self.f1.H17_D1_RECORDS), 7)
        self.assertEqual(
            self.f1.H17_D1_TERMINAL_RESULT_SHA256,
        ...
        value = {
            ...
            "records": [
                ...
                for name, size, sha in self.f1.H17_D1_RECORDS[:5]
            ],
            "terminal_result_sha256": self.f1.H17_D1_TERMINAL_RESULT_SHA256,
        }
        with self.assertRaisesRegex(self.f1.ContractError, "terminal binding"):
            self.f1.validate_h17_d1_terminal(value, {}, {})

    def test_h17_live_health_does_not_use_stale_shared_allowlist(self) -> None:
        receipts = {
    ...
    def test_reconcile_h17_health_checks_bridge_before_device_reads(self) -> None:
        events: list[str] = []
        receipts = {
    ...
    def test_d1_install_result_must_equal_deep_closed_f1_journal(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            transaction = Path(temporary) / "h18-f1-live"
    ...
    def test_approval_binds_persistent_mode_and_observer(self) -> None:
        manifest = {
            "run_id": "a90-h18-ufs-f1-20260810-01",
            "target": {
    ...
    def test_live_result_pass_stays_health_pending_and_open(self) -> None:
        ...
        self.assertEqual(result["status"], "PASS_A90_H18_PERSISTENT_SERVER_LIVE")
        self.assertEqual(
    ...
    def test_live_result_refuses_unconfirmed_visibility(self) -> None:
        ...
        self.assertEqual(result["status"], "NO_PROOF_A90_H18_PERSISTENT_SERVER_LIVE")
    ...
    def test_userdata_probe_accepts_dynamic_exact_devt(self) -> None:
        record = {
            "command": ["placeholder"],
            "text": "A90H18_POST_PHYSICAL_RETURN devt=260:9 "
    ...
    def test_same_intent_probe_requires_exact_enable_latch_and_evidence_bytes(self) -> None:
        ...
        enable = self.d1.hashlib.sha256(
            self.d1._expected_h18_state(intent, "armed-after-native-health")
        ).hexdigest()
        latch = self.d1.hashlib.sha256(
            self.d1._expected_h18_state(
                intent,
    ...
    def test_missing_diagnostic_is_no_proof_not_replay_authority(self) -> None:
        ...
        self.assertEqual(value["status"], "NO_PROOF_H18_FAILURE_ATTRIBUTION")
        self.assertTrue(value["cleanup_proof"])
        self.assertIn(
