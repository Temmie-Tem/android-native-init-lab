...
import s22plus_fyg8_p355_display_renderer as renderer
...
'''+source[b:]
 ...
 assert(v[0]==50&&v[1]==1&&v[2]==0&&v[3]==20&&v[4]==101&&v[5]==20&&v[6]==0&&v[7]==0&&v[8]==((uint64_t)WIDTH<<16)&&v[9]==((uint64_t)HEIGHT<<16)&&v[10]==0&&v[11]==0&&v[12]==WIDTH&&v[13]==HEIGHT&&v[14]==0x80ff00ffULL);
...
'''+source[b:]
        source=source[:source.index('int main(int argc')]+'''int main(int argc,char **argv){assert(argc==2);scenario=argv[1];char *args[]={"renderer","--supervised-drm","c355f1e0a90b5e6d7c8a9b0c1d2e3f0b",NULL};return renderer_main(3,args);}
'''
    ...
    def test_negative_inputs_stop_before_commit(self):
        for name in ('foreign-crtc','inherited-fb','duplicate-plane','bad-alpha','missing-property','bad-map','bad-mode','absent-noise','duplicate-noise','absent-fill','duplicate-fill'):
