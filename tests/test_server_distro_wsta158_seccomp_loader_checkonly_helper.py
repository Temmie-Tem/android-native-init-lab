...
runner = load_script("workspace/public/src/scripts/server-distro/run_wsta158_seccomp_loader_checkonly_helper.py")


class ServerDistroWsta158SeccompLoaderCheckonlyHelperTests(unittest.TestCase):
...
"""
    ...
    def test_valid_filter_artifact_builds_checkonly_helper_and_blocks_apply(self) -> None:
        ...
        with self.private_tmp() as tmp:
            ...
            result = runner.run(runner.build_arg_parser().parse_args([
                ...
                str(object_path),
                "--emit-seccomp-loader-checkonly-helper",
            ]))
            manifest = json.loads((root / "wsta158" / runner.MANIFEST_NAME).read_text(encoding="utf-8"))
            check_stdout = (root / "wsta158" / runner.CHECK_STDOUT_NAME).read_text(encoding="utf-8")
            service_stdout = (root / "wsta158" / runner.SERVICE_STDOUT_NAME).read_text(encoding="utf-8")
            apply_stdout = (root / "wsta158" / runner.APPLY_STDOUT_NAME).read_text(encoding="utf-8")
            binary_exists = (root / "wsta158" / runner.HELPER_BINARY_NAME).is_file()

        self.assertEqual(result["decision"], runner.PASS_DECISION)
        self.assertTrue(binary_exists)
        self.assertEqual(manifest["state"], "SECCOMP_LOADER_HELPER_CHECK_ONLY_LINKED")
        self.assertEqual(manifest["default_mode"], "check-only")
        self.assertFalse(manifest["load_enabled"])
        self.assertFalse(manifest["enforced"])
        self.assertIn("A90WSTA158_LINKED_SERVICE_COUNT=4", check_stdout)
        self.assertIn("policy_service=dpublic-smoke-httpd", check_stdout)
        self.assertIn("policy_service=cloudflared-quick-tunnel", check_stdout)
        self.assertIn("policy_service=dropbear-admin-usb", check_stdout)
        self.assertIn("policy_service=dpublic-hud-intent", check_stdout)
        self.assertIn("a90_seccomp_loader_decision=check-only", check_stdout)
        self.assertIn("service=dpublic-hud policy_service=dpublic-hud-intent", service_stdout)
        self.assertIn("A90WSTA158_SECCOMP_LOAD=0", apply_stdout)
        self.assertIn("blocked-allow-load-required", apply_stdout)
    ...
    def test_gate_blocks_without_explicit_flag_or_private_artifacts(self) -> None:
        ...
        self.assertEqual(result["decision"], "wsta158-blocked-explicit-gate-required")
        ...
        self.assertEqual(result["decision"], "wsta158-blocked-filter-manifest-nonprivate")
    ...
    def test_invalid_filter_object_sha_blocks(self) -> None:
        ...
        self.assertEqual(result["decision"], "wsta158-blocked-filter-artifact-invalid")
