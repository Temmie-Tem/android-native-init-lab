"""Tests for the V2657 custom-topology ACDB SET capture live wrapper."""
...
v2657 = load_revalidation("native_audio_acdb_custom_topology_real_common_setcal_capture_live_handoff_v2657")


def args(**overrides: object) -> argparse.Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2657-test-"))
    defaults: dict[str, object] = {
        "dry_run": True,
        "run_live": False,
        "write_report": False,
        "report_path": root / "report.md",
        "build_v2656_artifacts": False,
        "v2656_build_root": v2657.v2656.DEFAULT_BUILD_ROOT,
        "v2656_manifest_path": v2657.v2656.DEFAULT_MANIFEST,
        "helper_path": None,
        "helper_sha256": None,
        "preload_path": None,
        "preload_sha256": None,
        ...
        "helper_timeout": 90.0,
        "android_root_recheck_attempts": v2657.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_ATTEMPTS,
        "android_root_recheck_sleep_sec": v2657.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_SLEEP_SEC,
        "android_settle_adb_retry_attempts": v2657.v2490.DEFAULT_SETTLE_ADB_RETRY_ATTEMPTS,
        "android_settle_adb_retry_sleep_sec": v2657.v2490.DEFAULT_SETTLE_ADB_RETRY_SLEEP_SEC,
        "readelf": "readelf",
        "file": "file",
    }
...
def setcal_event(seq: int, cal_type: int, cal_size: int, mem_handle: int, dmabuf_status: str) -> dict[str, object]:
    """Build a synthetic setcal_capture event matching the V2656 preload schema."""
    return {
...
class NativeAudioAcdbCustomTopologySetcalCaptureLiveHandoffV2657(unittest.TestCase):
    def test_read_v2656_manifest_checks_setcal_contract(self) -> None:
        manifest = v2657.read_v2656_manifest(v2657.v2656.DEFAULT_MANIFEST)

        self.assertTrue(manifest["ok"], manifest)
        self.assertTrue(manifest["setcal_contract_ok"], manifest)

    def test_summarize_records_real_common_stages(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-real-common-"))
        write_jsonl(
            root / "acdb-v2608-preinit-no-send-events.jsonl",
            [
                {"event": "v2608_preinit_no_send", "stage": "entered_common_topology_hook"},
                {"event": "v2608_preinit_no_send", "stage": "before_real_common_topology"},
                {"event": "v2608_preinit_no_send", "stage": "real_common_topology_return"},
                {"event": "v2608_preinit_no_send", "stage": "patch_initialized_flag_return"},
            ],
        )
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))

        summary = v2657.summarize_setcal_capture(root)

        self.assertTrue(summary["real_common_topology_called"])
        self.assertFalse(summary["skipped_common_topology"])
        self.assertEqual(summary["classification"], "v2657-custom-topology-setcal-captured")

    def test_real_common_return_before_setcal_is_partial_frontier_signal(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-real-common-no-set-"))
        write_jsonl(
            root / "acdb-v2608-preinit-no-send-events.jsonl",
            [
                {"event": "v2608_preinit_no_send", "stage": "entered_common_topology_hook", "code": 0},
                {"event": "v2608_preinit_no_send", "stage": "before_real_common_topology", "code": 0},
                {"event": "v2608_preinit_no_send", "stage": "real_common_topology_return", "code": -92},
                {"event": "v2608_preinit_no_send", "stage": "patch_initialized_flag_return", "code": 0},
            ],
        )

        summary = v2657.summarize_setcal_capture(root)

        self.assertEqual(summary["classification"], "v2657-real-common-returned-before-setcal-no-setcal")
        ...
        self.assertEqual(summary["real_common_return_codes"], [-92])

    def test_summarize_custom_topology_captured(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-full-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))
    ...
    def test_summarize_partial_when_custom_dmabuf_fails(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-partial-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=False))

        summary = v2657.summarize_setcal_capture(root)

        self.assertEqual(summary["classification"], "v2657-custom-topology-setcal-partial")
        self.assertFalse(summary["success"])
        self.assertTrue(summary["partial_success"])
        self.assertTrue(summary["custom_topology_complete"])
    ...
    def test_summarize_records_without_custom_topology(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-nocustom-"))
        write_jsonl(
        ...
        self.assertEqual(summary["classification"], "v2657-setcal-records-no-custom-topology")
        self.assertTrue(summary["partial_success"])
    ...
    def test_summarize_preinit_skip_common_topology_is_partial_evidence(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-preinit-"))
        write_jsonl(
            root / "acdb-v2608-preinit-no-send-events.jsonl",
            [
                {"event": "v2608_preinit_no_send", "stage": "entered_common_topology_hook"},
        ...
        self.assertEqual(summary["classification"], "v2657-common-topology-hook-skipped-real-call-no-setcal")
        self.assertFalse(summary["success"])
        self.assertTrue(summary["partial_success"])
        self.assertTrue(summary["operator_valuable"])
        self.assertFalse(summary["counts_toward_fails_twice"])
    ...
    def test_summarize_missing_one_custom_type_is_partial(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-missing-"))
        write_jsonl(
            root / "setcal-events.jsonl",
            [
    ...
    def test_summarize_flags_real_set_pass_through(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2657-boundary-"))
        write_jsonl(root / "setcal-events.jsonl", custom_topology_rows(all_dumped=True))
        write_jsonl(
        ...
        self.assertEqual(summary["classification"], "v2657-boundary-violation-real-audio-set-passthrough")
        self.assertFalse(summary["success"])
    ...
    def test_summarize_no_pulled_artifacts_detects_preflash_bridge_timeout(self) -> None:
        run_dir = Path(tempfile.mkdtemp(prefix="a90-v2657-preflash-"))
        ...
        summary = v2657.summarize_no_pulled_artifacts(
            {
                "out_dir": str(run_dir.relative_to(v2657.ROOT)) if run_dir.is_relative_to(v2657.ROOT) else str(run_dir),
                "error": "flash-android failed rc=1",
                "counts_toward_fails_twice": None,
            }
        )

        self.assertEqual(summary["classification"], "v2657-preflash-native-bridge-unavailable")
        self.assertEqual(summary["failure_phase"], "native_to_recovery_before_android_flash")
        self.assertFalse(summary["counts_toward_fails_twice"])
        self.assertFalse(summary["operator_valuable"])

    def test_dry_run_uses_v2490_engine_with_v2656_artifacts(self) -> None:
        payload = v2657.dry_run_payload(args())
        ...
        self.assertTrue(payload["v2656_artifacts"]["ok"])
