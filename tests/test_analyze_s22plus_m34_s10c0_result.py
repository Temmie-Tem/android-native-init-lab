...
SCRIPT = Path("workspace/public/src/scripts/revalidation/analyze_s22plus_m34_s10c0_result.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("analyze_s22plus_m34_s10c0_result", SCRIPT)
...
class AnalyzeS22PlusM34S10C0ResultTest(unittest.TestCase):
    def setUp(self):
    ...
    def result_payload(self, result: str, rc: int = 0) -> dict:
        return {
            ...
            "module_load_probe": self.module.EXPECTED_PROBE,
            "probe_module": self.module.EXPECTED_PROBE_MODULE,
            "probe_proc_name": self.module.EXPECTED_PROBE_PROC_NAME,
        }
    ...
    def test_hit_with_complete_timeline_advances_to_s11(self):
        analysis = self.module.classify_result(
            self.result_payload("download-beacon-hit"),
            self.timeline_payload(),
        )

        self.assertEqual(analysis["decision"], self.module.DECISION_FINIT_ACCEPTED_PROCEED_S11)
        self.assertTrue(analysis["ok_to_advance"])
        self.assertTrue(analysis["cmd_db_direct_finit_observed"])
        self.assertTrue(analysis["cmd_db_direct_finit_accepted"])
        self.assertEqual(analysis["next_stage"], "S11")
        self.assertEqual(analysis["next_probe"], "per_module_rc_positive_control")
    ...
    def test_hit_with_stock_rollback_advances_but_requires_magisk_baseline_restore(self):
    ...
    def test_miss_with_complete_timeline_advances_to_s11_errno_capture(self):
        analysis = self.module.classify_result(
            self.result_payload("download-beacon-miss-parked-manual-download-required"),
            self.timeline_payload(),
        )

        self.assertEqual(analysis["decision"], self.module.DECISION_FINIT_MISS_PROCEED_S11)
        self.assertTrue(analysis["ok_to_advance"])
        self.assertTrue(analysis["cmd_db_direct_finit_observed"])
        self.assertFalse(analysis["cmd_db_direct_finit_accepted"])
        self.assertEqual(analysis["next_stage"], "S11")
        self.assertEqual(analysis["next_probe"], "cmd_db_direct_finit_errno_capture")
    ...
    def test_nonzero_rc_requires_recovery_even_if_cmd_db_direct_finit_was_observed(self):
        analysis = self.module.classify_result(
    ...
    def test_rollback_only_result_is_not_cmd_db_direct_finit_proof(self):
        analysis = self.module.classify_result(
    ...
    def test_cli_write_report_writes_analysis_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = json.loads((run_dir / "s22plus_m34_s10c0_result_analysis.json").read_text(encoding="utf-8"))
            self.assertEqual(report["decision"], self.module.DECISION_FINIT_ACCEPTED_PROCEED_S11)
            self.assertEqual(report["result_json"], str(result_json))
            self.assertEqual(report["timeline_json"], str(timeline_json))

    def test_cli_write_report_redacts_android_serial(self):
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp)
            result_json = run_dir / "result.json"
            timeline_json = run_dir / "timeline.json"
            payload = self.result_payload("download-beacon-hit")
            payload["android_serial"] = "S22_TEST_SERIAL_123"
            ...
            self.assertEqual(rc, 0)
            self.assertNotIn("S22_TEST_SERIAL_123", stdout.getvalue())
            report = json.loads((run_dir / "s22plus_m34_s10c0_result_analysis.json").read_text(encoding="utf-8"))
            self.assertEqual(report["android_serial"], self.module.DISPLAY_SERIAL_REDACTED)
            self.assertNotIn("S22_TEST_SERIAL_123", json.dumps(report))
    ...
    def test_cli_require_advance_allows_clean_miss(self):
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp)
            result_json = run_dir / "result.json"
            timeline_json = run_dir / "timeline.json"
    ...
    def test_cli_require_live_next_stage_allows_clean_miss_with_magisk_rollback(self):
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp)
            result_json = run_dir / "result.json"
            timeline_json = run_dir / "timeline.json"
