...
class ServerDistroWsta58RenewalManualStopProofTests(unittest.TestCase):
    ...
    def test_cloudflared_egress_allowlist_gate_and_nested_args(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            args = self.live_args(root)
            lease = self.make_artifact(root, "egress")
            args.enable_cloudflared_egress_allowlist = True

            self.assertEqual(
                runner.explicit_live_gate(args),
                (False, "wsta58-blocked-cloudflared-egress-allowlist-proof-required"),
            )
            args.force_cloudflared_egress_allowlist_proof = True
            self.assertEqual(
                runner.explicit_live_gate(args),
                (False, "wsta58-blocked-cloudflared-egress-route-required"),
            )
            ...
            nested = runner.wsta55_args(args, root, lease, "egress")
