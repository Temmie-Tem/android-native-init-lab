"""Focused host-only P344 static and preparation checks."""
...
import prepare_s22plus_fyg8_p344_process_v2 as prepare  # noqa: E402
import s22plus_fyg8_p344_process_v2_candidate_static as candidate_static  # noqa: E402


class P344ProcessV2Tests(unittest.TestCase):
    @classmethod
    ...
    def test_static_binds_initial_proof_and_named_catalog(self) -> None:
        self.assertEqual(
            self.static["schema"],
            "s22plus_fyg8_p344_process_v2_candidate_static_v1",
        )
        ...
        self.assertTrue(self.static["runtime_behavior_unchanged"])
        self.assertTrue(self.static["default_runtime_behavior_unchanged"])
        self.assertFalse(self.static["catalog_allowlist_expanded"])
        self.assertEqual(
            self.static["catalog_allowlist_actions"],
            ["kernel", "processes", "mounts", "memory", "usb-state"],
        )
        for name in (
            "p344_exploration_session",
            "p344_exploration_action",
            "p344_readonly_exploration",
    ...
    def test_prepare_reopens_three_promotion_payloads(self) -> None:
        ...
        self.assertEqual(
            ...
            "P344_STOCK_OBSERVER_V4_NAMED_EXPLORATION",
        )
        ...
        self.assertEqual(
            ...
            "device_action_f1_p344_stock_offline_contract_v1",
        )
    ...
    def test_predecessor_identity_cannot_be_relabelled_as_p344(self) -> None:
        ...
        with self.assertRaises(evidence.EvidenceError):
            evidence.validate_p344_open_read_branch_proof(
                evidence.p343_authenticated_open_read_branch_observer_spec()
            )
