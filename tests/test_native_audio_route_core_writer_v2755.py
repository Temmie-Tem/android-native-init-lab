...
class NativeAudioRouteCoreWriterV2755(unittest.TestCase):
    def test_only_core_layer_is_write_allowed(self) -> None:
        ...
        self.assertIn('static bool audio_route_layer_write_allowed(const char *layer)', text)
        self.assertIn('strcmp(layer, "core") == 0', text)
        self.assertIn('write_mode && !audio_route_layer_write_allowed(layer)', text)
    ...
    def test_writer_iterates_apply_forward_and_reset_reverse(self) -> None:
        ...
        self.assertRegex(
            text,
            re.compile(
                r'if \(reset_mode\).*?for \(index = audio_route_control_count\(\) - 1; index >= 0; --index\)',
                re.DOTALL,
            ),
        )
        self.assertRegex(
            text,
            re.compile(
                r'else \{\s*for \(index = 0; index < audio_route_control_count\(\); \+\+index\)',
                re.DOTALL,
            ),
        )
    ...
    def test_non_core_layers_remain_blocked_before_any_write_attempt(self) -> None:
        ...
        refusal = text.index('if (write_mode && !audio_route_layer_write_allowed(layer))')
