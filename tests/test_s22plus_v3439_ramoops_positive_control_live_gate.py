...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "s22plus_v3439_ramoops_positive_control_live_gate", SCRIPT
    )
...
class S22PlusV3439RamoopsPositiveControlLiveGateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            dtbo_text = " ".join(
                (
                    ...
                    self.module.V3438_POSTMORTEM_SHA256,
                    self.module.DTBO_ACK_TOKEN,
    ...
    def test_corrected_backend_gate_accepts_sysfs_proof_without_early_log(self):
        live = {
            "status": "6f6b617900",
        ...
        def fake_root_bytes(_serial, command, _timeout=30.0):
            if command.startswith("for n in mem_size"):
                return (
                    b"mem_size=2097152\n"
                    b"pmsg_size=1048576\n"
                    b"console_size=524288\n"
                    b"record_size=262144\n"
                )
            if command.startswith("set -eu"):
                return (
                    b"pstore_mount=1\n"
                    b"pmsg0=1\n"
                    b"pstore_backend=ramoops\n"
                    b"bound_device=ramoops_region\n"
                    b"bound_compatible=ramoops\n"
                    b"bound_status=okay\n"
                    b"bound_count=1\n"
                )
        ...
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            ...
            self.module, "live_dt_hex", side_effect=lambda _serial, name: live[name]
        ), mock.patch.object(
            self.module, "root_bytes", side_effect=fake_root_bytes
        ):
            result = self.module.verify_patched_backend(
                "SERIAL", Path(temp) / "gate.log"
            )
        self.assertEqual(result["backend"]["pstore_backend"], "ramoops")
        self.assertEqual(result["backend"]["bound_count"], "1")
        self.assertEqual(result["early_log"], "")

    def test_corrected_backend_gate_rejects_competing_backend(self):
        live = {
            "status": "6f6b617900",
            "size": "0000000000200000",
            "pmsg-size": "00100000",
        ...
        def fake_root_bytes(_serial, command, _timeout=30.0):
            ...
            if command.startswith("set -eu"):
                return (
                    b"pstore_mount=1\n"
                    b"pmsg0=1\n"
                    b"pstore_backend=samsung,pstore_pmsg\n"
                    b"bound_count=0\n"
                )
            if command.startswith("dmesg"):
                return b""
        ...
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            ...
            with self.assertRaisesRegex(
                self.module.GateError, "backend/binding proof mismatch"
            ):
                self.module.verify_patched_backend(
                    "SERIAL", Path(temp) / "gate.log"
                )
