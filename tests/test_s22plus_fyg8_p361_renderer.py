...
import s22plus_fyg8_p361_display_renderer as renderer
...
int fake_nanosleep(const struct timespec *d,struct timespec *r){
 assert(!r&&handles==2);
 assert(d->tv_sec==1);
 if(commits<=10){if((!strcmp(scenario,"sleep-error")&&commits==1)||(!strcmp(scenario,"middle-sleep-error")&&commits==6)){errno=EINTR;return -1;}return 0;}
 assert(commits==11);
 ...
 puts("H0_STATIC_HELD eleven_commits=1 two_buffers=1 cleanup=0");fflush(stdout);_Exit(0);
}
''' + source[b:]
...
'''+source[b:]
 ...
 assert(handles==2);
 if(commits>=1){assert(commits<=10);verify_paints();assert(q->flags==0&&q->user_data==0&&q->count_objs==1&&o[0]==30&&c[0]==1&&p[0]==4&&v[0]==(commits%2?102U:101U));
 commits++;fprintf(stderr,"H0_ATOMIC_COUNT=%u\\n",commits);
 if((!strcmp(scenario,"second-error")&&commits==2)||(!strcmp(scenario,"middle-error")&&commits==7)||(!strcmp(scenario,"last-error")&&commits==11)){errno=EIO;return -1;}return 0;}
...
'''+source[b:]
        source=source[:source.index('int main(int argc')]+'''int main(int argc,char **argv){assert(argc==2);scenario=argv[1];char *args[]={"renderer","--supervised-drm","c361f1e0a90b5e6d7c8a9b0c1d2e3f0b",NULL};return renderer_main(3,args);}
'''
    ...
    def test_two_retained_frames_and_inherited_plane_replacement(self):
        for name in ('success','inherited-zero-fb'):
            ...
            self.assertEqual(result.stderr.count(b'H0_ATOMIC_COUNT='),11)
    ...
    def test_second_error_has_no_retry(self):
        ...
        self.assertIn(b'stage=repeat-frame-commit',result.stderr);self.assertEqual(result.stderr.count(b'H0_ATOMIC_COUNT='),2)
    ...
    def test_middle_and_last_failure_stop_without_later_commit(self):
        for name,count in (('middle-error',7),('last-error',11),('middle-sleep-error',6)):
            result=self.run_case(name);self.assertEqual(result.returncode,1,(name,result.stderr))
            self.assertEqual(result.stderr.count(b'H0_ATOMIC_COUNT='),count)
            self.assertIn(b'DISPLAY_FAIL',result.stderr)
