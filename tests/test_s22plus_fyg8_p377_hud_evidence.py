...
def log():
    return b'METRICS_START 122\nHUD_START 123\n'+b''.join((
        'HUD_FRAME run='+observer.RUN_ID_HEX+f' seq={i} uptime_ms={i*1000} state=1 metrics_seq={i} valid=3 age_ms=10 mem_total=8192000 mem_available=4000000 cpu_permille=200 battery_pct=0 charge=0 temp_deci=0 event=matched visible=UNPROVED\n').encode() for i in range(1,4))
...
class StatusEvidence(unittest.TestCase):
    ...
    def test_partial_foreign_stale_invalid_or_duplicate_metrics(self):
        cases=[log()[:-1],log().replace(observer.RUN_ID_HEX.encode(),b'0'*32),
            ...
            log().replace(b'valid=3',b'valid=1'),log().replace(b'valid=3',b'valid=35'),
