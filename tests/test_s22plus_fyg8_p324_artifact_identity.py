...
P323_IMAGE = ROOT / (
    ...
    "stock-candidate-build-v1-20260831-03/inputs/fixed-Image"
)
P323_AP = ROOT / (
    "workspace/private/outputs/s22plus_fyg8_p323/"
    "stock-candidate-build-v1-20260831-03/candidate-a/odin4/AP.tar.md5"
)
...
def load_module():
    spec = importlib.util.spec_from_file_location("p324_artifact_identity", SOURCE)
    if spec is None or spec.loader is None:
        raise AssertionError("P324 artifact helper cannot load")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class P324ArtifactIdentityTests(unittest.TestCase):
    @classmethod
    ...
    def test_fresh_image_transform_is_deterministic_and_exactly_bound(self) -> None:
        ...
        self.assertEqual(first_meta["target"]["run_id_hex"], self.artifact.P324_RUN_ID_HEX)
        self.assertEqual(
            self.artifact.validate_image(first)["run_id_hex"],
            self.artifact.P324_RUN_ID_HEX,
        )
        for old in (
    ...
    def test_predecessor_image_and_ap_are_rejected(self) -> None:
        with self.assertRaises(self.artifact.ArtifactIdentityError):
            self.artifact.validate_image(P323_IMAGE.read_bytes())
        with self.assertRaises(self.artifact.ArtifactIdentityError):
            self.artifact.inspect_ap(P323_AP, label="P323 predecessor AP")
