...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m34_s7a_session_producer_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m34_s7a_session_producer_live_gate", SCRIPT)
...
class S22PlusM34S7ASessionProducerLiveGateTest(unittest.TestCase):
    def setUp(self):
        self.module = load_module()

    def test_policy_markers_include_hashes_tokens_and_s7a_semantics(self):
    ...
    def test_verify_agents_exception_rejects_draft_only_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(SystemExit) as caught:
                self.module.verify_agents_exception(root, log_path)
            self.assertIn("draft-only M34 S7A", str(caught.exception))
            self.assertIn("agents_exception_draft_only_present=1", log_path.read_text(encoding="utf-8"))

    @unittest.skipUnless(MANIFEST.exists(), "private M34 v0.6 manifest missing")
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            log_path = Path(tmp) / "m34_s7a_manifest_check.log"
            self.module.verify_m34_manifest(MANIFEST, log_path)
            text = log_path.read_text(encoding="utf-8")
            self.assertIn("m34_s7a_manifest_hashes=", text)
            self.assertIn("m34_manifest_safety=", text)
            self.assertIn("m34_s7a_manifest_runtime_steps=", text)
            self.assertIn("m34_s7a_manifest_closure=", text)
            self.assertEqual(len(self.module.EXPECTED_MODULES), 83)
    ...
    def test_acm_match_requires_stock_samsung_vid_pid(self):
        self.assertTrue(self.module.is_m34_s7a_acm({"vendor": "04E8", "product": "6860"}))
        self.assertFalse(self.module.is_m34_s7a_acm({"vendor": "04e8", "product": "685d"}))
        self.assertFalse(self.module.is_m34_s7a_acm({"vendor": "18d1", "product": "4ee7"}))
