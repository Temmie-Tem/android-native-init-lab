...
import s22plus_fyg8_p359_display_renderer as renderer
...
int fake_nanosleep(const struct timespec *d,struct timespec *r){
 ...
 for(unsigned y=0;y<HEIGHT;y++)for(unsigned x=0;x<PITCH/4;x++){
  ...
  if(x>=8&&x<WIDTH-8&&y>=8&&y<HEIGHT-8){
   want=y<1170?(x<540?0xff0000ff:0xffff0000):0xff00ff00;
...
'''+source[b:]
        source=source[:source.index('int main(int argc')]+'''int main(int argc,char **argv){assert(argc==2);scenario=argv[1];char *args[]={"renderer","--supervised-drm","c359f1e0a90b5e6d7c8a9b0c1d2e3f0a",NULL};return renderer_main(3,args);}
'''
    ...
    def test_negative_inputs_stop_before_commit(self):
        for name in ('foreign-crtc','inherited-fb','duplicate-plane','bad-alpha','missing-property','bad-map','bad-mode','absent-noise','duplicate-noise','absent-fill','duplicate-fill','unsupported-abgr'):
