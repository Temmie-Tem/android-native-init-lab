...
import s22plus_fyg8_p337_open_read_diag_runtime as p337_runtime  # noqa: E402
...
def _proof_fixture() -> dict[str, object]:
    ...
    return {
        ...
        "run_id_hex": runtime.P338_RUN_ID_HEX,
...
class P338ProcessV2Tests(unittest.TestCase):
    ...
    def test_p337_failure_fixture_is_not_relabelled_as_p338(self) -> None:
        frame = p336_observer.encode_frame(
            p337_runtime.DIAGNOSTIC_FRAME_TYPE,
            0,
            p337_observer.predecessor.DIAGNOSTIC.pack(
                p337_runtime.DIAGNOSTIC_STAGE_OPEN_READ_RESULT, -71
            ),
        )
        with self.assertRaises(observer.P338ObserverBindingError):
            observer.parse_retained_open_read_branch(
                runtime.DEVICE_BANNER + frame
            )

    def test_p337_proof_is_rejected_before_compatibility_projection(self) -> None:
        stale = _proof_fixture()
        stale["schema"] = p337_observer.SCHEMA
        stale["contract_id"] = p337_observer.CONTRACT_ID
        stale["target"] = p337_runtime.TARGET
        stale["run_id_hex"] = p337_runtime.P337_RUN_ID_HEX
        with mock.patch.object(
            evidence,
            "validate_p337_open_read_diagnostic_proof",
            side_effect=AssertionError("unexpected P337 projection"),
        ):
            with self.assertRaises(evidence.EvidenceError):
                evidence.validate_p338_open_read_branch_proof(stale)

    def test_fresh_proof_checks_every_command_and_session(self) -> None:
        value = _proof_fixture()
        self.assertIs(
            evidence.validate_p338_open_read_branch_proof(value), value
        )
        changed = copy.deepcopy(value)
        changed["sessions"][2]["commands"][1]["command_sha256"] = "0" * 64  # type: ignore[index]
        with self.assertRaises(evidence.EvidenceError):
            evidence.validate_p338_open_read_branch_proof(changed)

    def test_runtime_and_adapter_are_fresh_and_non_retrying(self) -> None:
        self.assertEqual(adapter.audit()["run_id"], runtime.P338_RUN_ID_HEX)
        self.assertEqual(
            adapter.audit()["predecessor_run_id_rejected"],
            runtime.P337_PREDECESSOR_RUN_ID_HEX,
        )
        self.assertEqual(
            runtime.audit_binding()["open_read_branch_count"], 4
        )
    ...
    def test_p338_receipt_keeps_own_namespace_and_no_retry(self) -> None:
        raw = json.loads(json.dumps({
            ...
            "open_read_branch_ordinals": dict(
                evidence.P338_AUTH_EXEC_OPEN_READ_BRANCH_ORDINALS
            ),
        ...
        with (
            ...
            value = live._p338_validate_receipt(  # noqa: SLF001
                object(), Path("/unused"), {}
            )
    ...
    def test_builder_static_and_ready_manifest_are_host_only(self) -> None:
        ...
        self.assertEqual(static_value["run_id"], runtime.P338_RUN_ID_HEX)
        ...
        self.assertEqual(
            ...
            runtime.P338_RUN_ID_HEX,
        )
        self.assertEqual(
            bundle.receipt["observation_contract"]["verification"]["schema"],
            "device_action_f1_p338_stock_offline_contract_v1",
        )
