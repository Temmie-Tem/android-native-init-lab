#!/usr/bin/env python3
"""Focused tests for the P2.94 Process-v2 offline-promotion adapter."""
...
import prepare_s22plus_fyg8_p294_process_v2 as adapter  # noqa: E402
import s22plus_fyg8_p294_candidate_static_checker as checker  # noqa: E402
import s22plus_fyg8_p294_e2_stock_closure as closure_selector  # noqa: E402
import s22plus_fyg8_p294_source_contract as contract  # noqa: E402
import s22plus_fyg8_p294_telemetry_decoder as telemetry_decoder  # noqa: E402


class P294ProcessV2PromotionAdapterTest(unittest.TestCase):
    ...
    def test_defaults_are_p294_scoped(self) -> None:
        args = adapter.parse_args([])
        for value in (args.candidate_static, args.candidate_ap, args.out):
            self.assertIn("s22plus_fyg8_p294", value.as_posix())

    def test_registered_decoder_and_e2_closure_select_p294(self) -> None:
        ...
        self.assertEqual(
            evidence.P294_CANDIDATE_STATIC_SCHEMA, checker.SCHEMA
        )
        self.assertEqual(
            evidence.P294_CANDIDATE_STATIC_VERDICT, checker.VERDICT
        )

    def test_execution_receipts_bind_all_three_p294_tiers(self) -> None:
        ...
        self.assertEqual(
            len([name for name in receipts if name.startswith("p294_tier2_")]),
            66,
        )
        self.assertEqual(
            len([name for name in receipts if name.startswith("p294_tier3_")]),
            3,
        )

    def test_adapter_has_no_live_or_device_transport(self) -> None:
        source = (
            SCRIPT_DIR / "prepare_s22plus_fyg8_p294_process_v2.py"
