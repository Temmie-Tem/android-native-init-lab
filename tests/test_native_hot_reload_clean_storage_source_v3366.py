...
import build_native_init_boot_v3366_hot_reload_clean_storage as runner  # noqa: E402


class NativeHotReloadCleanStorageSourceV3366Test(unittest.TestCase):
    def test_required_strings_cover_h3_storage_cleanliness_contract(self) -> None:
        required = b"\n".join(runner.REQUIRED_STRINGS)
        self.assertIn(b"0.11.127", required)
        self.assertIn(b"v3366-hot-reload-clean-storage", required)
    ...
    def test_report_states_source_build_not_live_h3(self) -> None:
        manifest = {
            "boot_image": "workspace/private/inputs/boot_images/boot_linux_v3366_hot_reload_clean_storage.img",
            "boot_sha256": "0" * 64,
            "helper_sha256": "1" * 64,
        }
        report = runner.render_report(manifest, ("flag-a",), ("flag-b",))
        self.assertIn("Decision: `v3366-hot-reload-clean-storage-source-build`", report)
        self.assertIn("H3 cleanup candidate", report)
        self.assertIn("already-mounted rw", report)
        self.assertIn("No live H3 reload result is claimed", report)
    ...
    def test_reload_command_and_fast_path_are_preserved(self) -> None:
        ...
        self.assertIn("Hot-reload: skipping autohud/netservice/rshell re-init", main_source)
