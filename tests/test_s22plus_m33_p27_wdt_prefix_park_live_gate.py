...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m33_p27_wdt_prefix_park_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m33_p27_wdt_prefix_park_live_gate", SCRIPT)
...
class S22PlusM33P27WdtPrefixParkLiveGateTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_policy_markers_include_hashes_tokens_and_prefix_semantics(self):
        ...
        self.assertIn("prefix_targets=27", markers)
        self.assertIn("module_load_only=1", markers)
        self.assertIn("SMMU and HS/eUSB2 PHY prefix", markers)
        self.assertIn("no DWC3", markers)
    ...
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m33_p27_manifest_hashes=", text)
            self.assertIn("m33_manifest_safety=", text)
            self.assertIn("m33_p27_manifest_closure=", text)
            self.assertEqual(len(self.module.EXPECTED_MODULES), 40)
