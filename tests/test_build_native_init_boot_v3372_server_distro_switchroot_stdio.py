"""Regression tests for V3372 server-distro D3B switch_root source build."""
...
builder = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3372_server_distro_switchroot_stdio.py"
)


class BuildNativeInitBootV3372ServerDistroSwitchrootTests(unittest.TestCase):
    def test_builder_identity_and_required_markers(self) -> None:
        self.assertEqual(builder.CYCLE, "V3372")
        self.assertEqual(builder.INIT_VERSION, "0.11.133")
        self.assertEqual(builder.INIT_BUILD, "v3372-server-distro-switchroot-stdio")
    ...
    def test_switchroot_module_is_gated_sd_only_and_execs_switch_root(self) -> None:
        ...
        self.assertIn("d3_path_is_mounted(\"/dev\")", source)
        self.assertIn("d3_prepare_new_dev", source)
        self.assertIn('d3_prepare_dev_node("dev/console", 0600, 5, 1)', source)
        self.assertIn('d3_prepare_dev_node("dev/null", 0666, 1, 3)', source)
        self.assertIn("rootfs=unmounted-after-fail", source)
        self.assertIn("console=reuse-stdio", source)
        self.assertIn('"switch_root"', source)
        self.assertNotIn('(char *)"-c"', source)
