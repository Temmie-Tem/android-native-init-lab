"""Host-only tests for the V2508 exec-linked own-process ACDB GET helper build."""
...
v2508 = load_revalidation("build_android_acdb_ownprocess_get_exec_linked_v2508")


def args(**overrides: object) -> Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2508-test-"))
    defaults: dict[str, object] = {
        "dry_run": True,
        "build": False,
        "build_root": root / "build",
        "manifest_path": root / "build/manifest.json",
        "clang": v2508.TOOLCHAIN_ROOT / "bin/clang",
        "lld": v2508.TOOLCHAIN_ROOT / "bin/ld.lld",
        "readelf": "readelf",
        "file": "file",
    }
...
class BuildAndroidAcdbOwnprocessGetExecLinkedV2508(unittest.TestCase):
    def test_source_state_matches_exec_linked_pure_read_boundary(self) -> None:
        state = v2508.source_state()
    ...
    def test_dry_run_is_host_only_and_live_blocked(self) -> None:
        payload = v2508.manifest(args())

        self.assertEqual(payload["decision"], "v2508-acdb-ownprocess-exec-linked-host-only")
        ...
        self.assertEqual(payload["capture_contract"]["required_dt_needed"], v2508.REQUIRED_NEEDED)
    ...
    def test_vendor_lib_state_has_required_symbols_and_closure(self) -> None:
        state = v2508.vendor_lib_state("readelf")
    ...
    @unittest.skipUnless(
        (v2508.TOOLCHAIN_ROOT / "bin/clang").exists()
        and (v2508.TOOLCHAIN_ROOT / "bin/ld.lld").exists()
        and all((v2508.VENDOR_LIB_DIR / name).exists() for name in v2508.REQUIRED_NEEDED),
        "private Android clang/lld or ACDB closure unavailable",
    )
    def test_build_outputs_arm32_pie_with_acdb_dt_needed(self) -> None:
        payload = v2508.manifest(args(build=True))
