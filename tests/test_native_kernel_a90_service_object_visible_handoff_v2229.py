"""Regression tests for native_kernel_a90_service_object_visible_handoff_v2229."""
...
v2229 = load_revalidation("native_kernel_a90_service_object_visible_handoff_v2229")


class CommandAndArtifactHelpers(unittest.TestCase):
    def test_sha256_load_manifest_and_command_rendering(self):
        old_manifest = v2229.V2228_MANIFEST
        try:
            with tempfile.TemporaryDirectory() as tmp:
                data = Path(tmp) / "data.bin"
                data.write_bytes(b"v2229")
                v2229.V2228_MANIFEST = Path(tmp) / "missing.json"

                self.assertEqual(
                    v2229.sha256(data),
                    "286c65cec14dcb6dbee1f7b9faa295ba994ce1d6b3072d139d6b2d84cb60bb56",
                )
                self.assertEqual(v2229.load_build_manifest(), {})
        finally:
            v2229.V2228_MANIFEST = old_manifest

        args = argparse.Namespace(bridge_host="127.0.0.1", bridge_port=54321, timeout=17.0)
        command = v2229.a90ctl_command(args, ["cat", "/cache/result"], allow_error=True)
    ...
    def test_flash_and_dry_run_commands_render_v2228_and_v2189_steps(self):
        image = Path("workspace/private/inputs/boot_images/test.img")
        native = v2229.flash_command(image, "expected version", "abc123", from_native=True)
        recovery = v2229.flash_command(image, "expected version", "abc123", from_native=False)
        ...
        self.assertEqual(len(plan["collect"]), len(v2229.HELPER_REMOTE_PATHS))
        self.assertIn("test-sha", plan["flash_test_boot"])
        self.assertIn("rollback-sha", plan["rollback"])

    def test_diagnose_artifacts_extracts_nonlog_and_service_object_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertEqual(diagnosis["nonlog_control_flow"]["wlfw_bdf_send_ret_hits"], "1")
        self.assertEqual(diagnosis["service_object_visible"]["label"], "service-object-ready-post-bdf")
        self.assertEqual(diagnosis["service_object_visible"]["vndservicemanager_ready"], "1")
        self.assertEqual(diagnosis["service_object_visible"]["wlfw_service_request_seen"], "1")
        self.assertEqual(diagnosis["service_object_visible"]["requested_wlanmdsp"], "0")
    ...
    def test_diagnose_artifacts_classifies_setup_errors_and_unknown(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(v2229.diagnose_artifacts([root_missing])["kind"], "property-root-missing")
            self.assertEqual(v2229.diagnose_artifacts([setup])["kind"], "setup-error")
            self.assertEqual(v2229.diagnose_artifacts([unknown])["kind"], "unknown")
...
class PreflightAndClassification(unittest.TestCase):
    ...
    def test_current_window_preflight_classifiers_accept_absent_and_busy_cases(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(v2229.is_current_window_a90_absent_preflight(absent_result))
            self.assertTrue(v2229.is_current_window_collector_busy_preflight(busy_result))
            self.assertFalse(v2229.is_current_window_a90_absent_preflight({"stdout": "not json"}))

    def test_classify_dry_run_and_live_branches(self):
        ready = v2229.classify({
            "execute": False,
            "preflight": {
                "v2228_manifest_pass": True,
                "test_image_exists": True,
                "test_image_sha_matches_manifest": True,
                "rollback_image_exists": True,
            },
        })
        blocked = v2229.classify({
            "execute": False,
            "preflight": {
                "v2228_manifest_pass": False,
                "test_image_exists": True,
                "test_image_sha_matches_manifest": True,
                "rollback_image_exists": True,
            },
        })
        self.assertEqual(ready["decision"], "v2229-service-object-visible-handoff-dry-run-ready")
        self.assertTrue(ready["pass"])
        self.assertEqual(blocked["decision"], "v2229-service-object-visible-handoff-dry-run-blocked")
        self.assertFalse(blocked["pass"])

        cases = [
            (
                {"execute": True, "live_block": "v2222-preflight-failed"},
                "v2229-service-object-visible-handoff-preflight-failed-no-flash",
                False,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": False}},
                "v2229-service-object-visible-handoff-rollback-selftest-failed",
                False,
            ),
            (
                {"execute": True, "live_block": "test-flash-failed", "rollback": {"ok": True, "selftest_ok": True}},
                "v2229-service-object-visible-handoff-test-flash-failed-rollback-pass",
                False,
            ),
            (
                {
                ...
                "v2229-service-object-visible-helper-property-root-missing-rollback-pass",
                False,
            ),
            (
                {
                ...
                "v2229-service-object-visible-helper-parsed-rollback-pass",
                True,
            ),
            (
                {
                ...
                "v2229-service-object-visible-helper-parse-incomplete-rollback-pass",
                False,
            ),
        ]
        for manifest, decision, passed in cases:
            with self.subTest(decision=decision):
                result = v2229.classify(manifest)
...
class ReportAndResidualState(unittest.TestCase):
    def manifest(self, **overrides):
        manifest = {
            "result": {
                "decision": "v2229-service-object-visible-handoff-dry-run-ready",
                "pass": True,
                "reason": "ready",
            },
            "preflight": {
                ...
                "test_image_sha256": "test-sha",
                "test_expect_version": v2229.TEST_EXPECT_VERSION,
                "rollback_image": "workspace/private/inputs/boot_images/rollback.img",
                "rollback_image_sha256": "rollback-sha",
                "rollback_expect_version": v2229.ROLLBACK_EXPECT_VERSION,
            },
            "execute": False,
            "out_dir": "workspace/private/runs/kernel/v2229",
            "steps": [],
        }
    ...
    def test_render_report_includes_dry_run_plan_and_safety_scope(self):
        report = v2229.render_report(self.manifest(dry_run_commands={"postflight": ["a90ctl.py", "selftest"]}))

        self.assertIn("# Native Init V2229 Service-Object-Visible Handoff Runner", report)
        self.assertIn("Decision: `v2229-service-object-visible-handoff-dry-run-ready`", report)
    ...
    def test_render_report_includes_post_bdf_no_wlan0_diagnosis(self):
        report = v2229.render_report(self.manifest(
            execute=True,
            result={
            ...
            collect={
                ...
                "diagnosis": {
                    ...
                    "wlan0_present": "0",
                    "nonlog_control_flow": {
                        "label": "post-bdf-no-wlan0",
                        "wlfw_bdf_send_ret_hits": "1",
                        "wlfw_bdf_result_log_hits": "1",
                        "wlfw_worker_done_signal_hits": "1",
                    },
        ...
        self.assertIn("CNSS reached PM registration", report)
        self.assertIn("WLFW capability QMI, and BDF send/result edges", report)
        self.assertIn("post-BDF window", report)

    def test_residual_state_tracks_flash_rollback_and_safety_flags(self):
        dry = v2229.residual_state(self.manifest())
        ok = v2229.residual_state(self.manifest(
            execute=True,
            steps=[{"name": "flash-v2228-from-native", "ok": True}],
            rollback={"ok": True, "selftest_ok": True, "attempt": "from-native"},
        ))
        bad = v2229.residual_state(self.manifest(
            execute=True,
            steps=[{"name": "flash-v2228-from-native", "ok": True}],
