...
class A90Phase2DFinalizerTests(unittest.TestCase):
    ...
    def test_minimal_h9_candidate_binds_fresh_rootfs_with_fast_receipt(self) -> None:
        selected = finalizer.select_candidate_profile(
            finalizer.MINIMAL_H9_CANDIDATE_PROFILE
        )
        contract = finalizer.candidate_first_boot_contract(selected)
        ...
        h8_contract = finalizer.candidate_first_boot_contract(
            finalizer.select_candidate_profile(
                finalizer.MINIMAL_H8_CANDIDATE_PROFILE
            )
        )
        self.assertNotEqual(
    ...
    def test_minimal_h12_candidate_binds_wifi_rootfs_and_fresh_namespace(self) -> None:
        selected = finalizer.select_candidate_profile(
            finalizer.MINIMAL_H12_CANDIDATE_PROFILE
        )
        contract = finalizer.candidate_first_boot_contract(selected)
        ...
        self.assertEqual(
            ...
            str(
                finalizer.staging.derive_remote_final(
                    "a90-v3406-debian-display-f1-20260810-07"
                )
            ),
        )
        self.assertEqual(
        ...
        self.assertNotEqual(
            contract["receipt_path"],
            finalizer.candidate_first_boot_contract(
                finalizer.MINIMAL_H11_CANDIDATE
            )["receipt_path"],
        )
