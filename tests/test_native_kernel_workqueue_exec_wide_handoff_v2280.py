"""Regression tests for native_kernel_workqueue_exec_wide_handoff_v2280."""
...
v2280 = load_revalidation("native_kernel_workqueue_exec_wide_handoff_v2280")
...
def write_artifacts(root: Path, *, stack="", codeword="codeword\n", helper=""):
    ...
    paths = {
        "workqueue_wide_log": root / "workqueue_stack.cmdv1.txt",
    ...
    paths["workqueue_wide_log"].write_text(stack, encoding="utf-8")
...
def stack_log(
    *,
    result="v2279-workqueue-exec-wide-sample-ring-complete",
    total=2,
    stored=2,
    printed=2,
...
class ParsingHelpers(unittest.TestCase):
    def test_parse_scalar_fields_and_int_helpers(self):
        self.assertEqual(v2280.parse_scalar_fields("total=0x2 ignored stored=3"), {"total": "0x2", "stored": "3"})
        self.assertEqual(v2280.parse_int("0x20"), 32)
        self.assertEqual(v2280.parse_int(7), 7)

    def test_parse_key_values_preserves_last_values_and_skips_protocol(self):
        values = v2280.parse_key_values(
            "A90P1 BEGIN rc=0\n"
            "helper_exit_code=20\n"
            "helper_exit_code=0\n"
            "not-key-value\n"
        ...
        self.assertEqual(v2280.last_value(values, "helper_exit_code"), "0")
        self.assertEqual(v2280.last_value(values, "missing", "fallback"), "fallback")
        self.assertNotIn("rc", values)

    def test_parse_workqueue_wide_log_reads_stats_samples_stack_ips_and_result(self):
        parsed = v2280.parse_workqueue_wide_log(
            "result=v2279-workqueue-exec-wide-sample-ring-complete\n"
        ...
        self.assertEqual(parsed["result"], "v2279-workqueue-exec-wide-sample-ring-complete")
...
class CodewordAndSymbolHelpers(unittest.TestCase):
    def test_symbol_resolver_returns_containing_symbol_and_none_before_text(self):
        with patch_symbols():
            resolve, symbol_index = v2280.symbol_resolver()
    ...
    def test_analyze_codeword_accepts_v2276_uao_patch_aware_slide(self):
        ...
        with mock.patch.multiple(
            ...
            result = v2280.analyze_codeword("helper stdout")
...
class ArtifactClassification(unittest.TestCase):
    def test_classify_artifacts_reports_function_and_stack_target_hits(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch_codeword(), patch_symbols():
            ...
            classification = v2280.classify_artifacts(paths)

        self.assertEqual(classification["classification"], "workqueue-exec-wide-target-hit")
...
class ManifestReportAndResidual(unittest.TestCase):
    ...
    def test_classify_manifest_covers_dry_run_live_failure_and_success_branches(self):
        ...
        blocked = v2280.classify_manifest({
            "execute": False,
            "preflight": {
                "build_manifest_exists": True,
        ...
        self.assertEqual(ready["decision"], "v2280-workqueue-exec-wide-dry-run-ready")
        self.assertTrue(ready["pass"])
        self.assertEqual(blocked["decision"], "v2280-workqueue-exec-wide-dry-run-blocked")
        self.assertFalse(blocked["pass"])

        cases = [
            (
                {"execute": True, "rollback": {"selftest_ok": False}},
                "v2280-workqueue-exec-wide-rollback-selftest-failed",
                False,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": True}, "live_block": "preflight-current-baseline-failed"},
                "v2280-workqueue-exec-wide-preflight-failed-no-flash",
                False,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": True}, "live_block": "test-flash-failed"},
                "v2280-workqueue-exec-wide-test-flash-failed-rollback-pass",
                False,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": True}, "collect": {"classification": {"classification": "codeword-slide-unusable"}}},
                "v2280-workqueue-exec-wide-live-codeword-slide-unusable-rollback-pass",
                False,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": True}, "collect": {"classification": {"classification": "workqueue-exec-wide-no-target-hit"}}},
                "v2280-workqueue-exec-wide-live-pass-workqueue-exec-wide-no-target-hit",
                True,
            ),
            (
                {"execute": True, "rollback": {"selftest_ok": True}, "collect": {"classification": {"classification": "workqueue-exec-wide-target-hit"}}},
                "v2280-workqueue-exec-wide-live-pass-workqueue-exec-wide-target-hit",
                True,
            ),
        ]
        for manifest, decision, passed in cases:
            with self.subTest(decision=decision):
                classified = v2280.classify_manifest(manifest)
                self.assertEqual(classified["decision"], decision)
                self.assertEqual(classified["pass"], passed)

    def manifest(self, *, execute=False, classification="workqueue-exec-wide-target-hit", **overrides):
        manifest = {
            "result": {"decision": "v2280-workqueue-exec-wide-dry-run-ready", "pass": True, "reason": "ready"},
            "execute": execute,
            "out_dir": "workspace/private/runs/kernel/unit",
            "preflight": {
                "test_image": "workspace/private/inputs/boot_images/test.img",
                "test_image_sha256": "test-sha",
                "test_expect_version": v2280.TEST_EXPECT_VERSION,
                "rollback_image": "workspace/private/inputs/boot_images/rollback.img",
                "rollback_image_sha256": "rollback-sha",
                "rollback_expect_version": v2280.ROLLBACK_EXPECT_VERSION,
            },
            ...
            "collect": {
                "classification": {
                    ...
                    "target_hit_count": 2 if classification == "workqueue-exec-wide-target-hit" else 0,
                    "function_target_hit_count": 1 if classification == "workqueue-exec-wide-target-hit" else 0,
                    "stack_target_hit_count": 1 if classification == "workqueue-exec-wide-target-hit" else 0,
                    "function_target_hits": [{"function_symbol": "request_firmware_work_func"}] if classification == "workqueue-exec-wide-target-hit" else [],
                    "stack_target_hits": [{"symbol": "request_firmware_work_func"}] if classification == "workqueue-exec-wide-target-hit" else [],
                    "workqueue": {
                        "sample_count": 2,
    ...
    def test_render_report_covers_dry_run_and_live_interpretation(self):
        dry_report = v2280.render_report(self.manifest(dry_run_commands={"collect": [["cat", "/cache/log"]]}))
        live_manifest = self.manifest(execute=True)
        live_manifest["result"] = {
            "decision": "v2280-workqueue-exec-wide-live-pass-workqueue-exec-wide-target-hit",
            "pass": True,
            "reason": "live pass",
        }

        live_report = v2280.render_report(live_manifest)

        self.assertIn("# Native Init V2280 Workqueue Execute Wide Live", dry_report)
        self.assertIn("Dry-Run Plan", dry_report)
        self.assertIn("/cache/log", dry_report)
        self.assertIn("Workqueue Wide Classification", live_report)
        self.assertIn("Target hits: total=`2` function=`1` stack=`1`", live_report)
        self.assertIn("same-boot wide execute-start oracle intersects", live_report)
    ...
    def test_residual_state_marks_dry_run_no_touch_and_failed_rollback_cleanup(self):
        dry = v2280.residual_state({"execute": False})
        failed = v2280.residual_state({
            "execute": True,
            "steps": [{"name": "flash-v2279-from-native"}],
            "test_flash": {"ok": True},
