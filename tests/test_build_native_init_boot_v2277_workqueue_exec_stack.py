"""Regression tests for build_native_init_boot_v2277_workqueue_exec_stack."""
...
v2277 = load_revalidation("build_native_init_boot_v2277_workqueue_exec_stack")
...
def fake_v2237_with_base(fake_base):
    ...
    prev2137 = types.SimpleNamespace(
        ...
        HELPER_FLAGS=(
            ...
            v2277.TAIL_SAMPLER_FLAGS[0],
            v2277.WORKQUEUE_SAMPLER_FLAGS[0],
        ),
    )
...
class PatchV2237:
    ...
    def __enter__(self):
        self.old = v2277.v2237
        v2277.v2237 = self.fake
        return self.fake

    def __exit__(self, exc_type, exc, tb):
        v2277.v2237 = self.old
...
class BuildWrapperConfiguration(unittest.TestCase):
    def test_sha256_hashes_file_content(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload.write_bytes(b"v2277")

            digest = v2277.sha256(payload)

        self.assertEqual(digest, "79c327d561cc70154b484d5ae52d682488939867dd8a5325fa9b977a1b569626")

    def test_with_workqueue_flags_deduplicates_bridge_tail_and_exec_stack_flags(self):
        ...
        with PatchV2237(fake):
            flags = v2277.with_workqueue_flags((
                "-DOTHER=1",
                v2277.TAIL_SAMPLER_FLAGS[0],
                v2277.WORKQUEUE_SAMPLER_FLAGS[0],
                "-DSECOND=1",
                v2277.TAIL_SAMPLER_FLAGS[-1],
                v2277.WORKQUEUE_SAMPLER_FLAGS[-1],
            ))

        combined = (*v2277.TAIL_SAMPLER_FLAGS, *v2277.WORKQUEUE_SAMPLER_FLAGS)
        self.assertEqual(flags[-len(combined):], combined)
        self.assertEqual(flags.count(v2277.TAIL_SAMPLER_FLAGS[0]), 1)
        self.assertEqual(flags.count(v2277.WORKQUEUE_SAMPLER_FLAGS[0]), 1)
    ...
    def test_configure_base_rewrites_v2237_axes_and_propagates_all_sampler_flags(self):
        ...
        with PatchV2237(fake):
            helper_flags = v2277.configure_base()

        args = dict(zip(fake_base.DEFAULT_ARGS[0::2], fake_base.DEFAULT_ARGS[1::2]))
        combined = (*v2277.TAIL_SAMPLER_FLAGS, *v2277.WORKQUEUE_SAMPLER_FLAGS)
        self.assertTrue(fake.configured)
        self.assertEqual(fake.OUT_DIR, v2277.OUT_DIR)
        self.assertEqual(fake.REPORT_PATH, v2277.REPORT_PATH)
        self.assertEqual(args["--cycle"], "V2277")
        self.assertEqual(args["--decision"], "v2277-workqueue-exec-stack-source-build-pass")
        self.assertEqual(args["--init-version"], "0.9.275")
        self.assertEqual(args["--init-build"], "v2277-workqueue-exec-stack")
        self.assertEqual(args["--wifi-test-klog-prefix"], "A90v2277")
        self.assertEqual(args["--wifi-test-watch-sec"], "190")
        self.assertEqual(args["--wifi-test-supervisor-timeout-sec"], "245")
        self.assertIn("a90_android_execns_probe_v433_workqueue_exec_stack", args["--helper-binary"])
        ...
        self.assertEqual(fake_base.base.EXTRA_INIT_FLAGS, v2277.EXTRA_INIT_FLAGS)

    def test_build_bpf_helpers_calls_static_builder_for_both_helpers(self):
        calls = []
        old = v2277.build_static_helper
        try:
            def fake_build(source, output):
                calls.append((source, output))
                return f"sha-{output.name}"

            v2277.build_static_helper = fake_build
            shas = v2277.build_bpf_helpers()
        finally:
            v2277.build_static_helper = old

        self.assertEqual(shas, {
            "workqueue": f"sha-{v2277.BPF_HELPER_BINARY.name}",
            "codeword": f"sha-{v2277.TAIL_BPF_HELPER_BINARY.name}",
        })
        self.assertEqual(calls, [
            (v2277.BPF_HELPER_SOURCE, v2277.BPF_HELPER_BINARY),
            (v2277.TAIL_BPF_HELPER_SOURCE, v2277.TAIL_BPF_HELPER_BINARY),
        ])
    ...
    def test_patch_ramdisk_helpers_adds_workqueue_stack_and_codeword_sampler_binaries(self):
        ...
        v2277.patch_ramdisk_helpers(base_wrapper)
        helpers = base_wrapper.base.ramdisk_helpers(types.SimpleNamespace())

        self.assertEqual(helpers["bin/existing"], Path("existing"))
        self.assertEqual(helpers[v2277.BPF_HELPER_RAMDISK_PATH], v2277.BPF_HELPER_BINARY)
        self.assertEqual(helpers[v2277.TAIL_BPF_HELPER_RAMDISK_PATH], v2277.TAIL_BPF_HELPER_BINARY)

    def test_render_report_includes_exec_stack_contract_and_safety_scope(self):
        manifest = {
            "decision": "v2277-workqueue-exec-stack-source-build-pass",
            "boot_image": "workspace/private/inputs/boot_images/boot_linux_v2277.img",
            "boot_sha256": "boot-sha",
            "init_version": "0.9.275",
            "init_build": "v2277-workqueue-exec-stack",
            "helper_marker": "a90_android_execns_probe v433",
        ...
        report = v2277.render_report(
            manifest,
            ("-DOTHER=1", *v2277.TAIL_SAMPLER_FLAGS, *v2277.WORKQUEUE_SAMPLER_FLAGS),
            {"workqueue": "workqueue-sha", "codeword": "codeword-sha"},
        )

        self.assertIn("# Native Init V2277 Workqueue Execute Stack Source Build", report)
        self.assertIn("Decision: `v2277-workqueue-exec-stack-source-build-pass`", report)
        self.assertIn("A90 Linux init 0.9.275 (v2277-workqueue-exec-stack)", report)
        self.assertIn("Workqueue sampler SHA256: `workqueue-sha`", report)
        self.assertIn("Codeword sampler SHA256: `codeword-sha`", report)
        self.assertIn("A90_WIFI_TEST_BOOT_WORKQUEUE_EXEC_STACK_SAMPLER=1", report)
        self.assertIn("A90_WIFI_TEST_BOOT_TAIL_PERF_REGS_CODEWORD_SAMPLER=1", report)
        self.assertIn("workqueue execute_start stack/callsite oracle", report)
...
class MainMetadataUpdate(unittest.TestCase):
    def test_main_rewrites_manifest_and_live_candidate_without_running_real_build(self):
        tmp_parent = v2277.REPO_ROOT / "tmp"
        tmp_parent.mkdir(exist_ok=True)
        with tempfile.TemporaryDirectory(dir=tmp_parent) as tmp:
            root = Path(tmp)
            out_dir = root / "out"
            out_dir.mkdir()
            boot_image = root / "boot_linux_v2277_workqueue_exec_stack.img"
            report_path = root / "report.md"
            manifest_path = out_dir / "manifest.json"
            manifest_path.write_text(json.dumps({
                "decision": "v2277-workqueue-exec-stack-source-build-pass",
                "boot_image": str(boot_image.relative_to(v2277.REPO_ROOT)),
                "boot_sha256": "boot-sha",
                "init_version": "0.9.275",
                "init_build": "v2277-workqueue-exec-stack",
                "helper_sha256": "helper-sha",
            ...
            old_values = {
                "OUT_DIR": v2277.OUT_DIR,
                "BOOT_IMAGE": v2277.BOOT_IMAGE,
                "REPORT_PATH": v2277.REPORT_PATH,
            }
            old_functions = {
                "configure_base": v2277.configure_base,
                "build_bpf_helpers": v2277.build_bpf_helpers,
                "helper_builder_module": v2277.helper_builder_module,
                "base_module": v2277.base_module,
            }
            ...
            v2277.OUT_DIR = out_dir
            v2277.BOOT_IMAGE = boot_image
            v2277.REPORT_PATH = report_path
            v2277.configure_base = lambda: ("-DTEST=1", *v2277.TAIL_SAMPLER_FLAGS, *v2277.WORKQUEUE_SAMPLER_FLAGS)
            v2277.build_bpf_helpers = lambda: {"workqueue": "workqueue-sha", "codeword": "codeword-sha"}
            v2277.helper_builder_module = lambda: helper_builder
            v2277.base_module = lambda: fake_base
            try:
                with PatchV2237(fake_v2237):
                    rc = v2277.main()
            finally:
                for name, value in old_values.items():
                    setattr(v2277, name, value)
        ...
        self.assertEqual(helper_builder.EXPECTED_HELPER_MARKER, v2277.EXPECTED_HELPER_MARKER)
        self.assertEqual(fake_base.base.EXPECTED_HELPER_MARKER, v2277.EXPECTED_HELPER_MARKER)
        self.assertEqual(manifest["candidate_tag"], "v2277-workqueue-exec-stack")
        self.assertEqual(manifest["parent_baseline"], "v2237-supplicant-terminate-poll")
        self.assertFalse(manifest["promoted_baseline"])
        self.assertEqual(manifest["helper_flags"], ["-DTEST=1", *v2277.TAIL_SAMPLER_FLAGS, *v2277.WORKQUEUE_SAMPLER_FLAGS])
        ...
        self.assertEqual(manifest["codeword_sampler"]["period_ns"], v2277.TAIL_PERIOD_NS)
        self.assertEqual(live_candidate["candidate_tag"], "v2277-workqueue-exec-stack")
        ...
        self.assertEqual(live_candidate["next_live_cycle"], "V2278")
