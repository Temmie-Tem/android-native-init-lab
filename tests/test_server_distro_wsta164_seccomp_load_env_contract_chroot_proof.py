...
runner = load_script("workspace/public/src/scripts/server-distro/run_wsta164_seccomp_load_env_contract_chroot_proof.py")


class ServerDistroWsta164SeccompLoadEnvContractChrootProofTests(unittest.TestCase):
    ...
    def write_filter_artifact(self, root: Path) -> tuple[Path, Path]:
        ...
        object_path.write_bytes(b"fake-wsta164-object")
    ...
    def test_load_env_contract_is_proven_without_correct_token(self) -> None:
        with self.private_tmp() as tmp:
            ...
            with (
                ...
                mock.patch.object(
                    runner,
                    "run_chroot_launcher_load_env",
                    side_effect=[
                        {"returncode": 65, "stdout": no_gate_stdout, "stderr": ""},
                        {"returncode": 65, "stdout": missing_token_stdout, "stderr": ""},
                        {"returncode": 65, "stdout": wrong_token_stdout, "stderr": ""},
                    ],
                ),
            ):
                result = runner.run(runner.build_arg_parser().parse_args([
                    ...
                    str(helper_binary),
                    "--execute-load-env-contract-chroot-proof",
                ]))

        self.assertEqual(result["decision"], runner.PASS_DECISION)
        self.assertTrue(result["checks"]["launcher_has_wsta164_load_gate"])
        self.assertTrue(result["checks"]["launcher_forwards_load_env"])
        self.assertTrue(result["checks"]["launcher_does_not_hardcode_wsta161_token"])
        self.assertTrue(result["proof_checks"]["no_gate_blocks_load_gate"])
        self.assertTrue(result["proof_checks"]["missing_token_blocks_before_helper"])
        self.assertTrue(result["proof_checks"]["wrong_token_blocks_token"])
        self.assertTrue(result["proof_checks"]["wrong_token_no_load_attempt"])
        self.assertFalse(result["proof"]["correct_wsta161_token_supplied"])
    ...
    def test_gate_blocks_without_explicit_flag_or_private_helper(self) -> None:
        ...
        self.assertEqual(result["decision"], "wsta164-blocked-explicit-gate-required")
        ...
        self.assertEqual(result["decision"], "wsta164-blocked-helper-manifest-nonprivate")
