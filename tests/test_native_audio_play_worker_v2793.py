"""Tests for the V2793 audio play worker fix."""
...
BUILDER = REPO / "workspace/public/src/scripts/revalidation/build_native_init_boot_v2793_audio_play_worker.py"
LIVE = REPO / "workspace/public/src/scripts/revalidation/native_audio_play_worker_live_handoff_v2793.py"


def load_live_module():
    sys.path.insert(0, str(LIVE.parent))
    spec = importlib.util.spec_from_file_location("v2793_live", LIVE)
...
class NativeAudioPlayWorkerV2793(unittest.TestCase):
    @classmethod
    ...
    def test_builder_uses_v2793_patch_artifact_identity(self) -> None:
        text = BUILDER.read_text(encoding="utf-8")

        self.assertIn('CYCLE = "V2793"', text)
        self.assertIn('INIT_VERSION = "0.9.306"', text)
        self.assertIn('INIT_BUILD = "v2793-audio-play-worker"', text)
        self.assertIn('boot_linux_v2793_audio_play_worker.img', text)
        self.assertIn('NATIVE_INIT_V2793_AUDIO_PLAY_WORKER_SOURCE_BUILD_2026-06-19.md', text)
        self.assertIn('"sound_control_wait_timeout_ms": 70000', text)
        self.assertIn('"play_worker_executor_compiled": True', text)

    def test_live_runner_targets_v2793_and_allowed_manifest_path(self) -> None:
        module = self.live
        args = module.parse_args(["--dry-run"])
        state = module.preflight_state()
        payload = module.dry_run_payload(args, state)

        self.assertEqual(module.CANDIDATE_VERSION, "0.9.306")
        self.assertIn("boot_linux_v2793_audio_play_worker.img", str(module.CANDIDATE_IMAGE))
        self.assertEqual(
        ...
        self.assertEqual(payload["commands"]["play_worker_log"], ["run", "/bin/cat", module.REMOTE_PLAY_LOG])
